...
static void
printErrorAndAbort(const std::string& error)
{
    std::cout << error << std::endl;
}

#define FATAL_STREAM(stream) printErrorAndAbort(std::string("Fatal error: ") + stream)
...
main(int argc, char** argv)
{
    ...
    LandmarkDetector::DetectionManager detection_manger;
    if (!detection_manger.Init(argc, argv))
    {
        return 1;
    }
    ...
    while (true)  // this is not a for loop as we might also be reading from a webcam
    {
        ...
        if (!sequence_reader.Open(arguments))
            break;
        const bool status = detection_manger.OpenSequenceReader(sequence_reader.fps,
                                                                sequence_reader.frame_width,
                                                                sequence_reader.frame_height,
                                                                sequence_reader.fx,
                                                                sequence_reader.fy,
                                                                sequence_reader.cx,
                                                                sequence_reader.cy);
        ...
        {
            INFO_STREAM("WARNING: using a webcam in feature extraction, Action Unit predictions "
                        "will not be as accurate in real-time webcam mode");
            INFO_STREAM("WARNING: using a webcam in feature extraction, forcing visualization of "
                        "tracking to allow quitting the application (press q)");
        ...
        Utilities::RecorderOpenFace open_face_rec(recording_params, arguments);
        ...
        while (!captured_image.empty())
        {
            ...
            {
                GazeAnalysis::EstimateGaze(face_model,
                                           gazeDirection0,
                                           sequence_reader.fx,
                                           sequence_reader.fy,
                                           sequence_reader.cx,
            ...
            if (recording_params.outputAlignedFaces() || recording_params.outputHOG()
                || recording_params.outputAUs() || visualizer.vis_align || visualizer.vis_hog
                || visualizer.vis_aus)
            {
            ...
            visualizer.SetImage(captured_image,
                                sequence_reader.fx,
                                sequence_reader.fy,
                                sequence_reader.cx,
                                sequence_reader.cy);
            ...
            visualizer.SetObservationGaze(
                    ...
                    LandmarkDetector::Calculate3DEyeLandmarks(face_model,
                                                              sequence_reader.fx,
                                                              sequence_reader.fy,
                                                              sequence_reader.cx,
                                                              sequence_reader.cy),
            ...
            // num_hog_cols, 31); // The number of channels in HOG is fixed at the moment, as using
            // FHOG
            open_face_rec.SetObservationVisualization(visualizer.GetVisImage());
            open_face_rec.SetObservationActionUnits(face_analyser.GetCurrentAUsReg(),
                                                    face_analyser.GetCurrentAUsClass());
            open_face_rec.SetObservationLandmarks(face_model.detected_landmarks,
                                                  face_model.GetShape(sequence_reader.fx,
                                                                      sequence_reader.fy,
                                                                      sequence_reader.cx,
                                                                      sequence_reader.cy),
                                                  face_model.params_global,
                                                  face_model.params_local,
                                                  face_model.detection_certainty,
                                                  detection_success);
            ...
            base::Value json_out;
            detection_manger.DetectFeatures(captured_image, &json_out, nullptr, false);

            // open_face_rec.WriteObservation(&json_out);
            std::cout << json_out.DebugString() << std::endl;
