...
struct static_pool
{
    ...
    explicit static_pool(std::size_t size) : size_(size), p_(reinterpret_cast<char*>(this + 1))
    {
    }
...
public:
    ...
    explicit fields_alloc(std::size_t size) : pool_(&detail::static_pool::construct(size))
    {
    }
    ...
    template <class U>
    friend bool
    operator==(fields_alloc const& lhs, fields_alloc<U> const& rhs)
    {
