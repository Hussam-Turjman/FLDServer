...
#include "fldserver/landmark_detection/image_manipulation_helpers.h"
...
bool
SequenceCapture::Open(std::vector<std::string>& arguments)
{
    ...
    // First check if there is a root argument (so that videos and input directories could be
    // defined more easily)
    for (size_t i = 0; i < arguments.size(); ++i)
    {
...
bool
SequenceCapture::OpenRemoteWebcam(
        double fps_guess, int image_width, int image_height, float fx, float fy, float cx, float cy)
{
    ...
    if (fps != fps || fps <= 0)
    {
        INFO_STREAM("FPS of the remote webcam cannot be determined, assuming 30");
        fps = 30;
    }

    SetCameraIntrinsics(fx, fy, cx, cy);
    std::string time = currentDateTime();
    this->name = "remote_webcam_" + time;
...
bool
SequenceCapture::OpenWebcam(
        int device, int image_width, int image_height, float fx, float fy, float cx, float cy)
{
...
bool
SequenceCapture::OpenVideoFile(std::string video_file, float fx, float fy, float cx, float cy)
{
...
bool
SequenceCapture::OpenImageSequence(std::string directory, float fx, float fy, float cx, float cy)
{
    ...
    copy(fs::directory_iterator(image_directory),
    ...
    for (std::vector<fs::path>::const_iterator file_iterator(file_in_directory.begin());
...
void
SequenceCapture::SetCameraIntrinsics(float fx, float fy, float cx, float cy)
{
...
bool
SequenceCapture::SetRemoteFrame(const cv::Mat& remote_frame)
{
    time_stamp = (cv::getTickCount() - start_time) / cv::getTickFrequency();
    latest_frame = remote_frame;
    ConvertToGrayscale_8bit(latest_frame, latest_gray_frame);
    capture_queue.push(std::make_tuple(time_stamp, latest_frame, latest_gray_frame));
    frame_num++;
    return true;
}
cv::Mat
SequenceCapture::GetNextFrame()
{
    if (!is_webcam)
    {
        ...
        {
            LOG(ERROR) << "Capture queue is empty ";
            return cv::Mat();
        }
