...
//       Erroll Wood, Tadas Baltrušaitis, Xucong Zhang, Yusuke Sugano, Peter Robinson, and Andreas
//       Bulling in IEEE International. Conference on Computer Vision (ICCV),  2015
//
//       Cross-dataset learning and person-specific normalisation for automatic Action Unit
//       detection Tadas Baltrušaitis, Marwa Mahmoud, and Peter Robinson in Facial Expression
//       Recognition and Analysis Challenge, IEEE International Conference on Automatic Face and
//       Gesture Recognition, 2015
//
///////////////////////////////////////////////////////////////////////////////
#include "fldserver/landmark_detection/stdafx_ut.h"
...
void
CreateDirectory(std::string output_path)
{
    ...
    if (!fs::exists(output_path))
    {
        ...
        if (!success)
        {
            std::cout << "ERROR: failed to create output directory:" << output_path
                      << ", do you have permission to create directory" << std::endl;
            exit(1);
        }
    }
}
...
void
RecorderOpenFace::VideoWritingTask(bool is_sequence)
{
...
void
RecorderOpenFace::AlignedImageWritingTask()
{
    std::pair<std::string, cv::Mat> tracked_data;
...
void
RecorderOpenFace::PrepareRecording()
{
    // Construct the directories required for the output
    // CreateDirectory(record_root);

    // Create the filename for the general output file that contains all of the meta information
    // about the recording
    ...
    {
        std::cout << "ERROR: could not open the output file:" << of_det_name << ", either the path
    of the output directory is wrong or you do not have the permissions to write to it" <<
    std::endl; exit(1);
    }*/
    ...
    // metadata_file << "Camera parameters:" << params.getFx() << "," << params.getFy() << "," <<
    // params.getCx() << "," << params.getCy() << std::endl;
    ...
    if (params.outputHOG())
    {
        // Output the data based on record_root, but do not include record_root in the meta file, as
        // it is also in that directory
    ...
    if (params.outputTracked())
    {
        ...
        {
            // Output the data based on record_root, but do not include record_root in the meta
            // file, as it is also in that directory
    ...
    if (params.outputAlignedFaces())
    {
        ...
        // metadata_file << "Output aligned directory:" << this->aligned_output_directory <<
        // std::endl; this->aligned_output_directory = (fs::path(record_root) /
        // this->aligned_output_directory).string(); CreateDirectory(aligned_output_directory);
    }
...
RecorderOpenFace::RecorderOpenFace(const RecorderOpenFaceParameters& parameters,
    ...
    /*video_writer(),*/ params(parameters)
{
    ...
    for (size_t i = 0; i < arguments.size(); ++i)
    {
        ...
        {
            // record_root = (fs::path(record_root) / fs::path(arguments[i +
            // 1])).remove_filename().string(); out_name = fs::path(fs::path(arguments[i +
            // 1])).replace_extension("").filename().string();
            valid[i] = false;
            valid[i + 1] = false;
            i++;
            output_found = true;
...
RecorderOpenFace::RecorderOpenFace(const std::string in_filename,
                                   const RecorderOpenFaceParameters& parameters,
...
void
RecorderOpenFace::SetObservationFaceAlign(const cv::Mat& aligned_face)
{
    // this->aligned_face = aligned_face;
}

void
RecorderOpenFace::SetObservationVisualization(const cv::Mat& vis_track)
{
    if (params.outputTracked())
    {
...
void
RecorderOpenFace::WriteObservation(base::Value* json_out)
{
    // Write out the CSV file (it will always be there, even if not outputting anything more but
    // frame/face numbers)
    if (!csv_recorder.isOpen())
    {
        // As we are writing out the header, work out some things like number of landmarks, names of
        // AUs etc.
        ...
        csv_recorder.OpenForJsonOnly(params.isSequence(),
                                     params.output2DLandmarks(),
                                     params.output3DLandmarks(),
                                     params.outputPDMParams(),
                                     params.outputPose(),
                                     params.outputAUs(),
                                     params.outputGaze(),
                                     num_face_landmarks,
    ...
    if (json_out)
    {
        *json_out = csv_recorder.WriteLineAsJson(face_id,
                                                 frame_number,
                                                 timestamp,
    ...
    if (params.outputAlignedFaces())
    {
        /*if (!aligned_writing_thread_started)
        {
            ...
            int capacity = (1024 * 1024 * ALIGNED_QUEUE_CAPACITY) / (aligned_face.size().width
        *aligned_face.size().height * aligned_face.channels()) + 1;
        ...
        std::string out_file = (fs::path(aligned_output_directory) / fs::path(std::string(name) +
        params.imageFormatAligned())).string();
...
/*void RecorderOpenFace::SetObservationHOG(bool good_frame, const cv::Mat_<double>& hog_descriptor,
int num_cols, int num_rows, int num_channels)
{
    this->hog_recorder.SetObservationHOG(good_frame, hog_descriptor, num_cols, num_rows,
num_channels);
}*/

void
RecorderOpenFace::SetObservationTimestamp(double timestamp)
{
...
void
RecorderOpenFace::SetObservationFrameNumber(int frame_number)
{
...
void
RecorderOpenFace::SetObservationFaceID(int face_id)
{
    this->face_id = face_id;
}

void
RecorderOpenFace::SetObservationLandmarks(const cv::Mat_<float>& landmarks_2D,
                                          const cv::Mat_<float>& landmarks_3D,
                                          const cv::Vec6f& pdm_params_global,
                                          const cv::Mat_<float>& pdm_params_local,
                                          double confidence,
                                          bool success)
{
...
void
RecorderOpenFace::SetObservationPose(const cv::Vec6f& pose)
{
    this->head_pose = pose;
}

void
RecorderOpenFace::SetObservationActionUnits(
...
void
RecorderOpenFace::SetObservationGaze(const cv::Point3f& gaze_direction0,
