...
{
double
writeWithPrecision(double value, int precision)
{
...
bool
RecorderCSV::OpenForJsonOnly(bool is_sequence,
...
bool
RecorderCSV::Open(std::string output_file_name,
                  bool is_sequence,
                  bool output_2D_landmarks,
                  bool output_3D_landmarks,
                  bool output_model_params,
...
base::Value
RecorderCSV::WriteLineAsJson(int face_id,
                             int frame_num,
                             double time_stamp,
                             ...
                             const std::vector<std::pair<std::string, double>>& au_occurences)
{
    ...
    const int landmarks_count = landmarks_2D.rows / 2;
    ...
    if (output_2D_landmarks)
    {
        ...
        {
            out.SetKey("x_" + std::to_string(i), base::Value(*iter));

            iter++;
        }
        for (int i = 0; i < landmarks_count; ++i)
        {
            out.SetKey("y_" + std::to_string(i), base::Value(*iter));
            iter++;
        }
    }
    ...
    if (output_3D_landmarks)
    {
        ...
        for (int i = 0; i < landmarks_count; ++i)
        {
            out.SetKey("Z_" + std::to_string(i), base::Value(*iter));
    ...
    if (output_model_params)
    {
        ...
        {
            out.SetKey("p_" + std::to_string(count), base::Value(lmk));
            count++;
        }
    }
    ...
    if (output_AUs)
    {
        ...
        for (std::string au_name : au_names_reg)
        {
            for (auto au_reg : au_intensities)
            {
                ...
                {
                    const std::string key = au_reg.first + std::string("_r");

                    out.SetKey(key, base::Value(writeWithPrecision(au_reg.second, 2)));
                    break;
                }
            }
        }
        ...
        {
            for (auto iter = au_names_reg.begin(); iter != au_names_reg.end(); iter++)
            {
                const std::string key = *iter + std::string("_r");

                out.SetKey(key, base::Value(0.0));
            }
        }
        ...
        for (std::string au_name : au_names_class)
        {
            for (auto au_class : au_occurences)
            {
                ...
                {
                    const std::string key = au_class.first + std::string("_c");

                    out.SetKey(key, base::Value(writeWithPrecision(au_class.second, 1)));
        ...
        {
            for (auto iter = au_names_class.begin(); iter != au_names_class.end(); iter++)
            {
                const std::string key = *iter + std::string("_c");
...
void
RecorderCSV::WriteLine(int face_id,
                       int frame_num,
                       double time_stamp,
                       bool landmark_detection_success,
                       double landmark_confidence,
                       ...
                       const std::vector<std::pair<std::string, double>>& au_occurences)
{
    if (json_only_)
    {
        std::cerr << "RecorderCSV is in json only mode !! " << std::endl;
        exit(1);
    }
    ...
    if (output_gaze)
    {
        ...
        output_file << ", " << gazeDirection0.x << ", " << gazeDirection0.y << ", "
                    << gazeDirection0.z << ", " << gazeDirection1.x << ", " << gazeDirection1.y
                    << ", " << gazeDirection1.z;
    ...
    {
        output_file << std::setprecision(1);
        output_file << ", " << pose_estimate[0] << ", " << pose_estimate[1] << ", "
                    << pose_estimate[2];
        output_file << std::setprecision(3);
        output_file << ", " << pose_estimate[3] << ", " << pose_estimate[4] << ", "
                    << pose_estimate[5];
    }
