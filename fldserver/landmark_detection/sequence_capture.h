...
#ifndef FLDSERVER_SEQUENCE_CAPTURE_H
#define FLDSERVER_SEQUENCE_CAPTURE_H
...
public:
    ...
    bool
    Open(std::vector<std::string>& arguments);

    // Direct opening

    // Remote Webcam
    bool
    OpenRemoteWebcam(double fps_guess,
                     int image_width = 640,
    ...
    bool
    OpenImageSequence(
            std::string directory, float fx = -1, float fy = -1, float cx = -1, float cy = -1);

    // Video file
    bool
    OpenVideoFile(
            std::string video_file, float fx = -1, float fy = -1, float cx = -1, float cy = -1);
    ...
    bool
    IsRemoteWebcam() const
    {
        return is_remote_webcam;
    }

    bool
    SetRemoteFrame(const cv::Mat& remote_frame);
    ...
    // Allows to differentiate if failed because no input specified or if failed to open a specified
    // input
...
private:
    ...
    // Blocking copy and move, as it doesn't make sense to have several readers pointed at the same
    // source, and this would cause issues, especially with webcams
    SequenceCapture&
    operator=(const SequenceCapture& other);
    SequenceCapture&
    operator=(const SequenceCapture&& other);
    ...
    bool is_webcam;
    bool is_image_seq;
    bool is_remote_webcam = false;

    void
    SetCameraIntrinsics(float fx, float fy, float cx, float cy);
};
}  // namespace Utilities
#endif  // FLDSERVER_SEQUENCE_CAPTURE_H