...
#include "fldserver/base/power_monitor/power_monitor.h"
...
{
void
PowerMonitor::Initialize(std::unique_ptr<PowerMonitorSource> source)
{
    ...
    PowerMonitor::NotifyPowerStateChange(PowerMonitor::Source()->IsOnBatteryPower());
...
// static
void
PowerMonitor::AddPowerSuspendObserver(PowerSuspendObserver* obs)
{
...
// static
void
PowerMonitor::RemovePowerSuspendObserver(PowerSuspendObserver* obs)
{
...
// static
void
PowerMonitor::AddPowerStateObserver(PowerStateObserver* obs)
{
...
// static
void
PowerMonitor::RemovePowerStateObserver(PowerStateObserver* obs)
{
...
// static
void
PowerMonitor::AddPowerThermalObserver(PowerThermalObserver* obs)
{
...
// static
void
PowerMonitor::RemovePowerThermalObserver(PowerThermalObserver* obs)
{
...
// static
bool
PowerMonitor::AddPowerSuspendObserverAndReturnSuspendedState(PowerSuspendObserver* obs)
{
    PowerMonitor* power_monitor = GetInstance();
...
// static
bool
PowerMonitor::AddPowerStateObserverAndReturnOnBatteryState(PowerStateObserver* obs)
{
    PowerMonitor* power_monitor = GetInstance();
...
PowerMonitor::AddPowerStateObserverAndReturnPowerThermalState(PowerThermalObserver* obs)
{
    PowerMonitor* power_monitor = GetInstance();
...
PowerMonitor::ShutdownForTesting()
{
    ...
    {
        AutoLock auto_lock(power_monitor->power_thermal_state_lock_);
        power_monitor->power_thermal_state_ = PowerThermalObserver::DeviceThermalState::kUnknown;
    }
}
...
// static
void
PowerMonitor::SetCurrentThermalState(PowerThermalObserver::DeviceThermalState state)
{
    DCHECK(IsInitialized());
...
int
PowerMonitor::GetRemainingBatteryCapacity()
{
    DCHECK(IsInitialized());
...
void
PowerMonitor::NotifyPowerStateChange(bool on_battery_power)
{
    DCHECK(IsInitialized());
    DVLOG(1) << "PowerStateChange: " << (on_battery_power ? "On" : "Off") << " battery";
...
PowerMonitor::NotifySuspend()
{
    ...
    if (!power_monitor->is_system_suspended_)
    {
        ...
        GetInstance()->power_suspend_observers_->Notify(FROM_HERE,
...
PowerMonitor::NotifyResume()
{
    ...
    {
        power_monitor->is_system_suspended_ = false;
        GetInstance()->power_suspend_observers_->Notify(FROM_HERE, &PowerSuspendObserver::OnResume);
    }
}

void
PowerMonitor::NotifyThermalStateChange(PowerThermalObserver::DeviceThermalState new_state)
{
    DCHECK(IsInitialized());
    DVLOG(1) << "ThermalStateChange: " << PowerMonitorSource::DeviceThermalStateToString(new_state);
