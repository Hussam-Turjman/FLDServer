...
class CORE_EXPORT PowerMonitor
{
public:
    ...
    static void
    Initialize(std::unique_ptr<PowerMonitorSource> source);
    ...
    static void
    AddPowerSuspendObserver(PowerSuspendObserver* observer);
    static void
    RemovePowerSuspendObserver(PowerSuspendObserver* observer);
    static void
    AddPowerStateObserver(PowerStateObserver* observer);
    static void
    RemovePowerStateObserver(PowerStateObserver* observer);
    static void
    AddPowerThermalObserver(PowerThermalObserver* observer);
    static void
    RemovePowerThermalObserver(PowerThermalObserver* observer);
    ...
    static bool
    AddPowerSuspendObserverAndReturnSuspendedState(PowerSuspendObserver* observer);
    // Returns true if the system is on-battery.
    static bool
    AddPowerStateObserverAndReturnOnBatteryState(PowerStateObserver* observer);
    // Returns the power thermal state.
    static PowerThermalObserver::DeviceThermalState
    AddPowerStateObserverAndReturnPowerThermalState(PowerThermalObserver* observer);
    ...
    static void
    SetCurrentThermalState(PowerThermalObserver::DeviceThermalState state);
...
private:
    ...
    static PowerMonitorSource*
    Source();

    static void
    NotifyPowerStateChange(bool on_battery_power);
    static void
    NotifySuspend();
    static void
    NotifyResume();
    static void
    NotifyThermalStateChange(PowerThermalObserver::DeviceThermalState new_state);
    ...
    PowerThermalObserver::DeviceThermalState power_thermal_state_ GUARDED_BY(
            power_thermal_state_lock_) = PowerThermalObserver::DeviceThermalState::kUnknown;
    Lock power_thermal_state_lock_;

    scoped_refptr<ObserverListThreadSafe<PowerStateObserver>> power_state_observers_;
    scoped_refptr<ObserverListThreadSafe<PowerSuspendObserver>> power_suspend_observers_;
    scoped_refptr<ObserverListThreadSafe<PowerThermalObserver>> thermal_state_observers_;
