...
{
void
ProcessPowerEventHelper(PowerMonitorSource::PowerEvent event)
{
...
void
ProcessWmPowerBroadcastMessage(WPARAM event_id)
{
    ...
    instance_(NULL), message_hwnd_(NULL)
{
    if (!CurrentUIThread::IsSet())
    {
        ...
        DLOG(ERROR) << "Cannot create windows on non-UI thread, power monitor disabled!";
        return;
    }
    ...
    message_hwnd_ = CreateWindowEx(WS_EX_NOACTIVATE,
                                   kWindowClassName,
                                   NULL,
                                   WS_POPUP,
                                   0,
                                                           ...
                                                           LPARAM lparam)
{
    switch (message)
    {
        case WM_POWERBROADCAST: ProcessWmPowerBroadcastMessage(wparam); return TRUE;
