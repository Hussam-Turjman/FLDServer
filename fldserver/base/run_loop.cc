...
namespace
{
ThreadLocalPointer<RunLoop::Delegate>&
GetTlsDelegate()
{
...
void
ProxyToTaskRunner(scoped_refptr<SequencedTaskRunner> task_runner, OnceClosure closure)
{
...
ThreadLocalPointer<const RunLoop::RunLoopTimeout>&
RunLoopTimeoutTLS()
{
...
void
OnRunLoopTimeout(RunLoop* run_loop,
...
bool
RunLoop::Delegate::ShouldQuitWhenIdle()
{
...
// static
void
RunLoop::RegisterDelegateForCurrentThread(Delegate* delegate)
{
...
void
RunLoop::Run(const Location& location)
{
    ...
    if (run_timeout)
    {
        cancelable_timeout.Reset(
                BindOnce(&OnRunLoopTimeout, Unretained(this), location, run_timeout->on_timeout));
    ...
    const bool application_tasks_allowed = delegate_->active_run_loops_.size() == 1U
...
RunLoop::Quit()
{
    ...
    {
        origin_task_runner_->PostTask(FROM_HERE, BindOnce(&RunLoop::Quit, Unretained(this)));
        return;
    }
...
RunLoop::QuitWhenIdle()
{
    ...
    if (!origin_task_runner_->RunsTasksInCurrentSequence())
    {
        origin_task_runner_->PostTask(FROM_HERE,
                                      BindOnce(&RunLoop::QuitWhenIdle, Unretained(this)));
        return;
    }
...
RunLoop::QuitClosure()
{
    ...
    return BindRepeating(&ProxyToTaskRunner,
                         origin_task_runner_,
...
// static
void
RunLoop::AddNestingObserverOnCurrentThread(NestingObserver* observer)
{
    Delegate* delegate = GetTlsDelegate().Get();
    DCHECK(delegate);
...
// static
void
RunLoop::RemoveNestingObserverOnCurrentThread(NestingObserver* observer)
{
    Delegate* delegate = GetTlsDelegate().Get();
    DCHECK(delegate);
...
RunLoop::ScopedDisallowRunning::ScopedDisallowRunning() :
    ...
    previous_run_allowance_(current_delegate_ ? current_delegate_->allow_running_for_testing_
                                              : false)
{
...
// static
void
RunLoop::SetTimeoutForCurrentThread(const RunLoopTimeout* timeout)
{
...
RunLoop::GetTimeoutForCurrentThread()
{
