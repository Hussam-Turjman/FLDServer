...
#include "fldserver/base/sequence_checker_impl.h"
...
public:
    ...
    TimerBase(const Location& posted_from, TimeDelta delay, const TickClock* tick_clock);
    ...
    virtual void
    SetTaskRunner(scoped_refptr<SequencedTaskRunner> task_runner);
...
protected:
    ...
    // to Now() + delay.
    void
    ScheduleNewTask(TimeDelta delay);

    void
    StartInternal(const Location& posted_from, TimeDelta delay);
...
private:
    ...
    void
    OnScheduledTaskInvoked(std::unique_ptr<TaskDestructionDetector> task_destruction_detector);
...
class CORE_EXPORT OneShotTimer : public internal::TimerBase
{
public:
    OneShotTimer();
    ...
    virtual void
    Start(const Location& posted_from, TimeDelta delay, OnceClosure user_task);
    ...
    void
    Start(const Location& posted_from,
          TimeDelta delay,
          Receiver* receiver,
...
class CORE_EXPORT RepeatingTimer : public internal::TimerBase
{
public:
    RepeatingTimer();
    ...
    RepeatingTimer(const Location& posted_from, TimeDelta delay, RepeatingClosure user_task);
    ...
    // already running, it will be replaced to call the given |user_task|.
    virtual void
    Start(const Location& posted_from, TimeDelta delay, RepeatingClosure user_task);
...
class CORE_EXPORT RetainingOneShotTimer : public internal::TimerBase
{
public:
    RetainingOneShotTimer();
    ...
    RetainingOneShotTimer(const Location& posted_from, TimeDelta delay, RepeatingClosure user_task);
...
public:
    ...
    DelayTimer(const Location& posted_from,
               ...
               void (Receiver::*method)(),
               const TickClock* tick_clock) :
        timer_(posted_from, delay, BindRepeating(method, Unretained(receiver)), tick_clock)
    {
    }
