...
#include "fldserver/base/timer/hi_res_timer_manager.h"
...
bool
HighResolutionTimerAllowed()
{
    return !CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableHighResTimer);
}

}  // namespace

HighResolutionTimerManager::HighResolutionTimerManager() : hi_res_clock_available_(false)
{
    ...
    if (HighResolutionTimerAllowed())
    {
        ...
        const bool on_battery = PowerMonitor::AddPowerStateObserverAndReturnOnBatteryState(this);
        UseHiResClock(!on_battery);

        // Start polling the high resolution timer usage.
        Time::ResetHighResolutionTimerUsage();
        timer_.Start(
                FROM_HERE, kUsageSampleInterval, BindRepeating(&ReportHighResolutionTimerUsage));
    }
}
...
void
HighResolutionTimerManager::OnPowerStateChange(bool on_battery_power)
{
...
void
HighResolutionTimerManager::OnSuspend()
{
    DCHECK(HighResolutionTimerAllowed());
...
void
HighResolutionTimerManager::OnResume()
{
    DCHECK(HighResolutionTimerAllowed());
...
void
HighResolutionTimerManager::UseHiResClock(bool use)
{
    DCHECK(HighResolutionTimerAllowed());
