...
TimerBase::TimerBase(const TickClock* tick_clock) :
    task_destruction_detector_(nullptr), tick_clock_(tick_clock), is_running_(false)
{
...
TimerBase::TimerBase(const Location& posted_from, TimeDelta delay, const TickClock* tick_clock) :
...
TimeDelta
TimerBase::GetCurrentDelay() const
{
    DCHECK(origin_sequence_checker_.CalledOnValidSequence());
    return delay_;
}
...
void
TimerBase::SetTaskRunner(scoped_refptr<SequencedTaskRunner> task_runner)
{
    DCHECK(origin_sequence_checker_.CalledOnValidSequence());
...
void
TimerBase::StartInternal(const Location& posted_from, TimeDelta delay)
{
    DCHECK(origin_sequence_checker_.CalledOnValidSequence());
...
void
TimerBase::ScheduleNewTask(TimeDelta delay)
{
    DCHECK(origin_sequence_checker_.CalledOnValidSequence());
    DCHECK(!task_destruction_detector_);
    is_running_ = true;
    auto task_destruction_detector = std::make_unique<TaskDestructionDetector>(this);
...
void
TimerBase::OnScheduledTaskInvoked(
...
OneShotTimer::OneShotTimer(const TickClock* tick_clock) : internal::TimerBase(tick_clock)
{
}
OneShotTimer::~OneShotTimer() = default;

void
OneShotTimer::Start(const Location& posted_from, TimeDelta delay, OnceClosure user_task)
{
...
RepeatingTimer::RepeatingTimer(const TickClock* tick_clock) : internal::TimerBase(tick_clock)
{
}
RepeatingTimer::~RepeatingTimer() = default;
...
RepeatingTimer::RepeatingTimer(const Location& posted_from,
    ...
    internal::TimerBase(posted_from, delay), user_task_(std::move(user_task))
{
}
RepeatingTimer::RepeatingTimer(const Location& posted_from,
                               TimeDelta delay,
    ...
    internal::TimerBase(posted_from, delay, tick_clock), user_task_(std::move(user_task))
{
}

void
RepeatingTimer::Start(const Location& posted_from, TimeDelta delay, RepeatingClosure user_task)
{
    user_task_ = std::move(user_task);
    StartInternal(posted_from, delay);
}
...
void
RepeatingTimer::OnStop()
{
}
void
RepeatingTimer::RunUserTask()
{
...
void
RetainingOneShotTimer::Start(const Location& posted_from,
                             TimeDelta delay,
                             RepeatingClosure user_task)
{
    user_task_ = std::move(user_task);
...
void
RetainingOneShotTimer::OnStop()
{
}
void
RetainingOneShotTimer::RunUserTask()
{
