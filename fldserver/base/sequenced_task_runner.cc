...
{
bool
SequencedTaskRunner::PostNonNestableTask(const Location& from_here, OnceClosure task)
{
    return PostNonNestableDelayedTask(from_here, std::move(task), base::TimeDelta());
}
...
OnTaskRunnerDeleter::OnTaskRunnerDeleter(scoped_refptr<SequencedTaskRunner> task_runner) :
...
OnTaskRunnerDeleter&
OnTaskRunnerDeleter::operator=(OnTaskRunnerDeleter&&) = default;
