...
#include "fldserver/base/as_const.h"
#include "fldserver/base/bit_cast.h"
#include "fldserver/base/check_op.h"
#include "fldserver/base/containers/checked_iterators.h"
...
#if 0
#include "fldserver/base/trace_event/memory_usage_estimator.h"  // no-presubmit-check
#endif                                                          // 0
...
static_assert(base::size(kTypeNames) == static_cast<size_t>(Value::Type::LIST) + 1,
              "kTypeNames Has Wrong Size");

std::unique_ptr<Value>
CopyWithoutEmptyChildren(const Value& node);

// Make a deep copy of |node|, but don't include empty lists or dictionaries
// in the copy. It's possible for this function to return NULL and it
// expects |node| to always be non-NULL.
std::unique_ptr<Value>
CopyListWithoutEmptyChildren(const Value& list)
{
    ...
    return copy.GetList().empty() ? nullptr : std::make_unique<Value>(std::move(copy));
}

std::unique_ptr<DictionaryValue>
CopyDictionaryWithoutEmptyChildren(const DictionaryValue& dict)
{
...
CopyWithoutEmptyChildren(const Value& node)
{
    switch (node.type())
    {
        ...
        case Value::Type::DICTIONARY:
            return CopyDictionaryWithoutEmptyChildren(static_cast<const DictionaryValue&>(node));

        default: return std::make_unique<Value>(node.Clone());
    }
}
...
// static
Value
Value::FromUniquePtrValue(std::unique_ptr<Value> val)
{
    return std::move(*val);
}
...
// static
std::unique_ptr<Value>
Value::ToUniquePtrValue(Value val)
{
...
Value::AsDictionaryValue(const Value& val)
{
    CHECK(val.is_dict());
...
// static
const ListValue&
Value::AsListValue(const Value& val)
{
    CHECK(val.is_list());
...
Value::Value(Type type)
{
    ...
    switch (type)
    {
        ...
        case Type::BOOLEAN: data_.emplace<bool>(false); return;
        case Type::INTEGER: data_.emplace<int>(0); return;
        case Type::DOUBLE: data_.emplace<DoubleStorage>(bit_cast<DoubleStorage>(0.0)); return;
        case Type::STRING: data_.emplace<std::string>(); return;
        case Type::BINARY: data_.emplace<BlobStorage>(); return;
        case Type::DICTIONARY: data_.emplace<LegacyDictStorage>(); return;
        case Type::LIST: data_.emplace<ListStorage>(); return;
    }
...
Value::Value(const std::vector<char>& in_blob) :
    data_(absl::in_place_type_t<BlobStorage>(), in_blob.begin(), in_blob.end())
{
}
...
Value::Value(const DictStorage& in_dict) : data_(absl::in_place_type_t<LegacyDictStorage>())
{
    dict().reserve(in_dict.size());
    for (const auto& it : in_dict)
    {
        dict().try_emplace(dict().end(), it.first, std::make_unique<Value>(it.second.Clone()));
    }
}

Value::Value(DictStorage&& in_dict) noexcept : data_(absl::in_place_type_t<LegacyDictStorage>())
{
    dict().reserve(in_dict.size());
    for (auto& it : in_dict)
    {
        dict().try_emplace(
                dict().end(), std::move(it.first), std::make_unique<Value>(std::move(it.second)));
    }
}

Value::Value(span<const Value> in_list) : data_(absl::in_place_type_t<ListStorage>())
{
...
Value&
Value::operator=(Value&& that) noexcept = default;

Value::Value(const LegacyDictStorage& storage) : data_(absl::in_place_type_t<LegacyDictStorage>())
{
...
Value::Value(LegacyDictStorage&& storage) noexcept : data_(std::move(storage))
{
}
...
// static
const char*
Value::GetTypeName(Value::Type type)
{
...
void
Value::Append(StringPiece value)
{
    list().emplace_back(value);
}

void
Value::Append(std::string&& value)
{
    list().emplace_back(std::move(value));
}

void
Value::Append(const char16_t* value)
{
    list().emplace_back(value);
}

void
Value::Append(StringPiece16 value)
{
    list().emplace_back(value);
}
...
CheckedContiguousIterator<Value>
Value::Insert(CheckedContiguousConstIterator<Value> pos, Value&& value)
{
...
bool
Value::EraseListIter(CheckedContiguousConstIterator<Value> iter)
{
...
size_t
Value::EraseListValue(const Value& val)
{
...
const Value*
Value::FindKey(StringPiece key) const
{
...
const Value*
Value::FindKeyOfType(StringPiece key, Type type) const
{
...
base::Optional<bool>
Value::FindBoolKey(StringPiece key) const
{
...
base::Optional<int>
Value::FindIntKey(StringPiece key) const
{
...
Value::FindDoubleKey(StringPiece key) const
{
...
const std::string*
Value::FindStringKey(StringPiece key) const
{
    const Value* result = FindKey(key);
...
std::string*
Value::FindStringKey(StringPiece key)
{
    return const_cast<std::string*>(as_const(*this).FindStringKey(key));
}

const Value::BlobStorage*
Value::FindBlobKey(StringPiece key) const
{
    const Value* result = FindKey(key);
...
const Value*
Value::FindDictKey(StringPiece key) const
{
...
const Value*
Value::FindListKey(StringPiece key) const
{
...
{
    return dict()
            .insert_or_assign(std::move(key), std::make_unique<Value>(std::move(value)))
            .first->second.get();
}
...
Value*
Value::SetBoolKey(StringPiece key, bool value)
{
    return SetKeyInternal(key, std::make_unique<Value>(value));
}

Value*
Value::SetIntKey(StringPiece key, int value)
{
    return SetKeyInternal(key, std::make_unique<Value>(value));
}

Value*
Value::SetDoubleKey(StringPiece key, double value)
{
    return SetKeyInternal(key, std::make_unique<Value>(value));
}

Value*
Value::SetStringKey(StringPiece key, StringPiece value)
{
    return SetKeyInternal(key, std::make_unique<Value>(value));
}

Value*
Value::SetStringKey(StringPiece key, StringPiece16 value)
{
    return SetKeyInternal(key, std::make_unique<Value>(value));
}

Value*
Value::SetStringKey(StringPiece key, const char* value)
{
    return SetKeyInternal(key, std::make_unique<Value>(value));
}

Value*
Value::SetStringKey(StringPiece key, std::string&& value)
{
    return SetKeyInternal(key, std::make_unique<Value>(std::move(value)));
}

bool
Value::RemoveKey(StringPiece key)
{
    return dict().erase(key) != 0;
}

Optional<Value>
Value::ExtractKey(StringPiece key)
{
    auto found = dict().find(key);
    if (found == dict().end())
        return nullopt;
...
const Value*
Value::FindPath(StringPiece path) const
{
    CHECK(is_dict());
...
const Value*
Value::FindPathOfType(StringPiece path, Type type) const
{
...
base::Optional<bool>
Value::FindBoolPath(StringPiece path) const
{
    const Value* cur = FindPath(path);
    if (!cur || !cur->is_bool())
        return base::nullopt;
...
base::Optional<int>
Value::FindIntPath(StringPiece path) const
{
    const Value* cur = FindPath(path);
    if (!cur || !cur->is_int())
        return base::nullopt;
...
base::Optional<double>
Value::FindDoublePath(StringPiece path) const
{
...
const std::string*
Value::FindStringPath(StringPiece path) const
{
...
std::string*
Value::FindStringPath(StringPiece path)
{
    return const_cast<std::string*>(as_const(*this).FindStringPath(path));
}

const Value::BlobStorage*
Value::FindBlobPath(StringPiece path) const
{
    const Value* result = FindPath(path);
    return result ? absl::get_if<BlobStorage>(&result->data_) : nullptr;
}
...
const Value*
Value::FindDictPath(StringPiece path) const
{
...
const Value*
Value::FindListPath(StringPiece path) const
{
...
bool
Value::RemovePath(StringPiece path)
{
    return ExtractPath(path).has_value();
}

Optional<Value>
Value::ExtractPath(StringPiece path)
{
...
const Value*
Value::FindPath(std::initializer_list<StringPiece> path) const
{
...
const Value*
Value::FindPath(span<const StringPiece> path) const
{
    const Value* cur = this;
...
const Value*
Value::FindPathOfType(std::initializer_list<StringPiece> path, Type type) const
{
...
const Value*
Value::FindPathOfType(span<const StringPiece> path, Type type) const
{
    const Value* result = FindPath(path);
    if (!result || result->type() != type)
        return nullptr;
...
Value::dict_iterator_proxy
Value::DictItems()
{
    return dict_iterator_proxy(&dict());
}

Value::const_dict_iterator_proxy
Value::DictItems() const
{
...
Value::TakeDict()
{
    ...
    for (auto& pair : dict())
    {
        storage.try_emplace(storage.end(), std::move(pair.first), std::move(*pair.second));
    }
...
size_t
Value::DictSize() const
{
    return dict().size();
}

bool
Value::DictEmpty() const
{
    return dict().empty();
}
...
void
Value::MergeDictionary(const Value* dictionary)
{
...
bool
Value::GetAsBoolean(bool* out_value) const
{
...
bool
Value::GetAsInteger(int* out_value) const
{
    if (out_value && is_int())
    {
        *out_value = GetInt();
...
bool
Value::GetAsDouble(double* out_value) const
{
...
bool
Value::GetAsString(std::string* out_value) const
{
...
bool
Value::GetAsString(std::u16string* out_value) const
{
    if (out_value && is_string())
    {
...
bool
Value::GetAsString(const Value** out_value) const
{
    if (out_value && is_string())
    {
        *out_value = this;
...
bool
Value::GetAsString(StringPiece* out_value) const
{
    if (out_value && is_string())
    {
        *out_value = GetString();
...
bool
Value::GetAsList(ListValue** out_value)
{
...
bool
Value::GetAsList(const ListValue** out_value) const
{
    if (out_value && is_list())
    {
...
bool
Value::GetAsDictionary(DictionaryValue** out_value)
{
...
bool
Value::GetAsDictionary(const DictionaryValue** out_value) const
{
    if (out_value && is_dict())
    {
...
operator==(const Value& lhs, const Value& rhs)
{
    ...
    switch (lhs.type())
    {
        ...
        case Value::Type::BOOLEAN: return lhs.GetBool() == rhs.GetBool();
        case Value::Type::INTEGER: return lhs.GetInt() == rhs.GetInt();
        case Value::Type::DOUBLE: return lhs.AsDoubleInternal() == rhs.AsDoubleInternal();
        case Value::Type::STRING: return lhs.GetString() == rhs.GetString();
        case Value::Type::BINARY: return lhs.GetBlob() == rhs.GetBlob();
        ...
        case Value::Type::DICTIONARY:
            if (lhs.dict().size() != rhs.dict().size())
                return false;
            return std::equal(std::begin(lhs.dict()),
                              std::end(lhs.dict()),
                              std::begin(rhs.dict()),
        ...
        case Value::Type::LIST: return lhs.list() == rhs.list();
    }
...
operator<(const Value& lhs, const Value& rhs)
{
    ...
    switch (lhs.type())
    {
        ...
        // are completely inlined.
        case Value::Type::DICTIONARY:
            return std::lexicographical_compare(std::begin(lhs.dict()),
                                                std::end(lhs.dict()),
                                                std::begin(rhs.dict()),
                                                std::end(rhs.dict()),
...
size_t
Value::EstimateMemoryUsage() const
{
    switch (type())
    {
#if 0
        case Type::STRING: return base::trace_event::EstimateMemoryUsage(GetString());
        case Type::BINARY: return base::trace_event::EstimateMemoryUsage(GetBlob());
        case Type::DICTIONARY: return base::trace_event::EstimateMemoryUsage(dict());
        case Type::LIST: return base::trace_event::EstimateMemoryUsage(list());
#endif  // 0
        default: return 0;
    }
}
...
#if 0
void
Value::WriteIntoTrace(base::trace_event::TracedValue context) const
{
    switch (type())
    {
        case Type::BOOLEAN: std::move(context).WriteBoolean(GetBool()); return;
        case Type::INTEGER: std::move(context).WriteInt64(GetInt()); return;
        case Type::DOUBLE: std::move(context).WriteDouble(GetDouble()); return;
        case Type::STRING: std::move(context).WriteString(GetString()); return;
        case Type::BINARY: std::move(context).WriteString("<binary data not supported>"); return;
        ...
        case Type::NONE: std::move(context).WriteString("<none>"); return;
    }
}
#endif  // 0
...
// static
std::unique_ptr<DictionaryValue>
DictionaryValue::From(std::unique_ptr<Value> value)
{
    DictionaryValue* out;
...
DictionaryValue::DictionaryValue(const LegacyDictStorage& storage) : Value(storage)
{
}

DictionaryValue::DictionaryValue(LegacyDictStorage&& storage) noexcept : Value(std::move(storage))
{
}

bool
DictionaryValue::HasKey(StringPiece key) const
{
...
Value*
DictionaryValue::Set(StringPiece path, std::unique_ptr<Value> in_value)
{
         ...
         delimiter_position = current_path.find('.'))
    {
        ...
        Value* child_dictionary = current_dictionary->FindKeyOfType(key, Type::DICTIONARY);
        if (!child_dictionary)
        {
            child_dictionary = current_dictionary->SetKey(key, Value(Type::DICTIONARY));
        }
...
Value*
DictionaryValue::SetBoolean(StringPiece path, bool in_value)
{
    return Set(path, std::make_unique<Value>(in_value));
}

Value*
DictionaryValue::SetInteger(StringPiece path, int in_value)
{
    return Set(path, std::make_unique<Value>(in_value));
}

Value*
DictionaryValue::SetDouble(StringPiece path, double in_value)
{
    return Set(path, std::make_unique<Value>(in_value));
}

Value*
DictionaryValue::SetString(StringPiece path, StringPiece in_value)
{
    return Set(path, std::make_unique<Value>(in_value));
}

Value*
DictionaryValue::SetString(StringPiece path, const std::u16string& in_value)
{
    return Set(path, std::make_unique<Value>(in_value));
}

DictionaryValue*
DictionaryValue::SetDictionary(StringPiece path, std::unique_ptr<DictionaryValue> in_value)
{
...
Value*
DictionaryValue::SetWithoutPathExpansion(StringPiece key, std::unique_ptr<Value> in_value)
{
    // NOTE: We can't use |insert_or_assign| here, as only |try_emplace| does
    // an explicit conversion from StringPiece to std::string if necessary.
...
bool
DictionaryValue::Get(StringPiece path, const Value** out_value) const
{
    DCHECK(IsStringUTF8AllowingNoncharacters(path));
...
bool
DictionaryValue::Get(StringPiece path, Value** out_value)
{
    return as_const(*this).Get(path, const_cast<const Value**>(out_value));
}

bool
DictionaryValue::GetBoolean(StringPiece path, bool* bool_value) const
{
    const Value* value;
    if (!Get(path, &value))
        return false;
...
bool
DictionaryValue::GetInteger(StringPiece path, int* out_value) const
{
    const Value* value;
    if (!Get(path, &value))
        return false;
...
bool
DictionaryValue::GetDouble(StringPiece path, double* out_value) const
{
    const Value* value;
    if (!Get(path, &value))
        return false;
...
bool
DictionaryValue::GetString(StringPiece path, std::string* out_value) const
{
    const Value* value;
    if (!Get(path, &value))
        return false;
...
bool
DictionaryValue::GetString(StringPiece path, std::u16string* out_value) const
{
    const Value* value;
    if (!Get(path, &value))
        return false;
...
bool
DictionaryValue::GetStringASCII(StringPiece path, std::string* out_value) const
{
    std::string out;
...
bool
DictionaryValue::GetBinary(StringPiece path, const Value** out_value) const
{
    const Value* value;
...
bool
DictionaryValue::GetBinary(StringPiece path, Value** out_value)
{
    return as_const(*this).GetBinary(path, const_cast<const Value**>(out_value));
}

bool
DictionaryValue::GetDictionary(StringPiece path, const DictionaryValue** out_value) const
{
    const Value* value;
    bool result = Get(path, &value);
...
bool
DictionaryValue::GetDictionary(StringPiece path, DictionaryValue** out_value)
{
    return as_const(*this).GetDictionary(path, const_cast<const DictionaryValue**>(out_value));
}

bool
DictionaryValue::GetList(StringPiece path, const ListValue** out_value) const
{
    const Value* value;
    bool result = Get(path, &value);
...
bool
DictionaryValue::GetList(StringPiece path, ListValue** out_value)
{
    return as_const(*this).GetList(path, const_cast<const ListValue**>(out_value));
}

bool
DictionaryValue::GetWithoutPathExpansion(StringPiece key, const Value** out_value) const
{
    DCHECK(IsStringUTF8AllowingNoncharacters(key));
...
bool
DictionaryValue::GetWithoutPathExpansion(StringPiece key, Value** out_value)
{
    return as_const(*this).GetWithoutPathExpansion(key, const_cast<const Value**>(out_value));
}

bool
DictionaryValue::GetBooleanWithoutPathExpansion(StringPiece key, bool* out_value) const
{
    const Value* value;
...
bool
DictionaryValue::GetIntegerWithoutPathExpansion(StringPiece key, int* out_value) const
{
    const Value* value;
    if (!GetWithoutPathExpansion(key, &value))
        return false;
...
bool
DictionaryValue::GetDoubleWithoutPathExpansion(StringPiece key, double* out_value) const
{
    const Value* value;
    if (!GetWithoutPathExpansion(key, &value))
        return false;
...
bool
DictionaryValue::GetStringWithoutPathExpansion(StringPiece key, std::string* out_value) const
{
    const Value* value;
    if (!GetWithoutPathExpansion(key, &value))
        return false;
...
bool
DictionaryValue::GetStringWithoutPathExpansion(StringPiece key, std::u16string* out_value) const
{
    const Value* value;
    if (!GetWithoutPathExpansion(key, &value))
        return false;
...
bool
DictionaryValue::GetDictionaryWithoutPathExpansion(StringPiece key,
...
bool
DictionaryValue::GetDictionaryWithoutPathExpansion(StringPiece key, DictionaryValue** out_value)
{
...
bool
DictionaryValue::GetListWithoutPathExpansion(StringPiece key, const ListValue** out_value) const
{
    const Value* value;
    bool result = GetWithoutPathExpansion(key, &value);
    if (!result || !value->is_list())
...
bool
DictionaryValue::GetListWithoutPathExpansion(StringPiece key, ListValue** out_value)
{
    return as_const(*this).GetListWithoutPathExpansion(key,
                                                       const_cast<const ListValue**>(out_value));
}

bool
DictionaryValue::Remove(StringPiece path, std::unique_ptr<Value>* out_value)
{
    DCHECK(IsStringUTF8AllowingNoncharacters(path));
    StringPiece current_path(path);
    ...
    {
        if (!GetDictionary(current_path.substr(0, delimiter_position), &current_dictionary))
            return false;
        current_path = current_path.substr(delimiter_position + 1);
    }
...
bool
DictionaryValue::RemoveWithoutPathExpansion(StringPiece key, std::unique_ptr<Value>* out_value)
{
    DCHECK(IsStringUTF8AllowingNoncharacters(key));
    auto entry_iterator = dict().find(key);
    if (entry_iterator == dict().end())
...
bool
DictionaryValue::RemovePath(StringPiece path, std::unique_ptr<Value>* out_value)
{
    bool result = false;
...
void
DictionaryValue::Swap(DictionaryValue* other)
{
    CHECK(other->is_dict());
...
ListValue::From(std::unique_ptr<Value> value)
{
    ListValue* out;
...
ListValue::ListValue(ListStorage&& in_list) noexcept : Value(std::move(in_list))
{
}
...
bool
ListValue::Set(size_t index, std::unique_ptr<Value> in_value)
{
    if (!in_value)
        return false;
...
bool
ListValue::Get(size_t index, const Value** out_value) const
{
    if (index >= list().size())
        return false;
...
bool
ListValue::Get(size_t index, Value** out_value)
{
    return as_const(*this).Get(index, const_cast<const Value**>(out_value));
}

bool
ListValue::GetBoolean(size_t index, bool* bool_value) const
{
    const Value* value;
    if (!Get(index, &value))
        return false;
...
bool
ListValue::GetInteger(size_t index, int* out_value) const
{
    const Value* value;
    if (!Get(index, &value))
        return false;
...
bool
ListValue::GetDouble(size_t index, double* out_value) const
{
    const Value* value;
    if (!Get(index, &value))
        return false;
...
bool
ListValue::GetString(size_t index, std::string* out_value) const
{
    const Value* value;
    if (!Get(index, &value))
        return false;
...
bool
ListValue::GetString(size_t index, std::u16string* out_value) const
{
    const Value* value;
    if (!Get(index, &value))
        return false;
...
bool
ListValue::GetDictionary(size_t index, const DictionaryValue** out_value) const
{
    const Value* value;
...
bool
ListValue::GetDictionary(size_t index, DictionaryValue** out_value)
{
    return as_const(*this).GetDictionary(index, const_cast<const DictionaryValue**>(out_value));
}

bool
ListValue::GetList(size_t index, const ListValue** out_value) const
{
    const Value* value;
    bool result = Get(index, &value);
    if (!result || !value->is_list())
...
bool
ListValue::GetList(size_t index, ListValue** out_value)
{
    return as_const(*this).GetList(index, const_cast<const ListValue**>(out_value));
}

bool
ListValue::Remove(size_t index, std::unique_ptr<Value>* out_value)
{
    if (index >= list().size())
        return false;
...
bool
ListValue::Remove(const Value& value, size_t* index)
{
...
ListValue::iterator
ListValue::Erase(iterator iter, std::unique_ptr<Value>* out_value)
{
    if (out_value)
...
void
ListValue::Append(std::unique_ptr<Value> in_value)
{
    list().push_back(std::move(*in_value));
}

void
ListValue::AppendBoolean(bool in_value)
{
    list().emplace_back(in_value);
}

void
ListValue::AppendInteger(int in_value)
{
    list().emplace_back(in_value);
}

void
ListValue::AppendDouble(double in_value)
{
    list().emplace_back(in_value);
}

void
ListValue::AppendString(StringPiece in_value)
{
    list().emplace_back(in_value);
}

void
ListValue::AppendString(const std::u16string& in_value)
{
    list().emplace_back(in_value);
}

void
ListValue::AppendStrings(const std::vector<std::string>& in_values)
{
...
bool
ListValue::AppendIfNotPresent(std::unique_ptr<Value> in_value)
{
    DCHECK(in_value);
...
bool
ListValue::Insert(size_t index, std::unique_ptr<Value> in_value)
{
    DCHECK(in_value);
    if (index > list().size())
        return false;
...
ListValue::const_iterator
ListValue::Find(const Value& value) const
{
    return ranges::find(GetList(), value);
}

void
ListValue::Swap(ListValue* other)
{
    CHECK(other->is_list());
...
std::ostream&
operator<<(std::ostream& out, const Value::Type& type)
{
    if (static_cast<int>(type) < 0 || static_cast<size_t>(type) >= base::size(kTypeNames))
