...
inline size_t
AlignDown(size_t size, size_t alignment)
{
...
inline T*
AlignDown(T* ptr, size_t alignment)
{
...
inline size_t
AlignUp(size_t size, size_t alignment)
{
    DCHECK(IsPowerOfTwo(alignment));
...
template <typename T, typename = typename std::enable_if<sizeof(T) == 1>::type>
inline T*
AlignUp(T* ptr, size_t alignment)
{
...
/*ALWAYS_INLINE*/ typename std::enable_if<std::is_unsigned<T>::value && sizeof(T) <= 4,
                                          unsigned>::type
CountLeadingZeroBits(T x)
{
    static_assert(bits > 0, "invalid instantiation");
    unsigned long index;
    return LIKELY(_BitScanReverse(&index, static_cast<uint32_t>(x))) ? (31 - index - (32 - bits))
                                                                     : bits;
}

template <typename T, unsigned bits = sizeof(T) * 8>
/*ALWAYS_INLINE*/ typename std::enable_if<std::is_unsigned<T>::value && sizeof(T) == 8,
                                          unsigned>::type
CountLeadingZeroBits(T x)
{
    static_assert(bits > 0, "invalid instantiation");
...
// MSVC only supplies _BitScanReverse64 when building for a 64-bit target.
    ...
    return LIKELY(_BitScanReverse64(&index, static_cast<uint64_t>(x))) ? (63 - index) : 64;
#else
...
CountTrailingZeroBits(T x)
{
    static_assert(bits > 0, "invalid instantiation");
    unsigned long index;
    return LIKELY(_BitScanForward(&index, static_cast<uint32_t>(x))) ? index : bits;
}
...
ALWAYS_INLINE constexpr
        typename std::enable_if<std::is_unsigned<T>::value && sizeof(T) <= 8, unsigned>::type
        CountLeadingZeroBits(T value)
{
    static_assert(bits > 0, "invalid instantiation");
    return LIKELY(value) ? bits == 64 ? __builtin_clzll(static_cast<uint64_t>(value))
...
{
    return LIKELY(value) ? bits == 64 ? __builtin_ctzll(static_cast<uint64_t>(value))
                                      : __builtin_ctz(static_cast<uint32_t>(value))
                         : bits;
}

ALWAYS_INLINE constexpr uint32_t
CountLeadingZeroBits32(uint32_t x)
{
    return CountLeadingZeroBits(x);
}

ALWAYS_INLINE constexpr uint64_t
CountLeadingZeroBits64(uint64_t x)
{
    return CountLeadingZeroBits(x);
}
...
/*ALWAYS_INLINE*/ CORE_EXPORT size_t
