...
static void
xdg_dir_time_list_add(char* file_name, time_t mtime)
{
    XdgDirTimeList* list;
...
static void
xdg_run_command_on_dirs(XdgDirectoryFunc func, void* user_data)
{
 ...
 */
static int
xdg_check_file(const char* file_path, int* exists)
{
    struct stat st;
...
static int
xdg_check_dir(const char* directory, void* user_data)
{
    int invalid, exists;
    char* file_name;
...
const char*
xdg_mime_get_mime_type_for_data(const void* data, size_t len, int* result_prio)
{
...
const char*
xdg_mime_get_mime_type_for_file(const char* file_name, struct stat* statbuf)
{
    const char* mime_type;
    ...
    mime_type = _xdg_mime_magic_lookup_data(global_magic, data, bytes_read, NULL, mime_types, n);
...
int
_xdg_mime_mime_type_equal(const char* mime_a, const char* mime_b)
{
...
int
xdg_mime_mime_type_equal(const char* mime_a, const char* mime_b)
{
    xdg_mime_init();
...
int
xdg_mime_media_type_equal(const char* mime_a, const char* mime_b)
{
    char* sep;
...
int
_xdg_mime_mime_type_subclass(const char* mime, const char* base)
{
...
#if 1
    /* Handle supertypes */
    if (xdg_mime_is_super_type(ubase) && xdg_mime_media_type_equal(umime, ubase))
        return 1;
#endif
...
int
xdg_mime_mime_type_subclass(const char* mime, const char* base)
{
    xdg_mime_init();
 ...
 */
int
xdg_mime_register_reload_callback(XdgMimeCallback callback, void* data, XdgMimeDestroy destroy)
{
