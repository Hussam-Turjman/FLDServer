...
const char*
_xdg_mime_alias_list_lookup(XdgAliasList* list, const char* alias)
{
    XdgAlias* entry;
    XdgAlias key;
    ...
    if (list->n_aliases > 0)
    {
        ...
        entry = bsearch(&key, list->aliases, list->n_aliases, sizeof(XdgAlias), alias_entry_cmp);
        if (entry)
...
void
_xdg_mime_alias_read_from_file(XdgAliasList* list, const char* file_name)
{
    FILE* file;
    char line[255];
    int alloc;
    ...
    if (list->n_aliases > 1)
        qsort(list->aliases, list->n_aliases, sizeof(XdgAlias), alias_entry_cmp);
}
