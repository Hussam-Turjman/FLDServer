...
_xdg_mime_magic_parse_magic_line(FILE* magic_file, XdgMimeMagicMatch* match)
{
    ...
    if (c == '~')
    {
        ...
        if (matchlet->word_size != 0 && matchlet->word_size != 1 && matchlet->word_size != 2
                                       ...
                                       int indent)
{
    while ((matchlet != NULL) && (matchlet->indent == indent))
    {
        if (_xdg_mime_magic_matchlet_compare_to_data(matchlet, data, len))
        {
            ...
            if (_xdg_mime_magic_matchlet_compare_level(matchlet->next, data, len, indent + 1))
                return TRUE;
        }
...
static int
_xdg_mime_magic_match_compare_to_data(XdgMimeMagicMatch* match, const void* data, size_t len)
{
    return _xdg_mime_magic_matchlet_compare_level(match->matchlet, data, len, 0);
}

static void
_xdg_mime_magic_insert_match(XdgMimeMagic* mime_magic, XdgMimeMagicMatch* match)
{
                            ...
                            int n_mime_types)
{
    ...
    for (match = mime_magic->match_list; match; match = match->next)
    {
        ...
        else
        {
            ...
            {
                if (mime_types[n] && _xdg_mime_mime_type_equal(mime_types[n], match->mime_type))
                    mime_types[n] = NULL;
            }
        }
    }
        ...
        switch (state)
        {
            ...
            case XDG_MIME_MAGIC_MAGIC:
                    ...
                    || (state == XDG_MIME_MAGIC_EOF && match->mime_type))
                {
                    ...
                    _xdg_mime_magic_match_free(match);
                break;
            case XDG_MIME_MAGIC_ERROR: state = _xdg_mime_magic_parse_error(magic_file); break;
...
void
_xdg_mime_magic_read_from_file(XdgMimeMagic* mime_magic, const char* file_name)
{
    FILE* magic_file;
    char header[12];
