                                     ...
                                     size_t len)
{
    ...
    for (i = range_start; i < range_start + range_length; i++)
    {
        ...
        {
            valid_matchlet =
                    memcmp(cache->buffer + data_offset, (unsigned char*) data + i, data_length)
                    == 0;
        }
...
static int
cache_magic_matchlet_compare(XdgMimeCache* cache, xdg_uint32_t offset, const void* data, size_t len)
{
    ...
    if (cache_magic_matchlet_compare_to_data(cache, offset, data, len))
    {
        ...
        {
            if (cache_magic_matchlet_compare(cache, child_offset + 32 * i, data, len))
                return TRUE;
        }
    }
...
static const char*
cache_magic_compare_to_data(
        XdgMimeCache* cache, xdg_uint32_t offset, const void* data, size_t len, int* prio)
{
...
static const char*
cache_magic_lookup_data(XdgMimeCache* cache, const void* data, size_t len, int* prio)
{
    ...
    *prio = 0;
    ...
    for (j = 0; j < n_entries; j++)
    {
        ...
        match = cache_magic_compare_to_data(cache, offset + 16 * j, data, len, prio);
        if (match)
            return match;
    }
...
static int
cache_glob_lookup_suffix(
        const char* file_name, int len, int ignore_case, MimeWeight mime_types[], int n_mime_types)
{
    int i, n;
    ...
    for (i = 0; _caches[i]; i++)
    {
        ...
        n = cache_glob_node_lookup_suffix(
...
static int
cache_glob_lookup_file_name(const char* file_name, const char* mime_types[], int n_mime_types)
{
    int n;
    MimeWeight mimes[10];
    int n_mimes = 10;
...
cache_get_mime_type_for_data(
        const void* data, size_t len, int* result_prio, const char* mime_types[], int n_mime_types)
{
    const char* mime_type;
    int i, n, priority;
...
const char*
_xdg_mime_cache_get_mime_type_for_data(const void* data, size_t len, int* result_prio)
{
    return cache_get_mime_type_for_data(data, len, result_prio, NULL, 0);
}

const char*
_xdg_mime_cache_get_mime_type_for_file(const char* file_name, struct stat* statbuf)
{
    const char* mime_type;
    ...
    mime_type = cache_get_mime_type_for_data(data, bytes_read, NULL, mime_types, n);
...
int
_xdg_mime_cache_mime_type_subclass(const char* mime, const char* base)
{
...
#if 1
    /* Handle supertypes */
    if (is_super_type(ubase) && xdg_mime_media_type_equal(umime, ubase))
        return 1;
#endif

    /*  Handle special cases text/plain and application/octet-stream */
    if (strcmp(ubase, "text/plain") == 0 && strncmp(umime, "text/", 5) == 0)
        return 1;

    if (strcmp(ubase, "application/octet-stream") == 0 && strncmp(umime, "inode/", 6) != 0)
        return 1;
        ...
        for (j = 0; j < n_entries; j++)
        {
            ...
            if (strcmp(cache->buffer + mimetype_offset, mime) == 0)
            {
                ...
                for (k = 0; k < n_parents && p < 127; k++)
                {
                     ...
                     * This can happen for instance if the same type is listed in multiple
                     * directories
                     */
                    for (l = 0; l < p; l++)
                    {
...
static void
dump_glob_node(XdgMimeCache* cache, xdg_uint32_t offset, int depth)
{
    xdg_unichar_t character;
