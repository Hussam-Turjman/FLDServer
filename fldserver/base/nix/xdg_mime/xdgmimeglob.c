...
_xdg_glob_list_append(
        XdgGlobList* glob_list, void* data, const char* mime_type, int weight, int case_sensitive)
{
    XdgGlobList* new_element;
    ...
    {
        if (strcmp(tmp_element->data, data) == 0 && strcmp(tmp_element->mime_type, mime_type) == 0)
            return glob_list;
                           ...
                           int case_sensitive)
{
    ...
    {
        node->child =
                _xdg_glob_hash_insert_ucs4(node->child, text, mime_type, weight, case_sensitive);
    }
    return glob_hash_node;
}
                                     ...
                                     int n_mime_types)
{
    ...
    for (node = glob_hash_node; node && character >= node->character; node = node->next)
    {
        if (character == node->character)
        {
            ...
            if (n == 0)
            {
                if (node->mime_type && (case_sensitive_check || !node->case_sensitive))
                {
                                ...
                                int n_mime_types)
{
    ...
    for (list = glob_hash->literal_list; list; list = list->next)
    {
        if (!list->case_sensitive && strcmp((const char*) list->data, lower_case) == 0)
        {
            mime_types[0] = list->mime_type;
            free(lower_case);
            return 1;
    ...
    n = _xdg_glob_hash_node_lookup_file_name(
            glob_hash->simple_node, lower_case, len, FALSE, mimes, n_mimes);
    if (n == 0)
        n = _xdg_glob_hash_node_lookup_file_name(
                glob_hash->simple_node, file_name, len, TRUE, mimes, n_mimes);
                           ...
                           int case_sensitive)
{
    ...
    switch (type)
    {
        case XDG_GLOB_LITERAL:
            glob_hash->literal_list = _xdg_glob_list_append(glob_hash->literal_list,
                                                            strdup(glob),
                                                            strdup(mime_type),
                                                            weight,
                                                            case_sensitive);
        ...
        case XDG_GLOB_SIMPLE:
            glob_hash->simple_node = _xdg_glob_hash_insert_text(
                    glob_hash->simple_node, glob + 1, mime_type, weight, case_sensitive);
            break;
        case XDG_GLOB_FULL:
            glob_hash->full_list = _xdg_glob_list_append(
                    glob_hash->full_list, strdup(glob), strdup(mime_type), weight, case_sensitive);
            break;
    }
}
...
void
_xdg_mime_glob_read_from_file(XdgGlobHash* glob_hash, const char* file_name, int version_two)
{
    FILE* glob_file;
    char line[255];
    char* p;
