...
_xdg_mime_parent_list_lookup(XdgParentList* list, const char* mime)
{
    ...
    if (list->n_mimes > 0)
    {
        ...
        entry = bsearch(
                &key, list->parents, list->n_mimes, sizeof(XdgMimeParents), &parent_entry_cmp);
        if (entry)
...
_xdg_mime_parent_read_from_file(XdgParentList* list, const char* file_name)
{
    ...
    while (fgets(line, 255, file) != NULL)
    {
        ...
        *(sep++) = '\000';
        ...
        {
            entry->n_parents += 1;
            entry->parents = realloc(entry->parents, (entry->n_parents + 2) * sizeof(char*));
        }
        entry->parents[entry->n_parents - 1] = strdup(sep);
        entry->parents[entry->n_parents] = NULL;
    }

    list->parents = realloc(list->parents, list->n_mimes * sizeof(XdgMimeParents));

    fclose(file);

    if (list->n_mimes > 1)
        qsort(list->parents, list->n_mimes, sizeof(XdgMimeParents), &parent_entry_cmp);
}
