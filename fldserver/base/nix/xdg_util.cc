...
#include "fldserver/base/nix/xdg_util.h"
...
#include "fldserver/base/nix/xdg_user_dirs/xdg_user_dir_lookup.h"
...
FilePath
GetXDGDirectory(Environment* env, const char* env_name, const char* fallback_dir)
{
    FilePath path;
...
FilePath
GetXDGUserDirectory(const char* dir_name, const char* fallback_dir)
{
    FilePath path;
...
DesktopEnvironment
GetDesktopEnvironment(Environment* env)
{
    ...
    if (env->GetVar(kXdgCurrentDesktopEnvVar, &xdg_current_desktop))
    {
        ...
        for (const auto& value :
             SplitStringPiece(xdg_current_desktop, ":", TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY))
        {
            if (value == "Unity")
            {
    ...
    if (env->GetVar("DESKTOP_SESSION", &desktop_session))
    {
        ...
        if (desktop_session.find("xfce") != std::string::npos || desktop_session == "xubuntu")
        {
            return DESKTOP_ENVIRONMENT_XFCE;
        }
    }
...
const char*
GetDesktopEnvironmentName(DesktopEnvironment env)
{
    switch (env)
    {
        case DESKTOP_ENVIRONMENT_OTHER: return nullptr;
        case DESKTOP_ENVIRONMENT_CINNAMON: return "CINNAMON";
        case DESKTOP_ENVIRONMENT_GNOME: return "GNOME";
        case DESKTOP_ENVIRONMENT_KDE3: return "KDE3";
        case DESKTOP_ENVIRONMENT_KDE4: return "KDE4";
        case DESKTOP_ENVIRONMENT_KDE5: return "KDE5";
        case DESKTOP_ENVIRONMENT_PANTHEON: return "PANTHEON";
        case DESKTOP_ENVIRONMENT_UNITY: return "UNITY";
        case DESKTOP_ENVIRONMENT_XFCE: return "XFCE";
    }
    return nullptr;
}

const char*
GetDesktopEnvironmentName(Environment* env)
{
