...
#include "fldserver/base/i18n/break_iterator.h"
...
BreakIterator::BreakIterator(const StringPiece16& str, BreakType break_type) :
    iter_(nullptr), string_(str), break_type_(break_type), prev_(npos), pos_(0)
{
}

BreakIterator::BreakIterator(const StringPiece16& str, const std::u16string& rules) :
    iter_(nullptr), string_(str), rules_(rules), break_type_(RULE_BASED), prev_(npos), pos_(0)
{
}
...
public:
    DefaultLocaleBreakIteratorCache() :
        main_status_(U_ZERO_ERROR), main_(nullptr), main_could_be_leased_(true)
    {
        main_ = ubrk_open(break_type, nullptr, nullptr, 0, &main_status_);
        if (U_FAILURE(main_status_))
        {
            NOTREACHED() << "ubrk_open failed for type " << break_type << " with error "
                         << main_status_;
        }
    }

    virtual ~DefaultLocaleBreakIteratorCache()
    {
        ubrk_close(main_);
    }

    UBreakIterator*
    Lease(UErrorCode& status)
    {
        if (U_FAILURE(status))
        {
            return nullptr;
        ...
        // object instead.
        UBreakIterator* result = ubrk_open(break_type, nullptr, nullptr, 0, &status);
        if (U_FAILURE(status))
        {
            NOTREACHED() << "ubrk_open failed for type " << break_type << " with error " << status;
        }
        return result;
    }

    void
    Return(UBreakIterator* item)
    {
...
static LazyInstance<DefaultLocaleBreakIteratorCache<UBRK_CHARACTER>>::Leaky char_break_cache =
        LAZY_INSTANCE_INITIALIZER;
static LazyInstance<DefaultLocaleBreakIteratorCache<UBRK_WORD>>::Leaky word_break_cache =
        LAZY_INSTANCE_INITIALIZER;
static LazyInstance<DefaultLocaleBreakIteratorCache<UBRK_SENTENCE>>::Leaky sentence_break_cache =
        LAZY_INSTANCE_INITIALIZER;
static LazyInstance<DefaultLocaleBreakIteratorCache<UBRK_LINE>>::Leaky line_break_cache =
        LAZY_INSTANCE_INITIALIZER;
...
BreakIterator::~BreakIterator()
{
    if (iter_)
    {
        ...
        switch (break_type_)
        {
            ...
            case RULE_BASED: ubrk_close(iter); break;
            // Otherwise, return the iter to the cache it leased from.`
            case BREAK_CHARACTER: char_break_cache.Pointer()->Return(iter); break;
            case BREAK_WORD: word_break_cache.Pointer()->Return(iter); break;
            case BREAK_SENTENCE: sentence_break_cache.Pointer()->Return(iter); break;
            case BREAK_LINE:
            case BREAK_NEWLINE: line_break_cache.Pointer()->Return(iter); break;
            default: NOTREACHED() << "invalid break_type_"; break;
        }
    }
}
...
BreakIterator::Init()
{
    ...
    switch (break_type_)
    {
        case BREAK_CHARACTER: iter_ = char_break_cache.Pointer()->Lease(status); break;
        case BREAK_WORD: iter_ = word_break_cache.Pointer()->Lease(status); break;
        case BREAK_SENTENCE: iter_ = sentence_break_cache.Pointer()->Lease(status); break;
        case BREAK_LINE:
        case BREAK_NEWLINE: iter_ = line_break_cache.Pointer()->Lease(status); break;
        case RULE_BASED:
            ...
            break;
        default: NOTREACHED() << "invalid break_type_"; return false;
    }
...
bool
BreakIterator::SetText(const char16_t* text, const size_t length)
{
    UErrorCode status = U_ZERO_ERROR;
...
bool
BreakIterator::IsWord() const
{
    return GetWordBreakStatus() == IS_WORD_BREAK;
}

BreakIterator::WordBreakStatus
BreakIterator::GetWordBreakStatus() const
{
    ...
    return (status == UBRK_WORD_NONE || pos_ == npos) ? IS_SKIPPABLE_WORD : IS_WORD_BREAK;
}

bool
BreakIterator::IsEndOfWord(size_t position) const
{
    if (break_type_ != BREAK_WORD && break_type_ != RULE_BASED)
        return false;
...
bool
BreakIterator::IsStartOfWord(size_t position) const
{
    if (break_type_ != BREAK_WORD && break_type_ != RULE_BASED)
        return false;
...
bool
BreakIterator::IsSentenceBoundary(size_t position) const
{
...
bool
BreakIterator::IsGraphemeBoundary(size_t position) const
{
