...
#include "fldserver/base/i18n/case_conversion.h"
...
int32_t
ToUpperMapper(
        UChar* dest, int32_t dest_capacity, const UChar* src, int32_t src_length, UErrorCode* error)
{
    // Use default locale.
...
FoldCaseMapper(
        UChar* dest, int32_t dest_capacity, const UChar* src, int32_t src_length, UErrorCode* error)
{
    return u_strFoldCase(dest, dest_capacity, src, src_length, U_FOLD_CASE_DEFAULT, error);
}

// Provides similar functionality as UnicodeString::caseMap but on
// std::u16string.
std::u16string
CaseMap(StringPiece16 string, CaseMapperFunction case_mapper)
{
    std::u16string dest;
    if (string.empty())
        return dest;
    ...
    do
    {
        ...
        int32_t new_length = case_mapper(&dest[0],
...
std::u16string
ToLower(StringPiece16 string)
{
    return CaseMap(string, &ToLowerMapper);
}

std::u16string
ToUpper(StringPiece16 string)
{
    return CaseMap(string, &ToUpperMapper);
}

std::u16string
FoldCase(StringPiece16 string)
{
