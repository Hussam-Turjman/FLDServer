...
#include "fldserver/base/i18n/time_formatting.h"

#include "fldserver/base/i18n/unicodestring.h"
...
{
std::u16string
TimeFormat(const icu::DateFormat* formatter, const Time& time)
{
    DCHECK(formatter);
...
std::u16string
TimeFormatWithoutAmPm(const icu::DateFormat* formatter, const Time& time)
{
    DCHECK(formatter);
    icu::UnicodeString time_string;

    icu::FieldPosition ampm_field(icu::DateFormat::kAmPmField);
    formatter->format(static_cast<UDate>(time.ToDoubleT() * 1000), time_string, ampm_field);
...
CreateSimpleDateFormatter(const char* pattern)
{
...
UMeasureFormatWidth
DurationWidthToMeasureWidth(DurationFormatWidth width)
{
    switch (width)
    {
...
const char*
DateFormatToString(DateFormat format)
{
    switch (format)
    {
        case DATE_FORMAT_YEAR_MONTH: return UDAT_YEAR_MONTH;
        case DATE_FORMAT_MONTH_WEEKDAY_DAY: return UDAT_MONTH_WEEKDAY_DAY;
    }
    NOTREACHED();
...
#if 0
std::u16string
TimeFormatMonthAndYear(const Time& time, const icu::TimeZone* time_zone)
{
...
bool
TimeDurationFormat(const TimeDelta time, const DurationFormatWidth width, std::u16string* out)
{
    DCHECK(out);
    UErrorCode status = U_ZERO_ERROR;
    ...
    if (U_FAILURE(status))
    {
        LOG(ERROR) << "Creating MeasureUnit or Measure for " << hours << "h" << minutes
                   << "m failed: " << u_errorName(status);
        return false;
    }
    ...
    if (U_FAILURE(status))
    {
        LOG(ERROR) << "Creating MeasureFormat for " << icu::Locale::getDefault().getName()
                              ...
                              std::u16string* out)
{
    ...
    const int64_t minutes =
            (total_seconds - hours * base::Time::kSecondsPerHour) / base::Time::kSecondsPerMinute;
...
DateIntervalFormat(const Time& begin_time, const Time& end_time, DateFormat format)
{
    ...
    std::unique_ptr<icu::DateIntervalFormat> formatter(
            icu::DateIntervalFormat::createInstance(DateFormatToString(format), status));
...
GetHourClockType()
{
    ...
    std::unique_ptr<icu::SimpleDateFormat> formatter(static_cast<icu::SimpleDateFormat*>(
            icu::DateFormat::createTimeInstance(icu::DateFormat::kShort)));
