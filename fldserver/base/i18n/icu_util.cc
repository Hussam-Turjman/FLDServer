...
#include "fldserver/base/i18n/icu_util.h"
...
#include "fldserver/base/android/apk_assets.h"
#include "fldserver/base/android/timezone_utils.h"
#endif
...
#include "fldserver/base/fuchsia/intl_profile_watcher.h"
#endif
...
std::unique_ptr<PfRegion>
OpenIcuDataFile(const std::string& filename, const std::string& split_name)
{
    auto result = std::make_unique<PfRegion>();
#if defined(OS_ANDROID)
    result->pf = android::OpenApkAsset(kAssetsPathPrefix + filename, split_name, &result->region);
    if (result->pf != -1)
    {
        return result;
    }
...
int
LoadIcuData(PlatformFile data_fd,
...
bool
InitializeICUWithFileDescriptorInternal(PlatformFile data_fd,
                                        const MemoryMappedFile::Region& data_region)
{
...
InitializeIcuTimeZone()
{
    ...
    std::string zone_id = FuchsiaIntlProfileWatcher::GetPrimaryTimeZoneIdForIcuInitialization();
    icu::TimeZone::adoptDefault(
            icu::TimeZone::createTimeZone(icu::UnicodeString::fromUTF8(zone_id)));
#elif (IS_LINUX || defined(OS_CHROMEOS)) && 1
...
TraceICUEntry(const void*, int32_t fn_number)
{
    switch (fn_number)
    {
...
static void U_CALLCONV
TraceICUData(const void* context, int32_t fn_number, int32_t level, const char* fmt, va_list args)
{
    switch (fn_number)
    {
        ...
        case UTRACE_UBRK_CREATE_LINE: {
            ...
            switch (lb_type[0])
            {
                case '\0': value = ICUCreateInstance::kLineBreakIterator; break;
                case 'l':
...
#if (ICU_UTIL_DATA_IMPL == ICU_UTIL_DATA_FILE)
bool
InitializeExtraICUWithFileDescriptor(PlatformFile data_fd,
                                     const MemoryMappedFile::Region& data_region)
{
    if (g_icudtl_pf != kInvalidPlatformFile)
    {
...
bool
InitializeICUWithFileDescriptor(PlatformFile data_fd, const MemoryMappedFile::Region& data_region)
{
#if DCHECK_IS_ON()
...
bool
InitializeExtraICU(const std::string& split_name)
{
    if (g_icudtl_pf != kInvalidPlatformFile)
    {
    ...
    std::unique_ptr<MemoryMappedFile> mapped_file;
    UErrorCode err;
    if (LoadIcuData(g_icudtl_extra_pf, g_icudtl_extra_region, &mapped_file, &err) != 0)
    {
        return false;
    }
    g_icudtl_extra_mapped_file = mapped_file.release();
...
void
SetIcuTimeZoneDataDirForTesting(const char* dir)
{
...
void
AllowMultipleInitializeCallsForTesting()
{
#if DCHECK_IS_ON()
