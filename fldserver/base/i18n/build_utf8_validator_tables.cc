...
void
ConstructPairAndAppend(const Character& character,
                       const Range& new_range,
                       const StringSet& existing_set,
                       PairVector* pairs)
{
    Pair new_pair = {character, StringSet(1, new_range)};
    new_pair.set.insert(new_pair.set.end(), existing_set.begin(), existing_set.end());
...
// [\xa0-\xbf][\x80-\xbf]).
void
MoveRightMostCharToSet(PairVector* pairs)
{
    ...
    while (it != pairs->end())
    {
        ...
        unconverted_bytes =
                Character(current_pair.character.begin(), current_pair.character.end() - 1);
...
void
MoveAllCharsToSets(PairVector* pairs)
{
...
void
LogStringSets(const PairVector& pairs)
{
    for (const auto& pair_it : pairs)
    {
        std::string set_as_string;
        for (auto set_it = pair_it.set.begin(); set_it != pair_it.set.end(); ++set_it)
        {
...
uint8_t
MakeState(const StringSet& set, std::vector<State>* states, StateMap* state_map)
{
    DCHECK(!set.empty());
    ...
    const uint8_t target_state = where == state_map->end() ? MakeState(rest, states, state_map)
                                                           : where->second;
    DCHECK_LT(0, range.from());
    DCHECK_LT(range.to(), 0xFF);
    const StateRange new_state_initializer[] = {
            {0, 1}, {range.from(), target_state}, {static_cast<uint8_t>(range.to() + 1), 1}};
    states->push_back(State(new_state_initializer,
...
std::vector<State>
GenerateStates(const PairVector& pairs)
{
...
void
PrintStates(const std::vector<State>& states, FILE* stream)
{
    ...
    for (const auto& state_it : states)
    {
        ...
        uint8_t shift = 7;
        for (auto range_it = state_it.begin(); range_it != state_it.end(); ++range_it)
        {
...
main(int argc, char* argv[])
{
    ...
    if (!filename.empty())
    {
        ...
        if (!output)
            PLOG(FATAL) << "Couldn't open '" << filename.AsUTF8Unsafe() << "' for writing";
    }
