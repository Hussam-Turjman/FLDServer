...
namespace i18n
{
class CORE_EXPORT BreakIterator
{
public:
    enum BreakType
    {
    ...
    bool
    SetText(const char16_t* text, const size_t length);
    ...
    bool
    IsEndOfWord(size_t position) const;
    bool
    IsStartOfWord(size_t position) const;
    ...
    // or RULE_BASED.
    bool
    IsSentenceBoundary(size_t position) const;

    // Under BREAK_CHARACTER mode, returns whether |position| is a Unicode
    // grapheme boundary.
    bool
    IsGraphemeBoundary(size_t position) const;
