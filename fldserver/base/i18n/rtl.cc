...
#include "fldserver/base/i18n/rtl.h"

#include "fldserver/base/command_line.h"
#include "fldserver/base/files/file_path.h"
#include "fldserver/base/i18n/base_i18n_switches.h"
...
#include "fldserver/base/ios/ios_util.h"
#endif
...
std::string
GetLocaleString(const icu::Locale& locale)
{
...
GetCharacterDirection(UChar32 character)
{
    ...
    if (has_switch)
    {
        ...
        std::string force_flag = command_line->GetSwitchValueASCII(switches::kForceTextDirection);
    ...
    switch (property)
    {
        ...
        case U_RIGHT_TO_LEFT_OVERRIDE: return base::i18n::RIGHT_TO_LEFT;
        case U_LEFT_TO_RIGHT:
        case U_LEFT_TO_RIGHT_EMBEDDING:
        case U_LEFT_TO_RIGHT_OVERRIDE: return base::i18n::LEFT_TO_RIGHT;
    }
...
std::string
GetCanonicalLocale(const std::string& locale)
{
...
std::string
ICULocaleName(const std::string& locale_string)
{
...
void
SetICUDefaultLocale(const std::string& locale_string)
{
#if defined(OS_IOS)
    static base::debug::CrashKeyString* crash_key_locale = base::debug::AllocateCrashKeyString(
            "icu_locale_input", base::debug::CrashKeySize::Size256);
...
TextDirection
GetStringDirection(const std::u16string& text)
{
    const char16_t* string = text.c_str();
    size_t length = text.length();
    size_t position = 0;
...
#if IS_WIN
bool
AdjustStringForLocaleDirection(std::u16string* text)
{
    if (!IsRTL() || text->empty())
        return false;
...
bool
UnadjustStringForLocaleDirection(std::u16string* text)
{
    if (!IsRTL() || text->empty())
        return false;
...
AdjustStringForLocaleDirection(std::u16string* text)
{
    ...
    {
        WrapStringWithRTLFormatting(text);
        text->insert(static_cast<size_t>(0), static_cast<size_t>(1), kLeftToRightMark);
        text->push_back(kLeftToRightMark);
    }
    else if (ui_direction_is_rtl && has_rtl_chars)
    {
        WrapStringWithRTLFormatting(text);
        text->insert(static_cast<size_t>(0), static_cast<size_t>(1), kRightToLeftMark);
...
UnadjustStringForLocaleDirection(std::u16string* text)
{
    ...
    std::u16string unmarked_text = text->substr(begin_index, end_index - begin_index + 1);
...
void
EnsureTerminatedDirectionalFormatting(std::u16string* text)
{
    int count = 0;
    for (auto c : *text)
    {
...
void
SanitizeUserSuppliedString(std::u16string* text)
{
...
bool
StringContainsStrongRTLChars(const std::u16string& text)
{
    const char16_t* string = text.c_str();
    size_t length = text.length();
    size_t position = 0;
...
void
WrapStringWithLTRFormatting(std::u16string* text)
{
    if (text->empty())
        return;

    // Inserting an LRE (Left-To-Right Embedding) mark as the first character.
    text->insert(static_cast<size_t>(0), static_cast<size_t>(1), kLeftToRightEmbeddingMark);
...
void
WrapStringWithRTLFormatting(std::u16string* text)
{
    if (text->empty())
        return;

    // Inserting an RLE (Right-To-Left Embedding) mark as the first character.
    text->insert(static_cast<size_t>(0), static_cast<size_t>(1), kRightToLeftEmbeddingMark);
...
void
WrapPathWithLTRFormatting(const FilePath& path, std::u16string* rtl_safe_path)
{
...
StripWrappingBidiControlCharacters(const std::u16string& text)
{
    ...
    if (begin == kLeftToRightEmbeddingMark || begin == kRightToLeftEmbeddingMark
        || begin == kLeftToRightOverride || begin == kRightToLeftOverride)
        ++begin_index;
    size_t end_index = text.length() - 1;
