...
#include "fldserver/base/i18n/file_util_icu.h"

#include "fldserver/base/check.h"
#include "fldserver/base/files/file_path.h"
#include "fldserver/base/i18n/icu_string_conversions.h"
#include "fldserver/base/i18n/string_compare.h"
...
public:
    ...
    IllegalCharacters&
    operator=(const IllegalCharacters&) = delete;

    static IllegalCharacters*
    GetInstance()
    {
    ...
    bool
    IsDisallowedEverywhere(UChar32 ucs4) const
    {
        return !!illegal_anywhere_.contains(ucs4);
    }

    bool
    IsDisallowedLeadingOrTrailing(UChar32 ucs4) const
    {
        return !!illegal_at_ends_.contains(ucs4);
    }

    bool
    IsAllowedName(const std::u16string& s) const
    {
        return s.empty()
               || (!!illegal_anywhere_.containsNone(icu::UnicodeString(s.c_str(), s.size()))
...
IllegalCharacters::IllegalCharacters()
{
    ...
    illegal_anywhere_ = icu::UnicodeSet(UNICODE_STRING_SIMPLE("[[\"~*/:<>?\\\\|][:Cc:][:Cf:]]"),
                                        everywhere_status);
    illegal_at_ends_ = icu::UnicodeSet(UNICODE_STRING_SIMPLE("[[:WSpace:][.]]"), ends_status);
...
UChar32
GetNextCodePoint(const FilePath::StringType* const file_name, int& cursor)
{
    UChar32 code_point;
#if IS_WIN
    // Windows uses UTF-16 encoding for filenames.
    U16_NEXT(file_name->data(), cursor, static_cast<int>(file_name->length()), code_point);
    ...
    // UTF-8.
    U8_NEXT(file_name->data(), cursor, static_cast<int>(file_name->length()), code_point);
#else
...
bool
IsFilenameLegal(const std::u16string& file_name)
{
    return IllegalCharacters::GetInstance()->IsAllowedName(file_name);
}

void
ReplaceIllegalCharactersInPath(FilePath::StringType* file_name, char replace_char)
{
    ...
    static const UChar32 kExtensionSeparator = checked_cast<UChar32>(FilePath::kExtensionSeparator);
    ...
    while (cursor < static_cast<int>(file_name->size()))
    {
        ...
        const bool is_illegal_at_ends = illegal->IsDisallowedLeadingOrTrailing(code_point);
...
bool
LocaleAwareCompareFilenames(const FilePath& a, const FilePath& b)
{
    ...
    return CompareString16WithCollator(
                   *collator, AsStringPiece16(a.value()), AsStringPiece16(b.value()))
           == UCOL_LESS;

#elif IS_POSIX || defined(OS_FUCHSIA)
    // On linux, the file system encoding is not defined. We assume
    // SysNativeMBToWide takes care of it.
    return CompareString16WithCollator(*collator,
                                       WideToUTF16(SysNativeMBToWide(a.value())),
                                       WideToUTF16(SysNativeMBToWide(b.value())))
           == UCOL_LESS;
#endif
}

void
NormalizeFileNameEncoding(FilePath* file_name)
{
#if 0
    std::string normalized_str;
    if (ConvertToUtf8AndNormalize(file_name->BaseName().value(), kCodepageUTF8, &normalized_str)
