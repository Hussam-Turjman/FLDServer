...
#include "fldserver/base/i18n/string_search.h"
...
namespace i18n
{
FixedPatternStringSearch::FixedPatternStringSearch(const std::u16string& find_this,
                                                   bool case_sensitive) :
    find_this_(find_this)
{
    ...
    search_ = usearch_open(find_this_.data(),
                           find_this_.size(),
                           dummy.data(),
                           dummy.size(),
                           uloc_getDefault(),
...
bool
FixedPatternStringSearch::Search(const std::u16string& in_this,
                                 size_t* match_index,
                                 size_t* match_length,
                                 bool forward_search)
{
...
FixedPatternStringSearchIgnoringCaseAndAccents::FixedPatternStringSearchIgnoringCaseAndAccents(
...
bool
FixedPatternStringSearchIgnoringCaseAndAccents::Search(const std::u16string& in_this,
                                                       size_t* match_index,
                                                       size_t* match_length)
{
    return base_search_.Search(in_this,
                               match_index,
                               match_length,
                               /*forward_search=*/true);
}
