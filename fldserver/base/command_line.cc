...
constexpr CommandLine::CharType kSwitchValueSeparator[] = FILE_PATH_LITERAL("=");
...
constexpr CommandLine::CharType kSingleArgument[] = FILE_PATH_LITERAL("single-argument");
#endif  // IS_WIN

size_t
GetSwitchPrefixLength(CommandLine::StringPieceType string)
{
...
bool
IsSwitch(const CommandLine::StringType& string,
...
bool
IsSwitchWithKey(CommandLine::StringPieceType string,
...
// Windows*.
std::wstring
QuoteForCommandLineToArgvW(const std::wstring& arg, bool allow_unsafe_insert_sequences)
{
...
// static
void
CommandLine::InitUsingArgvForTesting(int argc, const char* const* argv)
{
...
// static
bool
CommandLine::Init(int argc, const char* const* argv)
{
...
// static
CommandLine
CommandLine::FromString(StringPieceType command_line)
{
...
void
CommandLine::InitFromArgv(int argc, const CommandLine::CharType* const* argv)
{
...
void
CommandLine::InitFromArgv(const StringVector& argv)
{
...
void
CommandLine::SetProgram(const FilePath& program)
{
#if IS_WIN
...
bool
CommandLine::HasSwitch(const StringPiece& switch_string) const
{
...
bool
CommandLine::HasSwitch(const char switch_constant[]) const
{
    return HasSwitch(StringPiece(switch_constant));
}

std::string
CommandLine::GetSwitchValueASCII(const StringPiece& switch_string) const
{
...
FilePath
CommandLine::GetSwitchValuePath(const StringPiece& switch_string) const
{
    return FilePath(GetSwitchValueNative(switch_string));
}

CommandLine::StringType
CommandLine::GetSwitchValueNative(const StringPiece& switch_string) const
{
    DCHECK_EQ(ToLowerASCII(switch_string), switch_string);
...
void
CommandLine::AppendSwitch(const std::string& switch_string)
{
    AppendSwitchNative(switch_string, StringType());
}

void
CommandLine::AppendSwitchPath(const std::string& switch_string, const FilePath& path)
{
    AppendSwitchNative(switch_string, path.value());
}

void
CommandLine::AppendSwitchNative(const std::string& switch_string,
                                CommandLine::StringPieceType value)
{
    ...
    base::InsertOrAssign(switches_, switch_key.substr(prefix_length), StringType(value));
...
void
CommandLine::AppendSwitchASCII(const std::string& switch_string, const std::string& value_string)
{
#if IS_WIN
...
void
CommandLine::RemoveSwitch(base::StringPiece switch_key_without_prefix)
{
#if IS_WIN
    ...
    auto expell = std::remove_if(
            argv_switches_begin, argv_switches_end, [&switch_key_native](const StringType& arg) {
...
void
CommandLine::CopySwitchesFrom(const CommandLine& source, const char* const switches[], size_t count)
{
...
void
CommandLine::AppendArg(const std::string& value)
{
#if IS_WIN
...
void
CommandLine::AppendArgPath(const FilePath& path)
{
    AppendArgNative(path.value());
}

void
CommandLine::AppendArgNative(const CommandLine::StringType& value)
{
    argv_.push_back(value);
}

void
CommandLine::AppendArguments(const CommandLine& other, bool include_program)
{
    if (include_program)
...
void
CommandLine::PrependWrapper(const CommandLine::StringType& wrapper)
{
    if (wrapper.empty())
        return;
    // Split the wrapper command based on whitespace (with quoting).
    using CommandLineTokenizer = StringTokenizerT<StringType, StringType::const_iterator>;
...
#if IS_WIN
void
CommandLine::ParseFromString(StringPieceType command_line)
{
    ...
    HMODULE downlevel_shell32_dll = ::LoadLibraryEx(
            L"api-ms-win-downlevel-shell32-l1-1-0.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
    ...
    DPLOG_IF(FATAL, !args) << "CommandLineToArgvW failed on command line: " << command_line;
...
void
CommandLine::AppendSwitchesAndArguments(const CommandLine::StringVector& argv)
{
    ...
    for (size_t i = 1; i < argv.size(); ++i)
    {
        ...
        {
#if IS_WIN
            if (is_parsed_from_string && IsSwitchWithKey(switch_string, kSingleArgument))
            {
...
CommandLine::StringType
CommandLine::GetArgumentsStringInternal(bool allow_unsafe_insert_sequences) const
{
    StringType params;
...
#if IS_WIN
                switch_value =
                        QuoteForCommandLineToArgvW(switch_value, allow_unsafe_insert_sequences);
#endif
...
CommandLine::StringType
CommandLine::GetCommandLineStringForShell() const
{
    DCHECK(GetArgs().empty());
    StringType command_line_string = GetCommandLineString();
    return command_line_string + FILE_PATH_LITERAL(" ") + StringType(kSwitchPrefixes[0])
           + kSingleArgument + FILE_PATH_LITERAL(" %1");
}
...
CommandLine::GetCommandLineStringWithUnsafeInsertSequences() const
{
    ...
    StringType params(GetArgumentsStringInternal(/*allow_unsafe_insert_sequences=*/true));
    if (!params.empty())
    {
        string.append(FILE_PATH_LITERAL(" "));
        string.append(params);
...
#if IS_WIN
void
CommandLine::ParseAsSingleArgument(const CommandLine::StringType& single_arg_switch)
{
    ...
    // argument or switch.
    const size_t single_arg_switch_position = raw_command_line_string_.find(single_arg_switch);
    ...
    // argument is present.
    const size_t arg_position = single_arg_switch_position + single_arg_switch.length() + 1;
