...
public:
    ...
    static void
    InitUsingArgvForTesting(int argc, const char* const* argv);
#endif
...
#if IS_WIN
    static CommandLine
    FromString(StringPieceType command_line);
#endif

    // Initialize from an argv vector.
    void
    InitFromArgv(int argc, const CharType* const* argv);
    void
    InitFromArgv(const StringVector& argv);
    ...
    StringType
    GetCommandLineStringForShell() const;
    ...
    StringType
    GetCommandLineStringWithUnsafeInsertSequences() const;
#endif
    ...
    GetProgram() const;
    void
    SetProgram(const FilePath& program);
    ...
    // StringPiece.
    bool
    HasSwitch(const StringPiece& switch_string) const;
    bool
    HasSwitch(const char switch_constant[]) const;

    // Returns the value associated with the given switch. If the switch has no
    // value or isn't present, this method returns the empty string.
    // Switch names must be lowercase.
    std::string
    GetSwitchValueASCII(const StringPiece& switch_string) const;
    FilePath
    GetSwitchValuePath(const StringPiece& switch_string) const;
    StringType
    GetSwitchValueNative(const StringPiece& switch_string) const;
    ...
    void
    AppendSwitch(const std::string& switch_string);
    void
    AppendSwitchPath(const std::string& switch_string, const FilePath& path);
    void
    AppendSwitchNative(const std::string& switch_string, StringPieceType value);
    void
    AppendSwitchASCII(const std::string& switch_string, const std::string& value);
    ...
    void
    RemoveSwitch(const base::StringPiece switch_key_without_prefix);

    // Copy a set of switches (and any values) from another command line.
    // Commonly used when launching a subprocess.
    void
    CopySwitchesFrom(const CommandLine& source, const char* const switches[], size_t count);
    ...
    // Note: Switches will precede arguments regardless of appending order.
    void
    AppendArg(const std::string& value);
    void
    AppendArgPath(const FilePath& value);
    void
    AppendArgNative(const StringType& value);

    // Append the switches and arguments from another command line to this one.
    // If |include_program| is true, include |other|'s program as well.
    void
    AppendArguments(const CommandLine& other, bool include_program);

    // Insert a command before the current command.
    // Common for debuggers, like "gdb --args".
    void
    PrependWrapper(const StringType& wrapper);

#if IS_WIN
    // Initialize by parsing the given command line string.
    // The program name is assumed to be the first item in the string.
    void
    ParseFromString(StringPieceType command_line);
#endif
...
private:
    ...
    void
    AppendSwitchesAndArguments(const StringVector& argv);
    ...
    StringType
    GetArgumentsStringInternal(bool allow_unsafe_insert_sequences) const;
    ...
    void
    ParseAsSingleArgument(const StringType& single_arg_switch_string);
