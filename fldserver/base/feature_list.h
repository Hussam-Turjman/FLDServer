...
extern CORE_EXPORT const Feature kDCheckIsFatalFeature;
...
public:
    ...
    public:
        ...
        operator=(const ScopedDisallowOverrides&) = delete;
        ~ScopedDisallowOverrides();
    ...
    void
    InitializeFromCommandLine(const std::string& enable_features,
    ...
    void
    InitializeFromSharedMemory(PersistentMemoryAllocator* allocator);
    ...
    bool
    IsFeatureOverridden(const std::string& feature_name) const;
    ...
    bool
    IsFeatureOverriddenFromCommandLine(const std::string& feature_name) const;

    // Returns true if the state |feature_name| has been overridden by
    // |InitializeFromCommandLine()| and the state matches |state|.
    bool
    IsFeatureOverriddenFromCommandLine(const std::string& feature_name, OverrideState state) const;
    ...
    void
    AssociateReportingFieldTrial(const std::string& feature_name,
    ...
    void
    RegisterExtraFeatureOverrides(const std::vector<FeatureOverrideInfo>& extra_overrides);

    // Loops through feature overrides and serializes them all into |allocator|.
    void
    AddFeaturesToAllocator(PersistentMemoryAllocator* allocator);
    ...
    void
    GetFeatureOverrides(std::string* enable_overrides, std::string* disable_overrides);
    ...
    void
    GetCommandLineFeatureOverrides(std::string* enable_overrides, std::string* disable_overrides);
    ...
    static bool
    IsEnabled(const Feature& feature);
    ...
    static FieldTrial*
    GetFieldTrial(const Feature& feature);

    // Splits a comma-separated string containing feature names into a vector. The
    // resulting pieces point to parts of |input|.
    static std::vector<base::StringPiece>
    SplitFeatureListString(base::StringPiece input);
    ...
    static bool
    InitializeInstance(const std::string& enable_features, const std::string& disable_features);
    ...
    static bool
    InitializeInstance(const std::string& enable_features,
                       const std::string& disable_features,
    ...
    static void
    SetInstance(std::unique_ptr<FeatureList> instance);
    ...
    static std::unique_ptr<FeatureList>
    ...
    static void
    RestoreInstanceForTesting(std::unique_ptr<FeatureList> instance);
...
private:
    ...
    bool
    IsFeatureEnabled(const Feature& feature);
    ...
    // initialized.
    base::FieldTrial*
    GetAssociatedFieldTrial(const Feature& feature);
    ...
    void
    RegisterOverridesFromCommandLine(const std::string& feature_list,
                                     OverrideState overridden_state);
    ...
    // will not be changed.
    void
    RegisterOverride(StringPiece feature_name,
                     OverrideState overridden_state,
                     FieldTrial* field_trial);
    ...
    void
    GetFeatureOverridesImpl(std::string* enable_overrides,
    ...
    // DCHECKs and tests.
    bool
    CheckFeatureIdentity(const Feature& feature);
