...
#if defined(ARCH_CPU_ARM_FAMILY) && (defined(OS_ANDROID) || IS_LINUX || defined(OS_CHROMEOS))
#include "fldserver/base/files/file_util.h"
#include "fldserver/base/numerics/checked_math.h"
...
ComputeX86FamilyAndModel(const std::string& vendor, int signature)
{
...
#if defined(ARCH_CPU_ARM_FAMILY) && (defined(OS_ANDROID) || IS_LINUX || defined(OS_CHROMEOS))
StringPairs::const_iterator
FindFirstProcCpuKey(const StringPairs& pairs, StringPiece key)
{
...
ParseProcCpu()
{
    static const NoDestructor<ProcCpuInfo> info([]() {
        ...
        if (model_name != pairs.end())
        {
            info.brand = std::string(TrimWhitespaceASCII(model_name->second, TRIM_ALL));
        }
        ...
        if (implementer_string != pairs.end())
        {
            ...
            if (!CheckedNumeric<uint32_t>(implementer).AssignIfValid(&info.implementer))
            {
                info.implementer = 0;
            }
        }
    ...
    return *info;
        ...
        // IS_LINUX || defined(OS_CHROMEOS))

}  // namespace

void
CPU::Initialize(bool require_branding)
{
#if defined(ARCH_CPU_X86_FAMILY)
    int cpu_info[4] = {-1};
    ...
    static_assert(kVendorNameSize < base::size(cpu_string), "cpu_string too small");
...
namespace
{
constexpr char kTimeInStatePath[] = "/sys/devices/system/cpu/cpu%d/cpufreq/stats/time_in_state";
constexpr char kPhysicalPackageIdPath[] = "/sys/devices/system/cpu/cpu%d/topology/"
                                          "physical_package_id";
constexpr char kCoreIdleStateTimePath[] = "/sys/devices/system/cpu/cpu%d/cpuidle/state%d/time";
...
bool
ParseTimeInState(const std::string& content,
                 ...
                 CPU::TimeInState& time_in_state)
{
    ...
    for (size_t pos = 0; pos <= max_pos;)
    {
        ...
        uint64_t time;
        int matches =
                sscanf(begin + pos, "%" PRIu64 " %" PRIu64 "\n%n", &frequency, &time, &num_chars);
        if (matches != 2)
            return false;
        ...
        if (time > 0)
        {
            time_in_state.push_back(
                    {core_type, core_index, frequency, internal::ClockTicksToTimeDelta(time)});
        }
...
GuessCoreTypes()
{
    ...
    const char kCPUMaxFreqPath[] = "/sys/devices/system/cpu/cpu%d/cpufreq/cpuinfo_max_freq";
    int num_cpus = SysInfo::NumberOfProcessors();
    std::vector<CPU::CoreType> core_index_to_type(num_cpus, CPU::CoreType::kUnknown);
    ...
    for (int core_index = 0; core_index < num_cpus; ++core_index)
    {
        ...
        else if (num_frequencies == 2u || num_frequencies == 3u)
        {
            ...
            if (it != frequencies_mhz.end())
            {
                ...
                {
                    case 0:
                        core_type = num_frequencies == 2u ? CPU::CoreType::kBigLittle_Little
                                                          : CPU::CoreType::kBigLittleBigger_Little;
                        break;
                    case 1:
                        core_type = num_frequencies == 2u ? CPU::CoreType::kBigLittle_Big
...
// static
const std::vector<CPU::CoreType>&
CPU::GetGuessedCoreTypes()
{
...
// static
bool
CPU::GetTimeInState(TimeInState& time_in_state)
{
    time_in_state.clear();
    ...
    static NoDestructor<std::vector<int>> kFirstCoresIndexes([]() {
        ...
        for (int core_index = 0; core_index < SysInfo::NumberOfProcessors(); core_index++)
        {
            // Reading from physical_package_id doesn't block (it amounts to reading a
            // struct field from the kernel).
            ThreadRestrictions::ScopedAllowIO allow_io;

            FilePath package_id_path(StringPrintf(kPhysicalPackageIdPath, core_index));
            std::string package_id_str;
            if (!ReadFileToString(package_id_path, &package_id_str))
                return std::vector<int>();
            int package_id;
            base::StringPiece trimmed =
                    base::TrimWhitespaceASCII(package_id_str, base::TrimPositions::TRIM_ALL);
    ...
    {
        FilePath time_in_state_path(StringPrintf(kTimeInStatePath, cluster_core_index));
        ...
        if (!ParseTimeInState(
                    buffer, kCoreTypes[cluster_core_index], cluster_core_index, time_in_state))
        {
            return false;
        }
    }
...
// static
bool
CPU::GetCumulativeCoreIdleTimes(CoreIdleTimes& idle_times)
{
    idle_times.clear();
