...
#define SEQUENCE_CHECKER_INTERNAL_CONCAT(a, b) SEQUENCE_CHECKER_INTERNAL_CONCAT2(a, b)
#define SEQUENCE_CHECKER_INTERNAL_UID(prefix) SEQUENCE_CHECKER_INTERNAL_CONCAT(prefix, __LINE__)
...
class THREAD_ANNOTATION_ATTRIBUTE__(capability("context")) SequenceCheckerDoNothing
{
public:
    static void
    EnableStackLogging()
    ...
    operator=(SequenceCheckerDoNothing&& other) = default;
    SequenceCheckerDoNothing(const SequenceCheckerDoNothing&) = delete;
    SequenceCheckerDoNothing&
    operator=(const SequenceCheckerDoNothing&) = delete;

    bool
    CalledOnValidSequence(void* = nullptr) const WARN_UNUSED_RESULT
    {
        return true;
    }
    void
...
public:
    ...
    explicit ScopedValidateSequenceChecker(const SequenceChecker& checker, const StringPiece& msg)
            EXCLUSIVE_LOCK_FUNCTION(checker)
    {
        std::unique_ptr<debug::StackTrace> bound_at;
        DCHECK(checker.CalledOnValidSequence(&bound_at))
