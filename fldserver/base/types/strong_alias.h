...
public:
    ...
    constexpr const UnderlyingType*
    operator->() const
    {
        return &value_;
    }
    ...
    constexpr explicit operator const UnderlyingType&() const&
    {
        return value_;
    }

    constexpr bool
    operator==(const StrongAlias& other) const
    {
    ...
    struct Hasher
    {
        ...
        result_type
        operator()(const argument_type& id) const
        {
...
template <
        ...
        typename = std::enable_if_t<base::internal::SupportsOstreamOperator<UnderlyingType>::value>>
std::ostream&
operator<<(std::ostream& stream, const StrongAlias<TagType, UnderlyingType>& alias)
{
