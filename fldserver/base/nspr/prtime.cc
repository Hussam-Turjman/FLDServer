...
#if IS_WIN
static void
localtime_r(const time_t* secs, struct tm* time)
{
...
static const PRInt8 nDays[2][12] = {{31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31},
...
PR_NormalizeTime(PRExplodedTime* time, PRTimeParamFn params)
{
    ...
    time->tm_yday = static_cast<PRInt16>(
...
PRStatus
PR_ParseTimeString(const char* string, PRBool default_to_gmt, PRTime* result_imploded)
{
    PRExplodedTime tm;
        ...
        switch (*rest)
        {
            ...
            case '9': {
                ...
                else if ((*end == '/' || *end == '-') && end[1] >= '0' && end[1] <= '9')
                {
                    ...
                    year = (year < 0 ? ((rest[0] - '0') * 10000L + (rest[1] - '0') * 1000L
                                        + (rest[2] - '0') * 100L + (rest[3] - '0') * 10L
                                        + (rest[4] - '0'))
                                     : year);
    ...
    SKIP_MORE:
        ...
        if (*rest == '-'
            && ((rest > string && isalpha((unsigned char) rest[-1]) && year < 0) || rest[1] < '0'
                || rest[1] > '9'))
        {
            rest++;
            goto SKIP_MORE;
    ...
    if (zone_offset == -1)
    {
        ...
        PR_ASSERT(result->tm_month > -1 && result->tm_mday > 0 && result->tm_hour > -1
                  && result->tm_min > -1 && result->tm_sec > -1);
        ...
        zone_offset = localTime.tm_min + 60 * localTime.tm_hour + 1440 * (localTime.tm_mday - 2);
    }
