...
InnerPageDecoder::TryAdvance()
{
    ...
    const int cell_pointer_offset = kFirstCellOfsetInnerPageOffset + (read_index << 1);
    DCHECK_LE(cell_pointer_offset + 2, db_reader_->page_size()) << "ComputeCellCount() used an "
                                                                   "incorrect upper bound";
    const int cell_pointer = LoadBigEndianUint16(page_data + cell_pointer_offset);

    static_assert(std::numeric_limits<uint16_t>::max() + 4 < std::numeric_limits<int>::max(),
...
// static
bool
InnerPageDecoder::IsOnValidPage(DatabasePageReader* db_reader)
{
...
// static
int
InnerPageDecoder::ComputeCellCount(DatabasePageReader* db_reader)
{
    // The B-tree page header stores the cell count.
    int header_count = LoadBigEndianUint16(db_reader->page_data() + kCellCountPageOffset);
    static_assert(kCellCountPageOffset + 2 <= DatabasePageReader::kMinUsablePageSize,
    ...
    int upper_bound = (db_reader->page_size() - kFirstCellOfsetInnerPageOffset) >> 1;
    static_assert(kFirstCellOfsetInnerPageOffset <= DatabasePageReader::kMinUsablePageSize,
...
LeafPageDecoder::TryAdvance()
{
    ...
    const int cell_pointer_offset = kFirstCellOfsetLeafPageOffset + (read_index << 1);
    DCHECK_LE(cell_pointer_offset + 2, db_reader_->page_size()) << "ComputeCellCount() used an "
                                                                   "incorrect upper bound";
    const int cell_pointer = LoadBigEndianUint16(page_data + cell_pointer_offset);

    static_assert(std::numeric_limits<uint16_t>::max() + 3 < std::numeric_limits<int>::max(),
                  "The addition below may overflow");
    ...
    std::tie(last_record_rowid_, record_start) = ParseVarint(rowid_start, page_end);
...
// static
bool
LeafPageDecoder::IsOnValidPage(DatabasePageReader* db_reader)
{
    static_assert(kPageTypePageOffset < DatabasePageReader::kMinUsablePageSize,
                  "The check below may perform an out-of-bounds memory access");
...
// static
int
LeafPageDecoder::ComputeCellCount(DatabasePageReader* db_reader)
{
    ...
    int upper_bound = (db_reader->page_size() - kFirstCellOfsetLeafPageOffset) >> 1;
    static_assert(kFirstCellOfsetLeafPageOffset <= DatabasePageReader::kMinUsablePageSize,
                  "The |upper_bound| computation above may overflow");
