...
#include "fldserver/base/sql/recover_module/record.h"
...
// processing.
ParsedHeaderType
ParseHeaderType(int64_t encoded_type)
{
    ...
    const ValueType value_type = (encoded_type & 1) == 0 ? ValueType::kBlob : ValueType::kText;
...
RecordReader::Initialize()
{
    ...
    for (int i = 0; i < column_count_; ++i)
    {
        ...
        {
            std::tie(encoded_type, header_pointer) = ParseVarint(header_pointer, header_end);
        }
        ...
        value_headers_.emplace_back(next_value_offset,
                                    parsed_type.size,
                                    parsed_type.type,
                                    parsed_type.inline_value,
...
ValueType
RecordReader::GetValueType(int column_index) const
{
...
bool
RecordReader::ReadValue(int column_index, sqlite3_context* receiver) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(IsInitialized());
    DCHECK_GE(column_index, 0);
    ...
    if (header.type == ValueType::kInteger)
    {
        ...
        for (uint8_t* sign_extended_byte = &value_bytes[0]; sign_extended_byte < first_read_byte;
             ++sign_extended_byte)
        {
    ...
    if (header.type == ValueType::kFloat)
    {
        ...
        if (!payload_reader_->ReadPayload(
                    header.offset, sizeof(double), reinterpret_cast<uint8_t*>(&value)))
        {
            return false;
        }
    ...
    if (header.type == ValueType::kBlob || header.type == ValueType::kText)
    {
        ...
        if (header.type == ValueType::kBlob)
        {
            sqlite3_result_blob64(
                    receiver, value_bytes, static_cast<uint64_t>(size), &ValueBytesDeleter);
        }
        else
        {
            ...
            sqlite3_result_text64(receiver,
                                  reinterpret_cast<char*>(value_bytes),
...
int64_t
RecordReader::InitializeHeaderBuffer()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    const uint8_t* const inline_payload_start = payload_reader_->ReadInlinePayload();
    ...
    const uint8_t* const inline_payload_end = inline_payload_start + inline_payload_size;
    int64_t header_size;
    ...
    if (!payload_reader_->ReadPayload(header_size_size, header_data_size, header_buffer_.data()))
    {
        // Read failure.
        return 0;
    }
