...
public:
    ...
    DatabasePageReader&
    operator=(const DatabasePageReader&) = delete;
    ...
    page_data() const
    {
        ...
        DCHECK_NE(page_id_, kInvalidPageId) << "Successful ReadPage() required before accessing "
                                               "pager state";
    ...
    IsValidPageSize(int page_size) noexcept
    {
    ...
    static constexpr bool
    IsValidPageId(int64_t page_id) noexcept
    {
    ...
    static int
    RawRead(sqlite3_file* sqlite_file, int read_size, int64_t read_offset, uint8_t* buffer);
