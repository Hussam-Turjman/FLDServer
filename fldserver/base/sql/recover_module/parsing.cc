...
SplitToken(base::StringPiece sql)
{
...
base::Optional<ModuleColumnType>
ParseColumnType(base::StringPiece column_type_sql)
{
...
base::StringPiece
ColumnTypeToSql(ModuleColumnType column_type)
{
    switch (column_type)
    {
        case ModuleColumnType::kInteger: return kIntegerSql;
        case ModuleColumnType::kFloat: return kFloatSql;
        case ModuleColumnType::kText: return kTextSql;
        case ModuleColumnType::kBlob: return kBlobSql;
        case ModuleColumnType::kNumeric: return kNumericSql;
        case ModuleColumnType::kRowId: return kIntegerSql;  // rowids are ints.
        case ModuleColumnType::kAny: return base::StringPiece();
    }
    NOTREACHED();
}
...
std::string
RecoveredColumnSpec::ToCreateTableSql() const
{
    base::StringPiece not_null_sql = (is_non_null) ? " NOT NULL" : "";
    return base::StrCat({this->name, " ", ColumnTypeToSql(this->type), not_null_sql});
}

bool
RecoveredColumnSpec::IsAcceptableValue(ValueType value_type) const
{
    ...
    {
        return type == ModuleColumnType::kFloat || (!is_strict && type == ModuleColumnType::kFloat);
    }
    if (value_type == ValueType::kText)
        return type == ModuleColumnType::kText;
    if (value_type == ValueType::kBlob)
    {
        return type == ModuleColumnType::kBlob || (!is_strict && type == ModuleColumnType::kText);
    }
...
RecoveredColumnSpec
ParseColumnSpec(const char* sqlite_arg)
{
    ...
    base::Optional<ModuleColumnType> column_type = ParseColumnType(column_type_sql);
...
TargetTableSpec
ParseTableSpec(const char* sqlite_arg)
{
    base::StringPiece sql(sqlite_arg);
