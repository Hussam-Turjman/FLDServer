...
inline int
LoadBigEndianUint16(const uint8_t* buffer)
{
    static_assert(std::numeric_limits<uint16_t>::max() <= std::numeric_limits<int>::max(),
...
inline int32_t
LoadBigEndianInt32(const uint8_t* buffer)
{
    // The code gets optimized to mov + bswap on x86_64, and to ldr + rev on ARM.
    return (static_cast<int32_t>(buffer[0]) << 24) | (static_cast<int32_t>(buffer[1]) << 16)
           | (static_cast<int32_t>(buffer[2]) << 8) | static_cast<int32_t>(buffer[3]);
}
...
inline int64_t
LoadBigEndianInt64(const uint8_t* buffer)
{
    // The code gets optimized to mov + bswap on x86_64, and to ldr + rev on ARM.
    return (static_cast<int64_t>(buffer[0]) << 56) | (static_cast<int64_t>(buffer[1]) << 48)
           | (static_cast<int64_t>(buffer[2]) << 40) | (static_cast<int64_t>(buffer[3]) << 32)
           | (static_cast<int64_t>(buffer[4]) << 24) | (static_cast<int64_t>(buffer[5]) << 16)
...
std::pair<int64_t, const uint8_t*>
ParseVarint(const uint8_t* buffer, const uint8_t* buffer_end);
