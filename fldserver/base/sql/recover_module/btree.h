...
public:
    ...
    InnerPageDecoder&
    operator=(const InnerPageDecoder&) = delete;
    ...
    static bool
    IsOnValidPage(DatabasePageReader* db_reader);
...
private:
    ...
    static int
    ComputeCellCount(DatabasePageReader* db_reader);
...
public:
    ...
    LeafPageDecoder&
    operator=(const LeafPageDecoder&) = delete;

    // The rowid of the most recent record read by TryAdvance().
    //
    // Must only be called after a successful call to TryAdvance().
    int64_t
    last_record_rowid() const
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        DCHECK(last_record_size_ != 0) << "TryAdvance() not called / did not succeed";
