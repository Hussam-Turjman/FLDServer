...
#include "fldserver/base/sql/recover_module/cursor.h"
#include "fldserver/base/sql/recover_module/parsing.h"
#include "fldserver/base/sql/recover_module/table.h"
#include "sqlite3.h"
...
std::vector<RecoveredColumnSpec>
ParseColumnSpecs(int argc, const char* const* argv)
{
...
int
ModuleCreate(sqlite3* sqlite_db,
             void* /* client_data */,
             int argc,
             ...
             char** /* error_string */)
{
    ...
    TargetTableSpec backing_table_spec = ParseTableSpec(argv[kBackingTableSpecArgument]);
    ...
    std::tie(sqlite_status, table) =
            VirtualTable::Create(sqlite_db, std::move(backing_table_spec), std::move(column_specs));
...
int
ModuleConnect(sqlite3* sqlite_db,
              void* client_data,
              int argc,
              const char* const* argv,
              sqlite3_vtab** result_sqlite_table,
              char** error_string)
{
    ...
    return ModuleCreate(sqlite_db, client_data, argc, argv, result_sqlite_table, error_string);
}

int
ModuleBestIndex(sqlite3_vtab* sqlite_table, sqlite3_index_info* index_info)
{
...
int
ModuleDisconnect(sqlite3_vtab* sqlite_table)
{
    DCHECK(sqlite_table != nullptr);
...
int
ModuleDestroy(sqlite3_vtab* sqlite_table)
{
    return ModuleDisconnect(sqlite_table);
}

int
ModuleOpen(sqlite3_vtab* sqlite_table, sqlite3_vtab_cursor** result_sqlite_cursor)
{
    DCHECK(sqlite_table != nullptr);
...
int
ModuleClose(sqlite3_vtab_cursor* sqlite_cursor)
{
...
int
ModuleFilter(sqlite3_vtab_cursor* sqlite_cursor,
             int /* best_index_num */,
...
int
ModuleNext(sqlite3_vtab_cursor* sqlite_cursor)
{
    DCHECK(sqlite_cursor != nullptr);
...
int
ModuleEof(sqlite3_vtab_cursor* sqlite_cursor)
{
    DCHECK(sqlite_cursor != nullptr);
...
int
ModuleColumn(sqlite3_vtab_cursor* sqlite_cursor, sqlite3_context* result_context, int column_index)
{
    DCHECK(sqlite_cursor != nullptr);
...
int
ModuleRowid(sqlite3_vtab_cursor* sqlite_cursor, sqlite3_int64* result_rowid)
{
    DCHECK(sqlite_cursor != nullptr);
...
int
RegisterRecoverExtension(sqlite3* db)
{
