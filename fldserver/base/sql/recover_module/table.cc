...
base::Optional<int>
GetTableRootPageId(sqlite3* sqlite_db, const TargetTableSpec& table)
{
    ...
    std::string select_sql = base::StrCat({"SELECT rootpage FROM ",
                                           table.db_name,
                                           ".sqlite_master WHERE type='table' AND tbl_name=?"});
    sqlite3_stmt* sqlite_statement;
    if (sqlite3_prepare_v3(sqlite_db,
                           select_sql.c_str(),
                           select_sql.size() + 1,
    ...
    if (sqlite3_bind_text(sqlite_statement,
                          1,
...
std::pair<int, int>
GetDatabasePageSize(sqlite3_file* sqlite_file)
{
    ...
    int sqlite_status =
            DatabasePageReader::RawRead(sqlite_file, kHeaderSize, kHeaderOffset, header_bytes);
    ...
    const int page_size = LoadBigEndianUint16(header_bytes + kPageSizeHeaderOffset);
...
// static
std::pair<int, std::unique_ptr<VirtualTable>>
VirtualTable::Create(sqlite3* sqlite_db,
                     ...
                     std::vector<RecoveredColumnSpec> column_specs)
{
    ...
    int sqlite_status = sqlite3_file_control(
            sqlite_db, backing_table_spec.db_name.c_str(), SQLITE_FCNTL_FILE_POINTER, &sqlite_file);
    if (sqlite_status != SQLITE_OK)
    {
    ...
    return {SQLITE_OK,
            std::make_unique<VirtualTable>(sqlite_db,
                                           sqlite_file,
...
{
#if DCHECK_IS_ON()
    DCHECK_EQ(0, open_cursor_count_.load(std::memory_order_relaxed)) << "SQLite forgot to xClose() "
                                                                        "an xOpen()ed cursor";
#endif  // DCHECK_IS_ON()
}

std::string
VirtualTable::ToCreateTableSql() const
{
...
void
VirtualTable::WillDeleteCursor(VirtualCursor* cursor)
{
#if DCHECK_IS_ON()
