...
#include "fldserver/base/sql/recover_module/payload.h"
...
#include "fldserver/base/sql/recover_module/btree.h"
#include "fldserver/base/sql/recover_module/integers.h"
#include "fldserver/base/sql/recover_module/pager.h"
#include "sqlite3.h"
...
int
MaxInlinePayloadSize(int page_size)
{
    DCHECK_GE(page_size, DatabasePageReader::kMinUsablePageSize);
    DCHECK_LE(page_size, DatabasePageReader::kMaxPageSize);

    const int max_inline_payload_size = page_size - kMaxPageOverhead - kMaxCellOverhead;
    DCHECK_GE(max_inline_payload_size, LeafPayloadReader::kMinInlineSize);
    static_assert(DatabasePageReader::kMinPageSize - kMaxPageOverhead - kMaxCellOverhead
...
int
MinInlinePayloadSize(int page_size)
{
    DCHECK_GE(page_size, DatabasePageReader::kMinUsablePageSize);
    DCHECK_LE(page_size, DatabasePageReader::kMaxPageSize);

    const int min_inline_payload_size =
            ((page_size - kMaxPageOverhead) * kLeafPayloadFraction) / kPayloadFractionDenominator
            - kMaxCellOverhead;
    static_assert((DatabasePageReader::kMaxPageSize - kMaxPageOverhead) * kLeafPayloadFraction
    ...
    static_assert(((DatabasePageReader::kMinUsablePageSize - kMaxPageOverhead)
                   * kLeafPayloadFraction) / kPayloadFractionDenominator
                                  - kMaxCellOverhead
                          >= LeafPayloadReader::kMinInlineSize,
                  "The DCHECK above may fail");
...
// size.
int
MaxOverflowPayloadSize(int page_size)
{
    DCHECK_GE(page_size, DatabasePageReader::kMinUsablePageSize);
    DCHECK_LE(page_size, DatabasePageReader::kMaxPageSize);
    ...
    static_assert(DatabasePageReader::kMinUsablePageSize > 4, "The DCHECK above may fail");
...
bool
LeafPayloadReader::Initialize(int64_t payload_size, int payload_offset)
{
    ...
    else
    {
        ...
        static_assert(DatabasePageReader::kMaxPageSize * 2 <= std::numeric_limits<int>::max(),
        ...
        if (efficient_inline_payload_size <= max_inline_payload_size)
        {
            ...
            DCHECK_EQ(0, (payload_size - inline_payload_size_) % max_overflow_payload_size_)
        ...
        DCHECK_EQ(overflow_page_count_,
                  (payload_size - inline_payload_size_ + (max_overflow_payload_size_ - 1))
    ...
    const int first_overflow_page_id_size = (overflow_page_count_ == 0) ? 0 : kPageIdSize;

    if (inline_payload_offset_ + inline_payload_size_ + first_overflow_page_id_size > page_size)
    {
...
bool
LeafPayloadReader::ReadPayload(int64_t offset, int64_t size, uint8_t* buffer)
{
    DCHECK_GE(offset, 0);
    ...
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(page_id_ != DatabasePageReader::kInvalidPageId) << "Initialize() not called, or last "
    ...
    if (offset < inline_payload_size_)
    {
        ...
        const int read_size = (static_cast<int>(offset) + size <= inline_payload_size_)
        ...
        std::copy(db_reader_->page_data() + read_offset,
                  db_reader_->page_data() + read_offset + read_size,
                  buffer);
    ...
    while (size > 0)
    {
        ...
        while (overflow_page_ids_.size() <= static_cast<size_t>(overflow_page_index))
        {
...
const uint8_t*
LeafPayloadReader::ReadInlinePayload()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(page_id_ != DatabasePageReader::kInvalidPageId) << "Initialize() not called, or last "
                                                              "call did not succeed";
...
bool
LeafPayloadReader::PopulateNextOverflowPageId()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK_LT(overflow_page_ids_.size(), static_cast<size_t>(overflow_page_count_));
    ...
    const int next_page_id = LoadBigEndianInt32(db_reader_->page_data() + page_id_offset);
