...
public:
    ...
    LeafPayloadReader&
    operator=(const LeafPayloadReader&) = delete;
    ...
    // class can be called.
    bool
    Initialize(int64_t payload_size, int payload_offset);
    ...
    // payload_size().
    int
    inline_payload_size() const
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        DCHECK(page_id_ != DatabasePageReader::kInvalidPageId) << "Initialize() not called, or "
    ...
    bool
    ReadPayload(int64_t offset, int64_t size, uint8_t* buffer);
