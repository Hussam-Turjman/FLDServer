...
int
VirtualCursor::ReadColumn(int column_index, sqlite3_context* result_context)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void
VirtualCursor::AppendPageDecoder(int page_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(leaf_decoder_.get() == nullptr)
            << __func__ << " must only be called when the current path has no leaf decoder";
    ...
    if (InnerPageDecoder::IsOnValidPage(&db_reader_))
    {
        ...
        inner_decoders_.emplace_back(std::make_unique<InnerPageDecoder>(&db_reader_));
        return;
    }
}

bool
VirtualCursor::IsAcceptableRecord()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(record_reader_.IsInitialized());
