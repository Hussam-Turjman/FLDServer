...
#if defined(OS_MAC)
#include "fldserver/base/mac/mac_util.h"
#endif

#if defined(OS_FUCHSIA)
#include "fldserver/base/sql/vfs_wrapper_fuchsia.h"
#endif
...
sqlite3_vfs*
GetWrappedVfs(sqlite3_vfs* wrapped_vfs)
{
    return static_cast<sqlite3_vfs*>(wrapped_vfs->pAppData);
}

VfsFile*
AsVfsFile(sqlite3_file* wrapper_file)
{
    return reinterpret_cast<VfsFile*>(wrapper_file);
}

sqlite3_file*
GetWrappedFile(sqlite3_file* wrapper_file)
{
    return AsVfsFile(wrapper_file)->wrapped_file;
}

int
Close(sqlite3_file* sqlite_file)
{
...
int
Read(sqlite3_file* sqlite_file, void* buf, int amt, sqlite3_int64 ofs)
{
...
int
Write(sqlite3_file* sqlite_file, const void* buf, int amt, sqlite3_int64 ofs)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
int
Truncate(sqlite3_file* sqlite_file, sqlite3_int64 size)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
int
Sync(sqlite3_file* sqlite_file, int flags)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
int
FileSize(sqlite3_file* sqlite_file, sqlite3_int64* size)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
int
Lock(sqlite3_file* sqlite_file, int file_lock)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
int
Unlock(sqlite3_file* sqlite_file, int file_lock)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
int
CheckReservedLock(sqlite3_file* sqlite_file, int* result)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
int
FileControl(sqlite3_file* sqlite_file, int op, void* arg)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
int
SectorSize(sqlite3_file* sqlite_file)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
int
DeviceCharacteristics(sqlite3_file* sqlite_file)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
int
ShmMap(sqlite3_file* sqlite_file, int region, int size, int extend, void volatile** pp)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
    return wrapped_file->pMethods->xShmMap(wrapped_file, region, size, extend, pp);
}

int
ShmLock(sqlite3_file* sqlite_file, int ofst, int n, int flags)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
void
ShmBarrier(sqlite3_file* sqlite_file)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
int
ShmUnmap(sqlite3_file* sqlite_file, int del)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
int
Fetch(sqlite3_file* sqlite_file, sqlite3_int64 off, int amt, void** pp)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
...
int
Unfetch(sqlite3_file* sqlite_file, sqlite3_int64 off, void* p)
{
    sqlite3_file* wrapped_file = GetWrappedFile(sqlite_file);
     ...
     int* used_flags)
{
    ...
    sqlite3_file* wrapped_file = static_cast<sqlite3_file*>(sqlite3_malloc(wrapped_vfs->szOsFile));
    if (!wrapped_file)
        return SQLITE_NOMEM;
    ...
    int rc = wrapped_vfs->xOpen(wrapped_vfs, file_name, wrapped_file, desired_flags, used_flags);
    if (rc != SQLITE_OK)
    {
    ...
    static int kJournalFlags = SQLITE_OPEN_MAIN_JOURNAL | SQLITE_OPEN_TEMP_JOURNAL
    ...
    if (file_name && (desired_flags & kJournalFlags))
    {
        ...
        if (dash_index != base::StringPiece::npos)
        {
            ...
            {
                base::mac::SetFileBackupExclusion(base::FilePath(file_name_string_piece));
            }
        }
    }
#endif
...
int
Delete(sqlite3_vfs* vfs, const char* file_name, int sync_dir)
{
    sqlite3_vfs* wrapped_vfs = GetWrappedVfs(vfs);
...
int
Access(sqlite3_vfs* vfs, const char* file_name, int flag, int* res)
{
    sqlite3_vfs* wrapped_vfs = GetWrappedVfs(vfs);
...
int
FullPathname(sqlite3_vfs* vfs, const char* relative_path, int buf_size, char* absolute_path)
{
    sqlite3_vfs* wrapped_vfs = GetWrappedVfs(vfs);
    return wrapped_vfs->xFullPathname(wrapped_vfs, relative_path, buf_size, absolute_path);
}

int
Randomness(sqlite3_vfs* vfs, int buf_size, char* buffer)
{
    sqlite3_vfs* wrapped_vfs = GetWrappedVfs(vfs);
...
int
Sleep(sqlite3_vfs* vfs, int microseconds)
{
    sqlite3_vfs* wrapped_vfs = GetWrappedVfs(vfs);
...
int
GetLastError(sqlite3_vfs* vfs, int e, char* s)
{
    sqlite3_vfs* wrapped_vfs = GetWrappedVfs(vfs);
...
int
CurrentTimeInt64(sqlite3_vfs* vfs, sqlite3_int64* now)
{
    sqlite3_vfs* wrapped_vfs = GetWrappedVfs(vfs);
