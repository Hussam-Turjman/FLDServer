...
public:
    ...
    // transaction.
    static void
    RazeIfDeprecated(Database* db, int deprecated_version);
    ...
    static bool
    GetMmapStatus(Database* db, int64_t* status);
    static bool
    SetMmapStatus(Database* db, int64_t status);
    ...
    bool
    Init(Database* db, int version, int compatible_version);
    ...
    void
    SetCompatibleVersionNumber(int version);
    int
    GetCompatibleVersionNumber();

    // Set the given arbitrary key with the given data. Returns true on success.
    bool
    SetValue(const char* key, const std::string& value);
    bool
    SetValue(const char* key, int value);
    bool
    SetValue(const char* key, int64_t value);
    ...
    bool
    GetValue(const char* key, std::string* value);
    bool
    GetValue(const char* key, int* value);
    bool
    GetValue(const char* key, int64_t* value);
...
private:
    ...
    // MetaTableHelper.
    void
    PrepareSetStatement(Statement* statement, const char* key);
    bool
    PrepareGetStatement(Statement* statement, const char* key);
