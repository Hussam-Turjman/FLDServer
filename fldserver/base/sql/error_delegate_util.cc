...
#include "fldserver/base/sql/error_delegate_util.h"
...
std::string
GetCorruptFileDiagnosticsInfo(const base::FilePath& corrupted_file_path)
{
    std::string corrupted_file_info("Corrupted file: ");
    corrupted_file_info += corrupted_file_path.DirName().BaseName().AsUTF8Unsafe() + "/"
