...
#include "fldserver/base/format_macros.h"
...
#include "fldserver/base/sql/initialization.h"
#include "fldserver/base/sql/meta_table.h"
#include "fldserver/base/sql/sql_features.h"
#include "fldserver/base/sql/statement.h"
#include "fldserver/base/sql/vfs_wrapper.h"
#include "sqlite3.h"
...
public:
    ...
    int
    SetTimeout(base::TimeDelta timeout)
    {
        DCHECK_LT(timeout.InMilliseconds(), INT_MAX);
        return sqlite3_busy_timeout(db_, static_cast<int>(timeout.InMilliseconds()));
    }
...
int
BackupDatabase(sqlite3* src, sqlite3* dst, const char* db_name)
{
    DCHECK_NE(src, dst);
...
bool
ValidAttachmentPoint(const char* attachment_point)
{
    for (size_t i = 0; attachment_point[i]; ++i)
    {
        if (!(base::IsAsciiDigit(attachment_point[i]) || base::IsAsciiAlpha(attachment_point[i])
...
int
GetSqlite3File(sqlite3* db, sqlite3_file** file)
{
    *file = nullptr;
...
int
GetSqlite3FileAndSize(sqlite3* db, sqlite3_file** file, sqlite3_int64* db_size)
{
...
std::string
AsUTF8ForSQL(const base::FilePath& path)
{
#if IS_WIN
...
// static
bool
Database::IsExpectedSqliteError(int error)
{
...
// static
void
Database::SetErrorExpecter(Database::ErrorExpecterCallback* cb)
{
...
// static
base::FilePath
Database::JournalPath(const base::FilePath& db_path)
{
...
// static
base::FilePath
Database::WriteAheadLogPath(const base::FilePath& db_path)
{
...
// static
base::FilePath
Database::SharedMemoryFilePath(const base::FilePath& db_path)
{
    return base::FilePath(db_path.value() + FILE_PATH_LITERAL("-shm"));
}

Database::StatementRef::StatementRef(Database* database, sqlite3_stmt* stmt, bool was_valid) :
...
void
Database::StatementRef::Close(bool forced)
{
    if (stmt_)
    {
    ...
    options_(options), mmap_disabled_(!enable_mmap_by_default_)
{
    ...
    DCHECK(!(options.page_size & (options.page_size - 1))) << "page_size must be a power of two";
}
...
void
Database::DisableMmapByDefault()
{
    enable_mmap_by_default_ = false;
}

void
Database::RecordEvent(Events event, size_t count)
{
...
bool
Database::Open(const base::FilePath& path)
{
...
void
Database::CloseInternal(bool forced)
{
...
// large transactions.
void
Database::ReleaseCacheMemoryIfNeeded(bool implicit_change_performed)
{
...
std::string
Database::CollectErrorInfo(int error, Statement* stmt) const
{
...
// from posix.
    ...
    if (error == SQLITE_ERROR)
    {
        ...
        // |name| but no |sql|,
        static const char kSchemaSql[] = "SELECT COALESCE(sql,name) FROM sqlite_master";
        rc = sqlite3_prepare_v3(db_,
                                kSchemaSql,
                                sizeof(kSchemaSql),
                                SQLITE_PREPARE_NO_VTAB,
                                &sqlite_statement,
...
std::string
Database::CollectCorruptionInfo()
{
...
bool
Database::GetMmapAltStatus(int64_t* status)
{
...
bool
Database::SetMmapAltStatus(int64_t status)
{
    if (!BeginTransaction())
        return false;
    ...
    const std::string create_view_sql =
            base::StringPrintf("CREATE VIEW MmapStatus (value) AS SELECT %" PRId64, status);
...
size_t
Database::GetAppropriateMmapSize()
{
    base::Optional<base::ScopedBlockingCall> scoped_blocking_call;
    InitScopedBlockingCall(FROM_HERE, &scoped_blocking_call);
...
Database::Raze()
{
    ...
    const std::string page_size_sql = base::StringPrintf("PRAGMA page_size=%d", options_.page_size);
...
// static
bool
Database::Delete(const base::FilePath& path)
{
    ...
    CHECK(strncmp(vfs->zName, "unix", 4) == 0 || strncmp(vfs->zName, "win32", 5) == 0
...
void
Database::RollbackAllTransactions()
{
    if (transaction_nesting_ > 0)
    {
...
bool
Database::AttachDatabase(const base::FilePath& other_db_path,
...
bool
Database::DetachDatabase(const char* attachment_point, InternalApiToken)
{
    DCHECK(ValidAttachmentPoint(attachment_point));
...
int
Database::ExecuteAndReturnErrorCode(const char* sql)
{
    DCHECK(sql);
    ...
    while ((rc == SQLITE_OK) && *sql)
    {
        ...
        rc = sqlite3_prepare_v3(
                db_, sql, /* nByte= */ -1, /* prepFlags= */ 0, &sqlite_statement, &leftover_sql);
...
Database::Execute(const char* sql)
{
    ...
    DCHECK_NE(error, SQLITE_ERROR) << "SQL Error in " << sql << ", " << GetErrorMessage();
    return error == SQLITE_OK;
}

bool
Database::ExecuteWithTimeout(const char* sql, base::TimeDelta timeout)
{
    if (!db_)
    {
        DCHECK(poisoned_) << "Illegal use of Database without a db";
...
scoped_refptr<Database::StatementRef>
Database::GetCachedStatement(StatementID id, const char* sql)
{
...
scoped_refptr<Database::StatementRef>
Database::GetUniqueStatement(const char* sql)
{
    return GetStatementImpl(this, sql);
}

scoped_refptr<Database::StatementRef>
Database::GetStatementImpl(sql::Database* tracking_db, const char* sql) const
{
    DCHECK(sql);
    ...
    return base::MakeRefCounted<StatementRef>(tracking_db, sqlite_statement, true);
}

scoped_refptr<Database::StatementRef>
Database::GetUntrackedStatement(const char* sql) const
{
    return GetStatementImpl(nullptr, sql);
}

std::string
Database::GetSchema() const
{
    // The ORDER BY should not be necessary, but relying on organic
    // order for something like this is questionable.
    static const char kSql[] = "SELECT type, name, tbl_name, sql "
...
bool
Database::IsSQLValid(const char* sql)
{
    base::Optional<base::ScopedBlockingCall> scoped_blocking_call;
    InitScopedBlockingCall(FROM_HERE, &scoped_blocking_call);
    if (!db_)
...
bool
Database::DoesIndexExist(const char* index_name) const
{
    return DoesSchemaItemExist(index_name, "index");
}

bool
Database::DoesTableExist(const char* table_name) const
{
    return DoesSchemaItemExist(table_name, "table");
}

bool
Database::DoesViewExist(const char* view_name) const
{
    return DoesSchemaItemExist(view_name, "view");
}

bool
Database::DoesSchemaItemExist(const char* name, const char* type) const
{
    static const char kSql[] = "SELECT 1 FROM sqlite_master WHERE type=? AND name=?";
    Statement statement(GetUntrackedStatement(kSql));
...
bool
Database::DoesColumnExist(const char* table_name, const char* column_name) const
{
    ...
    int error = sqlite3_table_column_metadata(db_,
                                              "main",
                                              table_name,
                                              column_name,
                                              /* pzDataType= */ nullptr,
...
int64_t
Database::GetLastInsertRowId() const
{
    if (!db_)
    {
        DCHECK(poisoned_) << "Illegal use of Database without a db";
...
int
Database::GetLastChangeCount() const
{
    if (!db_)
    {
        DCHECK(poisoned_) << "Illegal use of Database without a db";
...
Database::GetMemoryUsage()
{
    ...
    int error = sqlite3_db_status(db_,
                                  SQLITE_DBSTATUS_CACHE_USED,
                                  &cache_memory,
                                  &highwater_should_always_be_zero,
                                  /*resetFlg=*/false);
    DCHECK_EQ(error, SQLITE_OK);

    error = sqlite3_db_status(db_,
...
const char*
Database::GetErrorMessage() const
{
    if (!db_)
...
bool
Database::OpenInternal(const std::string& file_name, Database::Retry retry_flag)
{
    if (db_)
    {
    ...
    {
        stats_histogram_ =
                base::LinearHistogram::FactoryGet("Sqlite.Stats2." + histogram_tag_,
                                                  1,
                                                  EVENT_MAX_VALUE,
                                                  EVENT_MAX_VALUE + 1,
    ...
    // isolation.
    int err = sqlite3_open_v2(file_name.c_str(),
                              &db_,
    ...
    const base::TimeDelta kBusyTimeout = base::TimeDelta::FromSeconds(kBusyTimeoutSeconds);
    ...
    // static_assert(SQLITE_SECURE_DELETE == 1,"Chrome assumes secure_delete is on by default.");
    ...
    std::string mmap_sql = base::StringPrintf("PRAGMA mmap_size=%" PRIuS, mmap_size);
...
void
Database::StatementRefCreated(StatementRef* ref)
{
    DCHECK(!open_statements_.count(ref)) << __func__ << " already called with this statement";
    open_statements_.insert(ref);
}

void
Database::StatementRefDeleted(StatementRef* ref)
{
    DCHECK(open_statements_.count(ref)) << __func__ << " called with non-existing statement";
    open_statements_.erase(ref);
}

void
Database::set_histogram_tag(const std::string& tag)
{
    DCHECK(!is_open());
...
void
Database::AddTaggedHistogram(const std::string& name, int sample) const
{
    if (histogram_tag_.empty())
        return;
...
int
Database::OnSqliteError(int err, sql::Statement* stmt, const char* sql) const
{
...
bool
Database::FullIntegrityCheck(std::vector<std::string>* messages)
{
    return IntegrityCheckHelper("PRAGMA integrity_check", messages);
}

bool
Database::QuickIntegrityCheck()
{
    std::vector<std::string> messages;
...
std::string
Database::GetDiagnosticInfo(int extended_error, Statement* statement)
{
    ...
    std::string result = (error == SQLITE_CORRUPT) ? CollectCorruptionInfo()
    ...
    // CollectErrorInfo().
    const bool has_valid_header = (ExecuteAndReturnErrorCode("PRAGMA auto_vacuum") == SQLITE_OK);
    const bool select_sqlite_master_result =
            (ExecuteAndReturnErrorCode("SELECT COUNT(*) FROM sqlite_master") == SQLITE_OK);
...
bool
Database::IntegrityCheckHelper(const char* pragma_sql, std::vector<std::string>* messages)
{
    messages->clear();
...
Database::CheckpointDatabase()
{
    ...
    int rc = sqlite3_wal_checkpoint_v2(db_,
                                       kMainDb,
