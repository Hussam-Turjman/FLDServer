...
#include "fldserver/base/sql/internal_api_token.h"
...
struct COMPONENT_EXPORT(SQL) DatabaseOptions
{
    ...
    bool wal_mode = base::FeatureList::IsEnabled(sql::features::kEnableWALModeByDefault);
...
public:
    ...
    void
    set_error_callback(const ErrorCallback& callback)
    {
        error_callback_ = callback;
    }
    bool
    has_error_callback() const
    {
    ...
    // before Open().
    void
    set_histogram_tag(const std::string& tag);

    // Record a sparse UMA histogram sample under
    // |name|+"."+|histogram_tag_|.  If |histogram_tag_| is empty, no
    // histogram is recorded.
    void
    AddTaggedHistogram(const std::string& name, int sample) const;
    ...
    void
    RecordEvent(Events event, size_t count);
    void
    RecordOneEvent(Events event)
    {
        RecordEvent(event, 1);
    }
    ...
    bool
    FullIntegrityCheck(std::vector<std::string>* messages);
    ...
    bool
    Open(const base::FilePath& path) WARN_UNUSED_RESULT;
    ...
    bool
    OpenInMemory() WARN_UNUSED_RESULT;
    ...
    bool
    OpenTemporary() WARN_UNUSED_RESULT;
    ...
    static bool
    Delete(const base::FilePath& path);
    ...
    bool
    AttachDatabase(const base::FilePath& other_db_path,
                   const char* attachment_point,
                   InternalApiToken);
    bool
    DetachDatabase(const char* attachment_point, InternalApiToken);
    ...
    bool
    Execute(const char* sql) WARN_UNUSED_RESULT;

    // Like Execute(), but returns the error code given by SQLite.
    int
    ExecuteAndReturnErrorCode(const char* sql) WARN_UNUSED_RESULT;
    ...
    scoped_refptr<StatementRef>
    GetCachedStatement(StatementID id, const char* sql);

    // Used to check a |sql| statement for syntactic validity. If the statement is
    // valid SQL, returns true.
    bool
    IsSQLValid(const char* sql);
    ...
    // CREATE statement.
    bool
    DoesIndexExist(const char* index_name) const;
    bool
    DoesTableExist(const char* table_name) const;
    bool
    DoesViewExist(const char* table_name) const;
    ...
    // current version.
    bool
    DoesColumnExist(const char* table_name, const char* column_name) const;
    ...
    static bool
    IsExpectedSqliteError(int error);
    ...
    JournalPath(const base::FilePath& db_path);
    ...
    // the existence of specific files.
    static base::FilePath
    WriteAheadLogPath(const base::FilePath& db_path);
    ...
    // the existence of specific files.
    static base::FilePath
    SharedMemoryFilePath(const base::FilePath& db_path);
    ...
    bool poisoned(InternalApiToken) const
    {
        return poisoned_;
    }
...
private:
    ...
    */
    ...
    void
    InitScopedBlockingCall(const base::Location& from_here,
    ...
    bool
    DoesSchemaItemExist(const char* name, const char* type) const;
    ...
    static void
    SetErrorExpecter(ErrorExpecterCallback* expecter);
    static void
    ResetErrorExpecter();
    ...
    class COMPONENT_EXPORT(SQL) StatementRef : public base::RefCounted<StatementRef>
    {
    public:
    ...
    void
    StatementRefCreated(StatementRef* ref);
    void
    StatementRefDeleted(StatementRef* ref);
    ...
    int
    OnSqliteError(int err, Statement* stmt, const char* sql) const;

    // Like |Execute()|, but retries if the database is locked.
    bool
    ExecuteWithTimeout(const char* sql, base::TimeDelta ms_timeout) WARN_UNUSED_RESULT;
    ...
    // not track.
    scoped_refptr<StatementRef>
    GetStatementImpl(sql::Database* tracking_db, const char* sql) const;
    ...
    scoped_refptr<StatementRef>
    GetUntrackedStatement(const char* sql) const;

    bool
    IntegrityCheckHelper(const char* pragma_sql,
    ...
    void
    ReleaseCacheMemoryIfNeeded(bool implicit_change_performed);
    ...
    std::string
    CollectErrorInfo(int error, Statement* stmt) const;
    ...
    size_t
    GetAppropriateMmapSize();

    // Helpers for GetAppropriateMmapSize().
    bool
    GetMmapAltStatus(int64_t* status);
    bool
    SetMmapAltStatus(int64_t status);
