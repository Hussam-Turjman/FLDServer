...
void
RecordDeprecationEvent(DeprecationEventType deprecation_event)
{
    UMA_HISTOGRAM_ENUMERATION(
            "Sqlite.DeprecationVersionResult", deprecation_event, DEPRECATION_EVENT_MAX);
}
...
// static
bool
MetaTable::DoesTableExist(sql::Database* db)
{
    DCHECK(db);
...
// static
bool
MetaTable::GetMmapStatus(Database* db, int64_t* status)
{
...
// static
bool
MetaTable::SetMmapStatus(Database* db, int64_t status)
{
...
// static
void
MetaTable::RazeIfDeprecated(Database* db, int deprecated_version)
{
    ...
    sql::Statement s(db->GetUniqueStatement("SELECT value FROM meta WHERE key=?"));
...
bool
MetaTable::Init(Database* db, int version, int compatible_version)
{
    DCHECK(!db_ && db);
    db_ = db;
...
void
MetaTable::SetVersionNumber(int version)
{
    DCHECK_GT(version, 0);
...
void
MetaTable::SetCompatibleVersionNumber(int version)
{
    DCHECK_GT(version, 0);
...
int
MetaTable::GetCompatibleVersionNumber()
{
    int version = 0;
...
bool
MetaTable::SetValue(const char* key, const std::string& value)
{
    Statement s;
...
bool
MetaTable::SetValue(const char* key, int value)
{
    Statement s;
    PrepareSetStatement(&s, key);
    s.BindInt(1, value);
...
bool
MetaTable::SetValue(const char* key, int64_t value)
{
    Statement s;
    PrepareSetStatement(&s, key);
    s.BindInt64(1, value);
...
bool
MetaTable::GetValue(const char* key, std::string* value)
{
    Statement s;
...
bool
MetaTable::GetValue(const char* key, int* value)
{
    Statement s;
    if (!PrepareGetStatement(&s, key))
        return false;
...
bool
MetaTable::GetValue(const char* key, int64_t* value)
{
    Statement s;
    if (!PrepareGetStatement(&s, key))
        return false;
...
bool
MetaTable::DeleteKey(const char* key)
{
    DCHECK(db_);
...
void
MetaTable::PrepareSetStatement(Statement* statement, const char* key)
{
...
bool
MetaTable::PrepareGetStatement(Statement* statement, const char* key)
{
    DCHECK(db_ && statement);
    statement->Assign(db_->GetCachedStatement(SQL_FROM_HERE, "SELECT value FROM meta WHERE key=?"));
    statement->BindCString(0, key);
