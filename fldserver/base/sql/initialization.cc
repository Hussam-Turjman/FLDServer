...
#if !defined(OS_IOS)
void
RecordSqliteMemory10Min()
{
    const int32_t used = base::saturated_cast<int32_t>(sqlite3_memory_used() / 1024);
    UMA_HISTOGRAM_COUNTS_1M("Sqlite.MemoryKB.TenMinutes", used);
}

void
RecordSqliteMemoryHour()
{
    const int32_t used = base::saturated_cast<int32_t>(sqlite3_memory_used() / 1024);
    ...
    if (first_call)
    {
        ...
        if (base::SequencedTaskRunnerHandle::IsSet())
        {
            base::SequencedTaskRunnerHandle::Get()->PostDelayedTask(
                    FROM_HERE,
                    base::BindOnce(&RecordSqliteMemory10Min),
                    base::TimeDelta::FromMinutes(10));
            base::SequencedTaskRunnerHandle::Get()->PostDelayedTask(
                    FROM_HERE,
                    base::BindOnce(&RecordSqliteMemoryHour),
                    base::TimeDelta::FromHours(1));
            base::SequencedTaskRunnerHandle::Get()->PostDelayedTask(
                    FROM_HERE,
                    base::BindOnce(&RecordSqliteMemoryDay),
                    base::TimeDelta::FromDays(1));
            base::SequencedTaskRunnerHandle::Get()->PostDelayedTask(
                    FROM_HERE,
                    base::BindOnce(&RecordSqliteMemoryWeek),
