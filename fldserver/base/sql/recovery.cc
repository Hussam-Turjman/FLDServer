...
#include "fldserver/base/sql/recovery.h"
...
#include "fldserver/base/sql/recover_module/module.h"
...
void
RecordRecoveryEvent(RecoveryEventType recovery_event)
{
    UMA_HISTOGRAM_ENUMERATION("Sqlite.RecoveryEvents", recovery_event, RECOVERY_EVENT_MAX);
}
...
Recovery::Begin(Database* database, const base::FilePath& db_path)
{
    ...
    if (!database->is_open())
    {
        ...
        DCHECK(database->poisoned(InternalApiToken())) << "Illegal to recover with closed Database";
        return nullptr;
    }
...
// static
bool
Recovery::Recovered(std::unique_ptr<Recovery> r)
{
    return r->Backup();
}
...
// static
void
Recovery::Unrecoverable(std::unique_ptr<Recovery> r)
{
    CHECK(r->db_);
...
// static
void
Recovery::Rollback(std::unique_ptr<Recovery> r)
{
...
Recovery::Recovery(Database* connection) :
    db_(connection), recover_db_({.exclusive_locking = false, .page_size = db_->page_size()})
{
...
bool
Recovery::Init(const base::FilePath& db_path)
{
    ...
    if (rc != SQLITE_OK)
    {
        ...
        LOG(ERROR) << "Failed to initialize recover module: " << recover_db_.GetErrorMessage();
        return false;
    }
    ...
    if (!recover_db_.AttachDatabase(db_path, "corrupt", InternalApiToken()))
    {
        ...
        base::UmaHistogramSparse("Sqlite.RecoveryAttachError", recover_db_.GetErrorCode());
        return false;
    }
...
Recovery::Backup()
{
    ...
    sqlite3_backup* backup = sqlite3_backup_init(
            db_->db(InternalApiToken()), kMain, recover_db_.db(InternalApiToken()), kMain);
    if (!backup)
    {
...
void
Recovery::Shutdown(Recovery::Disposition raze)
{
    if (!db_)
        return;
...
bool
Recovery::AutoRecoverTable(const char* table_name, size_t* rows_recovered)
{
    // Query the info for the recovered table in database [main].
    std::string query(base::StringPrintf("PRAGMA main.table_info(%s)", table_name));
    ...
    std::string recover_create(
            base::StringPrintf("CREATE VIRTUAL TABLE temp.recover_%s USING recover(corrupt.%s, %s)",
                               table_name,
                               table_name,
    ...
    std::string recover_insert(base::StringPrintf("INSERT OR IGNORE INTO main.%s SELECT %s FROM "
                                                  "temp.recover_%s",
                                                  table_name,
                                                  base::JoinString(insert_columns, ",").c_str(),
                                                  table_name));

    std::string recover_drop(base::StringPrintf("DROP TABLE temp.recover_%s", table_name));
...
Recovery::SetupMeta()
{
    static const char kCreateSql[] = "CREATE VIRTUAL TABLE temp.recover_meta USING recover"
                                     "("
                                     "corrupt.meta,"
                                     "key TEXT NOT NULL,"
...
bool
Recovery::GetMetaVersionNumber(int* version)
{
    DCHECK(version);
    ...
    static const char kVersionSql[] = "SELECT value FROM temp.recover_meta WHERE key = 'version'";
...
bool
SchemaCopyHelper(Database* db, const char* prefix)
{
    const size_t prefix_len = strlen(prefix);
    DCHECK_EQ(' ', prefix[prefix_len - 1]);

    sql::Statement s(db->GetUniqueStatement("SELECT DISTINCT sql FROM corrupt.sqlite_master "
...
// static
std::unique_ptr<Recovery>
Recovery::BeginRecoverDatabase(Database* db, const base::FilePath& db_path)
{
    ...
    if (db->is_open())
    {
        sql::Statement s(db->GetUniqueStatement("SELECT 1 FROM sqlite_master WHERE sql LIKE "
    ...
    {
        sql::Statement s(recovery->db()->GetUniqueStatement("SELECT name FROM sqlite_master WHERE "
    ...
    static const char kCreateMetaItemsSql[] = "INSERT INTO main.sqlite_master "
                                              "SELECT type, name, tbl_name, rootpage, sql "
                                              "FROM corrupt.sqlite_master WHERE type='view' OR "
                                              "type='trigger'";
...
void
Recovery::RecoverDatabase(Database* db, const base::FilePath& db_path)
{
...
void
Recovery::RecoverDatabaseWithMetaVersion(Database* db, const base::FilePath& db_path)
{
    std::unique_ptr<sql::Recovery> recovery = BeginRecoverDatabase(db, db_path);
    if (!recovery)
        return;
...
// static
bool
Recovery::ShouldRecover(int extended_error)
{
...
// static
int
Recovery::EnableRecoveryExtension(Database* db, InternalApiToken)
{
