...
#if DCHECK_IS_ON()
const char*
AdditionalHintForSystemErrorCode(int error_code)
{
    switch (error_code)
    {
        case EINVAL: return "Hint: This is often related to a use-after-free.";
        default: return "";
    }
}
#endif  // DCHECK_IS_ON()
...
{
#if DCHECK_IS_ON()
    return base::safe_strerror(error_code) + ". " + AdditionalHintForSystemErrorCode(error_code);
#else   // DCHECK_IS_ON()
    return std::string();
#endif  // DCHECK_IS_ON()
}
...
void
LockImpl::LockInternalWithTracking()
{
...
// static
bool
LockImpl::PriorityInheritanceAvailable()
{
#if ENABLE_MUTEX_PRIORITY_INHERITANCE
    return true;
#elif PRIORITY_INHERITANCE_LOCKS_POSSIBLE() && IS_APPLE
    return true;
#else
