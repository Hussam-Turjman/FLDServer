...
class LOCKABLE CORE_EXPORT Lock
{
public:
#if !DCHECK_IS_ON()
    ...
    void
    Acquire() EXCLUSIVE_LOCK_FUNCTION()
    {
        lock_.Lock();
    }
    void
    Release() UNLOCK_FUNCTION()
    {
        lock_.Unlock();
    }
    ...
    // assertion may fail).
    bool
    Try() EXCLUSIVE_TRYLOCK_FUNCTION(true)
    {
        return lock_.Try();
    }
    ...
    void
    AssertAcquired() const ASSERT_EXCLUSIVE_LOCK()
    {
    }
#else
    Lock();
