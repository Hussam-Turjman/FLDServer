...
{
public:
    SyncWaiter() : fired_(false), signaling_event_(nullptr), lock_(), cv_(&lock_)
    {
    }

    bool
    Fire(WaitableEvent* signaling_event) override
    {
...
bool
WaitableEvent::TimedWait(const TimeDelta& wait_delta)
{
    ...
    Optional<internal::ScopedBlockingCallWithBaseSyncPrimitives> scoped_blocking_call;
    ...
    const TimeTicks end_time = wait_delta.is_max()
                                       ? TimeTicks::Max()
...
size_t
WaitableEvent::WaitMany(WaitableEvent** raw_waitables, size_t count) NO_THREAD_SAFETY_ANALYSIS
{
...
// NO_THREAD_SAFETY_ANALYSIS: Complex control flow.
size_t
WaitableEvent::EnqueueMany(std::pair<WaitableEvent*, size_t>* waitables,
                           size_t count,
...
WaitableEvent::WaitableEventKernel::WaitableEventKernel(ResetPolicy reset_policy,
...
// -----------------------------------------------------------------------------
void
WaitableEvent::Enqueue(Waiter* waiter)
{
...
// -----------------------------------------------------------------------------
bool
WaitableEvent::WaitableEventKernel::Dequeue(Waiter* waiter, void* tag)
{
