...
public:
    AsyncWaiter(scoped_refptr<SequencedTaskRunner> task_runner,
                ...
                Flag* flag) :
        task_runner_(std::move(task_runner)), callback_(std::move(callback)), flag_(flag)
    {
    }

    bool
    Fire(WaitableEvent* event) override
    {
...
// -----------------------------------------------------------------------------
void
AsyncCallbackHelper(Flag* flag, WaitableEventWatcher::EventCallback callback, WaitableEvent* event)
{
...
// -----------------------------------------------------------------------------
bool
WaitableEventWatcher::StartWatching(WaitableEvent* event,
                                    ...
                                    scoped_refptr<SequencedTaskRunner> task_runner)
{
    ...
    OnceClosure internal_callback = base::BindOnce(
            &AsyncCallbackHelper, base::RetainedRef(cancel_flag_), std::move(callback), event);
    ...
    waiter_ = new AsyncWaiter(
            std::move(task_runner), std::move(internal_callback), cancel_flag_.get());
