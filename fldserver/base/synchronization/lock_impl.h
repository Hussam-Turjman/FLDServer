...
LockImpl::Try()
{
    return !!::TryAcquireSRWLockExclusive(reinterpret_cast<PSRWLOCK>(&native_handle_));
}
...
public:
    ...
    BasicAutoLock(LockType& lock, const AlreadyAcquired&) EXCLUSIVE_LOCKS_REQUIRED(lock) :
        lock_(lock)
    {
