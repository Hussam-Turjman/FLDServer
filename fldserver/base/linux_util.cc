...
#include "fldserver/base/linux_util.h"
...
#include "fldserver/base/strings/safe_sprintf.h"
...
#if !0
std::string
GetKeyValueFromOSReleaseFile(const std::string& input, const char* key)
{
...
bool
ReadDistroFromOSReleaseFile(const char* file)
{
    ...
    std::string pretty_name = GetKeyValueFromOSReleaseFile(os_release_content, kPrettyName);
...
public:
    DistroNameGetter()
    {
        static const char* const kFilesToCheck[] = {"/etc/os-release", "/usr/lib/os-release"};
...
char g_linux_distro[kDistroSize] =
        ...
        // "CORE_EXPORT" here again so that they are visible to tests.
        CORE_EXPORT std::string GetKeyValueFromOSReleaseFileForTesting(const std::string& input,
                                                                       const char* key)
{
#if !0
...
void
SetLinuxDistro(const std::string& distro)
{
...
bool
GetThreadsForProcess(pid_t pid, std::vector<pid_t>* tids)
{
...
pid_t
FindThreadIDWithSyscall(pid_t pid, const std::string& expected_data, bool* syscall_supported)
{
    ...
    for (pid_t tid : tids)
    {
        ...
        *syscall_supported = true;
        ...
        if (0 == strncmp(expected_data.c_str(), syscall_data.data(), expected_data.size()))
        {
            return tid;
        }
    }
...
pid_t
FindThreadID(pid_t pid, pid_t ns_tid, bool* ns_pid_supported)
{
        ...
        while (tokenizer.GetNext())
        {
            ...
            *ns_pid_supported = true;
            std::vector<StringPiece> split_value_str =
                    SplitStringPiece(value_str, "\t", TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
