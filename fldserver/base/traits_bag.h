...
#include "fldserver/base/parameter_pack.h"
...
{
    template <typename T,
              std::enable_if_t<ParameterPack<TraitsToExclude...>::template HasType<T>::value>* =
                      nullptr>
    static constexpr EmptyTrait
    Filter(T t)
    {
        return EmptyTrait();
    }
...
template <class TraitFilterType,
          class ArgType,
          class CheckArgumentIsCompatible =
                  std::enable_if_t<std::is_constructible<TraitFilterType, ArgType>::value>>
constexpr TraitFilterType
GetTraitFromArg(CallFirstTag, ArgType arg)
{
    return TraitFilterType(arg);
}

template <class TraitFilterType, class ArgType>
constexpr InvalidTrait
GetTraitFromArg(CallSecondTag, ArgType arg)
{
    return InvalidTrait();
}
...
template <class TraitFilterType,
          ...
          class TestCompatibleArgument = std::enable_if_t<
                  any_of({std::is_constructible<TraitFilterType, ArgTypes>::value...})>>
constexpr TraitFilterType
GetTraitFromArgListImpl(CallFirstTag, ArgTypes... args)
{
    return std::get<TraitFilterType>(
            std::make_tuple(GetTraitFromArg<TraitFilterType>(CallFirstTag(), args)...));
}

template <class TraitFilterType, class... ArgTypes>
constexpr TraitFilterType
GetTraitFromArgListImpl(CallSecondTag, ArgTypes... args)
{
...
template <class TraitFilterType, class... ArgTypes>
constexpr typename TraitFilterType::ValueType
GetTraitFromArgList(ArgTypes... args)
{
    static_assert(count({std::is_constructible<TraitFilterType, ArgTypes>::value...}, true) <= 1,
...
struct OptionalEnumTraitFilter : public BasicTraitFilter<ArgType, Optional<ArgType>>
{
    constexpr OptionalEnumTraitFilter() : BasicTraitFilter<ArgType, Optional<ArgType>>(nullopt)
    {
    }
...
{
    constexpr RequiredEnumTraitFilter(ArgType arg) : BasicTraitFilter<ArgType>(arg)
    {
    }
};
...
using IsValidTrait =
        disjunction<std::is_constructible<ValidTraits, T>, std::is_same<T, EmptyTrait>>;
...
using AreValidTraits = bool_constant<all_of({IsValidTrait<ValidTraits, ArgTypes>::value...})>;
...
// readable.
template <typename Enum, typename... Args>
static constexpr Optional<Enum>
GetOptionalEnum(Args... args)
{
...
{
    static_assert(count({std::is_constructible<Trait, Args>::value...}, true) <= 1,
                  "The traits bag contains multiple traits of the same type.");
};
