...
{
CheckError
CheckError::Check(const char* file, int line, const char* condition)
{
...
CheckError
CheckError::CheckOp(const char* file, int line, CheckOpResult* check_op_result)
{
    CheckError check_error(new LogMessage(file, line, LOGGING_FATAL));
...
CheckError
CheckError::DCheck(const char* file, int line, const char* condition)
{
...
CheckError
CheckError::DCheckOp(const char* file, int line, CheckOpResult* check_op_result)
{
    CheckError check_error(new LogMessage(file, line, LOGGING_DCHECK));
    check_error.stream() << "Check failed: " << check_op_result->message_;
    free(check_op_result->message_);
...
CheckError
CheckError::PCheck(const char* file, int line, const char* condition)
{
...
CheckError
CheckError::PCheck(const char* file, int line)
{
    return PCheck(file, line, "");
}

CheckError
CheckError::DPCheck(const char* file, int line, const char* condition)
{
    SystemErrorCode err_code = logging::GetLastSystemErrorCode();
#if IS_WIN
...
CheckError
CheckError::NotImplemented(const char* file, int line, const char* function)
{
