...
template <typename T>
inline void
ReadBigEndian(const char buf[], T* out)
{
    ...
    // without UB.
    typename std::make_unsigned<T>::type unsigned_result = static_cast<uint8_t>(buf[0]);
...
template <>
inline void
ReadBigEndian<uint8_t>(const char buf[], uint8_t* out)
{
    *out = buf[0];
}
...
template <>
inline void
WriteBigEndian<uint8_t>(char buf[], uint8_t val)
{
...
template <>
inline void
ReadBigEndian<int8_t>(const char buf[], int8_t* out)
{
    *out = buf[0];
}
...
template <>
inline void
WriteBigEndian<int8_t>(char buf[], int8_t val)
{
    buf[0] = static_cast<char>(val);
}
...
class CORE_EXPORT BigEndianReader
{
public:
    ...
    Skip(size_t len);
    bool
    ReadBytes(void* out, size_t len);
    // Creates a StringPiece in |out| that points to the underlying buffer.
    bool
    ReadPiece(base::StringPiece* out, size_t len);
    bool
    ...
    // as before the call.
    bool
    ReadU8LengthPrefixed(base::StringPiece* out);
    bool
    ReadU16LengthPrefixed(base::StringPiece* out);
...
private:
    ...
    template <typename T>
    bool
    ReadLengthPrefixed(base::StringPiece* out);
...
class CORE_EXPORT BigEndianWriter
{
public:
    ...
    Skip(size_t len);
    bool
    WriteBytes(const void* buf, size_t len);
    bool
