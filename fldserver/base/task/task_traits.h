...
#include "fldserver/base/task/task_traits_extension.h"
#include "fldserver/base/traits_bag.h"
...
public:
    ...
    template <class... ArgTypes,
            ...
            typename std::enable_if<sizeof...(ArgTypes) >= 1, bool>::type = true>
    constexpr TaskTraits(ArgTypes... args) :
        extension_(trait_helpers::GetTaskTraitsExtension(
                trait_helpers::AreValidTraits<ValidTrait, ArgTypes...>{}, args...)),
        priority_(trait_helpers::GetEnum<TaskPriority, TaskPriority::USER_BLOCKING>(args...)),
        shutdown_behavior_(
                static_cast<uint8_t>(
        ...
        thread_policy_(
                static_cast<uint8_t>(
                        trait_helpers::GetEnum<ThreadPolicy, ThreadPolicy::PREFER_BACKGROUND>(
                                args...))
                | (trait_helpers::HasTrait<ThreadPolicy, ArgTypes...>() ? kIsExplicitFlag : 0)),
        may_block_(trait_helpers::HasTrait<MayBlock, ArgTypes...>()),
        with_base_sync_primitives_(trait_helpers::HasTrait<WithBaseSyncPrimitives, ArgTypes...>())
    {
    }
    constexpr TaskTraits():extension_(),
        priority_(TaskPriority::USER_BLOCKING),
        shutdown_behavior_(static_cast<uint8_t>(TaskShutdownBehavior::SKIP_ON_SHUTDOWN)),
    ...
    TaskTraits&
    operator=(const TaskTraits& other) = default;

    // TODO(eseckler): Default the comparison operator once C++20 arrives.
    bool
    operator==(const TaskTraits& other) const
    {
        ...
        return extension_ == other.extension_ && priority_ == other.priority_
               ...
               && thread_policy_ == other.thread_policy_ && may_block_ == other.may_block_
    ...
    constexpr bool
    with_base_sync_primitives() const
    {
...
private:
        ...
        use_thread_pool_(use_thread_pool)
    {
        ...
        const bool has_extension =
                (extension_.extension_id != TaskTraitsExtensionStorage::kInvalidExtensionId);
...
TaskPriorityToString(TaskPriority task_priority);
CORE_EXPORT const char*
TaskShutdownBehaviorToString(TaskShutdownBehavior task_priority);
...
operator<<(std::ostream& os, const TaskPriority& shutdown_behavior);
CORE_EXPORT std::ostream&
operator<<(std::ostream& os, const TaskShutdownBehavior& shutdown_behavior);
