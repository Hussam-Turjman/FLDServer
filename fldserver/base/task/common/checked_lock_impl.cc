...
#include "fldserver/base/task/common/checked_lock_impl.h"
...
public:
    ...
    void
    RegisterLock(const CheckedLockImpl* const lock, const CheckedLockImpl* const predecessor)
    {
    ...
    void
    UnregisterLock(const CheckedLockImpl* const lock)
    {
        AutoLock auto_lock(allowed_predecessor_map_lock_);
    ...
    void
    RecordAcquisition(const CheckedLockImpl* const lock)
    {
    ...
    void
    RecordRelease(const CheckedLockImpl* const lock)
    {
...
private:
    ...
    using PredecessorMap = std::unordered_map<const CheckedLockImpl*, const CheckedLockImpl*>;
    ...
    void
    AssertSafeAcquire(const CheckedLockImpl* const lock)
    {
        ...
        const CheckedLockImpl* allowed_predecessor = allowed_predecessor_map_.at(lock);
    ...
    // set.
    void
    AssertSafePredecessor(const CheckedLockImpl* lock) const
    {
        ...
        if (predecessor)
        {
            DCHECK(allowed_predecessor_map_.find(predecessor) != allowed_predecessor_map_.end())
    ...
    LockVector*
    GetAcquiredLocksOnCurrentThread()
    {
...
CheckedLockImpl::CheckedLockImpl(UniversalPredecessor) : is_universal_predecessor_(true)
{
}

CheckedLockImpl::CheckedLockImpl(UniversalSuccessor) : is_universal_successor_(true)
{
...
void
CheckedLockImpl::AssertNoLockHeldOnCurrentThread()
{
    g_safe_acquisition_tracker.Get().AssertNoLockHeldOnCurrentThread();
}

void
CheckedLockImpl::Acquire()
{
    lock_.Acquire();
...
void
CheckedLockImpl::AssertAcquired() const
{
    lock_.AssertAcquired();
}

std::unique_ptr<ConditionVariable>
CheckedLockImpl::CreateConditionVariable()
{
