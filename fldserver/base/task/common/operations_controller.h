...
class CORE_EXPORT OperationsController
{
public:
    ...
    OperationsController&
    operator=(const OperationsController&) = delete;
...
private:
    ...
    static constexpr uint32_t kFlagsBitMask = (kShuttingDownBitMask | kAcceptingOperationsBitMask);
    ...
    static uint32_t
    ExtractCount(uint32_t value)
    {
        return value & kCountBitMask;
    }
    static State
    ExtractState(uint32_t value);

    // Decrements the counter by |n| and signals |shutdown_complete_| if needed.
    void
    DecrementBy(uint32_t n);
