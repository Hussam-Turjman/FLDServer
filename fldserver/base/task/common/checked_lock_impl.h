...
class CORE_EXPORT CheckedLockImpl
{
public:
    CheckedLockImpl();
    ...
    void
    Acquire() EXCLUSIVE_LOCK_FUNCTION(lock_);
    void
    Release() UNLOCK_FUNCTION(lock_);

    void
    AssertAcquired() const;

    std::unique_ptr<ConditionVariable>
    CreateConditionVariable();

    bool
    is_universal_predecessor() const
    {
        return is_universal_predecessor_;
    }
    bool
    is_universal_successor() const
    {
        return is_universal_successor_;
    }
