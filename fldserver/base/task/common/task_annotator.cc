...
static constexpr int kSentinelSequenceNum = static_cast<int>(0xF00DBAADF00DBAAD);
...
ThreadLocalPointer<PendingTask>*
...
{
    static NoDestructor<ThreadLocalPointer<TaskAnnotator::ScopedSetIpcHash>> instance;
    return instance.get();
}
...
bool
IsDummyPendingTask(const PendingTask* task)
{
    if (task->sequence_num == kSentinelSequenceNum && !task->posted_from.has_source_info()
...
void
TaskAnnotator::WillQueueTask(const char* trace_event_name,
                             ...
                             const char* task_queue_name)
{
    ...
    DCHECK(!pending_task->task_backtrace[0]) << "Task backtrace was already set, task posted "
                                                "twice??";
    ...
    pending_task->task_backtrace_overflow = parent_task->task_backtrace_overflow
                                            || parent_task->task_backtrace.back() != nullptr;
}

void
TaskAnnotator::RunTask(const char* trace_event_name, PendingTask* pending_task)
{
    DCHECK(trace_event_name);
    DCHECK(pending_task);
    ...
    static constexpr int kStackTaskTraceSnapshotSize = PendingTask::kTaskBacktraceLength + 4;
...
uint64_t
TaskAnnotator::GetTaskTraceID(const PendingTask& task) const
{
    return (static_cast<uint64_t>(task.sequence_num) << 32)
           | ((static_cast<uint64_t>(reinterpret_cast<intptr_t>(this)) << 32) >> 32);
}

// static
void
TaskAnnotator::RegisterObserverForTesting(ObserverForTesting* observer)
{
...
// static
void
TaskAnnotator::ClearObserverForTesting()
{
...
TaskAnnotator::ScopedSetIpcHash::ScopedSetIpcHash(const char* ipc_interface_name) :
    ScopedSetIpcHash(0, ipc_interface_name)
{
}

TaskAnnotator::ScopedSetIpcHash::ScopedSetIpcHash(uint32_t ipc_hash, const char* ipc_interface_name)
{
...
// Static
uint32_t
TaskAnnotator::ScopedSetIpcHash::MD5HashMetricName(base::StringPiece name)
{
