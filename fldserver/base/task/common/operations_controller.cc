...
OperationsController::~OperationsController()
{
    ...
    DCHECK(ExtractState(value) == State::kRejectingOperations
...
bool
OperationsController::StartAcceptingOperations()
{
    ...
    auto prev_value =
            state_and_count_.fetch_or(kAcceptingOperationsBitMask, std::memory_order_release);
...
OperationsController::OperationToken
OperationsController::TryBeginOperation()
{
    ...
    {
        case State::kRejectingOperations: return OperationToken(nullptr);
        case State::kAcceptingOperations: return OperationToken(this);
        case State::kShuttingDown: DecrementBy(1); return OperationToken(nullptr);
    }
}

void
OperationsController::ShutdownAndWaitForZeroOperations()
{
    ...
    auto prev_value = state_and_count_.fetch_or(kShuttingDownBitMask, std::memory_order_acquire);
...
// static
OperationsController::State
OperationsController::ExtractState(uint32_t value)
{
...
void
OperationsController::DecrementBy(uint32_t n)
{
    ...
    if (ExtractState(prev_value) == State::kShuttingDown && ExtractCount(prev_value) == n)
    {
