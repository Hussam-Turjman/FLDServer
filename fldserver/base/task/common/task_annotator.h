...
// usage.
class CORE_EXPORT TaskAnnotator
{
public:
    ...
    public:
        ...
        // be executed.
        virtual void
        BeforeRunTask(const PendingTask* pending_task) = 0;
    };
    ...
    static const PendingTask*
    CurrentTaskForThread();
    ...
    void
    WillQueueTask(const char* trace_event_name,
                  PendingTask* pending_task,
                  const char* task_queue_name);

    // Run a previously queued task.
    void NOT_TAIL_CALLED
    RunTask(const char* trace_event_name, PendingTask* pending_task);
    ...
    uint64_t
    GetTaskTraceID(const PendingTask& task) const;
...
private:
    ...
    static void
    RegisterObserverForTesting(ObserverForTesting* observer);
    static void
    ClearObserverForTesting();

    DISALLOW_COPY_AND_ASSIGN(TaskAnnotator);
};

class CORE_EXPORT TaskAnnotator::ScopedSetIpcHash
{
public:
    ...
    const char*
    GetIpcInterfaceName() const
    {
        return ipc_interface_name_;
    }

    static uint32_t
    MD5HashMetricName(base::StringPiece name);
