...
namespace base
{
SimpleTaskExecutor::SimpleTaskExecutor(scoped_refptr<SingleThreadTaskRunner> task_queue) :
    task_queue_(std::move(task_queue)), previous_task_executor_(GetTaskExecutorForCurrentThread())
{
    DCHECK(task_queue_);
...
bool
SimpleTaskExecutor::PostDelayedTask(const Location& from_here,
...
SimpleTaskExecutor::CreateTaskRunner(const TaskTraits& traits)
{
    return task_queue_;
}
...
SimpleTaskExecutor::CreateSingleThreadTaskRunner(const TaskTraits& traits,
...
#if IS_WIN
scoped_refptr<SingleThreadTaskRunner>
SimpleTaskExecutor::CreateCOMSTATaskRunner(const TaskTraits& traits,
                                           SingleThreadTaskRunnerThreadMode thread_mode)
{
