...
#include "fldserver/base/post_task_and_reply_with_result_internal.h"
...
class CORE_EXPORT CancelableTaskTracker
{
public:
    ...
    TaskId
    PostTask(TaskRunner* task_runner, const Location& from_here, OnceClosure task);

    TaskId
    PostTaskAndReply(TaskRunner* task_runner,
    ...
    TaskId
    PostTaskAndReplyWithResult(TaskRunner* task_runner,
                               const Location& from_here,
    ...
    TaskId
    NewTrackedTaskId(IsCanceledCallback* is_canceled_cb);
...
private:
    ...
    static void
    RunIfNotCanceled(const scoped_refptr<SequencedTaskRunner>& origin_task_runner,
                     const scoped_refptr<TaskCancellationFlag>& flag,
                     OnceClosure task);
    static void
    RunThenUntrackIfNotCanceled(const scoped_refptr<SequencedTaskRunner>& origin_task_runner,
                                const scoped_refptr<TaskCancellationFlag>& flag,
                                OnceClosure task,
                                OnceClosure untrack);
    static bool
    IsCanceled(const scoped_refptr<SequencedTaskRunner>& origin_task_runner,
               const scoped_refptr<TaskCancellationFlag>& flag,
               const ScopedClosureRunner& cleanup_runner);

    void
    Track(TaskId id, scoped_refptr<TaskCancellationFlag> flag);
    void
    Untrack(TaskId id);
    ...
    // tasks.
    small_map<std::map<TaskId, scoped_refptr<TaskCancellationFlag>>, 4> task_flags_;
