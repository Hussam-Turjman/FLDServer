...
#include "fldserver/base/task/cancelable_task_tracker.h"
...
{
void
RunOrPostToTaskRunner(scoped_refptr<SequencedTaskRunner> task_runner, OnceClosure closure)
{
...
void
UmaRecordTaskDuration(bool same_sequence, bool background, bool canceled, TimeDelta duration)
{
#define DECLARE_HISTOGRAM(suffix)                                                                  \
    Histogram::FactoryTimeGet("Scheduler.CancelableTaskTracker.TaskDuration2_" suffix,             \
                              TimeDelta::FromMilliseconds(1),                                      \
                              TimeDelta::FromSeconds(10),                                          \
                              50,                                                                  \
                              Histogram::kUmaTargetedHistogramFlag)

    static HistogramBase* histograms[] = {DECLARE_HISTOGRAM("LiveForegroundOffSequence"),
...
const base::Feature kAllowOffSequenceTaskCancelation{"AllowOffSequenceTaskCancelation",
...
CancelableTaskTracker::PostTask(TaskRunner* task_runner,
...
CancelableTaskTracker::TaskId
CancelableTaskTracker::PostTaskAndReply(TaskRunner* task_runner,
                                        const Location& from_here,
                                        OnceClosure task,
                                        OnceClosure reply)
{
    ...
    OnceClosure untrack_closure = BindOnce(&CancelableTaskTracker::Untrack, Unretained(this), id);
    bool success = task_runner->PostTaskAndReply(
            from_here,
            BindOnce(&RunIfNotCanceled, SequencedTaskRunnerHandle::Get(), flag, std::move(task)),
...
CancelableTaskTracker::TaskId
CancelableTaskTracker::NewTrackedTaskId(IsCanceledCallback* is_canceled_cb)
{
    DCHECK(sequence_checker_.CalledOnValidSequence());
    DCHECK(SequencedTaskRunnerHandle::IsSet());
    ...
    ScopedClosureRunner untrack_runner(BindOnce(&RunOrPostToTaskRunner,
                                                SequencedTaskRunnerHandle::Get(),
                                                BindOnce(&RunIfNotCanceled,
                                                         SequencedTaskRunnerHandle::Get(),
                                                         flag,
                                                         std::move(untrack_closure))));

    *is_canceled_cb = BindRepeating(
            &IsCanceled, SequencedTaskRunnerHandle::Get(), flag, std::move(untrack_runner));
...
void
CancelableTaskTracker::TryCancel(TaskId id)
{
    DCHECK(sequence_checker_.CalledOnValidSequence());
...
void
CancelableTaskTracker::TryCancelAll()
{
    DCHECK(sequence_checker_.CalledOnValidSequence());
...
bool
CancelableTaskTracker::HasTrackedTasks() const
{
    DCHECK(sequence_checker_.CalledOnValidSequence());
...
// static
void
CancelableTaskTracker::RunIfNotCanceled(
        ...
        OnceClosure task)
{
    ...
    const TaskStatus task_status = was_canceled ? (same_sequence ? TaskStatus::kSameSequenceCanceled
                                                                 : TaskStatus::kOffSequenceCanceled)
                                                : (same_sequence ? TaskStatus::kSameSequenceLive
                                                                 : TaskStatus::kOffSequenceLive);
    UMA_HISTOGRAM_ENUMERATION("Scheduler.CancelableTaskTracker.TaskStatus", task_status);

    // Skip tasks if they are canceled, taking into account the off-sequence
    // cancelation experiment.
    const bool skip_task = was_canceled && (AllowOffSequenceTaskCancelation() || same_sequence);
    if (skip_task)
        return;
    ...
    const bool is_background = internal::GetTaskPriorityForCurrentThread()
...
// static
void
CancelableTaskTracker::RunThenUntrackIfNotCanceled(
        const scoped_refptr<SequencedTaskRunner>& origin_task_runner,
        const scoped_refptr<TaskCancellationFlag>& flag,
        OnceClosure task,
        OnceClosure untrack)
...
// static
bool
CancelableTaskTracker::IsCanceled(const scoped_refptr<SequencedTaskRunner>& origin_task_runner,
                                  const scoped_refptr<TaskCancellationFlag>& flag,
...
void
CancelableTaskTracker::Track(TaskId id, scoped_refptr<TaskCancellationFlag> flag)
{
    DCHECK(sequence_checker_.CalledOnValidSequence());
    CHECK(weak_this_);
...
void
CancelableTaskTracker::Untrack(TaskId id)
{
    DCHECK(sequence_checker_.CalledOnValidSequence());
    CHECK(weak_this_);
