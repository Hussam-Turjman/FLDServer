...
#include "fldserver/base/task/lazy_thread_pool_task_runner.h"

#include "fldserver/base/check_op.h"
#include "fldserver/base/lazy_instance_helpers.h"
...
namespace
{
ScopedLazyTaskRunnerListForTesting* g_scoped_lazy_task_runner_list_for_testing = nullptr;
}  // namespace

template <typename TaskRunnerType, bool com_sta>
void
LazyThreadPoolTaskRunner<TaskRunnerType, com_sta>::Reset()
{
...
template <typename TaskRunnerType, bool com_sta>
TaskRunnerType*
LazyThreadPoolTaskRunner<TaskRunnerType, com_sta>::CreateRaw(void* void_self)
{
    auto self = reinterpret_cast<LazyThreadPoolTaskRunner<TaskRunnerType, com_sta>*>(void_self);
    ...
    if (g_scoped_lazy_task_runner_list_for_testing)
    {
        g_scoped_lazy_task_runner_list_for_testing->AddCallback(BindOnce(
                &LazyThreadPoolTaskRunner<TaskRunnerType, com_sta>::Reset, Unretained(self)));
    }
...
LazyThreadPoolTaskRunner<TaskRunnerType, com_sta>::Get()
{
    return WrapRefCounted(subtle::GetOrCreateLazyPointer(
            &state_,
            &LazyThreadPoolTaskRunner<TaskRunnerType, com_sta>::CreateRaw,
...
void
ScopedLazyTaskRunnerListForTesting::AddCallback(OnceClosure callback)
{
    internal::CheckedAutoLock auto_lock(lock_);
