...
#include "fldserver/base/debug/crash_logging.h"
...
#include "fldserver/base/task/sequence_manager/real_time_domain.h"
#include "fldserver/base/task/sequence_manager/task_time_observer.h"
#include "fldserver/base/task/sequence_manager/thread_controller_impl.h"
#include "fldserver/base/task/sequence_manager/thread_controller_with_message_pump_impl.h"
#include "fldserver/base/task/sequence_manager/work_queue.h"
#include "fldserver/base/task/sequence_manager/work_queue_sets.h"
#include "fldserver/base/threading/thread_id_name_manager.h"
#include "fldserver/base/threading/thread_local.h"
#include "fldserver/base/time/default_tick_clock.h"
#include "fldserver/base/time/tick_clock.h"
...
{
    static NoDestructor<ThreadLocalPointer<internal::SequenceManagerImpl>> lazy_tls_ptr;
...
{
    return internal::SequenceManagerImpl::CreateOnCurrentThread(std::move(settings));
}

std::unique_ptr<SequenceManager>
CreateSequenceManagerOnCurrentThreadWithPump(std::unique_ptr<MessagePump> message_pump,
...
std::unique_ptr<SequenceManager>
CreateUnboundSequenceManager(SequenceManager::Settings settings)
{
...
namespace internal
{
using TimeRecordingPolicy = base::sequence_manager::TaskQueue::TaskTiming::TimeRecordingPolicy;

namespace
{
constexpr TimeDelta kLongTaskTraceEventThreshold = TimeDelta::FromMilliseconds(50);
...
void
ReclaimMemoryFromQueue(internal::TaskQueueImpl* queue,
...
SequenceManager::MetricRecordingSettings
InitializeMetricRecordingSettings(bool randomised_sampling_enabled)
{
    if (!randomised_sampling_enabled)
        return SequenceManager::MetricRecordingSettings(0);
    bool records_cpu_time_for_each_task = base::RandDouble()
                                          < kThreadSamplingRateForRecordingCPUTime;
    return SequenceManager::MetricRecordingSettings(
            records_cpu_time_for_each_task ? 1 : kTaskSamplingRateForRecordingCPUTime);
}
...
#if !defined(OS_NACL)
char*
PrependHexAddress(char* output, const void* address)
{
...
{
public:
    NativeWorkHandleImpl(SequenceManagerImpl* sequence_manager, TaskQueue::QueuePriority priority) :
    ...
    ~NativeWorkHandleImpl() final
    {
        ...
        // to yield.
        DCHECK_GE(sequence_manager_->main_thread_only().pending_native_work.size(), 1u);
        if (prev_priority != effective_priority())
            sequence_manager_->ScheduleWork();
    }

    TaskQueue::QueuePriority
    effective_priority() const
    {
...
// static
SequenceManagerImpl*
SequenceManagerImpl::GetCurrent()
{
    return GetTLSSequenceManagerImpl()->Get();
}

SequenceManagerImpl::SequenceManagerImpl(std::unique_ptr<internal::ThreadController> controller,
                                         SequenceManager::Settings settings) :
    associated_thread_(controller->GetAssociatedThread()),
    controller_(std::move(controller)),
    settings_(std::move(settings)),
    metric_recording_settings_(
            InitializeMetricRecordingSettings(settings_.randomised_sampling_enabled)),
...
SequenceManagerImpl::~SequenceManagerImpl()
{
...
#if defined(OS_IOS)
    if (settings_.message_loop_type == MessagePumpType::UI && associated_thread_->IsBound())
    {
    ...
    // fine
    DCHECK(!controller_->GetBoundMessagePump() || main_thread_only().task_execution_stack.empty());
...
SequenceManagerImpl::MainThreadOnly::MainThreadOnly(
    ...
    selector(associated_thread, settings), real_time_domain(new internal::RealTimeDomain())
{
...
SequenceManagerImpl::CreateThreadControllerImplForCurrentThread(const TickClock* clock)
{
...
SequenceManagerImpl::CreateOnCurrentThread(SequenceManager::Settings settings)
{
    auto thread_controller = CreateThreadControllerImplForCurrentThread(settings.clock);
    std::unique_ptr<SequenceManagerImpl> manager(
            new SequenceManagerImpl(std::move(thread_controller), std::move(settings)));
...
// static
std::unique_ptr<SequenceManagerImpl>
SequenceManagerImpl::CreateUnbound(SequenceManager::Settings settings)
{
    auto thread_controller = ThreadControllerWithMessagePumpImpl::CreateUnbound(settings);
    return WrapUnique(new SequenceManagerImpl(std::move(thread_controller), std::move(settings)));
}

void
SequenceManagerImpl::BindToMessagePump(std::unique_ptr<MessagePump> pump)
{
...
void
SequenceManagerImpl::BindToCurrentThread()
{
...
void
SequenceManagerImpl::BindToCurrentThread(std::unique_ptr<MessagePump> pump)
{
    associated_thread_->BindToCurrentThread();
...
SequenceManagerImpl::GetTaskRunnerForCurrentTask()
{
    ...
    if (main_thread_only().task_execution_stack.empty())
        return nullptr;
    return main_thread_only().task_execution_stack.back().pending_task.task_runner;
}

void
SequenceManagerImpl::CompleteInitializationOnBoundThread()
{
    controller_->AddNestingObserver(this);
    main_thread_only().nesting_observer_registered_ = true;
    if (GetMessagePump())
    {
        DCHECK(!GetTLSSequenceManagerImpl()->Get()) << "Can't register a second "
                                                       "SequenceManagerImpl on the same thread.";
        GetTLSSequenceManagerImpl()->Set(this);
    }
}

void
SequenceManagerImpl::RegisterTimeDomain(TimeDomain* time_domain)
{
...
void
SequenceManagerImpl::UnregisterTimeDomain(TimeDomain* time_domain)
{
    main_thread_only().time_domains.erase(time_domain);
}

TimeDomain*
SequenceManagerImpl::GetRealTimeDomain() const
{
...
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    TimeDomain* time_domain = spec.time_domain ? spec.time_domain
...
void
SequenceManagerImpl::SetAddQueueTimeToTasks(bool enable)
{
    base::subtle::NoBarrier_Store(&add_queue_time_to_tasks_, enable ? 1 : 0);
}

bool
SequenceManagerImpl::GetAddQueueTimeToTasks()
{
    return base::subtle::NoBarrier_Load(&add_queue_time_to_tasks_);
}

void
SequenceManagerImpl::SetObserver(Observer* observer)
{
    main_thread_only().observer = observer;
}

void
SequenceManagerImpl::ShutdownTaskQueueGracefully(
        std::unique_ptr<internal::TaskQueueImpl> task_queue)
{
...
SequenceManagerImpl::GetFlagToRequestReloadForEmptyQueue(TaskQueueImpl* task_queue)
{
    return empty_queues_to_reload_.AddFlag(
            BindRepeating(&TaskQueueImpl::ReloadEmptyImmediateWorkQueue, Unretained(task_queue)));
}

void
SequenceManagerImpl::ReloadEmptyWorkQueues() const
{
...
void
SequenceManagerImpl::MoveReadyDelayedTasksToWorkQueues(LazyNow* lazy_now)
{
...
void
SequenceManagerImpl::OnBeginNestedRunLoop()
{
...
void
SequenceManagerImpl::OnExitNestedRunLoop()
{
    main_thread_only().nesting_depth--;
...
void
SequenceManagerImpl::SetNextDelayedDoWork(LazyNow* lazy_now, TimeTicks run_time)
{
...
{
const char*
RunTaskTraceNameForPriority(TaskQueue::QueuePriority priority)
{
    switch (priority)
    {
        case TaskQueue::QueuePriority::kControlPriority: return "RunControlPriorityTask";
        case TaskQueue::QueuePriority::kHighestPriority: return "RunHighestPriorityTask";
        case TaskQueue::QueuePriority::kVeryHighPriority: return "RunVeryHighPriorityTask";
        case TaskQueue::QueuePriority::kHighPriority: return "RunHighPriorityTask";
        case TaskQueue::QueuePriority::kNormalPriority: return "RunNormalPriorityTask";
        case TaskQueue::QueuePriority::kLowPriority: return "RunLowPriorityTask";
        case TaskQueue::QueuePriority::kBestEffortPriority: return "RunBestEffortPriorityTask";
        case TaskQueue::QueuePriority::kQueuePriorityCount: NOTREACHED(); return nullptr;
    }
}
...
SequenceManagerImpl::SelectNextTask(SelectTaskOption option)
{
    ...
    ExecutingTask& executing_task = *main_thread_only().task_execution_stack.rbegin();
...
void
SequenceManagerImpl::LogTaskDebugInfo(const WorkQueue* selected_work_queue) const
{
    const Task* task = selected_work_queue->GetFrontTask();
    switch (settings_.task_execution_logging)
    {
        case Settings::TaskLogging::kNone: break;
        ...
        case Settings::TaskLogging::kReorderedOnly: {
            ...
            main_thread_only().selector.CollectSkippedOverLowerPriorityTasks(selected_work_queue,
                                                                             &skipped_tasks);
...
SequenceManagerImpl::SelectNextTaskImpl(SelectTaskOption option)
{
    ...
    // idle.
    if (lazy_now.has_value() && lazy_now.Now() >= main_thread_only().next_time_to_reclaim_memory)
    {
    ...
    while (true)
    {
        ...
        if (UNLIKELY(work_queue->GetFrontTask()->nestable == Nestable::kNonNestable
                     && main_thread_only().nesting_depth > 0))
        {
            ...
            main_thread_only().non_nestable_task_queue.push_back(std::move(deferred_task));
            continue;
        }

        if (UNLIKELY(!ShouldRunTaskOfPriority(work_queue->task_queue()->GetQueuePriority())))
        {
...
bool
SequenceManagerImpl::ShouldRunTaskOfPriority(TaskQueue::QueuePriority priority) const
{
...
TimeDelta
SequenceManagerImpl::DelayTillNextTask(LazyNow* lazy_now, SelectTaskOption option) const
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);

    if (auto priority = main_thread_only().selector.GetHighestPendingPriority(option))
    {
...
TimeDelta
SequenceManagerImpl::GetDelayTillNextDelayedTask(LazyNow* lazy_now, SelectTaskOption option) const
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
bool
SequenceManagerImpl::HasPendingHighResolutionTasks()
{
    for (TimeDomain* time_domain : main_thread_only().time_domains)
    {
...
bool
SequenceManagerImpl::OnSystemIdle()
{
    bool have_work_to_do = false;
    for (TimeDomain* time_domain : main_thread_only().time_domains)
    {
        if (time_domain->MaybeFastForwardToNextTask(controller_->ShouldQuitRunLoopWhenIdle()))
        {
...
void
SequenceManagerImpl::WillQueueTask(Task* pending_task, const char* task_queue_name)
{
    controller_->WillQueueTask(pending_task, task_queue_name);
}

TaskQueue::TaskTiming
SequenceManagerImpl::InitializeTaskTiming(internal::TaskQueueImpl* task_queue)
{
    bool records_wall_time = ShouldRecordTaskTiming(task_queue) == TimeRecordingPolicy::DoRecord;
...
TimeRecordingPolicy
SequenceManagerImpl::ShouldRecordTaskTiming(const internal::TaskQueueImpl* task_queue)
{
    if (task_queue->RequiresTaskTiming())
        return TimeRecordingPolicy::DoRecord;
    if (main_thread_only().nesting_depth == 0 && !main_thread_only().task_time_observers.empty())
    {
        return TimeRecordingPolicy::DoRecord;
    }
...
void
SequenceManagerImpl::NotifyWillProcessTask(ExecutingTask* executing_task, LazyNow* time_before_task)
{
    ...
    TimeRecordingPolicy recording_policy = ShouldRecordTaskTiming(executing_task->task_queue);
    ...
    const bool was_blocked_or_low_priority = executing_task->task_queue->WasBlockedOrLowPriority(
...

    {
        ...
        {
            observer.WillProcessTask(executing_task->pending_task, was_blocked_or_low_priority);
        }
    }

    {
        executing_task->task_queue->NotifyWillProcessTask(executing_task->pending_task,
...
void
SequenceManagerImpl::NotifyDidProcessTask(ExecutingTask* executing_task, LazyNow* time_after_task)
{
    ...
    bool has_valid_start = task_timing.state() != TaskQueue::TaskTiming::State::NotStarted;
    TimeRecordingPolicy recording_policy = ShouldRecordTaskTiming(executing_task->task_queue);
    ...
    if (has_valid_start && task_timing.has_wall_time() && main_thread_only().nesting_depth == 0)
    {
...

    {
        ...
        executing_task->task_queue->NotifyDidProcessTask(executing_task->pending_task);
    }

    // TODO(altimin): Move this back to blink.
    if (task_timing.has_wall_time() && recording_policy == TimeRecordingPolicy::DoRecord
...
void
SequenceManagerImpl::SetWorkBatchSize(int work_batch_size)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
void
SequenceManagerImpl::SetTimerSlack(TimerSlack timer_slack)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
void
SequenceManagerImpl::AddTaskObserver(TaskObserver* task_observer)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
void
SequenceManagerImpl::RemoveTaskObserver(TaskObserver* task_observer)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
void
SequenceManagerImpl::AddTaskTimeObserver(TaskTimeObserver* task_time_observer)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
void
SequenceManagerImpl::RemoveTaskTimeObserver(TaskTimeObserver* task_time_observer)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
{
    bool task_was_run = main_thread_only().task_was_run_on_quiescence_monitored_queue;
    main_thread_only().task_was_run_on_quiescence_monitored_queue = false;
    return !task_was_run;
}

EnqueueOrder
SequenceManagerImpl::GetNextSequenceNumber()
{
...
Value
SequenceManagerImpl::AsValueWithSelectorResult(internal::WorkQueue* selected_work_queue,
                                               bool force_verbose) const
{
    ...
    {
        state.SetStringKey("selected_queue", selected_work_queue->task_queue()->GetName());
        state.SetStringKey("work_queue_name", selected_work_queue->name());
    }
    state.SetStringKey(
            "native_work_priority",
            TaskQueue::PriorityToString(*main_thread_only().pending_native_work.begin()));
...
void
SequenceManagerImpl::OnTaskQueueEnabled(internal::TaskQueueImpl* queue)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
SequenceManagerImpl::MaybeReclaimMemory()
{
    ...
    main_thread_only().next_time_to_reclaim_memory = NowTicks() + kReclaimMemoryInterval;
    main_thread_only().memory_reclaim_scheduled = false;
}

void
SequenceManagerImpl::ReclaimMemory()
{
    std::map<TimeDomain*, TimeTicks> time_domain_now;
    for (auto* const queue : main_thread_only().active_queues)
...
SequenceManagerImpl::GetWeakPtr()
{
    return weak_factory_.GetWeakPtr();
}

void
SequenceManagerImpl::SetDefaultTaskRunner(scoped_refptr<SingleThreadTaskRunner> task_runner)
{
    controller_->SetDefaultTaskRunner(task_runner);
}

const TickClock*
SequenceManagerImpl::GetTickClock() const
{
    return controller_->GetClock();
}

TimeTicks
SequenceManagerImpl::NowTicks() const
{
    return controller_->GetClock()->NowTicks();
}

bool
SequenceManagerImpl::ShouldRecordCPUTimeForTask()
{
    ...
    return metric_recording_settings_.records_cpu_time_for_some_tasks()
           && main_thread_only().uniform_distribution(main_thread_only().random_generator)
                      < metric_recording_settings_.task_sampling_rate_for_recording_cpu_time;
}
...
MessagePumpType
SequenceManagerImpl::GetType() const
{
    return settings_.message_loop_type;
}

void
SequenceManagerImpl::SetTaskExecutionAllowed(bool allowed)
{
    controller_->SetTaskExecutionAllowed(allowed);
}

bool
SequenceManagerImpl::IsTaskExecutionAllowed() const
{
...
#if defined(OS_IOS)
void
SequenceManagerImpl::AttachToMessagePump()
{
...
bool
SequenceManagerImpl::IsIdleForTesting()
{
    ReloadEmptyWorkQueues();
...
size_t
SequenceManagerImpl::GetPendingTaskCountForTesting() const
{
    size_t total = 0;
...
scoped_refptr<TaskQueue>
SequenceManagerImpl::CreateTaskQueue(const TaskQueue::Spec& spec)
{
    return WrapRefCounted(new TaskQueue(CreateTaskQueueImpl(spec), spec));
}

std::string
SequenceManagerImpl::DescribeAllPendingTasks() const
{
...
std::unique_ptr<NativeWorkHandle>
SequenceManagerImpl::OnNativeWorkPending(TaskQueue::QueuePriority priority)
{
    return std::make_unique<NativeWorkHandleImpl>(this, priority);
}

void
SequenceManagerImpl::AddDestructionObserver(
...
void
SequenceManagerImpl::RemoveDestructionObserver(
        CurrentThread::DestructionObserver* destruction_observer)
{
...
void
SequenceManagerImpl::SetTaskRunner(scoped_refptr<SingleThreadTaskRunner> task_runner)
{
    controller_->SetDefaultTaskRunner(task_runner);
}

scoped_refptr<SingleThreadTaskRunner>
SequenceManagerImpl::GetTaskRunner()
{
    return controller_->GetDefaultTaskRunner();
}

bool
SequenceManagerImpl::IsBoundToCurrentThread() const
{
    return associated_thread_->IsBoundToCurrentThread();
}

MessagePump*
SequenceManagerImpl::GetMessagePump() const
{
    return controller_->GetBoundMessagePump();
}

bool
SequenceManagerImpl::IsType(MessagePumpType type) const
{
    return settings_.message_loop_type == type;
}

NOINLINE bool
SequenceManagerImpl::Validate()
{
    return memory_corruption_sentinel_ == kMemoryCorruptionSentinelValue;
}

void
SequenceManagerImpl::EnableCrashKeys(const char* async_stack_crash_key)
{
    DCHECK(!main_thread_only().async_stack_crash_key);
#if !defined(OS_NACL)
    main_thread_only().async_stack_crash_key =
            debug::AllocateCrashKeyString(async_stack_crash_key, debug::CrashKeySize::Size64);
...
void
SequenceManagerImpl::RecordCrashKeys(const PendingTask& pending_task)
{
#if !defined(OS_NACL)
...
SequenceManagerImpl::currently_executing_task_queue() const
{
    if (main_thread_only().task_execution_stack.empty())
        return nullptr;
