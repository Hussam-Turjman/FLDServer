...
void
ThreadController::RunLevelTracker::OnRunLoopStarted(State initial_state)
{
    run_levels_.emplace(initial_state);
}

void
ThreadController::RunLevelTracker::OnRunLoopEnded()
{
...
void
ThreadController::RunLevelTracker::OnTaskStarted()
{
...
void
ThreadController::RunLevelTracker::OnTaskEnded()
{
    if (run_levels_.empty())
        return;
...
void
ThreadController::RunLevelTracker::OnIdle()
{
    if (run_levels_.empty())
        return;
...
// static
void
ThreadController::RunLevelTracker::SetTraceObserverForTesting(
...
void
ThreadController::RunLevelTracker::RunLevel::UpdateState(State new_state)
{
    // The only state that can be redeclared is idle, anything else should be a
    // transition.
    DCHECK(state_ != new_state || new_state == kIdle) << state_ << "," << new_state;
