...
// is thread-safe.
class CORE_EXPORT WorkDeduplicator
{
public:
    // Creates an unbound WorkDeduplicator. BindToCurrentThread must be called
    // before work can be scheduled.
    explicit WorkDeduplicator(scoped_refptr<AssociatedThreadId> associated_thread);
    ...
    ShouldScheduleWork
    DidCheckForMoreWork(NextTask next_task);
