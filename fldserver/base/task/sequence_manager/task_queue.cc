...
#include "fldserver/base/threading/thread_checker_impl.h"
...
public:
    ...
    bool
    PostDelayedTask(const Location& location, OnceClosure callback, TimeDelta delay) override
    {
        return false;
    }
...
TaskQueue::QueueEnabledVoter::QueueEnabledVoter(scoped_refptr<TaskQueue> task_queue) :
...
void
TaskQueue::QueueEnabledVoter::SetVoteToEnable(bool enabled)
{
    if (enabled == enabled_)
        return;
    enabled_ = enabled;
...
void
TaskQueue::AddQueueEnabledVoter(bool voter_is_enabled)
{
...
void
TaskQueue::RemoveQueueEnabledVoter(bool voter_is_enabled)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return;
...
bool
TaskQueue::AreAllQueueEnabledVotersEnabled() const
{
    return enabled_voter_count_ == voter_count_;
}

void
TaskQueue::OnQueueEnabledVoteChanged(bool enabled)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    bool was_enabled = AreAllQueueEnabledVotersEnabled();
    if (enabled)
...
TaskQueue::TaskQueue(std::unique_ptr<internal::TaskQueueImpl> impl, const TaskQueue::Spec& spec) :
    impl_(std::move(impl)),
    ...
    default_task_runner_(impl_ ? impl_->CreateTaskRunner(kTaskTypeNone) : CreateNullTaskRunner()),
...
void
TaskQueue::TaskTiming::RecordTaskStart(LazyNow* now)
{
...
void
TaskQueue::TaskTiming::RecordTaskEnd(LazyNow* now)
{
...
TaskQueue::ShutdownTaskQueue()
{
    ...
    impl_->SetOnTaskStartedHandler(internal::TaskQueueImpl::OnTaskStartedHandler());
    impl_->SetOnTaskCompletedHandler(internal::TaskQueueImpl::OnTaskCompletedHandler());
...
scoped_refptr<SingleThreadTaskRunner>
TaskQueue::CreateTaskRunner(TaskType task_type)
{
    // We only need to lock if we're not on the main thread.
    base::internal::CheckedAutoLockMaybe lock(IsOnMainThread() ? &impl_lock_ : nullptr);
    if (!impl_)
...
size_t
TaskQueue::GetNumberOfPendingTasks() const
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return 0;
...
bool
TaskQueue::HasTaskToRunImmediately() const
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return false;
...
Optional<TimeTicks>
TaskQueue::GetNextScheduledWakeUp()
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return nullopt;
...
void
TaskQueue::SetQueuePriority(TaskQueue::QueuePriority priority)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return;
...
TaskQueue::GetQueuePriority() const
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
...
void
TaskQueue::AddTaskObserver(TaskObserver* task_observer)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return;
...
void
TaskQueue::RemoveTaskObserver(TaskObserver* task_observer)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return;
...
void
TaskQueue::SetTimeDomain(TimeDomain* time_domain)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return;
...
void
TaskQueue::InsertFence(InsertFencePosition position)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return;
...
void
TaskQueue::InsertFenceAt(TimeTicks time)
{
    impl_->InsertFenceAt(time);
}

void
TaskQueue::RemoveFence()
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return;
...
bool
TaskQueue::BlockedByFence() const
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return false;
...
void
TaskQueue::SetObserver(Observer* observer)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return;
...
void
TaskQueue::SetShouldReportPostedTasksWhenDisabled(bool should_report)
{
    impl_->SetShouldReportPostedTasksWhenDisabled(should_report);
}

bool
TaskQueue::IsOnMainThread() const
{
    return associated_thread_->IsBoundToCurrentThread();
}

std::unique_ptr<internal::TaskQueueImpl>
TaskQueue::TakeTaskQueueImpl()
{
...
void
TaskQueue::SetOnTaskStartedHandler(OnTaskStartedHandler handler)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return;
...
void
TaskQueue::SetOnTaskCompletedHandler(OnTaskCompletedHandler handler)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return;
...
void
TaskQueue::SetOnTaskPostedHandler(OnTaskPostedHandler handler)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!impl_)
        return;
