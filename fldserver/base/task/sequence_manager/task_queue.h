...
#include "fldserver/base/task/sequence_manager/tasks.h"
...
class CORE_EXPORT TaskQueue : public RefCountedThreadSafe<TaskQueue>
{
public:
    class Observer
    {
    public:
        ...
        virtual void
        OnQueueNextWakeUpChanged(TimeTicks next_wake_up) = 0;
    };
    ...
    // Can be called on any thread.
    static const char*
    PriorityToString(QueuePriority priority);
    ...
    struct Spec
    {
        ...
        // which is not free.
        Spec
        SetDelayedFencesAllowed(bool allow_delayed_fences)
        {
        ...
        Spec
        SetTimeDomain(TimeDomain* domain)
        {
            time_domain = domain;
            return *this;
        }
    ...
    TaskQueue(std::unique_ptr<internal::TaskQueueImpl> impl, const TaskQueue::Spec& spec);
    ...
    public:
        ...
        enum class TimeRecordingPolicy
        {
            DoRecord,
            DoNotRecord
        };
        ...
        has_wall_time() const
        {
            return has_wall_time_;
        }
        bool
        has_thread_time() const
        {
            return has_thread_time_;
        }
        ...
        void
        RecordTaskStart(LazyNow* now);
        void
        RecordTaskEnd(LazyNow* now);
    ...
    class CORE_EXPORT QueueEnabledVoter
    {
    public:
        ~QueueEnabledVoter();

        QueueEnabledVoter(const QueueEnabledVoter&) = delete;
        const QueueEnabledVoter&
        operator=(const QueueEnabledVoter&) = delete;
        ...
        // created on.
        void
        SetVoteToEnable(bool enabled);

        bool
        IsVotingToEnable() const
        {
            return enabled_;
        }
    ...
    std::unique_ptr<QueueEnabledVoter>
...
//    void
//    WriteIntoTrace(base::trace_event::TracedValue context) const;

    // Set the priority of the queue to |priority|. NOTE this must be called on
    // the thread this TaskQueue was created by.
    void
    SetQueuePriority(QueuePriority priority);
...
//    void
//    SetBlameContext(trace_event::BlameContext* blame_context);
    ...
    void
    InsertFence(InsertFencePosition position);
    ...
    CreateTaskRunner(TaskType task_type);
...
private:
    ...
    void
    AddQueueEnabledVoter(bool voter_is_enabled);
    void
    RemoveQueueEnabledVoter(bool voter_is_enabled);
    bool
    AreAllQueueEnabledVotersEnabled() const;
    void
    OnQueueEnabledVoteChanged(bool enabled);

    bool
    IsOnMainThread() const;
