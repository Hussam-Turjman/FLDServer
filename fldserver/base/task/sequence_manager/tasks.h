...
struct CORE_EXPORT PostedTask
{
    ...
    PostedTask&
    operator=(const PostedTask&) = delete;
    ~PostedTask();
...
struct DelayedWakeUp
{
    ...
    bool
    operator!=(const DelayedWakeUp& other) const
    {
...
struct CORE_EXPORT Task : public PendingTask
{
    Task(internal::PostedTask posted_task,
         ...
         internal::WakeUpResolution wake_up_resolution = internal::WakeUpResolution::kLow);
    Task(Task&& move_from);
    ~Task();
    Task&
    operator=(Task&& other);
    ...
    void
    set_enqueue_order(EnqueueOrder enqueue_order)
    {
        DCHECK(!enqueue_order_);
