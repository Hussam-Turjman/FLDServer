...
class CORE_EXPORT ThreadControllerImpl : public ThreadController, public RunLoop::NestingObserver
{
public:
    ThreadControllerImpl(const ThreadControllerImpl&) = delete;
    ThreadControllerImpl&
    operator=(const ThreadControllerImpl&) = delete;
    ...
    Create(SequenceManagerImpl* funneled_sequence_manager, const TickClock* time_source);
    ...
    void
    SetSequencedTaskSource(SequencedTaskSource* sequence) override;
    void
...
private:
    ...
    const MainSequenceOnly&
    main_sequence_only() const
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(associated_thread_->sequence_checker);
        return main_sequence_only_;
    }
