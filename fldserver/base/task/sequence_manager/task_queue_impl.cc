...
#include "fldserver/base/task/sequence_manager/task_queue_impl.h"

#include "fldserver/base/logging.h"
#include "fldserver/base/ranges/algorithm.h"
#include "fldserver/base/strings/stringprintf.h"
#include "fldserver/base/task/common/scoped_defer_task_posting.h"
#include "fldserver/base/task/sequence_manager/sequence_manager_impl.h"
#include "fldserver/base/task/sequence_manager/time_domain.h"
#include "fldserver/base/task/sequence_manager/work_queue.h"
#include "fldserver/base/task/task_observer.h"
...
// static
const char*
TaskQueue::PriorityToString(TaskQueue::QueuePriority priority)
{
    switch (priority)
    {
        case kControlPriority: return "control";
        case kHighestPriority: return "highest";
        case kVeryHighPriority: return "very_high";
        case kHighPriority: return "high";
        case kNormalPriority: return "normal";
        case kLowPriority: return "low";
        case kBestEffortPriority: return "best_effort";
        default: NOTREACHED(); return nullptr;
    }
}

namespace internal
{
TaskQueueImpl::GuardedTaskPoster::GuardedTaskPoster(TaskQueueImpl* outer) : outer_(outer)
{
}
...
bool
TaskQueueImpl::GuardedTaskPoster::PostTask(PostedTask task)
{
...
TaskQueueImpl::TaskRunner::TaskRunner(scoped_refptr<GuardedTaskPoster> task_poster,
...
bool
TaskQueueImpl::TaskRunner::PostDelayedTask(const Location& location,
                                           OnceClosure callback,
                                           TimeDelta delay)
{
    return task_poster_->PostTask(PostedTask(
            this, std::move(callback), location, delay, Nestable::kNestable, task_type_));
}
...
{
    return task_poster_->PostTask(PostedTask(
            this, std::move(callback), location, delay, Nestable::kNonNestable, task_type_));
}

bool
TaskQueueImpl::TaskRunner::RunsTasksInCurrentSequence() const
{
...
TaskQueueImpl::TaskQueueImpl(SequenceManagerImpl* sequence_manager,
    ...
    associated_thread_(sequence_manager ? sequence_manager->associated_thread()
    ...
    empty_queues_to_reload_handle_(
            sequence_manager ? sequence_manager->GetFlagToRequestReloadForEmptyQueue(this)
...
TaskQueueImpl::~TaskQueueImpl()
{
    ...
    // queues.
    DCHECK(any_thread_.unregistered) << "UnregisterTaskQueue must be called first!";
#endif
}

TaskQueueImpl::AnyThread::AnyThread(TimeDomain* time_domain) : time_domain(time_domain)
{
}
...
TaskQueueImpl::MainThreadOnly::MainThreadOnly(TaskQueueImpl* task_queue, TimeDomain* time_domain) :
    time_domain(time_domain),
    delayed_work_queue(new WorkQueue(task_queue, "delayed", WorkQueue::QueueType::kDelayed)),
    immediate_work_queue(new WorkQueue(task_queue, "immediate", WorkQueue::QueueType::kImmediate))
{
}
...
TaskQueueImpl::CreateTaskRunner(TaskType task_type) const
{
    return MakeRefCounted<TaskRunner>(task_poster_, associated_thread_, task_type);
}
...
const char*
TaskQueueImpl::GetName() const
{
    return name_;
}

void
TaskQueueImpl::PostTask(PostedTask task)
{
    CurrentThread current_thread = associated_thread_->IsBoundToCurrentThread()
...
void
TaskQueueImpl::MaybeLogPostTask(PostedTask* task)
{
#if DCHECK_IS_ON()
    if (!sequence_manager_->settings().log_post_task)
        return;

    LOG(INFO) << name_ << " PostTask " << task->location.ToString() << " delay " << task->delay;
#endif  // DCHECK_IS_ON()
}

void
TaskQueueImpl::MaybeAdjustTaskDelay(PostedTask* task, CurrentThread current_thread)
{
#if DCHECK_IS_ON()
    if (current_thread == TaskQueueImpl::CurrentThread::kNotMainThread)
    {
        ...
        task->delay += sequence_manager_->settings()
                               .per_priority_cross_thread_task_delay[any_thread_.queue_set_index];
    }
    else
    {
        task->delay += sequence_manager_->settings().per_priority_same_thread_task_delay
...
void
TaskQueueImpl::PostImmediateTaskImpl(PostedTask task, CurrentThread current_thread)
{
    ...
    bool should_schedule_work = false;
    {
        ...
        any_thread_.immediate_incoming_queue.push_back(
                Task(std::move(task), delayed_run_time, sequence_number, sequence_number));
        ...
        sequence_manager_->WillQueueTask(&any_thread_.immediate_incoming_queue.back(), name_);
        ...
        {
            any_thread_.on_task_posted_handler.Run(any_thread_.immediate_incoming_queue.back());
        }
...
void
TaskQueueImpl::PostDelayedTaskImpl(PostedTask posted_task, CurrentThread current_thread)
{
    // Use CHECK instead of DCHECK to crash earlier. See http://crbug.com/711167
    // for details.
    ...
    // between 10 and 15ms.
    if (posted_task.delay.InMilliseconds() < (2 * Time::kMinLowResolutionThresholdMs))
    ...
    if (current_thread == CurrentThread::kMainThread)
    {
        ...
        TimeTicks time_domain_delayed_run_time = time_domain_now + posted_task.delay;
        if (sequence_manager_->GetAddQueueTimeToTasks())
            posted_task.queue_time = time_domain_now;

        PushOntoDelayedIncomingQueueFromMainThread(Task(std::move(posted_task),
    ...
    else
    {
        ...
        PushOntoDelayedIncomingQueue(Task(std::move(posted_task),
                                          time_domain_delayed_run_time,
                                          sequence_number,
                                          EnqueueOrder(),
                                          resolution));
...
void
TaskQueueImpl::PushOntoDelayedIncomingQueueFromMainThread(Task pending_task,
                                                          TimeTicks now,
...
void
TaskQueueImpl::PushOntoDelayedIncomingQueue(Task pending_task)
{
    sequence_manager_->WillQueueTask(&pending_task, name_);
...
void
TaskQueueImpl::ScheduleDelayedWorkTask(Task pending_task)
{
    ...
    else
    {
        ...
        PushOntoDelayedIncomingQueueFromMainThread(std::move(pending_task), time_domain_now, false);
    }
    TraceQueueSize();
}

void
TaskQueueImpl::ReloadEmptyImmediateWorkQueue()
{
    ...
    {
        main_thread_only().task_queue_observer->OnQueueNextWakeUpChanged(TimeTicks());
    }
}

void
TaskQueueImpl::TakeImmediateIncomingQueueTasks(TaskDeque* queue)
{
    base::internal::CheckedAutoLock lock(any_thread_lock_);
    DCHECK(queue->empty());
...
size_t
TaskQueueImpl::GetNumberOfPendingTasks() const
{
    size_t task_count = 0;
...
bool
TaskQueueImpl::HasTaskToRunImmediately() const
{
...
TaskQueueImpl::GetNextScheduledWakeUpImpl()
{
    ...
    return DelayedWakeUp{top_task.delayed_run_time, top_task.sequence_num, resolution};
}

Optional<TimeTicks>
TaskQueueImpl::GetNextScheduledWakeUp()
{
...
void
TaskQueueImpl::MoveReadyDelayedTasksToWorkQueue(LazyNow* lazy_now)
{
...
void
TaskQueueImpl::TraceQueueSize() const
{
    bool is_tracing;
...
void
TaskQueueImpl::SetQueuePriority(TaskQueue::QueuePriority priority)
{
    const TaskQueue::QueuePriority previous_priority = GetQueuePriority();
    if (priority == previous_priority)
        return;
    sequence_manager_->main_thread_only().selector.SetQueuePriority(this, priority);
...
TaskQueueImpl::GetQueuePriority() const
{
...
Value
TaskQueueImpl::AsValue(TimeTicks now, bool force_verbose) const
{
    base::internal::CheckedAutoLock lock(any_thread_lock_);
    ...
    state.SetStringKey("task_queue_id",
                       StringPrintf("0x%" PRIx64,
                                    static_cast<uint64_t>(reinterpret_cast<uintptr_t>(this))));
    state.SetBoolKey("enabled", IsQueueEnabled());
    state.SetStringKey("time_domain_name", main_thread_only().time_domain->GetName());
    ...
    state.SetIntKey("delayed_incoming_queue_size",
                    main_thread_only().delayed_incoming_queue.size());
    state.SetIntKey("immediate_work_queue_size", main_thread_only().immediate_work_queue->Size());
    state.SetIntKey("delayed_work_queue_size", main_thread_only().delayed_work_queue->Size());

    state.SetIntKey("any_thread_.immediate_incoming_queuecapacity",
                    any_thread_.immediate_incoming_queue.capacity());
    state.SetIntKey("immediate_work_queue_capacity", immediate_work_queue()->Capacity());
    state.SetIntKey("delayed_work_queue_capacity", delayed_work_queue()->Capacity());
    ...
    if (!main_thread_only().delayed_incoming_queue.empty())
    {
        TimeDelta delay_to_next_task =
                (main_thread_only().delayed_incoming_queue.top().delayed_run_time
                 - main_thread_only().time_domain->CreateLazyNow().Now());
        state.SetDoubleKey("delay_to_next_task_ms", delay_to_next_task.InMillisecondsF());
    }
    if (main_thread_only().current_fence)
        state.SetIntKey("current_fence", main_thread_only().current_fence);
    if (main_thread_only().delayed_fence)
    {
        state.SetDoubleKey("delayed_fence_seconds_from_now",
    ...
    if (verbose || force_verbose)
    {
        ...
        state.SetKey("delayed_work_queue", main_thread_only().delayed_work_queue->AsValue(now));
        state.SetKey("immediate_work_queue", main_thread_only().immediate_work_queue->AsValue(now));
        state.SetKey("delayed_incoming_queue",
                     main_thread_only().delayed_incoming_queue.AsValue(now));
    }
    state.SetStringKey("priority", TaskQueue::PriorityToString(GetQueuePriority()));
    return state;
}

void
TaskQueueImpl::AddTaskObserver(TaskObserver* task_observer)
{
    main_thread_only().task_observers.AddObserver(task_observer);
}

void
TaskQueueImpl::RemoveTaskObserver(TaskObserver* task_observer)
{
    main_thread_only().task_observers.RemoveObserver(task_observer);
}

void
TaskQueueImpl::NotifyWillProcessTask(const Task& task, bool was_blocked_or_low_priority)
{
...
void
TaskQueueImpl::NotifyDidProcessTask(const Task& task)
{
    DCHECK(should_notify_observers_);
    for (auto& observer : main_thread_only().task_observers)
...
void
TaskQueueImpl::SetTimeDomain(TimeDomain* time_domain)
{
    {
        base::internal::CheckedAutoLock lock(any_thread_lock_);
        DCHECK(time_domain);
...
TimeDomain*
TaskQueueImpl::GetTimeDomain() const
{
    DCHECK(associated_thread_->IsBoundToCurrentThread() || !associated_thread_->IsBound());
...
void
TaskQueueImpl::InsertFence(TaskQueue::InsertFencePosition position)
{
    ...
    bool front_task_unblocked = main_thread_only().immediate_work_queue->InsertFence(current_fence);
    front_task_unblocked |= main_thread_only().delayed_work_queue->InsertFence(current_fence);

    {
        base::internal::CheckedAutoLock lock(any_thread_lock_);
        if (!front_task_unblocked && previous_fence && previous_fence < current_fence)
        {
            if (!any_thread_.immediate_incoming_queue.empty()
                && any_thread_.immediate_incoming_queue.front().enqueue_order() > previous_fence
                && any_thread_.immediate_incoming_queue.front().enqueue_order() < current_fence)
            {
...
void
TaskQueueImpl::InsertFenceAt(TimeTicks time)
{
    DCHECK(delayed_fence_allowed_) << "Delayed fences are not supported for this queue. Enable "
                                      "them "
...
void
TaskQueueImpl::RemoveFence()
{
    EnqueueOrder previous_fence = main_thread_only().current_fence;
    main_thread_only().current_fence = EnqueueOrder::none();
    main_thread_only().delayed_fence = nullopt;

    bool front_task_unblocked = main_thread_only().immediate_work_queue->RemoveFence();
...
bool
TaskQueueImpl::BlockedByFence() const
{
    if (!main_thread_only().current_fence)
        return false;
...
bool
TaskQueueImpl::HasActiveFence()
{
    if (main_thread_only().delayed_fence
        && main_thread_only().time_domain->Now() > main_thread_only().delayed_fence.value())
    {
        return true;
    }
...
bool
TaskQueueImpl::CouldTaskRun(EnqueueOrder enqueue_order) const
{
    if (!IsQueueEnabled())
        return false;
...
bool
TaskQueueImpl::WasBlockedOrLowPriority(EnqueueOrder enqueue_order) const
{
...
// static
Value
TaskQueueImpl::QueueAsValue(const TaskDeque& queue, TimeTicks now)
{
...
// static
Value
TaskQueueImpl::TaskAsValue(const Task& task, TimeTicks now)
{
    Value state(Value::Type::DICTIONARY);
    ...
    state.SetDoubleKey("delayed_run_time", (task.delayed_run_time - TimeTicks()).InMillisecondsF());
    const TimeDelta delayed_run_time_milliseconds_from_now =
            task.delayed_run_time.is_null() ? TimeDelta() : (task.delayed_run_time - now);
...
bool
TaskQueueImpl::IsQueueEnabled() const
{
    return main_thread_only().is_enabled;
}

void
TaskQueueImpl::SetQueueEnabled(bool enabled)
{
...
void
TaskQueueImpl::SetShouldReportPostedTasksWhenDisabled(bool should_report)
{
    if (main_thread_only().should_report_posted_tasks_when_disabled == should_report)
        return;
    ...
    {
        base::internal::CheckedAutoLock lock(any_thread_lock_);
        any_thread_.tracing_only.should_report_posted_tasks_when_disabled = should_report;
    }
}

void
TaskQueueImpl::UpdateCrossThreadQueueStateLocked()
{
    any_thread_.immediate_work_queue_empty = main_thread_only().immediate_work_queue->Empty();
...
#if DCHECK_IS_ON()
    any_thread_.queue_set_index = main_thread_only().immediate_work_queue->work_queue_set_index();
#endif
}

void
TaskQueueImpl::ReclaimMemory(TimeTicks now)
{
    if (main_thread_only().delayed_incoming_queue.empty())
        return;
    main_thread_only().delayed_incoming_queue.SweepCancelledTasks(sequence_manager_);
...
void
TaskQueueImpl::PushImmediateIncomingTaskForTest(Task&& task)
{
    base::internal::CheckedAutoLock lock(any_thread_lock_);
...
void
TaskQueueImpl::RequeueDeferredNonNestableTask(DeferredNonNestableTask task)
{
    ...
    {
        main_thread_only().delayed_work_queue->PushNonNestableTaskToFront(std::move(task.task));
    }
    else
    {
...
void
TaskQueueImpl::SetObserver(TaskQueue::Observer* observer)
{
    if (observer)
    {
        DCHECK(!main_thread_only().task_queue_observer) << "Can't assign two different observers "
                                                           "to "
...
void
TaskQueueImpl::UpdateDelayedWakeUp(LazyNow* lazy_now)
{
    return UpdateDelayedWakeUpImpl(lazy_now, GetNextScheduledWakeUpImpl());
}

void
TaskQueueImpl::UpdateDelayedWakeUpImpl(LazyNow* lazy_now, Optional<DelayedWakeUp> wake_up)
{
    if (main_thread_only().scheduled_wake_up == wake_up)
        return;
    main_thread_only().scheduled_wake_up = wake_up;

    if (wake_up && main_thread_only().task_queue_observer && !HasPendingImmediateWork())
    {
        main_thread_only().task_queue_observer->OnQueueNextWakeUpChanged(wake_up->time);
    }

    main_thread_only().time_domain->SetNextWakeUpForQueue(this, wake_up, lazy_now);
}

void
TaskQueueImpl::SetDelayedWakeUpForTesting(Optional<DelayedWakeUp> wake_up)
{
    LazyNow lazy_now = main_thread_only().time_domain->CreateLazyNow();
...
bool
TaskQueueImpl::HasPendingImmediateWork()
{
    // Any work queue tasks count as immediate work.
    if (!main_thread_only().delayed_work_queue->Empty()
        || !main_thread_only().immediate_work_queue->Empty())
...
bool
TaskQueueImpl::HasPendingImmediateWorkLocked()
{
...
void
TaskQueueImpl::SetOnTaskStartedHandler(TaskQueueImpl::OnTaskStartedHandler handler)
{
...
void
TaskQueueImpl::OnTaskStarted(const Task& task, const TaskQueue::TaskTiming& task_timing)
{
...
void
TaskQueueImpl::SetOnTaskCompletedHandler(TaskQueueImpl::OnTaskCompletedHandler handler)
{
    DCHECK(should_notify_observers_ || handler.is_null());
...
void
TaskQueueImpl::OnTaskCompleted(const Task& task,
...
bool
TaskQueueImpl::RequiresTaskTiming() const
{
...
void
TaskQueueImpl::SetOnTaskPostedHandler(OnTaskPostedHandler handler)
{
    DCHECK(should_notify_observers_ || handler.is_null());
    base::internal::CheckedAutoLock lock(any_thread_lock_);
...
bool
TaskQueueImpl::IsUnregistered() const
{
    base::internal::CheckedAutoLock lock(any_thread_lock_);
    return any_thread_.unregistered;
}

WeakPtr<SequenceManagerImpl>
TaskQueueImpl::GetSequenceManagerWeakPtr()
{
    return sequence_manager_->GetWeakPtr();
}

void
TaskQueueImpl::ActivateDelayedFenceIfNeeded(TimeTicks now)
{
    if (!main_thread_only().delayed_fence)
        return;
...
void
TaskQueueImpl::MaybeReportIpcTaskQueuedFromMainThread(Task* pending_task,
                                                      const char* task_queue_name)
{
    ...
    base::TimeDelta time_since_disabled =
            main_thread_only().time_domain->Now() - main_thread_only().disabled_time.value();

    ReportIpcTaskQueued(pending_task, task_queue_name, time_since_disabled);
}

bool
TaskQueueImpl::ShouldReportIpcTaskQueuedFromAnyThreadLocked(base::TimeDelta* time_since_disabled)
{
    // It's possible that tracing was just enabled and no disabled time has been
    // stored. In that case, skip emitting the event.
    ...
    *time_since_disabled =
            any_thread_.time_domain->Now() - any_thread_.tracing_only.disabled_time.value();
    return true;
}

void
TaskQueueImpl::MaybeReportIpcTaskQueuedFromAnyThreadLocked(Task* pending_task,
                                                           const char* task_queue_name)
{
    if (!pending_task->ipc_hash)
        return;
...
void
TaskQueueImpl::MaybeReportIpcTaskQueuedFromAnyThreadUnlocked(Task* pending_task,
                                                             const char* task_queue_name)
{
    if (!pending_task->ipc_hash)
        return;
    ...
    {
        base::internal::CheckedAutoLock lock(any_thread_lock_);
        should_report = ShouldReportIpcTaskQueuedFromAnyThreadLocked(&time_since_disabled);
    }
...
void
TaskQueueImpl::ReportIpcTaskQueued(Task* pending_task,
                                   const char* task_queue_name,
...
void
TaskQueueImpl::OnQueueUnblocked()
{
...
void
TaskQueueImpl::DelayedIncomingQueue::push(Task&& task)
{
    if (task.is_high_res)
...
void
TaskQueueImpl::DelayedIncomingQueue::pop()
{
    DCHECK(!empty());
    if (top().is_high_res)
    {
...
void
TaskQueueImpl::DelayedIncomingQueue::swap(DelayedIncomingQueue* rhs)
{
...
void
TaskQueueImpl::DelayedIncomingQueue::SweepCancelledTasks(SequenceManagerImpl* sequence_manager)
{
    pending_high_res_tasks_ -= queue_.SweepCancelledTasks(sequence_manager);
}

size_t
TaskQueueImpl::DelayedIncomingQueue::PQueue::SweepCancelledTasks(
        SequenceManagerImpl* sequence_manager)
{
    ...
    auto keep_task = [sequence_manager, &num_high_res_tasks_swept](const Task& task) {
...
Value
TaskQueueImpl::DelayedIncomingQueue::AsValue(TimeTicks now) const
{
    return queue_.AsValue(now);
}

Value
TaskQueueImpl::DelayedIncomingQueue::PQueue::AsValue(TimeTicks now) const
{
    Value state(Value::Type::LIST);
