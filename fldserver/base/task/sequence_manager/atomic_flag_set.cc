...
namespace internal
{
AtomicFlagSet::AtomicFlagSet(scoped_refptr<AssociatedThreadId> associated_thread) :
...
AtomicFlagSet::AtomicFlag::AtomicFlag(AtomicFlagSet* outer, Group* element, size_t flag_bit) :
...
void
AtomicFlagSet::AtomicFlag::SetActive(bool active)
{
    DCHECK(group_);
    if (active)
    {
...
void
AtomicFlagSet::AtomicFlag::ReleaseAtomicFlag()
{
    if (!group_)
        return;
...
AtomicFlagSet::AddFlag(RepeatingClosure callback)
{
    ...
    size_t first_unoccupied_index = static_cast<size_t>(group->FindFirstUnallocatedFlag());
...
void
AtomicFlagSet::RunActiveCallbacks() const
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    for (Group* iter = alloc_list_head_.get(); iter; iter = iter->next.get())
    {
        ...
        size_t active_flags =
                std::atomic_exchange_explicit(&iter->flags, size_t{0}, std::memory_order_acquire);
...
bool
AtomicFlagSet::Group::IsFull() const
{
    return (~allocated_flags) == 0u;
}

bool
AtomicFlagSet::Group::IsEmpty() const
{
    return allocated_flags == 0u;
}

int
AtomicFlagSet::Group::FindFirstUnallocatedFlag() const
{
...
// static
int
AtomicFlagSet::Group::IndexOfFirstFlagSet(size_t flag)
{
    DCHECK_NE(flag, 0u);
...
void
AtomicFlagSet::AddToAllocList(std::unique_ptr<Group> group)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (alloc_list_head_)
...
void
AtomicFlagSet::RemoveFromAllocList(Group* group)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (group->next)
...
void
AtomicFlagSet::AddToPartiallyFreeList(Group* group)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
void
AtomicFlagSet::RemoveFromPartiallyFreeList(Group* group)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    DCHECK(partially_free_list_head_);
    ...
    {
        group->partially_free_list_next->partially_free_list_prev = group->partially_free_list_prev;
    }

    if (group->partially_free_list_prev)
    {
        group->partially_free_list_prev->partially_free_list_next = group->partially_free_list_next;
    }
    else
    {
