...
void
WorkQueueSets::AddQueue(WorkQueue* work_queue, size_t set_index)
{
...
void
WorkQueueSets::RemoveQueue(WorkQueue* work_queue)
{
...
void
WorkQueueSets::ChangeSetIndex(WorkQueue* work_queue, size_t set_index)
{
    DCHECK_EQ(this, work_queue->work_queue_sets());
    DCHECK_LT(set_index, work_queue_heaps_.size());
    EnqueueOrder enqueue_order;
...
void
WorkQueueSets::OnQueuesFrontTaskChanged(WorkQueue* work_queue)
{
    EnqueueOrder enqueue_order;
    size_t set_index = work_queue->work_queue_set_index();
    DCHECK_EQ(this, work_queue->work_queue_sets());
...
void
WorkQueueSets::OnTaskPushedToEmptyQueue(WorkQueue* work_queue)
{
...
void
WorkQueueSets::OnPopMinQueueInSet(WorkQueue* work_queue)
{
...
void
WorkQueueSets::OnQueueBlocked(WorkQueue* work_queue)
{
    DCHECK_EQ(this, work_queue->work_queue_sets());
...
WorkQueue*
WorkQueueSets::GetOldestQueueInSet(size_t set_index) const
{
    DCHECK_LT(set_index, work_queue_heaps_.size());
    if (work_queue_heaps_[set_index].empty())
        return nullptr;
...
WorkQueue*
WorkQueueSets::GetOldestQueueAndEnqueueOrderInSet(size_t set_index,
                                                  EnqueueOrder* out_enqueue_order) const
{
    ...
    *out_enqueue_order = oldest.key;
    EnqueueOrder enqueue_order;
    DCHECK(oldest.value->GetFrontTaskEnqueueOrder(&enqueue_order) && oldest.key == enqueue_order);
    return oldest.value;
}

#if DCHECK_IS_ON()
WorkQueue*
WorkQueueSets::GetRandomQueueInSet(size_t set_index) const
{
    DCHECK_LT(set_index, work_queue_heaps_.size());
    if (work_queue_heaps_[set_index].empty())
        return nullptr;

    WorkQueue* queue = work_queue_heaps_[set_index]
...
WorkQueue*
WorkQueueSets::GetRandomQueueAndEnqueueOrderInSet(size_t set_index,
                                                  EnqueueOrder* out_enqueue_order) const
{
    DCHECK_LT(set_index, work_queue_heaps_.size());
    if (work_queue_heaps_[set_index].empty())
    ...
    const OldestTaskEnqueueOrder& chosen =
            work_queue_heaps_[set_index].begin()[Random() % work_queue_heaps_[set_index].size()];
    *out_enqueue_order = chosen.key;
    EnqueueOrder enqueue_order;
    DCHECK(chosen.value->GetFrontTaskEnqueueOrder(&enqueue_order) && chosen.key == enqueue_order);
    return chosen.value;
}
#endif

bool
WorkQueueSets::IsSetEmpty(size_t set_index) const
{
    DCHECK_LT(set_index, work_queue_heaps_.size()) << " set_index = " << set_index;
...
bool
WorkQueueSets::ContainsWorkQueueForTest(const WorkQueue* work_queue) const
{
    EnqueueOrder enqueue_order;
    bool has_enqueue_order = work_queue->GetFrontTaskEnqueueOrder(&enqueue_order);

    for (const base::internal::IntrusiveHeap<OldestTaskEnqueueOrder>& heap : work_queue_heaps_)
    {
...
void
WorkQueueSets::CollectSkippedOverLowerPriorityTasks(const internal::WorkQueue* selected_work_queue,
