...
#include "fldserver/base/task/common/intrusive_heap.h"
#include "fldserver/base/task/common/operations_controller.h"
#include "fldserver/base/task/sequence_manager/associated_thread_id.h"
#include "fldserver/base/task/sequence_manager/atomic_flag_set.h"
#include "fldserver/base/task/sequence_manager/enqueue_order.h"
#include "fldserver/base/task/sequence_manager/lazily_deallocated_deque.h"
#include "fldserver/base/task/sequence_manager/sequenced_task_source.h"
#include "fldserver/base/task/sequence_manager/task_queue.h"
...
class CORE_EXPORT TaskQueueImpl
{
public:
    ...
    TaskQueueImpl&
    operator=(const TaskQueueImpl&) = delete;
    ~TaskQueueImpl();
    ...
    CreateTaskRunner(TaskType task_type) const;

    // TaskQueue implementation.
    const char*
    GetName() const;
    bool
    IsQueueEnabled() const;
    void
    SetQueueEnabled(bool enabled);
    void
    SetShouldReportPostedTasksWhenDisabled(bool should_report);
    bool
    IsEmpty() const;
    size_t
    GetNumberOfPendingTasks() const;
    bool
    HasTaskToRunImmediately() const;
    Optional<TimeTicks>
    GetNextScheduledWakeUp();
    void
    SetQueuePriority(TaskQueue::QueuePriority priority);
    TaskQueue::QueuePriority
    GetQueuePriority() const;
    void
    AddTaskObserver(TaskObserver* task_observer);
    void
    RemoveTaskObserver(TaskObserver* task_observer);
    void
    SetTimeDomain(TimeDomain* time_domain);
    TimeDomain*
    GetTimeDomain() const;

    void
    InsertFence(TaskQueue::InsertFencePosition position);
    void
    InsertFenceAt(TimeTicks time);
    void
    RemoveFence();
    bool
    HasActiveFence();
    bool
    BlockedByFence() const;

    // Implementation of TaskQueue::SetObserver.
    void
    SetObserver(TaskQueue::Observer* observer);
    ...
    // thread.
    bool
    CouldTaskRun(EnqueueOrder enqueue_order) const;
    ...
    bool
    WasBlockedOrLowPriority(EnqueueOrder enqueue_order) const;

    // Must only be called from the thread this task queue was created on.
    void
    ReloadEmptyImmediateWorkQueue();

    Value
    AsValue(TimeTicks now, bool force_verbose) const;

    bool
    GetQuiescenceMonitored() const
    {
        return should_monitor_quiescence_;
    }
    bool
    GetShouldNotifyObservers() const
    {
        return should_notify_observers_;
    }

    void
    NotifyWillProcessTask(const Task& task, bool was_blocked_or_low_priority);
    void
    NotifyDidProcessTask(const Task& task);
    ...
    bool
    HasPendingImmediateWorkLocked() EXCLUSIVE_LOCKS_REQUIRED(any_thread_lock_);

    bool
    has_pending_high_resolution_tasks() const
    {
        return main_thread_only().delayed_incoming_queue.has_pending_high_resolution_tasks();
    }
    ...
    void
    MoveReadyDelayedTasksToWorkQueue(LazyNow* lazy_now);

    base::internal::HeapHandle
    heap_handle() const
    {
    ...
    void
    set_heap_handle(base::internal::HeapHandle heap_handle)
    {
    ...
    void
    RequeueDeferredNonNestableTask(DeferredNonNestableTask task);

    void
    PushImmediateIncomingTaskForTest(Task&& task);
    ...
    void
    ReclaimMemory(TimeTicks now);

    // Allows wrapping TaskQueue to set a handler to subscribe for notifications
    // about started and completed tasks.
    void
    SetOnTaskStartedHandler(OnTaskStartedHandler handler);
    void
    OnTaskStarted(const Task& task, const TaskQueue::TaskTiming& task_timing);
    ...
    void
    SetOnTaskCompletedHandler(OnTaskCompletedHandler handler);
    void
    OnTaskCompleted(const Task& task, TaskQueue::TaskTiming* task_timing, LazyNow* lazy_now);
    bool
    RequiresTaskTiming() const;
    ...
    void
    SetOnTaskPostedHandler(OnTaskPostedHandler handler);

    WeakPtr<SequenceManagerImpl>
    GetSequenceManagerWeakPtr();

    SequenceManagerImpl*
    sequence_manager() const
    {
    ...
    bool
    IsUnregistered() const;

protected:
    void
    SetDelayedWakeUpForTesting(Optional<DelayedWakeUp> wake_up);
...
private:
    ...
    public:
        ...
        bool
        PostTask(PostedTask task);

        void
        StartAcceptingOperations()
        {
    ...
    public:
        ...
        bool
        PostDelayedTask(const Location& location, OnceClosure callback, TimeDelta delay) final;
        bool
        PostNonNestableDelayedTask(const Location& location,
                                   OnceClosure callback,
                                   TimeDelta delay) final;
        bool
        RunsTasksInCurrentSequence() const final;
    ...
    public:
        ...
        DelayedIncomingQueue&
        operator=(const DelayedIncomingQueue&) = delete;
        ~DelayedIncomingQueue();
        ...
        void
        swap(DelayedIncomingQueue* other);
        ...
        void
        SweepCancelledTasks(SequenceManagerImpl* sequence_manager);
    ...
    struct MainThreadOnly
    {
        ...
        EnqueueOrder enqueue_order_at_which_we_became_unblocked_with_normal_priority;
    ...
    void
    PostImmediateTaskImpl(PostedTask task, CurrentThread current_thread);
    void
    PostDelayedTaskImpl(PostedTask task, CurrentThread current_thread);

    // Push the task onto the |delayed_incoming_queue|. Lock-free main thread
    // only fast path.
    void
    PushOntoDelayedIncomingQueueFromMainThread(Task pending_task,
                                               TimeTicks now,
    ...
    // threads.
    void
    PushOntoDelayedIncomingQueue(Task pending_task);

    Optional<DelayedWakeUp>
    GetNextScheduledWakeUpImpl();

    void
    ScheduleDelayedWorkTask(Task pending_task);

    void
    MoveReadyImmediateTasksToImmediateWorkQueueLocked() EXCLUSIVE_LOCKS_REQUIRED(any_thread_lock_);

    // LazilyDeallocatedDeque use TimeTicks to figure out when to resize.  We
    // should use real time here always.
    using TaskDeque = LazilyDeallocatedDeque<Task, subtle::TimeTicksNowIgnoringOverride>;

    // Extracts all the tasks from the immediate incoming queue and swaps it with
    // |queue| which must be empty.
    // Can be called from any thread.
    void
    TakeImmediateIncomingQueueTasks(TaskDeque* queue);

    void
    TraceQueueSize() const;
    static Value
    QueueAsValue(const TaskDeque& queue, TimeTicks now);
    static Value
    TaskAsValue(const Task& task, TimeTicks now);

    // Schedules delayed work on time domain and calls the observer.
    void
    UpdateDelayedWakeUp(LazyNow* lazy_now);
    void
    UpdateDelayedWakeUpImpl(LazyNow* lazy_now, Optional<DelayedWakeUp> wake_up);

    // Activate a delayed fence if a time has come.
    void
    ActivateDelayedFenceIfNeeded(TimeTicks now);

    // Updates state protected by any_thread_lock_.
    void
    UpdateCrossThreadQueueStateLocked() EXCLUSIVE_LOCKS_REQUIRED(any_thread_lock_);

    void
    MaybeLogPostTask(PostedTask* task);
    void
    MaybeAdjustTaskDelay(PostedTask* task, CurrentThread current_thread);
    ...
    void
    MaybeReportIpcTaskQueuedFromMainThread(Task* pending_task, const char* task_queue_name);
    bool
    ShouldReportIpcTaskQueuedFromAnyThreadLocked(base::TimeDelta* time_since_disabled)
            EXCLUSIVE_LOCKS_REQUIRED(any_thread_lock_);
    void
    MaybeReportIpcTaskQueuedFromAnyThreadLocked(Task* pending_task, const char* task_queue_name)
            EXCLUSIVE_LOCKS_REQUIRED(any_thread_lock_);
    void
    MaybeReportIpcTaskQueuedFromAnyThreadUnlocked(Task* pending_task, const char* task_queue_name);
    void
    ReportIpcTaskQueued(Task* pending_task,
