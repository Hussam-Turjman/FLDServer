...
#include "fldserver/base/task/sequence_manager/task_queue_selector_logic.h"
...
class CORE_EXPORT TaskQueueSelector : public WorkQueueSets::Observer
{
public:
    ...
    WorkQueue*
    SelectWorkQueueToService(SelectTaskOption option = SelectTaskOption::kDefault);
    ...
    public:
        ...
        virtual void
        OnTaskQueueEnabled(internal::TaskQueueImpl* queue) = 0;
    };
    ...
    void
    SetTaskQueueSelectorObserver(Observer* observer);

    // Returns the priority of the most important pending task if one exists.
    // O(1).
    Optional<TaskQueue::QueuePriority>
    GetHighestPendingPriority(SelectTaskOption option = SelectTaskOption::kDefault) const;

    // WorkQueueSets::Observer implementation:
    void
    WorkQueueSetBecameEmpty(size_t set_index) override;
    void
    WorkQueueSetBecameNonEmpty(size_t set_index) override;
    ...
    void
    CollectSkippedOverLowerPriorityTasks(const internal::WorkQueue* selected_work_queue,
                                         std::vector<const Task*>* result) const;

protected:
    WorkQueueSets*
    delayed_work_queue_sets()
    {
    ...
    void
    SetImmediateStarvationCountForTest(size_t immediate_starvation_count);
    ...
    class CORE_EXPORT ActivePriorityTracker
    {
    public:
        ActivePriorityTracker();

        bool
        HasActivePriority() const
        {
            return active_priorities_ != 0;
        }

        bool
        IsActive(TaskQueue::QueuePriority priority) const
        {
            return active_priorities_ & (1u << static_cast<size_t>(priority));
        }

        void
        SetActive(TaskQueue::QueuePriority priority, bool is_active);

        TaskQueue::QueuePriority
        HighestActivePriority() const;

    private:
        static_assert(TaskQueue::QueuePriority::kQueuePriorityCount < sizeof(size_t) * 8,
    ...
    {
        static WorkQueue*
        GetWithPriority(const WorkQueueSets& sets, TaskQueue::QueuePriority priority)
        {
            return sets.GetOldestQueueInSet(priority);
        }

        static WorkQueue*
        GetWithPriorityAndEnqueueOrder(const WorkQueueSets& sets,
                                       TaskQueue::QueuePriority priority,
                                       EnqueueOrder* enqueue_order)
        {
    ...
    WorkQueue*
    ChooseWithPriority(TaskQueue::QueuePriority priority) const
    {
        ...
        {
            WorkQueue* queue = ChooseImmediateOnlyWithPriority<SetOperation>(priority);
            if (queue)
                return queue;
    ...
    template <typename SetOperation>
    WorkQueue*
    ChooseImmediateOnlyWithPriority(TaskQueue::QueuePriority priority) const
    {
...
private:
    void
    ChangeSetIndex(internal::TaskQueueImpl* queue, TaskQueue::QueuePriority priority);
    void
    AddQueueImpl(internal::TaskQueueImpl* queue, TaskQueue::QueuePriority priority);
    void
    RemoveQueueImpl(internal::TaskQueueImpl* queue);

#if DCHECK_IS_ON() || !defined(NDEBUG)
    bool
    CheckContainsQueueForTest(const internal::TaskQueueImpl* queue) const;
#endif

    template <typename SetOperation>
    WorkQueue*
    ChooseImmediateOrDelayedTaskWithPriority(TaskQueue::QueuePriority priority) const
    {
    ...
    static TaskQueue::QueuePriority
    NextPriority(TaskQueue::QueuePriority priority);

    // Returns true if there are pending tasks with priority |priority|.
    bool
    HasTasksWithPriority(TaskQueue::QueuePriority priority) const;
