...
namespace internal
{
TaskQueueSelector::TaskQueueSelector(scoped_refptr<AssociatedThreadId> associated_thread,
...
void
TaskQueueSelector::AddQueue(internal::TaskQueueImpl* queue)
{
...
void
TaskQueueSelector::RemoveQueue(internal::TaskQueueImpl* queue)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
void
TaskQueueSelector::EnableQueue(internal::TaskQueueImpl* queue)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    DCHECK(queue->IsQueueEnabled());
...
void
TaskQueueSelector::DisableQueue(internal::TaskQueueImpl* queue)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    DCHECK(!queue->IsQueueEnabled());
    RemoveQueueImpl(queue);
...
void
TaskQueueSelector::SetQueuePriority(internal::TaskQueueImpl* queue,
...
TaskQueueSelector::NextPriority(TaskQueue::QueuePriority priority)
{
...
void
TaskQueueSelector::AddQueueImpl(internal::TaskQueueImpl* queue, TaskQueue::QueuePriority priority)
{
#if DCHECK_IS_ON()
...
void
TaskQueueSelector::ChangeSetIndex(internal::TaskQueueImpl* queue, TaskQueue::QueuePriority priority)
{
#if DCHECK_IS_ON()
    DCHECK(CheckContainsQueueForTest(queue));
#endif
    delayed_work_queue_sets_.ChangeSetIndex(queue->delayed_work_queue(), priority);
    immediate_work_queue_sets_.ChangeSetIndex(queue->immediate_work_queue(), priority);
#if DCHECK_IS_ON()
    DCHECK(CheckContainsQueueForTest(queue));
#endif
}
...
void
TaskQueueSelector::RemoveQueueImpl(internal::TaskQueueImpl* queue)
{
#if DCHECK_IS_ON()
    DCHECK(CheckContainsQueueForTest(queue));
#endif
...
void
TaskQueueSelector::WorkQueueSetBecameEmpty(size_t set_index)
{
    ...
    {
        active_priority_tracker_.SetActive(static_cast<TaskQueue::QueuePriority>(set_index), false);
    }
}

void
TaskQueueSelector::WorkQueueSetBecameNonEmpty(size_t set_index)
{
    non_empty_set_counts_[set_index]++;
    ...
    {
        TaskQueue::QueuePriority priority = static_cast<TaskQueue::QueuePriority>(set_index);
        active_priority_tracker_.SetActive(priority, true);
    }
}

void
TaskQueueSelector::CollectSkippedOverLowerPriorityTasks(
        const internal::WorkQueue* selected_work_queue, std::vector<const Task*>* result) const
{
    delayed_work_queue_sets_.CollectSkippedOverLowerPriorityTasks(selected_work_queue, result);
    immediate_work_queue_sets_.CollectSkippedOverLowerPriorityTasks(selected_work_queue, result);
}

#if DCHECK_IS_ON() || !defined(NDEBUG)
bool
TaskQueueSelector::CheckContainsQueueForTest(const internal::TaskQueueImpl* queue) const
{
    bool contains_delayed_work_queue =
            delayed_work_queue_sets_.ContainsWorkQueueForTest(queue->delayed_work_queue());

    bool contains_immediate_work_queue =
            immediate_work_queue_sets_.ContainsWorkQueueForTest(queue->immediate_work_queue());
...
WorkQueue*
TaskQueueSelector::SelectWorkQueueToService(SelectTaskOption option)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
Value
TaskQueueSelector::AsValue() const
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
void
TaskQueueSelector::SetTaskQueueSelectorObserver(Observer* observer)
{
...
TaskQueueSelector::GetHighestPendingPriority(SelectTaskOption option) const
{
    ...
    TaskQueue::QueuePriority highest_priority = active_priority_tracker_.HighestActivePriority();
    if (option != SelectTaskOption::kSkipDelayedTask)
...
void
TaskQueueSelector::SetImmediateStarvationCountForTest(size_t immediate_starvation_count)
{
    immediate_starvation_count_ = immediate_starvation_count;
}

bool
TaskQueueSelector::HasTasksWithPriority(TaskQueue::QueuePriority priority) const
{
...
void
TaskQueueSelector::ActivePriorityTracker::SetActive(TaskQueue::QueuePriority priority,
                                                    bool is_active)
{
...
{
    DCHECK_NE(active_priorities_, 0u) << "CountTrailingZeroBits(0) has undefined behavior";
    return static_cast<TaskQueue::QueuePriority>(bits::CountTrailingZeroBits(active_priorities_));
}
