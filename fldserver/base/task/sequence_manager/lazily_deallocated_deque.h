...
public:
    ...
    operator=(const LazilyDeallocatedDeque&) = delete;
    ~LazilyDeallocatedDeque()
    {
        clear();
    }
    ...
    push_back(T t)
    {
        ...
        if (!tail_->CanPush())
        {
            ...
            tail_->next_ = std::make_unique<Ring>(2 + tail_->capacity() + (tail_->capacity() / 2));
    ...
    void
    swap(LazilyDeallocatedDeque& other)
    {
    ...
    MaybeShrinkQueue()
    {
        ...
        next_resize_time_ = current_time + TimeDelta::FromSeconds(kMinimumShrinkIntervalInSeconds);
    }

    void
    SetCapacity(size_t new_capacity)
    {
...
*/
    ...
    struct Ring
    {
        ...
        size_t
        CircularDecrement(size_t index) const
        {
            if (index == 0)
                return capacity_ - 1;
            return index - 1;
        ...
        size_t
        CircularIncrement(size_t index) const
        {
