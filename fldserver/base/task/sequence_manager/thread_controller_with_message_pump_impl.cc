...
#if defined(OS_IOS)
#include "fldserver/base/message_loop/message_pump_mac.h"
#elif defined(OS_ANDROID)
#include "fldserver/base/message_loop/message_pump_android.h"
#endif
...
TimeTicks
CapAtOneDay(TimeTicks next_run_time, LazyNow* lazy_now)
{
...
ThreadControllerWithMessagePumpImpl::ThreadControllerWithMessagePumpImpl(
        std::unique_ptr<MessagePump> message_pump, const SequenceManager::Settings& settings) :
...
ThreadControllerWithMessagePumpImpl::CreateUnbound(const SequenceManager::Settings& settings)
{
...
void
ThreadControllerWithMessagePumpImpl::SetSequencedTaskSource(SequencedTaskSource* task_source)
{
    DCHECK(task_source);
...
void
ThreadControllerWithMessagePumpImpl::BindToCurrentThread(std::unique_ptr<MessagePump> message_pump)
{
    ...
    scoped_set_sequence_local_storage_map_for_current_thread_ =
            std::make_unique<base::internal::ScopedSetSequenceLocalStorageMapForCurrentThread>(
    ...
    if (work_deduplicator_.BindToCurrentThread() == ShouldScheduleWork::kScheduleImmediate)
    {
        pump_->ScheduleWork();
    }
}
...
void
ThreadControllerWithMessagePumpImpl::SetWorkBatchSize(int work_batch_size)
{
...
void
ThreadControllerWithMessagePumpImpl::SetTimerSlack(TimerSlack timer_slack)
{
...
void
ThreadControllerWithMessagePumpImpl::WillQueueTask(PendingTask* pending_task,
...
void
ThreadControllerWithMessagePumpImpl::SetNextDelayedDoWork(LazyNow* lazy_now, TimeTicks run_time)
{
    ...
    if (work_deduplicator_.OnDelayedWorkRequested() == ShouldScheduleWork::kScheduleImmediate)
    {
...
const TickClock*
ThreadControllerWithMessagePumpImpl::GetClock()
{
    return time_source_;
}

bool
ThreadControllerWithMessagePumpImpl::RunsTasksInCurrentSequence()
{
    return associated_thread_->IsBoundToCurrentThread();
}

void
ThreadControllerWithMessagePumpImpl::SetDefaultTaskRunner(
...
void
ThreadControllerWithMessagePumpImpl::InitializeThreadTaskRunnerHandle()
{
...
void
ThreadControllerWithMessagePumpImpl::MaybeStartWatchHangsInScope()
{
...
void
ThreadControllerWithMessagePumpImpl::RestoreDefaultTaskRunner()
{
...
void
ThreadControllerWithMessagePumpImpl::AddNestingObserver(RunLoop::NestingObserver* observer)
{
...
void
ThreadControllerWithMessagePumpImpl::RemoveNestingObserver(RunLoop::NestingObserver* observer)
{
...
void
ThreadControllerWithMessagePumpImpl::OnBeginNativeWork()
{
...
void
ThreadControllerWithMessagePumpImpl::OnEndNativeWork()
{
    work_id_provider_->IncrementWorkId();
...
void
ThreadControllerWithMessagePumpImpl::BeforeWait()
{
    work_id_provider_->IncrementWorkId();
...
ThreadControllerWithMessagePumpImpl::DoWork()
{
    ...
    WorkDeduplicator::NextTask next_task = delay_till_next_task.is_zero()
    ...
    main_thread_only().next_delayed_do_work = continuation_lazy_now.Now() + delay_till_next_task;

    // Don't request a run time past |main_thread_only().quit_runloop_after|.
    if (main_thread_only().next_delayed_do_work > main_thread_only().quit_runloop_after)
    {
        main_thread_only().next_delayed_do_work = main_thread_only().quit_runloop_after;
    ...
    return {CapAtOneDay(main_thread_only().next_delayed_do_work, &continuation_lazy_now),
            continuation_lazy_now.Now()};
}

TimeDelta
ThreadControllerWithMessagePumpImpl::DoWorkImpl(LazyNow* continuation_lazy_now)
{
    ...
    for (int i = 0; i < main_thread_only().work_batch_size; i++)
    {
        ...
        main_thread_only().run_level_tracker.OnTaskStarted();
        {
            ...
            AutoReset<bool> ban_nested_application_tasks(&main_thread_only().task_execution_allowed,
                                                         false);
...
bool
ThreadControllerWithMessagePumpImpl::DoIdleWork()
{
...
void
ThreadControllerWithMessagePumpImpl::Run(bool application_tasks_allowed, TimeDelta timeout)
{
    DCHECK(RunsTasksInCurrentSequence());
    ...
    AutoReset<TimeTicks> quit_runloop_after(
            ...
            (timeout == TimeDelta::Max()) ? TimeTicks::Max() : time_source_->NowTicks() + timeout);

    main_thread_only().run_level_tracker.OnRunLoopStarted(RunLevelTracker::kSelectingNextTask);
...
void
ThreadControllerWithMessagePumpImpl::OnBeginNestedRunLoop()
{
...
void
ThreadControllerWithMessagePumpImpl::OnExitNestedRunLoop()
{
    if (main_thread_only().nesting_observer)
...
void
ThreadControllerWithMessagePumpImpl::Quit()
{
    DCHECK(RunsTasksInCurrentSequence());
...
void
ThreadControllerWithMessagePumpImpl::EnsureWorkScheduled()
{
    if (work_deduplicator_.OnWorkRequested() == ShouldScheduleWork::kScheduleImmediate)
    {
        pump_->ScheduleWork();
...
void
ThreadControllerWithMessagePumpImpl::SetTaskExecutionAllowed(bool allowed)
{
    if (allowed)
    {
...
bool
ThreadControllerWithMessagePumpImpl::IsTaskExecutionAllowed() const
{
    return main_thread_only().task_execution_allowed;
}

MessagePump*
ThreadControllerWithMessagePumpImpl::GetBoundMessagePump() const
{
    return pump_.get();
}
...
#if defined(OS_IOS)
void
ThreadControllerWithMessagePumpImpl::AttachToMessagePump()
{
    static_cast<MessagePumpCFRunLoopBase*>(pump_.get())->Attach(this);
}

void
ThreadControllerWithMessagePumpImpl::DetachFromMessagePump()
{
...
bool
ThreadControllerWithMessagePumpImpl::ShouldQuitRunLoopWhenIdle()
{
    if (main_thread_only().run_level_tracker.num_run_levels() == 0)
        return false;
