...
namespace internal
{
WorkQueue::WorkQueue(TaskQueueImpl* task_queue, const char* name, QueueType queue_type) :
    task_queue_(task_queue), name_(name), queue_type_(queue_type)
{
}

Value
WorkQueue::AsValue(TimeTicks now) const
{
...
WorkQueue::~WorkQueue()
{
    DCHECK(!work_queue_sets_) << task_queue_->GetName() << " : " << work_queue_sets_->GetName()
                              << " : " << name_;
}

const Task*
WorkQueue::GetFrontTask() const
{
    if (tasks_.empty())
        return nullptr;
    return &tasks_.front();
...
bool
WorkQueue::BlockedByFence() const
{
    if (!fence_)
        return false;
...
bool
WorkQueue::GetFrontTaskEnqueueOrder(EnqueueOrder* enqueue_order) const
{
    if (tasks_.empty() || BlockedByFence())
        return false;
    // Quick sanity check.
    DCHECK_LE(tasks_.front().enqueue_order(), tasks_.back().enqueue_order())
            << task_queue_->GetName() << " : " << work_queue_sets_->GetName() << " : " << name_;
...
void
WorkQueue::TaskPusher::Push(Task* task)
{
    DCHECK(work_queue_);
...
WorkQueue::TaskPusher::~TaskPusher()
{
    ...
    if (was_empty_ && work_queue_ && !work_queue_->Empty() && work_queue_->work_queue_sets_
        && !work_queue_->BlockedByFence())
    {
        work_queue_->work_queue_sets_->OnTaskPushedToEmptyQueue(work_queue_);
    }
}

WorkQueue::TaskPusher
WorkQueue::CreateTaskPusher()
{
    return TaskPusher(this);
}

void
WorkQueue::PushNonNestableTaskToFront(Task task)
{
...
void
WorkQueue::TakeImmediateIncomingQueueTasks()
{
    DCHECK(tasks_.empty());
...
bool
WorkQueue::RemoveAllCanceledTasksFromFront()
{
    if (!work_queue_sets_)
        return false;
    ...
    while (!tasks_.empty())
    {
        ...
        DEBUG_ALIAS_FOR_CSTR(
                debug_file_name,
                pending_task.posted_from.file_name() ? pending_task.posted_from.file_name() : "",
                16);
        ...
        const void* debug_bind_state = reinterpret_cast<const void*>(&pending_task.task);
...
void
WorkQueue::AssignToWorkQueueSets(WorkQueueSets* work_queue_sets)
{
    work_queue_sets_ = work_queue_sets;
}

void
WorkQueue::AssignSetIndex(size_t work_queue_set_index)
{
    work_queue_set_index_ = work_queue_set_index;
}

bool
WorkQueue::InsertFenceImpl(EnqueueOrder fence)
{
    DCHECK_NE(fence, 0u);
...
void
WorkQueue::InsertFenceSilently(EnqueueOrder fence)
{
...
bool
WorkQueue::InsertFence(EnqueueOrder fence)
{
...
bool
WorkQueue::ShouldRunBefore(const WorkQueue* other_queue) const
{
    DCHECK(!tasks_.empty());
...
void
WorkQueue::MaybeShrinkQueue()
{
    tasks_.MaybeShrinkQueue();
}

void
WorkQueue::PopTaskForTesting()
{
    if (tasks_.empty())
        return;
    tasks_.pop_front();
...
void
WorkQueue::CollectTasksOlderThan(EnqueueOrder reference, std::vector<const Task*>* result) const
{
    for (const Task& task : tasks_)
    {
