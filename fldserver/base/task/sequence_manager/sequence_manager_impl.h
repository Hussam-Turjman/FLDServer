...
#include "fldserver/base/cancelable_callback.h"
#include "fldserver/base/containers/circular_deque.h"
#include "fldserver/base/debug/crash_logging.h"
#include "fldserver/base/memory/scoped_refptr.h"
#include "fldserver/base/memory/weak_ptr.h"
#include "fldserver/base/message_loop/message_pump_type.h"
#include "fldserver/base/pending_task.h"
#include "fldserver/base/run_loop.h"
...
#include "fldserver/base/task/common/task_annotator.h"
#include "fldserver/base/task/current_thread.h"
#include "fldserver/base/task/sequence_manager/associated_thread_id.h"
#include "fldserver/base/task/sequence_manager/enqueue_order.h"
#include "fldserver/base/task/sequence_manager/enqueue_order_generator.h"
#include "fldserver/base/task/sequence_manager/sequence_manager.h"
#include "fldserver/base/task/sequence_manager/task_queue_impl.h"
#include "fldserver/base/task/sequence_manager/task_queue_selector.h"
#include "fldserver/base/task/sequence_manager/thread_controller.h"
...
//
class CORE_EXPORT SequenceManagerImpl : public SequenceManager,
...
public:
    ...
    SequenceManagerImpl&
    operator=(const SequenceManagerImpl&) = delete;
    ...
    CreateOnCurrentThread(SequenceManager::Settings settings = SequenceManager::Settings());
    ...
    // methods.
    static std::unique_ptr<SequenceManagerImpl>
    CreateUnbound(SequenceManager::Settings settings);

    // SequenceManager implementation:
    void
    BindToCurrentThread() override;
    scoped_refptr<SequencedTaskRunner>
    GetTaskRunnerForCurrentTask() override;
    void
    BindToMessagePump(std::unique_ptr<MessagePump> message_pump) override;
    void
    SetObserver(Observer* observer) override;
    void
    AddTaskTimeObserver(TaskTimeObserver* task_time_observer) override;
    void
    RemoveTaskTimeObserver(TaskTimeObserver* task_time_observer) override;
    void
    RegisterTimeDomain(TimeDomain* time_domain) override;
    void
    UnregisterTimeDomain(TimeDomain* time_domain) override;
    TimeDomain*
    GetRealTimeDomain() const override;
    const TickClock*
    GetTickClock() const override;
    TimeTicks
    NowTicks() const override;
    void
    SetDefaultTaskRunner(scoped_refptr<SingleThreadTaskRunner> task_runner) override;
    void
    ReclaimMemory() override;
    bool
    GetAndClearSystemIsQuiescentBit() override;
    void
    SetWorkBatchSize(int work_batch_size) override;
    void
    SetTimerSlack(TimerSlack timer_slack) override;
    void
    EnableCrashKeys(const char* async_stack_crash_key) override;
    const MetricRecordingSettings&
    GetMetricRecordingSettings() const override;
    size_t
    GetPendingTaskCountForTesting() const override;
    scoped_refptr<TaskQueue>
    CreateTaskQueue(const TaskQueue::Spec& spec) override;
    std::string
    DescribeAllPendingTasks() const override;
    std::unique_ptr<NativeWorkHandle>
    OnNativeWorkPending(TaskQueue::QueuePriority priority) override;
    void
    AddTaskObserver(TaskObserver* task_observer) override;
    void
    RemoveTaskObserver(TaskObserver* task_observer) override;

    // SequencedTaskSource implementation:
    Task*
    SelectNextTask(SelectTaskOption option = SelectTaskOption::kDefault) override;
    void
    DidRunTask() override;
    TimeDelta
    DelayTillNextTask(LazyNow* lazy_now,
                      SelectTaskOption option = SelectTaskOption::kDefault) const override;
    bool
    HasPendingHighResolutionTasks() override;
    bool
    OnSystemIdle() override;

    void
    AddDestructionObserver(CurrentThread::DestructionObserver* destruction_observer);
    void
    RemoveDestructionObserver(CurrentThread::DestructionObserver* destruction_observer);
    ...
    MessagePump*
    GetMessagePump() const;
    bool
    IsType(MessagePumpType type) const;
    void
    SetAddQueueTimeToTasks(bool enable);
    void
    SetTaskExecutionAllowed(bool allowed);
    bool
    IsTaskExecutionAllowed() const;
#if defined(OS_IOS)
    void
    AttachToMessagePump();
#endif
    bool
    IsIdleForTesting() override;
    void
    BindToCurrentThread(std::unique_ptr<MessagePump> pump);
    MessagePumpType
    GetType() const;
    ...
    void
    SetNextDelayedDoWork(LazyNow* lazy_now, TimeTicks run_time);
    ...
    currently_executing_task_queue() const;

    // Unregisters a TaskQueue previously created by |NewTaskQueue()|.
    // No tasks will run on this queue after this call.
    void
    UnregisterTaskQueueImpl(std::unique_ptr<internal::TaskQueueImpl> task_queue);

    // Schedule a call to UnregisterTaskQueueImpl as soon as it's safe to do so.
    void
    ShutdownTaskQueueGracefully(std::unique_ptr<internal::TaskQueueImpl> task_queue);

    const scoped_refptr<AssociatedThreadId>&
    associated_thread() const
    {
    ...
    static constexpr TimeDelta kReclaimMemoryInterval = TimeDelta::FromSeconds(30);
...
private:
    ...
    using NonNestableTaskDeque = circular_deque<internal::TaskQueueImpl::DeferredNonNestableTask>;
    ...
    {
        explicit MainThreadOnly(const scoped_refptr<AssociatedThreadId>& associated_thread,
        ...
        std::array<char, static_cast<size_t>(debug::CrashKeySize::Size64)> async_stack_buffer = {};
        ...
        ObserverList<CurrentThread::DestructionObserver>::Unchecked destruction_observers;

        // By default native work is not prioritized at all.
        std::multiset<TaskQueue::QueuePriority> pending_native_work{TaskQueue::kBestEffortPriority};
    };
    ...
    void
    OnTaskQueueEnabled(internal::TaskQueueImpl* queue) override;

    // RunLoop::NestingObserver:
    void
    OnBeginNestedRunLoop() override;
    void
    OnExitNestedRunLoop() override;
    ...
    void
    WillQueueTask(Task* pending_task, const char* task_queue_name);

    // Delayed Tasks with run_times <= Now() are enqueued onto the work queue and
    // reloads any empty work queues.
    void
    MoveReadyDelayedTasksToWorkQueues(LazyNow* lazy_now);

    void
    NotifyWillProcessTask(ExecutingTask* task, LazyNow* time_before_task);
    void
    NotifyDidProcessTask(ExecutingTask* task, LazyNow* time_after_task);
    ...
    Value
    AsValueWithSelectorResult(internal::WorkQueue* selected_work_queue, bool force_verbose) const;
    ...
    GetFlagToRequestReloadForEmptyQueue(TaskQueueImpl* task_queue);

    // Calls |TakeImmediateIncomingQueueTasks| on all queues with their reload
    // flag set in |empty_queues_to_reload_|.
    void
    ReloadEmptyWorkQueues() const;

    std::unique_ptr<internal::TaskQueueImpl>
    CreateTaskQueueImpl(const TaskQueue::Spec& spec) override;
    ...
    TaskQueue::TaskTiming::TimeRecordingPolicy
    ShouldRecordTaskTiming(const internal::TaskQueueImpl* task_queue);
    bool
    ShouldRecordCPUTimeForTask();
    void
    RecordCrashKeys(const PendingTask&);

    // Helper to terminate all scoped trace events to allow starting new ones
    // in SelectNextTask().
    Task*
    SelectNextTaskImpl(SelectTaskOption option);

    // Check if a task of priority |priority| should run given the pending set of
    // native work.
    bool
    ShouldRunTaskOfPriority(TaskQueue::QueuePriority priority) const;

    // Ignores any immediate work.
    TimeDelta
    GetDelayTillNextDelayedTask(LazyNow* lazy_now, SelectTaskOption option) const;

#if DCHECK_IS_ON()
    void
    LogTaskDebugInfo(const internal::WorkQueue* work_queue) const;
#endif

    // Determines if wall time or thread time should be recorded for the next
    // task.
    TaskQueue::TaskTiming
    InitializeTaskTiming(internal::TaskQueueImpl* task_queue);
    ...
    const MainThreadOnly&
    main_thread_only() const
    {
        DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
        return main_thread_only_;
    }
