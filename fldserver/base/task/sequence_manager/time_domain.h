...
class CORE_EXPORT TimeDomain
{
public:
    TimeDomain(const TimeDomain&) = delete;
    TimeDomain&
    operator=(const TimeDomain&) = delete;
    virtual ~TimeDomain();
    ...
    virtual Optional<TimeDelta>
    DelayTillNextTask(LazyNow* lazy_now) = 0;
    ...
    virtual bool
    MaybeFastForwardToNextTask(bool quit_when_idle_requested) = 0;
...
protected:
    ...
    Optional<TimeTicks>
    NextScheduledRunTime() const;

    size_t
    NumberOfScheduledWakeUps() const
    {
    ...
    virtual void
    OnRegisterWithSequenceManager(internal::SequenceManagerImpl* sequence_manager);
...
private:
    ...
    void
    SetNextWakeUpForQueue(internal::TaskQueueImpl* queue,
                          Optional<internal::DelayedWakeUp> wake_up,
                          LazyNow* lazy_now);

    // Remove the TaskQueue from any internal data sctructures.
    void
    UnregisterQueue(internal::TaskQueueImpl* queue);
    ...
    struct ScheduledDelayedWakeUp
    {
        ...
        bool
        operator<=(const ScheduledDelayedWakeUp& other) const
        {
