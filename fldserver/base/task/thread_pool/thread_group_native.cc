...
#include "fldserver/base/task/thread_pool/thread_group_native.h"
...
namespace internal
{
class ThreadGroupNative::ScopedCommandsExecutor : public ThreadGroup::BaseScopedCommandsExecutor
{
public:
    ...
    void
    set_num_threadpool_work_to_submit(size_t num)
    {
...
ThreadGroupNative::ThreadGroupNative(TrackedRef<TaskTracker> task_tracker,
    ...
    ThreadGroup(std::move(task_tracker), std::move(delegate), predecessor_thread_group)
{
}
...
void
ThreadGroupNative::Start(WorkerEnvironment worker_environment)
{
    ThreadGroup::Start();
...
void
ThreadGroupNative::JoinForTesting()
{
    {
        CheckedAutoLock auto_lock(lock_);
...
void
ThreadGroupNative::RunNextTaskSourceImpl()
{
    ...
    if (task_source)
    {
        ...
        if (task_source)
        {
            ...
            CheckedAutoLock auto_lock(lock_);
            ReEnqueueTaskSourceLockRequired(&workers_executor,
                                            &reenqueue_executor,
...
void
ThreadGroupNative::UpdateMinAllowedPriorityLockRequired()
{
...
void
ThreadGroupNative::UpdateSortKey(TaskSource::Transaction transaction)
{
    ScopedCommandsExecutor executor(this);
...
void
ThreadGroupNative::PushTaskSourceAndWakeUpWorkers(
...
void
ThreadGroupNative::EnsureEnoughWorkersLockRequired(BaseScopedCommandsExecutor* executor)
{
    if (!started_)
        return;
    ...
    {
        static_cast<ScopedCommandsExecutor*>(executor)->set_num_threadpool_work_to_submit(
...
size_t
ThreadGroupNative::GetMaxConcurrentNonBlockedTasksDeprecated() const
{
...
void
ThreadGroupNative::DidUpdateCanRunPolicy()
{
    ScopedCommandsExecutor executor(this);
    CheckedAutoLock auto_lock(lock_);
    EnsureEnoughWorkersLockRequired(&executor);
