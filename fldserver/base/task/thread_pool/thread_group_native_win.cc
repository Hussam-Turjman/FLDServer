...
namespace internal
{
class ThreadGroupNativeWin::ScopedCallbackMayRunLongObserver : public BlockingObserver
{
public:
    explicit ScopedCallbackMayRunLongObserver(PTP_CALLBACK_INSTANCE callback) : callback_(callback)
    {
        SetBlockingObserverForCurrentThread(this);
    }

    ScopedCallbackMayRunLongObserver(const ScopedCallbackMayRunLongObserver&) = delete;
    ScopedCallbackMayRunLongObserver&
    operator=(const ScopedCallbackMayRunLongObserver&) = delete;
...
ThreadGroupNativeWin::ThreadGroupNativeWin(TrackedRef<TaskTracker> task_tracker,
    ...
    ThreadGroupNative(std::move(task_tracker), std::move(delegate), predecessor_thread_group)
{
}
...
void
ThreadGroupNativeWin::StartImpl()
{
...
void
ThreadGroupNativeWin::JoinImpl()
{
    ::WaitForThreadpoolWorkCallbacks(work_, true);
}

void
ThreadGroupNativeWin::SubmitWork()
{
...
ThreadGroupNativeWin::RunNextTaskSource(PTP_CALLBACK_INSTANCE callback_instance,
                                        ...
                                        PTP_WORK)
{
    auto* thread_group = static_cast<ThreadGroupNativeWin*>(thread_group_windows_impl);
    ...
    auto win_thread_environment =
            thread_group->GetScopedWindowsThreadEnvironment(thread_group->worker_environment_);

    ScopedCallbackMayRunLongObserver callback_may_run_long_observer(callback_instance);
