...
// process's instance.
class CORE_EXPORT ThreadPoolInstance
{
public:
    struct CORE_EXPORT InitParams
    {
    ...
    class CORE_EXPORT ScopedExecutionFence
    {
    public:
        ScopedExecutionFence();
        ScopedExecutionFence(const ScopedExecutionFence&) = delete;
        ScopedExecutionFence&
        operator=(const ScopedExecutionFence&) = delete;
        ~ScopedExecutionFence();
    };

    class CORE_EXPORT ScopedBestEffortExecutionFence
    {
    public:
        ScopedBestEffortExecutionFence();
        ScopedBestEffortExecutionFence(const ScopedBestEffortExecutionFence&) = delete;
        ScopedBestEffortExecutionFence&
        operator=(const ScopedBestEffortExecutionFence&) = delete;
        ~ScopedBestEffortExecutionFence();
    };
    ...
    // CHECKs on failure.
    virtual void
    Start(const InitParams& init_params,
    ...
    virtual void
    FlushAsyncForTesting(OnceClosure flush_callback) = 0;
    ...
    static void
    CreateAndStartWithDefaultParams(StringPiece name);
    ...
    // (ensures isolation).
    static void
    Set(std::unique_ptr<ThreadPoolInstance> thread_pool);
...
private:
    ...
    virtual int
    GetMaxConcurrentNonBlockedTasksWithTraitsDeprecated(const TaskTraits& traits) const = 0;
    ...
    EndFence() = 0;
    virtual void
    BeginBestEffortFence() = 0;
    virtual void
    EndBestEffortFence() = 0;
};
