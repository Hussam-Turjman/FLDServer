...
struct CORE_EXPORT ExecutionEnvironment
{
    SequenceToken token;
...
class CORE_EXPORT TaskSource : public RefCountedThreadSafe<TaskSource>
{
public:
    ...
    class CORE_EXPORT Transaction
    {
    public:
        Transaction(Transaction&& other);
        Transaction(const Transaction&) = delete;
        Transaction&
        operator=(const Transaction&) = delete;
        ~Transaction();
        ...
        void
        UpdatePriority(TaskPriority priority);

        // Returns the traits of all Tasks in the TaskSource.
        TaskTraits
        traits() const
        {
            return task_source_->traits_;
        }
    ...
    TaskSource&
    operator=(const TaskSource&) = delete;

    // Begins a Transaction. This method cannot be called on a thread which has an
    // active TaskSource::Transaction.
    Transaction
    BeginTransaction() WARN_UNUSED_RESULT;

    virtual ExecutionEnvironment
    GetExecutionEnvironment() = 0;
    ...
    virtual size_t
    GetRemainingConcurrency() const = 0;

    // Returns a TaskSourceSortKey representing the priority of the TaskSource.
    virtual TaskSourceSortKey
    GetSortKey(bool disable_fair_scheduling) const = 0;

    // Support for IntrusiveHeap.
    void
    SetHeapHandle(const HeapHandle& handle);
    void
    ClearHeapHandle();
    HeapHandle
    GetHeapHandle() const
...
protected:
    ...
    virtual Task
    TakeTask(TaskSource::Transaction* transaction) = 0;
    virtual bool
    DidProcessTask(TaskSource::Transaction* transaction) = 0;
    ...
    virtual Task
    Clear(TaskSource::Transaction* transaction) = 0;
...
class CORE_EXPORT RegisteredTaskSource
{
public:
    RegisteredTaskSource();
    ...
    RegisteredTaskSource&
    operator=(const RegisteredTaskSource&) = delete;
    ~RegisteredTaskSource();

    RegisteredTaskSource&
    operator=(RegisteredTaskSource&& other);
    ...
    static RegisteredTaskSource
    CreateForTesting(scoped_refptr<TaskSource> task_source, TaskTracker* task_tracker = nullptr);
    ...
    // a transaction.
    Task
    TakeTask(TaskSource::Transaction* transaction = nullptr) WARN_UNUSED_RESULT;
    ...
    bool
    DidProcessTask(TaskSource::Transaction* transaction = nullptr);
    ...
    // of a transaction.
    Task
    Clear(TaskSource::Transaction* transaction = nullptr) WARN_UNUSED_RESULT;

private:
    friend class TaskTracker;
    RegisteredTaskSource(scoped_refptr<TaskSource> task_source, TaskTracker* task_tracker);
...
struct CORE_EXPORT TransactionWithRegisteredTaskSource
{
public:
    TransactionWithRegisteredTaskSource(RegisteredTaskSource task_source_in,
                                        TaskSource::Transaction transaction_in);

    TransactionWithRegisteredTaskSource(TransactionWithRegisteredTaskSource&& other) = default;
    TransactionWithRegisteredTaskSource(const TransactionWithRegisteredTaskSource&) = delete;
    TransactionWithRegisteredTaskSource&
    operator=(const TransactionWithRegisteredTaskSource&) = delete;
    ~TransactionWithRegisteredTaskSource() = default;

    static TransactionWithRegisteredTaskSource
    FromTaskSource(RegisteredTaskSource task_source_in);
