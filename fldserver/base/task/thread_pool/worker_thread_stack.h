...
class CORE_EXPORT WorkerThreadStack
{
public:
    WorkerThreadStack();
    WorkerThreadStack(const WorkerThreadStack&) = delete;
    WorkerThreadStack&
    operator=(const WorkerThreadStack&) = delete;
    ~WorkerThreadStack();
    ...
    bool
    Contains(const WorkerThread* worker) const;

    // Removes |worker| from the stack. Must not be invoked for the first worker
    // on the stack.
    void
    Remove(const WorkerThread* worker);
