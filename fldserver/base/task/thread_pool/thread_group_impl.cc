...
#include "fldserver/base/atomicops.h"
...
#include "fldserver/base/threading/scoped_blocking_call_internal.h"
#include "fldserver/base/threading/thread_checker.h"
#include "fldserver/base/threading/thread_restrictions.h"
#include "fldserver/base/time/time_override.h"
...
#if IS_WIN
#include "fldserver/base/win/scoped_com_initializer.h"
#include "fldserver/base/win/scoped_windows_thread_environment.h"
#include "fldserver/base/win/scoped_winrt_initializer.h"
...
namespace
{
constexpr char kNumTasksBeforeDetachHistogramPrefix[] = "ThreadPool.NumTasksBeforeDetach.";
...
// BlockedWorkersPoll.
constexpr TimeDelta kForegroundMayBlockThreshold = TimeDelta::FromMilliseconds(1000);
constexpr TimeDelta kForegroundBlockedWorkersPoll = TimeDelta::FromMilliseconds(1200);
...
bool
ContainsWorker(const std::vector<scoped_refptr<WorkerThread>>& workers, const WorkerThread* worker)
{
    auto it = ranges::find_if(workers, [worker](const scoped_refptr<WorkerThread>& i) {
        return i.get() == worker;
    });
...
class ThreadGroupImpl::ScopedCommandsExecutor : public ThreadGroup::BaseScopedCommandsExecutor
{
public:
    ...
    operator=(const ScopedCommandsExecutor&) = delete;
    ~ScopedCommandsExecutor()
    {
        FlushImpl();
    }
    ...
    void
    ScheduleWakeUp(scoped_refptr<WorkerThread> worker)
    {
        workers_to_wake_up_.AddWorker(std::move(worker));
    }

    void
    ScheduleStart(scoped_refptr<WorkerThread> worker)
    {
        workers_to_start_.AddWorker(std::move(worker));
    }

    void
    FlushWorkerCreation(CheckedLock* held_lock)
    {
    ...
    void
    ScheduleAddHistogramSample(HistogramBase* histogram, HistogramBase::Sample sample)
    {
...
private:
    ...
    public:
        ...
        WorkerContainer&
        operator=(const WorkerContainer&) = delete;

        void
        AddWorker(scoped_refptr<WorkerThread> worker)
        {
            if (!worker)
                return;
            if (!first_worker_)
    ...
    FlushImpl()
    {
        ...
        // Wake up workers.
        workers_to_wake_up_.ForEachWorker([](WorkerThread* worker) { worker->WakeUp(); });
        ...
        {
            DCHECK_LE(scheduled_histogram_samples_->size(), kHistogramSampleStackSize);
    ...
    StackVector<std::pair<HistogramBase*, HistogramBase::Sample>, kHistogramSampleStackSize>
            scheduled_histogram_samples_;
};

// static
constexpr size_t ThreadGroupImpl::ScopedCommandsExecutor::kHistogramSampleStackSize;
...
public:
    ...
    WorkerThreadDelegateImpl&
    operator=(const WorkerThreadDelegateImpl&) = delete;
    ...
    GetThreadLabel() const override;
    void
    OnMainEntry(const WorkerThread* worker) override;
    RegisteredTaskSource
    GetWork(WorkerThread* worker) override;
    void
    DidProcessTask(RegisteredTaskSource task_source) override;
    TimeDelta
    GetSleepTimeout() override;
    void
    OnMainExit(WorkerThread* worker) override;

    // BlockingObserver:
    void
    BlockingStarted(BlockingType blocking_type) override;
    void
    BlockingTypeUpgraded() override;
    void
    BlockingEnded() override;

    // Returns true iff the worker can get work. Cleans up the worker or puts it
    // on the idle stack if it can't get work.
    bool
    CanGetWorkLockRequired(ScopedCommandsExecutor* executor, WorkerThread* worker)
    ...
    void
    MaybeIncrementMaxTasksLockRequired() EXCLUSIVE_LOCKS_REQUIRED(outer_->lock_);

    TaskPriority
    current_task_priority_lock_required() const EXCLUSIVE_LOCKS_REQUIRED(outer_->lock_)
    {
    ...
    const CheckedLock&
    lock() const LOCK_RETURNED(outer_->lock_)
    {
        return outer_->lock_;
    }
...
private:
    ...
    bool
    CanCleanupLockRequired(const WorkerThread* worker) const
            EXCLUSIVE_LOCKS_REQUIRED(outer_->lock_);
    ...
    // true.
    void
    CleanupLockRequired(ScopedCommandsExecutor* executor, WorkerThread* worker)
            EXCLUSIVE_LOCKS_REQUIRED(outer_->lock_);

    // Called in GetWork() when a worker becomes idle.
    void
    OnWorkerBecomesIdleLockRequired(WorkerThread* worker) EXCLUSIVE_LOCKS_REQUIRED(outer_->lock_);
    ...
    WriteWorkerReadAny&
    write_worker() EXCLUSIVE_LOCKS_REQUIRED(outer_->lock_)
    {
        DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
        return write_worker_read_any_;
    }

    const WriteWorkerReadAny&
    read_any() const EXCLUSIVE_LOCKS_REQUIRED(outer_->lock_)
    {
        return write_worker_read_any_;
    }
    ...
    bool incremented_max_best_effort_tasks_since_blocked_ GUARDED_BY(outer_->lock_) = false;
...
void
ThreadGroupImpl::Start(int max_tasks,
                       ...
                       Optional<TimeDelta> may_block_threshold)
{
    ...
    in_start().may_block_without_delay = FeatureList::IsEnabled(kMayBlockWithoutDelay);
    ...
    in_start().blocked_workers_poll_period = priority_hint_ == ThreadPriority::NORMAL
...
void
ThreadGroupImpl::UpdateSortKey(TaskSource::Transaction transaction)
{
    ScopedCommandsExecutor executor(this);
...
void
ThreadGroupImpl::PushTaskSourceAndWakeUpWorkers(
        TransactionWithRegisteredTaskSource transaction_with_task_source)
{
    ScopedCommandsExecutor executor(this);
    PushTaskSourceAndWakeUpWorkersImpl(&executor, std::move(transaction_with_task_source));
}

size_t
ThreadGroupImpl::GetMaxConcurrentNonBlockedTasksDeprecated() const
{
#if DCHECK_IS_ON()
    CheckedAutoLock auto_lock(lock_);
    DCHECK_NE(after_start().initial_max_tasks, 0U) << "GetMaxConcurrentTasksDeprecated() should "
...
void
ThreadGroupImpl::WaitForWorkersIdleForTesting(size_t n)
{
    CheckedAutoLock auto_lock(lock_);
...
void
ThreadGroupImpl::WaitForAllWorkersIdleForTesting()
{
    CheckedAutoLock auto_lock(lock_);
...
void
ThreadGroupImpl::WaitForWorkersCleanedUpForTesting(size_t n)
{
    CheckedAutoLock auto_lock(lock_);
...
void
ThreadGroupImpl::JoinForTesting()
{
    decltype(workers_) workers_copy;
    {
        ...
        DCHECK_GT(workers_.size(), size_t(0)) << "Joined an unstarted thread group.";
...
size_t
ThreadGroupImpl::NumberOfWorkersForTesting() const
{
    CheckedAutoLock auto_lock(lock_);
...
size_t
ThreadGroupImpl::GetMaxTasksForTesting() const
{
    CheckedAutoLock auto_lock(lock_);
    return max_tasks_;
}
...
size_t
ThreadGroupImpl::GetMaxBestEffortTasksForTesting() const
{
    CheckedAutoLock auto_lock(lock_);
...
size_t
ThreadGroupImpl::NumberOfIdleWorkersForTesting() const
{
    CheckedAutoLock auto_lock(lock_);
...
void
ThreadGroupImpl::WorkerThreadDelegateImpl::OnMainEntry(const WorkerThread* worker)
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
    ...
    worker_only().win_thread_environment =
            GetScopedWindowsThreadEnvironment(outer_->after_start().worker_environment);
#endif  // IS_WIN
...
ThreadGroupImpl::WorkerThreadDelegateImpl::GetWork(WorkerThread* worker)
{
    ...
    if (!outer_->after_start().wakeup_after_getwork
        && outer_->after_start().wakeup_strategy != WakeUpStrategy::kCentralizedWakeUps)
    {
    ...
    while (!task_source && !outer_->priority_queue_.IsEmpty())
    {
        ...
        if (!outer_->task_tracker_->CanRunPriority(priority)
            || (priority == TaskPriority::BEST_EFFORT
                && outer_->num_running_best_effort_tasks_ >= outer_->max_best_effort_tasks_))
        {
            break;
        }
...
void
ThreadGroupImpl::WorkerThreadDelegateImpl::DidProcessTask(RegisteredTaskSource task_source)
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
    DCHECK(worker_only().is_running_task);
    ...
    if (task_source)
    {
        transaction_with_task_source.emplace(
                TransactionWithRegisteredTaskSource::FromTaskSource(std::move(task_source)));
    }
    ...
    // Running task bookkeeping.
    outer_->DecrementTasksRunningLockRequired(*read_worker().current_task_priority);
    worker_only().is_running_task = false;

    if (transaction_with_task_source)
    {
        outer_->ReEnqueueTaskSourceLockRequired(&workers_executor,
                                                &reenqueue_executor,
...
TimeDelta
ThreadGroupImpl::WorkerThreadDelegateImpl::GetSleepTimeout()
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
...
bool
ThreadGroupImpl::WorkerThreadDelegateImpl::CanCleanupLockRequired(const WorkerThread* worker) const
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
...
void
ThreadGroupImpl::WorkerThreadDelegateImpl::CleanupLockRequired(ScopedCommandsExecutor* executor,
                                                               WorkerThread* worker)
{
    ...
    {
        executor->ScheduleAddHistogramSample(outer_->num_tasks_before_detach_histogram_,
                                             worker_only().num_tasks_since_last_detach);
    }
    worker->Cleanup();
...
void
ThreadGroupImpl::WorkerThreadDelegateImpl::OnWorkerBecomesIdleLockRequired(WorkerThread* worker)
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
...
void
ThreadGroupImpl::WorkerThreadDelegateImpl::OnMainExit(WorkerThread* worker)
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
...
void
ThreadGroupImpl::WorkerThreadDelegateImpl::BlockingStarted(BlockingType blocking_type)
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
    DCHECK(worker_only().is_running_task);
...
void
ThreadGroupImpl::WorkerThreadDelegateImpl::BlockingTypeUpgraded()
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
    DCHECK(worker_only().is_running_task);
...
void
ThreadGroupImpl::WorkerThreadDelegateImpl::BlockingEnded()
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
    DCHECK(worker_only().is_running_task);
                                                                  ...
                                                                  WorkerThread* worker)
{
    ...
    // the idle stack.
    const bool is_on_idle_workers_stack = outer_->idle_workers_stack_.Peek() == worker
                                          || !worker->GetLastUsedTime().is_null();
    DCHECK_EQ(is_on_idle_workers_stack, outer_->idle_workers_stack_.Contains(worker));
...
void
ThreadGroupImpl::WorkerThreadDelegateImpl::MaybeIncrementMaxTasksLockRequired()
{
...
void
ThreadGroupImpl::WaitForWorkersIdleLockRequiredForTesting(size_t n)
{
...
void
ThreadGroupImpl::MaintainAtLeastOneIdleWorkerLockRequired(ScopedCommandsExecutor* executor)
{
    ...
    scoped_refptr<WorkerThread> new_worker = CreateAndRegisterWorkerLockRequired(executor);
    DCHECK(new_worker);
    idle_workers_stack_.Push(new_worker.get());
}

scoped_refptr<WorkerThread>
ThreadGroupImpl::CreateAndRegisterWorkerLockRequired(ScopedCommandsExecutor* executor)
{
    ...
    scoped_refptr<WorkerThread> worker = MakeRefCounted<WorkerThread>(
            priority_hint_,
            std::make_unique<WorkerThreadDelegateImpl>(tracked_ref_factory_.GetTrackedRef()),
            task_tracker_,
            &lock_);
...
size_t
ThreadGroupImpl::GetNumAwakeWorkersLockRequired() const
{
...
size_t
ThreadGroupImpl::GetDesiredNumAwakeWorkersLockRequired() const
{
    ...
    return std::min({workers_for_best_effort_task_sources + workers_for_foreground_task_sources,
                     max_tasks_,
                     kMaxNumberOfWorkers});
}

void
ThreadGroupImpl::DidUpdateCanRunPolicy()
{
    ScopedCommandsExecutor executor(this);
    CheckedAutoLock auto_lock(lock_);
    EnsureEnoughWorkersLockRequired(&executor);
...
void
ThreadGroupImpl::EnsureEnoughWorkersLockRequired(BaseScopedCommandsExecutor* base_executor)
{
    // Don't do anything if the thread group isn't started.
    if (max_tasks_ == 0 || UNLIKELY(join_for_testing_started_))
        return;

    ScopedCommandsExecutor* executor = static_cast<ScopedCommandsExecutor*>(base_executor);

    const size_t desired_num_awake_workers = GetDesiredNumAwakeWorkersLockRequired();
    const size_t num_awake_workers = GetNumAwakeWorkersLockRequired();

    size_t num_workers_to_wake_up = ClampSub(desired_num_awake_workers, num_awake_workers);
    ...
    else if (after_start().wakeup_strategy == WakeUpStrategy::kSerializedWakeUps)
    {
...
ThreadGroupImpl::AdjustMaxTasks()
{
    DCHECK(after_start().service_thread_task_runner->RunsTasksInCurrentSequence());
...
void
ThreadGroupImpl::ScheduleAdjustMaxTasks()
{
    ...
    after_start().service_thread_task_runner->PostDelayedTask(
            FROM_HERE,
            BindOnce(&ThreadGroupImpl::AdjustMaxTasks, Unretained(this)),
            after_start().blocked_workers_poll_period);
}

void
ThreadGroupImpl::MaybeScheduleAdjustMaxTasksLockRequired(ScopedCommandsExecutor* executor)
{
    if (!adjust_max_tasks_posted_ && ShouldPeriodicallyAdjustMaxTasksLockRequired())
    {
...
bool
ThreadGroupImpl::ShouldPeriodicallyAdjustMaxTasksLockRequired()
{
    ...
    const size_t num_running_or_queued_task_sources =
            num_running_tasks_ + GetNumAdditionalWorkersForBestEffortTaskSourcesLockRequired()
            + GetNumAdditionalWorkersForForegroundTaskSourcesLockRequired();
...
void
ThreadGroupImpl::UpdateMinAllowedPriorityLockRequired()
{
...
void
ThreadGroupImpl::DecrementTasksRunningLockRequired(TaskPriority priority)
{
...
void
ThreadGroupImpl::IncrementTasksRunningLockRequired(TaskPriority priority)
{
    ++num_running_tasks_;
...
void
ThreadGroupImpl::DecrementMaxTasksLockRequired()
{
    DCHECK_GT(num_running_tasks_, 0U);
...
void
ThreadGroupImpl::IncrementMaxTasksLockRequired()
{
    DCHECK_GT(num_running_tasks_, 0U);
    ++max_tasks_;
    UpdateMinAllowedPriorityLockRequired();
...
void
ThreadGroupImpl::DecrementMaxBestEffortTasksLockRequired()
{
    DCHECK_GT(num_running_tasks_, 0U);
...
void
ThreadGroupImpl::IncrementMaxBestEffortTasksLockRequired()
{
    DCHECK_GT(num_running_tasks_, 0U);
    ++max_best_effort_tasks_;
    UpdateMinAllowedPriorityLockRequired();
