...
#include "fldserver/base/task/thread_pool/worker_thread_observer.h"
...
#if IS_APPLE
#include "fldserver/base/mac/scoped_nsautorelease_pool.h"
#endif

#if 0 && defined(PA_THREAD_CACHE_SUPPORTED)
#include "fldserver/base/allocator/partition_allocator/thread_cache.h"
#endif
...
void
WorkerThread::Delegate::WaitForWork(WaitableEvent* wake_up_event)
{
    ...
    // many times.
#if 0 && defined(PA_THREAD_CACHE_SUPPORTED)
    bool was_signaled = wake_up_event->TimedWait(std::min(sleep_time, kPurgeThreadCacheIdleDelay));
    ...
    if (!was_signaled)
    {
        ...
        {
            wake_up_event->TimedWait(sleep_time.is_max() ? base::TimeDelta::Max()
...
bool
WorkerThread::Start(WorkerThreadObserver* worker_thread_observer)
{
    ...
    PlatformThread::CreateWithPriority(
            kDefaultStackSize, this, &thread_handle_, current_thread_priority_);
...
bool
WorkerThread::ThreadAliveForTesting() const
{
    CheckedAutoLock auto_lock(thread_lock_);
...
void
WorkerThread::BeginUnusedPeriod()
{
    CheckedAutoLock auto_lock(thread_lock_);
...
TimeTicks
WorkerThread::GetLastUsedTime() const
{
    CheckedAutoLock auto_lock(thread_lock_);
...
ThreadPriority
WorkerThread::GetDesiredThreadPriority() const
{
...
void
WorkerThread::UpdateThreadPriority(ThreadPriority desired_thread_priority)
{
...
WorkerThread::ThreadMain()
{
    if (priority_hint_ == ThreadPriority::BACKGROUND)
    {
        ...
        {
            case ThreadLabel::POOLED: RunBackgroundPooledWorker(); return;
            case ThreadLabel::SHARED: RunBackgroundSharedWorker(); return;
            case ThreadLabel::DEDICATED: RunBackgroundDedicatedWorker(); return;
#if IS_WIN
            case ThreadLabel::SHARED_COM: RunBackgroundSharedCOMWorker(); return;
            case ThreadLabel::DEDICATED_COM: RunBackgroundDedicatedCOMWorker(); return;
#endif  // IS_WIN
        }
    }
    ...
    switch (delegate_->GetThreadLabel())
    {
        case ThreadLabel::POOLED: RunPooledWorker(); return;
        case ThreadLabel::SHARED: RunSharedWorker(); return;
        case ThreadLabel::DEDICATED: RunDedicatedWorker(); return;
#if IS_WIN
        case ThreadLabel::SHARED_COM: RunSharedCOMWorker(); return;
        case ThreadLabel::DEDICATED_COM: RunDedicatedCOMWorker(); return;
#endif  // IS_WIN
    }
}

NOINLINE void
WorkerThread::RunPooledWorker()
{
    RunWorker();
    NO_CODE_FOLDING();
}
...
NOINLINE void
WorkerThread::RunBackgroundPooledWorker()
{
    RunWorker();
    NO_CODE_FOLDING();
}
...
NOINLINE void
WorkerThread::RunBackgroundSharedWorker()
{
    RunWorker();
    NO_CODE_FOLDING();
}
...
NOINLINE void
WorkerThread::RunDedicatedWorker()
{
    RunWorker();
    NO_CODE_FOLDING();
}
...
NOINLINE void
WorkerThread::RunBackgroundDedicatedWorker()
{
    RunWorker();
    NO_CODE_FOLDING();
}
...
#if IS_WIN
NOINLINE void
WorkerThread::RunSharedCOMWorker()
{
    RunWorker();
    NO_CODE_FOLDING();
}
...
NOINLINE void
WorkerThread::RunBackgroundSharedCOMWorker()
{
    RunWorker();
    NO_CODE_FOLDING();
}
...
NOINLINE void
WorkerThread::RunDedicatedCOMWorker()
{
    RunWorker();
    NO_CODE_FOLDING();
}
...
NOINLINE void
WorkerThread::RunBackgroundDedicatedCOMWorker()
{
    RunWorker();
    NO_CODE_FOLDING();
}
...
WorkerThread::RunWorker()
{
    ...
    const bool watch_for_hangs = base::HangWatcher::IsThreadPoolHangWatchingEnabled()
    ...
    if (watch_for_hangs)
    {
        unregister_for_hang_watching =
                base::HangWatcher::RegisterThread(base::HangWatcher::ThreadType::kThreadPoolThread);
    }
