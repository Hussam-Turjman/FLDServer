...
namespace internal
{
Sequence::Transaction::Transaction(Sequence* sequence) : TaskSource::Transaction(sequence)
{
}
...
bool
Sequence::Transaction::WillPushTask() const
{
...
void
Sequence::Transaction::PushTask(Task task)
{
    ...
    task.task = sequence()->traits_.shutdown_behavior() == TaskShutdownBehavior::BLOCK_SHUTDOWN
                        ? MakeCriticalClosure(task.posted_from, std::move(task.task))
                        : std::move(task.task);

    if (sequence()->queue_.empty())
    {
        sequence()->ready_time_.store(task.GetDesiredExecutionTime(), std::memory_order_relaxed);
    }
...
size_t
Sequence::GetRemainingConcurrency() const
{
    return 1;
}

Task
Sequence::TakeTask(TaskSource::Transaction* transaction)
{
...
bool
Sequence::DidProcessTask(TaskSource::Transaction* transaction)
{
    CheckedAutoLockMaybe auto_lock(transaction ? nullptr : &lock_);
...
TaskSourceSortKey
Sequence::GetSortKey(bool /* disable_fair_scheduling */) const
{
    return TaskSourceSortKey(priority_racy(), ready_time_.load(std::memory_order_relaxed));
}

Task
Sequence::Clear(TaskSource::Transaction* transaction)
{
    CheckedAutoLockMaybe auto_lock(transaction ? nullptr : &lock_);
    // See comment on TaskSource::task_runner_ for lifetime management details.
...
void
Sequence::ReleaseTaskRunner()
{
    if (!task_runner())
        return;
    if (execution_mode() == TaskSourceExecutionMode::kParallel)
    {
        static_cast<PooledParallelTaskRunner*>(task_runner())->UnregisterSequence(this);
    }
...
Sequence::GetExecutionEnvironment()
{
