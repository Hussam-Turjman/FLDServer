...
#include "fldserver/base/task/thread_pool/job_task_source.h"
...
static_assert(kMaxWorkersPerJob <= std::numeric_limits<
                      std::result_of<decltype (&JobDelegate::GetTaskId)(JobDelegate)>::type>::max(),
...
JobTaskSource::State::Cancel()
{
    return {value_.fetch_or(kCanceledMask, std::memory_order_relaxed)};
}

JobTaskSource::State::Value
JobTaskSource::State::DecrementWorkerCount()
{
...
JobTaskSource::State::Value
JobTaskSource::State::IncrementWorkerCount()
{
    size_t value_before_add = value_.fetch_add(kWorkerCountIncrement, std::memory_order_relaxed);
    return {static_cast<uint32_t>(value_before_add)};
}

JobTaskSource::State::Value
JobTaskSource::State::Load() const
{
...
JobTaskSource::JoinFlag::ShouldWorkerYield()
{
    ...
    return value_.fetch_and(kWaitingForWorkerToSignal, std::memory_order_relaxed)
           == kWaitingForWorkerToYield;
}

bool
JobTaskSource::JoinFlag::ShouldWorkerSignal()
{
...
JobTaskSource::GetExecutionEnvironment()
{
    return {SequenceToken::Create(), nullptr};
}

bool
JobTaskSource::WillJoin()
{
    ...
    if (!state_before_add.is_canceled()
        && state_before_add.worker_count() < GetMaxConcurrency(state_before_add.worker_count()))
    {
        return true;
    }
...
JobTaskSource::RunJoinTask()
{
    ...
    if (!state.is_canceled() && state.worker_count() <= GetMaxConcurrency(state.worker_count() - 1))
    {
        return true;
    }
...
void
JobTaskSource::Cancel(TaskSource::Transaction* transaction)
{
...
bool
JobTaskSource::WaitForParticipationOpportunity()
{
...
JobTaskSource::WillRunTask()
{
    ...
    const size_t max_concurrency = GetMaxConcurrency(state_before_add.worker_count());
    ...
    return max_concurrency == worker_count_before_add + 1 ? RunStatus::kAllowedSaturated
                                                          : RunStatus::kAllowedNotSaturated;
}

size_t
JobTaskSource::GetRemainingConcurrency() const
{
    // It is safe to read |state_| without a lock since this variable is atomic,
...
bool
JobTaskSource::IsActive() const
{
    CheckedAutoLock auto_lock(worker_lock_);
    auto state = state_.Load();
    return GetMaxConcurrency(state.worker_count()) != 0 || state.worker_count() != 0;
}

size_t
JobTaskSource::GetWorkerCount() const
{
    return TS_UNCHECKED_READ(state_).Load().worker_count();
}

void
JobTaskSource::NotifyConcurrencyIncrease()
{
...
size_t
JobTaskSource::GetMaxConcurrency() const
{
    return GetMaxConcurrency(TS_UNCHECKED_READ(state_).Load().worker_count());
}

size_t
JobTaskSource::GetMaxConcurrency(size_t worker_count) const
{
    return std::min(max_concurrency_callback_.Run(worker_count), kMaxWorkersPerJob);
}

uint8_t
JobTaskSource::AcquireTaskId()
{
    static_assert(kMaxWorkersPerJob <= sizeof(assigned_task_ids_) * 8,
                  "TaskId bitfield isn't big enough to fit kMaxWorkersPerJob.");
    uint32_t assigned_task_ids = assigned_task_ids_.load(std::memory_order_relaxed);
    ...
    } while (!assigned_task_ids_.compare_exchange_weak(assigned_task_ids,
                                                       new_assigned_task_ids,
...
void
JobTaskSource::ReleaseTaskId(uint8_t task_id)
{
    // memory_order_release to match AcquireTaskId().
    uint32_t previous_task_ids =
            assigned_task_ids_.fetch_and(~(uint32_t(1) << task_id), std::memory_order_release);
    DCHECK(previous_task_ids & (uint32_t(1) << task_id));
}

bool
JobTaskSource::ShouldYield()
{
...
Task
JobTaskSource::TakeTask(TaskSource::Transaction* transaction)
{
...
bool
JobTaskSource::DidProcessTask(TaskSource::Transaction* /*transaction*/)
{
...
TaskSourceSortKey
JobTaskSource::GetSortKey(bool disable_fair_scheduling) const
{
    ...
    return TaskSourceSortKey(
            priority_racy(), ready_time_, TS_UNCHECKED_READ(state_).Load().worker_count());
}

Task
JobTaskSource::Clear(TaskSource::Transaction* transaction)
{
    Cancel();
