...
class CORE_EXPORT ThreadGroup
{
public:
    // Delegate interface for ThreadGroup.
    class CORE_EXPORT Delegate
    {
    public:
        ...
        virtual ThreadGroup*
        GetThreadGroupForTraits(const TaskTraits& traits) = 0;
    };
    ...
    ThreadGroup&
    operator=(const ThreadGroup&) = delete;
    virtual ~ThreadGroup();
    ...
    RemoveTaskSource(const TaskSource& task_source);
    ...
    virtual void
    UpdateSortKey(TaskSource::Transaction transaction) = 0;
    ...
    void
    InvalidateAndHandoffAllTaskSourcesToOtherThreadGroup(ThreadGroup* destination_thread_group);
    ...
    // re-scheduled).
    bool
    ShouldYield(TaskSourceSortKey sort_key);
    ...
    virtual size_t
    GetMaxConcurrentNonBlockedTasksDeprecated() const = 0;
...
protected:
    ...
    public:
        ...
        operator=(const BaseScopedCommandsExecutor&) = delete;

        void
        ScheduleReleaseTaskSource(RegisteredTaskSource task_source);
    ...
    public:
        ...
        ScopedReenqueueExecutor&
        operator=(const ScopedReenqueueExecutor&) = delete;
        ~ScopedReenqueueExecutor();
    ...
    size_t
    GetNumAdditionalWorkersForBestEffortTaskSourcesLockRequired() const
    ...
    // CanRunPolicy.
    size_t
    GetNumAdditionalWorkersForForegroundTaskSourcesLockRequired() const
            EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    virtual void
    EnsureEnoughWorkersLockRequired(BaseScopedCommandsExecutor* executor)
    ...
    void
    ReEnqueueTaskSourceLockRequired(
    ...
    RegisteredTaskSource
    TakeRegisteredTaskSource(BaseScopedCommandsExecutor* executor) EXCLUSIVE_LOCKS_REQUIRED(lock_);

    // Must be invoked by implementations of the corresponding non-Impl() methods.
    void
    UpdateSortKeyImpl(BaseScopedCommandsExecutor* executor, TaskSource::Transaction transaction);
    void
    PushTaskSourceAndWakeUpWorkersImpl(
    ...
    static constexpr YieldSortKey kMaxYieldSortKey = {TaskPriority::BEST_EFFORT, 0U};
    ...
    std::atomic<YieldSortKey> max_allowed_sort_key_ GUARDED_BY(lock_){kMaxYieldSortKey};
