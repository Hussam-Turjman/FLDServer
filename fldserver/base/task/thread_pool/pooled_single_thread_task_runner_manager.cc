...
#include "fldserver/base/task/thread_pool/priority_queue.h"
...
{
    const bool is_background = traits.priority() == TaskPriority::BEST_EFFORT
...
public:
    ...
    operator=(const AtomicThreadRefChecker&) = delete;
    ~AtomicThreadRefChecker() = default;
    ...
    bool
    IsCurrentThreadSameAsSetThread()
    {
...
public:
    ...
    operator=(const WorkerThreadDelegate&) = delete;
    ...
    bool
    PostTaskNow(scoped_refptr<Sequence> sequence, Task task)
    {
...
protected:
    RegisteredTaskSource
    GetWorkLockRequired(WorkerThread* worker) EXCLUSIVE_LOCKS_REQUIRED(lock_)
    {
...
private:
    ...
    bool
    EnqueueTaskSource(TransactionWithRegisteredTaskSource transaction_with_task_source)
    {
        CheckedAutoLock auto_lock(lock_);
        auto sort_key = transaction_with_task_source.task_source->GetSortKey(
                /* disable_fair_scheduling */ false);
        priority_queue_.Push(std::move(transaction_with_task_source.task_source), sort_key);
        if (!worker_awake_ && CanRunNextTaskSource())
        {
            worker_awake_ = true;
            return true;
    ...
    bool
    CanRunNextTaskSource() EXCLUSIVE_LOCKS_REQUIRED(lock_)
    {
        return !priority_queue_.IsEmpty()
               && task_tracker_->CanRunPriority(priority_queue_.PeekSortKey().priority());
    }
...
public:
    WorkerThreadCOMDelegate(const std::string& thread_name,
                            WorkerThread::ThreadLabel thread_label,
                            TrackedRef<TaskTracker> task_tracker) :
        WorkerThreadDelegate(thread_name, thread_label, std::move(task_tracker))
    {
    }

    WorkerThreadCOMDelegate(const WorkerThreadCOMDelegate&) = delete;
    WorkerThreadCOMDelegate&
    operator=(const WorkerThreadCOMDelegate&) = delete;
    ...
    void
    WaitForWork(WaitableEvent* wake_up_event) override
    {
        DCHECK(wake_up_event);
        const TimeDelta sleep_time = GetSleepTimeout();
        const DWORD milliseconds_wait =
                checked_cast<DWORD>(sleep_time.is_max() ? INFINITE : sleep_time.InMilliseconds());
        const HANDLE wake_up_event_handle = wake_up_event->handle();
        MsgWaitForMultipleObjectsEx(1, &wake_up_event_handle, milliseconds_wait, QS_ALLINPUT, 0);
    }
...
private:
    ...
    GetWorkFromWindowsMessageQueue()
    {
        ...
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE) != FALSE)
        {
            Task pump_message_task(FROM_HERE,
                                   ...
                                   TimeTicks::Now(),
                                   TimeDelta());
            if (task_tracker()->WillPostTask(&pump_message_task,
                                             TaskShutdownBehavior::SKIP_ON_SHUTDOWN))
            {
                auto transaction = message_pump_sequence_->BeginTransaction();
                const bool sequence_should_be_queued = transaction.WillPushTask();
                DCHECK(sequence_should_be_queued) << "GetWorkFromWindowsMessageQueue() does not "
                                                     "expect "
                                                     "queueing of pump tasks.";
                auto registered_task_source =
                        task_tracker_->RegisterTaskSource(std::move(message_pump_sequence_));
    ...
    const scoped_refptr<Sequence> message_pump_sequence_ = MakeRefCounted<Sequence>(
            TaskTraits{MayBlock()}, nullptr, TaskSourceExecutionMode::kParallel);
...
public:
    ...
    PooledSingleThreadTaskRunner(PooledSingleThreadTaskRunnerManager* const outer,
        ...
        sequence_(MakeRefCounted<Sequence>(traits, this, TaskSourceExecutionMode::kSingleThread))
    {
        DCHECK(outer_);
        DCHECK(worker_);
    }
    PooledSingleThreadTaskRunner(const PooledSingleThreadTaskRunner&) = delete;
    PooledSingleThreadTaskRunner&
    operator=(const PooledSingleThreadTaskRunner&) = delete;
    ...
    PostDelayedTask(const Location& from_here, OnceClosure closure, TimeDelta delay) override
    {
        ...
        if (!outer_->task_tracker_->WillPostTask(&task, sequence_->shutdown_behavior()))
        {
            return false;
        }
...
private:
    ~PooledSingleThreadTaskRunner() override
    {
        ...
        // here.
        if (g_manager_is_alive && thread_mode_ == SingleThreadTaskRunnerThreadMode::DEDICATED)
        {
...
PooledSingleThreadTaskRunnerManager::PooledSingleThreadTaskRunnerManager(
        TrackedRef<TaskTracker> task_tracker, DelayedTaskManager* delayed_task_manager) :
    task_tracker_(std::move(task_tracker)), delayed_task_manager_(delayed_task_manager)
{
    ...
    static_assert(
            std::extent<std::remove_reference<decltype(shared_com_worker_threads_[0])>>()
                    == std::extent<std::remove_reference<decltype(shared_worker_threads_[0])>>(),
            "The size of |shared_com_worker_threads_| must match "
            "|shared_worker_threads_|");
#endif  // IS_WIN
...
void
PooledSingleThreadTaskRunnerManager::Start(WorkerThreadObserver* worker_thread_observer)
{
...
void
PooledSingleThreadTaskRunnerManager::DidUpdateCanRunPolicy()
{
    ...
    {
        static_cast<WorkerThreadDelegate*>(worker->delegate())->DidUpdateCanRunPolicy();
    }
}
...
PooledSingleThreadTaskRunnerManager::TraitsToContinueOnShutdown(const TaskTraits& traits)
{
        ...
        const TaskTraits& traits, SingleThreadTaskRunnerThreadMode thread_mode)
{
    ...
    WorkerThread*& worker = thread_mode == SingleThreadTaskRunnerThreadMode::DEDICATED
                                    ? dedicated_worker
    ...
    bool started;
    {
        ...
        if (!worker)
        {
            ...
            worker = CreateAndRegisterWorkerThread<DelegateType>(
                    worker_name,
                    thread_mode,
                    CanUseBackgroundPriorityForWorkerThread() ? environment_params.priority_hint
    ...
    return MakeRefCounted<PooledSingleThreadTaskRunner>(this, traits, worker, thread_mode);
}

void
PooledSingleThreadTaskRunnerManager::JoinForTesting()
{
    ...
    {
        CheckedAutoLock auto_lock(lock_);
        DCHECK(workers_.empty()) << "New worker(s) unexpectedly registered during join.";
...
PooledSingleThreadTaskRunnerManager::CreateWorkerThreadDelegate<WorkerThreadDelegate>(
        const std::string& name, int id, SingleThreadTaskRunnerThreadMode thread_mode)
{
...
template <>
std::unique_ptr<WorkerThreadDelegate>
PooledSingleThreadTaskRunnerManager::CreateWorkerThreadDelegate<WorkerThreadCOMDelegate>(
        const std::string& name, int id, SingleThreadTaskRunnerThreadMode thread_mode)
{
        ...
        ThreadPriority priority_hint)
{
    ...
    scoped_refptr<WorkerThread> worker =
            MakeRefCounted<WorkerThread>(priority_hint, std::move(delegate), task_tracker_);
...
template <>
WorkerThread*&
PooledSingleThreadTaskRunnerManager::GetSharedWorkerThreadForTraits<WorkerThreadDelegate>(
        const TaskTraits& traits)
{
...
template <>
WorkerThread*&
PooledSingleThreadTaskRunnerManager::GetSharedWorkerThreadForTraits<WorkerThreadCOMDelegate>(
        const TaskTraits& traits)
{
...
void
PooledSingleThreadTaskRunnerManager::UnregisterWorkerThread(WorkerThread* worker)
{
...
void
PooledSingleThreadTaskRunnerManager::ReleaseSharedWorkerThreads()
{
...
#if IS_WIN
                local_shared_com_worker_threads[i][j] = shared_com_worker_threads_[i][j];
