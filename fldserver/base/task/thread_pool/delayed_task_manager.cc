...
DelayedTaskManager::DelayedTask::DelayedTask(Task task,
    ...
    task(std::move(task)), callback(std::move(callback)), task_runner(std::move(task_runner))
{
}

DelayedTaskManager::DelayedTask::DelayedTask(DelayedTaskManager::DelayedTask&& other) = default;

DelayedTaskManager::DelayedTask::~DelayedTask() = default;

DelayedTaskManager::DelayedTask&
DelayedTaskManager::DelayedTask::operator=(DelayedTaskManager::DelayedTask&& other) = default;

bool
DelayedTaskManager::DelayedTask::operator<=(const DelayedTask& other) const
{
...
bool
DelayedTaskManager::DelayedTask::IsScheduled() const
{
    return scheduled_;
}
void
DelayedTaskManager::DelayedTask::SetScheduled()
{
    DCHECK(!scheduled_);
    scheduled_ = true;
}
...
DelayedTaskManager::DelayedTaskManager(const TickClock* tick_clock) :
    process_ripe_tasks_closure_(
            BindRepeating(&DelayedTaskManager::ProcessRipeTasks, Unretained(this))),
...
void
DelayedTaskManager::Start(scoped_refptr<SequencedTaskRunner> service_thread_task_runner)
{
...
void
DelayedTaskManager::AddDelayedTask(Task task,
                                   ...
                                   scoped_refptr<TaskRunner> task_runner)
{
    ...
    {
        CheckedAutoLock auto_lock(queue_lock_);
        delayed_task_queue_.insert(DelayedTask(
                std::move(task), std::move(post_task_now_callback), std::move(task_runner)));
        // Not started yet.
...
void
DelayedTaskManager::ProcessRipeTasks()
{
...
Optional<TimeTicks>
DelayedTaskManager::NextScheduledRunTime() const
{
    CheckedAutoLock auto_lock(queue_lock_);
...
TimeTicks
DelayedTaskManager::GetTimeToScheduleProcessRipeTasksLockRequired()
{
    ...
    DelayedTask& ripest_delayed_task = const_cast<DelayedTask&>(delayed_task_queue_.Min());
...
void
DelayedTaskManager::ScheduleProcessRipeTasksOnServiceThread(TimeTicks next_delayed_task_run_time)
{
    ...
    service_thread_task_runner_->PostDelayedTask(FROM_HERE, process_ripe_tasks_closure_, delay);
}
