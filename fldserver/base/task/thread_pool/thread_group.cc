...
LazyInstance<ThreadLocalPointer<const ThreadGroup>>::Leaky tls_current_thread_group =
...
void
ThreadGroup::BaseScopedCommandsExecutor::ScheduleReleaseTaskSource(RegisteredTaskSource task_source)
{
...
void
ThreadGroup::ScopedReenqueueExecutor::SchedulePushTaskSourceAndWakeUpWorkers(
        ...
        ThreadGroup* destination_thread_group)
{
    ...
    transaction_with_task_source_.emplace(std::move(transaction_with_task_source));
...
ThreadGroup::ThreadGroup(TrackedRef<TaskTracker> task_tracker,
    ...
    lock_(predecessor_thread_group ? &predecessor_thread_group->lock_ : nullptr)
{
...
void
ThreadGroup::BindToCurrentThread()
{
...
void
ThreadGroup::UnbindFromCurrentThread()
{
    DCHECK(GetCurrentThreadGroup());
...
bool
ThreadGroup::IsBoundToCurrentThread() const
{
...
size_t
ThreadGroup::GetNumAdditionalWorkersForBestEffortTaskSourcesLockRequired() const
{
    ...
    if (priority_queue_.PeekSortKey().priority() == TaskPriority::BEST_EFFORT)
    {
        ...
        return std::max<size_t>(
                1, num_queued + priority_queue_.PeekTaskSource()->GetRemainingConcurrency() - 1);
    }
    return num_queued;
}

size_t
ThreadGroup::GetNumAdditionalWorkersForForegroundTaskSourcesLockRequired() const
{
    // For simplicity, only 1 worker is assigned to each task source regardless of
    // its max concurrency, with the exception of the top task source.
    const size_t num_queued =
            priority_queue_.GetNumTaskSourcesWithPriority(TaskPriority::USER_VISIBLE)
            + priority_queue_.GetNumTaskSourcesWithPriority(TaskPriority::USER_BLOCKING);
    if (num_queued == 0 || !task_tracker_->CanRunPriority(TaskPriority::HIGHEST))
    {
        return 0U;
    }
    auto priority = priority_queue_.PeekSortKey().priority();
    if (priority == TaskPriority::USER_VISIBLE || priority == TaskPriority::USER_BLOCKING)
    {
        // Assign the correct number of workers for the top TaskSource (-1 for the
        // worker that is already accounted for in |num_queued|).
        return std::max<size_t>(
...
ThreadGroup::RemoveTaskSource(const TaskSource& task_source)
{
    CheckedAutoLock auto_lock(lock_);
...
void
ThreadGroup::ReEnqueueTaskSourceLockRequired(
        ...
        TransactionWithRegisteredTaskSource transaction_with_task_source)
{
    ...
    ThreadGroup* destination_thread_group =
            delegate_->GetThreadGroupForTraits(transaction_with_task_source.transaction.traits());
    ...
    if (destination_thread_group == this)
    {
        ...
        else
        {
            ...
            auto sort_key =
                    transaction_with_task_source.task_source->GetSortKey(disable_fair_scheduling_);
...
RegisteredTaskSource
ThreadGroup::TakeRegisteredTaskSource(BaseScopedCommandsExecutor* executor)
{
    ...
    {
        priority_queue_.UpdateSortKey(*task_source.get(), task_source->GetSortKey(false));
    }
    return task_source;
}

void
ThreadGroup::UpdateSortKeyImpl(BaseScopedCommandsExecutor* executor,
...
void
ThreadGroup::PushTaskSourceAndWakeUpWorkersImpl(
        BaseScopedCommandsExecutor* executor,
        TransactionWithRegisteredTaskSource transaction_with_task_source)
{
    CheckedAutoLock auto_lock(lock_);
    DCHECK(!replacement_thread_group_);
    DCHECK_EQ(delegate_->GetThreadGroupForTraits(transaction_with_task_source.transaction.traits()),
              this);
    if (transaction_with_task_source.task_source->heap_handle().IsValid())
    {
        ...
        executor->ScheduleReleaseTaskSource(std::move(transaction_with_task_source.task_source));
        return;
    }
    auto sort_key = transaction_with_task_source.task_source->GetSortKey(disable_fair_scheduling_);
    priority_queue_.Push(std::move(transaction_with_task_source.task_source), sort_key);
    EnsureEnoughWorkersLockRequired(executor);
}

void
ThreadGroup::InvalidateAndHandoffAllTaskSourcesToOtherThreadGroup(
        ThreadGroup* destination_thread_group)
{
    CheckedAutoLock current_thread_group_lock(lock_);
    CheckedAutoLock destination_thread_group_lock(destination_thread_group->lock_);
    destination_thread_group->priority_queue_ = std::move(priority_queue_);
    replacement_thread_group_ = destination_thread_group;
}

bool
ThreadGroup::ShouldYield(TaskSourceSortKey sort_key)
{
    ...
    max_allowed_sort_key = TS_UNCHECKED_READ(max_allowed_sort_key_)
