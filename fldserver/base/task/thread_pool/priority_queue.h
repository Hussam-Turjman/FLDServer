...
class CORE_EXPORT PriorityQueue
{
public:
    PriorityQueue();
    PriorityQueue(const PriorityQueue&) = delete;
    PriorityQueue&
    operator=(const PriorityQueue&) = delete;
    ~PriorityQueue();

    PriorityQueue&
    operator=(PriorityQueue&& other);

    // Inserts |task_source| in the PriorityQueue with |task_source_sort_key|.
    void
    Push(RegisteredTaskSource task_source, TaskSourceSortKey task_source_sort_key);
    ...
    void
    UpdateSortKey(const TaskSource& task_source, TaskSourceSortKey sort_key);
    ...
    size_t
    GetNumTaskSourcesWithPriority(TaskPriority priority) const
    {
    ...
    void
    EnableFlushTaskSourcesOnDestroyForTesting();
...
private:
    ...
    void
    DecrementNumTaskSourcesForPriority(TaskPriority priority);
    void
    IncrementNumTaskSourcesForPriority(TaskPriority priority);

    ContainerType container_;

    std::array<size_t, static_cast<int>(TaskPriority::HIGHEST) + 1> num_task_sources_per_priority_ =
            {};
