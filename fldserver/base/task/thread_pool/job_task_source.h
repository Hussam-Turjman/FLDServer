...
#include "fldserver/base/task/post_job.h"
...
class CORE_EXPORT JobTaskSource : public TaskSource
{
public:
    ...
    JobTaskSource&
    operator=(const JobTaskSource&) = delete;

    static JobHandle
    CreateJobHandle(scoped_refptr<internal::JobTaskSource> task_source)
    {
    ...
    void
    Cancel(TaskSource::Transaction* transaction = nullptr);

    // TaskSource:
    ExecutionEnvironment
    GetExecutionEnvironment() override;
    size_t
    GetRemainingConcurrency() const override;
    TaskSourceSortKey
    GetSortKey(bool disable_fair_scheduling = false) const override;
...
private:
    ...
    public:
        ...
        struct Value
        {
            ...
            worker_count() const
            {
                return value >> kWorkerCountBitOffset;
            }
            // Returns true if canceled.
            bool
            is_canceled() const
            {
                return value & kCanceledMask;
            }
    ...
    bool
    WaitForParticipationOpportunity() EXCLUSIVE_LOCKS_REQUIRED(worker_lock_);

    size_t
    GetMaxConcurrency(size_t worker_count) const;

    // TaskSource:
    RunStatus
    WillRunTask() override;
    Task
    TakeTask(TaskSource::Transaction* transaction) override;
    Task
    Clear(TaskSource::Transaction* transaction) override;
    bool
    DidProcessTask(TaskSource::Transaction* transaction) override;
    ...
    std::unique_ptr<ConditionVariable> worker_released_condition_ GUARDED_BY(worker_lock_);
