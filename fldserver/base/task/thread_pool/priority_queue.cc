...
public:
    ...
    TaskSourceAndSortKey(RegisteredTaskSource task_source, const TaskSourceSortKey& sort_key) :
        task_source_(std::move(task_source)), sort_key_(sort_key)
    {
        DCHECK(task_source_);
    }
    TaskSourceAndSortKey(const TaskSourceAndSortKey&) = delete;
    TaskSourceAndSortKey&
    operator=(const TaskSourceAndSortKey&) = delete;
    ...
    TaskSourceAndSortKey&
    operator=(TaskSourceAndSortKey&& other) = default;
    ...
    bool
    operator<=(const TaskSourceAndSortKey& other) const
    {
...
void
PriorityQueue::Push(RegisteredTaskSource task_source, TaskSourceSortKey task_source_sort_key)
{
    container_.insert(TaskSourceAndSortKey(std::move(task_source), task_source_sort_key));
    IncrementNumTaskSourcesForPriority(task_source_sort_key.priority());
}

const TaskSourceSortKey&
PriorityQueue::PeekSortKey() const
{
    DCHECK(!IsEmpty());
...
RegisteredTaskSource&
PriorityQueue::PeekTaskSource() const
{
    DCHECK(!IsEmpty());
...
PriorityQueue::PopTaskSource()
{
    ...
    auto& task_source_and_sort_key = const_cast<TaskSourceAndSortKey&>(container_.Min());
    DecrementNumTaskSourcesForPriority(task_source_and_sort_key.sort_key().priority());
    RegisteredTaskSource task_source = task_source_and_sort_key.take_task_source();
    container_.Pop();
    return task_source;
}

RegisteredTaskSource
PriorityQueue::RemoveTaskSource(const TaskSource& task_source)
{
    if (IsEmpty())
        return nullptr;
    ...
    TaskSourceAndSortKey& task_source_and_sort_key =
            const_cast<PriorityQueue::TaskSourceAndSortKey&>(container_.at(heap_handle));
    DCHECK_EQ(task_source_and_sort_key.task_source().get(), &task_source);
    RegisteredTaskSource registered_task_source = task_source_and_sort_key.take_task_source();
...
void
PriorityQueue::UpdateSortKey(const TaskSource& task_source, TaskSourceSortKey sort_key)
{
    if (IsEmpty())
        return;
...
void
PriorityQueue::EnableFlushTaskSourcesOnDestroyForTesting()
{
...
void
PriorityQueue::DecrementNumTaskSourcesForPriority(TaskPriority priority)
{
...
void
PriorityQueue::IncrementNumTaskSourcesForPriority(TaskPriority priority)
{
    ++num_task_sources_per_priority_[static_cast<int>(priority)];
}
