...
#include "fldserver/base/task/task_features.h"
#include "fldserver/base/task/thread_pool/pooled_parallel_task_runner.h"
#include "fldserver/base/task/thread_pool/pooled_sequenced_task_runner.h"
#include "fldserver/base/task/thread_pool/task.h"
#include "fldserver/base/task/thread_pool/task_source.h"
#include "fldserver/base/task/thread_pool/task_source_sort_key.h"
#include "fldserver/base/task/thread_pool/thread_group_impl.h"
#include "fldserver/base/task/thread_pool/worker_thread.h"
...
#if IS_WIN
#include "fldserver/base/task/thread_pool/thread_group_native_win.h"
#endif

#if IS_APPLE
#include "fldserver/base/task/thread_pool/thread_group_native_mac.h"
#endif
...
namespace
{
constexpr EnvironmentParams kForegroundPoolEnvironmentParams{"Foreground",
                                                             base::ThreadPriority::NORMAL};

constexpr EnvironmentParams kBackgroundPoolEnvironmentParams{"Background",
...
bool
HasDisableBestEffortTasksSwitch()
{
...
void
AssertNoExtensionInTraits(const base::TaskTraits& traits)
{
    DCHECK_EQ(traits.extension_id(), TaskTraitsExtensionStorage::kInvalidExtensionId);
}
...
ThreadPoolImpl::ThreadPoolImpl(StringPiece histogram_label,
    ...
    single_thread_task_runner_manager_(task_tracker_->GetTrackedRef(), &delayed_task_manager_),
    ...
    tracked_ref_factory_(this)
{
    foreground_thread_group_ = std::make_unique<ThreadGroupImpl>(
            histogram_label.empty()
                    ? std::string()
                    : JoinString({histogram_label, kForegroundPoolEnvironmentParams.name_suffix},
                                 "."),
    ...
    if (CanUseBackgroundPriorityForWorkerThread())
    {
        background_thread_group_ = std::make_unique<ThreadGroupImpl>(
                histogram_label.empty() ? std::string()
...
void
ThreadPoolImpl::Start(const ThreadPoolInstance::InitParams& init_params,
                      WorkerThreadObserver* worker_thread_observer)
{
    ...
    disable_job_update_priority_ = FeatureList::IsEnabled(kDisableJobUpdatePriority);
    ...
    {
        std::unique_ptr<ThreadGroup> old_group = std::move(foreground_thread_group_);
    ...
    {
        std::unique_ptr<ThreadGroup> old_group = std::move(background_thread_group_);
...
#endif  // IS_POSIX && !defined(OS_NACL_SFI)
    ...
    switch (init_params.common_thread_pool_environment)
    {
        ...
        case InitParams::CommonThreadPoolEnvironment::COM_MTA:
            ...
            break;
        case InitParams::CommonThreadPoolEnvironment::DEPRECATED_COM_STA_IN_FOREGROUND_GROUP:
    ...
    if (FeatureList::IsEnabled(kUseNativeThreadPool))
    {
        static_cast<ThreadGroupNative*>(foreground_thread_group_.get())->Start(worker_environment);
    }
    else
#endif
    {
        ...
        static_cast<ThreadGroupImpl*>(foreground_thread_group_.get())
                ->Start(init_params.max_num_foreground_threads,
...
bool
ThreadPoolImpl::PostDelayedTask(const Location& from_here,
                                ...
                                TimeDelta delay)
{
    ...
    return PostTaskWithSequence(
            ...
            MakeRefCounted<Sequence>(traits, nullptr, TaskSourceExecutionMode::kParallel));
}

scoped_refptr<TaskRunner>
ThreadPoolImpl::CreateTaskRunner(const TaskTraits& traits)
{
    AssertNoExtensionInTraits(traits);
...
ThreadPoolImpl::CreateSequencedTaskRunner(const TaskTraits& traits)
{
    AssertNoExtensionInTraits(traits);
...
ThreadPoolImpl::CreateSingleThreadTaskRunner(const TaskTraits& traits,
                                             SingleThreadTaskRunnerThreadMode thread_mode)
{
    AssertNoExtensionInTraits(traits);
    return single_thread_task_runner_manager_.CreateSingleThreadTaskRunner(traits, thread_mode);
}

#if IS_WIN
scoped_refptr<SingleThreadTaskRunner>
ThreadPoolImpl::CreateCOMSTATaskRunner(const TaskTraits& traits,
                                       SingleThreadTaskRunnerThreadMode thread_mode)
{
    AssertNoExtensionInTraits(traits);
    return single_thread_task_runner_manager_.CreateCOMSTATaskRunner(traits, thread_mode);
}
#endif  // IS_WIN
...
Optional<TimeTicks>
ThreadPoolImpl::NextScheduledRunTimeForTesting() const
{
...
void
ThreadPoolImpl::ProcessRipeDelayedTasksForTesting()
{
...
// static
void
ThreadPoolImpl::SetSynchronousThreadStartForTesting(bool enabled)
{
...
int
ThreadPoolImpl::GetMaxConcurrentNonBlockedTasksWithTraitsDeprecated(const TaskTraits& traits) const
{
    ...
    return GetThreadGroupForTraits(traits)->GetMaxConcurrentNonBlockedTasksDeprecated();
}
...
void
ThreadPoolImpl::FlushForTesting()
{
    task_tracker_->FlushForTesting();
}

void
ThreadPoolImpl::FlushAsyncForTesting(OnceClosure flush_callback)
{
    task_tracker_->FlushAsyncForTesting(std::move(flush_callback));
}

void
ThreadPoolImpl::JoinForTesting()
{
#if DCHECK_IS_ON()
    DCHECK(!join_for_testing_returned_.IsSet());
#endif
...
void
ThreadPoolImpl::BeginBestEffortFence()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void
ThreadPoolImpl::EndBestEffortFence()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool
ThreadPoolImpl::PostTaskWithSequenceNow(Task task, scoped_refptr<Sequence> sequence)
{
...
bool
ThreadPoolImpl::PostTaskWithSequence(Task task, scoped_refptr<Sequence> sequence)
{
    ...
    else
    {
        ...
        delayed_task_manager_.AddDelayedTask(std::move(task),
                                             BindOnce(
                                                     [](scoped_refptr<Sequence> sequence,
                                                        ThreadPoolImpl* thread_pool_impl,
                                                        Task task) {
                                                         thread_pool_impl->PostTaskWithSequenceNow(
                                                                 std::move(task),
                                                                 std::move(sequence));
                                                     },
                                                     std::move(sequence),
...
bool
ThreadPoolImpl::ShouldYield(const TaskSource* task_source)
{
    if (disable_job_yield_)
        return false;
    const TaskPriority priority = task_source->priority_racy();
    auto* const thread_group = GetThreadGroupForTraits({priority, task_source->thread_policy()});
...
bool
ThreadPoolImpl::EnqueueJobTaskSource(scoped_refptr<JobTaskSource> task_source)
{
    auto registered_task_source = task_tracker_->RegisterTaskSource(std::move(task_source));
...
void
ThreadPoolImpl::RemoveJobTaskSource(scoped_refptr<JobTaskSource> task_source)
{
    auto transaction = task_source->BeginTransaction();
    ThreadGroup* const current_thread_group = GetThreadGroupForTraits(transaction.traits());
    current_thread_group->RemoveTaskSource(*task_source);
}

void
ThreadPoolImpl::UpdatePriority(scoped_refptr<TaskSource> task_source, TaskPriority priority)
{
    auto transaction = task_source->BeginTransaction();
    ...
    ThreadGroup* const new_thread_group = GetThreadGroupForTraits(transaction.traits());
    ...
    else
    {
        ...
        auto registered_task_source = current_thread_group->RemoveTaskSource(*task_source);
        if (registered_task_source)
        {
            DCHECK(task_source);
...
void
ThreadPoolImpl::UpdateJobPriority(scoped_refptr<TaskSource> task_source, TaskPriority priority)
{
...
const ThreadGroup*
ThreadPoolImpl::GetThreadGroupForTraits(const TaskTraits& traits) const
{
    return const_cast<ThreadPoolImpl*>(this)->GetThreadGroupForTraits(traits);
}

ThreadGroup*
ThreadPoolImpl::GetThreadGroupForTraits(const TaskTraits& traits)
{
    if (traits.priority() == TaskPriority::BEST_EFFORT
        && traits.thread_policy() == ThreadPolicy::PREFER_BACKGROUND && background_thread_group_)
    {
...
void
ThreadPoolImpl::UpdateCanRunPolicy()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    CanRunPolicy can_run_policy;
    if ((num_fences_ == 0 && num_best_effort_fences_ == 0 && !has_disable_best_effort_switch_)
