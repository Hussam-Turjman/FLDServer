...
#include "fldserver/base/synchronization/condition_variable.h"
#include "fldserver/base/task/scoped_set_task_priority_for_current_thread.h"
#include "fldserver/base/task/task_executor.h"
#include "fldserver/base/threading/sequence_local_storage_map.h"
...
namespace
{
constexpr const char* kExecutionModeString[] = {"parallel", "sequenced", "single thread", "job"};
static_assert(size(kExecutionModeString) == static_cast<size_t>(TaskSourceExecutionMode::kMax) + 1,
              "Array kExecutionModeString is out of sync with TaskSourceExecutionMode.");


bool
HasLogBestEffortTasksSwitch()
{
    ...
    return CommandLine::InitializedForCurrentProcess()
           && CommandLine::ForCurrentProcess()->HasSwitch(switches::kLogBestEffortTasks);
}
...
public:
                    ...
                    TimeDelta delay) override
    {
        ...
        return sequenced_task_runner_->PostDelayedTask(from_here, std::move(task), delay);
    }
...
private:
    ...
    void
    CheckTraitsCompatibleWithSequenceTraits(const TaskTraits& traits)
    {
        if (traits.shutdown_behavior_set_explicitly())
        {
            DCHECK_EQ(traits.shutdown_behavior(), sequence_traits_->shutdown_behavior());
        }

        DCHECK(!traits.may_block() || traits.may_block() == sequence_traits_->may_block());
...
public:
    ...
    StartShutdown()
    {
        const auto new_value = subtle::NoBarrier_AtomicIncrement(&bits_, kShutdownHasStartedMask);
        ...
        const auto num_items_blocking_shutdown = new_value >> kNumItemsBlockingShutdownBitOffset;
        return num_items_blocking_shutdown != 0;
    }

    // Returns true if shutdown has started.
    bool
    HasShutdownStarted() const
    {
    ...
    bool
    AreItemsBlockingShutdown() const
    {
        const auto num_items_blocking_shutdown = subtle::NoBarrier_Load(&bits_)
    ...
    bool
    IncrementNumItemsBlockingShutdown()
    {
#if DCHECK_IS_ON()
        ...
        const auto new_bits =
                subtle::NoBarrier_AtomicIncrement(&bits_, kNumItemsBlockingShutdownIncrement);
    ...
    DecrementNumItemsBlockingShutdown()
    {
        ...
        const auto num_items_blocking_shutdown = new_bits >> kNumItemsBlockingShutdownBitOffset;
        DCHECK_GE(num_items_blocking_shutdown, 0);
...
void
TaskTracker::FlushAsyncForTesting(OnceClosure flush_callback)
{
    DCHECK(flush_callback);
    {
        CheckedAutoLock auto_lock(flush_lock_);
        DCHECK(!flush_callback_for_testing_) << "Only one FlushAsyncForTesting() may be pending at "
                                                "any time.";
        flush_callback_for_testing_ = std::move(flush_callback);
    }

    if (num_incomplete_task_sources_.load(std::memory_order_acquire) == 0 || IsShutdownComplete())
    {
        CallFlushCallbackForTesting();
    }
}
...
void
TaskTracker::SetCanRunPolicy(CanRunPolicy can_run_policy)
{
    can_run_policy_.store(can_run_policy);
}

bool
TaskTracker::WillPostTask(Task* task, TaskShutdownBehavior shutdown_behavior)
{
    DCHECK(task);
    DCHECK(task->task);
...
bool
TaskTracker::WillPostTaskNow(const Task& task, TaskPriority priority)
{
    ...
    if (has_log_best_effort_tasks_switch_ && priority == TaskPriority::BEST_EFFORT)
    {
...
TaskTracker::RegisterTaskSource(scoped_refptr<TaskSource> task_source)
{
    DCHECK(task_source);
...
bool
TaskTracker::CanRunPriority(TaskPriority priority) const
{
    ...
    if (can_run_policy == CanRunPolicy::kForegroundOnly && priority >= TaskPriority::USER_VISIBLE)
    {
        return true;
    }
...
RegisteredTaskSource
TaskTracker::RunAndPopNextTask(RegisteredTaskSource task_source)
{
    DCHECK(task_source);
...
bool
TaskTracker::HasShutdownStarted() const
{
    return state_->HasShutdownStarted();
}

bool
TaskTracker::IsShutdownComplete() const
{
    CheckedAutoLock auto_lock(shutdown_lock_);
...
void
TaskTracker::RunTask(Task task, TaskSource* task_source, const TaskTraits& traits)
{
    DCHECK(task_source);

    const auto environment = task_source->GetExecutionEnvironment();

    const bool previous_singleton_allowed = ThreadRestrictions::SetSingletonAllowed(
            traits.shutdown_behavior() != TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN);
    const bool previous_io_allowed = ThreadRestrictions::SetIOAllowed(traits.may_block());
...

    {
        ...
        ScopedSetSequenceTokenForCurrentThread scoped_set_sequence_token_for_current_thread(
                environment.token);
        ScopedSetTaskPriorityForCurrentThread scoped_set_task_priority_for_current_thread(
                traits.priority());
        ...
        ScopedSetSequenceLocalStorageMapForCurrentThread
                scoped_set_sequence_local_storage_map_for_current_thread(
                        environment.sequence_local_storage ? environment.sequence_local_storage
        ...
        switch (task_source->execution_mode())
        {
            ...
            case TaskSourceExecutionMode::kParallel: break;
...
void
TaskTracker::BeginCompleteShutdown(base::WaitableEvent& shutdown_event)
{
    // Do nothing in production, tests may override this.
}

bool
TaskTracker::HasIncompleteTaskSourcesForTesting() const
{
    return num_incomplete_task_sources_.load(std::memory_order_acquire) != 0;
}

bool
TaskTracker::BeforeQueueTaskSource(TaskShutdownBehavior shutdown_behavior)
{
    if (shutdown_behavior == TaskShutdownBehavior::BLOCK_SHUTDOWN)
    {
...
bool
TaskTracker::BeforeRunTask(TaskShutdownBehavior shutdown_behavior)
{
...
void
TaskTracker::AfterRunTask(TaskShutdownBehavior shutdown_behavior)
{
...
TaskTracker::UnregisterTaskSource(scoped_refptr<TaskSource> task_source)
{
    DCHECK(task_source);
    if (task_source->shutdown_behavior() == TaskShutdownBehavior::BLOCK_SHUTDOWN)
    {
        DecrementNumItemsBlockingShutdown();
    }
...
void
TaskTracker::DecrementNumItemsBlockingShutdown()
{
...
void
TaskTracker::DecrementNumIncompleteTaskSources()
{
    const auto prev_num_incomplete_task_sources = num_incomplete_task_sources_.fetch_sub(1);
...
void
TaskTracker::CallFlushCallbackForTesting()
{
...
NOINLINE void
TaskTracker::RunContinueOnShutdown(Task* task)
{
    task_annotator_.RunTask("ThreadPool_RunTask_ContinueOnShutdown", task);
}

NOINLINE void
TaskTracker::RunSkipOnShutdown(Task* task)
{
    task_annotator_.RunTask("ThreadPool_RunTask_SkipOnShutdown", task);
}

NOINLINE void
TaskTracker::RunBlockShutdown(Task* task)
{
    task_annotator_.RunTask("ThreadPool_RunTask_BlockShutdown", task);
}

void
TaskTracker::RunTaskWithShutdownBehavior(TaskShutdownBehavior shutdown_behavior, Task* task)
{
    switch (shutdown_behavior)
    {
        case TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN: RunContinueOnShutdown(task); return;
        case TaskShutdownBehavior::SKIP_ON_SHUTDOWN: RunSkipOnShutdown(task); return;
        case TaskShutdownBehavior::BLOCK_SHUTDOWN: RunBlockShutdown(task); return;
    }
}
