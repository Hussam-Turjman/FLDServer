...
class CORE_EXPORT TaskTracker
{
public:
    TaskTracker();
    TaskTracker(const TaskTracker&) = delete;
    TaskTracker&
    operator=(const TaskTracker&) = delete;
    virtual ~TaskTracker();
    ...
    void
    FlushAsyncForTesting(OnceClosure flush_callback);
    ...
    void
    SetCanRunPolicy(CanRunPolicy can_run_policy);
    ...
    bool
    WillPostTask(Task* task, TaskShutdownBehavior shutdown_behavior);
    ...
    bool
    WillPostTaskNow(const Task& task, TaskPriority priority) WARN_UNUSED_RESULT;
    ...
    RegisterTaskSource(scoped_refptr<TaskSource> task_source);

    // Returns true if a task with |priority| can run under to the current policy.
    bool
    CanRunPriority(TaskPriority priority) const;
    ...
    // called.
    RegisteredTaskSource
    RunAndPopNextTask(RegisteredTaskSource task_source);
    ...
    bool
    IsShutdownComplete() const;

    TrackedRef<TaskTracker>
    GetTrackedRef()
    {
    ...
    bool
    HasIncompleteTaskSourcesForTesting() const;
...
protected:
    ...
    // after doing so.
    virtual void
    RunTask(Task task, TaskSource* task_source, const TaskTraits& traits);

    // Allow a subclass to wait more interactively for any running shutdown tasks
    // before blocking the thread.
    virtual void
    BeginCompleteShutdown(base::WaitableEvent& shutdown_event);
...
private:
    ...
    bool
    BeforeQueueTaskSource(TaskShutdownBehavior shutdown_behavior);
    ...
    bool
    BeforeRunTask(TaskShutdownBehavior shutdown_behavior);
    ...
    void
    AfterRunTask(TaskShutdownBehavior shutdown_behavior);
    ...
    UnregisterTaskSource(scoped_refptr<TaskSource> task_source);
    ...
    void
    RunContinueOnShutdown(Task* task);
    void
    RunSkipOnShutdown(Task* task);
    void
    RunBlockShutdown(Task* task);
    void
    RunTaskWithShutdownBehavior(TaskShutdownBehavior shutdown_behavior, Task* task);
