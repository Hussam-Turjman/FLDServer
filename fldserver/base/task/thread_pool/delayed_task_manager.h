...
class CORE_EXPORT DelayedTaskManager
{
public:
    ...
    DelayedTaskManager(const TickClock* tick_clock = DefaultTickClock::GetInstance());
    DelayedTaskManager(const DelayedTaskManager&) = delete;
    DelayedTaskManager&
    operator=(const DelayedTaskManager&) = delete;
    ~DelayedTaskManager();
    ...
    // thread.
    void
    Start(scoped_refptr<SequencedTaskRunner> service_thread_task_runner);
...
private:
    ...
    {
        DelayedTask();
        DelayedTask(Task task, PostTaskNowCallback callback, scoped_refptr<TaskRunner> task_runner);
        DelayedTask(DelayedTask&& other);
        DelayedTask(const DelayedTask&) = delete;
        DelayedTask&
        operator=(const DelayedTask&) = delete;
        ~DelayedTask();

        // Required by IntrusiveHeap::insert().
        DelayedTask&
        operator=(DelayedTask&& other);

        // Required by IntrusiveHeap.
        bool
        operator<=(const DelayedTask& other) const;
    ...
    TimeTicks
    GetTimeToScheduleProcessRipeTasksLockRequired() EXCLUSIVE_LOCKS_REQUIRED(queue_lock_);
    ...
    // TimeTicks::Max().
    void
    ScheduleProcessRipeTasksOnServiceThread(TimeTicks process_ripe_tasks_time);
