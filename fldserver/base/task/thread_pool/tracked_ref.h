...
public:
    ...
    TrackedRef(TrackedRef<T>&& other) : ptr_(other.ptr_), factory_(other.factory_)
    {
        // Null out |other_|'s factory so its destructor doesn't decrement
        // |live_tracked_refs_|.
        other.factory_ = nullptr;
    }
    TrackedRef(const TrackedRef<T>& other) : ptr_(other.ptr_), factory_(other.factory_)
    {
    ...
    // this at the moment.
    TrackedRef&
    operator=(TrackedRef<T>&& other) = delete;
    TrackedRef&
    operator=(const TrackedRef<T>& other) = delete;
    ...
    bool
    operator==(const void* compared_ptr) const
    {
...
private:
    ...
    TrackedRef(T* ptr, TrackedRefFactory<T>* factory) : ptr_(ptr), factory_(factory)
    {
        factory_->live_tracked_refs_.Increment();
    }
...
{
public:
    explicit TrackedRefFactory(T* ptr) : ptr_(ptr), self_ref_(TrackedRef<T>(ptr_, this))
    {
        DCHECK(ptr_);
    }
    ...
    TrackedRefFactory&
    operator=(const TrackedRefFactory&) = delete;
