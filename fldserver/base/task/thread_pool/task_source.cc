...
namespace internal
{
TaskSource::Transaction::Transaction(TaskSource* task_source) : task_source_(task_source)
{
...
void
TaskSource::Transaction::UpdatePriority(TaskPriority priority)
{
    task_source_->traits_.UpdatePriority(priority);
    task_source_->priority_racy_.store(task_source_->traits_.priority(), std::memory_order_relaxed);
}

void
TaskSource::SetHeapHandle(const HeapHandle& handle)
{
    heap_handle_ = handle;
}

void
TaskSource::ClearHeapHandle()
{
...
RegisteredTaskSource::RegisteredTaskSource(std::nullptr_t) : RegisteredTaskSource()
{
}

RegisteredTaskSource::RegisteredTaskSource(RegisteredTaskSource&& other) noexcept :
#if DCHECK_IS_ON()
...
RegisteredTaskSource::CreateForTesting(scoped_refptr<TaskSource> task_source,
...
RegisteredTaskSource::Unregister()
{
#if DCHECK_IS_ON()
...
RegisteredTaskSource::WillRunTask()
{
...
Task
RegisteredTaskSource::TakeTask(TaskSource::Transaction* transaction)
{
...
Task
RegisteredTaskSource::Clear(TaskSource::Transaction* transaction)
{
    DCHECK(!transaction || transaction->task_source() == get());
...
bool
RegisteredTaskSource::DidProcessTask(TaskSource::Transaction* transaction)
{
    DCHECK(!transaction || transaction->task_source() == get());
#if DCHECK_IS_ON()
    DCHECK_EQ(State::kReady, run_step_);
...
RegisteredTaskSource::RegisteredTaskSource(scoped_refptr<TaskSource> task_source,
                                           TaskTracker* task_tracker) :
...
TransactionWithRegisteredTaskSource::TransactionWithRegisteredTaskSource(
        RegisteredTaskSource task_source_in, TaskSource::Transaction transaction_in) :
    task_source(std::move(task_source_in)), transaction(std::move(transaction_in))
{
...
TransactionWithRegisteredTaskSource::FromTaskSource(RegisteredTaskSource task_source_in)
{
    auto transaction = task_source_in->BeginTransaction();
    return TransactionWithRegisteredTaskSource(std::move(task_source_in), std::move(transaction));
}
