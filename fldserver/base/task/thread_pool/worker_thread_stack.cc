...
void
WorkerThreadStack::Push(WorkerThread* worker)
{
...
bool
WorkerThreadStack::Contains(const WorkerThread* worker) const
{
    return base::Contains(stack_, worker);
}

void
WorkerThreadStack::Remove(const WorkerThread* worker)
{
    DCHECK(!IsEmpty());
