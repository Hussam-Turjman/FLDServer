...
JobDelegate::JobDelegate(internal::JobTaskSource* task_source,
    ...
    task_source_(task_source), pooled_task_runner_delegate_(pooled_task_runner_delegate)
{
    DCHECK(task_source_);
}
...
void
JobDelegate::YieldIfNeeded()
{
    // TODO(crbug.com/839091): Implement this.
}

void
JobDelegate::NotifyConcurrencyIncrease()
{
...
JobHandle::~JobHandle()
{
    DCHECK(!task_source_) << "The Job must be cancelled, detached or joined before its "
...
bool
JobHandle::IsActive() const
{
    return task_source_->IsActive();
}

void
JobHandle::UpdatePriority(TaskPriority new_priority)
{
    task_source_->delegate()->UpdateJobPriority(task_source_, new_priority);
}

void
JobHandle::NotifyConcurrencyIncrease()
{
    task_source_->NotifyConcurrencyIncrease();
}
...
JobHandle::Join()
{
    DCHECK_GE(internal::GetTaskPriorityForCurrentThread(), task_source_->priority_racy())
...
{
    DCHECK(ThreadPoolInstance::Get()) << "Ref. Prerequisite section of post_task.h.\n\n"
                                         "Hint: if this is in a unit test, you're likely merely "
                                         "missing a "
    ...
    const bool queued = static_cast<internal::ThreadPoolImpl*>(ThreadPoolInstance::Get())
