...
class CORE_EXPORT CurrentThread
{
public:
    ...
    CurrentThread&
    operator=(const CurrentThread& other) = default;

    bool
    operator==(const CurrentThread& other) const;
    ...
    // immediately.
    void
    AddDestructionObserver(DestructionObserver* destruction_observer);

    // Remove a DestructionObserver.  It is safe to call this method while a
    // DestructionObserver is receiving a notification callback.
    void
    RemoveDestructionObserver(DestructionObserver* destruction_observer);
    ...
    void
    AddTaskTimeObserver(sequence_manager::TaskTimeObserver* task_observer);
    void
    RemoveTaskTimeObserver(sequence_manager::TaskTimeObserver* task_observer);
    ...
    class CORE_EXPORT ScopedAllowApplicationTasksInNativeNestedLoop
    {
    public:
    ...
    using ScopedNestableTaskAllower = ScopedAllowApplicationTasksInNativeNestedLoop;
...
protected:
    explicit CurrentThread(sequence_manager::internal::SequenceManagerImpl* sequence_manager) :
...
class CORE_EXPORT CurrentUIThread : public CurrentThread
{
public:
    // Returns an interface for the CurrentUIThread of the current thread.
    // Asserts that IsSet().
    static CurrentUIThread
    Get();
...
#if IS_WIN
    void
    AddMessagePumpObserver(MessagePumpForUI::Observer* observer);
    void
    RemoveMessagePumpObserver(MessagePumpForUI::Observer* observer);
#endif

private:
    explicit CurrentUIThread(sequence_manager::internal::SequenceManagerImpl* current) :
        CurrentThread(current)
    {
    }

    MessagePumpForUI*
    GetMessagePumpForUI() const;
};

#endif  // !defined(OS_NACL)

// ForIO extension of CurrentThread.
class CORE_EXPORT CurrentIOThread : public CurrentThread
{
public:
    // Returns an interface for the CurrentIOThread of the current thread.
    // Asserts that IsSet().
    static CurrentIOThread
    Get();
    ...
    HRESULT
    RegisterIOHandler(HANDLE file, MessagePumpForIO::IOHandler* handler);
    bool
    RegisterJobObject(HANDLE job, MessagePumpForIO::IOHandler* handler);
...
#if defined(OS_MAC)
    bool
    WatchMachReceivePort(mach_port_t port,
    ...
    bool
    WatchZxHandle(zx_handle_t handle,
                  bool persistent,
...
private:
    explicit CurrentIOThread(sequence_manager::internal::SequenceManagerImpl* current) :
        CurrentThread(current)
    {
    }

    MessagePumpForIO*
    GetMessagePumpForIO() const;
};
