...
GetThreadPoolImpl()
{
    ...
    DCHECK(instance) << "Ref. Prerequisite section of base/task/thread_pool.h.\n"
...
// static
bool
ThreadPool::PostTask(const Location& from_here, OnceClosure task)
{
...
// static
bool
ThreadPool::PostDelayedTask(const Location& from_here, OnceClosure task, TimeDelta delay)
{
...
// static
bool
ThreadPool::PostTaskAndReply(const Location& from_here, OnceClosure task, OnceClosure reply)
{
    return ThreadPool::PostTaskAndReply(from_here, {}, std::move(task), std::move(reply));
}

// static
bool
ThreadPool::PostTask(const Location& from_here, const TaskTraits& traits, OnceClosure task)
{
    return ThreadPool::PostDelayedTask(from_here, traits, std::move(task), TimeDelta());
}

// static
bool
ThreadPool::PostDelayedTask(const Location& from_here,
                            const TaskTraits& traits,
                            OnceClosure task,
                            TimeDelta delay)
{
    return GetThreadPoolImpl()->PostDelayedTask(from_here, traits, std::move(task), delay);
}

// static
bool
ThreadPool::PostTaskAndReply(const Location& from_here,
                             const TaskTraits& traits,
                             OnceClosure task,
                             OnceClosure reply)
{
...
ThreadPool::CreateTaskRunner(const TaskTraits& traits)
{
...
ThreadPool::CreateSequencedTaskRunner(const TaskTraits& traits)
{
...
ThreadPool::CreateSingleThreadTaskRunner(const TaskTraits& traits,
...
// static
scoped_refptr<SingleThreadTaskRunner>
ThreadPool::CreateCOMSTATaskRunner(const TaskTraits& traits,
                                   SingleThreadTaskRunnerThreadMode thread_mode)
{
