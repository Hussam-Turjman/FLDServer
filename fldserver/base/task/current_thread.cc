...
#include "fldserver/base/message_loop/message_pump_for_ui.h"
...
void
CurrentThread::AddDestructionObserver(DestructionObserver* destruction_observer)
{
...
void
CurrentThread::RemoveDestructionObserver(DestructionObserver* destruction_observer)
{
    DCHECK(current_->IsBoundToCurrentThread());
...
void
CurrentThread::SetTaskRunner(scoped_refptr<SingleThreadTaskRunner> task_runner)
{
    DCHECK(current_->IsBoundToCurrentThread());
...
bool
CurrentThread::IsBoundToCurrentThread() const
{
    return current_ == GetCurrentSequenceManagerImpl();
}

bool
CurrentThread::IsIdleForTesting()
{
    DCHECK(current_->IsBoundToCurrentThread());
...
void
CurrentThread::AddTaskObserver(TaskObserver* task_observer)
{
    DCHECK(current_->IsBoundToCurrentThread());
...
void
CurrentThread::RemoveTaskObserver(TaskObserver* task_observer)
{
    DCHECK(current_->IsBoundToCurrentThread());
...
void
CurrentThread::AddTaskTimeObserver(sequence_manager::TaskTimeObserver* task_observer)
{
    DCHECK(current_->IsBoundToCurrentThread());
...
void
CurrentThread::RemoveTaskTimeObserver(sequence_manager::TaskTimeObserver* task_observer)
{
    DCHECK(current_->IsBoundToCurrentThread());
...
void
CurrentThread::SetAddQueueTimeToTasks(bool enable)
{
    DCHECK(current_->IsBoundToCurrentThread());
...
bool
CurrentThread::NestableTasksAllowed() const
{
    return current_->IsTaskExecutionAllowed();
}

bool
CurrentThread::operator==(const CurrentThread& other) const
{
...
MessagePumpForUI*
CurrentUIThread::GetMessagePumpForUI() const
{
    return static_cast<MessagePumpForUI*>(current_->GetMessagePump());
}

#if defined(USE_OZONE) && !defined(OS_FUCHSIA) && !IS_WIN
bool
CurrentUIThread::WatchFileDescriptor(int fd,
                                     bool persistent,
...
{
    DCHECK(current_->IsBoundToCurrentThread());
    return GetMessagePumpForUI()->WatchFileDescriptor(fd, persistent, mode, controller, delegate);
}
#endif

#if defined(OS_IOS)
void
CurrentUIThread::Attach()
{
...
#if IS_WIN
void
CurrentUIThread::AddMessagePumpObserver(MessagePumpForUI::Observer* observer)
{
    GetMessagePumpForUI()->AddObserver(observer);
}

void
CurrentUIThread::RemoveMessagePumpObserver(MessagePumpForUI::Observer* observer)
{
...
MessagePumpForIO*
CurrentIOThread::GetMessagePumpForIO() const
{
...
#if IS_WIN
HRESULT
CurrentIOThread::RegisterIOHandler(HANDLE file, MessagePumpForIO::IOHandler* handler)
{
    DCHECK(current_->IsBoundToCurrentThread());
...
bool
CurrentIOThread::RegisterJobObject(HANDLE job, MessagePumpForIO::IOHandler* handler)
{
    DCHECK(current_->IsBoundToCurrentThread());
...
bool
CurrentIOThread::WatchFileDescriptor(int fd,
                                     bool persistent,
...
{
    DCHECK(current_->IsBoundToCurrentThread());
    return GetMessagePumpForIO()->WatchFileDescriptor(fd, persistent, mode, controller, delegate);
}
#endif  // IS_WIN

#if defined(OS_MAC)
bool
CurrentIOThread::WatchMachReceivePort(mach_port_t port,
                                      MessagePumpForIO::MachPortWatchController* controller,
                                      MessagePumpForIO::MachPortWatcher* delegate)
{
    DCHECK(current_->IsBoundToCurrentThread());
    return GetMessagePumpForIO()->WatchMachReceivePort(port, controller, delegate);
}
#endif
...
bool
CurrentIOThread::WatchZxHandle(zx_handle_t handle,
                               bool persistent,
...
{
    DCHECK(current_->IsBoundToCurrentThread());
    return GetMessagePumpForIO()->WatchZxHandle(handle, persistent, signals, controller, delegate);
}
#endif
