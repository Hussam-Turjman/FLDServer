...
class CORE_EXPORT TaskExecutor
{
public:
    ...
    virtual scoped_refptr<TaskRunner>
    CreateTaskRunner(const TaskTraits& traits) = 0;
    ...
    CreateSequencedTaskRunner(const TaskTraits& traits) = 0;
...
// supported.
void CORE_EXPORT
RegisterTaskExecutor(uint8_t extension_id, TaskExecutor* task_executor);
void CORE_EXPORT
UnregisterTaskExecutorForTesting(uint8_t extension_id);

// Stores the provided TaskExecutor in TLS for the current thread, to be used by
// tasks with the CurrentThread() trait.
void CORE_EXPORT
SetTaskExecutorForCurrentThread(TaskExecutor* task_executor);

// Returns the task executor registered for the current thread.
CORE_EXPORT TaskExecutor*
...
TaskExecutor*
GetRegisteredTaskExecutorForTraits(const TaskTraits& traits);
