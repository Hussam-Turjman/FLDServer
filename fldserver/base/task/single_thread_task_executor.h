...
class CORE_EXPORT SingleThreadTaskExecutor
{
public:
    // For MessagePumpType::CUSTOM use the constructor that takes a pump.
    explicit SingleThreadTaskExecutor(MessagePumpType type = MessagePumpType::DEFAULT);
    ...
    void
    SetWorkBatchSize(size_t work_batch_size);

private:
    explicit SingleThreadTaskExecutor(MessagePumpType type, std::unique_ptr<MessagePump> pump);
