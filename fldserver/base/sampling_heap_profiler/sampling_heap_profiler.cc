...
#include "fldserver/base/sampling_heap_profiler/sampling_heap_profiler.h"
...
#include "fldserver/base/sampling_heap_profiler/lock_free_address_hash_set.h"
#include "fldserver/base/threading/thread_local_storage.h"
#include "fldserver/base/trace_event/heap_profiler_allocation_context_tracker.h"  // no-presubmit-check
#include "fldserver/fldserver_config.h"
#include "fldserver/fldserver_config.h"
...
#if defined(OS_ANDROID) && BUILDFLAG(CAN_UNWIND_WITH_CFI_TABLE) && defined(OFFICIAL_BUILD)
#include "fldserver/base/trace_event/cfi_backtrace_android.h"  // no-presubmit-check
#endif
...
{
    const char* thread_name = base::ThreadIdNameManager::GetInstance()->GetNameForCurrentThread();
...
const char*
UpdateAndGetThreadName(const char* name)
{
...
SamplingHeapProfiler::Sample::Sample(size_t size, size_t total, uint32_t ordinal) :
...
uint32_t
SamplingHeapProfiler::Start()
{
#if defined(OS_ANDROID) && BUILDFLAG(CAN_UNWIND_WITH_CFI_TABLE) && defined(OFFICIAL_BUILD)
    if (!trace_event::CFIBacktraceAndroid::GetInitializedInstance()->can_unwind_stack_frames())
    {
...
void
SamplingHeapProfiler::SetSamplingInterval(size_t sampling_interval)
{
    PoissonAllocationSampler::Get()->SetSamplingInterval(sampling_interval);
}

void
SamplingHeapProfiler::SetRecordThreadNames(bool value)
{
...
// static
const char*
SamplingHeapProfiler::CachedThreadName()
{
...
// static
void**
SamplingHeapProfiler::CaptureStackTrace(void** frames, size_t max_entries, size_t* count)
{
    // Skip top frames as they correspond to the profiler itself.
    size_t skip_frames = 3;
#if defined(OS_ANDROID) && BUILDFLAG(CAN_UNWIND_WITH_CFI_TABLE) && defined(OFFICIAL_BUILD)
    size_t frame_count = base::trace_event::CFIBacktraceAndroid::GetInitializedInstance()->Unwind(
    ...
    size_t frame_count = base::debug::CollectStackTrace(const_cast<void**>(frames), max_entries);
#endif
...
void
SamplingHeapProfiler::SampleAdded(void* address,
                                  size_t size,
                                  size_t total,
                                  ...
                                  const char* context)
{
    ...
    CaptureMode capture_mode = trace_event::AllocationContextTracker::capture_mode();
...
void
SamplingHeapProfiler::CaptureMixedStack(const char* context, Sample* sample)
{
    auto* tracker = trace_event::AllocationContextTracker::GetInstanceForCurrentThread();
    if (!tracker)
        return;
    ...
    for (int i = base::checked_cast<int>(backtrace.frame_count) - 1; i >= 0; --i)
    {
...
void
SamplingHeapProfiler::CaptureNativeStack(const char* context, Sample* sample)
{
    void* stack[kMaxStackEntries];
    size_t frame_count;
    // One frame is reserved for the thread name.
    void** first_frame = CaptureStackTrace(stack, kMaxStackEntries - 1, &frame_count);
    ...
    if (!context)
    {
        const auto* tracker = trace_event::AllocationContextTracker::GetInstanceForCurrentThread();
        if (tracker)
...
const char*
SamplingHeapProfiler::RecordString(const char* string)
{
    return string ? *strings_.insert(string).first : nullptr;
}

void
SamplingHeapProfiler::SampleRemoved(void* address)
{
...
SamplingHeapProfiler::GetSamples(uint32_t profile_id)
{
...
SamplingHeapProfiler::GetStrings()
{
    PoissonAllocationSampler::ScopedMuteThreadSamples no_samples_scope;
    AutoLock lock(mutex_);
...
void
SamplingHeapProfiler::OnThreadNameChanged(const char* name)
{
