...
#include "fldserver/base/sampling_heap_profiler/poisson_allocation_sampler.h"

#include "fldserver/fldserver_config.h"
#include "fldserver/base/allocator/allocator_shim.h"
...
#if IS_APPLE || defined(OS_ANDROID)
#include <pthread.h>
#endif
...
void*
AllocFn(const AllocatorDispatch* self, size_t size, void* context)
{
    ReentryGuard guard;
...
void*
AllocUncheckedFn(const AllocatorDispatch* self, size_t size, void* context)
{
    ReentryGuard guard;
    void* address = self->next->alloc_unchecked_function(self->next, size, context);
    if (LIKELY(guard))
    {
        PoissonAllocationSampler::RecordAlloc(
                address, size, PoissonAllocationSampler::kMalloc, nullptr);
...
void*
AllocZeroInitializedFn(const AllocatorDispatch* self, size_t n, size_t size, void* context)
{
    ReentryGuard guard;
    void* address = self->next->alloc_zero_initialized_function(self->next, n, size, context);
    if (LIKELY(guard))
    {
        PoissonAllocationSampler::RecordAlloc(
...
void*
AllocAlignedFn(const AllocatorDispatch* self, size_t alignment, size_t size, void* context)
{
    ReentryGuard guard;
    void* address = self->next->alloc_aligned_function(self->next, alignment, size, context);
    if (LIKELY(guard))
    {
        PoissonAllocationSampler::RecordAlloc(
                address, size, PoissonAllocationSampler::kMalloc, nullptr);
...
void*
ReallocFn(const AllocatorDispatch* self, void* address, size_t size, void* context)
{
    ReentryGuard guard;
...
void
FreeFn(const AllocatorDispatch* self, void* address, void* context)
{
...
size_t
GetSizeEstimateFn(const AllocatorDispatch* self, void* address, void* context)
{
    return self->next->get_size_estimate_function(self->next, address, context);
}

unsigned
BatchMallocFn(const AllocatorDispatch* self,
              size_t size,
              void** results,
              ...
              void* context)
{
    ...
    unsigned num_allocated =
            self->next->batch_malloc_function(self->next, size, results, num_requested, context);
    if (LIKELY(guard))
    {
...
void
BatchFreeFn(const AllocatorDispatch* self,
            void** to_be_freed,
            ...
            void* context)
{
    ...
    self->next->batch_free_function(self->next, to_be_freed, num_to_be_freed, context);
}

void
FreeDefiniteSizeFn(const AllocatorDispatch* self, void* address, size_t size, void* context)
{
    PoissonAllocationSampler::RecordFree(address);
...
static void*
AlignedMallocFn(const AllocatorDispatch* self, size_t size, size_t alignment, void* context)
{
    ReentryGuard guard;
    void* address = self->next->aligned_malloc_function(self->next, size, alignment, context);
    if (LIKELY(guard))
    {
        PoissonAllocationSampler::RecordAlloc(
                address, size, PoissonAllocationSampler::kMalloc, nullptr);
...
static void*
AlignedReallocFn(
        const AllocatorDispatch* self, void* address, size_t size, size_t alignment, void* context)
{
    ReentryGuard guard;
    // Note: size == 0 actually performs free.
    PoissonAllocationSampler::RecordFree(address);
    address = self->next->aligned_realloc_function(self->next, address, size, alignment, context);
    if (LIKELY(guard))
    {
        PoissonAllocationSampler::RecordAlloc(
                address, size, PoissonAllocationSampler::kMalloc, nullptr);
...
static void
AlignedFreeFn(const AllocatorDispatch* self, void* address, void* context)
{
    PoissonAllocationSampler::RecordFree(address);
...
void
PartitionAllocHook(void* address, size_t size, const char* type)
{
    PoissonAllocationSampler::RecordAlloc(
...
void
PartitionFreeHook(void* address)
{
    PoissonAllocationSampler::RecordFree(address);
}
...
// static
bool
PoissonAllocationSampler::ScopedMuteThreadSamples::IsMuted()
{
...
// static
bool
PoissonAllocationSampler::InstallAllocatorHooks()
{
#if 0
...
#if BUILDFLAG(USE_PARTITION_ALLOC) && !defined(OS_NACL)
    PartitionAllocHooks::SetObserverHooks(&PartitionAllocHook, &PartitionFreeHook);
#endif  // BUILDFLAG(USE_PARTITION_ALLOC) && !defined(OS_NACL)
...
// static
void
PoissonAllocationSampler::SetHooksInstallCallback(void (*hooks_install_callback)())
{
...
void
PoissonAllocationSampler::SetSamplingInterval(size_t sampling_interval)
{
...
// static
size_t
PoissonAllocationSampler::GetNextSampleInterval(size_t interval)
{
...
// static
void
PoissonAllocationSampler::RecordAlloc(void* address,
                                      size_t size,
                                      AllocatorType type,
                                      const char* context)
{
...
void
PoissonAllocationSampler::DoRecordAlloc(intptr_t accumulated_bytes,
                                        size_t size,
                                        void* address,
                                        AllocatorType type,
                                        const char* context)
...
void
PoissonAllocationSampler::DoRecordFree(void* address)
{
    if (UNLIKELY(ScopedMuteThreadSamples::IsMuted()))
        return;
...
void
PoissonAllocationSampler::BalanceAddressesHashSet()
{
    ...
    if (current_set.load_factor() < 1)
        return;
    auto new_set = std::make_unique<LockFreeAddressHashSet>(current_set.buckets_count() * 2);
...
PoissonAllocationSampler::sampled_addresses_set()
{
...
// static
void
PoissonAllocationSampler::SuppressRandomnessForTest(bool suppress)
{
    g_deterministic = suppress;
}

void
PoissonAllocationSampler::AddSamplesObserver(SamplesObserver* observer)
{
    ScopedMuteThreadSamples no_reentrancy_scope;
    AutoLock lock(mutex_);
...
void
PoissonAllocationSampler::RemoveSamplesObserver(SamplesObserver* observer)
{
    ScopedMuteThreadSamples no_reentrancy_scope;
    AutoLock lock(mutex_);
