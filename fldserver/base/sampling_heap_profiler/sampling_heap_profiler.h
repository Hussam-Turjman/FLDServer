...
class CORE_EXPORT SamplingHeapProfiler : private PoissonAllocationSampler::SamplesObserver,
                                         public base::ThreadIdNameManager::Observer
{
public:
    class CORE_EXPORT Sample
    {
    public:
        Sample(const Sample&);
        ~Sample();
    ...
    // set to 0.
    std::vector<Sample>
    GetSamples(uint32_t profile_id);
    ...
    static void**
    CaptureStackTrace(void** frames, size_t max_entries, size_t* count);

    static void
    Init();
    static SamplingHeapProfiler*
    Get();

    // ThreadIdNameManager::Observer implementation:
    void
    OnThreadNameChanged(const char* name) override;
...
private:
    ...
    void
    SampleRemoved(void* address) override;

    void
    CaptureMixedStack(const char* context, Sample* sample);
    void
    CaptureNativeStack(const char* context, Sample* sample);
    const char*
    RecordString(const char* string) EXCLUSIVE_LOCKS_REQUIRED(mutex_);
