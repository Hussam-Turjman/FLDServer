...
// ...
// N-1: {*}--> {keyM,*}--> NULL
class CORE_EXPORT LockFreeAddressHashSet
{
public:
    ...
    // Concurrent execution of |Insert|, |Remove|, or |Copy| is not supported.
    void
    Copy(const LockFreeAddressHashSet& other);
...
ALWAYS_INLINE
LockFreeAddressHashSet::Node::Node(void* key, Node* next) : next(next)
{
...
ALWAYS_INLINE void
LockFreeAddressHashSet::Remove(void* key)
{
...
ALWAYS_INLINE LockFreeAddressHashSet::Node*
LockFreeAddressHashSet::FindNode(void* key) const
{
    ...
    for (Node* node = bucket.load(std::memory_order_acquire); node != nullptr; node = node->next)
    {
...
LockFreeAddressHashSet::Hash(void* key)
{
