...
ThreadLocalPointer<FileDescriptorWatcher>&
GetTlsFdWatcher()
{
...
class FileDescriptorWatcher::Controller::Watcher : public MessagePumpForIO::FdWatcher,
...
private:
    ...
    void
    OnFileCanReadWithoutBlocking(int fd) override;
    void
    OnFileCanWriteWithoutBlocking(int fd) override;

    // CurrentThread::DestructionObserver:
    void
    WillDestroyCurrentMessageLoop() override;
...
FileDescriptorWatcher::Controller::Watcher::Watcher(WeakPtr<Controller> controller,
                                                    base::WaitableEvent& on_destroyed,
                                                    MessagePumpForIO::Mode mode,
                                                    int fd) :
...
void
FileDescriptorWatcher::Controller::Watcher::StartWatching()
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
void
FileDescriptorWatcher::Controller::Watcher::OnFileCanReadWithoutBlocking(int fd)
{
    DCHECK_EQ(fd_, fd);
    ...
    callback_task_runner_->PostTask(FROM_HERE, BindOnce(&Controller::RunCallback, controller_));
}

void
FileDescriptorWatcher::Controller::Watcher::OnFileCanWriteWithoutBlocking(int fd)
{
    DCHECK_EQ(fd_, fd);
...
void
FileDescriptorWatcher::Controller::Watcher::WillDestroyCurrentMessageLoop()
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
FileDescriptorWatcher::Controller::Controller(MessagePumpForIO::Mode mode,
    ...
    callback_(callback), io_thread_task_runner_(GetTlsFdWatcher().Get()->io_thread_task_runner())
{
    DCHECK(!callback_.is_null());
    DCHECK(io_thread_task_runner_);
    watcher_ = new Watcher(weak_factory_.GetWeakPtr(), on_watcher_destroyed_, mode, fd);
    StartWatching();
}
...
void
FileDescriptorWatcher::Controller::StartWatching()
{
    DCHECK(sequence_checker_.CalledOnValidSequence());
    if (io_thread_task_runner_->BelongsToCurrentThread())
    {
    ...
    else
    {
        ...
        io_thread_task_runner_->PostTask(FROM_HERE,
                                         BindOnce(&Watcher::StartWatching, Unretained(watcher_)));
    }
}

void
FileDescriptorWatcher::Controller::RunCallback()
{
    DCHECK(sequence_checker_.CalledOnValidSequence());
...
{
    return WrapUnique(new Controller(MessagePumpForIO::WATCH_WRITE, fd, callback));
}

#if DCHECK_IS_ON()
void
FileDescriptorWatcher::AssertAllowed()
{
    DCHECK(GetTlsFdWatcher().Get());
}
#endif
