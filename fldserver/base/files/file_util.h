...
CORE_EXPORT int64_t
ComputeDirectorySize(const FilePath& root_path);
...
CORE_EXPORT OnceCallback<void(const FilePath&)>
...
CORE_EXPORT bool
Move(const FilePath& from_path, const FilePath& to_path);
...
// |outfile|.
CORE_EXPORT bool
CopyFileContents(File& infile, File& outfile);
...
CORE_EXPORT bool
ContentsEqual(const FilePath& filename1, const FilePath& filename2);

// Returns true if the contents of the two text files given are equal, false
// otherwise.  This routine treats "\r\n" and "\n" as equivalent.
CORE_EXPORT bool
TextContentsEqual(const FilePath& filename1, const FilePath& filename2);
...
CORE_EXPORT bool
ReadFileToString(const FilePath& path, std::string* contents);
...
CORE_EXPORT ScopedFD
CreateAndOpenFdForTemporaryFileInDir(const FilePath& dir, FilePath* path);
...
// false on failure.
CORE_EXPORT bool
CreateSymbolicLink(const FilePath& target, const FilePath& symlink);

// Reads the given |symlink| and returns where it points to in |target|.
// Returns false upon failure.
CORE_EXPORT bool
ReadSymbolicLink(const FilePath& symlink, FilePath* target);
...
CORE_EXPORT bool
IsDirectoryEmpty(const FilePath& dir_path);
...
CORE_EXPORT bool
CreateTemporaryFile(FilePath* path);
...
CORE_EXPORT ScopedFILE
CreateAndOpenTemporaryStream(FilePath* path);
...
CORE_EXPORT bool
CreateDirectory(const FilePath& full_path);

// Returns the file size. Returns true on success.
CORE_EXPORT bool
GetFileSize(const FilePath& file_path, int64_t* file_size);
...
CORE_EXPORT bool
DevicePathToDriveLetterPath(const FilePath& device_path, FilePath* drive_letter_path);
...
CORE_EXPORT bool
GetFileInfo(const FilePath& file_path, File::Info* info);

// Sets the time of the last access and the time of the last modification.
CORE_EXPORT bool
TouchFile(const FilePath& path, const Time& last_accessed, const Time& last_modified);
...
CORE_EXPORT bool
WriteFile(const FilePath& filename, span<const uint8_t> data);
...
CORE_EXPORT bool
WriteFile(const FilePath& filename, StringPiece data);
...
CORE_EXPORT bool
GetCurrentDirectory(FilePath* path);
...
CORE_EXPORT int
GetUniquePathNumber(const FilePath& path);
...
// number can be found.
CORE_EXPORT FilePath
GetUniquePath(const FilePath& path);
...
struct PrefetchResult
{
    ...
    succeeded() const
    {
        return code_ == PrefetchResultCode::kSuccess || code_ == PrefetchResultCode::kSlowSuccess;
    }
...
CORE_EXPORT bool
CreatePipe(ScopedFD* read_fd, ScopedFD* write_fd, bool non_blocking = false);
...
CORE_EXPORT bool
VerifyPathControlledByUser(const base::FilePath& base,
...
CORE_EXPORT bool
VerifyPathControlledByAdmin(const base::FilePath& path);
#endif  // defined(OS_MAC)

// Returns the maximum length of path component on the volume containing
// the directory |path|, in the number of FilePath::CharType, or -1 on failure.
CORE_EXPORT int
GetMaximumPathComponentLength(const base::FilePath& path);
...
CORE_EXPORT bool
GetFileSystemType(const FilePath& path, FileSystemType* type);
#endif
...
bool
PreReadFileSlow(const FilePath& file_path, int64_t max_bytes);
