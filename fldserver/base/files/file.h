...
#include "fldserver/base/files/file_tracing.h"
#include "fldserver/base/files/platform_file.h"
...
public:
    ...
    void
    Initialize(const FilePath& path, uint32_t flags);
    ...
    int64_t
    Seek(Whence whence, int64_t offset);
    ...
    bool
    ReadAndCheck(int64_t offset, span<uint8_t> data);
    bool
    ReadAtCurrentPosAndCheck(span<uint8_t> data);
    bool
    WriteAndCheck(int64_t offset, span<const uint8_t> data);
    bool
    WriteAtCurrentPosAndCheck(span<const uint8_t> data);
    ...
    // an error.
    int
    Read(int64_t offset, char* data, int size);

    // Same as above but without seek.
    int
    ReadAtCurrentPos(char* data, int size);

    // Reads the given number of bytes (or until EOF is reached) starting with the
    // given offset, but does not make any effort to read all data on all
    // platforms. Returns the number of bytes read, or -1 on error.
    int
    ReadNoBestEffort(int64_t offset, char* data, int size);

    // Same as above but without seek.
    int
    ReadAtCurrentPosNoBestEffort(char* data, int size);
    ...
    // with FLAG_APPEND.
    int
    Write(int64_t offset, const char* data, int size);

    // Save as above but without seek.
    int
    WriteAtCurrentPos(const char* data, int size);

    // Save as above but does not make any effort to write all data on all
    // platforms. Returns the number of bytes written, or -1 on error.
    int
    WriteAtCurrentPosNoBestEffort(const char* data, int size);
    ...
    bool
    DeleteOnClose(bool delete_on_close);
#endif
    ...
    static int
    Stat(const char* path, stat_wrapper_t* sb);
    static int
    Fstat(int fd, stat_wrapper_t* sb);
    static int
    Lstat(const char* path, stat_wrapper_t* sb);
#endif
...
private:
    ...
    void
    DoInitialize(const FilePath& path, uint32_t flags);

    void
    SetPlatformFile(PlatformFile file);
