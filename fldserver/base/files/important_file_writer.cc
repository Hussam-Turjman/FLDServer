...
#include "fldserver/base/critical_closure.h"
...
#include "fldserver/base/files/important_file_writer_cleaner.h"
...
#include "fldserver/base/task_runner_util.h"
                                  ...
                                  SampleType max_sample)
{
    ...
    if (!histogram_suffix.empty())
    {
        ...
        histogram_full_name.append(histogram_suffix.data(), histogram_suffix.length());
    }
    UmaHistogramExactLinear(
            histogram_full_name, static_cast<int>(add_sample), static_cast<int>(max_sample));
}

void
UmaHistogramTimesWithSuffix(const char* histogram_name,
                            StringPiece histogram_suffix,
...
void
LogFailure(const FilePath& path,
           StringPiece histogram_suffix,
...
void
DeleteTmpFileWithRetry(File tmp_file,
                       ...
                       int attempt = 0)
{
    ...
    if (!DeleteFile(tmp_file_path))
    {
        ...
        if (++attempt >= kMaxDeleteAttempts)
        {
            ...
            UmaHistogramExactLinearWithSuffix("ImportantFile.FileDeleteRetryExceededError",
                                              histogram_suffix,
                                              -last_file_error,
                                              -File::FILE_ERROR_MAX);
        }
                 ...
                 || !SequencedTaskRunnerHandle::Get()->PostDelayedTask(
                         FROM_HERE,
                         BindOnce(&DeleteTmpFileWithRetry,
                                  base::File(),
                                  tmp_file_path,
    ...
    else if (attempt)
    {
        ...
        // needed.
        UmaHistogramExactLinearWithSuffix("ImportantFile.FileDeleteRetrySuccessCount",
                                          histogram_suffix,
                                          attempt,
                                          kMaxDeleteAttempts);
    }
...
// static
bool
ImportantFileWriter::WriteFileAtomically(const FilePath& path,
                                         StringPiece data,
                                         StringPiece histogram_suffix)
{
    // Calling the impl by way of the public WriteFileAtomically, so
    // |from_instance| is false.
    return WriteFileAtomicallyImpl(path,
                                   data,
                                   histogram_suffix,
                                   /*from_instance=*/false);
}
...
// static
void
ImportantFileWriter::ProduceAndWriteStringToFileAtomically(
        const FilePath& path,
        ...
        const std::string& histogram_suffix)
{
    ...
    const bool result = WriteFileAtomicallyImpl(path,
                                                data,
                                                histogram_suffix,
                                                /*from_instance=*/true);
...
// static
bool
ImportantFileWriter::WriteFileAtomicallyImpl(const FilePath& path,
                                             StringPiece data,
                                             StringPiece histogram_suffix,
                                             bool from_instance)
{
    ...
    File tmp_file = CreateAndOpenTemporaryFileInDir(path.DirName(), &tmp_file_path);
    if (!tmp_file.IsValid())
    {
        UmaHistogramExactLinearWithSuffix("ImportantFile.FileCreateError",
                                          ...
                                          -File::FILE_ERROR_MAX);
        LogFailure(path, histogram_suffix, FAILED_CREATING, "could not create temporary file");
        return false;
    }
    ...
    int bytes_written = 0;
    for (const char *scan = data.data(), *const end = scan + data.length(); scan < end;
         scan += bytes_written)
    {
        ...
        if (bytes_written != write_amount)
        {
            ...
            DeleteTmpFileWithRetry(std::move(tmp_file), tmp_file_path, histogram_suffix);
            return false;
        }
    }
    ...
    UmaHistogramExactLinear(
            "ImportantFile.FileReplaceRetryCount", retry_count, kReplaceRetryFailure);
#endif  // IS_WIN
...
#endif
        LogFailure(path, histogram_suffix, FAILED_RENAMING, "could not rename temporary file");
...
ImportantFileWriter::ImportantFileWriter(const FilePath& path,
                                         ...
                                         StringPiece histogram_suffix) :
    ImportantFileWriter(path, std::move(task_runner), kDefaultCommitInterval, histogram_suffix)
{
}
...
bool
ImportantFileWriter::HasPendingWrite() const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void
ImportantFileWriter::WriteNow(std::unique_ptr<std::string> data)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void
ImportantFileWriter::WriteNowWithBackgroundDataProducer(
        BackgroundDataProducerCallback background_data_producer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    auto split_task = SplitOnceCallback(BindOnce(&ProduceAndWriteStringToFileAtomically,
                                                 path_,
...
void
ImportantFileWriter::ScheduleWrite(DataSerializer* serializer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void
ImportantFileWriter::ScheduleWriteWithBackgroundDataSerializer(BackgroundDataSerializer* serializer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
ImportantFileWriter::DoScheduledWrite()
{
    ...
    if (absl::holds_alternative<DataSerializer*>(serializer_))
    {
        ...
        {
            DLOG(WARNING) << "Failed to serialize data to be saved in " << path_.value();
            ClearPendingWrite();
            return;
        }
    ...
    const TimeDelta serialization_duration = TimeTicks::Now() - serialization_start;

    UmaHistogramTimesWithSuffix(
            "ImportantFile.SerializationDuration", histogram_suffix_, serialization_duration);

    WriteNowWithBackgroundDataProducer(std::move(data_producer_for_background_sequence));
    DCHECK(!HasPendingWrite());
}

void
ImportantFileWriter::RegisterOnNextWriteCallbacks(
...
void
ImportantFileWriter::ClearPendingWrite()
{
    timer().Stop();
...
void
ImportantFileWriter::SetTimerForTesting(OneShotTimer* timer_override)
{
