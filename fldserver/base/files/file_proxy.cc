...
#include "fldserver/base/files/file_proxy.h"
...
public:
    FileHelper(FileProxy* proxy, File file) :
        file_(std::move(file)), task_runner_(proxy->task_runner()), proxy_(AsWeakPtr(proxy))
    {
    }
    FileHelper(const FileHelper&) = delete;
    FileHelper&
    operator=(const FileHelper&) = delete;
...
{
public:
    GenericFileHelper(FileProxy* proxy, File file) : FileHelper(proxy, std::move(file))
    {
    }
    GenericFileHelper(const GenericFileHelper&) = delete;
    GenericFileHelper&
    operator=(const GenericFileHelper&) = delete;
    ...
    void
    SetTimes(Time last_access_time, Time last_modified_time)
    {
    ...
    void
    Reply(FileProxy::StatusCallback callback)
    {
        PassFile();
        if (!callback.is_null())
...
{
public:
    CreateOrOpenHelper(FileProxy* proxy, File file) : FileHelper(proxy, std::move(file))
    {
    }
    CreateOrOpenHelper(const CreateOrOpenHelper&) = delete;
    CreateOrOpenHelper&
    operator=(const CreateOrOpenHelper&) = delete;

    void
    RunWork(const FilePath& file_path, int file_flags)
    {
...
{
public:
    CreateTemporaryHelper(FileProxy* proxy, File file) : FileHelper(proxy, std::move(file))
    {
    }
    CreateTemporaryHelper(const CreateTemporaryHelper&) = delete;
    CreateTemporaryHelper&
    operator=(const CreateTemporaryHelper&) = delete;
    ...
    RunWork(uint32_t additional_file_flags)
    {
        ...
        uint32_t file_flags = File::FLAG_WRITE | File::FLAG_TEMPORARY | File::FLAG_CREATE_ALWAYS
    ...
    void
    Reply(FileProxy::CreateTemporaryCallback callback)
    {
        DCHECK(!callback.is_null());
        PassFile();
...
{
public:
    GetInfoHelper(FileProxy* proxy, File file) : FileHelper(proxy, std::move(file))
    {
    }
    GetInfoHelper(const GetInfoHelper&) = delete;
    GetInfoHelper&
    operator=(const GetInfoHelper&) = delete;
    ...
    void
    Reply(FileProxy::GetFileInfoCallback callback)
    {
        PassFile();
        DCHECK(!callback.is_null());
...
public:
    ...
    ReadHelper&
    operator=(const ReadHelper&) = delete;

    void
    RunWork(int64_t offset)
    {
    ...
    void
    Reply(FileProxy::ReadCallback callback)
    {
        PassFile();
        DCHECK(!callback.is_null());
...
{
public:
    WriteHelper(FileProxy* proxy, File file, const char* buffer, int bytes_to_write) :
        FileHelper(proxy, std::move(file)),
    ...
    WriteHelper&
    operator=(const WriteHelper&) = delete;
    ...
    void
    Reply(FileProxy::WriteCallback callback)
    {
        PassFile();
        if (!callback.is_null())
...
bool
FileProxy::CreateOrOpen(const FilePath& file_path, uint32_t file_flags, StatusCallback callback)
{
    DCHECK(!file_.IsValid());
    CreateOrOpenHelper* helper = new CreateOrOpenHelper(this, File());
    return task_runner_->PostTaskAndReply(
            FROM_HERE,
            BindOnce(&CreateOrOpenHelper::RunWork, Unretained(helper), file_path, file_flags),
            BindOnce(&CreateOrOpenHelper::Reply, Owned(helper), std::move(callback)));
}

bool
FileProxy::CreateTemporary(uint32_t additional_file_flags, CreateTemporaryCallback callback)
{
    DCHECK(!file_.IsValid());
    CreateTemporaryHelper* helper = new CreateTemporaryHelper(this, File());
    return task_runner_->PostTaskAndReply(
            FROM_HERE,
            BindOnce(&CreateTemporaryHelper::RunWork, Unretained(helper), additional_file_flags),
            BindOnce(&CreateTemporaryHelper::Reply, Owned(helper), std::move(callback)));
}

bool
FileProxy::IsValid() const
{
    return file_.IsValid();
}

void
FileProxy::SetFile(File file)
{
    DCHECK(!file_.IsValid());
    file_ = std::move(file);
}
...
bool
FileProxy::Close(StatusCallback callback)
{
    DCHECK(file_.IsValid());
...
bool
FileProxy::GetInfo(GetFileInfoCallback callback)
{
    DCHECK(file_.IsValid());
    GetInfoHelper* helper = new GetInfoHelper(this, std::move(file_));
    return task_runner_->PostTaskAndReply(
            FROM_HERE,
            BindOnce(&GetInfoHelper::RunWork, Unretained(helper)),
            BindOnce(&GetInfoHelper::Reply, Owned(helper), std::move(callback)));
}

bool
FileProxy::Read(int64_t offset, int bytes_to_read, ReadCallback callback)
{
    DCHECK(file_.IsValid());
    if (bytes_to_read < 0)
        return false;
...
bool
FileProxy::Write(int64_t offset, const char* buffer, int bytes_to_write, WriteCallback callback)
{
    DCHECK(file_.IsValid());
...
bool
FileProxy::SetTimes(Time last_access_time, Time last_modified_time, StatusCallback callback)
{
    DCHECK(file_.IsValid());
    GenericFileHelper* helper = new GenericFileHelper(this, std::move(file_));
    return task_runner_->PostTaskAndReply(
            FROM_HERE,
            BindOnce(&GenericFileHelper::SetTimes,
                     Unretained(helper),
                     last_access_time,
                     last_modified_time),
            BindOnce(&GenericFileHelper::Reply, Owned(helper), std::move(callback)));
...
bool
FileProxy::SetLength(int64_t length, StatusCallback callback)
{
    DCHECK(file_.IsValid());
    GenericFileHelper* helper = new GenericFileHelper(this, std::move(file_));
    return task_runner_->PostTaskAndReply(
...
bool
FileProxy::Flush(StatusCallback callback)
{
    DCHECK(file_.IsValid());
    GenericFileHelper* helper = new GenericFileHelper(this, std::move(file_));
    return task_runner_->PostTaskAndReply(
