...
{
FilePath
BuildSearchFilter(FileEnumerator::FolderSearchPolicy policy,
                  ...
                  const FilePath::StringType& pattern)
{
    ...
    switch (policy)
    {
        case FileEnumerator::FolderSearchPolicy::MATCH_ONLY: return root_path.Append(pattern);
...
FileEnumerator::Next()
{
    ...
    while (has_find_data_ || !pending_paths_.empty())
    {
        ...
        else
        {
            ...
            if (!FindNextFile(find_handle_, reinterpret_cast<LPWIN32_FIND_DATAW>(&find_data_)))
            {
                FindClose(find_handle_);
        ...
        if (INVALID_HANDLE_VALUE == find_handle_)
        {
            ...
            if (last_error == ERROR_NO_MORE_FILES || error_policy_ == ErrorPolicy::IGNORE_ERRORS)
            {
                continue;
            }
...
bool
FileEnumerator::IsPatternMatched(const FilePath& src) const
{
