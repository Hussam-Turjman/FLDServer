...
namespace
{
const char* const kCommonDoubleExtensionSuffixes[] = {"gz", "xz", "bz2", "z", "bz"};
...
// returns npos.
StringPieceType::size_type
FindDriveLetter(StringPieceType path)
{
...
#if defined(FILE_PATH_USES_DRIVE_LETTERS)
bool
EqualDriveLetterCaseInsensitive(StringPieceType a, StringPieceType b)
{
...
bool
IsPathAbsolute(StringPieceType path)
{
#if defined(FILE_PATH_USES_DRIVE_LETTERS)
...
bool
AreAllSeparators(const StringType& input)
{
    for (auto it : input)
    {
...
FinalExtensionSeparatorPosition(const StringType& path)
{
    // Special case "." and ".."
...
StringType::size_type
ExtensionSeparatorPosition(const StringType& path)
{
    ...
    const StringType::size_type last_separator =
            path.find_last_of(FilePath::kSeparators, last_dot - 1, FilePath::kSeparatorsLength - 1);

    if (penultimate_dot == StringType::npos
        || (last_separator != StringType::npos && penultimate_dot < last_separator))
    {
        return last_dot;
    }
...
bool
IsEmptyOrSpecialCase(const StringType& path)
{
    // Special cases "", ".", and ".."
...
bool
FilePath::operator==(const FilePath& that) const
{
#if defined(FILE_PATH_USES_DRIVE_LETTERS)
...
std::ostream&
operator<<(std::ostream& out, const FilePath& file_path)
{
...
// static
bool
FilePath::IsSeparator(CharType character)
{
...
void
FilePath::GetComponents(std::vector<StringType>* components) const
{
    DCHECK(components);
    if (!components)
        return;
...
bool
FilePath::IsParent(const FilePath& child) const
{
    return AppendRelativePath(child, nullptr);
}

bool
FilePath::AppendRelativePath(const FilePath& child, FilePath* path) const
{
...
FilePath::InsertBeforeExtension(StringPieceType suffix) const
{
    ...
    return FilePath(base::StrCat({RemoveExtension().value(), suffix, Extension()}));
}
...
bool
FilePath::MatchesExtension(StringPieceType extension) const
{
...
bool
FilePath::IsNetwork() const
{
    return path_.length() > 1 && FilePath::IsSeparator(path_[0]) && FilePath::IsSeparator(path_[1]);
}

bool
FilePath::EndsWithSeparator() const
{
    if (empty())
        return false;
...
bool
FilePath::ReferencesParent() const
{
...
std::u16string
FilePath::LossyDisplayName() const
{
    return AsString16(path_);
}

std::string
FilePath::MaybeAsASCII() const
{
    return base::IsStringASCII(path_) ? WideToASCII(path_) : std::string();
}

std::string
FilePath::AsUTF8Unsafe() const
{
    return WideToUTF8(value());
}

std::u16string
FilePath::AsUTF16Unsafe() const
{
...
// static
FilePath
FilePath::FromUTF8Unsafe(StringPiece utf8)
{
...
// static
FilePath
FilePath::FromUTF16Unsafe(StringPiece16 utf16)
{
...
void
FilePath::WriteToPickle(Pickle* pickle) const
{
#if IS_WIN
...
bool
FilePath::ReadFromPickle(PickleIterator* iter)
{
#if IS_WIN
    std::u16string path;
...
int
FilePath::CompareIgnoreCase(StringPieceType string1, StringPieceType string2)
{
...
inline int
HFSReadNextNonIgnorableCodepoint(const char* string, int length, int* index)
{
    int codepoint = 0;
...
int
FilePath::HFSFastUnicodeCompare(StringPieceType string1, StringPieceType string2)
{
    ...
    for (;;)
    {
        int codepoint1 = HFSReadNextNonIgnorableCodepoint(string1.data(), length1, &index1);
        int codepoint2 = HFSReadNextNonIgnorableCodepoint(string2.data(), length2, &index2);
...
StringType
FilePath::GetHFSDecomposedForm(StringPieceType string)
{
    StringType result;
    ...
    if (hfs1.empty() || hfs2.empty())
    {
        ...
        if (!cfstring1 || !cfstring2)
        {
            int comparison = memcmp(
                    string1.data(), string2.data(), std::min(string1.length(), string2.length()));
            if (comparison < 0)
                return -1;
            if (comparison > 0)
                return 1;
...
void
FilePath::StripTrailingSeparatorsInternal()
{
