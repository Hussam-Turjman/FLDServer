...
bool
MemoryMappedFile::MapImageToMemory(Access access)
{
    ...
    file_mapping_.Set(::CreateFileMapping(
            file_.GetPlatformFile(), nullptr, PAGE_READONLY | SEC_IMAGE_NO_EXECUTE, 0, 0, NULL));
    if (!file_mapping_.IsValid())
        return false;

    data_ = static_cast<uint8_t*>(::MapViewOfFile(file_mapping_.Get(), FILE_MAP_READ, 0, 0, 0));
    if (!data_)
        return false;
...
bool
MemoryMappedFile::MapFileRegionToMemory(const MemoryMappedFile::Region& region, Access access)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    ...
    switch (access)
    {
        case READ_ONLY: flags |= PAGE_READONLY; break;
        case READ_WRITE: flags |= PAGE_READWRITE; break;
        case READ_WRITE_EXTEND:
            flags |= PAGE_READWRITE;
            size.QuadPart = region.size;
            break;
        case READ_CODE_IMAGE: return MapImageToMemory(access);
    }

    file_mapping_.Set(::CreateFileMapping(
            file_.GetPlatformFile(), NULL, flags, size.HighPart, size.LowPart, NULL));
    if (!file_mapping_.IsValid())
        return false;
    ...
    else
    {
        ...
        if (aligned_start < 0 || full_map_size < 0
            || !IsValueInRangeForNumericType<SIZE_T>(static_cast<uint64_t>(full_map_size)))
        {
