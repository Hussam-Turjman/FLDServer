...
private:
    ...
    static bool
    SetupWatchHandle(const FilePath& dir, bool recursive, HANDLE* handle) WARN_UNUSED_RESULT;

    // (Re-)Initialize the watch handle.
    bool
    UpdateWatch() WARN_UNUSED_RESULT;
...
void
FilePathWatcherImpl::OnObjectSignaled(HANDLE object)
{
    DCHECK(task_runner()->RunsTasksInCurrentSequence());
...
// static
bool
FilePathWatcherImpl::SetupWatchHandle(const FilePath& dir, bool recursive, HANDLE* handle)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    *handle = FindFirstChangeNotification(
    ...
    if (error_code != ERROR_FILE_NOT_FOUND && error_code != ERROR_PATH_NOT_FOUND
        && error_code != ERROR_ACCESS_DENIED && error_code != ERROR_SHARING_VIOLATION
...
bool
FilePathWatcherImpl::UpdateWatch()
{
    if (handle_ != INVALID_HANDLE_VALUE)
        DestroyWatch();
    ...
    while (!child_dirs.empty())
    {
        ...
        if (!SetupWatchHandle(watched_path, type_ == Type::kRecursive, &temp_handle))
        {
            return false;
        }
...
void
FilePathWatcherImpl::DestroyWatch()
{
