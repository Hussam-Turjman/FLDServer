...
class CORE_EXPORT FileProxy : public SupportsWeakPtr<FileProxy>
{
public:
    ...
    using CreateTemporaryCallback = OnceCallback<void(File::Error, const FilePath&)>;
    using GetFileInfoCallback = OnceCallback<void(File::Error, const File::Info&)>;
    using ReadCallback = OnceCallback<void(File::Error, const char* data, int bytes_read)>;
    ...
    FileProxy&
    operator=(const FileProxy&) = delete;
    ~FileProxy();
    ...
    bool
    CreateOrOpen(const FilePath& file_path, uint32_t file_flags, StatusCallback callback);
    ...
    // This returns false if task posting to |task_runner| has failed.
    bool
    CreateTemporary(uint32_t additional_file_flags, CreateTemporaryCallback callback);
    ...
    // This returns false if task posting to |task_runner| has failed.
    bool
    Close(StatusCallback callback);

    // Proxies File::GetInfo. The callback can't be null.
    // This returns false if task posting to |task_runner| has failed.
    bool
    GetInfo(GetFileInfoCallback callback);
    ...
    bool
    Read(int64_t offset, int bytes_to_read, ReadCallback callback);
    ...
    bool
    Write(int64_t offset, const char* buffer, int bytes_to_write, WriteCallback callback);

    // Proxies File::SetTimes. The callback can be null.
    // This returns false if task posting to |task_runner| has failed.
    bool
    SetTimes(Time last_access_time, Time last_modified_time, StatusCallback callback);

    // Proxies File::SetLength. The callback can be null.
    // This returns false if task posting to |task_runner| has failed.
    bool
    SetLength(int64_t length, StatusCallback callback);

    // Proxies File::Flush. The callback can be null.
    // This returns false if task posting to |task_runner| has failed.
    bool
    Flush(StatusCallback callback);
