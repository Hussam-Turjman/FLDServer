...
#include "fldserver/base/files/scoped_temp_dir.h"
...
namespace
{
constexpr FilePath::CharType kScopedDirPrefix[] = FILE_PATH_LITERAL("scoped_dir");

}  // namespace

ScopedTempDir::ScopedTempDir() = default;

ScopedTempDir::ScopedTempDir(ScopedTempDir&& other) noexcept : path_(other.Take())
{
}

ScopedTempDir&
ScopedTempDir::operator=(ScopedTempDir&& other)
{
...
bool
ScopedTempDir::CreateUniqueTempDir()
{
    if (!path_.empty())
        return false;
...
bool
ScopedTempDir::CreateUniqueTempDirUnderPath(const FilePath& base_path)
{
    if (!path_.empty())
        return false;
...
bool
ScopedTempDir::Set(const FilePath& path)
{
    if (!path_.empty())
        return false;
...
ScopedTempDir::GetTempDirPrefix()
{
