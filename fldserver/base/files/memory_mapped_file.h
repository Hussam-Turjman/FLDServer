...
class CORE_EXPORT MemoryMappedFile
{
public:
    enum Access
    {
    ...
    MemoryMappedFile&
    operator=(const MemoryMappedFile&) = delete;
    ~MemoryMappedFile();

    // Used to hold information about a region [offset + size] of a file.
    struct CORE_EXPORT Region
    {
    ...
    Initialize(const FilePath& file_name, Access access);
    WARN_UNUSED_RESULT bool
    Initialize(const FilePath& file_name)
    {
    ...
    // return false.
    WARN_UNUSED_RESULT bool
    Initialize(File file, Access access);
    WARN_UNUSED_RESULT bool
    ...
    WARN_UNUSED_RESULT bool
    Initialize(File file, const Region& region, Access access);
    WARN_UNUSED_RESULT bool
    Initialize(File file, const Region& region)
    {
...
private:
    ...
    static void
    CalculateVMAlignedBoundaries(int64_t start,
                                 size_t size,
    ...
    bool
    MapImageToMemory(Access access);
#endif

    // Map the file to memory, set data_ to that memory address. Return true on
    // success, false on any kind of failure. This is a helper for Initialize().
    bool
    MapFileRegionToMemory(const Region& region, Access access);
