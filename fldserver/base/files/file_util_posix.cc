...
#include "fldserver/base/strings/sys_string_conversions.h"
...
#if IS_APPLE
#include "fldserver/base/mac/foundation_util.h"

#include <AvailabilityMacros.h>
#endif

#if defined(OS_ANDROID)
#include "fldserver/base/android/content_uri_utils.h"
#include "fldserver/base/os_compat_android.h"
#endif
...
bool
VerifySpecificPathControlledByUser(const FilePath& path,
                                   uid_t owner_uid,
                                   const std::set<gid_t>& group_gids)
{
    ...
    if ((stat_info.st_mode & S_IWGRP) && !Contains(group_gids, stat_info.st_gid))
    {
        DLOG(ERROR) << "Path " << path.value() << " is writable by an unprivileged group.";
        return false;
    }
...
bool
DoCopyDirectory(const FilePath& from_path,
                ...
                bool open_exclusive)
{
    ...
    {
        DPLOG(ERROR) << "CopyDirectory() couldn't stat source directory: " << from_path.value();
        return false;
    }
    ...
    do
    {
        ...
        if (from_path_base != current && !from_path_base.AppendRelativePath(current, &target_path))
        {
            return false;
        }
        ...
        if (S_ISDIR(from_stat.st_mode))
        {
            ...
            DPLOG(ERROR) << "CopyDirectory() couldn't create directory: " << target_path.value();
            return false;
        }

        if (!S_ISREG(from_stat.st_mode))
        {
            DLOG(WARNING) << "CopyDirectory() skipping non-regular file: " << current.value();
            continue;
        }
        ...
        if (!outfile.IsValid())
        {
            DPLOG(ERROR) << "CopyDirectory() couldn't create file: " << target_path.value();
            return false;
        }
...
// here.
bool
DoDeleteFile(const FilePath& path, bool recursive)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    ...
    FileEnumerator traversal(path,
                             true,
                             FileEnumerator::FILES | FileEnumerator::DIRECTORIES
                                     | FileEnumerator::SHOW_SYM_LINKS);
    for (FilePath current = traversal.Next(); !current.empty(); current = traversal.Next())
    {
...
// details.
std::string
AppendModeCharacter(StringPiece mode, char mode_char)
{
...
#if !defined(OS_NACL_NONSFI)
FilePath
MakeAbsoluteFilePath(const FilePath& input)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
...
bool
DeleteFile(const FilePath& path)
{
    return DoDeleteFile(path, /*recursive=*/false);
}

bool
DeletePathRecursively(const FilePath& path)
{
    return DoDeleteFile(path, /*recursive=*/true);
}

bool
ReplaceFile(const FilePath& from_path, const FilePath& to_path, File::Error* error)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
...
bool
CopyDirectoryExcl(const FilePath& from_path, const FilePath& to_path, bool recursive)
{
...
bool
CreatePipe(ScopedFD* read_fd, ScopedFD* write_fd, bool non_blocking)
{
    int fds[2];
    bool created = non_blocking ? CreateLocalNonBlockingPipe(fds) : (0 == pipe(fds));
    if (!created)
        return false;
    read_fd->reset(fds[0]);
...
bool
CreateLocalNonBlockingPipe(int fds[2])
{
...
bool
PathExists(const FilePath& path)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
#if defined(OS_ANDROID)
    if (path.IsContentUri())
...
#if !defined(OS_NACL_NONSFI)
bool
PathIsReadable(const FilePath& path)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
...
#if !defined(OS_NACL_NONSFI)
bool
PathIsWritable(const FilePath& path)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
...
bool
ReadFromFD(int fd, char* buffer, size_t bytes)
{
    size_t total_read = 0;
...
ScopedFD
CreateAndOpenFdForTemporaryFileInDir(const FilePath& directory, FilePath* path)
{
...
bool
CreateSymbolicLink(const FilePath& target_path, const FilePath& symlink_path)
{
    DCHECK(!symlink_path.empty());
    DCHECK(!target_path.empty());
    return ::symlink(target_path.value().c_str(), symlink_path.value().c_str()) != -1;
}

bool
ReadSymbolicLink(const FilePath& symlink_path, FilePath* target_path)
{
    DCHECK(!symlink_path.empty());
    DCHECK(target_path);
    char buf[PATH_MAX];
    ssize_t count = ::readlink(symlink_path.value().c_str(), buf, base::size(buf));
...
bool
GetPosixFilePermissions(const FilePath& path, int* mode)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    DCHECK(mode);
...
bool
SetPosixFilePermissions(const FilePath& path, int mode)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
...
bool
ExecutableExistsInPath(Environment* env, const FilePath::StringType& executable)
{
    std::string path;
...
#if !IS_APPLE  // Mac implementation is in file_util_mac.mm.
FilePath
GetHomeDir()
{
#if 0
...
File
CreateAndOpenTemporaryFileInDir(const FilePath& dir, FilePath* temp_file)
{
...
ScopedFILE
CreateAndOpenTemporaryStreamInDir(const FilePath& dir, FilePath* path)
{
...
static bool
CreateTemporaryDirInDirImpl(const FilePath& base_dir, const FilePath& name_tmpl, FilePath* new_dir)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE,
                                            BlockingType::MAY_BLOCK);  // For call to mkdtemp().
    DCHECK(EndsWith(name_tmpl.value(), "XXXXXX")) << "Directory name template must end with "
                                                     "\"XXXXXX\".";
...
bool
CreateTemporaryDirInDir(const FilePath& base_dir,
                        const FilePath::StringType& prefix,
                        FilePath* new_dir)
{
    FilePath::StringType mkdtemp_template = prefix;
    mkdtemp_template.append("XXXXXX");
    return CreateTemporaryDirInDirImpl(base_dir, FilePath(mkdtemp_template), new_dir);
}
...
bool
CreateDirectoryAndGetError(const FilePath& full_path, File::Error* error)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE,
...
// file.
bool
ReadFileToStringNonBlocking(const base::FilePath& file, std::string* ret)
{
    DCHECK(ret);
    ret->clear();

    base::ScopedFD fd(
            HANDLE_EINTR(open(file.MaybeAsASCII().c_str(), O_CLOEXEC | O_NONBLOCK | O_RDONLY)));
    if (!fd.is_valid())
    {
        return false;
    }
...
bool
NormalizeFilePath(const FilePath& path, FilePath* normalized_path)
{
...
bool
IsLink(const FilePath& file_path)
{
    stat_wrapper_t st;
...
bool
GetFileInfo(const FilePath& file_path, File::Info* results)
{
    stat_wrapper_t file_info;
#if defined(OS_ANDROID)
...
FILE*
OpenFile(const FilePath& filename, const char* mode)
{
...
#if !defined(OS_NACL)
FILE*
FileToFILE(File file, const char* mode)
{
...
int
ReadFile(const FilePath& filename, char* data, int max_size)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
...
int
WriteFile(const FilePath& filename, const char* data, int size)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
...
bool
WriteFileDescriptor(const int fd, const char* data, int size)
{
...
bool
AllocateFileRegion(File* file, int64_t offset, size_t size)
{
    DCHECK(file);
    ...
    if (File::Fstat(file->GetPlatformFile(), &statbuf) == 0 && statbuf.st_blksize > 0
    ...
    const int64_t extension_start = base::bits::AlignUp(original_file_len, block_size);
    for (int64_t i = extension_start; i < new_file_len; i += block_size)
    {
        char existing_byte;
        if (HANDLE_EINTR(pread(file->GetPlatformFile(), &existing_byte, 1, i)) != 1)
        {
        ...
        if (HANDLE_EINTR(pwrite(file->GetPlatformFile(), &existing_byte, 1, i)) != 1)
        {
...
bool
AppendToFile(const FilePath& filename, const char* data, int size)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    bool ret = true;
...
bool
GetCurrentDirectory(FilePath* dir)
{
...
bool
SetCurrentDirectory(const FilePath& path)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
...
bool
VerifyPathControlledByUser(const FilePath& base,
                           const FilePath& path,
                           uid_t owner_uid,
                           const std::set<gid_t>& group_gids)
{
    ...
    {
        DLOG(ERROR) << "|base| must be a subdirectory of |path|.  base = \"" << base.value()
                    << "\", path = \"" << path.value() << "\"";
        return false;
    }
    ...
    for (ib = base_components.begin(), ip = path_components.begin(); ib != base_components.end();
         ++ib, ++ip)
    {
...
#if defined(OS_MAC)
bool
VerifyPathControlledByAdmin(const FilePath& path)
{
    ...
    for (int i = 0, ie = base::size(kAdminGroupNames); i < ie; ++i)
    {
        ...
        if (!group_record)
        {
            DPLOG(ERROR) << "Could not get the group ID of group \"" << kAdminGroupNames[i]
                         << "\".";
            continue;
        }
    ...
    return VerifyPathControlledByUser(kFileSystemRoot, path, kRootUid, allowed_group_ids);
}
#endif  // defined(OS_MAC)

int
GetMaximumPathComponentLength(const FilePath& path)
{
#if defined(OS_FUCHSIA)
...
bool
GetShmemTempDir(bool executable, FilePath* path)
{
#if IS_LINUX || defined(OS_CHROMEOS) || defined(OS_AIX)
    bool disable_dev_shm = false;
#if 0
    disable_dev_shm = CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableDevShmUsage);
#endif
    bool use_dev_shm = true;
    if (executable)
    {
        static const bool s_dev_shm_executable = IsPathExecutable(FilePath("/dev/shm"));
...
#endif  // IS_LINUX || defined(OS_CHROMEOS) || defined(OS_AIX)
...
bool
CopyFile(const FilePath& from_path, const FilePath& to_path)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    File infile;
#if defined(OS_ANDROID)
...
PrefetchResult
PreReadFile(const FilePath& file_path, bool is_executable, int64_t max_bytes)
{
...
#if IS_LINUX || defined(OS_CHROMEOS) || (defined(OS_ANDROID) && __ANDROID_API__ >= 21)
    ...
    const PlatformFile fd = file.GetPlatformFile();
    ::radvisory read_advise_data = {.ra_offset = 0,
                                    .ra_count = base::saturated_cast<int>(max_bytes)};
...
#if IS_LINUX || defined(OS_CHROMEOS) || defined(OS_ANDROID)
bool
CopyFileContentsWithSendfile(File& infile, File& outfile, bool& retry_slow)
{
    ...
    retry_slow = (copied == 0 && res < 0 && (errno == EINVAL || errno == ENOSYS || errno == EPERM));
...
#if IS_LINUX || defined(OS_CHROMEOS) || defined(OS_AIX)
CORE_EXPORT bool
IsPathExecutable(const FilePath& path)
{
    bool result = false;
