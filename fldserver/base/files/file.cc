...
#include "fldserver/base/timer/elapsed_timer.h"
...
#if !defined(OS_NACL)
void
File::Initialize(const FilePath& path, uint32_t flags)
{
...
bool
File::ReadAndCheck(int64_t offset, span<uint8_t> data)
{
...
bool
File::ReadAtCurrentPosAndCheck(span<uint8_t> data)
{
    int size = checked_cast<int>(data.size());
...
bool
File::WriteAndCheck(int64_t offset, span<const uint8_t> data)
{
    int size = checked_cast<int>(data.size());
...
bool
File::WriteAtCurrentPosAndCheck(span<const uint8_t> data)
{
    int size = checked_cast<int>(data.size());
