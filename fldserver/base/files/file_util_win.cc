...
#include "fldserver/base/strings/string_util_win.h"
...
#include "fldserver/base/threading/sequenced_task_runner_handle.h"
...
void
RecordPostOperationState(const FilePath& path, StringPiece operation, bool operation_succeeded)
{
    ...
    std::string histogram_name = base::StrCat({"Windows.PostOperationState.", operation});
...
void
RecordFilesystemError(StringPiece operation, DWORD error)
{
    std::string histogram_name = base::StrCat({"Windows.FilesystemError.", operation});
...
DWORD
DeleteFileRecursive(const FilePath& path, const FilePath::StringType& pattern, bool recursive)
{
    ...
    for (FilePath current = traversal.Next(); !current.empty(); current = traversal.Next())
    {
        ...
        if (info.IsDirectory())
        {
            if (recursive)
            {
                ...
                if (this_result == ERROR_SUCCESS && !::RemoveDirectory(current.value().c_str()))
                {
...
void
AppendModeCharacter(wchar_t mode_char, std::wstring* mode)
{
...
bool
DoCopyFile(const FilePath& from_path, const FilePath& to_path, bool fail_if_exists)
{
                ...
                bool fail_if_exists)
{
    ...
    while (success && !current.empty())
    {
        ...
        {
            DLOG(ERROR) << "CopyDirectory() couldn't create file: " << target_path.value().c_str();
            success = false;
        }
...
DoDeleteFile(const FilePath& path, bool recursive)
{
    ...
    if ((attr & FILE_ATTRIBUTE_READONLY)
        && !::SetFileAttributes((path.value().c_str()), attr & ~FILE_ATTRIBUTE_READONLY))
    {
    ...
    if (recursive)
    {
        const DWORD error_code = DeleteFileRecursive(path, FILE_PATH_LITERAL("*"), true);
        DCHECK_NE(static_cast<LONG>(error_code), ERROR_FILE_NOT_FOUND);
        DCHECK_NE(static_cast<LONG>(error_code), ERROR_PATH_NOT_FOUND);
...
bool
DeleteFileAndRecordMetrics(const FilePath& path, bool recursive)
{
...
void
LogFileDeleteRetryCount(int attempt)
{
    UmaHistogramExactLinear("Windows.FileDeleteRetryCount", attempt, kMaxDeleteAttempts);
}

void
DeleteFileWithRetry(int attempt, const FilePath& file_path)
{
    ...
    static constexpr TimeDelta kDeleteFileRetryDelay = TimeDelta::FromMilliseconds(250);
...
OnceCallback<void(const FilePath&)>
...
bool
DeleteFileAfterReboot(const FilePath& path)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    ...
    return ::MoveFileEx((path.value().c_str()), nullptr, MOVEFILE_DELAY_UNTIL_REBOOT);
}
...
ReplaceFile(const FilePath& from_path, const FilePath& to_path, File::Error* error)
{
    ...
    base::wcslcpy(from_path_str, from_path.value().c_str(), base::size(from_path_str));
    ...
    if (::ReplaceFile(to_path.value().c_str(),
                      from_path.value().c_str(),
                      NULL,
    ...
    if (error)
    {
        *error = replace_error == File::FILE_ERROR_NOT_FOUND ? File::GetLastFileError()
                                                             : replace_error;
    }
    return false;
}
...
{
bool
PathHasAccess(const FilePath& path, DWORD dir_desired_access, DWORD file_desired_access)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    ...
    win::ScopedHandle file(CreateFile(path_str,
                                      desired_access,
                                      kFileShareAll,
                                      nullptr,
                                      OPEN_EXISTING,
...
GetHomeDir()
{
    ...
    if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_PROFILE, NULL, SHGFP_TYPE_CURRENT, (result)))
        && result[0])
    {
...
CreateAndOpenTemporaryFileInDir(const FilePath& dir, FilePath* temp_file)
{
    ...
    constexpr uint32_t kFlags = File::FLAG_CREATE | File::FLAG_READ | File::FLAG_WRITE
                                | File::FLAG_EXCLUSIVE_READ | File::FLAG_EXCLUSIVE_WRITE
    ...
    {
        *temp_file = FilePath(FilePath::StringPieceType(long_temp_name, long_name_len));
    }
    else
    {
...
CreateAndOpenTemporaryStreamInDir(const FilePath& dir, FilePath* path)
{
    ...
    return ScopedFILE(FileToFILE(CreateAndOpenTemporaryFileInDir(dir, path), "wb+"));
}
                        ...
                        FilePath* new_dir)
{
    ...
    for (int count = 0; count < 50; ++count)
    {
        ...
        new_dir_name.append(
                AsWString(NumberToString16(RandInt(0, std::numeric_limits<int32_t>::max()))));
...
bool
NormalizeFilePath(const FilePath& path, FilePath* real_path)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    ...
    DWORD used_wchars = ::GetFinalPathNameByHandle(file.GetPlatformFile(),
                                                   (native_file_path),
                                                   kMaxPathLength - 1,
    ...
    return DevicePathToDriveLetterPath(
            FilePath(FilePath::StringPieceType(native_file_path, used_wchars)), real_path);
}

bool
DevicePathToDriveLetterPath(const FilePath& nt_device_path, FilePath* out_drive_letter_path)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
        ...
        if (QueryDosDevice((drive), (device_path_as_string), MAX_PATH))
        {
            ...
            if (device_path == nt_device_path || device_path.IsParent(nt_device_path))
            {
                *out_drive_letter_path = FilePath(
                        drive + nt_device_path.value().substr(wcslen(device_path_as_string)));
                return true;
            }
        }
...
FilePath
MakeLongFilePath(const FilePath& input)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    ...
    path_long_len = ::GetLongPathName(
            (input.value().c_str()), (WriteInto(&path_long_str, path_long_len)), path_long_len);
    if (path_long_len == 0UL)
        return FilePath();
...
bool
CreateWinHardLink(const FilePath& to_file, const FilePath& from_file)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
...
GetFileInfo(const FilePath& file_path, File::Info* results)
{
    ...
    if (!GetFileAttributesEx((file_path.value().c_str()), GetFileExInfoStandard, &attr))
    {
        return false;
    }
...
WriteFile(const FilePath& filename, const char* data, int size)
{
    ...
    else
    {
        ...
        DLOG(WARNING) << "wrote" << written << " bytes to " << filename.value() << " expected "
                      << size;
    }
    return -1;
}
...
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    win::ScopedHandle file(CreateFile(
            filename.value().c_str(), FILE_APPEND_DATA, 0, NULL, OPEN_EXISTING, 0, NULL));
    if (!file.IsValid())
    {
...
GetCurrentDirectory(FilePath* dir)
{
    ...
    // trailing slash.  We duplicate this here, but it shouldn't be necessary
    // when everyone is using the appropriate FilePath APIs.
    *dir = FilePath(FilePath::StringPieceType(system_buffer)).StripTrailingSeparators();
    return true;
}

bool
SetCurrentDirectory(const FilePath& directory)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
...
GetMaximumPathComponentLength(const FilePath& path)
{
    ...
    if (!GetVolumePathNameW(
                path.NormalizePathSeparators().value().c_str(), volume_path, size(volume_path)))
    {
        return -1;
    }

    DWORD max_length = 0;
    if (!GetVolumeInformationW(volume_path, NULL, 0, NULL, &max_length, NULL, NULL, 0))
    {
        return -1;
    }
...
PreReadFile(const FilePath& file_path, bool is_executable, int64_t max_bytes)
{
    ...
    static PrefetchVirtualMemoryPtr prefetch_virtual_memory = GetPrefetchVirtualMemoryPtr();
    ...
    MemoryMappedFile::Access access = is_executable ? MemoryMappedFile::READ_CODE_IMAGE
    ...
    const ::SIZE_T length = std::min(base::saturated_cast<::SIZE_T>(max_bytes),
...
MoveUnsafe(const FilePath& from_path, const FilePath& to_path)
{
    ...
    if (MoveFileEx((from_path.value().c_str()),
                   (to_path.value().c_str()),
...
bool
CopyAndDeleteDirectory(const FilePath& from_path, const FilePath& to_path)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
