...
ImportantFileWriterCleaner::GetInstance()
{
...
// static
void
ImportantFileWriterCleaner::AddDirectory(const FilePath& directory)
{
    ...
    else
    {
        ...
        task_runner->PostTask(FROM_HERE,
                              BindOnce(&ImportantFileWriterCleaner::AddDirectoryImpl,
                                       Unretained(&instance),
                                       directory));
    }
}
...
void
ImportantFileWriterCleaner::Initialize()
{
...
void
ImportantFileWriterCleaner::Start()
{
#if DCHECK_IS_ON()
    {
        AutoLock scoped_lock(task_runner_lock_);
...
void
ImportantFileWriterCleaner::Stop()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void
ImportantFileWriterCleaner::UninitializeForTesting()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(!is_started());
    {
...
base::Time
ImportantFileWriterCleaner::GetUpperBoundTimeForTest() const
{
    return upper_bound_time_;
}

ImportantFileWriterCleaner::ImportantFileWriterCleaner() : upper_bound_time_(GetUpperBoundTime())
{
    DETACH_FROM_SEQUENCE(sequence_checker_);
}

void
ImportantFileWriterCleaner::AddDirectoryImpl(const FilePath& directory)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void
ImportantFileWriterCleaner::ScheduleTask()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(is_started());
    DCHECK(!is_running());
    ...
    running_ = ThreadPool::PostTaskAndReplyWithResult(
            FROM_HERE,
            {TaskPriority::BEST_EFFORT, TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN, MayBlock()},
            BindOnce(&ImportantFileWriterCleaner::CleanInBackground,
                     upper_bound_time_,
                     std::move(pending_directories_),
                     std::ref(stop_flag_)),
            // Unretained is safe here since the cleaner instance is never destroyed.
            BindOnce(&ImportantFileWriterCleaner::OnBackgroundTaskFinished, Unretained(this)));
}

// static
bool
ImportantFileWriterCleaner::CleanInBackground(Time upper_bound_time,
                                              std::vector<FilePath> directories,
                                              std::atomic_bool& stop_flag)
{
    DCHECK(!directories.empty());
    bool stop = false;
    for (auto scan = directories.begin(), end = directories.end(); scan != end; ++scan)
    {
        const auto& directory = *scan;
        ClampedNumeric<int> successes;
        ClampedNumeric<int> fails;
        FileEnumerator file_enum(directory,
                                 /*recursive=*/false,
                                 FileEnumerator::FILES,
                                 FormatTemporaryFileName(FILE_PATH_LITERAL("*")).value());
        for (FilePath path = file_enum.Next(); !path.empty(); path = file_enum.Next())
        {
...
void
ImportantFileWriterCleaner::OnBackgroundTaskFinished(bool processing_completed)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void
ImportantFileWriterCleaner::DoStop()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(is_started());
    DCHECK(!is_running());
