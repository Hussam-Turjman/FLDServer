...
{
void
GetStat(const FilePath& path, bool show_links, stat_wrapper_t* st)
{
    DCHECK(st);
...
bool
ShouldShowSymLinks(int file_type)
{
    return false;
}
#else
...
#if defined(OS_FUCHSIA)
bool
ShouldTrackVisitedDirectories(int file_type)
{
    return false;
}
#else
...
bool
FileEnumerator::FileInfo::IsDirectory() const
{
...
FileEnumerator::FileEnumerator(const FilePath& root_path, bool recursive, int file_type) :
    FileEnumerator(
            root_path, recursive, file_type, FilePath::StringType(), FolderSearchPolicy::MATCH_ONLY)
{
}
...
FileEnumerator::FileEnumerator(const FilePath& root_path,
    ...
    FileEnumerator(root_path, recursive, file_type, pattern, FolderSearchPolicy::MATCH_ONLY)
{
}
...
FileEnumerator::Next()
{
    ...
    while (current_directory_entry_ >= directory_entries_.size())
    {
        ...
        while (errno = 0, dent = readdir(dir))
        {
            ...
            if (folder_search_policy_ == FolderSearchPolicy::MATCH_ONLY && !is_pattern_matched)
                continue;
    ...
    return root_path_.Append(directory_entries_[current_directory_entry_].filename_);
}
...
bool
FileEnumerator::IsPatternMatched(const FilePath& path) const
{
    return pattern_.empty() || !fnmatch(pattern_.c_str(), path.value().c_str(), FNM_NOESCAPE);
}
