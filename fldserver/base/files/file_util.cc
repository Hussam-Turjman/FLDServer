...
CopyFileContents(File& infile, File& outfile)
{
    ...
    bool res = internal::CopyFileContentsWithSendfile(infile, outfile, retry_slow);
    if (res || !retry_slow)
    {
        return res;
    }
...
#if IS_WIN
    std::ifstream file1(filename1.value().c_str(), std::ios::in | std::ios::binary);
    std::ifstream file2(filename2.value().c_str(), std::ios::in | std::ios::binary);
...
{
    return ReadStreamToStringWithMaxSize(stream, std::numeric_limits<size_t>::max(), contents);
}
...
ReadStreamToStringWithMaxSize(FILE* stream, size_t max_size, std::string* contents)
{
    ...
    if (::GetFileInformationByHandle(reinterpret_cast<HANDLE>(_get_osfhandle(_fileno(stream))),
                                     &file_info))
    {
        LARGE_INTEGER size;
    ...
    while ((bytes_read_this_pass = fread(&local_contents[bytes_read_so_far], 1, chunk_size, stream))
           > 0)
    {
...
{
    return ReadFileToStringWithMaxSize(path, contents, std::numeric_limits<size_t>::max());
}
...
{
    FileEnumerator files(dir_path, false, FileEnumerator::FILES | FileEnumerator::DIRECTORIES);
    if (files.Next().empty())
        return true;
    return false;
}
...
WriteFile(const FilePath& filename, span<const uint8_t> data)
{
    ...
    return WriteFile(filename, reinterpret_cast<const char*>(data.data()), size) == size;
}
...
PreReadFileSlow(const FilePath& file_path, int64_t max_bytes)
{
    ...
    while (max_bytes > 0)
    {
        ...
        const int read_size = static_cast<int>(std::min<int64_t>(max_bytes, kBufferSize));
