...
#include "fldserver/base/files/file_path_watcher_linux.h"
...
{
public:
    explicit InotifyReaderThreadDelegate(int inotify_fd) : inotify_fd_(inotify_fd)
    {
    }
    InotifyReaderThreadDelegate(const InotifyReaderThreadDelegate&) = delete;
    InotifyReaderThreadDelegate&
    operator=(const InotifyReaderThreadDelegate&) = delete;
...
public:
    ...
    void
    OnFilePathChanged(InotifyReader::Watch fired_watch,
...
private:
    ...
    bool
    UpdateWatches() WARN_UNUSED_RESULT;
    ...
    bool
    UpdateRecursiveWatches(InotifyReader::Watch fired_watch, bool is_dir) WARN_UNUSED_RESULT;

    // Enumerate recursively through |path| and add / update watches.
    // Returns true if watch limit is not hit. Otherwise, returns false.
    bool
    UpdateRecursiveWatchesForPath(const FilePath& path) WARN_UNUSED_RESULT;

    // Do internal bookkeeping to update mappings between |watch| and its
    // associated full path |path|.
    void
    TrackWatchForRecursion(InotifyReader::Watch watch, const FilePath& path);
    ...
    // Returns true if watch limit is not hit. Otherwise, returns false.
    bool
    AddWatchForBrokenSymlink(const FilePath& path, WatchEntry* watch_entry) WARN_UNUSED_RESULT;

    bool
    HasValidWatchVector() const;
...
void
InotifyReaderThreadDelegate::ThreadMain()
{
    ...
    while (true)
    {
        ...
        int select_result = HANDLE_EINTR(select(inotify_fd_ + 1, &rfds, nullptr, nullptr, nullptr));
        if (select_result < 0)
        {
...
InotifyReader::InotifyReader() : inotify_fd_(inotify_init()), thread_delegate_(inotify_fd_)
{
    if (inotify_fd_ < 0)
    {
...
InotifyReader::Watch
InotifyReader::AddWatch(const FilePath& path, FilePathWatcherImpl* watcher)
{
    if (!valid_)
    ...
    Watch watch = inotify_add_watch(inotify_fd_,
                                    path.value().c_str(),
                                    IN_ATTRIB | IN_CREATE | IN_DELETE | IN_CLOSE_WRITE | IN_MOVE
                                            | IN_ONLYDIR);
    ...
    watchers_[watch].emplace(
            std::make_pair(watcher, WatcherEntry{watcher->GetTaskRunner(), watcher->GetWeakPtr()}));

    return watch;
}

void
InotifyReader::RemoveWatch(Watch watch, FilePathWatcherImpl* watcher)
{
...
void
InotifyReader::OnInotifyEvent(const inotify_event* event)
{
    ...
    for (const auto& entry : watcher_map)
    {
        ...
        watcher_entry.task_runner->PostTask(FROM_HERE,
                                            BindOnce(&FilePathWatcherImpl::OnFilePathChanged,
...
void
FilePathWatcherImpl::OnFilePathChanged(InotifyReader::Watch fired_watch,
                                       const FilePath::StringType& child,
                                       bool created,
                                       bool deleted,
                                       bool is_dir)
{
    ...
    for (size_t i = 0; i < watches_.size(); ++i)
    {
        ...
        bool change_on_target_path = child.empty() || (child == watch_entry.linkname)
...
bool
FilePathWatcherImpl::WouldExceedWatchLimit() const
{
    DCHECK(task_runner()->RunsTasksInCurrentSequence());
    ...
    // FilePathWatcherImpl.
    const size_t number_of_inotify_watches = watches_.size() + recursive_paths_by_watch_.size();
    return number_of_inotify_watches >= GetMaxNumberOfInotifyWatches();
}

scoped_refptr<SequencedTaskRunner>
FilePathWatcherImpl::GetTaskRunner() const
{
    DCHECK(task_runner()->RunsTasksInCurrentSequence());
    return task_runner();
}
...
WeakPtr<FilePathWatcherImpl>
FilePathWatcherImpl::GetWeakPtr() const
{
    DCHECK(task_runner()->RunsTasksInCurrentSequence());
...
bool
FilePathWatcherImpl::Watch(const FilePath& path,
                           Type type,
...
bool
FilePathWatcherImpl::UpdateWatches()
{
...
bool
FilePathWatcherImpl::UpdateRecursiveWatches(InotifyReader::Watch fired_watch, bool is_dir)
{
    DCHECK(HasValidWatchVector());
...
bool
FilePathWatcherImpl::UpdateRecursiveWatchesForPath(const FilePath& path)
{
...
void
FilePathWatcherImpl::TrackWatchForRecursion(InotifyReader::Watch watch, const FilePath& path)
{
    DCHECK_EQ(type_, Type::kRecursive);
    DCHECK(!path.empty());
...
void
FilePathWatcherImpl::RemoveRecursiveWatches()
{
    if (type_ != Type::kRecursive)
        return;
...
bool
FilePathWatcherImpl::AddWatchForBrokenSymlink(const FilePath& path, WatchEntry* watch_entry)
{
...
bool
FilePathWatcherImpl::HasValidWatchVector() const
{
    if (watches_.empty())
        return false;
...
ScopedMaxNumberOfInotifyWatchesOverrideForTest::ScopedMaxNumberOfInotifyWatchesOverrideForTest(
        size_t override_max)
{
...
// Put inside "IS_LINUX || defined(OS_CHROMEOS)" because Android
// includes file_path_watcher_linux.cc.

// static
bool
FilePathWatcher::HasWatchesForTest()
{
