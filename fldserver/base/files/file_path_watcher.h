...
class CORE_EXPORT FilePathWatcher
{
public:
    enum class Type
    {
    ...
    using Callback = base::RepeatingCallback<void(const FilePath& path, bool error)>;
    ...
    public:
        ...
        PlatformDelegate&
        operator=(const PlatformDelegate&) = delete;
        virtual ~PlatformDelegate();

        // Start watching for the given |path| and notify |delegate| about changes.
        virtual bool
        Watch(const FilePath& path, Type type, const Callback& callback) WARN_UNUSED_RESULT = 0;
    ...
    protected:
        ...
        void
        set_task_runner(scoped_refptr<SequencedTaskRunner> runner)
        {
    ...
    FilePathWatcher&
    operator=(const FilePathWatcher&) = delete;
    ~FilePathWatcher();
    ...
    bool
    Watch(const FilePath& path, Type type, const Callback& callback);
