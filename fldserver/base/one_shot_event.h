...
class CORE_EXPORT OneShotEvent
{
public:
    OneShotEvent();
    ...
    // one.
    void
    Post(const Location& from_here,
         OnceClosure task,
         scoped_refptr<SingleThreadTaskRunner> runner = ThreadTaskRunnerHandle::Get()) const;
    void
    PostDelayed(const Location& from_here, OnceClosure task, const TimeDelta& delay) const;

private:
    struct TaskInfo;

    void
    PostImpl(const Location& from_here,
             OnceClosure task,
