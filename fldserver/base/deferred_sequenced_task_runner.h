...
class CORE_EXPORT DeferredSequencedTaskRunner : public SequencedTaskRunner
{
public:
    explicit DeferredSequencedTaskRunner(scoped_refptr<SequencedTaskRunner> target_runner);
    ...
    DeferredSequencedTaskRunner&
    operator=(const DeferredSequencedTaskRunner&) = delete;

    // TaskRunner implementation
    bool
    PostDelayedTask(const Location& from_here, OnceClosure task, TimeDelta delay) override;
    bool
    ...
    void
    StartWithTaskRunner(scoped_refptr<SequencedTaskRunner> target_task_runner);
...
private:
    ...
    void
    QueueDeferredTask(const Location& from_here,
                      OnceClosure task,
                      TimeDelta delay,
                      bool is_non_nestable);
