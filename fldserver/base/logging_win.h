...
EXTERN_C CORE_EXPORT const GUID kLogEventId;
...
class CORE_EXPORT LogEventProvider : public base::win::EtwTraceProvider
{
public:
    LogEventProvider(const LogEventProvider&) = delete;
    LogEventProvider&
    operator=(const LogEventProvider&) = delete;
    static LogEventProvider*
    GetInstance();

    static bool
    LogMessage(logging::LogSeverity severity,
               const char* file,
               int line,
    ...
    static void
    Initialize(const GUID& provider_name);
    static void
    Uninitialize();
...
protected:
    ...
    void
    OnEventsEnabled() override;
    void
    OnEventsDisabled() override;
