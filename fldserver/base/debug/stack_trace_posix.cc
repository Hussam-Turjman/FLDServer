...
#include "fldserver/base/memory/free_deleter.h"
...
void
DemangleSymbols(std::string* text)
{
...
public:
    virtual void
    HandleOutput(const char* output) = 0;
...
#if !defined(__UCLIBC__) && !defined(_AIX)
void
OutputPointer(void* pointer, BacktraceOutputHandler* handler)
{
    // This should be more than enough to store a 64-bit number in hex:
    // 16 hex digits + 1 for null-terminator.
    char buf[17] = {'\0'};
    handler->HandleOutput("0x");
    internal::itoa_r(reinterpret_cast<intptr_t>(pointer), buf, sizeof(buf), 16, 12);
    handler->HandleOutput(buf);
}

#if defined(USE_SYMBOLIZE)
void
OutputFrameId(intptr_t frame_id, BacktraceOutputHandler* handler)
{
...
void
ProcessBacktrace(void* const* trace,
                 size_t size,
                 ...
                 BacktraceOutputHandler* handler)
{
    ...
    {
        std::unique_ptr<char*, FreeDeleter> trace_symbols(backtrace_symbols(trace, size));
...
void
PrintToStderr(const char* output)
{
    // NOTE: This code MUST be async-signal safe (it's used by in-process
    // stack dumping signal handler). NO malloc or stdio is allowed here.
...
void
StackDumpSignalHandler(int signal, siginfo_t* info, void* void_context)
{
    ...
    if (try_handle_signal != nullptr && try_handle_signal(signal, info, void_context))
    {
...
// and the risk is low given we're likely crashing already.
    ...
    {
        internal::itoa_r(reinterpret_cast<intptr_t>(info->si_addr), buf, sizeof(buf), 16, 12);
        PrintToStderr(buf);
    }
    PrintToStderr("\n");

#if 1  // BUILDFLAG(CFI_ENFORCEMENT_TRAP)
    if (signal == SIGILL && info->si_code == ILL_ILLOPN)
    {
        PrintToStderr("CFI: Most likely a control flow integrity violation; for more "
                      "information see:\n");
    ...
    for (size_t i = 0; i < base::size(registers); i++)
    {
        ...
        internal::itoa_r(registers[i].value, buf, sizeof(buf), 16, kRegisterPadding);
        PrintToStderr(buf);
    ...
    PrintToStderr("Calling _exit(EXIT_FAILURE). Core file will not be generated.\n");
    _exit(EXIT_FAILURE);
#endif  // !IS_LINUX
...
public:
    ...
    void
    HandleOutput(const char* output) override
    {
        // NOTE: This code MUST be async-signal safe (it's used by in-process
        // stack dumping signal handler). NO malloc or stdio is allowed here.
...
private:
    ...
    SandboxSymbolizeHelper() : is_initialized_(false)
    {
        Init();
    }
    ...
    int
    GetFileDescriptor(const char* file_path)
    {
        int fd = -1;
    ...
    // called from a signal handler (symbolizing stack frames for a crash).
    static int
    OpenObjectFileContainingPc(uint64_t pc,
    ...
    {
        base::ScopedFD mem_fd(HANDLE_EINTR(open("/proc/self/mem", O_RDONLY | O_CLOEXEC)));
        if (!mem_fd.is_valid())
            return;
    ...
    OpenSymbolFiles()
    {
        ...
        for (it = regions_.begin(); it != regions_.end(); ++it)
        {
            ...
            if ((region.permissions & MappedMemoryRegion::READ) == MappedMemoryRegion::READ
                           ...
                           == MappedMemoryRegion::EXECUTE)
            {
                ...
                if (base::EndsWith(region.path, " (deleted)", base::CompareCase::SENSITIVE))
                {
                    // Skip deleted files.
                    continue;
                }
    ...
    Init()
    {
        ...
        {
            OpenSymbolFiles();
            google::InstallSymbolizeOpenObjectFileCallback(&OpenObjectFileContainingPc);
        }
    }
...
SetStackDumpFirstChanceCallback(bool (*handler)(int, siginfo_t*, void*))
{
    ...
    {
        LOG(WARNING) << "WARNING: sanitizers are preventing signal handler installation. "
