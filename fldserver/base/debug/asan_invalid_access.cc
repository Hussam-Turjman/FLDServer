...
#include "fldserver/base/debug/asan_invalid_access.h"
...
namespace
{
#if IS_WIN && defined(ADDRESS_SANITIZER)
...
// true).
NOINLINE void
CorruptMemoryBlock(bool induce_crash)
{
    ...
    auto InterlockedIncrementFn = reinterpret_cast<LONG (*)(LONG volatile * addend)>(
...
AsanHeapOverflow()
{
    ...
    std::unique_ptr<volatile int[]> array(const_cast<volatile int*>(new int[kArraySize]));
