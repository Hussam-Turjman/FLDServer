...
#if (IS_LINUX || defined(OS_CHROMEOS)) && defined(__GLIBC__)
...
uintptr_t
GetNextStackFrame(uintptr_t fp)
{
...
bool
IsStackFrameValid(uintptr_t fp, uintptr_t prev_fp, uintptr_t stack_end)
{
...
uintptr_t
ScanStackForNextFrame(uintptr_t fp, uintptr_t stack_end)
{
    ...
    uintptr_t last_fp_to_scan = std::min(fp + kMaxStackScanArea, stack_end) - sizeof(uintptr_t);
...
void*
LinkStackFrames(void* fpp, void* parent_fp)
{
...
#elif (IS_LINUX || defined(OS_CHROMEOS)) && defined(__GLIBC__)
...
StackTrace::WillSymbolizeToStreamForTesting()
{
    ...
    return false;
#elif defined(OFFICIAL_BUILD) && ((IS_POSIX && !IS_APPLE) || defined(OS_FUCHSIA))
    ...
    return false;
#elif defined(OFFICIAL_BUILD) && IS_APPLE
...
std::string
StackTrace::ToStringWithPrefix(const char* prefix_string) const
{
...
bool
IsWithinRange(uintptr_t address, const AddressRange& range)
{
    return address >= range.start && address <= range.end;
}

size_t
TraceStackFramePointersInternal(base::Optional<uintptr_t> fp,
                                uintptr_t stack_end,
                                size_t max_depth,
                                size_t skip_initial,
                                ...
                                const void** out_trace)
{
    ...
    if (!fp)
    {
        ...
        // be valid.
#if !IS_WIN
        fp = reinterpret_cast<uintptr_t>(__builtin_frame_address(0)) - kStackFrameAdjustment;
#endif
    }
...
TraceStackFramePointers_start:
    AddressRange current_fn_range = {reinterpret_cast<uintptr_t>(&&TraceStackFramePointers_start),
...
size_t
TraceStackFramePointersFromBuffer(uintptr_t fp,
                                  uintptr_t stack_end,
                                  const void** out_trace,
                                  size_t max_depth,
                                  size_t skip_initial,
                                  bool enable_scanning)
{
    return TraceStackFramePointersInternal(
            fp, stack_end, max_depth, skip_initial, enable_scanning, base::nullopt, out_trace);
}

ScopedStackFrameLinker::ScopedStackFrameLinker(void* fp, void* parent_fp) :
    fp_(fp), parent_fp_(parent_fp), original_parent_fp_(LinkStackFrames(fp, parent_fp))
{
}
...
ScopedStackFrameLinker::~ScopedStackFrameLinker()
{
    ...
    CHECK_EQ(parent_fp_, previous_parent_fp) << "Stack frame's parent pointer has changed!";
}
