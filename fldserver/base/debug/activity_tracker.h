...
#include <string>
#include <vector>
//#include "fldserver/base/gtest_prod_util.h"
...
struct OwningProcess
{
    ...
    void
    Release_Initialize(int64_t pid = 0);

    // Explicitly sets the process ID.
    void
    SetOwningProcessIdForTesting(int64_t pid, int64_t stamp);
    ...
    static bool
    GetOwningProcessId(const void* memory, int64_t* out_id, int64_t* out_stamp);
...
union ActivityData
{
    ...
    static ActivityData
    ForGeneric(uint32_t id, int32_t info)
    {
        ActivityData data;
        data.generic.id = id;
    ...
    static ActivityData
    ForThread(const PlatformThreadHandle& handle);
    static ActivityData
    ForThread(const int64_t id)
    {
        ActivityData data;
    ...
    static ActivityData
    ForException(const uint32_t code)
    {
        ActivityData data;
...
class CORE_EXPORT ActivityTrackerMemoryAllocator
{
public:
    ...
    template <typename T>
    T*
    GetAsArray(Reference ref, size_t count)
    {
...
struct Activity
{
    ...
    static constexpr size_t kExpectedInstanceSize = 48 + 8 * kActivityCallStackSize;
...
class CORE_EXPORT ActivityUserData
{
public:
    ...
    // be stored!
    void
    Set(StringPiece name, const void* memory, size_t size)
    {
        Set(name, RAW_VALUE, memory, size);
    }
    void
    SetString(StringPiece name, StringPiece value)
    {
        Set(name, STRING_VALUE, value.data(), value.length());
    }
    void
    SetString(StringPiece name, StringPiece16 value)
    {
        SetString(name, UTF16ToUTF8(value));
    }
    std::atomic<bool>*
    SetBool(StringPiece name, bool value)
    {
    ...
    // of information.
    void
    SetReference(StringPiece name, const void* memory, size_t size)
    {
        SetReference(name, RAW_VALUE_REFERENCE, memory, size);
    }
    void
    SetStringReference(StringPiece name, StringPiece value)
    {
    ...
    // detected.
    bool
    CreateSnapshot(Snapshot* output_snapshot) const;
...
protected:
    virtual void*
    Set(StringPiece name, ValueType type, const void* memory, size_t size);
...
private:
    ...
    enum : size_t
    {
        kMemoryAlignment = sizeof(uint64_t)
    };
    ...
    void
    SetReference(StringPiece name, ValueType type, const void* memory, size_t size);
...
// objects.
class CORE_EXPORT ThreadActivityTracker
{
public:
    ...
    class CORE_EXPORT ScopedActivity
    {
    public:
        ...
        void
        ChangeTypeAndData(Activity::Type type, const ActivityData& data);
    ...
    ActivityId
    PushActivity(const void* program_counter,
                 const void* origin,
                 Activity::Type type,
                 const ActivityData& data);
    ...
    ALWAYS_INLINE
    ActivityId
    PushActivity(const void* origin, Activity::Type type, const ActivityData& data)
    {
    ...
    void
    ChangeActivity(ActivityId id, Activity::Type type, const ActivityData& data);

    // Indicates that an activity has completed.
    void
    PopActivity(ActivityId id);

    // Indicates if an activity is actually being recorded.
    bool
    IsRecorded(ActivityId id);

    // Sets the user-data information for an activity.
    std::unique_ptr<ActivityUserData>
    GetUserData(ActivityId id, ActivityTrackerMemoryAllocator* allocator);
    ...
    bool
    HasUserData(ActivityId id);

    // Release the user-data information for an activity.
    void
    ReleaseUserData(ActivityId id, ActivityTrackerMemoryAllocator* allocator);

    // Save an exception. |origin| is the location of the exception.
    void
    RecordExceptionActivity(const void* program_counter,
                            const void* origin,
                            Activity::Type type,
                            const ActivityData& data);
    ...
    static size_t
    SizeForStackDepth(int stack_depth);
...
private:
    ...
    std::unique_ptr<ActivityUserData>
    CreateUserDataForActivity(Activity* activity, ActivityTrackerMemoryAllocator* allocator);
...
class CORE_EXPORT GlobalActivityTracker
{
public:
    ...
    using ProcessExitCallback = RepeatingCallback<void(int64_t process_id,
                                                       int64_t exit_stamp,
                                                       int exit_code,
    ...
    struct CORE_EXPORT ModuleInfo
    {
        ...
        ModuleInfo&
        operator=(ModuleInfo&& rhs);
        ModuleInfo&
        operator=(const ModuleInfo& rhs);
    ...
    class CORE_EXPORT ScopedThreadActivity : public ThreadActivityTracker::ScopedActivity
    {
    public:
    ...
    private:
        ...
        static ThreadActivityTracker*
        GetOrCreateTracker(bool lock_allowed)
        {
    ...
    // purposes.
    static void
    CreateWithAllocator(std::unique_ptr<PersistentMemoryAllocator> allocator,
                        int stack_depth,
                        int64_t process_id);
    ...
    static bool
    CreateWithSharedMemory(base::WritableSharedMemoryMapping mapping,
                           uint64_t id,
                           StringPiece name,
                           int stack_depth);

    // Gets the global activity-tracker or null if none exists.
    static GlobalActivityTracker*
    Get()
    {
    ...
    static void
    SetForTesting(std::unique_ptr<GlobalActivityTracker> tracker);
    ...
    // stopped.
    static std::unique_ptr<GlobalActivityTracker>
    ReleaseForTesting();
    ...
    void
    ReleaseTrackerForCurrentThreadForTesting();

    // Sets a task-runner that can be used for background work.
    void
    SetBackgroundTaskRunner(const scoped_refptr<SequencedTaskRunner>& runner);

    // Sets an optional callback to be called when a process exits.
    void
    SetProcessExitCallback(ProcessExitCallback callback);
    ...
    void
    RecordProcessLaunch(ProcessId process_id, const FilePath::StringType& cmd);
    void
    RecordProcessLaunch(ProcessId process_id,
                        const FilePath::StringType& exe,
                        const FilePath::StringType& args);
    void
    RecordProcessExit(ProcessId process_id, int exit_code);
    static void
    RecordProcessLaunchIfEnabled(ProcessId process_id, const FilePath::StringType& cmd)
    {
    ...
    static void
    RecordProcessLaunchIfEnabled(ProcessId process_id,
                                 const FilePath::StringType& exe,
                                 const FilePath::StringType& args)
    {
        GlobalActivityTracker* tracker = Get();
    ...
    static void
    RecordProcessExitIfEnabled(ProcessId process_id, int exit_code)
    {
        GlobalActivityTracker* tracker = Get();
        if (tracker)
    ...
    void
    SetProcessPhase(ProcessPhase phase);
    static void
    SetProcessPhaseIfEnabled(ProcessPhase phase)
    {
        GlobalActivityTracker* tracker = Get();
        if (tracker)
    ...
    void
    RecordLogMessage(StringPiece message);
    static void
    RecordLogMessageIfEnabled(StringPiece message)
    {
        GlobalActivityTracker* tracker = Get();
        if (tracker)
    ...
    void
    RecordModuleInfo(const ModuleInfo& info);
    static void
    ...
    ALWAYS_INLINE
    void
    RecordException(const void* origin, uint32_t code)
    {
        return RecordExceptionImpl(GetProgramCounter(), origin, code);
    }
    void
    RecordException(const void* pc, const void* origin, uint32_t code);
...
private:
    ...
    private:
        void*
        Set(StringPiece name, ValueType type, const void* memory, size_t size) override;
    ...
    struct CORE_EXPORT ModuleInfoRecord
    {
        // SHA1(ModuleInfoRecord): Increment this if structure changes!
        static constexpr uint32_t kPersistentTypeId = 0x05DB5F41 + 1;

        // Expected size for 32/64-bit check by PersistentMemoryAllocator.
        static constexpr size_t kExpectedInstanceSize = OwningProcess::kExpectedInstanceSize + 56;
        ...
        bool
        DecodeTo(GlobalActivityTracker::ModuleInfo* info, size_t record_size) const;
        static ModuleInfoRecord*
        CreateFrom(const GlobalActivityTracker::ModuleInfo& info,
    ...
    void
    ReturnTrackerMemory(ManagedActivityTracker* tracker);

    // Records exception information.
    void
    RecordExceptionImpl(const void* pc, const void* origin, uint32_t code);
    ...
    static void
    OnTLSDestroy(void* value);

    // Does process-exit work. This can be run on any thread.
    void
    CleanupAfterProcess(int64_t process_id,
                        int64_t exit_stamp,
                        int exit_code,
                        std::string&& command_line);
    ...
    ActivityTrackerMemoryAllocator
            thread_tracker_allocator_ GUARDED_BY(thread_tracker_allocator_lock_);
    Lock thread_tracker_allocator_lock_;

    // A caching memory allocator for user data attached to activity data.
    ActivityTrackerMemoryAllocator user_data_allocator_ GUARDED_BY(user_data_allocator_lock_);
    ...
    std::map<const std::string, ModuleInfoRecord*> modules_ GUARDED_BY(modules_lock_);
    Lock modules_lock_;
    ...
    std::map<int64_t, std::string> known_processes_ GUARDED_BY(global_tracker_lock_);

    // A task-runner that can be used for doing background processing.
    scoped_refptr<SequencedTaskRunner> background_task_runner_ GUARDED_BY(global_tracker_lock_);
...
class CORE_EXPORT ScopedActivity : public GlobalActivityTracker::ScopedThreadActivity
{
public:
    ...
    void
    ChangeAction(uint8_t action);
    void
    ChangeInfo(int32_t info);
    void
    ChangeActionAndInfo(uint8_t action, int32_t info);

private:
    // Constructs the object using a passed-in program-counter.
    ScopedActivity(const void* program_counter, uint8_t action, uint32_t id, int32_t info);
...
class CORE_EXPORT ScopedTaskRunActivity : public GlobalActivityTracker::ScopedThreadActivity
{
public:
    ALWAYS_INLINE
...
class CORE_EXPORT ScopedLockAcquireActivity : public GlobalActivityTracker::ScopedThreadActivity
{
public:
    ALWAYS_INLINE
...
private:
    ScopedLockAcquireActivity(const void* program_counter, const base::internal::LockImpl* lock);
    DISALLOW_COPY_AND_ASSIGN(ScopedLockAcquireActivity);
};

class CORE_EXPORT ScopedEventWaitActivity : public GlobalActivityTracker::ScopedThreadActivity
{
public:
    ALWAYS_INLINE
...
private:
    ScopedEventWaitActivity(const void* program_counter, const WaitableEvent* event);
    DISALLOW_COPY_AND_ASSIGN(ScopedEventWaitActivity);
};

class CORE_EXPORT ScopedThreadJoinActivity : public GlobalActivityTracker::ScopedThreadActivity
{
public:
    ALWAYS_INLINE
...
private:
    ScopedThreadJoinActivity(const void* program_counter, const PlatformThreadHandle* thread);
    DISALLOW_COPY_AND_ASSIGN(ScopedThreadJoinActivity);
};

// Some systems don't have base::Process
#if !defined(OS_NACL) && !defined(OS_IOS)
class CORE_EXPORT ScopedProcessWaitActivity : public GlobalActivityTracker::ScopedThreadActivity
{
public:
    ALWAYS_INLINE
...
private:
    ScopedProcessWaitActivity(const void* program_counter, const Process* process);
