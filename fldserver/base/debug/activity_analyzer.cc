...
#include "fldserver/base/debug/activity_analyzer.h"
...
void
LogAnalyzerCreationError(AnalyzerCreationError error)
{
    UmaHistogramEnumeration(
            "ActivityTracker.Collect.AnalyzerCreationError", error, kAnalyzerCreationErrorMax);
}
...
ThreadActivityAnalyzer::ThreadActivityAnalyzer(const ThreadActivityTracker& tracker) :
...
ThreadActivityAnalyzer::ThreadActivityAnalyzer(PersistentMemoryAllocator* allocator,
                                               PersistentMemoryAllocator::Reference reference) :
    ThreadActivityAnalyzer(
            allocator->GetAsArray<char>(reference,
...
void
ThreadActivityAnalyzer::AddGlobalInformation(GlobalActivityAnalyzer* global)
{
    if (!IsValid())
        return;
    ...
    for (auto& activity : activity_snapshot_.activity_stack)
    {
        ...
        activity_snapshot_.user_data_stack.push_back(global->GetUserDataSnapshot(
                activity_snapshot_.process_id, activity.user_data_ref, activity.user_data_id));
    }
}

GlobalActivityAnalyzer::GlobalActivityAnalyzer(
        std::unique_ptr<PersistentMemoryAllocator> allocator) :
    allocator_(std::move(allocator)), analysis_stamp_(0LL), allocator_iterator_(allocator_.get())
{
    DCHECK(allocator_);
}
...
GlobalActivityAnalyzer::CreateWithAllocator(std::unique_ptr<PersistentMemoryAllocator> allocator)
{
    if (allocator->GetMemoryState() == PersistentMemoryAllocator::MEMORY_UNINITIALIZED)
    {
        LogAnalyzerCreationError(kPmaUninitialized);
        return nullptr;
    }
    if (allocator->GetMemoryState() == PersistentMemoryAllocator::MEMORY_DELETED)
    {
...
// static
std::unique_ptr<GlobalActivityAnalyzer>
GlobalActivityAnalyzer::CreateWithFile(const FilePath& file_path)
{
...
// static
std::unique_ptr<GlobalActivityAnalyzer>
GlobalActivityAnalyzer::CreateWithSharedMemory(base::ReadOnlySharedMemoryMapping mapping)
{
    if (!mapping.IsValid()
        || !ReadOnlySharedPersistentMemoryAllocator::IsSharedMemoryAcceptable(mapping))
    {
        return nullptr;
    }
    return CreateWithAllocator(std::make_unique<ReadOnlySharedPersistentMemoryAllocator>(
            std::move(mapping), 0, StringPiece()));
}

int64_t
GlobalActivityAnalyzer::GetFirstProcess()
{
    PrepareAllAnalyzers();
...
int64_t
GlobalActivityAnalyzer::GetNextProcess()
{
...
ThreadActivityAnalyzer*
GlobalActivityAnalyzer::GetFirstAnalyzer(int64_t pid)
{
...
ThreadActivityAnalyzer*
GlobalActivityAnalyzer::GetNextAnalyzer()
{
    ...
    do
    {
        ...
        if (analyzers_iterator_ == analyzers_.end())
            return nullptr;
    } while (analyzers_iterator_->second->GetProcessId(&create_stamp) != analyzers_iterator_pid_
             || create_stamp > analysis_stamp_);
    return analyzers_iterator_->second.get();
}

ThreadActivityAnalyzer*
GlobalActivityAnalyzer::GetAnalyzerForThread(const ThreadKey& key)
{
...
ActivityUserData::Snapshot
GlobalActivityAnalyzer::GetUserDataSnapshot(int64_t pid, uint32_t ref, uint32_t id)
{
    ActivityUserData::Snapshot snapshot;

    void* memory = allocator_->GetAsArray<char>(
            ref, GlobalActivityTracker::kTypeIdUserDataRecord, PersistentMemoryAllocator::kSizeAny);
    if (memory)
    {
        ...
        int64_t process_id;
        int64_t create_stamp;
        if (!ActivityUserData::GetOwningProcessId(memory, &process_id, &create_stamp)
...
GlobalActivityAnalyzer::GetLogMessages()
{
    std::vector<std::string> messages;
    PersistentMemoryAllocator::Reference ref;

    PersistentMemoryAllocator::Iterator iter(allocator_.get());
    while ((ref = iter.GetNextOfType(GlobalActivityTracker::kTypeIdGlobalLogMessage)) != 0)
    {
        const char* message =
                allocator_->GetAsArray<char>(ref,
                                             GlobalActivityTracker::kTypeIdGlobalLogMessage,
                                             PersistentMemoryAllocator::kSizeAny);
        if (message)
...
GlobalActivityAnalyzer::GetModules(int64_t pid)
{
    ...
    while ((record = iter.GetNextOfObject<GlobalActivityTracker::ModuleInfoRecord>()) != nullptr)
    {
        int64_t process_id;
        int64_t create_stamp;
        if (!OwningProcess::GetOwningProcessId(&record->owner, &process_id, &create_stamp)
            || pid != process_id || create_stamp > analysis_stamp_)
        {
            continue;
        }
        GlobalActivityTracker::ModuleInfo info;
        if (record->DecodeTo(&info, allocator_->GetAllocSize(allocator_->GetAsReference(record))))
        {
...
bool
GlobalActivityAnalyzer::IsDataComplete() const
{
    DCHECK(allocator_);
...
GlobalActivityAnalyzer::UserDataSnapshot::UserDataSnapshot(const UserDataSnapshot& rhs) = default;
GlobalActivityAnalyzer::UserDataSnapshot::UserDataSnapshot(UserDataSnapshot&& rhs) = default;
GlobalActivityAnalyzer::UserDataSnapshot::~UserDataSnapshot() = default;

void
GlobalActivityAnalyzer::PrepareAllAnalyzers()
{
    ...
    for (PersistentMemoryAllocator::Reference memory_ref : memory_references_)
    {
        ...
        // is checked below.
        void* const base = allocator_->GetAsArray<char>(memory_ref,
                                                        PersistentMemoryAllocator::kTypeIdAny,
                                                        PersistentMemoryAllocator::kSizeAny);
        ...
        switch (allocator_->GetType(memory_ref))
        {
            ...
            case GlobalActivityTracker::kTypeIdProcessDataRecord: {
                ...
                ActivityUserData::GetOwningProcessId(base, &process_id, &create_stamp);
                if (process_id != snapshot.process_id || create_stamp != snapshot.create_stamp)
                {
