...
#include "fldserver/base/debug/proc_maps_linux.h"
...
static bool
ContainsGateVMA(std::string* proc_maps, size_t pos)
{
...
bool
ReadProcMaps(std::string* proc_maps)
{
...
bool
ParseProcMaps(const std::string& input, std::vector<MappedMemoryRegion>* regions_out)
{
    CHECK(regions_out);
    ...
    for (size_t i = 0; i < lines.size(); ++i)
    {
        ...
        if (sscanf(line,
                   "%" SCNxPTR "-%" SCNxPTR " %4c %llx %hhx:%hhx %ld %n",
                   &region.start,
                   &region.end,
                   permissions,
                   &region.offset,
