...
#include "fldserver/base/debug/close_handle_hook_win.h"

#include "fldserver/base/macros.h"
#include "fldserver/base/win/iat_patch_function.h"
                    ...
                    DWORD options)
{
    ...
    return g_duplicate_function(source_process,
                                source_handle,
                                target_process,
                                target_handle,
                                desired_access,
...
{
public:
    AutoProtectMemory() : changed_(false), address_(nullptr), bytes_(0), old_protect_(0)
    {
    }
...
bool
AutoProtectMemory::ChangeProtection(void* address, size_t bytes)
{
    DCHECK(!changed_);
    DCHECK(address);
...
void
EATPatch(HMODULE module, const char* function_name, void* new_function, void** old_function)
{
    if (!module)
        return;
...
IATPatch(HMODULE module, const char* function_name, void* new_function, void** old_function)
{
    if (!module)
        return NULL;
    ...
    if (patch->PatchFromModule(module, "kernel32.dll", function_name, new_function))
    {
        delete patch;
        return NULL;
    }
...
void
HandleHooks::AddIATPatch(HMODULE module)
{
    if (!module)
        return;

    base::win::IATPatchFunction* patch = NULL;
    patch = IATPatch(module,
                     "CloseHandle",
...
void
PatchLoadedModules(HandleHooks* hooks)
{
    const DWORD kSize = 256;
    DWORD returned;
    std::unique_ptr<HMODULE[]> modules(new HMODULE[kSize]);
    if (!EnumProcessModules(GetCurrentProcess(), modules.get(), kSize * sizeof(HMODULE), &returned))
    {
        return;
    }
