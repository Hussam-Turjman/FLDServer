...
// not initialized.
CORE_EXPORT CrashKeyString*
AllocateCrashKeyString(const char name[], CrashKeySize size);
...
public:
    ...
    ScopedCrashKeyString&
    operator=(const ScopedCrashKeyString&) = delete;
    ...
    ScopedCrashKeyString&
    operator=(ScopedCrashKeyString&&) = delete;
...
#define SCOPED_CRASH_KEY_STRING_INTERNAL2(category, name, nonce, data, key_size)                   \
    ...
    ::base::debug::ScopedCrashKeyString scoped_crash_key_helper##nonce(                            \
            [] {                                                                                   \
                static auto* const key =                                                           \
                        ::base::debug::AllocateCrashKeyString(category "-" name, key_size);        \
                return key;                                                                        \
            }(),                                                                                   \
            (data))

// This indirection is needed to expand __COUNTER__.
#define SCOPED_CRASH_KEY_STRING_INTERNAL(category, name, nonce, data, key_size)                    \
...
#define SCOPED_CRASH_KEY_STRING32(category, name, data)                                            \
    SCOPED_CRASH_KEY_STRING_INTERNAL(                                                              \
            category, name, __COUNTER__, (data), ::base::debug::CrashKeySize::Size32)

#define SCOPED_CRASH_KEY_STRING64(category, name, data)                                            \
    SCOPED_CRASH_KEY_STRING_INTERNAL(                                                              \
            category, name, __COUNTER__, (data), ::base::debug::CrashKeySize::Size64)

#define SCOPED_CRASH_KEY_STRING256(category, name, data)                                           \
    SCOPED_CRASH_KEY_STRING_INTERNAL(                                                              \
            category, name, __COUNTER__, (data), ::base::debug::CrashKeySize::Size256)
...
public:
    ...
    virtual CrashKeyString*
    Allocate(const char name[], CrashKeySize size) = 0;
    virtual void
    Set(CrashKeyString* crash_key, base::StringPiece value) = 0;
    virtual void
    Clear(CrashKeyString* crash_key) = 0;
};
...
CORE_EXPORT void
SetCrashKeyImplementation(std::unique_ptr<CrashKeyImplementation> impl);

// The base structure for a crash key, storing the allocation metadata.
struct CrashKeyString
{
    constexpr CrashKeyString(const char name[], CrashKeySize size) : name(name), size(size)
    {
    }
