...
#include "fldserver/base/debug/invalid_access_win.h"
...
__TerminateWithHeapCorruption()
{
    ...
    CHECK(heap);
    CHECK(HeapSetInformation(heap, HeapEnableTerminationOnCorruption, nullptr, 0));
...
TerminateWithControlFlowViolation()
{
    ...
    FuncType func = reinterpret_cast<FuncType>((reinterpret_cast<uintptr_t>(nop_sled))
                                               + kNopInstructionSize);
    //__try
    // {
