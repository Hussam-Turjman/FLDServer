...
CORE_EXPORT bool
SetStackDumpFirstChanceCallback(bool (*handler)(int, siginfo_t*, void*));
#endif

// Returns end of the stack, or 0 if we couldn't get it.
#if BUILDFLAG(CAN_UNWIND_WITH_FRAME_POINTERS)
CORE_EXPORT uintptr_t
GetStackEnd();
#endif
...
public:
    ...
    static bool
    WillSymbolizeToStreamForTesting();
    ...
    std::string
    ToStringWithPrefix(const char* prefix_string) const;

private:
#if IS_WIN
    void
    InitTrace(const _CONTEXT* context_record);
#endif
...
class CORE_EXPORT ScopedStackFrameLinker
{
public:
...
namespace internal
{
#if IS_POSIX && !defined(OS_ANDROID)
...
// appended.
CORE_EXPORT char*
itoa_r(intptr_t i, char* buf, size_t sz, int base, size_t padding);
#endif  // IS_POSIX && !defined(OS_ANDROID)
