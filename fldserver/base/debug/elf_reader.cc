...
#include "fldserver/base/debug/elf_reader.h"
...
const Ehdr*
GetElfHeader(const void* elf_mapped_base)
{
    if (strncmp(reinterpret_cast<const char*>(elf_mapped_base), ELFMAG, SELFMAG) != 0)
        return nullptr;
...
ReadElfBuildId(const void* elf_mapped_base, bool uppercase, ElfBuildIdBuffer build_id)
{
    ...
    for (const Phdr& header : GetElfProgramHeaders(elf_mapped_base))
    {
        ...
        while (current_section < section_end)
        {
            ...
            {
                StringPiece note_name(current_section + sizeof(Nhdr), current_note->n_namesz);
            ...
            size_t section_size = bits::AlignUp(current_note->n_namesz, 4)
                                  + bits::AlignUp(current_note->n_descsz, 4) + sizeof(Nhdr);
        ...
        const uint8_t* build_id_raw = reinterpret_cast<const uint8_t*>(current_note) + sizeof(Nhdr)
                                      + bits::AlignUp(current_note->n_namesz, 4);
        size_t i = 0;
        for (i = 0; i < current_note->n_descsz; ++i)
        {
            strings::SafeSNPrintf(
                    &build_id[i * 2], 3, (uppercase ? "%02X" : "%02x"), build_id_raw[i]);
        }
        build_id[i * 2] = '\0';
...
ReadElfLibraryName(const void* elf_mapped_base)
{
    ...
    for (const Phdr& header : GetElfProgramHeaders(elf_mapped_base))
    {
        ...
        // name string.
        const Dyn* dynamic_start = reinterpret_cast<const Dyn*>(header.p_vaddr + relocation_offset);
        const Dyn* dynamic_end =
                reinterpret_cast<const Dyn*>(header.p_vaddr + relocation_offset + header.p_memsz);
        Word soname_strtab_offset = 0;
        const char* strtab_addr = 0;
        for (const Dyn* dynamic_iter = dynamic_start; dynamic_iter < dynamic_end; ++dynamic_iter)
        {
...
GetElfProgramHeaders(const void* elf_mapped_base)
{
    ...
    const char* phdr_start = reinterpret_cast<const char*>(elf_header) + elf_header->e_phoff;
    return span<const Phdr>(reinterpret_cast<const Phdr*>(phdr_start), elf_header->e_phnum);
}
