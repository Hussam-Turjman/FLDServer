...
#include "fldserver/base/debug/gdi_debug_util_win.h"
...
struct RegularProcessTypes
{
    ...
    using ReadVirtualMemoryFunc = NTSTATUS
    NTAPI(IN HANDLE ProcessHandle,
...
// static
const char* RegularProcessTypes::query_information_process_name = "NtQueryInformationProcess";

// static
const char* RegularProcessTypes::read_virtual_memory_func_name = "NtReadVirtualMemory";
...
// static
const char* WowProcessTypes::query_information_process_name = "NtWow64QueryInformationProcess64";

// static
const char* WowProcessTypes::read_virtual_memory_func_name = "NtWow64ReadVirtualMemory64";
...
{
    using GdiTableEntryVector = std::vector<GdiTableEntry<typename ProcessType::NativePointerType>>;
    ...
    NTSTATUS result = query_information_process_func(
            process.Handle(), ProcessBasicInformation, &basic_info, sizeof(basic_info), nullptr);
    if (result != 0)
    {
        LOG(ERROR) << ProcessType::query_information_process_name << " Failed " << std::hex
                   << result;
        return GdiTableEntryVector();
    }
    ...
    result = read_virtual_mem_func(
            process.Handle(), basic_info.PebBaseAddress, &peb, sizeof(peb), nullptr);
    if (result != 0)
    {
        LOG(ERROR) << ProcessType::read_virtual_memory_func_name << " PEB Failed " << std::hex
                   << result;
        return GdiTableEntryVector();
    }
    ...
    result =
            read_virtual_mem_func(process.Handle(),
    ...
    if (result != 0)
    {
        LOG(ERROR) << ProcessType::read_virtual_memory_func_name << " GDI Handle Table Failed "
                   << std::hex << result;
        return GdiTableEntryVector();
    }
...
template <typename PointerType>
base::debug::GdiHandleCounts
CountHandleTypesFromTable(DWORD pid, const std::vector<GdiTableEntry<PointerType>>& gdi_table)
{
    ...
    for (const auto& entry : gdi_table)
    {
        ...
        {
            case GdiHandleType::kDC: ++counts.dcs; break;
            case GdiHandleType::kRegion: ++counts.regions; break;
            case GdiHandleType::kBitmap: ++counts.bitmaps; break;
            case GdiHandleType::kPalette: ++counts.palettes; break;
            case GdiHandleType::kFont: ++counts.fonts; break;
            case GdiHandleType::kBrush: ++counts.brushes; break;
            case GdiHandleType::kPen: ++counts.pens; break;
            default: ++counts.unknown; break;
        }
    }
    counts.total_tracked = counts.dcs + counts.regions + counts.bitmaps + counts.palettes
                           + counts.fonts + counts.brushes + counts.pens + counts.unknown;
    return counts;
}

template <typename ProcessType>
base::Optional<base::debug::GdiHandleCounts>
CollectGdiHandleCountsImpl(DWORD pid)
{
    base::Process process = base::Process::OpenWithExtraPrivileges(pid);
    if (!process.IsValid())
        return base::nullopt;

    std::vector<GdiTableEntry<typename ProcessType::NativePointerType>> gdi_entries =
            GetGdiTableEntries<ProcessType>(process);
...
// failure.
base::Optional<base::debug::GdiHandleCounts>
CollectGdiHandleCounts(DWORD pid)
{
...
void NOINLINE
GetFirstProcess(HANDLE snapshot, PROCESSENTRY32* proc_entry)
{
...
void NOINLINE
CrashIfExcessiveHandles(DWORD num_gdi_handles)
{
...
void NOINLINE
CrashIfCannotAllocateSmallBitmap(BITMAPINFOHEADER* header, HANDLE shared_section)
{
    ...
    HBITMAP small_bitmap = CreateDIBSection(
            nullptr, reinterpret_cast<BITMAPINFO*>(&header), 0, &small_data, shared_section, 0);
    CHECK(small_bitmap != nullptr);
    DeleteObject(small_bitmap);
}

void NOINLINE
GetProcessMemoryInfo(PROCESS_MEMORY_COUNTERS_EX* pmc)
{
    pmc->cb = sizeof(*pmc);
...
void
CollectChildGDIUsageAndDie(DWORD parent_pid)
{
    ...
    do
    {
        base::win::ScopedHandle process(
                OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, proc_entry.th32ProcessID));
        if (!process.IsValid())
            continue;
...
{
void
CollectGDIUsageAndDie(BITMAPINFOHEADER* header, HANDLE shared_section)
{
    ...
    DWORD peak_gdi_handles = GetGuiResources(GetCurrentProcess(), GR_GDIOBJECTS_PEAK);
    ...
    GdiHandleCounts handle_counts = optional_handle_counts.value_or(GdiHandleCounts());
...
GdiHandleCounts
GetGDIHandleCountsInCurrentProcessForTesting()
{
    base::Optional<GdiHandleCounts> handle_counts = CollectGdiHandleCounts(GetCurrentProcessId());
