...
class CORE_EXPORT ThreadActivityAnalyzer
{
public:
    struct CORE_EXPORT Snapshot : ThreadActivityTracker::Snapshot
    {
        Snapshot();
        ~Snapshot();
    ...
    public:
        ...
        bool
        operator<(const ThreadKey& rhs) const
        {
            if (pid_ != rhs.pid_)
    ...
    void
    AddGlobalInformation(GlobalActivityAnalyzer* global);
    ...
    IsValid()
    {
        return activity_snapshot_valid_;
    }

    // Gets the process id and its creation stamp.
    int64_t
    GetProcessId(int64_t* out_stamp = nullptr)
    {
        if (out_stamp)
    ...
    GetThreadKey()
    {
        return ThreadKey(activity_snapshot_.process_id, activity_snapshot_.thread_id);
    }

    const Snapshot&
    activity_snapshot()
    {
        return activity_snapshot_;
    }
...
class CORE_EXPORT GlobalActivityAnalyzer
{
public:
    ...
    explicit GlobalActivityAnalyzer(std::unique_ptr<PersistentMemoryAllocator> allocator);

    ~GlobalActivityAnalyzer();

    // Creates a global analyzer using a given persistent-memory |allocator|.
    static std::unique_ptr<GlobalActivityAnalyzer>
    ...
    // |file_path|.
    static std::unique_ptr<GlobalActivityAnalyzer>
    CreateWithFile(const FilePath& file_path);
#endif  // !defined(OS_NACL)

    // Like above but accesses an allocator in a mapped shared-memory segment.
    static std::unique_ptr<GlobalActivityAnalyzer>
    CreateWithSharedMemory(base::ReadOnlySharedMemoryMapping mapping);
    ...
    ThreadActivityAnalyzer*
    GetAnalyzerForThread(const ThreadKey& key);

    // Extract user data based on a reference and its identifier.
    ActivityUserData::Snapshot
    GetUserDataSnapshot(int64_t pid, uint32_t ref, uint32_t id);
    ...
    GetProcessDataSnapshot(int64_t pid);
