...
#include "fldserver/base/debug/activity_tracker.h"

#include "fldserver/base/atomic_sequence_num.h"
#include "fldserver/base/bits.h"
#include "fldserver/base/containers/contains.h"
#include "fldserver/base/debug/stack_trace.h"
#include "fldserver/base/files/file.h"
#include "fldserver/base/files/file_path.h"
#include "fldserver/base/files/memory_mapped_file.h"
#include "fldserver/base/logging.h"
#include "fldserver/base/memory/ptr_util.h"
#include "fldserver/base/metrics/field_trial.h"
...
#include "fldserver/base/process/process_handle.h"
...
const size_t kMaxUserDataNameLength = static_cast<size_t>(std::numeric_limits<uint8_t>::max());
...
AllocateFrom(PersistentMemoryAllocator* allocator,
             uint32_t from_type,
             size_t size,
             uint32_t to_type)
{
...
Time
WallTimeFromTickTime(int64_t ticks_start, int64_t ticks, Time time_start)
{
...
void
OwningProcess::Release_Initialize(int64_t pid)
{
...
void
OwningProcess::SetOwningProcessIdForTesting(int64_t pid, int64_t stamp)
{
...
// static
bool
OwningProcess::GetOwningProcessId(const void* memory, int64_t* out_id, int64_t* out_stamp)
{
...
ActivityData
ActivityData::ForThread(const PlatformThreadHandle& handle)
{
...
ActivityTrackerMemoryAllocator::ActivityTrackerMemoryAllocator(PersistentMemoryAllocator* allocator,
...
void
ActivityTrackerMemoryAllocator::ReleaseObjectReference(Reference ref)
{
    // Mark object as free.
    bool success = allocator_->ChangeType(ref,
                                          object_free_type_,
                                          object_type_,
                                          /*clear=*/true);
    DCHECK(success);
...
// static
void
Activity::FillFrom(Activity* activity,
...
StringPiece
ActivityUserData::TypedValue::Get() const
{
...
StringPiece
ActivityUserData::TypedValue::GetString() const
{
...
bool
ActivityUserData::TypedValue::GetBool() const
{
...
char
ActivityUserData::TypedValue::GetChar() const
{
...
int64_t
ActivityUserData::TypedValue::GetInt() const
{
...
uint64_t
ActivityUserData::TypedValue::GetUint() const
{
...
StringPiece
ActivityUserData::TypedValue::GetReference() const
{
...
StringPiece
ActivityUserData::TypedValue::GetStringReference() const
{
    ...
    orig_create_stamp(0)
{
    ...
    *const_cast<uint32_t*>(&orig_data_id) = header_->owner.data_id.load(std::memory_order_acquire);
...
bool
ActivityUserData::CreateSnapshot(Snapshot* output_snapshot) const
{
    ...
    for (const auto& entry : values_)
    {
        ...
        switch (entry.second.type)
        {
            case RAW_VALUE:
            case STRING_VALUE:
                value.long_value_ = std::string(reinterpret_cast<char*>(entry.second.memory), size);
                break;
            case RAW_VALUE_REFERENCE:
            case STRING_VALUE_REFERENCE: {
                ReferenceRecord* ref = reinterpret_cast<ReferenceRecord*>(entry.second.memory);
                value.ref_value_ =
                        StringPiece(reinterpret_cast<char*>(static_cast<uintptr_t>(ref->address)),
                                    static_cast<size_t>(ref->size));
            }
            break;
            case BOOL_VALUE:
            case CHAR_VALUE:
                value.short_value_ = reinterpret_cast<std::atomic<char>*>(entry.second.memory)
                                             ->load(std::memory_order_relaxed);
                break;
            case SIGNED_VALUE:
            case UNSIGNED_VALUE:
                value.short_value_ = reinterpret_cast<std::atomic<uint64_t>*>(entry.second.memory)
                                             ->load(std::memory_order_relaxed);
                break;
            case END_OF_VALUES:  // Included for completeness purposes.
                NOTREACHED();
        }
        auto inserted = output_snapshot->emplace(std::string(entry.second.name), std::move(value));
...
const void*
ActivityUserData::GetBaseAddress() const
{
...
void
ActivityUserData::SetOwningProcessIdForTesting(int64_t pid, int64_t stamp)
{
    if (!header_)
        return;
...
// static
bool
ActivityUserData::GetOwningProcessId(const void* memory, int64_t* out_id, int64_t* out_stamp)
{
...
void*
ActivityUserData::Set(StringPiece name, ValueType type, const void* memory, size_t size)
{
    DCHECK_GE(std::numeric_limits<uint8_t>::max(), name.length());
    size = std::min(std::numeric_limits<uint16_t>::max() - (kMemoryAlignment - 1), size);
    ...
    else
    {
        ...
        size_t name_extent = bits::AlignUp(sizeof(FieldHeader) + name_size, kMemoryAlignment)
                             - sizeof(FieldHeader);
        ...
        void* value_memory = reinterpret_cast<char*>(header) + sizeof(FieldHeader) + name_extent;
        ...
        auto inserted = values_.insert(std::make_pair(persistent_name, ValueInfo()));
        DCHECK(inserted.second);  // True if inserted, false if existed.
...
void
ActivityUserData::SetReference(StringPiece name, ValueType type, const void* memory, size_t size)
{
    ReferenceRecord rec;
...
void
ActivityUserData::ImportExistingData() const
{
    // It's possible that no user data is being stored.
    if (!memory_)
        return;
    ...
    {
        FieldHeader* header = reinterpret_cast<FieldHeader*>(memory_);
        ValueType type = static_cast<ValueType>(header->type.load(std::memory_order_acquire));
        ...
        size_t value_offset =
                bits::AlignUp(sizeof(FieldHeader) + header->name_size, kMemoryAlignment);
...
struct ThreadActivityTracker::Header
{
    ...
    static constexpr uint32_t kPersistentTypeId = GlobalActivityTracker::kTypeIdActivityTracker;

    // Expected size for 32/64-bit check.
    static constexpr size_t kExpectedInstanceSize =
            OwningProcess::kExpectedInstanceSize + Activity::kExpectedInstanceSize + 72;
...
ThreadActivityTracker::ScopedActivity::ScopedActivity(ThreadActivityTracker* tracker,
                                                      const void* program_counter,
                                                      const void* origin,
                                                      Activity::Type type,
                                                      const ActivityData& data) :
...
bool
ThreadActivityTracker::ScopedActivity::IsRecorded()
{
    return tracker_ && tracker_->IsRecorded(activity_id_);
}

void
ThreadActivityTracker::ScopedActivity::ChangeTypeAndData(Activity::Type type,
                                                         const ActivityData& data)
{
    if (tracker_)
...
ThreadActivityTracker::ThreadActivityTracker(void* base, size_t size) :
    ...
    stack_(reinterpret_cast<Activity*>(reinterpret_cast<char*>(base) + sizeof(Header))),
#if DCHECK_IS_ON()
    thread_id_(PlatformThreadRef()),
#endif
    stack_slots_(static_cast<uint32_t>((size - sizeof(Header)) / sizeof(Activity)))
{
    ...
    if (!base ||
        ...
        (size - sizeof(Header)) / sizeof(Activity) > std::numeric_limits<uint32_t>::max())
    {
        NOTREACHED();
        return;
    }
    ...
    static_assert(sizeof(header_->thread_ref) == sizeof(header_->thread_ref.as_id),
    ...
    if (header_->owner.data_id.load(std::memory_order_relaxed) == 0)
    {
        ...
        strlcpy(header_->thread_name, PlatformThread::GetName(), sizeof(header_->thread_name));
...
ThreadActivityTracker::ActivityId
ThreadActivityTracker::PushActivity(const void* program_counter,
                                    const void* origin,
                                    Activity::Type type,
                                    const ActivityData& data)
{
...
void
ThreadActivityTracker::ChangeActivity(ActivityId id, Activity::Type type, const ActivityData& data)
{
...
void
ThreadActivityTracker::PopActivity(ActivityId id)
{
    ...
    uint32_t depth = header_->current_depth.fetch_sub(1, std::memory_order_relaxed) - 1;
    ...
    // A thread-checker creates a lock to check the thread-id which means
    // re-entry into this code if lock acquisitions are being tracked.
    DCHECK(stack_[depth].activity_type == Activity::ACT_LOCK_ACQUIRE || CalledOnValidThread());
...
bool
ThreadActivityTracker::IsRecorded(ActivityId id)
{
    return id < stack_slots_;
}

std::unique_ptr<ActivityUserData>
ThreadActivityTracker::GetUserData(ActivityId id, ActivityTrackerMemoryAllocator* allocator)
{
...
bool
ThreadActivityTracker::HasUserData(ActivityId id)
{
    // User-data is only stored for activities actually held in the stack.
...
void
ThreadActivityTracker::ReleaseUserData(ActivityId id, ActivityTrackerMemoryAllocator* allocator)
{
    // User-data is only stored for activities actually held in the stack.
...
void
ThreadActivityTracker::RecordExceptionActivity(const void* program_counter,
                                               const void* origin,
                                               Activity::Type type,
                                               const ActivityData& data)
{
    ...
    Activity::FillFrom(&header_->last_exception, program_counter, origin, type, data);
...
bool
ThreadActivityTracker::IsValid() const
{
    if (header_->owner.data_id.load(std::memory_order_acquire) == 0
...
bool
ThreadActivityTracker::CreateSnapshot(Snapshot* output_snapshot) const
{
    DCHECK(output_snapshot);
    ...
    for (int attempt = 0; attempt < kMaxAttempts; ++attempt)
    {
        ...
        const uint32_t starting_id = header_->owner.data_id.load(std::memory_order_acquire);
        ...
        const uint32_t pre_version = header_->data_version.load(std::memory_order_seq_cst);
        ...
        if (count > 0)
        {
            ...
            memcpy(&output_snapshot->activity_stack[0], stack_, count * sizeof(Activity));
        }

        // Capture the last exception.
        memcpy(&output_snapshot->last_exception, &header_->last_exception, sizeof(Activity));
        ...
        // name is shorter.
        output_snapshot->thread_name.resize(strlen(output_snapshot->thread_name.c_str()));
        ...
        output_snapshot->last_exception.time_internal =
                WallTimeFromTickTime(
                        start_ticks, output_snapshot->last_exception.time_internal, start_time)
                        .ToInternalValue();
...
const void*
ThreadActivityTracker::GetBaseAddress()
{
    return header_;
}

uint32_t
ThreadActivityTracker::GetDataVersionForTesting()
{
    return header_->data_version.load(std::memory_order_relaxed);
}

void
ThreadActivityTracker::SetOwningProcessIdForTesting(int64_t pid, int64_t stamp)
{
    header_->owner.SetOwningProcessIdForTesting(pid, stamp);
}
...
// static
bool
ThreadActivityTracker::GetOwningProcessId(const void* memory, int64_t* out_id, int64_t* out_stamp)
{
...
// static
size_t
ThreadActivityTracker::SizeForStackDepth(int stack_depth)
{
    return static_cast<size_t>(stack_depth) * sizeof(Activity) + sizeof(Header);
}

bool
ThreadActivityTracker::CalledOnValidThread()
{
#if DCHECK_IS_ON()
...
std::unique_ptr<ActivityUserData>
ThreadActivityTracker::CreateUserDataForActivity(Activity* activity,
...
GlobalActivityTracker::ModuleInfo&
GlobalActivityTracker::ModuleInfo::operator=(ModuleInfo&& rhs) = default;
GlobalActivityTracker::ModuleInfo&
GlobalActivityTracker::ModuleInfo::operator=(const ModuleInfo& rhs) = default;

GlobalActivityTracker::ModuleInfoRecord::ModuleInfoRecord() = default;
GlobalActivityTracker::ModuleInfoRecord::~ModuleInfoRecord() = default;

bool
GlobalActivityTracker::ModuleInfoRecord::DecodeTo(GlobalActivityTracker::ModuleInfo* info,
...
GlobalActivityTracker::ModuleInfoRecord::CreateFrom(const GlobalActivityTracker::ModuleInfo& info,
...
bool
GlobalActivityTracker::ModuleInfoRecord::UpdateFrom(const GlobalActivityTracker::ModuleInfo& info)
{
    ...
    if ((old_changes & kModuleInformationChanging) != 0
        || !changes.compare_exchange_strong(
                old_changes, new_changes, std::memory_order_acquire, std::memory_order_acquire))
    {
    ...
    bool success = changes.compare_exchange_strong(
            new_changes, old_changes + 1, std::memory_order_release, std::memory_order_relaxed);
    DCHECK(success);
    return true;
}

GlobalActivityTracker::ScopedThreadActivity::ScopedThreadActivity(const void* program_counter,
                                                                  const void* origin,
                                                                  Activity::Type type,
                                                                  const ActivityData& data,
                                                                  bool lock_allowed) :
    ThreadActivityTracker::ScopedActivity(
            GetOrCreateTracker(lock_allowed), program_counter, origin, type, data)
{
}
...
ActivityUserData&
GlobalActivityTracker::ScopedThreadActivity::user_data()
{
    if (!user_data_)
    {
        if (tracker_)
        {
            GlobalActivityTracker* global = GlobalActivityTracker::Get();
            AutoLock lock(global->user_data_allocator_lock_);
            user_data_ = tracker_->GetUserData(activity_id_, &global->user_data_allocator_);
        }
        else
        {
...
void*
GlobalActivityTracker::ThreadSafeUserData::Set(StringPiece name,
                                               ValueType type,
                                               const void* memory,
                                               size_t size)
{
...
GlobalActivityTracker::ManagedActivityTracker::ManagedActivityTracker(
        PersistentMemoryAllocator::Reference mem_reference, void* base, size_t size) :
    ThreadActivityTracker(base, size), mem_reference_(mem_reference), mem_base_(base)
{
}
...
void
GlobalActivityTracker::CreateWithAllocator(std::unique_ptr<PersistentMemoryAllocator> allocator,
                                           int stack_depth,
                                           int64_t process_id)
{
...
GlobalActivityTracker::CreateWithFile(
        const FilePath& file_path, size_t size, uint64_t id, StringPiece name, int stack_depth)
{
...
// static
bool
GlobalActivityTracker::CreateWithLocalMemory(
        size_t size, uint64_t id, StringPiece name, int stack_depth, int64_t process_id)
{
    CreateWithAllocator(std::make_unique<LocalPersistentMemoryAllocator>(size, id, name),
                        stack_depth,
                        process_id);
    return true;
}
...
// static
bool
GlobalActivityTracker::CreateWithSharedMemory(base::WritableSharedMemoryMapping mapping,
                                              uint64_t id,
                                              StringPiece name,
                                              int stack_depth)
{
    if (!mapping.IsValid()
        || !WritableSharedPersistentMemoryAllocator::IsSharedMemoryAcceptable(mapping))
    {
        return false;
    }
    CreateWithAllocator(
            std::make_unique<WritableSharedPersistentMemoryAllocator>(std::move(mapping), id, name),
            stack_depth,
            0);
    return true;
}
...
// static
void
GlobalActivityTracker::SetForTesting(std::unique_ptr<GlobalActivityTracker> tracker)
{
...
GlobalActivityTracker::CreateTrackerForCurrentThread()
{
    ...
    void* mem_base;
    mem_base = allocator_->GetAsObject<ThreadActivityTracker::Header>(mem_reference);
    ...
    auto tracker =
            std::make_unique<ManagedActivityTracker>(mem_reference, mem_base, stack_memory_size_);
...
void
GlobalActivityTracker::ReleaseTrackerForCurrentThreadForTesting()
{
...
void
GlobalActivityTracker::SetBackgroundTaskRunner(const scoped_refptr<SequencedTaskRunner>& runner)
{
...
void
GlobalActivityTracker::SetProcessExitCallback(ProcessExitCallback callback)
{
    AutoLock lock(global_tracker_lock_);
...
void
GlobalActivityTracker::RecordProcessLaunch(ProcessId process_id, const FilePath::StringType& cmd)
{
    ...
    if (base::Contains(known_processes_, pid))
    {
        ...
        NOTREACHED() << "Process #" << process_id << " was previously recorded as \"launched\""
...
void
GlobalActivityTracker::RecordProcessLaunch(ProcessId process_id,
...
void
GlobalActivityTracker::RecordProcessExit(ProcessId process_id, int exit_code)
{
    const int64_t pid = process_id;
    DCHECK_NE(GetProcessId(), pid);
    DCHECK_NE(0, pid);
    ...
    if (task_runner && !task_runner->RunsTasksInCurrentSequence())
    {
        task_runner->PostTask(FROM_HERE,
                              BindOnce(&GlobalActivityTracker::CleanupAfterProcess,
                                       Unretained(this),
                                       pid,
                                       now_stamp,
                                       exit_code,
...
void
GlobalActivityTracker::SetProcessPhase(ProcessPhase phase)
{
    process_data().SetInt(kProcessPhaseDataKey, phase);
}

void
GlobalActivityTracker::CleanupAfterProcess(int64_t process_id,
                                           int64_t exit_stamp,
                                           int exit_code,
                                           std::string&& command_line)
{
    ...
    if (process_exit_callback)
    {
        ...
        while ((ref = iter.GetNextOfType(kTypeIdProcessDataRecord)) != 0)
        {
            ...
            if (ActivityUserData::GetOwningProcessId(memory, &found_id, &create_stamp))
            {
        ...
        process_exit_callback.Run(process_id,
                                  exit_stamp,
                                  exit_code,
                                  exit_phase,
                                  std::move(command_line),
    ...
    while ((ref = iter.GetNext(&type)) != 0)
    {
        switch (type)
        {
            ...
            case ModuleInfoRecord::kPersistentTypeId: {
                ...
                // cases separately.
                if (OwningProcess::GetOwningProcessId(memory, &found_id, &create_stamp))
                {
...
void
GlobalActivityTracker::RecordLogMessage(StringPiece message)
{
    ...
    char* memory = allocator_->GetAsArray<char>(ref, kTypeIdGlobalLogMessage, message.size() + 1);
    if (memory)
    {
...
void
GlobalActivityTracker::RecordModuleInfo(const ModuleInfo& info)
{
    ...
    ModuleInfoRecord* record = ModuleInfoRecord::CreateFrom(info, allocator_.get());
    if (!record)
        return;
...
void
GlobalActivityTracker::RecordException(const void* pc, const void* origin, uint32_t code)
{
    RecordExceptionImpl(pc, origin, code);
}

void
GlobalActivityTracker::MarkDeleted()
{
    allocator_->SetMemoryState(PersistentMemoryAllocator::MEMORY_DELETED);
}

GlobalActivityTracker::GlobalActivityTracker(std::unique_ptr<PersistentMemoryAllocator> allocator,
                                             int stack_depth,
                                             int64_t process_id) :
    ...
    user_data_allocator_(allocator_.get(),
                         ...
                         /*make_iterable=*/true),
    process_data_(allocator_->GetAsArray<char>(AllocateFrom(allocator_.get(),
                  ...
                  process_id_)
{
    ...
    allocator_->MakeIterable(
            allocator_->GetAsReference(process_data_.GetBaseAddress(), kTypeIdProcessDataRecord));
...
void
GlobalActivityTracker::ReturnTrackerMemory(ManagedActivityTracker* tracker)
{
...
void
GlobalActivityTracker::RecordExceptionImpl(const void* pc, const void* origin, uint32_t code)
{
    ...
    tracker->RecordExceptionActivity(
            pc, origin, Activity::ACT_EXCEPTION, ActivityData::ForException(code));
}
...
void
ScopedActivity::ChangeAction(uint8_t action)
{
    DCHECK_EQ(0, action & Activity::ACT_CATEGORY_MASK);
...
void
ScopedActivity::ChangeInfo(int32_t info)
{
    ChangeTypeAndData(Activity::ACT_NULL, ActivityData::ForGeneric(id_, info));
}

void
ScopedActivity::ChangeActionAndInfo(uint8_t action, int32_t info)
{
    DCHECK_EQ(0, action & Activity::ACT_CATEGORY_MASK);
    ChangeTypeAndData(static_cast<Activity::Type>(Activity::ACT_GENERIC | action),
...
ScopedTaskRunActivity::ScopedTaskRunActivity(const void* program_counter,
    ...
    GlobalActivityTracker::ScopedThreadActivity(program_counter,
...
ScopedLockAcquireActivity::ScopedLockAcquireActivity(const void* program_counter,
...
ScopedEventWaitActivity::ScopedEventWaitActivity(const void* program_counter,
...
ScopedThreadJoinActivity::ScopedThreadJoinActivity(const void* program_counter,
...
ScopedProcessWaitActivity::ScopedProcessWaitActivity(const void* program_counter,
