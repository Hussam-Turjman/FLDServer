...
#include "fldserver/base/one_shot_event.h"
...
struct OneShotEvent::TaskInfo
{
    ...
    TaskInfo(const Location& from_here,
        ...
        from_here(from_here), runner(std::move(runner)), task(std::move(task)), delay(delay)
    {
        CHECK(this->runner.get());  // Detect mistakes with a decent stack frame.
    }
    TaskInfo(TaskInfo&&) = default;
    TaskInfo&
    operator=(TaskInfo&&) = default;
...
void
OneShotEvent::Post(const Location& from_here,
                   OnceClosure task,
...
void
OneShotEvent::PostDelayed(const Location& from_here, OnceClosure task, const TimeDelta& delay) const
{
...
OneShotEvent::Signal()
{
    ...
    for (TaskInfo& task : moved_tasks)
    {
        if (task.delay.is_zero())
            ...
            task.runner->PostDelayedTask(task.from_here, std::move(task.task), task.delay);
    }
    DCHECK(tasks_.empty()) << "No new tasks should be added during task running!";
}

void
OneShotEvent::PostImpl(const Location& from_here,
                       OnceClosure task,
                       scoped_refptr<SingleThreadTaskRunner> runner,
