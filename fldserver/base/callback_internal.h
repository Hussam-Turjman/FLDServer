...
class CORE_EXPORT BindStateBase
...
public:
    ...
    BindStateBase&
    operator=(const BindStateBase&) = delete;

private:
    BindStateBase(InvokeFuncStorage polymorphic_invoke, void (*destructor)(const BindStateBase*));
...
class CORE_EXPORT CallbackBase
{
public:
    inline CallbackBase(CallbackBase&& c) noexcept;
    CallbackBase&
    operator=(CallbackBase&& c) noexcept;

    explicit CallbackBase(const CallbackBaseCopyable& c);
    CallbackBase&
    operator=(const CallbackBaseCopyable& c);

    explicit CallbackBase(CallbackBaseCopyable&& c) noexcept;
    CallbackBase&
    operator=(CallbackBaseCopyable&& c) noexcept;
...
protected:
    ...
    bool
    EqualsInternal(const CallbackBase& other) const;
...
CallbackBase::CallbackBase(BindStateBase* bind_state) : bind_state_(AdoptRef(bind_state))
{
}

// CallbackBase<Copyable> is a direct base class of Copyable Callbacks.
class CORE_EXPORT CallbackBaseCopyable : public CallbackBase
{
public:
...
protected:
    ...
    explicit CallbackBaseCopyable(BindStateBase* bind_state) : CallbackBase(bind_state)
    {
    }
...
template <template <typename> class OriginalCallback,
          ...
          typename ThenR,
          typename... ThenArgs>
struct ThenHelper<OriginalCallback<void(OriginalArgs...)>, ThenCallback<ThenR(ThenArgs...)>>
{
...
template <template <typename> class OriginalCallback,
          ...
          typename ThenR,
          typename... ThenArgs>
struct ThenHelper<OriginalCallback<OriginalR(OriginalArgs...)>, ThenCallback<ThenR(ThenArgs...)>>
{
    ...
    CreateTrampoline()
    {
        return [](OriginalCallback<OriginalR(OriginalArgs...)> c1,
                  ThenCallback<ThenR(ThenArgs...)> c2,
                  OriginalArgs... c1_args) {
            return std::move(c2).Run(std::move(c1).Run(std::forward<OriginalArgs>(c1_args)...));
        };
    }
};
