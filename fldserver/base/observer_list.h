...
#include "fldserver/base/observer_list_internal.h"
...
public:
    ...
    public:
        ...
        explicit Iter(const ObserverList* list) :
            ...
            index_(0),
            max_index_(list->policy_ == ObserverListPolicy::ALL ? std::numeric_limits<size_t>::max()
        ...
        Iter(const Iter& other) : index_(other.index_), max_index_(other.max_index_)
        {
            if (other.list_)
    ...
    private:
        ...
        GetCurrent() const
        {
            ...
            return ObserverStorageType::template Get<ObserverType>(list_->observers_[index_]);
        }
        ...
        EnsureValidIndex()
        {
            ...
            while (index_ < max_index && list_->observers_[index_].IsMarkedForRemoval())
            {
                ++index_;
            }
        }
    ...
    explicit ObserverList(ObserverListPolicy policy = ObserverListPolicy::ALL) : policy_(policy)
    {
    ...
    ObserverList&
    operator=(const ObserverList&) = delete;
    ~ObserverList()
    {
    ...
    // not in this list.
    void
    RemoveObserver(const ObserverType* obs)
    {
        DCHECK(obs);
        const auto it =
                ranges::find_if(observers_, [obs](const auto& o) { return o.IsEqual(obs); });
    ...
    bool
    HasObserver(const ObserverType* obs) const
    {
        ...
        if (obs == nullptr)
            return false;
        return ranges::find_if(observers_, [obs](const auto& o) { return o.IsEqual(obs); })
               != observers_.end();
    }
...
private:
    ...
    std::string
    GetObserversCreationStackString() const
    {
        std::string result;
#if DCHECK_IS_ON()
