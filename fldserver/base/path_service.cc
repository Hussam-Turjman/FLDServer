...
{
bool
PathProvider(int key, FilePath* result);

#if IS_WIN
bool
PathProviderWin(int key, FilePath* result);
#elif IS_APPLE
bool
PathProviderMac(int key, FilePath* result);
#elif defined(OS_ANDROID)
bool
PathProviderAndroid(int key, FilePath* result);
#elif defined(OS_FUCHSIA)
bool
PathProviderFuchsia(int key, FilePath* result);
#elif IS_POSIX
// PathProviderPosix is the default path provider on POSIX OSes other than
// Mac and Android.
bool
PathProviderPosix(int key, FilePath* result);
#endif
...
Provider base_provider = {PathProvider,
                          nullptr,
#ifndef NDEBUG
                          PATH_START,
                          PATH_END,
...
#if IS_WIN
Provider base_provider_win = {PathProviderWin,
                              &base_provider,
#ifndef NDEBUG
                              PATH_WIN_START,
                              PATH_WIN_END,
...
#if IS_APPLE
Provider base_provider_mac = {PathProviderMac,
                              &base_provider,
#ifndef NDEBUG
                              PATH_MAC_START,
                              PATH_MAC_END,
...
Provider base_provider_android = {PathProviderAndroid,
                                  &base_provider,
#ifndef NDEBUG
                                  PATH_ANDROID_START,
                                  PATH_ANDROID_END,
...
Provider base_provider_fuchsia = {PathProviderFuchsia,
                                  &base_provider,
#ifndef NDEBUG
                                  0,
                                  0,
...
#if IS_POSIX && !IS_APPLE && !defined(OS_ANDROID) && !defined(OS_FUCHSIA)
Provider base_provider_posix = {PathProviderPosix,
                                &base_provider,
#ifndef NDEBUG
                                PATH_POSIX_START,
                                PATH_POSIX_END,
...
bool
LockedGetFromCache(int key, const PathData* path_data, FilePath* result)
...
bool
LockedGetFromOverrides(int key, PathData* path_data, FilePath* result)
        EXCLUSIVE_LOCKS_REQUIRED(path_data->lock)
{
...
// static
bool
PathService::Get(int key, FilePath* result)
{
...
FilePath
PathService::CheckedGet(int key)
{
    FilePath path;
...
// static
bool
PathService::Override(int key, const FilePath& path)
{
...
// static
bool
PathService::OverrideAndCreateIfNeeded(int key, const FilePath& path, bool is_absolute, bool create)
{
    PathData* path_data = GetPathData();
    DCHECK(path_data);
...
// static
bool
PathService::RemoveOverrideForTests(int key)
{
    PathData* path_data = GetPathData();
    DCHECK(path_data);
...
// static
bool
PathService::IsOverriddenForTests(int key)
{
    PathData* path_data = GetPathData();
    DCHECK(path_data);
...
// static
void
PathService::RegisterProvider(ProviderFunc func, int key_start, int key_end)
{
    PathData* path_data = GetPathData();
    DCHECK(path_data);
    ...
    while (iter)
    {
        DCHECK(key_start >= iter->key_end || key_end <= iter->key_start) << "path provider "
                                                                            "collision";
        iter = iter->next;
    }
#endif
