...
public:
    ...
    operator=(const CancelableCallbackImpl&) = delete;

    // |callback| must not be null.
    explicit CancelableCallbackImpl(CallbackType callback) : callback_(std::move(callback))
    {
        DCHECK(callback_);
    }
...
private:
    ...
    void
    MakeForwarder(RepeatingCallback<void(Args...)>* out) const
    {
    ...
    template <typename... Args>
    void
    MakeForwarder(OnceCallback<void(Args...)>* out) const
    {
        using ForwarderType = void (CancelableCallbackImpl::*)(Args...);
