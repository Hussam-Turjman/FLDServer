...
// #include "fldserver/base/callback_forward.h" instead of this file.
...
#include "fldserver/base/callback_internal.h"
...
public:
    ...
    explicit OnceCallback(internal::BindStateBase* bind_state) : internal::CallbackBase(bind_state)
    {
    }

    OnceCallback(const OnceCallback&) = delete;
    OnceCallback&
    operator=(const OnceCallback&) = delete;

    OnceCallback(OnceCallback&&) noexcept = default;
    OnceCallback&
    operator=(OnceCallback&&) noexcept = default;

    OnceCallback(RepeatingCallback<RunType> other) : internal::CallbackBase(std::move(other))
    {
    }

    OnceCallback&
    operator=(RepeatingCallback<RunType> other)
    {
    ...
    Then(OnceCallback<ThenR(ThenArgs...)> then) &&
    {
        CHECK(then);
        return BindOnce(internal::ThenHelper<OnceCallback,
                                             OnceCallback<ThenR(ThenArgs...)>>::CreateTrampoline(),
                        std::move(*this),
                        std::move(then));
    }
    ...
    Then(RepeatingCallback<ThenR(ThenArgs...)> then) &&
    {
        CHECK(then);
        return BindOnce(
                internal::ThenHelper<OnceCallback,
...
public:
    ...
    RepeatingCallback&
    operator=(const RepeatingCallback&) = default;
    RepeatingCallback(RepeatingCallback&&) noexcept = default;
    RepeatingCallback&
    operator=(RepeatingCallback&&) noexcept = default;

    bool
    operator==(const RepeatingCallback& other) const
    {
    ...
    Run(Args... args) const&
    {
        PolymorphicInvoke f = reinterpret_cast<PolymorphicInvoke>(this->polymorphic_invoke());
    ...
    Then(RepeatingCallback<ThenR(ThenArgs...)> then) const&
    {
        ...
        return BindRepeating(
                internal::ThenHelper<RepeatingCallback,
                                     RepeatingCallback<ThenR(ThenArgs...)>>::CreateTrampoline(),
                *this,
                std::move(then));
    }
