...
#include "fldserver/base/version.h"
...
bool
ParseVersionNumbers(StringPiece version_str, std::vector<uint32_t>* parsed)
{
...
int
CompareVersionComponents(const std::vector<uint32_t>& components1,
...
Version::Version(std::vector<uint32_t> components) : components_(std::move(components))
{
}
...
// static
bool
Version::IsValidWildcardString(StringPiece wildcard_string)
{
...
int
Version::CompareToWildcardString(StringPiece wildcard_string) const
{
    DCHECK(IsValid());
    ...
    std::vector<uint32_t> parsed;
    const bool success =
            ParseVersionNumbers(wildcard_string.substr(0, wildcard_string.length() - 2), &parsed);
    DCHECK(success);
...
std::ostream&
operator<<(std::ostream& stream, const Version& v)
{
