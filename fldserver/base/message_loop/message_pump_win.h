...
#include "fldserver/base/win/message_window.h"
...
class CORE_EXPORT MessagePumpWin : public MessagePump
{
public:
    MessagePumpWin();
...
//
class CORE_EXPORT MessagePumpForUI : public MessagePumpWin
{
public:
    MessagePumpForUI();
    ...
    public:
        virtual void
        WillDispatchMSG(const MSG& msg) = 0;
        virtual void
        DidDispatchMSG(const MSG& msg) = 0;
    };

    void
    AddObserver(Observer* observer);
    void
    RemoveObserver(Observer* obseerver);

private:
    bool
    MessageCallback(UINT message, WPARAM wparam, LPARAM lparam, LRESULT* result);
    void
    DoRunLoop() override;
    ...
    HandleTimerMessage();
    void
    ScheduleNativeTimer(Delegate::NextWorkInfo next_work_info);
    void
    KillNativeTimer();
    bool
    ProcessNextWindowsMessage();
    bool
    ProcessMessageHelper(const MSG& msg);
    bool
...
//
class CORE_EXPORT MessagePumpForIO : public MessagePumpWin
{
public:
    struct CORE_EXPORT IOContext
    {
        IOContext();
    ...
    public:
        ...
        IOHandler&
        operator=(const IOHandler&) = delete;
        ...
        // on error.
        virtual void
        OnIOCompleted(IOContext* context, DWORD bytes_transfered, DWORD error) = 0;

        const Location&
        io_handler_location()
        {
            return io_handler_location_;
        }
    ...
    HRESULT
    RegisterIOHandler(HANDLE file_handle, IOHandler* handler);
    ...
    bool
    RegisterJobObject(HANDLE job_handle, IOHandler* handler);
...
private:
    ...
    WaitForWork(Delegate::NextWorkInfo next_work_info);
    bool
    GetIOItem(DWORD timeout, IOItem* item);
    bool
    ProcessInternalIOItem(const IOItem& item);
    ...
    bool
    WaitForIOCompletion(DWORD timeout);
