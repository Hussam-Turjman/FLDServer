...
inline bool
PickleIterator::ReadBuiltinType(Type* result)
{
...
inline void
PickleIterator::Advance(size_t size)
{
...
template <typename Type>
inline const char*
PickleIterator::GetReadPointerAndAdvance()
{
...
const char*
PickleIterator::GetReadPointerAndAdvance(int num_bytes)
{
...
inline const char*
PickleIterator::GetReadPointerAndAdvance(int num_elements, size_t size_element)
{
...
bool
PickleIterator::ReadBool(bool* result)
{
    return ReadBuiltinType(result);
}

bool
PickleIterator::ReadInt(int* result)
{
    return ReadBuiltinType(result);
}

bool
PickleIterator::ReadLong(long* result)
{
...
bool
PickleIterator::ReadUInt16(uint16_t* result)
{
    return ReadBuiltinType(result);
}

bool
PickleIterator::ReadUInt32(uint32_t* result)
{
    return ReadBuiltinType(result);
}

bool
PickleIterator::ReadInt64(int64_t* result)
{
    return ReadBuiltinType(result);
}

bool
PickleIterator::ReadUInt64(uint64_t* result)
{
    return ReadBuiltinType(result);
}

bool
PickleIterator::ReadFloat(float* result)
{
    // crbug.com/315213
...
bool
PickleIterator::ReadDouble(double* result)
{
    // crbug.com/315213
...
bool
PickleIterator::ReadString(std::string* result)
{
    int len;
    if (!ReadInt(&len))
        return false;
...
bool
PickleIterator::ReadStringPiece(StringPiece* result)
{
    int len;
    if (!ReadInt(&len))
        return false;
...
bool
PickleIterator::ReadString16(std::u16string* result)
{
    int len;
    if (!ReadInt(&len))
        return false;
...
bool
PickleIterator::ReadStringPiece16(StringPiece16* result)
{
    int len;
    if (!ReadInt(&len))
        return false;
...
bool
PickleIterator::ReadData(const char** data, int* length)
{
    *length = 0;
    *data = nullptr;
...
bool
PickleIterator::ReadData(base::span<const uint8_t>* data)
{
    const char* ptr;
    int length;
...
bool
PickleIterator::ReadBytes(const char** data, int length)
{
...
Pickle::Pickle() :
    header_(nullptr), header_size_(sizeof(Header)), capacity_after_header_(0), write_offset_(0)
{
...
void
Pickle::WriteString(const StringPiece& value)
{
...
void
Pickle::WriteString16(const StringPiece16& value)
{
    WriteInt(static_cast<int>(value.size()));
...
void
Pickle::WriteData(const char* data, int length)
{
    DCHECK_GE(length, 0);
    WriteInt(length);
...
void
Pickle::WriteBytes(const void* data, int length)
{
    WriteBytesCommon(data, length);
}

void
Pickle::Reserve(size_t length)
{
...
bool
Pickle::WriteAttachment(scoped_refptr<Attachment> attachment)
{
    return false;
}

bool
Pickle::ReadAttachment(base::PickleIterator* iter, scoped_refptr<Attachment>* attachment) const
{
    return false;
}

bool
Pickle::HasAttachments() const
{
    return false;
}

void
Pickle::Resize(size_t new_capacity)
{
...
void*
Pickle::ClaimBytes(size_t num_bytes)
{
...
size_t
Pickle::GetTotalAllocatedSize() const
{
    if (capacity_after_header_ == kCapacityReadOnly)
        return 0;
...
// static
const char*
Pickle::FindNext(size_t header_size, const char* start, const char* end)
{
...
// static
bool
Pickle::PeekNext(size_t header_size, const char* start, const char* end, size_t* pickle_size)
{
...
void
Pickle::WriteBytesStatic(const void* data)
{
    WriteBytesCommon(data, length);
}

template void
Pickle::WriteBytesStatic<2>(const void* data);
template void
Pickle::WriteBytesStatic<4>(const void* data);
template void
Pickle::WriteBytesStatic<8>(const void* data);

inline void*
Pickle::ClaimUninitializedBytesInternal(size_t length)
{
    ...
    if (new_size > capacity_after_header_)
    {
        ...
        {
            new_capacity = bits::AlignUp(new_capacity, kPickleHeapAlign) - kPayloadUnit;
        }
...
inline void
Pickle::WriteBytesCommon(const void* data, size_t length)
{
    DCHECK_NE(kCapacityReadOnly, capacity_after_header_) << "oops: pickle is readonly";
