...
//
#include "fldserver/base/win/event_trace_provider.h"
...
namespace win
{
TRACE_GUID_REGISTRATION EtwTraceProvider::obligatory_guid_registration_ = {&GUID_NULL, nullptr};

EtwTraceProvider::EtwTraceProvider(const GUID& provider_name) : provider_name_(provider_name)
{
}
...
ULONG
EtwTraceProvider::Callback(WMIDPREQUESTCODE request, void* buffer)
{
    switch (request)
    {
        case WMI_ENABLE_EVENTS: return EnableEvents(buffer);
        case WMI_DISABLE_EVENTS: return DisableEvents();
...
EtwTraceProvider::Register()
{
    ...
    return ::RegisterTraceGuids(ControlCallback,
                                this,
                                &provider_name_,
                                1,
...
ULONG
EtwTraceProvider::Log(const EtwEventClass& event_class,
                      EtwEventType type,
                      EtwEventLevel level,
                      const char* message)
{
    ...
    event.fields[0].Length =
            message ? static_cast<ULONG>(sizeof(message[0]) * (1 + strlen(message))) : 0;
                      ...
                      const wchar_t* message)
{
    ...
    event.fields[0].DataPtr = reinterpret_cast<ULONG64>(message);
    event.fields[0].Length =
            message ? static_cast<ULONG>(sizeof(message[0]) * (1 + wcslen(message))) : 0;

    return ::TraceEvent(session_handle_, &event.header);
}

ULONG
EtwTraceProvider::Log(EVENT_TRACE_HEADER* event)
{
