...
using AbiType = typename ABI::Windows::Foundation::Internal::GetAbiType<TComplex>::type;

template <typename TComplex>
using LogicalType = typename ABI::Windows::Foundation::Internal::GetLogicalType<TComplex>::type;
...
template <typename TComplex>
using StorageType =
        std::conditional_t<std::is_convertible<AbiType<TComplex>, IUnknown*>::value,
...
template <typename T>
HRESULT
CopyTo(const Microsoft::WRL::ComPtr<T>& value, T** ptr)
{
...
template <typename T>
HRESULT
CopyTo(const base::Optional<T>& value, T* ptr)
{
    *ptr = *value;
    return S_OK;
}
...
template <typename T>
HRESULT
CopyN(typename std::vector<T>::const_iterator first, unsigned count, T* result)
{
...
template <typename T>
HRESULT
CopyN(typename std::vector<Microsoft::WRL::ComPtr<T>>::const_iterator first,
      unsigned count,
      T** result)
{
...
inline bool
IsEqual(const HSTRING& lhs, const HSTRING& rhs)
{
    INT32 result;
...
template <typename T>
bool
IsEqual(const Microsoft::WRL::ComPtr<T>& com_ptr, const T* ptr)
{
...
{
    bool
    operator()(const HSTRING& lhs, const HSTRING& rhs) const
    {
        INT32 result;
        HRESULT hr = HStringCompare(lhs, rhs, &result);
        DCHECK(SUCCEEDED(hr));
    ...
    template <typename T>
    bool
    operator()(const Microsoft::WRL::ComPtr<T>& com_ptr, const T* ptr) const
    {
        return com_ptr.Get() < ptr;
    }
