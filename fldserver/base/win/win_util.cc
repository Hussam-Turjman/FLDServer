...
#include "fldserver/base/win/win_util.h"

#include "fldserver/base/win/fldserver_winrt_util.h"
...
#include "fldserver/base/scoped_native_library.h"
...
#include "fldserver/base/win/propvarutil.h"
#include "fldserver/base/win/registry.h"
#include "fldserver/base/win/scoped_co_mem.h"
#include "fldserver/base/win/scoped_handle.h"
#include "fldserver/base/win/scoped_hstring.h"
#include "fldserver/base/win/scoped_propvariant.h"
#include "fldserver/base/win/shlwapi.h"
...
bool
SetPropVariantValueForPropertyStore(IPropertyStore* property_store,
                                    ...
                                    const ScopedPropVariant& property_value)
{
    ...
    // PID definitions.
    DPLOG(ERROR) << "Failed to set property with GUID " << WStringFromGUID(property_key.fmtid)
...
bool
SetProcessDpiAwarenessWrapper(PROCESS_DPI_AWARENESS value)
{
    ...
    if (set_process_dpi_awareness_func)
    {
        ...
        if (SUCCEEDED(hr))
            return true;
        DLOG_IF(ERROR, hr == E_ACCESSDENIED) << "Access denied error from "
                                                "SetProcessDpiAwarenessInternal. "
                                                "Function called twice, or manifest was used.";
        NOTREACHED() << "SetProcessDpiAwarenessInternal failed with unexpected error: " << hr;
        return false;
    }
...
EnablePerMonitorV2()
{
    ...
    {
        return set_process_dpi_awareness_context_func(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2);
    }

    DCHECK_LT(GetVersion(), Version::WIN10_RS2) << "SetProcessDpiAwarenessContext should be "
...
GetRegisteredWithManagementStateStorage()
{
    static bool state = []() {
        ...
        ScopedNativeLibrary library(FilePath(FILE_PATH_LITERAL("MDMRegistration.dll")));
        if (!library.is_valid())
            return false;

        using IsDeviceRegisteredWithManagementFunction =
                decltype(&::IsDeviceRegisteredWithManagement);
        IsDeviceRegisteredWithManagementFunction is_device_registered_with_management_function =
        ...
        HRESULT hr = is_device_registered_with_management_function(&is_managed, 0, nullptr);
...
NativeLibrary
PinUser32Internal(NativeLibraryLoadError* error)
{
...
bool
IsWindows10TabletMode(HWND hwnd)
{
    if (GetVersion() < Version::WIN10)
        return false;

    if (!ResolveCoreWinRTDelayload() || !ScopedHString::ResolveCoreWinRTStringDelayload())
    {
        return false;
    }

    ScopedHString view_settings_guid =
            ScopedHString::Create(RuntimeClass_Windows_UI_ViewManagement_UIViewSettings);
    ...
    Microsoft::WRL::ComPtr<ABI::Windows::UI::ViewManagement::IUIViewSettings> view_settings;
...
bool
IsKeyboardPresentOnSlate(HWND hwnd, std::string* reason)
{
    bool result = false;
    ...
    *reason += "Not a tablet device";
    ...
    static const auto get_rotation_state =
            reinterpret_cast<GetAutoRotationState>(GetUser32FunctionPointer("GetAutoRotationStat"
                                                                            "e"));
    if (get_rotation_state)
    {
        AR_STATE auto_rotation_state = AR_ENABLED;
        get_rotation_state(&auto_rotation_state);
        if ((auto_rotation_state & AR_NOSENSOR) || (auto_rotation_state & AR_NOT_SUPPORTED))
        {
    ...
    HDEVINFO device_info =
            SetupDiGetClassDevs(&KEYBOARD_CLASS_GUID, nullptr, nullptr, DIGCF_PRESENT);
    ...
    for (DWORD i = 0;; ++i)
    {
        ...
        CONFIGRET status =
                CM_Get_Device_ID(device_info_data.DevInst, device_id, MAX_DEVICE_ID_LEN, 0);
...
bool
GetUserSidString(std::wstring* user_sid)
{
...
bool
SetBooleanValueForPropertyStore(IPropertyStore* property_store,
                                const PROPERTYKEY& property_key,
...
bool
SetStringValueForPropertyStore(IPropertyStore* property_store,
                               const PROPERTYKEY& property_key,
...
bool
SetClsidForPropertyStore(IPropertyStore* property_store,
                         const PROPERTYKEY& property_key,
...
bool
SetAppIdForPropertyStore(IPropertyStore* property_store, const wchar_t* app_id)
{
...
bool
AddCommandToAutoRun(HKEY root_key, const std::wstring& name, const std::wstring& command)
{
...
bool
RemoveCommandFromAutoRun(HKEY root_key, const std::wstring& name)
{
    RegKey autorun_key(root_key, kAutoRunKeyPath, KEY_SET_VALUE);
...
bool
ReadCommandFromAutoRun(HKEY root_key, const std::wstring& name, std::wstring* command)
{
...
void
SetShouldCrashOnProcessDetach(bool crash)
{
...
bool
IsTabletDevice(std::string* reason, HWND hwnd)
{
    if (GetVersion() < Version::WIN8)
    {
        if (reason)
...
bool
IsDeviceUsedAsATablet(std::string* reason)
{
    if (GetVersion() < Version::WIN8)
    {
        if (reason)
    ...
    static const auto get_auto_rotation_state_func =
            reinterpret_cast<GetAutoRotationStateType>(GetUser32FunctionPointer("GetAutoRotationSta"
                                                                                "te"));
...
IsUser32AndGdi32Available()
{
    static auto is_user32_and_gdi32_available = []() {
        ...
        using GetProcessMitigationPolicyType = decltype(GetProcessMitigationPolicy)*;
        ...
        if (get_process_mitigation_policy_func(
                    GetCurrentProcess(), ProcessSystemCallDisablePolicy, &policy, sizeof(policy)))
        {
...
bool
GetLoadedModulesSnapshot(HANDLE process, std::vector<HMODULE>* snapshot)
{
    DCHECK(snapshot);
    ...
    do
    {
        ...
        // small.
        DCHECK_GE(std::numeric_limits<DWORD>::max(), snapshot->size() * sizeof(HMODULE));
...
void
EnableFlicks(HWND hwnd)
{
    ::RemoveProp(hwnd, reinterpret_cast<LPCWSTR>(MICROSOFT_TABLETPENSERVICE_PROPERTY));
}

void
DisableFlicks(HWND hwnd)
{
    ::SetProp(hwnd,
              reinterpret_cast<LPCWSTR>(MICROSOFT_TABLETPENSERVICE_PROPERTY),
...
EnableHighDPISupport()
{
    ...
    PROCESS_DPI_AWARENESS process_dpi_awareness = GetVersion() >= Version::WIN10
...
WStringFromGUID(REFGUID rguid)
{
    ...
    CHECK(SUCCEEDED(StringCchPrintfW(guid_string,
...
bool
PinUser32(NativeLibraryLoadError* error)
{
    return PinUser32Internal(error) != nullptr;
}

void*
GetUser32FunctionPointer(const char* function_name, NativeLibraryLoadError* error)
{
...
GetWindowObjectName(HANDLE handle)
{
    ...
    if (!::GetUserObjectInformation(
                handle, UOI_NAME, WriteInto(&object_name, size / sizeof(wchar_t)), size, &size))
    {
        DPCHECK(false);
    }
...
bool
IsRunningUnderDesktopName(WStringPiece desktop_name)
{
...
IsCurrentSessionRemote()
{
    ...
    static constexpr wchar_t kRdpSettingsKeyName[] = L"SYSTEM\\CurrentControlSet\\Control\\Terminal"
                                                     L" Server";
    ...
    if (key.ReadValueDW(kGlassSessionIdValueName, &glass_session_id) != ERROR_SUCCESS)
        return false;
