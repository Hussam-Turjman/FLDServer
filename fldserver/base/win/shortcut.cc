...
#include "fldserver/base/win/shortcut.h"
...
// be released.
void
InitializeShortcutInterfaces(const wchar_t* shortcut,
...
bool
CreateOrUpdateShortcutLink(const FilePath& shortcut_path,
                           ...
                           ShortcutOperation operation)
{
    ...
    switch (operation)
    {
        ...
        case SHORTCUT_UPDATE_EXISTING:
            InitializeShortcutInterfaces(
                    shortcut_path.value().c_str(), &i_shell_link, &i_persist_file);
            break;
        case SHORTCUT_REPLACE_EXISTING:
            InitializeShortcutInterfaces(
                    shortcut_path.value().c_str(), &old_i_shell_link, &old_i_persist_file);
    ...
    if ((properties.options & ShortcutProperties::PROPERTIES_WORKING_DIR)
        && FAILED(i_shell_link->SetWorkingDirectory(properties.working_dir.value().c_str())))
    {
        return false;
    }
    ...
    bool has_toast_activator_clsid =
            (properties.options & ShortcutProperties::PROPERTIES_TOAST_ACTIVATOR_CLSID) != 0;
    if (has_app_id || has_dual_mode || has_toast_activator_clsid)
    {
        ...
        if (has_app_id
            && !SetAppIdForPropertyStore(property_store.Get(), properties.app_id.c_str()))
        {
            return false;
        }
        if (has_dual_mode
            && !SetBooleanValueForPropertyStore(
                    property_store.Get(), PKEY_AppUserModel_IsDualMode, properties.dual_mode))
        {
            return false;
        }
    ...
    if (succeeded)
    {
        ...
        {
            SHChangeNotify(SHCNE_CREATE, SHCNF_PATH, shortcut_path.value().c_str(), nullptr);
        }
    }
...
bool
ResolveShortcutProperties(const FilePath& shortcut_path,
                          uint32_t options,
                          ShortcutProperties* properties)
{
    ...
    if (FAILED(::CoCreateInstance(
                CLSID_ShellLink, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&i_shell_link))))
    {
        return false;
    }
    ...
    {
        if (FAILED(i_shell_link->GetPath(temp, MAX_PATH, nullptr, SLGP_UNCPRIORITY)))
        {
            return false;
        }
    ...
    if (options
        & (ShortcutProperties::PROPERTIES_APP_ID | ShortcutProperties::PROPERTIES_DUAL_MODE
           | ShortcutProperties::PROPERTIES_TOAST_ACTIVATOR_CLSID))
    {
        ...
        if (options & ShortcutProperties::PROPERTIES_APP_ID)
        {
            ...
            if (property_store->GetValue(PKEY_AppUserModel_ID, pv_app_id.Receive()) != S_OK)
            {
                return false;
            }
            switch (pv_app_id.get().vt)
            {
                case VT_EMPTY: properties->set_app_id(std::wstring()); break;
                case VT_LPWSTR: properties->set_app_id(pv_app_id.get().pwszVal); break;
                default:
        ...
        if (options & ShortcutProperties::PROPERTIES_DUAL_MODE)
        {
            ...
            if (property_store->GetValue(PKEY_AppUserModel_IsDualMode, pv_dual_mode.Receive())
                != S_OK)
            {
                return false;
            }
            ...
            {
                case VT_EMPTY: properties->set_dual_mode(false); break;
                case VT_BOOL:
        ...
        if (options & ShortcutProperties::PROPERTIES_TOAST_ACTIVATOR_CLSID)
        {
            ...
            {
                case VT_EMPTY: properties->set_toast_activator_clsid(CLSID_NULL); break;
                case VT_CLSID:
                    properties->set_toast_activator_clsid(*(pv_toast_activator_clsid.get().puuid));
                    break;
                default:
...
bool
ResolveShortcut(const FilePath& shortcut_path, FilePath* target_path, std::wstring* args)
{
    uint32_t options = 0;
    if (target_path)
...
bool
PinShortcutToTaskbar(const FilePath& shortcut)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    DCHECK(CanPinShortcutToTaskbar());

    intptr_t result = reinterpret_cast<intptr_t>(
            ShellExecute(nullptr, L"taskbarpin", shortcut.value().c_str(), nullptr, nullptr, 0));
    return result > 32;
}

bool
UnpinShortcutFromTaskbar(const FilePath& shortcut)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);

    intptr_t result = reinterpret_cast<intptr_t>(
            ShellExecute(nullptr, L"taskbarunpin", shortcut.value().c_str(), nullptr, nullptr, 0));
    return result > 32;
}
