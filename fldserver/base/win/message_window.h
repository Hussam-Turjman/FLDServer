...
class CORE_EXPORT MessageWindow
{
public:
    ...
    using MessageCallback = base::RepeatingCallback<bool(
    ...
    bool
    Create(MessageCallback message_callback);

    // Same as Create() but assigns the name to the created window.
    bool
    CreateNamed(MessageCallback message_callback, const std::wstring& window_name);
    ...
    // |window_name|.
    static HWND
    FindWindow(const std::wstring& window_name);
...
private:
    ...
    bool
    DoCreate(MessageCallback message_callback, const wchar_t* window_name);
