...
// if source is NULL.
bool
CheckAndDuplicateHandle(HANDLE source, ScopedHandle* target)
{
    if (!source)
        return true;
...
ScopedProcessInformation::ScopedProcessInformation(const PROCESS_INFORMATION& process_info)
{
    Set(process_info);
}
...
bool
ScopedProcessInformation::IsValid() const
{
...
void
ScopedProcessInformation::Set(const PROCESS_INFORMATION& process_info)
{
    if (IsValid())
        Close();
...
bool
ScopedProcessInformation::DuplicateFrom(const ScopedProcessInformation& other)
{
