...
// WindowsCreateString().
struct CORE_EXPORT ScopedHStringTraits
{
    static HSTRING
    InvalidValue()
    {
...
//
class CORE_EXPORT ScopedHString : public ScopedGeneric<HSTRING, base::internal::ScopedHStringTraits>
{
public:
    // Constructs a ScopedHString from an HSTRING, and takes ownership of |hstr|.
    explicit ScopedHString(HSTRING hstr);

    static ScopedHString
    Create(WStringPiece str);
    static ScopedHString
    Create(StringPiece str);

    // Loads all required HSTRING functions, available from Win8 and onwards.
    static bool
    ResolveCoreWinRTStringDelayload() WARN_UNUSED_RESULT;
