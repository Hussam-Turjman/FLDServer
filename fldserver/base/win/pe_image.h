...
public:
    ...
    // Returns true to continue the enumeration.
    using EnumImportChunksFunction =
            bool (*)(const PEImage&, LPCSTR, PIMAGE_THUNK_DATA, PIMAGE_THUNK_DATA, PVOID);

    // Callback to enumerate imports.
    // module is the dll that exports this symbol. cookie is the value passed to
    // the enumerate method.
    // Returns true to continue the enumeration.
    using EnumImportsFunction =
            bool (*)(const PEImage&, LPCSTR, DWORD, LPCSTR, DWORD, PIMAGE_THUNK_DATA, PVOID);
    ...
    using EnumDelayImportChunksFunction = bool (*)(
            const PEImage&, PImgDelayDescr, LPCSTR, PIMAGE_THUNK_DATA, PIMAGE_THUNK_DATA, PVOID);
    ...
    // out of bounds.
    PVOID
    GetImageDirectoryEntryAddr(UINT directory) const;
    ...
    PIMAGE_SECTION_HEADER
    GetImageSectionFromAddr(PVOID address) const;

    // Returns the section header for a given section.
    PIMAGE_SECTION_HEADER
    GetImageSectionHeaderByName(LPCSTR section_name) const;
    ...
    // properly terminated.
    bool
    GetDebugId(LPGUID guid, LPDWORD age, LPCSTR* pdb_filename, size_t* pdb_filename_length) const;
    ...
    PDWORD
    GetExportEntry(LPCSTR name) const;
    ...
    FARPROC
    GetProcAddress(LPCSTR function_name) const;

    // Retrieves the ordinal for a given exported symbol.
    // Returns true if the symbol was found.
    bool
    GetProcOrdinal(LPCSTR function_name, WORD* ordinal) const;

    // Enumerates PE sections.
    // cookie is a generic cookie to pass to the callback.
    // Returns true on success.
    bool
    EnumSections(EnumSectionsFunction callback, PVOID cookie) const;

    // Enumerates PE exports.
    // cookie is a generic cookie to pass to the callback.
    // Returns true on success.
    bool
    EnumExports(EnumExportsFunction callback, PVOID cookie) const;
    ...
    // specified module.
    bool
    EnumAllImports(EnumImportsFunction callback, PVOID cookie, LPCSTR target_module_name) const;
    ...
    // specified module.
    bool
    EnumImportChunks(EnumImportChunksFunction callback,
                     PVOID cookie,
    ...
    // imports are resolved when the target module is found.
    bool
    EnumDelayImportChunks(EnumDelayImportChunksFunction callback,
                          PVOID cookie,
                          LPCSTR target_module_name) const;
    ...
    // Returns true on success.
    bool
    EnumRelocs(EnumRelocsFunction callback, PVOID cookie) const;
    ...
    virtual PVOID
    RVAToAddr(uintptr_t rva) const;

    // Converts an rva value to an offset on disk.
    // Returns true on success.
    bool
    ImageRVAToOnDiskOffset(uintptr_t rva, DWORD* on_disk_offset) const;

    // Converts an address to an offset on disk.
    // Returns true on success.
    bool
    ImageAddrToOnDiskOffset(LPVOID address, DWORD* on_disk_offset) const;

private:
    // Returns a pointer to a data directory, or NULL if |directory| is out of
    // range.
    const IMAGE_DATA_DIRECTORY*
    GetDataDirectory(UINT directory) const;
...
public:
    ...
    PVOID
    RVAToAddr(uintptr_t rva) const override;
};

inline bool
PEImage::IsOrdinal(LPCSTR name)
{
...
inline PIMAGE_IMPORT_DESCRIPTOR
...
inline PIMAGE_EXPORT_DIRECTORY
