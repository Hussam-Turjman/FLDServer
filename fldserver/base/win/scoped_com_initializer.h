...
#include "fldserver/base/win/com_init_balancer.h"
...
class CORE_EXPORT ScopedCOMInitializer : public ScopedWindowsThreadEnvironment
{
public:
    ...
    explicit ScopedCOMInitializer(Uninitialization uninitialization = Uninitialization::kAllow);
    ...
    bool
    Succeeded() const override;

    // Used for testing. Returns the COM balancer's apartment thread ref count.
    DWORD
    GetCOMBalancerReferenceCountForTesting() const;

private:
    void
    Initialize(COINIT init, Uninitialization uninitialization);
