...
class CORE_EXPORT EtwTraceProperties
        {
                public:
                EtwTraceProperties();
                ...
                HRESULT
                SetLoggerName(const wchar_t* logger_name);
                const wchar_t*
                GetLoggerFileName() const
                {
                ...
                HRESULT
                SetLoggerFileName(const wchar_t* logger_file_name);
...
class CORE_EXPORT EtwTraceController
        {
                public:
                EtwTraceController();
                ~EtwTraceController();
                ...
                HRESULT
                Start(const wchar_t* session_name, EtwTraceProperties* prop);
                ...
                HRESULT
                StartRealtimeSession(const wchar_t* session_name, size_t buffer_size);
                ...
                HRESULT
                EnableProvider(const GUID& provider, UCHAR level, ULONG flags = 0xFFFFFFFF);
                // Disables "provider".
                HRESULT
                DisableProvider(const GUID& provider);

                // Stops our session and retrieve the new properties of the session,
                // properties may be NULL.
                HRESULT
                Stop(EtwTraceProperties* properties);

                // Flushes our session and retrieve the current properties,
                // properties may be NULL.
                HRESULT
                Flush(EtwTraceProperties* properties);

                // Static utility functions for controlling
                // sessions we don't necessarily own.
                static HRESULT
                Start(const wchar_t* session_name, EtwTraceProperties* properties, TRACEHANDLE* session_handle);

                static HRESULT
                Query(const wchar_t* session_name, EtwTraceProperties* properties);

                static HRESULT
                Update(const wchar_t* session_name, EtwTraceProperties* properties);

                static HRESULT
                Stop(const wchar_t* session_name, EtwTraceProperties* properties);
                static HRESULT
                Flush(const wchar_t* session_name, EtwTraceProperties* properties);
