...
LoadComBaseFunction(const char* function_name)
{
    static HMODULE const handle = []() {
        ...
        return ::LoadLibraryEx(
                (L"combase.dll"), nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
    }();
    return handle ? ::GetProcAddress(handle, function_name) : nullptr;
}

decltype(&::RoActivateInstance)
GetRoActivateInstanceFunction()
{
    static decltype(&::RoActivateInstance) const function =
            reinterpret_cast<decltype(&::RoActivateInstance)>(LoadComBaseFunction("RoActivateInstan"
                                                                                  "ce"));
    return function;
}

decltype(&::RoGetActivationFactory)
GetRoGetActivationFactoryFunction()
{
    static decltype(&::RoGetActivationFactory) const function =
            reinterpret_cast<decltype(&::RoGetActivationFactory)>(LoadComBaseFunction("RoGetActivat"
                                                                                      "ionFactor"
                                                                                      "y"));
    return function;
}
...
HRESULT
RoGetActivationFactory(HSTRING class_id, const IID& iid, void** out_factory)
{
...
HRESULT
RoActivateInstance(HSTRING class_id, IInspectable** instance)
{
