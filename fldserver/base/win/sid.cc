...
#include "fldserver/base/win/sid.h"
...
#include "fldserver/base/win/scoped_localalloc.h"
...
WellKnownCapabilityToRid(WellKnownCapability capability)
{
    switch (capability)
    {
        case WellKnownCapability::kInternetClient: return SECURITY_CAPABILITY_INTERNET_CLIENT;
        ...
        case WellKnownCapability::kPicturesLibrary: return SECURITY_CAPABILITY_PICTURES_LIBRARY;
        case WellKnownCapability::kVideosLibrary: return SECURITY_CAPABILITY_VIDEOS_LIBRARY;
        case WellKnownCapability::kMusicLibrary: return SECURITY_CAPABILITY_MUSIC_LIBRARY;
        case WellKnownCapability::kDocumentsLibrary: return SECURITY_CAPABILITY_DOCUMENTS_LIBRARY;
        ...
        case WellKnownCapability::kRemovableStorage: return SECURITY_CAPABILITY_REMOVABLE_STORAGE;
        case WellKnownCapability::kAppointments: return SECURITY_CAPABILITY_APPOINTMENTS;
        case WellKnownCapability::kContacts: return SECURITY_CAPABILITY_CONTACTS;
    }
    return nullopt;
}

Optional<WELL_KNOWN_SID_TYPE>
WellKnownSidToEnum(WellKnownSid sid)
{
    switch (sid)
    {
        case WellKnownSid::kNull: return WinNullSid;
        case WellKnownSid::kWorld: return WinWorldSid;
        case WellKnownSid::kCreatorOwner: return WinCreatorOwnerSid;
        case WellKnownSid::kNetwork: return WinNetworkSid;
        case WellKnownSid::kBatch: return WinBatchSid;
        case WellKnownSid::kInteractive: return WinInteractiveSid;
        case WellKnownSid::kService: return WinServiceSid;
        case WellKnownSid::kAnonymous: return WinAnonymousSid;
        case WellKnownSid::kSelf: return WinSelfSid;
        case WellKnownSid::kAuthenticatedUser: return WinAuthenticatedUserSid;
        case WellKnownSid::kRestricted: return WinRestrictedCodeSid;
        case WellKnownSid::kLocalSystem: return WinLocalSystemSid;
        case WellKnownSid::kLocalService: return WinLocalServiceSid;
        case WellKnownSid::kNetworkService: return WinNetworkServiceSid;
        case WellKnownSid::kBuiltinAdministrators: return WinBuiltinAdministratorsSid;
        case WellKnownSid::kBuiltinUsers: return WinBuiltinUsersSid;
        case WellKnownSid::kBuiltinGuests: return WinBuiltinGuestsSid;
        case WellKnownSid::kUntrustedLabel: return WinUntrustedLabelSid;
        case WellKnownSid::kLowLabel: return WinLowLabelSid;
        case WellKnownSid::kMediumLabel: return WinMediumLabelSid;
        case WellKnownSid::kHighLabel: return WinHighLabelSid;
        case WellKnownSid::kSystemLabel: return WinSystemLabelSid;
        case WellKnownSid::kWriteRestricted: return WinWriteRestrictedCodeSid;
        case WellKnownSid::kCreatorOwnerRights: return WinCreatorOwnerRightsSid;
        case WellKnownSid::kAllApplicationPackages: return WinBuiltinAnyPackageSid;
...
Optional<Sid>
FromSubAuthorities(PSID_IDENTIFIER_AUTHORITY identifier_authority,
...
Optional<std::vector<Sid>>
FromStringVector(const std::vector<const wchar_t*>& strs, decltype(Sid::FromSddlString)* create_sid)
{
...
Sid::Sid(const void* sid, size_t length) :
    sid_(static_cast<const char*>(sid), static_cast<const char*>(sid) + length)
{
}

Optional<Sid>
Sid::FromKnownCapability(WellKnownCapability capability)
{
    Optional<DWORD> capability_rid = WellKnownCapabilityToRid(capability);
    if (!capability_rid)
        return nullopt;
    SID_IDENTIFIER_AUTHORITY capability_authority = {SECURITY_APP_PACKAGE_AUTHORITY};
    DWORD sub_authorities[] = {SECURITY_CAPABILITY_BASE_RID, *capability_rid};
    return FromSubAuthorities(&capability_authority, size(sub_authorities), sub_authorities);
}

Optional<Sid>
Sid::FromNamedCapability(const wchar_t* capability_name)
{
    ...
    typedef decltype(::DeriveCapabilitySidsFromName)* DeriveCapabilitySidsFromNameFunc;
    ...
    if (!derive_capability_sids(capability_name,
                                &capability_groups,
...
Optional<Sid>
Sid::FromKnownSid(WellKnownSid type)
{
...
Sid::GenerateRandomSid()
{
    ...
    return FromSubAuthorities(&package_authority, _countof(sub_authorities), sub_authorities);
}
...
Optional<Sid>
Sid::FromIntegrityLevel(DWORD integrity_level)
{
    SID_IDENTIFIER_AUTHORITY package_authority = {SECURITY_MANDATORY_LABEL_AUTHORITY};
    return FromSubAuthorities(&package_authority, 1, &integrity_level);
}

Optional<std::vector<Sid>>
Sid::FromSddlStringVector(const std::vector<const wchar_t*>& sddl_sids)
{
    return FromStringVector(sddl_sids, Sid::FromSddlString);
}

Optional<std::vector<Sid>>
Sid::FromNamedCapabilityVector(const std::vector<const wchar_t*>& capability_names)
{
