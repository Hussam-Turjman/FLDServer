...
void*
GetIATFunction(IMAGE_THUNK_DATA* iat_thunk)
{
    if (!iat_thunk)
    {
        NOTREACHED();
...
bool
InterceptEnumCallback(const base::win::PEImage& image,
                      const char* module,
                      DWORD ordinal,
                      const char* name,
                      DWORD hint,
                      ...
                      void* cookie)
{
    ...
    if (name && (0 == lstrcmpiA(name, intercept_information->function_name)))
    {
        ...
        intercept_information->return_code =
                internal::ModifyCode(&(iat->u1.Function),
...
InterceptImportedFunction(HMODULE module_handle,
                          ...
                          IMAGE_THUNK_DATA** iat_thunk)
{
    if (!module_handle || !imported_from_module || !function_name || !new_function)
    {
        NOTREACHED();
                        ...
                        IMAGE_THUNK_DATA* iat_thunk)
{
    ...
    return internal::ModifyCode(
            &(iat_thunk->u1.Function), &original_function, sizeof(original_function));
}
                        ...
                        void* new_function)
{
    ...
    DWORD error = PatchFromModule(module_handle, imported_from_module, function_name, new_function);
    if (NO_ERROR == error)
    {
...
DWORD
IATPatchFunction::PatchFromModule(HMODULE module,
                                  const char* imported_from_module,
                                  const char* function_name,
                                  void* new_function)
{
...
{
    DWORD error = RestoreImportedFunction(intercept_function_, original_function_, iat_thunk_);
    DCHECK_EQ(static_cast<DWORD>(NO_ERROR), error);
...
void*
IATPatchFunction::original_function() const
{
    DCHECK(is_patched());
