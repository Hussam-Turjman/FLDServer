...
using HandleMap = std::unordered_map<HANDLE, ScopedHandleVerifierInfo, HandleHash>;
using NativeLock = base::internal::LockImpl;

NOINLINE void
ReportErrorOnScopedHandleOperation(const base::debug::StackTrace& creation_stack)
{
...
ScopedHandleVerifierInfo::ScopedHandleVerifierInfo(const void* owner,
                                                   const void* pc1,
                                                   const void* pc2,
                                                   std::unique_ptr<debug::StackTrace> stack,
                                                   DWORD thread_id) :
    owner(owner), pc1(pc1), pc2(pc2), stack(std::move(stack)), thread_id(thread_id)
{
}

ScopedHandleVerifierInfo::~ScopedHandleVerifierInfo() = default;

ScopedHandleVerifierInfo::ScopedHandleVerifierInfo(ScopedHandleVerifierInfo&&) noexcept = default;
ScopedHandleVerifierInfo&
ScopedHandleVerifierInfo::operator=(ScopedHandleVerifierInfo&&) noexcept = default;

ScopedHandleVerifier::ScopedHandleVerifier(bool enabled) : enabled_(enabled), lock_(GetLock())
{
}

// static
ScopedHandleVerifier*
ScopedHandleVerifier::Get()
{
...
bool
CloseHandleWrapper(HANDLE handle)
{
...
// static
void
ScopedHandleVerifier::ThreadSafeAssignOrCreateScopedHandleVerifier(
        ScopedHandleVerifier* existing_verifier, bool enabled)
{
    ...
    if (g_active_verifier)
        return;
    g_active_verifier = existing_verifier ? existing_verifier : new ScopedHandleVerifier(enabled);
}
...
ScopedHandleVerifier::InstallVerifier()
{
    ...
    GetHandleVerifierFn get_handle_verifier = reinterpret_cast<GetHandleVerifierFn>(
...
bool
ScopedHandleVerifier::CloseHandle(HANDLE handle)
{
    if (!enabled_)
...
void
ScopedHandleVerifier::StartTracking(HANDLE handle,
                                    const void* owner,
                                    const void* pc1,
                                    const void* pc2)
{
...
void
ScopedHandleVerifier::StopTracking(HANDLE handle,
                                   const void* owner,
                                   const void* pc1,
                                   const void* pc2)
{
...
void
ScopedHandleVerifier::Disable()
{
    enabled_ = false;
}

void
ScopedHandleVerifier::OnHandleBeingClosed(HANDLE handle)
{
    if (enabled_)
...
HMODULE
ScopedHandleVerifier::GetModule() const
{
    return CURRENT_MODULE();
}

NOINLINE void
ScopedHandleVerifier::StartTrackingImpl(HANDLE handle,
                                        const void* owner,
                                        const void* pc1,
                                        const void* pc2)
{
    ...
    std::pair<HandleMap::iterator, bool> result = map_.emplace(
            handle, ScopedHandleVerifierInfo{owner, pc1, pc2, std::move(stacktrace), thread_id});
    if (!result.second)
    {
...
NOINLINE void
ScopedHandleVerifier::StopTrackingImpl(HANDLE handle,
                                       const void* owner,
                                       const void* pc1,
                                       const void* pc2)
{
...
NOINLINE void
ScopedHandleVerifier::OnHandleBeingClosedImpl(HANDLE handle)
{
    if (closing_.Get())
        return;
