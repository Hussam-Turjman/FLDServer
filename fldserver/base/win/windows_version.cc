...
#include "fldserver/base/file_version_info_win.h"
...
constexpr wchar_t kRegKeyWindowsNTCurrentVersion[] = L"SOFTWARE\\Microsoft\\Windows "
                                                     L"NT\\CurrentVersion";
...
OSInfo::GetInstanceStorage()
{
    ...
    static OSInfo* info = []() {
        ...
        DWORD os_type = 0;
        ::GetProductInfo(version_info.dwMajorVersion, version_info.dwMinorVersion, 0, 0, &os_type);
...
OSInfo::OSInfo(const _OSVERSIONINFOEXW& version_info,
               ...
               int os_type) :
    version_(Version::PRE_XP), wow64_status_(GetWOW64StatusForProcess(GetCurrentProcess()))
{
    ...
    if (version_info.dwMajorVersion == 6 || version_info.dwMajorVersion == 10)
    {
        ...
        switch (os_type)
        {
            ...
            case PRODUCT_WEB_SERVER: version_type_ = SUITE_SERVER; break;
            case PRODUCT_PROFESSIONAL:
            case PRODUCT_ULTIMATE: version_type_ = SUITE_PROFESSIONAL; break;
            ...
            case PRODUCT_BUSINESS_N: version_type_ = SUITE_ENTERPRISE; break;
            case PRODUCT_EDUCATION:
            case PRODUCT_EDUCATION_N: version_type_ = SUITE_EDUCATION; break;
            case PRODUCT_HOME_BASIC:
            case PRODUCT_HOME_PREMIUM:
            case PRODUCT_STARTER:
            default: version_type_ = SUITE_HOME; break;
        }
    }
...
OSInfo::VersionNumber
OSInfo::Kernel32VersionNumber() const
{
    DCHECK(Kernel32BaseVersion().components().size() == 4);
    static const VersionNumber version = {.major = static_cast<int>(Kernel32BaseVersion().components()[0]),
                                          .minor = static_cast<int>(Kernel32BaseVersion().components()[1]),
                                          .build = static_cast<int>(Kernel32BaseVersion().components()[2]),
                                          .patch = static_cast<int>(Kernel32BaseVersion().components()[3])};
    return version;
}
...
base::Version
OSInfo::Kernel32BaseVersion() const
{
    static const NoDestructor<base::Version> version([] {
        std::unique_ptr<FileVersionInfoWin> file_version_info =
                FileVersionInfoWin::CreateFileVersionInfoWin(FilePath(FILE_PATH_LITERAL("kernel32."
                                                                                        "dll")));
...
std::string
OSInfo::processor_model_name()
{
    if (processor_model_name_.empty())
    {
        const wchar_t kProcessorNameString[] = L"HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\"
                                               L"0";
...
// static
OSInfo::WOW64Status
OSInfo::GetWOW64StatusForProcess(HANDLE process_handle)
{
    BOOL is_wow64 = FALSE;
...
// static
Version
OSInfo::MajorMinorBuildToVersion(int major, int minor, int build)
{
    if (major == 10)
    {
        if (build >= 19043)
    ...
    if (major == 6)
    {
        switch (minor)
        {
            case 0: return Version::VISTA;
            case 1: return Version::WIN7;
            case 2: return Version::WIN8;
            default: DCHECK_EQ(minor, 3); return Version::WIN8_1;
        }
    }
