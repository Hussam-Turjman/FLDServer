...
class CORE_EXPORT VariantVector final
{
public:
    VariantVector();
    VariantVector(VariantVector&& other);
    VariantVector&
    operator=(VariantVector&& other);
    VariantVector(const VariantVector&) = delete;
    VariantVector&
    operator=(const VariantVector&) = delete;
    ~VariantVector();

    bool
    operator==(const VariantVector& other) const;
    bool
    operator!=(const VariantVector& other) const;
    ...
    void
    Insert(typename internal::VariantUtil<ExpectedVartype>::Type value)
    {
    ...
    template <>
    void
    Insert<VT_DATE>(typename internal::VariantUtil<VT_DATE>::Type value)
    {
        if (vartype_ == VT_EMPTY)
            vartype_ = VT_DATE;
    ...
    int
    Compare(const VARIANT& other, bool ignore_case = false) const;

    // Lexicographical comparison between a VariantVector and a SAFEARRAY.
    int
    Compare(SAFEARRAY* safearray, bool ignore_case = false) const;

    // Lexicographical comparison between two VariantVectors.
    int
    Compare(const VariantVector& other, bool ignore_case = false) const;
...
private:
    ...
    AssertVartype() const
    {
        DCHECK(internal::VariantUtil<ExpectedVartype>::IsConvertibleTo(vartype_))
                << "Type mismatch, " << ExpectedVartype << " is not convertible to " << Type();
    }
