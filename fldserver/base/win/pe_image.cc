...
#include "fldserver/base/win/pe_image.h"

#include "fldserver/base/no_destructor.h"
#include "fldserver/base/win/current_module.h"
...
int
StrCmpByByte(LPCSTR s1, LPCSTR s2)
{
    while (*s1 != '\0' && *s1 == *s2)
    {
        ++s1;
...
bool
ProcessImportChunk(const PEImage& image,
                   LPCSTR module,
                   ...
                   PVOID cookie)
{
    ...
    return image.EnumOneImportChunk(storage.callback, module, name_table, iat, storage.cookie);
}
                        ...
                        PVOID cookie)
{
    ...
    return image.EnumOneDelayImportChunk(
            storage.callback, delay_descriptor, module, name_table, iat, storage.cookie);
}

void
PEImage::set_module(HMODULE module)
{
    module_ = module;
}
...
PEImage::GetNTHeaders() const
{
    ...
    return reinterpret_cast<PIMAGE_NT_HEADERS>(reinterpret_cast<char*>(dos_header)
                                               + dos_header->e_lfanew);
}

PIMAGE_SECTION_HEADER
PEImage::GetSectionHeader(UINT section) const
{
...
DWORD
PEImage::GetImageDirectoryEntrySize(UINT directory) const
{
...
PVOID
PEImage::GetImageDirectoryEntryAddr(UINT directory) const
{
    const IMAGE_DATA_DIRECTORY* const entry = GetDataDirectory(directory);
...
PIMAGE_SECTION_HEADER
PEImage::GetImageSectionFromAddr(PVOID address) const
{
    ...
    for (UINT i = 0; nullptr != (section = GetSectionHeader(i)); i++)
    {
        ...
        PBYTE start = reinterpret_cast<PBYTE>(PEImage::RVAToAddr(section->VirtualAddress));
...
PIMAGE_SECTION_HEADER
PEImage::GetImageSectionHeaderByName(LPCSTR section_name) const
{
    ...
    for (int i = 0; i < num_sections; i++)
    {
        ...
        if (_strnicmp(reinterpret_cast<const char*>(section->Name), section_name, sizeof(section->Name))
            == 0)
        {
            ret = section;
            break;
...
bool
PEImage::GetDebugId(LPGUID guid,
                    LPDWORD age,
                    LPCSTR* pdb_filename,
                    size_t* pdb_filename_length) const
{
    ...
    for (size_t index = 0; index < directory_count; ++index)
    {
        ...
        if (pdb_filename)
        {
            const size_t length_max = entry.SizeOfData - offsetof(PdbInfo, PdbFileName);
            const char* eos = pdb_info->PdbFileName;
            for (const char* const end = pdb_info->PdbFileName + length_max; eos < end && *eos;
                 ++eos)
                ;
...
PDWORD
PEImage::GetExportEntry(LPCSTR name) const
{
    ...
    PDWORD functions = reinterpret_cast<PDWORD>(RVAToAddr(exports->AddressOfFunctions));

    return functions + ordinal - exports->Base;
}

FARPROC
PEImage::GetProcAddress(LPCSTR function_name) const
{
    ...
    PBYTE exports =
            reinterpret_cast<PBYTE>(GetImageDirectoryEntryAddr(IMAGE_DIRECTORY_ENTRY_EXPORT));
...
bool
PEImage::GetProcOrdinal(LPCSTR function_name, WORD* ordinal) const
{
    if (nullptr == ordinal)
        return false;
    ...
    else
    {
        ...
        while (lower != upper)
        {
            ...
            LPCSTR name = static_cast<LPCSTR>(RVAToAddr(*middle));
        ...
        PWORD ordinals = reinterpret_cast<PWORD>(RVAToAddr(exports->AddressOfNameOrdinals));
...
bool
PEImage::EnumSections(EnumSectionsFunction callback, PVOID cookie) const
{
    PIMAGE_NT_HEADERS nt_headers = GetNTHeaders();
...
bool
PEImage::EnumExports(EnumExportsFunction callback, PVOID cookie) const
{
    ...
    PIMAGE_EXPORT_DIRECTORY exports = reinterpret_cast<PIMAGE_EXPORT_DIRECTORY>(directory);
    ...
    for (UINT count = 0; count < num_funcs; count++)
    {
        ...
        for (hint = 0; hint < num_names; hint++)
        {
            ...
            {
                name = static_cast<LPCSTR>(RVAToAddr(names[hint]));
                break;
            }
        }
        ...
        if (reinterpret_cast<char*>(func) >= reinterpret_cast<char*>(directory)
            && reinterpret_cast<char*>(func) <= reinterpret_cast<char*>(directory) + size)
        {
            forward = static_cast<LPCSTR>(func);
            func = nullptr;
        }
...
bool
PEImage::EnumRelocs(EnumRelocsFunction callback, PVOID cookie) const
{
    ...
    PIMAGE_BASE_RELOCATION base = reinterpret_cast<PIMAGE_BASE_RELOCATION>(directory);
    while (size >= sizeof(IMAGE_BASE_RELOCATION) && base->SizeOfBlock && size >= base->SizeOfBlock)
    {
        ...
        base = reinterpret_cast<PIMAGE_BASE_RELOCATION>(reinterpret_cast<char*>(base)
                                                        + base->SizeOfBlock);
    }
...
bool
PEImage::EnumImportChunks(EnumImportChunksFunction callback,
                          PVOID cookie,
                          LPCSTR target_module_name) const
{
    ...
    {
        LPCSTR module_name = static_cast<LPCSTR>(RVAToAddr(import->Name));
        PIMAGE_THUNK_DATA name_table =
                reinterpret_cast<PIMAGE_THUNK_DATA>(RVAToAddr(import->OriginalFirstThunk));
        PIMAGE_THUNK_DATA iat = reinterpret_cast<PIMAGE_THUNK_DATA>(RVAToAddr(import->FirstThunk));

        if (target_module_name == nullptr || (lstrcmpiA(module_name, target_module_name) == 0))
        {
...
bool
PEImage::EnumAllImports(EnumImportsFunction callback, PVOID cookie, LPCSTR target_module_name) const
{
...
bool
PEImage::EnumDelayImportChunks(EnumDelayImportChunksFunction callback,
                               PVOID cookie,
                               LPCSTR target_module_name) const
{
    ...
    for (; delay_descriptor->rvaHmod; delay_descriptor++)
    {
        ...
        if (rvas)
        {
            module_name = static_cast<LPCSTR>(RVAToAddr(delay_descriptor->rvaDLLName));
            name_table = reinterpret_cast<PIMAGE_THUNK_DATA>(RVAToAddr(delay_descriptor->rvaINT));
            iat = reinterpret_cast<PIMAGE_THUNK_DATA>(RVAToAddr(delay_descriptor->rvaIAT));
        }
        else
        {
            ...
            module_name =
                    reinterpret_cast<LPCSTR>(static_cast<uintptr_t>(delay_descriptor->rvaDLLName));
        ...
        if (target_module_name == nullptr || (lstrcmpiA(module_name, target_module_name) == 0))
        {
            ...
            if (!callback(*this, delay_descriptor, module_name, name_table, iat, cookie))
                return false;
        }
    }
...
bool
PEImage::EnumOneDelayImportChunk(EnumImportsFunction callback,
                                 PImgDelayDescr delay_descriptor,
                                 LPCSTR module_name,
                                 PIMAGE_THUNK_DATA name_table,
                                 PIMAGE_THUNK_DATA iat,
                                 PVOID cookie) const
{
    for (; name_table->u1.Ordinal; name_table++, iat++)
    {
        ...
        else
        {
            ...
            {
                import = reinterpret_cast<PIMAGE_IMPORT_BY_NAME>(name_table->u1.ForwarderString);
            }
...
{
    EnumAllImportsStorage temp = {callback, cookie};
    return EnumDelayImportChunks(ProcessDelayImportChunk, &temp, target_module_name);
}

bool
PEImage::VerifyMagic() const
{
    PIMAGE_DOS_HEADER dos_header = GetDosHeader();
...
bool
PEImage::ImageRVAToOnDiskOffset(uintptr_t rva, DWORD* on_disk_offset) const
{
...
bool
PEImage::ImageAddrToOnDiskOffset(LPVOID address, DWORD* on_disk_offset) const
{
    if (nullptr == address)
        return false;
    ...
    DWORD offset_within_section = static_cast<DWORD>(reinterpret_cast<uintptr_t>(address))
...
PVOID
PEImage::RVAToAddr(uintptr_t rva) const
{
    if (rva == 0)
        return nullptr;
...
PEImage::GetDataDirectory(UINT directory) const
{
    PIMAGE_NT_HEADERS nt_headers = GetNTHeaders();
...
PVOID
PEImageAsData::RVAToAddr(uintptr_t rva) const
{
    if (rva == 0)
        return nullptr;
