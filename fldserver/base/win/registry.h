...
public:
    ...
    RegKey&
    operator=(RegKey&& other);
    ~RegKey();

    LONG
    Create(HKEY rootkey, const wchar_t* subkey, REGSAM access);

    LONG
    CreateWithDisposition(HKEY rootkey, const wchar_t* subkey, DWORD* disposition, REGSAM access);

    // Creates a subkey or open it if it already exists.
    LONG
    CreateKey(const wchar_t* name, REGSAM access);

    // Opens an existing reg key.
    LONG
    Open(HKEY rootkey, const wchar_t* subkey, REGSAM access);

    // Opens an existing reg key, given the relative key name.
    LONG
    OpenKey(const wchar_t* relative_key_name, REGSAM access);
    ...
    bool
    HasValue(const wchar_t* value_name) const;
    ...
    LONG
    GetValueNameAt(int index, std::wstring* name) const;
    ...
    LONG
    ReadValueDW(const wchar_t* name, DWORD* out_value) const;

    // Reads a REG_QWORD (int64_t) into |out_value|. If |name| is null or empty,
    // reads the key's default value, if any.
    LONG
    ReadInt64(const wchar_t* name, int64_t* out_value) const;

    // Reads a string into |out_value|. If |name| is null or empty, reads
    // the key's default value, if any.
    LONG
    ReadValue(const wchar_t* name, std::wstring* out_value) const;
    ...
    LONG
    ReadValues(const wchar_t* name, std::vector<std::wstring>* values);

    // Reads raw data into |data|. If |name| is null or empty, reads the key's
    // default value, if any.
    LONG
    ReadValue(const wchar_t* name, void* data, DWORD* dsize, DWORD* dtype) const;

    // Setters:

    // Sets an int32_t value.
    LONG
    WriteValue(const wchar_t* name, DWORD in_value);

    // Sets a string value.
    LONG
    WriteValue(const wchar_t* name, const wchar_t* in_value);

    // Sets raw data, including type.
    LONG
    WriteValue(const wchar_t* name, const void* data, DWORD dsize, DWORD dtype);
    ...
    bool
    StartWatching(ChangeCallback callback);
...
private:
    ...
    static LONG
    RegDelRecurse(HKEY root_key, const wchar_t* name, REGSAM access);
...
class CORE_EXPORT RegistryValueIterator
{
public:
    ...
    RegistryValueIterator(HKEY root_key, const wchar_t* folder_key, REGSAM wow64access);
...
private:
    ...
    void
    Initialize(HKEY root_key, const wchar_t* folder_key, REGSAM wow64access);
...
class CORE_EXPORT RegistryKeyIterator
{
public:
