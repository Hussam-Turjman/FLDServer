...
bool
StartupInformation::InitializeProcThreadAttributeList(DWORD attribute_count)
{
    ...
    auto* attribute_list_ptr = reinterpret_cast<LPPROC_THREAD_ATTRIBUTE_LIST>(attribute_list.get());
    if (!::InitializeProcThreadAttributeList(attribute_list_ptr, attribute_count, 0, &size))
    {
        return false;
    }
...
bool
StartupInformation::UpdateProcThreadAttribute(DWORD_PTR attribute, void* value, size_t size)
{
    if (!startup_info_.lpAttributeList)
        return false;
    return !!::UpdateProcThreadAttribute(
            startup_info_.lpAttributeList, 0, attribute, value, size, nullptr, nullptr);
}
