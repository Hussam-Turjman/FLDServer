...
#include "fldserver/base/win/security_util.h"
...
{
bool
GrantAccessToPath(const FilePath& path,
                  ...
                  DWORD inheritance)
{
    ...
    DWORD error = ::GetNamedSecurityInfo(object_name.c_str(),
                                         SE_FILE_OBJECT,
    ...
    error = ::SetEntriesInAcl(access_entries.size(), access_entries.data(), dacl, &new_dacl);
    if (error != ERROR_SUCCESS)
    {
        ::SetLastError(error);
    ...
    error = ::SetNamedSecurityInfo(&object_name[0],
                                   SE_FILE_OBJECT,
                                   DACL_SECURITY_INFORMATION,
                                   nullptr,
                                   nullptr,
