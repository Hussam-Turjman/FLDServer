...
#include "fldserver/base/win/wrapped_window_proc.h"
...
static LazyInstance<MessageWindow::WindowClass>::DestructorAtExit g_window_class =
...
MessageWindow::WindowClass::WindowClass()
{
    ...
    if (atom_ == 0)
    {
        PLOG(ERROR) << "Failed to register the window class for a message-only window";
    }
}
...
bool
MessageWindow::Create(MessageCallback message_callback)
{
    return DoCreate(std::move(message_callback), nullptr);
}

bool
MessageWindow::CreateNamed(MessageCallback message_callback, const std::wstring& window_name)
{
...
// static
HWND
MessageWindow::FindWindow(const std::wstring& window_name)
{
    return FindWindowEx(HWND_MESSAGE, nullptr, kMessageWindowClassName, window_name.c_str());
}

bool
MessageWindow::DoCreate(MessageCallback message_callback, const wchar_t* window_name)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    window_ = CreateWindow(MAKEINTATOM(window_class.atom()),
                           window_name,
                           0,
                           0,
                           0,
...
// static
LRESULT CALLBACK
MessageWindow::WindowProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    MessageWindow* self = reinterpret_cast<MessageWindow*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
