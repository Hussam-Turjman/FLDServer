...
LoadComBaseFunction(const char* function_name)
{
    static HMODULE const handle = []() {
        ...
        return ::LoadLibraryEx(L"combase.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
    }();
    return handle ? ::GetProcAddress(handle, function_name) : nullptr;
}

decltype(&::RoInitialize)
GetRoInitializeFunction()
{
...
HRESULT
CallRoInitialize(RO_INIT_TYPE init_type)
{
...
ScopedWinrtInitializer::ScopedWinrtInitializer() : hr_(CallRoInitialize(RO_INIT_MULTITHREADED))
{
...
bool
ScopedWinrtInitializer::Succeeded() const
{
