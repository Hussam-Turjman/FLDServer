...
bool
GetPreferredUILanguageList(GetPreferredUILanguages_Fn function,
                           ULONG flags,
                           std::vector<std::wstring>* languages)
{
    ...
    if (!function(call_flags, &language_count, base::data(buffer), &buffer_length)
        || !language_count)
    {
    ...
    for (auto token : base::SplitStringPiece(
                 buffer, kNullTerminator, base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY))
    {
...
{
bool
GetUserPreferredUILanguageList(std::vector<std::wstring>* languages)
{
    DCHECK(languages);
...
bool
GetThreadPreferredUILanguageList(std::vector<std::wstring>* languages)
{
    DCHECK(languages);
    return GetPreferredUILanguageList(::GetThreadPreferredUILanguages,
