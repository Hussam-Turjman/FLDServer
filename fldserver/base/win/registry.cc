...
public:
    ...
    bool
    StartWatching(HKEY key, ChangeCallback callback);

    // ObjectWatcher::Delegate:
    void
    OnObjectSignaled(HANDLE object) override
    {
...
bool
RegKey::Watcher::StartWatching(HKEY key, ChangeCallback callback)
{
    DCHECK(key);
    ...
    LONG result = RegNotifyChangeKeyValue(
            key, /*bWatchSubtree=*/TRUE, filter, watch_event_.Get(), /*fAsynchronous=*/TRUE);
...
RegKey::RegKey(RegKey&& other) noexcept :
    key_(other.key_), wow64access_(other.wow64access_), key_watcher_(std::move(other.key_watcher_))
{
    other.key_ = nullptr;
...
LONG
RegKey::Create(HKEY rootkey, const wchar_t* subkey, REGSAM access)
{
...
LONG
RegKey::CreateKey(const wchar_t* name, REGSAM access)
{
    DCHECK(name && access);
    ...
    LONG result = RegCreateKeyEx(
            key_, name, 0, nullptr, REG_OPTION_NON_VOLATILE, access, nullptr, &subkey, nullptr);
    if (result == ERROR_SUCCESS)
    {
        Close();
        key_ = subkey;
...
LONG
RegKey::Open(HKEY rootkey, const wchar_t* subkey, REGSAM access)
{
...
LONG
RegKey::OpenKey(const wchar_t* relative_key_name, REGSAM access)
{
...
bool
RegKey::HasValue(const wchar_t* name) const
{
    return RegQueryValueEx(key_, name, nullptr, nullptr, nullptr, nullptr) == ERROR_SUCCESS;
}
...
{
    DWORD count = 0;
    LONG result = RegQueryInfoKey(key_,
                                  nullptr,
                                  nullptr,
                                  nullptr,
                                  nullptr,
...
LONG
RegKey::GetValueNameAt(int index, std::wstring* name) const
{
    wchar_t buf[256];
    DWORD bufsize = size(buf);
    LONG r = ::RegEnumValue(key_, index, buf, &bufsize, nullptr, nullptr, nullptr, nullptr);
    if (r == ERROR_SUCCESS)
...
RegKey::DeleteEmptyKey(const wchar_t* name)
{
    ...
    LONG result = RegOpenKeyEx(key_, name, 0, KEY_READ | wow64access_, &target_key);

    if (result != ERROR_SUCCESS)
        return result;

    DWORD count = 0;
    result = RegQueryInfoKey(target_key,
                             nullptr,
                             nullptr,
                             nullptr,
                             nullptr,
...
LONG
RegKey::ReadValueDW(const wchar_t* name, DWORD* out_value) const
{
    DCHECK(out_value);
...
LONG
RegKey::ReadInt64(const wchar_t* name, int64_t* out_value) const
{
    DCHECK(out_value);
...
LONG
RegKey::ReadValue(const wchar_t* name, std::wstring* out_value) const
{
    DCHECK(out_value);
...
LONG
RegKey::ReadValue(const wchar_t* name, void* data, DWORD* dsize, DWORD* dtype) const
{
    LONG result =
            RegQueryValueEx(key_, name, nullptr, dtype, reinterpret_cast<LPBYTE>(data), dsize);
    return result;
}

LONG
RegKey::ReadValues(const wchar_t* name, std::vector<std::wstring>* values)
{
    values->clear();
...
LONG
RegKey::WriteValue(const wchar_t* name, DWORD in_value)
{
    return WriteValue(name, &in_value, static_cast<DWORD>(sizeof(in_value)), REG_DWORD);
}

LONG
RegKey::WriteValue(const wchar_t* name, const wchar_t* in_value)
{
...
LONG
RegKey::WriteValue(const wchar_t* name, const void* data, DWORD dsize, DWORD dtype)
{
    DCHECK(data || !dsize);
...
bool
RegKey::StartWatching(ChangeCallback callback)
{
    if (!key_watcher_)
...
// static
LONG
RegKey::RegDelRecurse(HKEY root_key, const wchar_t* name, REGSAM access)
{
    ...
    HKEY target_key = nullptr;
    result = RegOpenKeyEx(root_key, name, 0, KEY_ENUMERATE_SUB_KEYS | access, &target_key);
...
void
RegistryValueIterator::Initialize(HKEY root_key, const wchar_t* folder_key, REGSAM wow64access)
{
...
bool
RegistryValueIterator::Valid() const
{
    return key_ != nullptr && index_ >= 0;
}

void
RegistryValueIterator::operator++()
{
    --index_;
    Read();
}
...
RegistryValueIterator::Read()
{
    if (Valid())
    {
        ...
        LONG result = ::RegEnumValue(key_,
                                     index_,
                                     WriteInto(&name_, name_size),
                                     &name_size,
                                     nullptr,
                                     &type_,
...
bool
RegistryKeyIterator::Valid() const
{
    return key_ != nullptr && index_ >= 0;
}
...
bool
RegistryKeyIterator::Read()
{
    if (Valid())
    {
        DWORD ncount = static_cast<DWORD>(size(name_));
        FILETIME written;
        LONG r = ::RegEnumKeyEx(key_, index_, name_, &ncount, nullptr, nullptr, nullptr, &written);
        if (ERROR_SUCCESS == r)
            return true;
    }
...
void
RegistryKeyIterator::Initialize(HKEY root_key, const wchar_t* folder_key, REGSAM wow64access)
{
    DCHECK_EQ(wow64access & ~kWow64AccessMask, static_cast<REGSAM>(0));
    LONG result = RegOpenKeyEx(root_key, folder_key, 0, KEY_READ | wow64access, &key_);
    if (result != ERROR_SUCCESS)
