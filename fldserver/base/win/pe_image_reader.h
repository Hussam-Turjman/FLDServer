...
class CORE_EXPORT PeImageReader
{
public:
    enum WordSize
    {
    ...
    bool
    Initialize(const uint8_t* image_data, size_t image_size);
    ...
    const uint8_t*
    GetOptionalHeaderData(size_t* optional_data_size);
    size_t
    GetNumberOfSections();
    ...
    const uint8_t*
    GetExportSection(size_t* section_size);

    size_t
    GetNumberOfDebugEntries();
    const IMAGE_DEBUG_DIRECTORY*
    GetDebugEntry(size_t index, const uint8_t** raw_data, size_t* raw_data_size);
    ...
    bool
    EnumCertificates(EnumCertificatesCallback callback, void* context);
...
private:
    ...
    public:
        ...
        virtual const IMAGE_DATA_DIRECTORY*
    ...
    const uint8_t*
    GetImageData(size_t index, size_t* data_length);
    ...
    template <typename T>
    bool
    GetStructureAt(size_t offset, const T** structure)
    {
    ...
    template <typename T>
    bool
    GetStructureAt(size_t offset, size_t structure_size, const T** structure)
    {
