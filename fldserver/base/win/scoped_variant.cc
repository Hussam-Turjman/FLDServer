...
#include "fldserver/base/win/variant_util.h"
...
void
ScopedVariant::Reset(const VARIANT& var)
{
    if (&var != &var_)
    {
        ::VariantClear(&var_);
...
void
ScopedVariant::Swap(ScopedVariant& var)
{
    VARIANT tmp = var_;
    var_ = var.var_;
    var.var_ = tmp;
...
int
ScopedVariant::Compare(const VARIANT& other, bool ignore_case) const
{
    DCHECK(!V_ISARRAY(&var_)) << "Comparison is not supported when |this| owns a SAFEARRAY";
    DCHECK(!V_ISARRAY(&other)) << "Comparison is not supported when |other| owns a SAFEARRAY";
    ...
    const bool other_is_unknown = other.vt == VT_UNKNOWN || other.vt == VT_DISPATCH;
    ...
    if (shared_vartype == VT_BSTR)
    {
        ...
        HRESULT hr = ::VarBstrCmp(V_BSTR(&var_), V_BSTR(&other), LOCALE_USER_DEFAULT, flags);
...
void
ScopedVariant::Set(const wchar_t* str)
{
...
void
ScopedVariant::Set(uint16_t ui16)
{
    DCHECK(!IsLeakableVarType(var_.vt)) << "leaking variant: " << var_.vt;
    var_.vt = VT_UI2;
    var_.uiVal = ui16;
...
void
ScopedVariant::Set(uint32_t ui32)
{
    DCHECK(!IsLeakableVarType(var_.vt)) << "leaking variant: " << var_.vt;
    var_.vt = VT_UI4;
    var_.ulVal = ui32;
...
void
ScopedVariant::Set(uint64_t ui64)
{
    DCHECK(!IsLeakableVarType(var_.vt)) << "leaking variant: " << var_.vt;
    var_.vt = VT_UI8;
    var_.ullVal = ui64;
...
void
ScopedVariant::SetDate(DATE date)
{
    DCHECK(!IsLeakableVarType(var_.vt)) << "leaking variant: " << var_.vt;
    var_.vt = VT_DATE;
    var_.date = date;
...
void
ScopedVariant::Set(IDispatch* disp)
{
    DCHECK(!IsLeakableVarType(var_.vt)) << "leaking variant: " << var_.vt;
    var_.vt = VT_DISPATCH;
    var_.pdispVal = disp;
...
void
ScopedVariant::Set(IUnknown* unk)
{
    DCHECK(!IsLeakableVarType(var_.vt)) << "leaking variant: " << var_.vt;
    var_.vt = VT_UNKNOWN;
    var_.punkVal = unk;
...
void
ScopedVariant::Set(SAFEARRAY* array)
{
    DCHECK(!IsLeakableVarType(var_.vt)) << "leaking variant: " << var_.vt;
...
void
ScopedVariant::Set(const VARIANT& var)
{
    DCHECK(!IsLeakableVarType(var_.vt)) << "leaking variant: " << var_.vt;
...
bool
ScopedVariant::IsLeakableVarType(VARTYPE vt)
{
    bool leakable = false;
    switch (vt & VT_TYPEMASK)
    {
        ...
        case VT_BSTR_BLOB: leakable = true; break;
    }
