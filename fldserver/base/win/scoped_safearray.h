...
class CORE_EXPORT ScopedSafearray
{
public:
    ...
    class CORE_EXPORT LockScope final
    {
    public:
        ...
        LockScope<ElementVartype>&
        operator=(LockScope<ElementVartype>&& other)
        {
    ...
    private:
        LockScope(SAFEARRAY* safearray, VARTYPE vartype, pointer array, size_t array_size) :
            safearray_(safearray), vartype_(vartype), array_(array), array_size_(array_size)
        {
        }
    ...
    explicit ScopedSafearray(SAFEARRAY* safearray = nullptr) : safearray_(safearray)
    {
    }
    ...
    ScopedSafearray&
    operator=(ScopedSafearray&& rvalue)
    {
    ...
    template <VARTYPE ElementVartype>
    base::Optional<LockScope<ElementVartype>>
    CreateLockScope() const
    {
        ...
        if (FAILED(hr) || !internal::VariantUtil<ElementVartype>::IsConvertibleTo(vartype))
        {
            return base::nullopt;
        }
    ...
    // nullptr.
    void
    Reset(SAFEARRAY* safearray = nullptr)
    {
    ...
    size_t
    GetCount(UINT dimension = 0) const
    {
        DCHECK(safearray_);
    ...
    bool
    operator==(const ScopedSafearray& safearray2) const = delete;
    bool
    operator!=(const ScopedSafearray& safearray2) const = delete;
