...
#include "fldserver/base/win/embedded_i18n/language_selector.h"
...
#include "fldserver/base/win/i18n.h"
...
bool
IsArraySortedAndLowerCased(span<const LangToOffset> languages_to_offset)
{
    return std::is_sorted(languages_to_offset.begin(), languages_to_offset.end())
           && std::all_of(
                   languages_to_offset.begin(), languages_to_offset.end(), [](const auto& lang) {
...
AvailableLanguageAliases
DetermineAvailableAliases(span<const LangToOffset> languages_to_offset)
{
...
bool
GetExactLanguageOffset(span<const LangToOffset> languages_to_offset,
                       ...
                       const LangToOffset** matched_language_to_offset)
{
    ...
    auto search_result =
            std::lower_bound(languages_to_offset.begin(),
                             languages_to_offset.end(),
                             language,
                             [](const LangToOffset& left, const std::wstring& to_find) {
                                 return left.first < to_find;
                             });
    if (languages_to_offset.end() != search_result && search_result->first == language)
    {
...
bool
GetAliasedLanguageOffset(const AvailableLanguageAliases& available_aliases,
                         const std::wstring& language,
                         const LangToOffset** matched_language_to_offset)
{
    DCHECK(matched_language_to_offset);
...
// language.
bool
GetCompatibleNeutralLanguageOffset(const AvailableLanguageAliases& available_aliases,
         ...
         std::wstring* matched_name)
{
    ...
    {
        std::wstring lower_case_candidate = AsWString(ToLowerASCII(AsStringPiece16(scan)));
        if (GetExactLanguageOffset(
                    languages_to_offset, lower_case_candidate, matched_language_to_offset)
            || GetAliasedLanguageOffset(
                    available_aliases, lower_case_candidate, matched_language_to_offset))
        {
    ...
    for (const std::wstring& scan : candidates)
    {
        ...
        if (GetCompatibleNeutralLanguageOffset(
                    available_aliases, neutral_language, matched_language_to_offset))
        {
            matched_name->assign(scan);
            return true;
        }
...
void
SelectLanguageMatchingCandidate(const std::vector<std::wstring>& candidates,
                                span<const LangToOffset> languages_to_offset,
                                int* selected_offset,
                                ...
                                std::wstring* selected_language)
{
    ...
    // otherwise.
#if DCHECK_IS_ON()
    DCHECK(IsArraySortedAndLowerCased(languages_to_offset)) << "languages_to_offset is not sorted "
                                                               "and lower cased";
#endif  // DCHECK_IS_ON()

    // Get which languages that are commonly used as aliases and wildcards are
    // available for use to match candidates.
    AvailableLanguageAliases available_aliases = DetermineAvailableAliases(languages_to_offset);
    ...
    {
        matched_language_to_offset = available_aliases.en_us_language_offset;
        *matched_candidate = std::wstring(available_aliases.en_us_language_offset->first);
    }
...
GetCandidatesFromSystem(WStringPiece preferred_language)
{
...
LanguageSelector::LanguageSelector(WStringPiece preferred_language,
                                   span<const LangToOffset> languages_to_offset) :
    LanguageSelector(GetCandidatesFromSystem(preferred_language), languages_to_offset)
{
}
...
{
    SelectLanguageMatchingCandidate(candidates,
                                    languages_to_offset,
                                    &selected_offset_,
                                    &matched_candidate_,
                                    &selected_language_);
