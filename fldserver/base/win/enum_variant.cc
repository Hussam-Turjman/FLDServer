...
#include "fldserver/base/win/enum_variant.h"
...
VARIANT*
EnumVariant::ItemAt(ULONG index)
{
...
HRESULT
EnumVariant::Next(ULONG requested_count, VARIANT* out_elements, ULONG* out_elements_received)
{
    if (!out_elements)
        return E_INVALIDARG;
...
EnumVariant::Clone(IEnumVARIANT** out_cloned_object)
{
    ...
    Microsoft::WRL::ComPtr<EnumVariant> other = Microsoft::WRL::Make<EnumVariant>(ULONG(count));
