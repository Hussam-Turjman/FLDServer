...
FARPROC
LoadComBaseFunction(const char* function_name)
{
...
decltype(&::WindowsCreateString)
GetWindowsCreateString()
{
    static decltype(&::WindowsCreateString) const function =
            reinterpret_cast<decltype(&::WindowsCreateString)>(LoadComBaseFunction("WindowsCreateSt"
                                                                                   "ring"));
    return function;
}
...
GetWindowsDeleteString()
{
    static decltype(&::WindowsDeleteString) const function =
            reinterpret_cast<decltype(&::WindowsDeleteString)>(LoadComBaseFunction("WindowsDeleteSt"
                                                                                   "ring"));
    return function;
}

decltype(&::WindowsGetStringRawBuffer)
GetWindowsGetStringRawBuffer()
{
    static decltype(&::WindowsGetStringRawBuffer) const function =
            reinterpret_cast<decltype(&::WindowsGetStringRawBuffer)>(LoadComBaseFunction("WindowsGe"
                                                                                         "tStringRa"
                                                                                         "wBuffe"
                                                                                         "r"));
    return function;
...
HRESULT
WindowsCreateString(const wchar_t* src, uint32_t len, HSTRING* out_hstr)
{
...
HRESULT
WindowsDeleteString(HSTRING hstr)
{
...
const wchar_t*
WindowsGetStringRawBuffer(HSTRING hstr, uint32_t* out_len)
{
...
// static
void
ScopedHStringTraits::Free(HSTRING hstr)
{
...
// static
ScopedHString
ScopedHString::Create(WStringPiece str)
{
    DCHECK(g_load_succeeded);
    HSTRING hstr;
    HRESULT hr = base::WindowsCreateString(str.data(), checked_cast<UINT32>(str.length()), &hstr);
    if (SUCCEEDED(hr))
...
// static
ScopedHString
ScopedHString::Create(StringPiece str)
{
