...
#include "fldserver/base/win/pe_image_reader.h"
...
public:
    ...
    explicit OptionalHeaderImpl(const uint8_t* optional_header_start) :
        optional_header_(reinterpret_cast<const OPTIONAL_HEADER_TYPE*>(optional_header_start))
    {
    }
    ...
    size_t
    GetDataDirectoryOffset() override
    {
    ...
    DWORD
    GetSizeOfImage() override
    {
        return optional_header_->SizeOfImage;
    }
...
bool
PeImageReader::Initialize(const uint8_t* image_data, size_t image_size)
{
    image_data_ = image_data;
    image_size_ = image_size;

    if (!ValidateDosHeader() || !ValidatePeSignature() || !ValidateCoffFileHeader()
        || !ValidateOptionalHeader() || !ValidateSectionHeaders())
    {
        Clear();
        return false;
    }
...
PeImageReader::GetCoffFileHeader()
{
    ...
    return reinterpret_cast<const IMAGE_FILE_HEADER*>(image_data_ + GetDosHeader()->e_lfanew
                                                      + sizeof(DWORD));
}

const uint8_t*
PeImageReader::GetOptionalHeaderData(size_t* optional_header_size)
{
...
size_t
PeImageReader::GetNumberOfSections()
{
...
PeImageReader::GetSectionHeaderAt(size_t index)
{
    ...
    return reinterpret_cast<const IMAGE_SECTION_HEADER*>(GetOptionalHeaderStart()
                                                         + GetOptionalHeaderSize()
                                                         + (sizeof(IMAGE_SECTION_HEADER) * index));
}

const uint8_t*
PeImageReader::GetExportSection(size_t* section_size)
{
    size_t data_size = 0;
...
size_t
PeImageReader::GetNumberOfDebugEntries()
{
    size_t data_size = 0;
...
PeImageReader::GetDebugEntry(size_t index, const uint8_t** raw_data, size_t* raw_data_size)
{
...
bool
PeImageReader::EnumCertificates(EnumCertificatesCallback callback, void* context)
{
    size_t data_size = 0;
    const uint8_t* data = GetImageData(IMAGE_DIRECTORY_ENTRY_SECURITY, &data_size);
    if (!data)
        return false;  // Certificate table is out of bounds.
    const size_t kWinCertificateSize = offsetof(WIN_CERTIFICATE, bCertificate);
    while (data_size)
    {
        const WIN_CERTIFICATE* win_certificate = reinterpret_cast<const WIN_CERTIFICATE*>(data);
        if (kWinCertificateSize > data_size || kWinCertificateSize > win_certificate->dwLength
...
bool
PeImageReader::ValidateDosHeader()
{
    const IMAGE_DOS_HEADER* dos_header = nullptr;
    if (!GetStructureAt(0, &dos_header) || dos_header->e_magic != IMAGE_DOS_SIGNATURE
...
bool
PeImageReader::ValidatePeSignature()
{
    const DWORD* signature = nullptr;
    if (!GetStructureAt(GetDosHeader()->e_lfanew, &signature) || *signature != IMAGE_NT_SIGNATURE)
    {
        return false;
    }
...
bool
PeImageReader::ValidateCoffFileHeader()
{
    DCHECK_NE((validation_state_ & VALID_PE_SIGNATURE), 0U);
    const IMAGE_FILE_HEADER* file_header = nullptr;
    if (!GetStructureAt(GetDosHeader()->e_lfanew + offsetof(IMAGE_NT_HEADERS32, FileHeader),
                        &file_header))
    {
        return false;
    }
...
bool
PeImageReader::ValidateOptionalHeader()
{
    ...
    {
        optional_header = std::make_unique<OptionalHeaderImpl<IMAGE_OPTIONAL_HEADER32>>(
                image_data_ + optional_header_offset);
    }
    else if (*optional_header_magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC)
    {
        optional_header = std::make_unique<OptionalHeaderImpl<IMAGE_OPTIONAL_HEADER64>>(
                image_data_ + optional_header_offset);
    }
    else
    {
...
bool
PeImageReader::ValidateSectionHeaders()
{
    const uint8_t* first_section_header = GetOptionalHeaderStart() + GetOptionalHeaderSize();
...
const uint8_t*
PeImageReader::GetOptionalHeaderStart()
{
    DCHECK_NE((validation_state_ & VALID_OPTIONAL_HEADER), 0U);
    return (image_data_ + GetDosHeader()->e_lfanew + offsetof(IMAGE_NT_HEADERS32, OptionalHeader));
}

size_t
PeImageReader::GetOptionalHeaderSize()
{
    return GetCoffFileHeader()->SizeOfOptionalHeader;
}

const IMAGE_DATA_DIRECTORY*
PeImageReader::GetDataDirectoryEntryAt(size_t index)
{
    DCHECK_NE((validation_state_ & VALID_OPTIONAL_HEADER), 0U);
...
const IMAGE_SECTION_HEADER*
PeImageReader::FindSectionFromRva(uint32_t relative_address)
{
    const size_t number_of_sections = GetNumberOfSections();
    for (size_t i = 0; i < number_of_sections; ++i)
    {
        ...
        if (!GetStructureAt(
                    section_header->PointerToRawData, section_header->SizeOfRawData, &section_data))
        {
            continue;
        }
...
const uint8_t*
PeImageReader::GetImageData(size_t index, size_t* data_length)
{
    ...
    const IMAGE_SECTION_HEADER* header = FindSectionFromRva(entry->VirtualAddress);
    if (!header)
        return nullptr;
    ...
    if (data_offset >= header->SizeOfRawData || header->SizeOfRawData - data_offset < entry->Size)
    {
        return nullptr;
    }
