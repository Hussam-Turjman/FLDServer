...
#include "fldserver/base/win/event_trace_controller.h"
...
EtwTraceProperties::SetLoggerFileName(const wchar_t* logger_file_name)
{
    ...
    memcpy(buffer_ + get()->LogFileNameOffset, logger_file_name, sizeof(wchar_t) * len);
    return S_OK;
}
...
HRESULT
EtwTraceController::Start(const wchar_t* session_name, EtwTraceProperties* prop)
{
...
HRESULT
EtwTraceController::StartRealtimeSession(const wchar_t* session_name, size_t buffer_size)
{
    DCHECK(NULL == session_ && session_name_.empty());
...
HRESULT
EtwTraceController::EnableProvider(REFGUID provider, UCHAR level, ULONG flags)
{
...
HRESULT
EtwTraceController::Stop(EtwTraceProperties* properties)
{
    EtwTraceProperties ignore;
    if (properties == nullptr)
        properties = &ignore;

    ULONG error = ::ControlTrace(session_, nullptr, properties->get(), EVENT_TRACE_CONTROL_STOP);
...
HRESULT
EtwTraceController::Flush(EtwTraceProperties* properties)
{
    EtwTraceProperties ignore;
    if (properties == nullptr)
        properties = &ignore;

    ULONG error = ::ControlTrace(session_, nullptr, properties->get(), EVENT_TRACE_CONTROL_FLUSH);
    if (ERROR_SUCCESS != error)
        return HRESULT_FROM_WIN32(error);
...
HRESULT
EtwTraceController::Query(const wchar_t* session_name, EtwTraceProperties* properties)
{
    ULONG err = ::ControlTrace(NULL, session_name, properties->get(), EVENT_TRACE_CONTROL_QUERY);
    return HRESULT_FROM_WIN32(err);
}

HRESULT
EtwTraceController::Update(const wchar_t* session_name, EtwTraceProperties* properties)
{
    DCHECK(properties != nullptr);
    ULONG err = ::ControlTrace(NULL, session_name, properties->get(), EVENT_TRACE_CONTROL_UPDATE);
    return HRESULT_FROM_WIN32(err);
}

HRESULT
EtwTraceController::Stop(const wchar_t* session_name, EtwTraceProperties* properties)
{
    DCHECK(properties != nullptr);
    ULONG err = ::ControlTrace(NULL, session_name, properties->get(), EVENT_TRACE_CONTROL_STOP);
    return HRESULT_FROM_WIN32(err);
}

HRESULT
EtwTraceController::Flush(const wchar_t* session_name, EtwTraceProperties* properties)
{
    DCHECK(properties != nullptr);
    ULONG err = ::ControlTrace(NULL, session_name, properties->get(), EVENT_TRACE_CONTROL_FLUSH);
    return HRESULT_FROM_WIN32(err);
}
