...
{
BSTR
AllocBstrOrDie(WStringPiece non_bstr)
{
    BSTR result = ::SysAllocStringLen(non_bstr.data(), checked_cast<UINT>(non_bstr.length()));
    if (!result)
    {
        base::TerminateBecauseOutOfMemory((non_bstr.length() + 1) * sizeof(wchar_t));
    }
    return result;
}

BSTR
AllocBstrBytesOrDie(size_t bytes)
{
...
ScopedBstr::ScopedBstr(WStringPiece non_bstr) : bstr_(AllocBstrOrDie(non_bstr))
{
}
...
void
ScopedBstr::Swap(ScopedBstr& bstr2)
{
    BSTR tmp = bstr_;
    bstr_ = bstr2.bstr_;
    bstr2.bstr_ = tmp;
...
BSTR
ScopedBstr::Allocate(WStringPiece str)
{
...
void
ScopedBstr::SetByteLen(size_t bytes)
{
    DCHECK(bstr_);
