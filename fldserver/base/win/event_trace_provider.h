...
public:
    ...
    EtwMofEvent(const EtwEventClass& event_class, EtwEventType type, EtwEventLevel level)
    {
        memset(static_cast<Super*>(this), 0, sizeof(Super));
    ...
    void
    SetField(size_t field, size_t size, const void* data)
    {
        // DCHECK(field < N);
...
// messages themselves.
class CORE_EXPORT EtwTraceProvider
        {
                public:
                ...
                // Accessors.
                void
                set_provider_name(const GUID& provider_name)
                {
                ...
                TRACEHANDLE
                registration_handle() const
                {
                    return registration_handle_;
                }
                TRACEHANDLE
                session_handle() const
                {
                ...
                bool
                ShouldLog(EtwEventLevel level, EtwEventFlags flags)
                {
                ...
                ULONG
                Log(const EtwEventClass& event_class,
                EtwEventType type,
                EtwEventLevel level,
                const char* message);
                ULONG
                ...
                DisableEvents();
                ULONG
                Callback(WMIDPREQUESTCODE request, PVOID buffer);
                static ULONG WINAPI
                ControlCallback(WMIDPREQUESTCODE request, PVOID context, ULONG* reserved, PVOID buffer);
