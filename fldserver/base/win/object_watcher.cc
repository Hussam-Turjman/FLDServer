...
bool
ObjectWatcher::StartWatchingOnce(HANDLE object, Delegate* delegate, const Location& from_here)
{
    return StartWatchingInternal(object, delegate, true, from_here);
}

bool
ObjectWatcher::StartWatchingMultipleTimes(HANDLE object,
                                          Delegate* delegate,
...
bool
ObjectWatcher::IsWatching() const
{
...
// static
void CALLBACK
ObjectWatcher::DoneWaiting(void* param, BOOLEAN timed_out)
{
    DCHECK(!timed_out);
...
bool
ObjectWatcher::StartWatchingInternal(HANDLE object,
                                     Delegate* delegate,
                                     ...
                                     const Location& from_here)
{
    ...
    callback_ = BindRepeating(&ObjectWatcher::Signal, weak_factory_.GetWeakPtr(), delegate);
    object_ = object;

    if (!RegisterWaitForSingleObject(
                &wait_object_, object, DoneWaiting, this, INFINITE, wait_flags))
    {
...
void
ObjectWatcher::Signal(Delegate* delegate)
{
