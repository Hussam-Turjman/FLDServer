...
public:
    BindPostTaskTrampoline(scoped_refptr<TaskRunner> task_runner,
        ...
        task_runner_(std::move(task_runner)), location_(location), callback_(std::move(callback))
    {
        DCHECK(task_runner_);
    ...
    operator=(const BindPostTaskTrampoline& other) = delete;
    ...
    ~BindPostTaskTrampoline()
    {
        if (callback_)
        {
            ...
            task_runner_->PostTask(location_,
                                   BindOnce(&DestroyCallbackOnTaskRunner, std::move(callback_)));
        }
    }
...
private:
    static OnceClosure
    GetClosure(OnceClosure* callback)
    {
    ...
    template <typename... Args>
    static OnceClosure
    GetClosure(OnceCallback<void(Args...)>* callback, Args&&... args)
    {
        return BindOnce(std::move(*callback), std::forward<Args>(args)...);
    }

    static OnceClosure
    GetClosure(RepeatingClosure* callback)
    {
        // `callback` is already a closure, no need to call BindOnce().
        return *callback;
    }
    ...
    template <typename... Args>
    static OnceClosure
    GetClosure(RepeatingCallback<void(Args...)>* callback, Args&&... args)
    {
        return BindOnce(*callback, std::forward<Args>(args)...);
    }

    static void
    DestroyCallbackOnTaskRunner(CallbackType callback)
    {
    }
