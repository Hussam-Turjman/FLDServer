...
#if defined(OS_NACL)
#include "fldserver/base/hash/md5_nacl.h"
#else
#include "fldserver/base/hash/md5_boringssl.h"
#endif
...
CORE_EXPORT void
MD5Update(MD5Context* context, const StringPiece& data);

// Finalizes the MD5 operation and fills the buffer with the digest.
CORE_EXPORT void
MD5Final(MD5Digest* digest, MD5Context* context);

// Converts a digest into human-readable hexadecimal.
CORE_EXPORT std::string
MD5DigestToBase16(const MD5Digest& digest);

// Computes the MD5 sum of the given data buffer with the given length.
// The given 'digest' structure will be filled with the result data.
CORE_EXPORT void
MD5Sum(const void* data, size_t length, MD5Digest* digest);

// Returns the MD5 (in hexadecimal) of a string.
CORE_EXPORT std::string
MD5String(const StringPiece& str);
