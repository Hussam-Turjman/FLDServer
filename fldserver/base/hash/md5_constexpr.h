...
#include "fldserver/base/hash/md5_constexpr_internal.h"
...
constexpr MD5Digest
MD5SumConstexpr(const char* data, uint32_t length);
...
constexpr uint64_t
MD5Hash64Constexpr(const char* data, uint32_t length);
constexpr uint32_t
MD5Hash32Constexpr(const char* string);
constexpr uint32_t
MD5Hash32Constexpr(const char* data, uint32_t length);
