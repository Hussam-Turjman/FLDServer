...
#include "fldserver/base/hash/md5.h"
...
struct MD5CE
{
    ...
    GetPaddedMessageLength(const uint32_t n)
    {
    ...
    ExtractByte(const uint64_t value, const uint32_t i)
    {
        DCHECK(i < 8);
    ...
    static constexpr uint8_t
    GetPaddedMessageByte(const char* data, const uint32_t n, const uint32_t m, const uint32_t i)
    {
        DCHECK(i < m);
        DCHECK(n < m);
        DCHECK(m % 64 == 0);
    ...
    static constexpr uint32_t
    GetPaddedMessageWord(const char* data, const uint32_t n, const uint32_t m, const uint32_t i)
    {
        DCHECK(i % 4 == 0);
        DCHECK(i < m);
        DCHECK(n < m);
    ...
    static constexpr RoundData
    GetRoundData(const char* data, const uint32_t n, const uint32_t m, const uint32_t i)
    {
        DCHECK(i % 64 == 0);
        DCHECK(i < m);
        DCHECK(n < m);
    ...
    static constexpr uint32_t
    CalcF(const uint32_t i, const uint32_t b, const uint32_t c, const uint32_t d)
    {
        DCHECK(i < 64);
        if (i < 16)
        {
    ...
    static constexpr uint32_t
    CalcF(const uint32_t i, const IntermediateData& intermediate)
    {
    ...
    static constexpr uint32_t
    LeftRotate(const uint32_t value, const uint32_t bits)
    {
        DCHECK(bits < 32);
    ...
    static constexpr IntermediateData
    Add(const IntermediateData& intermediate1, const IntermediateData& intermediate2)
    {
        return IntermediateData{intermediate1.a + intermediate2.a,
                                intermediate1.b + intermediate2.b,
                                intermediate1.c + intermediate2.c,
                                intermediate1.d + intermediate2.d};
    }

    // Processes an entire message.
    static constexpr IntermediateData
    ProcessMessage(const char* message, const uint32_t n)
    {
    ...
    static constexpr MD5Digest
    IntermediateDataToMD5Digest(const IntermediateData& intermediate)
    {
    ...
    StringLength(const char* string)
    {
        ...
        DCHECK(end >= string);
        DCHECK(static_cast<std::ptrdiff_t>(static_cast<uint32_t>(end - string)) == (end - string));
        return static_cast<uint32_t>(end - string);
    }

    static constexpr uint32_t
    SwapEndian(uint32_t a)
    {
        return ((a & 0xff) << 24) | (((a >> 8) & 0xff) << 16) | (((a >> 16) & 0xff) << 8)
               | ((a >> 24) & 0xff);
    }
    ...
    static constexpr MD5Digest
    Sum(const char* data, uint32_t n)
    {
        return IntermediateDataToMD5Digest(ProcessMessage(data, n));
    }

    static constexpr uint64_t
    Hash64(const char* data, uint32_t n)
    {
    ...
    static constexpr uint32_t
    Hash32(const char* data, uint32_t n)
    {
        IntermediateData intermediate = ProcessMessage(data, n);
