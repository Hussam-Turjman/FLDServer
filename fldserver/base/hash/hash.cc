...
#include "fldserver/base/hash/hash.h"

#include "fldserver/base/check_op.h"
#include "fldserver/base/cityhash/city.h"
...
{
size_t
FastHashImpl(base::span<const uint8_t> data)
{
    // We use the updated CityHash within our namespace (not the deprecated
    // version from third_party/smhasher).
#if defined(ARCH_CPU_64_BITS)
    return base::internal::cityhash_v111::CityHash64(reinterpret_cast<const char*>(data.data()),
                                                     data.size());
#else
    return base::internal::cityhash_v111::CityHash32(reinterpret_cast<const char*>(data.data()),
                                                     data.size());
#endif
}
...
size_t
HashInts32Impl(uint32_t value1, uint32_t value2)
{
...
size_t
HashInts64Impl(uint64_t value1, uint64_t value2)
{
...
size_t
FastHash(base::span<const uint8_t> data)
{
...
uint32_t
PersistentHash(span<const uint8_t> data)
{
...
uint32_t
PersistentHash(const void* data, size_t length)
{
    return PersistentHash(make_span(static_cast<const uint8_t*>(data), length));
}

uint32_t
PersistentHash(const std::string& str)
{
    return PersistentHash(str.data(), str.size());
}

size_t
HashInts32(uint32_t value1, uint32_t value2)
{
...
// Then we reduce our result to 32 bits if required, similar to above.
size_t
HashInts64(uint64_t value1, uint64_t value2)
{
