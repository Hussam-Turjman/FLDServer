...
#include "fldserver/base/ios/scoped_critical_action.h"
#endif
...
public:
    ...
    CriticalClosure&
    operator=(const CriticalClosure&) = delete;
    ~CriticalClosure();
    void
    Run();
...
#if defined(OS_IOS)
inline OnceClosure
MakeCriticalClosure(StringPiece task_name, OnceClosure closure)
{
    DCHECK(internal::IsMultiTaskingSupported());
    return base::BindOnce(&internal::CriticalClosure::Run,
                          Owned(new internal::CriticalClosure(task_name, std::move(closure))));
}

inline OnceClosure
MakeCriticalClosure(const Location& posted_from, OnceClosure closure)
{
