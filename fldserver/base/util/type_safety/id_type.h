...
#include "fldserver/base/types/strong_alias.h"
...
{
public:
    static_assert(std::is_unsigned<WrappedType>::value || kInvalidValue <= 0,
                  "If signed, the invalid value should be negative or equal to zero to "
                  "avoid overflow issues.");

    static_assert(kFirstGeneratedId != kInvalidValue, "The first generated ID cannot be invalid.");

    static_assert(std::is_unsigned<WrappedType>::value || kFirstGeneratedId > kInvalidValue,
    ...
    public:
        ...
        GenerateNextId()
        {
            return FromUnsafeValue(next_id_++);
        }
    ...
    constexpr IdType() : base::StrongAlias<TypeMarker, WrappedType>::StrongAlias(kInvalidValue)
    {
    }
    ...
    // StrongAlias).
    constexpr static IdType
    FromUnsafeValue(WrappedType value)
    {
        return IdType(value);
    }
