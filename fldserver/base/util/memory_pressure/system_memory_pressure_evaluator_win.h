...
class SystemMemoryPressureEvaluator : public util::SystemMemoryPressureEvaluator
{
public:
    ...
    explicit SystemMemoryPressureEvaluator(std::unique_ptr<MemoryPressureVoter> voter);
    ...
    void
    CheckMemoryPressureSoon();

    // Returns the moderate pressure level free memory threshold, in MB.
    int
    moderate_threshold_mb() const
    {
        return moderate_threshold_mb_;
    }

    // Returns the critical pressure level free memory threshold, in MB.
    int
    critical_threshold_mb() const
    {
        return critical_threshold_mb_;
    }
    ...
    void
    CreateOSSignalPressureEvaluator(std::unique_ptr<MemoryPressureVoter> voter);
...
protected:
    ...
    // thread.
    virtual bool
    GetSystemMemoryStatus(MEMORYSTATUSEX* mem_status);
