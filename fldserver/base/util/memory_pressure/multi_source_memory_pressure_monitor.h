...
#include "fldserver/base/memory/memory_pressure_monitor.h"
...
class MultiSourceMemoryPressureMonitor : public base::MemoryPressureMonitor,
...
public:
    ...
    MemoryPressureLevel
    GetCurrentPressureLevel() const override;
    void
    SetDispatchCallback(const DispatchCallback& callback) override;
    ...
    aggregator_for_testing()
    {
        return &aggregator_;
    }

    void
    ResetSystemEvaluatorForTesting();

    void
    SetSystemEvaluator(std::unique_ptr<SystemMemoryPressureEvaluator> evaluator);
...
private:
    ...
    void
    OnMemoryPressureLevelChanged(MemoryPressureLevel level) override;
    void
    OnNotifyListenersRequested() override;

    void
    RecordCurrentPressureLevel();
