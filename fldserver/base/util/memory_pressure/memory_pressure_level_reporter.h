...
public:
    ...
    explicit MemoryPressureLevelReporter(MemoryPressureLevel initial_pressure_level);
    ~MemoryPressureLevelReporter();

    // Should be called whenever the current memory pressure level changes.
    void
    OnMemoryPressureLevelChanged(MemoryPressureLevel new_level);
...
private:
    ...
    // exceeds one second.
    std::array<base::TimeDelta, MemoryPressureLevel::kMaxValue + 1> accumulator_buckets_;
