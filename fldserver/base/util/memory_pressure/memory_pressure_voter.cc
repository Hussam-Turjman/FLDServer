...
#include "fldserver/base/util/memory_pressure/memory_pressure_voter.h"
...
public:
    ...
    MemoryPressureVoterImpl&
    operator=(MemoryPressureVoterImpl&&) = delete;

    void
    SetVote(base::MemoryPressureListener::MemoryPressureLevel level, bool notify_listeners) override
    {
...
MemoryPressureVoteAggregator::MemoryPressureVoteAggregator(Delegate* delegate) : delegate_(delegate)
{
}
...
void
MemoryPressureVoteAggregator::OnVoteForTesting(base::Optional<MemoryPressureLevel> old_vote,
...
void
MemoryPressureVoteAggregator::NotifyListenersForTesting()
{
    NotifyListeners();
}
...
void
MemoryPressureVoteAggregator::OnVote(base::Optional<MemoryPressureLevel> old_vote,
                                     base::Optional<MemoryPressureLevel> new_vote)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (old_pressure_level == MemoryPressureLevel::MEMORY_PRESSURE_LEVEL_CRITICAL)
    {
    }
    else if (old_pressure_level == MemoryPressureLevel::MEMORY_PRESSURE_LEVEL_MODERATE)
    {
    }
    ...
    if (current_pressure_level_ == MemoryPressureLevel::MEMORY_PRESSURE_LEVEL_CRITICAL)
    {
    }
    else if (current_pressure_level_ == MemoryPressureLevel::MEMORY_PRESSURE_LEVEL_MODERATE)
    {
    }
...
void
MemoryPressureVoteAggregator::NotifyListeners()
{
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    static_assert(base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL == 2,
...
void
MemoryPressureVoteAggregator::SetVotesForTesting(size_t none_votes,
