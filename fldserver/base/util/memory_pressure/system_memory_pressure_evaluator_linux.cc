...
int
GetAvailableSystemMemoryMiB(const base::SystemMemoryInfoKB& mem_info)
{
    ...
    // state)
    int mem_available = mem_info.available ? mem_info.available
...
SystemMemoryPressureEvaluator::SystemMemoryPressureEvaluator(
    ...
    util::SystemMemoryPressureEvaluator(std::move(voter)), moderate_pressure_repeat_count_(0)
{
    if (InferThresholds())
        StartObserving();
}
...
bool
SystemMemoryPressureEvaluator::GetSystemMemoryInfo(base::SystemMemoryInfoKB* mem_info)
{
...
SystemMemoryPressureEvaluator::InferThresholds()
{
    ...
    // overflow.
    static_assert(std::is_same<decltype(mem_info.total), int>::value,
                  "SystemMemoryInfoKB::total is expected to be stored as an integer.");
    critical_threshold_mb_ =
            base::checked_cast<int>(static_cast<uint64_t>(mem_info.total)
