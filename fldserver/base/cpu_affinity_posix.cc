...
#include "fldserver/base/cpu_affinity_posix.h"
    ...
    static const cpu_set_t kLittleCores = []() {
        ...
        for (size_t core_index = 0; core_index < core_types.size(); core_index++)
        {
            switch (core_types[core_index])
            {
                ...
                case CPU::CoreType::kBigLittleBigger_Little: CPU_SET(core_index, &set); break;
                case CPU::CoreType::kBigLittle_Big:
                case CPU::CoreType::kBigLittleBigger_Big:
                case CPU::CoreType::kBigLittleBigger_Bigger: break;
            }
        }
        return set;
    }();
    ...
    static const bool kHasBigCores = []() {
        ...
        for (CPU::CoreType core_type : core_types)
        {
            switch (core_type)
            {
                case CPU::CoreType::kUnknown:
                case CPU::CoreType::kOther:
                case CPU::CoreType::kSymmetric: return false;
                case CPU::CoreType::kBigLittle_Little:
                case CPU::CoreType::kBigLittleBigger_Little:
                case CPU::CoreType::kBigLittle_Big:
                case CPU::CoreType::kBigLittleBigger_Big:
                case CPU::CoreType::kBigLittleBigger_Bigger: return true;
            }
        }
        return false;
    }();
...
bool
SetThreadCpuAffinityMode(PlatformThreadId thread_id, CpuAffinityMode affinity)
{
    int result = 0;
    switch (affinity)
    {
        ...
        case CpuAffinityMode::kLittleCoresOnly: {
            ...
            result = sched_setaffinity(thread_id, sizeof(little_cores), &little_cores);
            break;
        }
    }
    return result == 0;
...
bool
SetProcessCpuAffinityMode(ProcessHandle process_handle, CpuAffinityMode affinity)
{
    bool any_threads = false;
    bool result = true;

    internal::ForEachProcessTask(process_handle,
                                 [&any_threads, &result, affinity](PlatformThreadId tid,
...
Optional<CpuAffinityMode>
CurrentThreadCpuAffinityMode()
{
    if (HasBigCpuCores())
    {
        cpu_set_t set;
