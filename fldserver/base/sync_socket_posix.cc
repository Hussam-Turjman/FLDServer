...
size_t
SendHelper(SyncSocket::Handle handle, const void* buffer, size_t length)
{
    DCHECK_GT(length, 0u);
    DCHECK_LE(length, kMaxMessageLength);
    DCHECK_NE(handle, SyncSocket::kInvalidHandle);
    const char* charbuffer = static_cast<const char*>(buffer);
    return WriteFileDescriptor(handle, charbuffer, length) ? static_cast<size_t>(length) : 0;
}
...
SyncSocket::CreatePair(SyncSocket* socket_a, SyncSocket* socket_b)
{
    ...
    if (0 != setsockopt(handles[0].get(), SOL_SOCKET, SO_NOSIGPIPE, &nosigpipe, sizeof(nosigpipe))
        || 0
                   != setsockopt(handles[1].get(),
                                 SOL_SOCKET,
                                 SO_NOSIGPIPE,
                                 &nosigpipe,
                                 sizeof(nosigpipe)))
...
SyncSocket::ReceiveWithTimeout(void* buffer, size_t length, TimeDelta timeout)
{
    ...
    DCHECK_LT(timeout.InMicroseconds(), TimeDelta::FromSeconds(1).InMicroseconds());
    ...
    while (bytes_read_total < length)
    {
        ...
        const int timeout_ms = static_cast<int>(this_timeout.InMillisecondsRoundedUp());
        if (timeout_ms <= 0)
            break;
