...
#include "fldserver/base/win/resource_util.h"
...
GetVsFixedFileInfo(const void* data)
{
...
// static
std::unique_ptr<FileVersionInfo>
FileVersionInfo::CreateFileVersionInfo(const base::FilePath& file_path)
{
...
FileVersionInfoWin::CreateFileVersionInfoWin(const base::FilePath& file_path)
{
    ...
    return base::WrapUnique(
            new FileVersionInfoWin(std::move(data), translate->language, translate->code_page));
}
...
bool
FileVersionInfoWin::GetValue(const char16_t* name, std::u16string* value) const
{
    ...
    for (const auto& lang_codepage : lang_codepages)
    {
        ...
        _snwprintf_s(sub_block,
                     ...
                     lang_codepage.language,
                     lang_codepage.code_page,
                     base::as_wcstr(name));
...
std::u16string
FileVersionInfoWin::GetStringValue(const char16_t* name) const
{
    std::u16string str;
    GetValue(name, &str);
    return str;
...
base::Version
FileVersionInfoWin::GetFileVersion() const
{
...
FileVersionInfoWin::FileVersionInfoWin(std::vector<uint8_t>&& data, WORD language, WORD code_page) :
...
FileVersionInfoWin::FileVersionInfoWin(void* data, WORD language, WORD code_page) :
    data_(data),
    language_(language),
    code_page_(code_page),
