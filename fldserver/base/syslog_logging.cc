...
#include "fldserver/base/syslog_logging.h"
...
using ScopedEventLogHandle =
        base::win::GenericScopedHandle<EventLogHandleTraits, base::win::DummyVerifierTraits>;

}  // namespace

void
SetEventSource(const std::string& name, uint16_t category, uint32_t event_id)
{
...
EventLogMessage::EventLogMessage(const char* file, int line, LogSeverity severity) :
...
EventLogMessage::~EventLogMessage()
{
    ...
    {
        case LOGGING_INFO: log_type = EVENTLOG_INFORMATION_TYPE; break;
        case LOGGING_WARNING: log_type = EVENTLOG_WARNING_TYPE; break;
        case LOGGING_ERROR:
        case LOGGING_FATAL:
    ...
    if (!ReportEventA(event_log_handle.Get(),
                      log_type,
                      g_category,
                      g_event_id,
                      user_sid,
    ...
    switch (log_message_.severity())
    {
        case LOGGING_INFO: priority = 6; break;
        case LOGGING_WARNING: priority = 4; break;
        case LOGGING_ERROR: priority = 3; break;
        case LOGGING_FATAL: priority = 2; break;
    }
