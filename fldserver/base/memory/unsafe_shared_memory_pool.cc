...
#include "fldserver/base/memory/unsafe_shared_memory_pool.h"
...
UnsafeSharedMemoryPool::Handle::Handle(PassKey<UnsafeSharedMemoryPool>,
                                       UnsafeSharedMemoryRegion region,
                                       WritableSharedMemoryMapping mapping,
                                       scoped_refptr<UnsafeSharedMemoryPool> pool) :
    region_(std::move(region)), mapping_(std::move(mapping)), pool_(std::move(pool))
{
    CHECK(pool_);
...
const UnsafeSharedMemoryRegion&
UnsafeSharedMemoryPool::Handle::GetRegion() const
{
    return region_;
}

const WritableSharedMemoryMapping&
...
UnsafeSharedMemoryPool::MaybeAllocateBuffer(size_t region_size)
{
    ...
    return std::make_unique<Handle>(
            PassKey<UnsafeSharedMemoryPool>(), std::move(region), std::move(mapping), this);
}

void
UnsafeSharedMemoryPool::Shutdown()
{
    AutoLock lock(lock_);
    DCHECK(!is_shutdown_);
    is_shutdown_ = true;
...
void
UnsafeSharedMemoryPool::ReleaseBuffer(UnsafeSharedMemoryRegion region,
                                      WritableSharedMemoryMapping mapping)
{
    AutoLock lock(lock_);
    // Only return regions which are at least as big as the current configuration.
    if (is_shutdown_ || regions_.size() >= kMaxStoredBuffers || !region.IsValid()
        || region.GetSize() < region_size_)
    {
