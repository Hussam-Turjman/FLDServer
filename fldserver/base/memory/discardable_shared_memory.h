...
class CORE_EXPORT DiscardableSharedMemory
{
public:
    enum LockResult
    {
    ...
    // or 0 if not.
    LockResult
    Lock(size_t offset, size_t length);
    ...
    Time
    last_known_usage() const
    {
        return last_known_usage_;
    }
    ...
    // page aligned.
    void
    ReleaseMemoryIfPossible(size_t offset, size_t length);
    ...
    void
    CreateSharedMemoryOwnershipEdge(trace_event::MemoryAllocatorDump* local_segment_dump,
    ...
    static bool
    IsAshmemDeviceSupportedForTesting();
#endif
...
private:
    ...
    static LockResult
    LockPages(const UnsafeSharedMemoryRegion& region, size_t offset, size_t length);
    // UnlockPages() is a no-op on platforms not supporting discardable pages.
    static void
    UnlockPages(const UnsafeSharedMemoryRegion& region, size_t offset, size_t length);
