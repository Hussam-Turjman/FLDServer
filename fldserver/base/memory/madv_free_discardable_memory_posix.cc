...
void*
AllocatePages(size_t size_in_pages)
{
    void* data = mmap(nullptr,
                      size_in_pages * base::GetPageSize(),
...
// memory.
base::MadvFreeSupport
ProbePlatformMadvFreeSupport()
{
...
MadvFreeDiscardableMemoryPosix::MadvFreeDiscardableMemoryPosix(
    ...
    allocated_pages_((size_in_bytes_ + base::GetPageSize() - 1) / base::GetPageSize()),
    allocator_byte_count_(allocator_byte_count),
    page_first_word_((size_in_bytes_ + base::GetPageSize() - 1) / base::GetPageSize())
{
...
MadvFreeDiscardableMemoryPosix::~MadvFreeDiscardableMemoryPosix()
{
    if (Deallocate())
    {
        DVLOG(1) << "Region evicted during destructor with " << allocated_pages_ << " pages";
    }
}

bool
MadvFreeDiscardableMemoryPosix::Lock()
{
    ...
    {
        DVLOG(1) << "Region eviction discovered during lock with " << allocated_pages_ << " pages";
        Deallocate();
        return false;
    }
    DCHECK(IsResident());
...
void
MadvFreeDiscardableMemoryPosix::Unlock()
{
    DFAKE_SCOPED_LOCK(thread_collision_warner_);
    DCHECK(is_locked_);
    ...
    {
        int retval = madvise(data_, allocated_pages_ * base::GetPageSize(), MADV_FREE);
        DPCHECK(!retval);
    }
#endif
...
void*
MadvFreeDiscardableMemoryPosix::data() const
{
    DFAKE_SCOPED_LOCK(thread_collision_warner_);
    DCHECK(is_locked_);
    DCHECK(data_ != nullptr);
...
bool
MadvFreeDiscardableMemoryPosix::LockPage(size_t page_index)
{
    ...
    std::atomic<intptr_t>* page_as_atomic = reinterpret_cast<std::atomic<intptr_t>*>(
...
void
MadvFreeDiscardableMemoryPosix::UnlockPage(size_t page_index)
{
    DCHECK(std::atomic<intptr_t>{}.is_lock_free());
    ...
    page_first_word_[page_index].store(*page_as_atomic, std::memory_order_relaxed);
...
void
MadvFreeDiscardableMemoryPosix::DiscardPage(size_t page_index)
{
    DFAKE_SCOPED_LOCK(thread_collision_warner_);
    DCHECK(!is_locked_);
    DCHECK(page_index < allocated_pages_);
    int retval = madvise(static_cast<uint8_t*>(data_) + base::GetPageSize() * page_index,
                         base::GetPageSize(),
                         MADV_DONTNEED);
    DPCHECK(!retval);
}
...
bool
MadvFreeDiscardableMemoryPosix::IsLockedForTesting() const
{
    DFAKE_SCOPED_LOCK(thread_collision_warner_);
    return is_locked_;
}
...
void
MadvFreeDiscardableMemoryPosix::DiscardForTesting()
{
    DFAKE_SCOPED_LOCK(thread_collision_warner_);
    DCHECK(!is_locked_);
    int retval = madvise(data_, base::GetPageSize() * allocated_pages_, MADV_DONTNEED);
    DPCHECK(!retval);
}

/*trace_event::MemoryAllocatorDump*
MadvFreeDiscardableMemoryPosix::CreateMemoryAllocatorDump(const char* name,
                                                          trace_event::ProcessMemoryDump* pmd) const
{
    ...
    std::string allocator_dump_name = base::StringPrintf("discardable/segment_0x%" PRIXPTR,
                                                         reinterpret_cast<uintptr_t>(this));

    MemoryAllocatorDump* allocator_dump = pmd->CreateAllocatorDump(allocator_dump_name);
    ...
    allocator_dump->AddScalar(
            ...
            MemoryAllocatorDump::kUnitsBytes,
            is_discarded ? 0U : static_cast<uint64_t>(allocated_pages_ * base::GetPageSize()));
    allocator_dump->AddScalar(
            MemoryAllocatorDump::kNameObjectCount, MemoryAllocatorDump::kUnitsObjects, 1U);
    allocator_dump->AddScalar(
            "wasted_size",
            MemoryAllocatorDump::kUnitsBytes,
            static_cast<uint64_t>(allocated_pages_ * base::GetPageSize() - size_in_bytes_));
    allocator_dump->AddScalar(
            "locked_size", MemoryAllocatorDump::kUnitsBytes, is_locked_ ? size_in_bytes_ : 0U);
    allocator_dump->AddScalar("page_count",
    ...
    allocator_dump->AddScalar("discarded_size",
                              MemoryAllocatorDump::kUnitsBytes,
...
bool
MadvFreeDiscardableMemoryPosix::IsValid() const
{
...
void
MadvFreeDiscardableMemoryPosix::SetKeepMemoryForTesting(bool keep_memory)
{
    DFAKE_SCOPED_LOCK(thread_collision_warner_);
    DCHECK(is_locked_);
...
bool
MadvFreeDiscardableMemoryPosix::IsResident() const
{
    DFAKE_SCOPED_RECURSIVE_LOCK(thread_collision_warner_);
#if IS_APPLE
    ...
    int retval = mincore(data_, allocated_pages_ * base::GetPageSize(), vec.data());
...
bool
MadvFreeDiscardableMemoryPosix::IsDiscarded() const
{
    return !is_locked_ && !IsResident();
}

bool
MadvFreeDiscardableMemoryPosix::Deallocate()
{
    DFAKE_SCOPED_RECURSIVE_LOCK(thread_collision_warner_);
    if (data_)
    {
