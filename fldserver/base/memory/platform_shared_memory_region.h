...
#if defined(OS_MAC)
#include "fldserver/base/mac/scoped_mach_port.h"
...
namespace subtle
{
#if IS_POSIX && !defined(OS_MAC) && !defined(OS_ANDROID)
// Helper structs to keep two descriptors on POSIX. It's needed to support
// ConvertToReadOnly().
struct CORE_EXPORT FDPair
{
...
class CORE_EXPORT PlatformSharedMemoryRegion
{
public:
...
// Platform-specific shared memory type used by this class.
    ...
    static PlatformSharedMemoryRegion
    CreateWritable(size_t size);
    static PlatformSharedMemoryRegion
    CreateUnsafe(size_t size);
    ...
    // are invalid.
    static PlatformSharedMemoryRegion
    Take(ScopedPlatformHandle handle, Mode mode, size_t size, const UnguessableToken& guid);
#if IS_POSIX && !defined(OS_ANDROID) && !defined(OS_MAC)
    ...
    static PlatformSharedMemoryRegion
    Take(ScopedFD handle, Mode mode, size_t size, const UnguessableToken& guid);
#endif
    ...
    PlatformSharedMemoryRegion&
    operator=(PlatformSharedMemoryRegion&&);
    ...
    bool
    ConvertToReadOnly(void* mapped_addr);
    ...
    bool
    MapAt(off_t offset, size_t size, void** memory, size_t* mapped_size) const;
...
private:
    ...
    static bool
    CheckPlatformHandlePermissionsCorrespondToMode(PlatformHandle handle, Mode mode, size_t size);
    ...
    bool
    MapAtInternal(off_t offset, size_t size, void** memory, size_t* mapped_size) const;
