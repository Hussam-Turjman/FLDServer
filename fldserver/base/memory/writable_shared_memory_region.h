...
class CORE_EXPORT WritableSharedMemoryRegion
{
public:
    ...
    // address space.
    static WritableSharedMemoryRegion
    Create(size_t size);
    ...
    // boundaries.
    static WritableSharedMemoryRegion
    Deserialize(subtle::PlatformSharedMemoryRegion handle);
    ...
    // process to another.
    static subtle::PlatformSharedMemoryRegion
    TakeHandleForSerialization(WritableSharedMemoryRegion region);
    ...
    ConvertToReadOnly(WritableSharedMemoryRegion region);
    ...
    ConvertToUnsafe(WritableSharedMemoryRegion region);
    ...
    WritableSharedMemoryRegion&
    operator=(WritableSharedMemoryRegion&&);
    ...
    WritableSharedMemoryMapping
    MapAt(off_t offset, size_t size) const;
...
private:
    ...
    explicit WritableSharedMemoryRegion(subtle::PlatformSharedMemoryRegion handle);

    static void
    set_create_hook(CreateFunction* hook)
    {
        create_hook_ = hook;
    }
