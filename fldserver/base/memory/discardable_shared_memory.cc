...
#include "fldserver/base/memory/discardable_shared_memory.h"
...
#if 0
#include "fldserver/base/trace_event/memory_allocator_dump.h"  // no-presubmit-check
#include "fldserver/base/trace_event/process_memory_dump.h"    // no-presubmit-check
#endif                                                         // 0
...
template <int>
Time
TimeFromWireFormat(int64_t value);
template <int>
int64_t
...
template <>
Time ALLOW_UNUSED_TYPE
TimeFromWireFormat<4>(int64_t value)
{
    return value ? Time::UnixEpoch() + TimeDelta::FromSeconds(value) : Time();
}
template <>
int64_t ALLOW_UNUSED_TYPE
TimeToWireFormat<4>(Time time)
{
...
template <>
Time ALLOW_UNUSED_TYPE
TimeFromWireFormat<8>(int64_t value)
{
    return Time::FromInternalValue(value);
}
template <>
int64_t ALLOW_UNUSED_TYPE
TimeToWireFormat<8>(Time time)
{
...
struct SharedState
{
    ...
    explicit SharedState(AtomicType ivalue)
    {
        value.i = ivalue;
    }
    ...
    GetLockState() const
    {
        return static_cast<LockState>(value.u & 1);
    }
...
SharedState*
SharedStateFromSharedMemory(const WritableSharedMemoryMapping& shared_memory)
{
...
DiscardableSharedMemory::DiscardableSharedMemory() : mapped_size_(0), locked_page_count_(0)
{
}

DiscardableSharedMemory::DiscardableSharedMemory(UnsafeSharedMemoryRegion shared_memory_region) :
    shared_memory_region_(std::move(shared_memory_region)), mapped_size_(0), locked_page_count_(0)
{
}
...
bool
DiscardableSharedMemory::CreateAndMap(size_t size)
{
    ...
    shared_memory_region_ = UnsafeSharedMemoryRegion::Create(checked_size.ValueOrDie());
    ...
    mapped_size_ = shared_memory_mapping_.mapped_size() - AlignToPageSize(sizeof(SharedState));
    ...
    subtle::Release_Store(&SharedStateFromSharedMemory(shared_memory_mapping_)->value.i,
                          new_state.value.i);
    return true;
}

bool
DiscardableSharedMemory::Map(size_t size)
{
    ...
    shared_memory_mapping_ =
            shared_memory_region_.MapAt(0, AlignToPageSize(sizeof(SharedState)) + size);
    if (!shared_memory_mapping_.IsValid())
        return false;
...
DiscardableSharedMemory::LockResult
DiscardableSharedMemory::Lock(size_t offset, size_t length)
{
    ...
    return LockPages(shared_memory_region_, AlignToPageSize(sizeof(SharedState)) + offset, length);
#elif IS_APPLE
...
void
DiscardableSharedMemory::Unlock(size_t offset, size_t length)
{
    DCHECK_EQ(AlignToPageSize(offset), offset);
    DCHECK_EQ(AlignToPageSize(length), length);
    ...
    UnlockPages(shared_memory_region_, AlignToPageSize(sizeof(SharedState)) + offset, length);
...
void*
DiscardableSharedMemory::memory() const
{
...
bool
DiscardableSharedMemory::Purge(Time current_time)
{
    // Calls to this function must be synchronized properly.
    DFAKE_SCOPED_LOCK(thread_collision_warner_);
    DCHECK(shared_memory_mapping_.IsValid());
    ...
    if (result.value.u != old_state.value.u)
    {
        last_known_usage_ = result.GetLockState() == SharedState::LOCKED ? current_time
                                                                         : result.GetTimestamp();
        return false;
    }
...
#define MADV_PURGE_ARGUMENT MADV_FREE
    ...
    static DiscardVirtualMemoryFunction discard_virtual_memory =
            reinterpret_cast<DiscardVirtualMemoryFunction>(
                    GetProcAddress(GetModuleHandle((L"Kernel32.dll")), "DiscardVirtualMemory"));
    ...
    uint64_t address_int =
            reinterpret_cast<uint64_t>(static_cast<char*>(shared_memory_mapping_.memory())
                                       + AlignToPageSize(sizeof(SharedState)));
    zx_status_t status = zx::vmar::root_self()->op_range(
            ZX_VMO_OP_DECOMMIT, address_int, AlignToPageSize(mapped_size_), nullptr, 0);
...
void
DiscardableSharedMemory::ReleaseMemoryIfPossible(size_t offset, size_t length)
{
#if IS_POSIX && !defined(OS_NACL)
// Linux and Android provide MADV_REMOVE which is preferred as it has a
// behavior that can be verified in tests. Other POSIX flavors (MacOSX, BSDs),
...
// correctly track the pages.
#define MADV_PURGE_ARGUMENT MADV_FREE_REUSABLE
#else  // IS_LINUX || defined(OS_CHROMEOS) || defined(OS_ANDROID)
#define MADV_PURGE_ARGUMENT MADV_FREE
...
#else   // IS_POSIX && !defined(OS_NACL)
    // DiscardSystemPages(static_cast<char*>(shared_memory_mapping_.memory()) + offset, length);
#endif  // IS_POSIX && !defined(OS_NACL)
}

bool
DiscardableSharedMemory::IsMemoryResident() const
{
    DCHECK(shared_memory_mapping_.IsValid());

    SharedState result(
            subtle::NoBarrier_Load(&SharedStateFromSharedMemory(shared_memory_mapping_)->value.i));

    return result.GetLockState() == SharedState::LOCKED || !result.GetTimestamp().is_null();
}

bool
DiscardableSharedMemory::IsMemoryLocked() const
{
    DCHECK(shared_memory_mapping_.IsValid());
...
void
DiscardableSharedMemory::Close()
{
    shared_memory_region_ = UnsafeSharedMemoryRegion();
}

void
DiscardableSharedMemory::CreateSharedMemoryOwnershipEdge(
...
// Memory dumps are only supported when tracing support is enabled,.
    ...
    auto* shared_memory_dump =
            SharedMemoryTracker::GetOrCreateSharedMemoryDump(shared_memory_mapping_, pmd);
    ...
    if (is_owned)
    {
        pmd->CreateWeakSharedMemoryOwnershipEdge(
                local_segment_dump->guid(), shared_memory_guid, kImportance);
    }
    else
    {
...
// static
DiscardableSharedMemory::LockResult
DiscardableSharedMemory::LockPages(const UnsafeSharedMemoryRegion& region,
                                   size_t offset,
                                   size_t length)
{
#if defined(OS_ANDROID)
    if (region.IsValid())
    {
        ...
        {
            int pin_result = ashmem_pin_region(region.GetPlatformHandle(), offset, length);
...
// static
void
DiscardableSharedMemory::UnlockPages(const UnsafeSharedMemoryRegion& region,
                                     size_t offset,
                                     size_t length)
{
#if defined(OS_ANDROID)
    if (region.IsValid())
    {
        if (UseAshmemUnpinningForDiscardableMemory())
        {
            int unpin_result = ashmem_unpin_region(region.GetPlatformHandle(), offset, length);
...
// static
bool
DiscardableSharedMemory::IsAshmemDeviceSupportedForTesting()
{
