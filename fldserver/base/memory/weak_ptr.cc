...
bool
WeakReference::Flag::IsValid() const
{
...
bool
WeakReference::Flag::MaybeValid() const
{
    return !invalidated_.IsSet();
}

void
WeakReference::Flag::DetachFromSequence()
{
    DETACH_FROM_SEQUENCE(sequence_checker_);
}
...
bool
WeakReference::IsValid() const
{
    return flag_ && flag_->IsValid();
}

bool
WeakReference::MaybeValid() const
{
    return flag_ && flag_->MaybeValid();
}

WeakReferenceOwner::WeakReferenceOwner() : flag_(MakeRefCounted<WeakReference::Flag>())
{
}
...
WeakReference
WeakReferenceOwner::GetRef() const
{
