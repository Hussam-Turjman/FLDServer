...
//
class CORE_EXPORT MadvFreeDiscardableMemoryPosix : public DiscardableMemory
{
public:
    MadvFreeDiscardableMemoryPosix(size_t size_in_pages, std::atomic<size_t>* allocator_byte_count);
    ...
    bool
    IsLockedForTesting() const;
    void
    DiscardForTesting() override;

    /*   trace_event::MemoryAllocatorDump*
       CreateMemoryAllocatorDump(const char* name, trace_event::ProcessMemoryDump* pmd) const
       override;*/
...
protected:
    ...
    void
    SetKeepMemoryForTesting(bool keep_memory);
    ...
    void
    DiscardPage(size_t page_index);

private:
    bool
    LockPage(size_t page_index);
    void
    UnlockPage(size_t page_index);
...
enum class MadvFreeSupport
{
    kUnsupported,
    kSupported
};
CORE_EXPORT MadvFreeSupport
GetMadvFreeSupport();
