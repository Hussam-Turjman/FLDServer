...
typedef ULONG(__stdcall* NtQuerySectionType)(HANDLE SectionHandle,
...
size_t
GetMemorySectionSize(void* address)
{
    MEMORY_BASIC_INFORMATION memory_info;
    if (!::VirtualQuery(address, &memory_info, sizeof(memory_info)))
        return 0;
    return memory_info.RegionSize
           - (static_cast<char*>(address) - static_cast<char*>(memory_info.AllocationBase));
}

// Checks if the section object is safe to map. At the moment this just means
// it's not an image section.
bool
IsSectionSafeToMap(HANDLE handle)
{
    static NtQuerySectionType nt_query_section_func = reinterpret_cast<NtQuerySectionType>(
            ::GetProcAddress(::GetModuleHandle((L"ntdll.dll")), "NtQuerySection"));
    DCHECK(nt_query_section_func);

    // The handle must have SECTION_QUERY access for this to succeed.
    SECTION_BASIC_INFORMATION basic_information = {};
    ULONG status = nt_query_section_func(handle,
                                         SectionBasicInformation,
                                         &basic_information,
...
HANDLE
CreateFileMappingWithReducedPermissions(SECURITY_ATTRIBUTES* sa, size_t rounded_size, LPCWSTR name)
{
    HANDLE h = CreateFileMapping(
            INVALID_HANDLE_VALUE, sa, PAGE_READWRITE, 0, static_cast<DWORD>(rounded_size), (name));
    if (!h)
    {
        return nullptr;
    }
    ...
    BOOL success = ::DuplicateHandle(
            process, h, process, &h2, FILE_MAP_READ | FILE_MAP_WRITE | SECTION_QUERY, FALSE, 0);
...
PlatformSharedMemoryRegion::Take(win::ScopedHandle handle,
                                 Mode mode,
                                 size_t size,
...
PlatformSharedMemoryRegion::Duplicate() const
{
    ...
    ProcessHandle process = GetCurrentProcess();
    BOOL success = ::DuplicateHandle(
            process, handle_.Get(), process, &duped_handle, 0, FALSE, DUPLICATE_SAME_ACCESS);
    if (!success)
        return {};
...
PlatformSharedMemoryRegion::ConvertToReadOnly()
{
    ...
    HANDLE duped_handle;
    ProcessHandle process = GetCurrentProcess();
    BOOL success = ::DuplicateHandle(process,
                                     handle_copy.Get(),
                                     process,
                                     &duped_handle,
                                          ...
                                          size_t* mapped_size) const
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        *memory = MapViewOfFile(handle_.Get(),
                                FILE_MAP_READ | (write_allowed ? FILE_MAP_WRITE : 0),
...
// static
PlatformSharedMemoryRegion
PlatformSharedMemoryRegion::Create(Mode mode, size_t size)
{
                                                                           ...
                                                                           size_t size)
{
    ...
    HANDLE duped_handle;
    BOOL success =
            ::DuplicateHandle(process, handle, process, &duped_handle, FILE_MAP_WRITE, FALSE, 0);
    if (success)
    {
...
PlatformSharedMemoryRegion::PlatformSharedMemoryRegion(win::ScopedHandle handle,
                                                       Mode mode,
                                                       size_t size,
                                                       const UnguessableToken& guid) :
