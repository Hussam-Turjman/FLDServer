...
constexpr bool
IsRefCountPreferenceOverridden(const T*, const RefCounted<U, V>*)
{
...
template <typename T>
scoped_refptr<T>
AdoptRefIfNeeded(T* obj, StartRefCountFromZeroTag)
{
...
template <typename T>
scoped_refptr<T>
AdoptRefIfNeeded(T* obj, StartRefCountFromOneTag)
{
    return AdoptRef(obj);
}
...
public:
    ...
    scoped_refptr&
    operator=(scoped_refptr r) noexcept
    {
        swap(r);
        return *this;
    }
    ...
    template <typename U>
    bool
    operator==(const scoped_refptr<U>& rhs) const
    {
        return ptr_ == rhs.get();
    }
...
template <typename T>
bool
operator==(const scoped_refptr<T>& lhs, std::nullptr_t null)
{
...
template <typename T>
bool
operator==(std::nullptr_t null, const scoped_refptr<T>& rhs)
{
...
template <typename T>
std::ostream&
operator<<(std::ostream& out, const scoped_refptr<T>& p)
{
    return out << p.get();
}
...
template <typename T>
void
swap(scoped_refptr<T>& lhs, scoped_refptr<T>& rhs) noexcept
{
    lhs.swap(rhs);
}
