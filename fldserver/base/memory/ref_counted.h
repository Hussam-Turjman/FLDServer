...
#include "fldserver/base/atomic_ref_count.h"
...
#include "fldserver/base/threading/thread_collision_warner.h"
#include "fldserver/fldserver_config.h"
...
namespace subtle
{
class CORE_EXPORT RefCountedBase
{
public:
    bool
    HasOneRef() const
...
#if DCHECK_IS_ON()
        DCHECK(!in_dtor_);
        DCHECK(!needs_adopt_ref_) << "This RefCounted object is created with non-zero reference "
                                     "count."
                                  << " The first reference to such a object has to be made by "
                                     "AdoptRef or"
                                  << " MakeRefCounted.";
        if (ref_count_ >= 1)
        {
...
class CORE_EXPORT RefCountedThreadSafeBase
{
public:
    bool
    HasOneRef() const;
...
class CORE_EXPORT ScopedAllowCrossThreadRefCountAccess final
{
public:
#if DCHECK_IS_ON()
    ScopedAllowCrossThreadRefCountAccess();
...
template <typename T>
bool
operator==(const RefCountedData<T>& lhs, const RefCountedData<T>& rhs)
{
