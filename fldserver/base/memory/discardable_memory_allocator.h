...
class CORE_EXPORT DiscardableMemoryAllocator
{
public:
    ...
    virtual std::unique_ptr<DiscardableMemory>
    ...
    AllocateLockedDiscardableMemoryWithRetryOrDie(size_t size, OnceClosure on_no_memory);

    // Gets the total number of bytes allocated by this allocator which have not
    // been discarded.
    virtual size_t
    GetBytesAllocated() const = 0;
