...
using ScopedPathUnlinker = ScopedGeneric<const FilePath*, ScopedPathUnlinkerTraits>;

#if !defined(OS_NACL)
bool
CheckFDAccessMode(int fd, int expected_mode)
{
    ...
    {
        // TODO(crbug.com/838365): convert to DLOG when bug fixed.
        LOG(ERROR) << "Descriptor access mode (" << mode << ") differs from expected ("
                   << expected_mode << ")";
        return false;
    }
...
ScopedFDPair&
ScopedFDPair::operator=(ScopedFDPair&&) = default;
...
// static
ScopedFD
PlatformSharedMemoryRegion::ExecutableRegion::CreateFD(size_t size)
{
    PlatformSharedMemoryRegion region = Create(Mode::kUnsafe, size, true /* executable */);
    if (region.IsValid())
...
PlatformSharedMemoryRegion::Take(ScopedFDPair handle,
                                 Mode mode,
                                 size_t size,
                                 const UnguessableToken& guid)
{
    ...
    switch (mode)
    {
        ...
        case Mode::kWritable:
            if (!handle.readonly_fd.is_valid())
            {
                DLOG(ERROR) << "Readonly handle must be valid for writable memory region";
                return {};
            }
            break;
        default: DLOG(ERROR) << "Invalid permission mode: " << static_cast<int>(mode); return {};
    }
...
// static
PlatformSharedMemoryRegion
PlatformSharedMemoryRegion::Take(ScopedFD handle,
                                 Mode mode,
                                 size_t size,
                                 const UnguessableToken& guid)
{
...
FDPair
PlatformSharedMemoryRegion::GetPlatformHandle() const
{
    return handle_.get();
}

bool
PlatformSharedMemoryRegion::IsValid() const
{
...
PlatformSharedMemoryRegion::Duplicate() const
{
    ...
    return PlatformSharedMemoryRegion({std::move(duped_fd), ScopedFD()}, mode_, size_, guid_);
}
...
PlatformSharedMemoryRegion::ConvertToReadOnly()
{
    ...
    CHECK_EQ(mode_, Mode::kWritable) << "Only writable shared memory region can be converted to "
                                        "read-only";
...
bool
PlatformSharedMemoryRegion::MapAtInternal(off_t offset,
                                          size_t size,
                                          void** memory,
                                          size_t* mapped_size) const
{
    ...
    *memory = mmap(nullptr,
                   size,
                   PROT_READ | (write_allowed ? PROT_WRITE : 0),
                   MAP_SHARED,
                   handle_.fd.get(),
                   offset);
...
// static
PlatformSharedMemoryRegion
PlatformSharedMemoryRegion::Create(Mode mode,
                                   size_t size
#if IS_LINUX || defined(OS_CHROMEOS)
                                   ,
                                   bool executable
...
)
{
    ...
    if (readonly_fd.is_valid())
    {
        ...
        if (shm_stat.st_dev != readonly_stat.st_dev || shm_stat.st_ino != readonly_stat.st_ino)
        {
    ...
    return PlatformSharedMemoryRegion({ScopedFD(shm_file.TakePlatformFile()),
...
bool
PlatformSharedMemoryRegion::CheckPlatformHandlePermissionsCorrespondToMode(PlatformHandle handle,
                                                                           Mode mode,
                                                                           size_t size)
{
#if !defined(OS_NACL)
    if (!CheckFDAccessMode(handle.fd, mode == Mode::kReadOnly ? O_RDONLY : O_RDWR))
    {
        return false;
    }
...
PlatformSharedMemoryRegion::PlatformSharedMemoryRegion(ScopedFDPair handle,
                                                       Mode mode,
                                                       size_t size,
                                                       const UnguessableToken& guid) :
