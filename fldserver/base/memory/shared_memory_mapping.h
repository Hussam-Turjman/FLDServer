...
#include "fldserver/base/containers/buffer_iterator.h"
...
// shared memory.
class CORE_EXPORT SharedMemoryMapping
{
public:
    ...
    SharedMemoryMapping&
    operator=(SharedMemoryMapping&& mapping) noexcept;
...
// instances.
class CORE_EXPORT ReadOnlySharedMemoryMapping : public SharedMemoryMapping
{
public:
    // Default constructor initializes an invalid instance.
    ...
    ReadOnlySharedMemoryMapping&
    operator=(ReadOnlySharedMemoryMapping&&) noexcept;
    ...
    template <typename T>
    span<const T>
    GetMemoryAsSpan(size_t count) const
    {
        static_assert(std::is_trivially_copyable<T>::value,
                      "Copying non-trivially-copyable object across memory spaces "
                      "is dangerous");
    ...
    template <typename T>
    BufferIterator<const T>
    GetMemoryAsBufferIterator() const
    {
...
// instances.
class CORE_EXPORT WritableSharedMemoryMapping : public SharedMemoryMapping
{
public:
    // Default constructor initializes an invalid instance.
    ...
    WritableSharedMemoryMapping&
    operator=(WritableSharedMemoryMapping&&) noexcept;
    ...
    template <typename T>
    BufferIterator<T>
    GetMemoryAsBufferIterator()
    {
        return BufferIterator<T>(GetMemoryAsSpan<T>());
    }

private:
    friend WritableSharedMemoryMapping
    MapAtForTesting(subtle::PlatformSharedMemoryRegion* region, off_t offset, size_t size);
    friend class ReadOnlySharedMemoryRegion;
