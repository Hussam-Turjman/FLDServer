...
#if 0
#include "fldserver/base/allocator/partition_allocator/address_pool_manager_bitmap.h"
#include "fldserver/base/allocator/partition_allocator/partition_address_space.h"
#include "fldserver/base/allocator/partition_allocator/partition_alloc_constants.h"
#include "fldserver/base/allocator/partition_allocator/partition_alloc_forward.h"
#include "fldserver/base/allocator/partition_allocator/partition_ref_count.h"
#endif
...
struct CheckedPtrNoOpImpl
{
    ...
    static ALWAYS_INLINE void*
    SafelyUnwrapPtrForDereference(void* wrapped_ptr)
    {
        return wrapped_ptr;
    }
    ...
    static ALWAYS_INLINE void*
    SafelyUnwrapPtrForExtraction(void* wrapped_ptr)
    {
        return wrapped_ptr;
    }
    ...
    // freed or not.
    static ALWAYS_INLINE void*
    UnsafelyUnwrapPtrForComparison(void* wrapped_ptr)
    {
        return wrapped_ptr;
    }
    ...
    static ALWAYS_INLINE constexpr void*
    Upcast(void* wrapped_ptr)
    {
    ...
    static ALWAYS_INLINE void*
    Advance(void* wrapped_ptr, ptrdiff_t delta)
    {
...
struct BackupRefPtrImpl
{
    ...
    IsSupportedAndNotNull(void* ptr)
    {
        ...
        if (ret)
        {
            DCHECK(reinterpret_cast<uintptr_t>(ptr) % kSuperPageSize >= PartitionPageSize());
        }
#else
    ...
    // Notifies the allocator when a wrapped pointer is being removed or replaced.
    static ALWAYS_INLINE void
    ReleaseWrappedPtr(void* wrapped_ptr)
    {
        if (IsSupportedAndNotNull(wrapped_ptr))
        {
            ...
            AddressPoolManagerBitmap::DecrementOutsideOfBRPPoolPtrRefCount(wrapped_ptr);
#endif
    }
...
#endif
        void* new_wrapped_ptr = WrapRawPtr(reinterpret_cast<char*>(wrapped_ptr) + delta);
...
private:
    ...
    static CORE_EXPORT NOINLINE void
    AcquireInternal(void* ptr);
    static CORE_EXPORT NOINLINE void
    ReleaseInternal(void* ptr);
    static CORE_EXPORT NOINLINE bool
    IsPointeeAlive(void* ptr);
    static CORE_EXPORT NOINLINE bool
    IsValidDelta(void* ptr, ptrdiff_t delta);
};
...
public:
    ...
    CheckedPtr&
    operator=(const CheckedPtr&) noexcept = default;
    CheckedPtr&
    operator=(CheckedPtr&&) noexcept = default;
    ...
    constexpr ALWAYS_INLINE CheckedPtr(std::nullptr_t) noexcept : wrapped_ptr_(nullptr)
    {
    }
    ...
    // NOLINTNEXTLINE(google-explicit-constructor)
    ALWAYS_INLINE
    CheckedPtr(T* p) noexcept : wrapped_ptr_(CastFromVoidPtr(Impl::WrapRawPtr(CastToVoidPtr(p))))
    {
    }
    ...
    // NOLINTNEXTLINE(google-explicit-constructor)
    ALWAYS_INLINE
    CheckedPtr(const CheckedPtr<U, Impl>& ptr) noexcept :
        wrapped_ptr_(CastFromVoidPtr(Impl::Duplicate(Impl::template Upcast<T, U>(ptr.AsVoidPtr()))))
    {
    }
    // Deliberately implicit in order to support implicit upcast.
    template <typename U,
    ...
    // NOLINTNEXTLINE(google-explicit-constructor)
    ALWAYS_INLINE
    CheckedPtr(CheckedPtr<U, Impl>&& ptr) noexcept :
        wrapped_ptr_(CastFromVoidPtr(Impl::template Upcast<T, U>(ptr.AsVoidPtr())))
    {
#if 0
    ...
    template <typename U,
              typename Unused =
                      std::enable_if_t<std::is_convertible<U*, T*>::value
                                       && !std::is_void<typename std::remove_cv<T>::type>::value>>
    ALWAYS_INLINE CheckedPtr&
    operator=(const CheckedPtr<U, Impl>& ptr) noexcept
    {
        // Make sure that pointer isn't assigned to itself (look at pointer address,
        // not its value).
        DCHECK(reinterpret_cast<uintptr_t>(this) != reinterpret_cast<uintptr_t>(&ptr));
        Impl::ReleaseWrappedPtr(AsVoidPtr());
        SetFromVoidPtr(Impl::Duplicate(Impl::template Upcast<T, U>(ptr.AsVoidPtr())));
        return *this;
    }
    template <typename U,
              typename Unused =
                      std::enable_if_t<std::is_convertible<U*, T*>::value
                                       && !std::is_void<typename std::remove_cv<T>::type>::value>>
    ALWAYS_INLINE CheckedPtr&
    operator=(CheckedPtr<U, Impl>&& ptr) noexcept
    {
        // Make sure that pointer isn't assigned to itself (look at pointer address,
        // not its value).
        DCHECK(reinterpret_cast<uintptr_t>(this) != reinterpret_cast<uintptr_t>(&ptr));
    ...
    template <typename U = T,
              typename Unused =
                      std::enable_if_t<!std::is_void<typename std::remove_cv<U>::type>::value>>
    ALWAYS_INLINE U&
    operator*() const
    {
    ...
    ALWAYS_INLINE CheckedPtr&
    operator+=(ptrdiff_t delta_elems)
    {
    ...
    friend ALWAYS_INLINE bool
    operator==(const CheckedPtr& lhs, T* rhs)
    {
    ...
    friend ALWAYS_INLINE bool
    operator==(T* lhs, const CheckedPtr& rhs)
    {
    ...
    template <typename U>
    friend ALWAYS_INLINE bool
    operator==(const CheckedPtr& lhs, const CheckedPtr<U, Impl>& rhs)
    {
        // Add |const volatile| when casting, in case |U| has any. Even if |T|
        // doesn't, comparison between |T*| and |const volatile T*| is fine.
        return lhs.GetForComparison() == static_cast<std::add_cv_t<T>*>(rhs.GetForComparison());
    }
    template <typename U>
    friend ALWAYS_INLINE bool
    operator!=(const CheckedPtr& lhs, const CheckedPtr<U, Impl>& rhs)
    ...
    template <typename U>
    friend ALWAYS_INLINE bool
    operator==(const CheckedPtr& lhs, U* rhs)
    {
        // Add |const volatile| when casting, in case |U| has any. Even if |T|
        // doesn't, comparison between |T*| and |const volatile T*| is fine.
    ...
    template <typename U>
    friend ALWAYS_INLINE bool
    operator==(U* lhs, const CheckedPtr& rhs)
    {
        return rhs == lhs;  // Reverse order to call the operator above.
    }
    template <typename U>
    ...
    friend ALWAYS_INLINE bool
    operator==(const CheckedPtr& lhs, std::nullptr_t)
    {
        return !lhs;
    }
    friend ALWAYS_INLINE bool
    ...
    friend ALWAYS_INLINE bool
    operator==(std::nullptr_t, const CheckedPtr& rhs)
    {
        return !rhs;
    }
    friend ALWAYS_INLINE bool
    ...
    friend ALWAYS_INLINE void
    swap(CheckedPtr& lhs, CheckedPtr& rhs) noexcept
    {
...
private:
    ALWAYS_INLINE static constexpr T*
    CastFromVoidPtr(void* ptr)
    {
    ...
    CastToVoidPtr(T* ptr)
    {
        return static_cast<void*>(const_cast<typename std::remove_cv<T>::type*>(ptr));
    }
    ALWAYS_INLINE void*
    AsVoidPtr() const
    {
        return CastToVoidPtr(wrapped_ptr_);
    }
    ALWAYS_INLINE void
    SetFromVoidPtr(void* ptr)
    {
