...
#include "fldserver/base/file_descriptor_store.h"
...
namespace base
{
FileDescriptorStore::Descriptor::Descriptor(const std::string& key, base::ScopedFD fd) :
    key(key), fd(std::move(fd)), region(base::MemoryMappedFile::Region::kWholeFile)
{
}
...
FileDescriptorStore::Descriptor::Descriptor(FileDescriptorStore::Descriptor&& other) :
...
// static
FileDescriptorStore&
FileDescriptorStore::GetInstance()
{
...
base::ScopedFD
FileDescriptorStore::TakeFD(const std::string& key, base::MemoryMappedFile::Region* region)
{
...
base::ScopedFD
FileDescriptorStore::MaybeTakeFD(const std::string& key, base::MemoryMappedFile::Region* region)
{
...
void
FileDescriptorStore::Set(const std::string& key, base::ScopedFD fd)
{
    Set(key, std::move(fd), base::MemoryMappedFile::Region::kWholeFile);
}

void
FileDescriptorStore::Set(const std::string& key,
                         base::ScopedFD fd,
                         base::MemoryMappedFile::Region region)
{
