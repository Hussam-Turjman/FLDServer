...
template <typename T>
constexpr size_t
CharTraits<T>::length(const T* s) noexcept
{
    size_t i = 0;
    for (; *s; ++s)
        ++i;
...
template <typename T>
constexpr const T*
CharTraits<T>::find(const T* s, size_t n, T c)
{
    for (; n; --n, ++s)
    {
...
{
    static constexpr int
    compare(const char* s1, const char* s2, size_t n) noexcept
    {
        return __builtin_memcmp(s1, s2, n);
    }

    static constexpr size_t
    length(const char* s) noexcept
    {
...
{
    static constexpr int
    compare(const wchar_t* s1, const wchar_t* s2, size_t n) noexcept
    {
        return __builtin_wmemcmp(s1, s2, n);
    }

    static constexpr size_t
    length(const wchar_t* s) noexcept
    {
