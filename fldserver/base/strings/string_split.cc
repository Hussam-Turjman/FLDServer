...
#include "fldserver/base/strings/string_split_internal.h"
...
{
bool
AppendStringKeyValue(StringPiece input, char delimiter, StringPairs* result)
{
    ...
    result_pair.second =
            std::string(remains.substr(begin_value_pos, remains.size() - begin_value_pos));
...
{
    return internal::SplitStringT<std::string>(input, separators, whitespace, result_type);
}
...
SplitString(StringPiece16 input,
            ...
            SplitResult result_type)
{
    return internal::SplitStringT<std::u16string>(input, separators, whitespace, result_type);
}
...
SplitStringPiece(StringPiece input,
                 ...
                 SplitResult result_type)
{
    return internal::SplitStringT<StringPiece>(input, separators, whitespace, result_type);
}
...
SplitStringPiece(StringPiece16 input,
                 ...
                 SplitResult result_type)
{
    return internal::SplitStringT<StringPiece16>(input, separators, whitespace, result_type);
}

bool
SplitStringIntoKeyValuePairs(StringPiece input,
                             char key_value_delimiter,
                             char key_value_pair_delimiter,
                             StringPairs* key_value_pairs)
{
    return SplitStringIntoKeyValuePairsUsingSubstr(
            input, key_value_delimiter, StringPiece(&key_value_pair_delimiter, 1), key_value_pairs);
}

bool
SplitStringIntoKeyValuePairsUsingSubstr(StringPiece input,
                                        char key_value_delimiter,
...
std::vector<std::u16string>
SplitStringUsingSubstr(StringPiece16 input,
...
std::vector<std::string>
SplitStringUsingSubstr(StringPiece input,
...
std::vector<StringPiece>
SplitStringPieceUsingSubstr(StringPiece input,
                            StringPiece delimiter,
                            WhitespaceHandling whitespace,
                            SplitResult result_type)
{
