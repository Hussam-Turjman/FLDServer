...
class CORE_EXPORT OffsetAdjuster
{
public:
    struct CORE_EXPORT Adjustment
    {
        Adjustment(size_t original_offset, size_t original_length, size_t output_length);
    ...
    static void
    AdjustOffsets(const Adjustments& adjustments,
    ...
    // |adjustments|.
    static void
    AdjustOffset(const Adjustments& adjustments,
                 size_t* offset,
                 size_t limit = std::u16string::npos);
    ...
    // std::u16string::npos.
    static void
    UnadjustOffsets(const Adjustments& adjustments, std::vector<size_t>* offsets_for_unadjustment);

    // Adjusts the single |offset| to reflect the reverse of the adjustments
    // recorded in |adjustments|.
    static void
    UnadjustOffset(const Adjustments& adjustments, size_t* offset);
    ...
    static void
    MergeSequentialAdjustments(const Adjustments& first_adjustments,
