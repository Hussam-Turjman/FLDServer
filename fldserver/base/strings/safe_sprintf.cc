...
{
void
SetSafeSPrintfSSizeMaxForTest(size_t max)
{
    kSSizeMax = max;
}
...
// complain, but clang does.
#if __cplusplus >= 201103 && !(defined(__clang__) && IS_WIN)
    ...
    inline bool
    Pad(char pad, size_t padding, size_t len)
    {
        DEBUG_CHECK(pad);
...
bool
Buffer::IToASCII(
        bool sign, bool upcase, int64_t i, int base, char pad, size_t padding, const char* prefix)
{
    ...
    do
    {
        ...
        if (count_ >= size_)
        {
            if (start < size_)
            {
                ...
                for (char *move = buffer_ + start, *end = buffer_ + size_ - 1; move < end; ++move)
                {
                    *move = move[1];
                }
                ++discarded;
...
{
ssize_t
SafeSNPrintf(char* buf, size_t sz, const char* fmt, const Arg* args, const size_t max_args)
{
...
ssize_t
SafeSNPrintf(char* buf, size_t sz, const char* fmt)
{
    // Make sure that at least one NUL byte can be written, and that the buffer
    // never overflows kSSizeMax. Not only does that use up most or all of the
    // address space, it also would result in a return code that cannot be
