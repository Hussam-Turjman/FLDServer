...
#include "fldserver/base/strings/pattern.h"
...
{
constexpr bool
IsWildcard(base_icu::UChar32 character)
{
...
constexpr bool
SearchForChars(const CHAR** pattern,
...
template <typename CHAR, typename NEXT>
constexpr int
EatWildcards(const CHAR** pattern, const CHAR* end, NEXT next)
{
              ...
              NEXT next)
{
    do
    {
        ...
        if (!SearchForChars(&pattern, pattern_end, &eval, eval_end, maximum_wildcard_length, next))
        {
            return false;
        }
...
{
    base_icu::UChar32
    operator()(const char** p, const char* end)
    {
        base_icu::UChar32 c;
        int offset = 0;
...
{
    base_icu::UChar32
    operator()(const char16_t** p, const char16_t* end)
    {
        base_icu::UChar32 c;
        int offset = 0;
...
bool
MatchPattern(StringPiece eval, StringPiece pattern)
{
    return MatchPatternT(eval.data(),
                         eval.data() + eval.size(),
                         pattern.data(),
...
bool
MatchPattern(StringPiece16 eval, StringPiece16 pattern)
{
    return MatchPatternT(eval.data(),
                         eval.data() + eval.size(),
                         pattern.data(),
