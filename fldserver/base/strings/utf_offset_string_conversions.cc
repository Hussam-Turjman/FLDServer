...
#include "fldserver/base/strings/utf_offset_string_conversions.h"
...
OffsetAdjuster::Adjustment::Adjustment(size_t original_offset,
    ...
    original_offset(original_offset), original_length(original_length), output_length(output_length)
{
}

// static
void
OffsetAdjuster::AdjustOffsets(const Adjustments& adjustments,
...
// static
void
OffsetAdjuster::AdjustOffset(const Adjustments& adjustments, size_t* offset, size_t limit)
{
    DCHECK(offset);
...
// static
void
OffsetAdjuster::UnadjustOffsets(const Adjustments& adjustments,
...
// static
void
OffsetAdjuster::UnadjustOffset(const Adjustments& adjustments, size_t* offset)
{
    if (*offset == std::u16string::npos)
        return;
    int adjustment = 0;
...
// static
void
OffsetAdjuster::MergeSequentialAdjustments(const Adjustments& first_adjustments,
                                           Adjustments* adjustments_on_adjusted_string)
{
    ...
    while (adjusted_iter != adjustments_on_adjusted_string->end())
    {
        if ((first_iter == first_adjustments.end())
            || ((adjusted_iter->original_offset + shift + adjusted_iter->original_length)
    ...
    if (first_iter != first_adjustments.end())
    {
        ...
        adjustments_builder.insert(adjustments_builder.end(), first_iter, first_adjustments.end());
    }
    ...
    for (int32_t i = 0; i < src_len32; i++)
    {
        ...
        if (adjustments && ((i - original_i + 1) != chars_written))
        {
            adjustments->push_back(
                    OffsetAdjuster::Adjustment(original_i, i - original_i + 1, chars_written));
        }
    }
    return success;
}
...
bool
UTF8ToUTF16WithAdjustments(const char* src,
                           size_t src_len,
...
std::u16string
UTF8ToUTF16WithAdjustments(const base::StringPiece& utf8,
                           base::OffsetAdjuster::Adjustments* adjustments)
{
...
std::u16string
UTF8ToUTF16AndAdjustOffsets(const base::StringPiece& utf8,
                            std::vector<size_t>* offsets_for_adjustment)
{
...
std::string
UTF16ToUTF8AndAdjustOffsets(const base::StringPiece16& utf16,
                            std::vector<size_t>* offsets_for_adjustment)
{
    for (size_t& offset : *offsets_for_adjustment)
    {
