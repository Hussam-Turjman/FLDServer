...
static bool
CompareParameter(const ReplacementOffset& elem1, const ReplacementOffset& elem2)
{
...
inline bool
IsMachineWordAligned(const void* pointer)
{
...
ToLowerASCIIImpl(BasicStringPiece<CharT> str)
{
...
template <typename CharT>
std::basic_string<CharT>
ToUpperASCIIImpl(BasicStringPiece<CharT> str)
{
    std::basic_string<CharT> ret;
    ret.reserve(str.size());
    for (size_t i = 0; i < str.size(); i++)
...
int
CompareCaseInsensitiveASCIIT(BasicStringPiece<CharT> a, BasicStringPiece<CharT> b)
{
...
template <typename CharT>
TrimPositions
TrimStringT(BasicStringPiece<CharT> input,
            ...
            std::basic_string<CharT>* output)
{
    ...
    const size_t first_good_char = (positions & TRIM_LEADING) ? input.find_first_not_of(trim_chars)
                                                              : 0;
    const size_t last_good_char = (positions & TRIM_TRAILING) ? input.find_last_not_of(trim_chars)
                                                              : last_char;
...
TrimStringPieceT(BasicStringPiece<CharT> input,
                 BasicStringPiece<CharT> trim_chars,
                 TrimPositions positions)
{
    size_t begin = (positions & TRIM_LEADING) ? input.find_first_not_of(trim_chars) : 0;
    size_t end = (positions & TRIM_TRAILING) ? input.find_last_not_of(trim_chars) + 1
                                             : input.size();
...
template <typename CharT>
std::basic_string<CharT>
CollapseWhitespaceT(BasicStringPiece<CharT> text, bool trim_sequences_with_line_breaks)
{
    ...
    for (auto c : text)
    {
        if (IsUnicodeWhitespace(c))
        {
            ...
            if (trim_sequences_with_line_breaks && !already_trimmed && ((c == '\n') || (c == '\r')))
            {
...
template <class Char>
bool
DoIsStringASCII(const Char* characters, size_t length)
{
...
inline bool
DoIsStringUTF8(StringPiece str)
{
...
template <typename CharT>
inline bool
DoLowerCaseEqualsASCII(BasicStringPiece<CharT> str, StringPiece lowercase_ascii)
{
...
template <typename CharT>
bool
StartsWithT(BasicStringPiece<CharT> str,
            ...
            CompareCase case_sensitivity)
{
    ...
    {
        case CompareCase::SENSITIVE: return source == search_for;
...
template <typename CharT>
bool
EndsWithT(BasicStringPiece<CharT> str,
          BasicStringPiece<CharT> search_for,
          CompareCase case_sensitivity)
{
    if (search_for.size() > str.size())
    ...
    BasicStringPiece<CharT> source = str.substr(str.size() - search_for.size(), search_for.size());
...
struct SubstringMatcher
{
    ...
    size_t
    Find(const std::basic_string<CharT>& input, size_t pos)
    {
...
template <class CharT>
auto
MakeSubstringMatcher(BasicStringPiece<CharT> find_this)
{
...
template <class CharT>
auto
MakeCharacterMatcher(BasicStringPiece<CharT> find_any_of_these)
{
...
bool
DoReplaceMatchesAfterOffset(std::basic_string<CharT>* str,
...
template <class CharT>
bool
ReplaceCharsT(BasicStringPiece<CharT> input,
              BasicStringPiece<CharT> find_any_of_these,
              BasicStringPiece<CharT> replace_with,
              std::basic_string<CharT>* output)
{
    ...
    return DoReplaceMatchesAfterOffset(output,
                                       0,
...
JoinStringT(list_type parts, BasicStringPiece<CharT> sep)
{
    if (base::empty(parts))
...
template <class CharT>
std::basic_string<CharT>
DoReplaceStringPlaceholders(BasicStringPiece<CharT> format_string,
...
size_t
lcpyT(CHAR* dst, const CHAR* src, size_t dst_size)
{
