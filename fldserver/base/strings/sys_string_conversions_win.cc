...
// Do not assert in this function since it is used by the asssertion code!
std::wstring
SysUTF8ToWide(StringPiece utf8)
{
...
std::wstring
SysNativeMBToWide(StringPiece native_mb)
{
...
// Do not assert in this function since it is used by the asssertion code!
std::wstring
SysMultiByteToWide(StringPiece mb, uint32_t code_page)
{
    if (mb.empty())
        return std::wstring();
...
// Do not assert in this function since it is used by the asssertion code!
std::string
SysWideToMultiByte(const std::wstring& wide, uint32_t code_page)
{
    ...
    int charcount =
            WideCharToMultiByte(code_page, 0, wide.data(), wide_length, NULL, 0, NULL, NULL);
    if (charcount == 0)
        return std::string();
