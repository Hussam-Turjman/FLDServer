...
#include "fldserver/base/double_conversion/double-conversion.h"
...
public:
    ...
    public:
        static Result
        CheckBounds(Number value, uint8_t new_digit)
        {
        ...
        static Number
        Increment(Number lhs, uint8_t rhs)
        {
            return lhs + rhs;
        }
    };
...
auto
StringToNumber(BasicStringPiece<CharT> input)
{
...
bool
StringToIntImpl(BasicStringPiece<CharT> input, VALUE& output)
{
...
template <typename CharT, typename VALUE>
bool
HexStringToIntImpl(BasicStringPiece<CharT> input, VALUE& output)
{
...
StringT
ToString(const typename StringT::value_type* data, size_t size)
{
...
StringT
ToString(const CharT* data, size_t size)
{
...
bool
StringToDoubleImpl(STRING input, const CHAR* data, double& output)
{
    ...
    output = converter->StringToDouble(data, input.size(), &processed_characters_count);
...
static bool
HexStringToByteContainer(StringPiece input, OutIter output)
{
