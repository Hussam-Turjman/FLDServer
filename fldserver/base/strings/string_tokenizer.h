...
public:
    ...
    void
    set_quote_chars(const str& quotes)
    {
        quotes_ = quotes;
    }
    ...
    bool
    token_is_delim() const
    {
        return token_is_delim_;
    }
...
private:
    void
    Init(const_iterator string_begin,
         const_iterator string_end,
         const str& delims,
    ...
    bool
    ShouldSkip(char_type c) const
    {
        return whitespace_policy_ == WhitespacePolicy::kSkipOver && IsAsciiWhitespace(c);
    }
    ...
    QuickGetNext()
    {
        ...
        for (;;)
        {
            ...
            ++token_end_;
            if (delims_.find(*token_begin_) == str::npos && !ShouldSkip(*token_begin_))
            {
                break;
            }
    ...
    bool
    IsDelim(char_type c) const
    {
        return delims_.find(c) != str::npos;
    }

    bool
    IsQuote(char_type c) const
    {
    ...
    // hit.
    bool
    AdvanceOne(AdvanceState* state, char_type c)
    {
        if (state->in_quote)
        {
            if (state->in_escape)
...
typedef StringTokenizerT<std::u16string, std::u16string::const_iterator> String16Tokenizer;
