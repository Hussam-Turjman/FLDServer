...
{
public:
    OnceCallbackHolder(OnceCallback<void(Args...)> callback, bool ignore_extra_runs) :
    ...
    OnceCallbackHolder&
    operator=(const OnceCallbackHolder&) = delete;
...
{
    using Helper = internal::OnceCallbackHolder<Args...>;
    auto wrapped_once = base::BindRepeating(&Helper::Run,
                                            std::make_unique<Helper>(std::move(callback),
...
class CORE_EXPORT ScopedClosureRunner
{
public:
    ScopedClosureRunner();
    ...
    ScopedClosureRunner&
    operator=(ScopedClosureRunner&& other);
    ~ScopedClosureRunner();
    ...
    void
    ReplaceClosure(OnceClosure closure);
...
class CORE_EXPORT NullCallback
{
public:
    template <typename R, typename... Args>
...
public:
    ...
    // deduce it.
    template <typename... Args>
    static RepeatingCallback<void(Args...)>
    Repeatedly()
    {
        return BindRepeating([](Args... args) {});
    }
    template <typename... Args>
    static OnceCallback<void(Args...)>
    Once()
    {
