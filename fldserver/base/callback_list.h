...
class CORE_EXPORT CallbackListSubscription
{
public:
    CallbackListSubscription();
    CallbackListSubscription(CallbackListSubscription&& subscription);
    CallbackListSubscription&
    operator=(CallbackListSubscription&& subscription);
    ~CallbackListSubscription();
...
{
public:
    using CallbackType = typename CallbackListTraits<CallbackListImpl>::CallbackType;
    ...
    CallbackListBase&
    operator=(const CallbackListBase&) = delete;
    ...
    CallbackListSubscription
    Add(CallbackType cb) WARN_UNUSED_RESULT
    {
        DCHECK(!cb.is_null());
        return CallbackListSubscription(
                base::BindOnce(&CallbackListBase::CancelCallback,
    ...
    void
    set_removal_callback(const RepeatingClosure& removal_callback)
    {
    ...
    empty() const
    {
        return ranges::all_of(callbacks_, [](const auto& callback) { return callback.is_null(); });
    }
    ...
    Notify(RunArgs&&... args)
    {
        ...
        if (removal_callback_ && (erased_callbacks || IsOnceCallback<CallbackType>::value))
...
private:
    ...
    void
    CancelCallback(const typename Callbacks::iterator& it)
    {
...
template <typename Signature>
class OnceCallbackList : public internal::CallbackListBase<OnceCallbackList<Signature>>
{
private:
    ...
    template <typename... RunArgs>
    void
    RunCallback(typename Traits::Callbacks::iterator it, RunArgs&&... args)
    {
    ...
    bool
    CancelNullCallback(const typename Traits::Callbacks::iterator& it)
    {
        if (it->is_null())
        {
...
template <typename Signature>
class RepeatingCallbackList : public internal::CallbackListBase<RepeatingCallbackList<Signature>>
{
private:
