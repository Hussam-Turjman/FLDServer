...
// in place.
class CORE_EXPORT HeapHandle
{
public:
    enum : size_t
    {
        kInvalidIndex = std::numeric_limits<size_t>::max()
    };

    constexpr HeapHandle() = default;
    constexpr HeapHandle(const HeapHandle& other) = default;
    HeapHandle(HeapHandle&& other) noexcept : index_(std::exchange(other.index_, kInvalidIndex))
    {
    }
    ~HeapHandle() = default;

    HeapHandle&
    operator=(const HeapHandle& other) = default;
    HeapHandle&
    operator=(HeapHandle&& other) noexcept
    {
    ...
    friend bool
    operator==(const HeapHandle& lhs, const HeapHandle& rhs)
    {
...
{
    void
    SetHeapHandle(T* element, HeapHandle handle) const
    {
        element->SetHeapHandle(handle);
    }

    void
    ClearHeapHandle(T* element) const
    {
        element->ClearHeapHandle();
    }

    HeapHandle
    GetHeapHandle(const T* element) const
    {
...
public:
    ...
    using const_reverse_iterator = typename UnderlyingType::const_reverse_iterator;
    ...
    IntrusiveHeap&
    operator=(IntrusiveHeap&&) noexcept;
    IntrusiveHeap&
    operator=(const IntrusiveHeap&);
    IntrusiveHeap&
    ...
    {
        return impl_.heap_.at(pos);
    }
    const_reference
    at(HeapHandle pos) const
    {
    ...
    {
        return impl_.heap_[pos];
    }
    const_reference
    operator[](HeapHandle pos) const
    {
    ...
    front() const
    {
        return impl_.heap_.front();
    }
    const_reference
    back() const
    {
        return impl_.heap_.back();
    }
    const_reference
    top() const
    {
    ...
    {
        impl_.heap_.reserve(new_capacity);
    }
    size_type
    capacity() const
    {
        return impl_.heap_.capacity();
    }
    void
    shrink_to_fit()
    {
        impl_.heap_.shrink_to_fit();
    }
    ...
    size() const
    {
        return impl_.heap_.size();
    }
    size_type
    max_size() const
    {
        return impl_.heap_.max_size();
    }
    bool
    empty() const
    {
        return impl_.heap_.empty();
    }
    ...
    const_iterator
    Replace(size_type pos, const T& element)
    {
        return ReplaceImpl(pos, element);
    }
    const_iterator
    Replace(size_type pos, T&& element)
    {
    ...
    template <typename P>
    const_iterator
    Replace(P pos, const T& element)
    {
        return ReplaceImpl(ToIndex(pos), element);
    }
    template <typename P>
    const_iterator
    Replace(P pos, T&& element)
    {
    ...
    const_iterator
    ReplaceTop(const T& element)
    {
        return ReplaceTopImpl(element);
    }
    const_iterator
    ReplaceTop(T&& element)
    {
    ...
    const_iterator
    Modify(P pos, UnaryOperation unary_op)
    {
    ...
    value_comp() const
    {
        return impl_.get_value_compare();
    }

    const heap_handle_accessor&
    heap_handle_access() const
    {
    ...
    void
    swap(IntrusiveHeap& other) noexcept;
    friend void
    swap(IntrusiveHeap& lhs, IntrusiveHeap& rhs)
    {
        lhs.swap(rhs);
    }
    ...
    friend bool
    operator==(const IntrusiveHeap& lhs, const IntrusiveHeap& rhs)
    {
    ...
    size_type
    ToIndex(HeapHandle handle)
    {
        return handle.index();
    }
    size_type
    ToIndex(const_iterator pos);
    size_type
    ToIndex(const_reverse_iterator pos);
...
private:
    ...
    GetLastIndex() const
    {
        return impl_.heap_.size() - 1;
    }

    // Helper functions for setting heap handles.
    void
    SetHeapHandle(size_type i);
    void
    ClearHeapHandle(size_type i);
    HeapHandle
    GetHeapHandle(size_type i);

    // Helpers for doing comparisons between elements inside and outside of the
    // heap.
    bool
    Less(size_type i, size_type j);
    bool
    Less(const T& element, size_type i);
    bool
    Less(size_type i, const T& element);
    ...
    // hole.
    void
    MakeHole(size_type pos);
    template <typename U>
    void
    FillHole(size_type hole, U element);
    void
    MoveHole(size_type new_hole_pos, size_type old_hole_pos);

    // Moves a hold up the tree and fills it with the provided |element|. Returns
    // the final index of the element.
    template <typename U>
    size_type
    MoveHoleUpAndFill(size_type hole_pos, U element);
    ...
    size_type
    MoveHoleDownAndFill(size_type hole_pos, U element);

    // Implementation of Insert and Replace built on top of the MoveHole
    // primitives.
    template <typename U>
    const_iterator
    InsertImpl(U element);
    template <typename U>
    const_iterator
    ReplaceImpl(size_type pos, U element);
    template <typename U>
    const_iterator
    ReplaceTopImpl(U element);
    ...
    {
        Impl(const value_compare& value_comp, const heap_handle_accessor& heap_handle_access) :
        ...
        const heap_handle_accessor&
        get_heap_handle_access() const
        {
            return *this;
        }
        heap_handle_accessor&
        get_heap_handle_access()
        {
            return *this;
        }
...
class CORE_EXPORT InternalHeapHandleStorage
{
public:
    ...
    InternalHeapHandleStorage&
    operator=(const InternalHeapHandleStorage&) = delete;
    InternalHeapHandleStorage&
    operator=(InternalHeapHandleStorage&& other) noexcept;
    ...
    // as possible.
    void
    SetHeapHandle(HeapHandle handle)
    {
    ...
    // Utility functions.
    void
    swap(InternalHeapHandleStorage& other) noexcept;
    friend void
    swap(InternalHeapHandleStorage& lhs, InternalHeapHandleStorage& rhs)
    {
        lhs.swap(rhs);
    }
...
public:
    ...
    WithHeapHandle&
    operator=(const WithHeapHandle&) = delete;
    WithHeapHandle&
    operator=(WithHeapHandle&& other) = default;
    ...
    // Utility functions.
    void
    swap(WithHeapHandle& other) noexcept;
    friend void
    swap(WithHeapHandle& lhs, WithHeapHandle& rhs)
    {
        lhs.swap(rhs);
    }
    ...
    friend bool
    operator==(const WithHeapHandle& lhs, const WithHeapHandle& rhs)
    {
...
{
CORE_EXPORT inline size_t
ParentIndex(size_t i)
{
    DCHECK_NE(0u, i);
    return (i - 1) / 2;
}
...
bool
IsInvalid(const HandleType& handle)
{
    return !handle || !handle->IsValid();
}

CORE_EXPORT inline void
CheckInvalidOrEqualTo(HeapHandle handle, size_t index)
{
    if (handle.IsValid())
...
template <typename T, typename Compare, typename HeapHandleAccessor>
IntrusiveHeap<T, Compare, HeapHandleAccessor>::IntrusiveHeap(const IntrusiveHeap& other) :
    impl_(other.impl_)
{
...
IntrusiveHeap<T, Compare, HeapHandleAccessor>::operator=(IntrusiveHeap&& other) noexcept
{
    clear();
...
template <typename T, typename Compare, typename HeapHandleAccessor>
IntrusiveHeap<T, Compare, HeapHandleAccessor>&
IntrusiveHeap<T, Compare, HeapHandleAccessor>::operator=(const IntrusiveHeap& other)
{
    clear();
    impl_ = other.impl_;
    for (size_t i = 0; i < size(); ++i)
...
template <typename T, typename Compare, typename HeapHandleAccessor>
IntrusiveHeap<T, Compare, HeapHandleAccessor>&
IntrusiveHeap<T, Compare, HeapHandleAccessor>::operator=(std::initializer_list<value_type> ilist)
{
    clear();
    insert(std::begin(ilist), std::end(ilist));
}
...
template <typename T, typename Compare, typename HeapHandleAccessor>
void
IntrusiveHeap<T, Compare, HeapHandleAccessor>::clear()
{
...
template <class InputIterator>
void
IntrusiveHeap<T, Compare, HeapHandleAccessor>::insert(InputIterator first, InputIterator last)
{
...
IntrusiveHeap<T, Compare, HeapHandleAccessor>::take(size_type pos)
{
    ...
    if (pos != GetLastIndex())
    {
        ...
        MoveHoleDownAndFill<WithLeafElement>(pos, std::move(impl_.heap_[GetLastIndex()]));
    }
...
template <typename T, typename Compare, typename HeapHandleAccessor>
void
IntrusiveHeap<T, Compare, HeapHandleAccessor>::erase(size_type pos)
{
    DCHECK_LT(pos, size());
    // Make a hole by taking the element out of the heap.
    MakeHole(pos);
    ...
    if (pos != GetLastIndex())
    {
        ...
        // Move the hole down the heap, filling it with the current leaf at the
        // very end of the heap.
        MoveHoleDownAndFill<WithLeafElement>(pos,
                                             std::move_if_noexcept(impl_.heap_[GetLastIndex()]));
    }
...
IntrusiveHeap<T, Compare, HeapHandleAccessor>::Update(size_type pos)
{
    ...
    {
        i = MoveHoleDownAndFill<WithElement>(pos, std::move_if_noexcept(impl_.heap_[pos]));
    }
...
template <typename T, typename Compare, typename HeapHandleAccessor>
void
IntrusiveHeap<T, Compare, HeapHandleAccessor>::swap(IntrusiveHeap& other) noexcept
{
    std::swap(impl_.get_value_compare(), other.impl_.get_value_compare());
    std::swap(impl_.get_heap_handle_access(), other.impl_.get_heap_handle_access());
...
template <typename T, typename Compare, typename HeapHandleAccessor>
typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::size_type
IntrusiveHeap<T, Compare, HeapHandleAccessor>::ToIndex(const_reverse_iterator pos)
{
    DCHECK(crbegin() <= pos);
    DCHECK(pos <= crend());
    if (pos == crend())
...
template <typename T, typename Compare, typename HeapHandleAccessor>
void
IntrusiveHeap<T, Compare, HeapHandleAccessor>::SetHeapHandle(size_type i)
{
...
template <typename T, typename Compare, typename HeapHandleAccessor>
void
IntrusiveHeap<T, Compare, HeapHandleAccessor>::ClearHeapHandle(size_type i)
{
...
template <typename T, typename Compare, typename HeapHandleAccessor>
HeapHandle
IntrusiveHeap<T, Compare, HeapHandleAccessor>::GetHeapHandle(size_type i)
{
...
template <typename T, typename Compare, typename HeapHandleAccessor>
bool
IntrusiveHeap<T, Compare, HeapHandleAccessor>::Less(size_type i, size_type j)
{
    DCHECK_LT(i, size());
    DCHECK_LT(j, size());
...
template <typename T, typename Compare, typename HeapHandleAccessor>
bool
IntrusiveHeap<T, Compare, HeapHandleAccessor>::Less(const T& element, size_type i)
{
    DCHECK_LT(i, size());
...
template <typename T, typename Compare, typename HeapHandleAccessor>
bool
IntrusiveHeap<T, Compare, HeapHandleAccessor>::Less(size_type i, const T& element)
{
    DCHECK_LT(i, size());
...
template <typename T, typename Compare, typename HeapHandleAccessor>
void
IntrusiveHeap<T, Compare, HeapHandleAccessor>::MakeHole(size_type pos)
{
    DCHECK_LT(pos, size());
    ClearHeapHandle(pos);
}
...
template <typename U>
void
IntrusiveHeap<T, Compare, HeapHandleAccessor>::FillHole(size_type hole_pos, U element)
{
...
template <typename T, typename Compare, typename HeapHandleAccessor>
void
IntrusiveHeap<T, Compare, HeapHandleAccessor>::MoveHole(size_type new_hole_pos,
                                                        size_type old_hole_pos)
{
    ...
    {
        impl_.heap_[old_hole_pos] = std::move_if_noexcept(impl_.heap_[new_hole_pos]);
    }
...
template <typename U>
typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::size_type
IntrusiveHeap<T, Compare, HeapHandleAccessor>::MoveHoleUpAndFill(size_type hole_pos, U element)
{
...
template <typename FillElementType, typename U>
typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::size_type
IntrusiveHeap<T, Compare, HeapHandleAccessor>::MoveHoleDownAndFill(size_type hole_pos, U element)
{
...
template <typename U>
typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::const_iterator
IntrusiveHeap<T, Compare, HeapHandleAccessor>::ReplaceImpl(size_type pos, U element)
{
...
{
    MakeHole(0u);
    size_type i = MoveHoleDownAndFill<WithElement>(0u, std::move_if_noexcept(element));
    return cbegin() + i;
}
...
template <typename T>
template <class... Args>
WithHeapHandle<T>::WithHeapHandle(Args&&... args) : value_(std::forward<Args>(args)...)
{
}

template <typename T>
void
WithHeapHandle<T>::swap(WithHeapHandle& other) noexcept
{
