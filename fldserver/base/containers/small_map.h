...
struct select_equal_key
{
    ...
    {
        bool
        operator()(const typename M::key_type& left, const typename M::key_type& right)
        {
...
template <typename NormalMap,
          size_t kArraySize = 4,
          typename EqualKey = typename internal::
                  select_equal_key<NormalMap, internal::has_key_equal<NormalMap>::value>::equal_key,
          typename MapInit = internal::small_map_default_init<NormalMap>>
class small_map
{
    static_assert(kArraySize > 0, "Initial size must be greater than 0");
    static_assert(kArraySize != kUsingFullMapSentinel, "Initial size out of range");
...
public:
    ...
    private:
        friend class small_map;
        inline explicit const_iterator(const value_type* init) : array_iter_(init)
        {
        }
    ...
    void
    insert(InputIterator f, InputIterator l)
    {
        while (f != l)
        {
            insert(*f);
    ...
    emplace(Args&&... args)
    {
        ...
        {
            ConvertToRealMap();  // Invalidates all iterators!
            std::pair<typename NormalMap::iterator, bool> ret = map_.emplace(std::move(x));
            return std::make_pair(iterator(ret.first), ret.second);
        }
    ...
    begin() const
    {
        return UsingFullMap() ? const_iterator(map_.begin()) : const_iterator(array_);
    }
    ...
    {
        return UsingFullMap() ? const_iterator(map_.end()) : const_iterator(array_ + size_);
    }
    ...
    size() const
    {
        return UsingFullMap() ? map_.size() : size_;
    }

    bool
    empty() const
    {
        return UsingFullMap() ? map_.empty() : size_ == 0;
    }
    ...
    UsingFullMap() const
    {
        return size_ == kUsingFullMapSentinel;
    }
...
template <typename NormalMap, size_t kArraySize, typename EqualKey, typename Functor>
inline bool
small_map<NormalMap, kArraySize, EqualKey, Functor>::iterator::operator==(
