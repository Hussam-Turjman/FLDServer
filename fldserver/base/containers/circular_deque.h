...
#include "fldserver/base/containers/vector_buffer.h"
...
public:
    ...
    operator+(const circular_deque_const_iterator& iter, difference_type offset)
    {
    ...
    operator-(const circular_deque_const_iterator& lhs, const circular_deque_const_iterator& rhs)
    {
...
protected:
    ...
    void
    CheckComparable(const circular_deque_const_iterator& other) const
    {
    ...
    inline void
    CheckComparable(const circular_deque_const_iterator&) const
    {
    }
#endif  // DCHECK_IS_ON()
...
public:
    ...
    // Random access mutation.
    friend circular_deque_iterator
    operator+(const circular_deque_iterator& iter, difference_type offset)
    {
...
public:
    ...
    circular_deque(size_type count, const T& value)
    {
        resize(count, value);
    }
    ...
    circular_deque&
    operator=(const circular_deque& other)
    {
        if (&other == this)
            return *this;
    ...
    circular_deque&
    operator=(circular_deque&& other) noexcept
    {
        if (&other == this)
            return *this;
    ...
    typename std::enable_if<::base::internal::is_iterator<InputIterator>::value, void>::type
    ...
    void
    assign(std::initializer_list<value_type> value)
    {
    ...
    friend void
    swap(circular_deque& lhs, circular_deque& rhs)
    {
        lhs.swap(rhs);
    }
...
private:
    ...
    static void
    MoveBuffer(VectorBuffer& from_buf,
               size_t from_begin,
               size_t from_end,
               VectorBuffer* to_buf,
               size_t* to_begin,
    ...
    void
    SetCapacityTo(size_t new_capacity)
    {
    ...
    void
    ExpandCapacityIfNecessary(size_t additional_elts)
    {
    ...
    // not changed.
    void
    DestructRange(size_t begin, size_t end)
    {
        if (end == begin)
        {
            return;
    ...
    void
    MakeRoomFor(size_t count, iterator* insert_begin, iterator* insert_end)
    {
        if (count == 0)
        {
        ...
        *insert_end = iterator(this, (insert_begin->index_ + count) % buffer_.capacity());
        ...
        while (true)
        {
            ...
            --dest;
            buffer_.MoveRange(
                    &buffer_[src.index_], &buffer_[src.index_ + 1], &buffer_[dest.index_]);
        }
    }
    ...
    // begin.
    void
    CheckValidIndex(size_t i) const
    {
        if (begin_ <= end_)
    ...
    void
    CheckValidIndexOrEnd(size_t i) const
    {
        if (i != end_)
            CheckValidIndex(i);
    }
    ...
    void
    ValidateIterator(const const_iterator& i) const
    {
...
size_t
Erase(circular_deque<T>& container, const Value& value)
{
...
size_t
EraseIf(circular_deque<T>& container, Predicate pred)
{
