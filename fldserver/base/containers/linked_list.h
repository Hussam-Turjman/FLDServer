...
class CORE_EXPORT LinkNodeBase
{
public:
    void
    RemoveFromList();
...
protected:
    ...
    LinkNodeBase&
    operator=(const LinkNodeBase&) = delete;
    ...
    // ensure safety.
    void
    InsertBeforeBase(LinkNodeBase* e);
    void
    InsertAfterBase(LinkNodeBase* e);
...
public:
    LinkNode() = default;
    LinkNode(LinkNode<T>* previous, LinkNode<T>* next) : internal::LinkNodeBase(previous, next)
    {
    }
    ...
    next() const
    {
        return static_cast<LinkNode<T>*>(next_base());
    }
