...
struct UniquePtrComparator
{
    ...
    bool
    operator()(const std::unique_ptr<T, Deleter>& lhs, const std::unique_ptr<T, Deleter>& rhs) const
    {
        return lhs < rhs;
    }
    ...
    template <typename T, class Deleter = std::default_delete<T>>
    bool
    operator()(const T* lhs, const std::unique_ptr<T, Deleter>& rhs) const
    {
        return lhs < rhs.get();
    }
    ...
    template <typename T, class Deleter = std::default_delete<T>>
    bool
    operator()(const std::unique_ptr<T, Deleter>& lhs, const T* rhs) const
    {
        return lhs.get() < rhs;
    }
};
...
struct UniquePtrMatcher
{
    ...
    bool
    operator()(const std::unique_ptr<T, Deleter>& o)
    {
        return o.get() == t_;
    }
...
template <class T, class Deleter = std::default_delete<T>>
UniquePtrMatcher<T, Deleter>
MatchesUniquePtr(T* t)
{
