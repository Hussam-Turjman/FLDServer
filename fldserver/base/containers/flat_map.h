...
#include "fldserver/base/containers/flat_tree.h"
...
struct GetFirst
{
    ...
    constexpr const Key&
    operator()(const std::pair<Key, Mapped>& p) const
    {
        return p.first;
    }
};
...
public:
    ...
    template <class K, class M>
    iterator
    insert_or_assign(const_iterator hint, K&& key, M&& obj);

    template <class K, class... Args>
    std::enable_if_t<std::is_constructible<key_type, K&&>::value, std::pair<iterator, bool>>
    ...
    void
    swap(flat_map& other) noexcept;

    friend void
    swap(flat_map& lhs, flat_map& rhs) noexcept
    {
        lhs.swap(rhs);
    }
};
...
template <class K>
auto
flat_map<Key, Mapped, Compare, Container>::at(const K& key) -> mapped_type&
{
...
template <class K>
auto
flat_map<Key, Mapped, Compare, Container>::at(const K& key) const -> const mapped_type&
{
...
template <class Key, class Mapped, class Compare, class Container>
auto
flat_map<Key, Mapped, Compare, Container>::operator[](const key_type& key) -> mapped_type&
{
...
template <class Key, class Mapped, class Compare, class Container>
auto
flat_map<Key, Mapped, Compare, Container>::operator[](key_type&& key) -> mapped_type&
{
    iterator found = tree::lower_bound(key);
    if (found == tree::end() || tree::key_comp()(key, found->first))
...
template <class K, class M>
auto
flat_map<Key, Mapped, Compare, Container>::insert_or_assign(K&& key, M&& obj)
        -> std::pair<iterator, bool>
{
...
template <class K, class M>
auto
flat_map<Key, Mapped, Compare, Container>::insert_or_assign(const_iterator hint, K&& key, M&& obj)
        -> iterator
{
    auto result =
...
template <class K, class... Args>
auto
flat_map<Key, Mapped, Compare, Container>::try_emplace(K&& key, Args&&... args)
        -> std::enable_if_t<std::is_constructible<key_type, K&&>::value, std::pair<iterator, bool>>
{
    return tree::emplace_key_args(key,
...
template <class K, class... Args>
auto
flat_map<Key, Mapped, Compare, Container>::try_emplace(const_iterator hint, K&& key, Args&&... args)
        -> std::enable_if_t<std::is_constructible<key_type, K&&>::value, iterator>
{
    return tree::emplace_hint_key_args(hint,
                                       key,
                                       std::piecewise_construct,
                                       std::forward_as_tuple(std::forward<K>(key)),
                                       std::forward_as_tuple(std::forward<Args>(args)...))
...
template <class Key, class Mapped, class Compare, class Container>
void
flat_map<Key, Mapped, Compare, Container>::swap(flat_map& other) noexcept
{
    tree::swap(other);
}
