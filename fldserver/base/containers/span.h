...
#include "fldserver/base/containers/contiguous_iterator.h"
...
using EnableIfCompatibleContiguousIterator =
        std::enable_if_t<conjunction<IsContiguousIterator<Iter>,
...
using ContainerHasIntegralSize = std::is_integral<decltype(base::size(std::declval<Container>()))>;
...
{
    static_assert(kExtent != dynamic_extent,
...
public:
    ...
    template <typename U,
              size_t N,
              typename = internal::EnableIfSpanCompatibleArray<std::array<U, N>&, T, Extent>>
    constexpr span(std::array<U, N>& array) noexcept : span(base::data(array), N)
    {
    }

    template <typename U,
              size_t N,
              typename = internal::EnableIfSpanCompatibleArray<const std::array<U, N>&, T, Extent>>
    constexpr span(const std::array<U, N>& array) noexcept : span(base::data(array), N)
    {
    }
    ...
    template <typename Container,
              typename = internal::
                      EnableIfSpanCompatibleContainerAndSpanIsDynamic<Container&, T, Extent>>
    ...
    template <typename Container,
              typename = internal::
                      EnableIfSpanCompatibleContainerAndSpanIsDynamic<const Container&, T, Extent>>
    ...
    template <typename U,
              size_t OtherExtent,
              typename = internal::EnableIfLegalSpanConversion<U, OtherExtent, T, Extent>>
...
constexpr auto
make_span(It it, EndOrSize end_or_size) noexcept
{
...
constexpr auto
make_span(Container&& container) noexcept
{
    using T = std::remove_pointer_t<decltype(base::data(std::declval<Container>()))>;
...
template <size_t N, int&... ExplicitArgumentBarrier, typename It, typename EndOrSize>
constexpr auto
make_span(It it, EndOrSize end_or_size) noexcept
{
    using T = std::remove_reference_t<iter_reference_t<It>>;
...
//     preferred.
#define EXTENT(x)                                                                                  \
    ::base::internal::must_not_be_dynamic_extent<decltype(::base::make_span(x))::extent>()
