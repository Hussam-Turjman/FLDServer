...
constexpr auto
ContainsImpl(const Container& container, const Value& value, priority_tag<2>)
...
template <typename Container, typename Value>
constexpr auto
ContainsImpl(const Container& container, const Value& value, priority_tag<1>)
...
template <typename Container, typename Value>
constexpr bool
ContainsImpl(const Container& container, const Value& value, priority_tag<0>)
{
    static_assert(!HasKeyType<Container>::value,
...
template <typename Container, typename Value>
constexpr bool
Contains(const Container& container, const Value& value)
{
...
constexpr bool
Contains(const Container& container, const Value& value, Proj proj)
{
    return ranges::find(container, value, std::move(proj)) != ranges::end(container);
}
