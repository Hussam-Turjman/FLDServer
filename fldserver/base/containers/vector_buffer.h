...
#include "fldserver/base/containers/util.h"
...
public:
    ...
    VectorBuffer(size_t count) :
        buffer_(reinterpret_cast<T*>(malloc(CheckMul(sizeof(T), count).ValueOrDie()))),
        capacity_(count)
    {
    }
    ...
    VectorBuffer&
    operator=(const VectorBuffer&) = delete;
    ...
    template <typename T2 = T,
              typename std::enable_if<base::is_trivially_copyable<T2>::value, int>::type = 0>
    static void
    MoveRange(T* from_begin, T* from_end, T* to)
    {
        CHECK(!RangesOverlap(from_begin, from_end, to))
                // TODO(crbug.com/1172816): Remove logging once root cause is found.
                << std::hex << "from_begin: 0x" << get_uintptr(from_begin) << ", from_end: 0x"
                << get_uintptr(from_end) << ", to: 0x" << get_uintptr(to);
...
private:
    static bool
    RangesOverlap(const T* from_begin, const T* from_end, const T* to)
    {
        ...
        return !(
                to >= from_end
                || CheckAdd(to_uintptr, CheckSub(from_end_uintptr, from_begin_uintptr)).ValueOrDie()
                           <= from_begin_uintptr);
    }
