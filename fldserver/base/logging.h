...
#include "fldserver/base/dcheck_is_on.h"
#include "fldserver/base/scoped_clear_last_error.h"
...
#elif IS_WIN
    LOG_DEFAULT = LOG_TO_SYSTEM_DEBUG_LOG | LOG_TO_STDERR /*LOG_TO_FILE*/,
#elif IS_POSIX
...
enum class CORE_EXPORT LogFormat
{
    LOG_FORMAT_CHROME,
    LOG_FORMAT_SYSLOG
};
...
struct CORE_EXPORT LoggingSettings
{
...
// twice.
inline bool
InitLogging(const LoggingSettings& settings)
{
...
CORE_EXPORT int
GetVlogLevelHelper(const char* file_start, size_t N);

// Gets the current vlog level for the given file (usually taken from __FILE__).
template <size_t N>
int
GetVlogLevel(const char (&file)[N])
{
...
using LogAssertHandlerFunction = base::RepeatingCallback<void(const char* file,
                                                              int line,
                                                              const base::StringPiece message,
                                                              const base::StringPiece stack_trace)>;

class CORE_EXPORT ScopedLogAssertHandler
{
public:
    ...
    operator=(const ScopedLogAssertHandler&) = delete;
    ~ScopedLogAssertHandler();
};
...
CORE_EXPORT LogMessageHandlerFunction
...
#define COMPACT_GOOGLE_LOG_EX_INFO(ClassName, ...)                                                 \
    ::logging::ClassName(__FILE__, __LINE__, ::logging::LOGGING_INFO, ##__VA_ARGS__)
#define COMPACT_GOOGLE_LOG_EX_WARNING(ClassName, ...)                                              \
    ::logging::ClassName(__FILE__, __LINE__, ::logging::LOGGING_WARNING, ##__VA_ARGS__)
#define COMPACT_GOOGLE_LOG_EX_ERROR(ClassName, ...)                                                \
    ::logging::ClassName(__FILE__, __LINE__, ::logging::LOGGING_ERROR, ##__VA_ARGS__)
#define COMPACT_GOOGLE_LOG_EX_FATAL(ClassName, ...)                                                \
    ::logging::ClassName(__FILE__, __LINE__, ::logging::LOGGING_FATAL, ##__VA_ARGS__)
#define COMPACT_GOOGLE_LOG_EX_DFATAL(ClassName, ...)                                               \
    ::logging::ClassName(__FILE__, __LINE__, ::logging::LOGGING_DFATAL, ##__VA_ARGS__)
#define COMPACT_GOOGLE_LOG_EX_DCHECK(ClassName, ...)                                               \
    ::logging::ClassName(__FILE__, __LINE__, ::logging::LOGGING_DCHECK, ##__VA_ARGS__)
...
// may be slow.
#define VLOG_IS_ON(verboselevel) ((verboselevel) <= ::logging::GetVlogLevel(__FILE__))
...
#define LOG_ASSERT(condition)                                                                      \
    LOG_IF(FATAL, !(ANALYZER_ASSUME_TRUE(condition))) << "Assert failed: " #condition ". "

#if IS_WIN
#define PLOG_STREAM(severity)                                                                      \
    COMPACT_GOOGLE_LOG_EX_##severity(Win32ErrorLogMessage, ::logging::GetLastSystemErrorCode())    \
            .stream()
#elif IS_POSIX || defined(OS_FUCHSIA)
#define PLOG_STREAM(severity)                                                                      \
...
CORE_EXPORT extern std::ostream* g_swallow_stream;
...
CORE_EXPORT extern LogSeverity LOGGING_DCHECK;
#else
...
public:
    ...
    LogMessage&
    operator=(const LogMessage&) = delete;
    virtual ~LogMessage();
...
#if 0
    void
    InitWithSyslogPrefix(base::StringPiece filename,
                         int line,
                         uint64_t tick_count,
...
operator<<(std::ostream& out, const wchar_t* wstr);
CORE_EXPORT std::ostream&
operator<<(std::ostream& out, const std::wstring& wstr);

CORE_EXPORT std::ostream&
operator<<(std::ostream& out, const char16_t* str16);
CORE_EXPORT std::ostream&
operator<<(std::ostream& out, const std::u16string& str16);
}  // namespace std
