...
#include "fldserver/base/supports_user_data.h"

namespace base
{
std::unique_ptr<SupportsUserData::Data>
SupportsUserData::Data::Clone()
{
    return nullptr;
}
...
SupportsUserData&
SupportsUserData::operator=(SupportsUserData&&) = default;

SupportsUserData::Data*
SupportsUserData::GetUserData(const void* key) const
{
...
void
SupportsUserData::SetUserData(const void* key, std::unique_ptr<Data> data)
{
    DCHECK(sequence_checker_.CalledOnValidSequence());
    // Avoid null keys; they are too vulnerable to collision.
    DCHECK(key);
...
void
SupportsUserData::RemoveUserData(const void* key)
{
    DCHECK(sequence_checker_.CalledOnValidSequence());
...
void
SupportsUserData::DetachFromSequence()
{
    sequence_checker_.DetachFromSequence();
}

void
SupportsUserData::CloneDataFrom(const SupportsUserData& other)
{
...
void
SupportsUserData::ClearAllUserData()
{
    DCHECK(sequence_checker_.CalledOnValidSequence());
    user_data_.clear();
}
