...
#include "fldserver/base/numerics/safe_math.h"
...
public:
    ...
    static constexpr TimeDelta
    FromSeconds(int64_t secs);
    static constexpr TimeDelta
    FromMillisecondsD(double ms);
    static constexpr TimeDelta
    FromMilliseconds(int64_t ms);
    static constexpr TimeDelta
    FromMicrosecondsD(double us);
    static constexpr TimeDelta
    FromMicroseconds(int64_t us);
    static constexpr TimeDelta
    FromNanosecondsD(double ns);
    static constexpr TimeDelta
    FromNanoseconds(int64_t ns);
    ...
    static TimeDelta
    FromWinrtDateTime(ABI::Windows::Foundation::DateTime dt);
...
#if defined(OS_FUCHSIA)
    static TimeDelta
    FromZxDuration(zx_duration_t nanos);
#endif
#if defined(OS_MAC)
    static TimeDelta
    FromMachTime(uint64_t mach_time);
    ...
    // appropriate.
    static Optional<TimeDelta>
    FromString(StringPiece duration_string);
    ...
    static constexpr TimeDelta
    FromInternalValue(int64_t delta)
    {
    ...
    double
    InMicrosecondsF() const;
    constexpr int64_t
    InNanoseconds() const;

    // Computations with other deltas.
    constexpr TimeDelta
    operator+(TimeDelta other) const;
    constexpr TimeDelta
    operator-(TimeDelta other) const;
    ...
    TimeDelta
    CeilToMultiple(TimeDelta interval) const;
    TimeDelta
    FloorToMultiple(TimeDelta interval) const;
    TimeDelta
    RoundToMultiple(TimeDelta interval) const;
...
operator<<(std::ostream& os, TimeDelta time_delta);
...
public:
    ...
    constexpr TimeClass
    operator+(TimeDelta delta) const
    {
        return TimeClass((TimeDelta::FromMicroseconds(us_) + delta).InMicroseconds());
    }
    constexpr TimeClass
    operator-(TimeDelta delta) const
    {
...
template <class TimeClass>
inline constexpr TimeClass
operator+(TimeDelta delta, TimeClass t)
{
    return t + delta;
}
...
class CORE_EXPORT Time : public time_internal::TimeBase<Time>
{
public:
...
// UTCExplode() and LocalExplode(). See method comments for further details.
    ...
    static Time
    FromDeltaSinceWindowsEpoch(TimeDelta delta);
    TimeDelta
    ToDeltaSinceWindowsEpoch() const;
    ...
    static Time
    FromJavaTime(int64_t ms_since_epoch);
    int64_t
    ToJavaTime() const;
    ...
    static void
    EnableHighResolutionTimer(bool enable);
    ...
    static bool
    ActivateHighResolutionTimer(bool activate);
    ...
    static bool
    FromUTCExploded(const Exploded& exploded, Time* time) WARN_UNUSED_RESULT
    {
        return FromExploded(false, exploded, time);
    }
    static bool
    FromLocalExploded(const Exploded& exploded, Time* time) WARN_UNUSED_RESULT
    {
    ...
    static bool
    FromString(const char* time_string, Time* parsed_time) WARN_UNUSED_RESULT
    {
        return FromStringInternal(time_string, true, parsed_time);
    }
    static bool
    FromUTCString(const char* time_string, Time* parsed_time) WARN_UNUSED_RESULT
    {
    ...
    void
    UTCExplode(Exploded* exploded) const
    {
        Explode(false, exploded);
    }
    void
    LocalExplode(Exploded* exploded) const
    {
        Explode(true, exploded);
    }
...
private:
    ...
    // |is_local = false|.
    void
    Explode(bool is_local, Exploded* exploded) const;
    ...
    static bool
    FromExploded(bool is_local, const Exploded& exploded, Time* time) WARN_UNUSED_RESULT;
    ...
    static void
    ExplodeUsingIcu(int64_t millis_since_unix_epoch, bool is_local, Exploded* exploded);
    static bool
    FromExplodedUsingIcu(bool is_local,
                         const Exploded& exploded,
                         int64_t* millis_since_unix_epoch) WARN_UNUSED_RESULT;

    // Rounds down the time to the nearest day in either local time
    // |is_local = true| or UTC |is_local = false|.
    Time
    Midnight(bool is_local) const;
...
{
    return (days == std::numeric_limits<int>::max()) ? Max()
...
{
    return TimeDelta(saturated_cast<int64_t>(secs * Time::kMicrosecondsPerSecond));
}

// static
constexpr TimeDelta
TimeDelta::FromSeconds(int64_t secs)
{
...
{
    return TimeDelta(saturated_cast<int64_t>(ms * Time::kMicrosecondsPerMillisecond));
}

// static
constexpr TimeDelta
TimeDelta::FromMilliseconds(int64_t ms)
{
    return TimeDelta(int64_t{base::ClampMul(ms, Time::kMicrosecondsPerMillisecond)});
}

// static
constexpr TimeDelta
TimeDelta::FromMicrosecondsD(double us)
{
...
// static
constexpr TimeDelta
TimeDelta::FromMicroseconds(int64_t us)
{
    return TimeDelta(us);
}
...
// static
constexpr TimeDelta
TimeDelta::FromNanosecondsD(double ns)
{
    return TimeDelta(saturated_cast<int64_t>(ns / Time::kNanosecondsPerMicrosecond));
}
...
// static
constexpr TimeDelta
TimeDelta::FromHz(double frequency)
{
    return FromSeconds(1) / frequency;
}

constexpr int
TimeDelta::InHours() const
{
...
// For logging use only.
CORE_EXPORT std::ostream&
operator<<(std::ostream& os, Time time);

// TimeTicks ------------------------------------------------------------------

// Represents monotonically non-decreasing clock time.
class CORE_EXPORT TimeTicks : public time_internal::TimeBase<TimeTicks>
{
public:
    ...
    static bool
    IsHighResolution() WARN_UNUSED_RESULT;
    ...
    static bool
    IsConsistentAcrossProcesses() WARN_UNUSED_RESULT;

#if defined(OS_FUCHSIA)
    // Converts between TimeTicks and an ZX_CLOCK_MONOTONIC zx_time_t value.
    static TimeTicks
    FromZxTime(zx_time_t nanos_since_boot);
    zx_time_t
    ToZxTime() const;
#endif
...
#if defined(OS_MAC)
    static TimeTicks
    FromMachAbsoluteTime(uint64_t mach_absolute_time);
#endif  // defined(OS_MAC)
    ...
    static TimeTicks
    FromUptimeMillis(int64_t uptime_millis_value);
#endif
    ...
    TimeTicks
    SnappedToNextTick(TimeTicks tick_phase, TimeDelta tick_interval) const;
...
// For logging use only.
CORE_EXPORT std::ostream&
operator<<(std::ostream& os, TimeTicks time_ticks);

// ThreadTicks ----------------------------------------------------------------

// Represents a clock, specific to a particular thread, than runs only while the
// thread is running.
class CORE_EXPORT ThreadTicks : public time_internal::TimeBase<ThreadTicks>
{
public:
    ...
    static bool
    IsSupported() WARN_UNUSED_RESULT
    {
#if (defined(_POSIX_THREAD_CPUTIME) && (_POSIX_THREAD_CPUTIME >= 0)) || defined(OS_MAC)            \
    ...
    // method as well.
    static ThreadTicks
    GetForThread(const PlatformThreadHandle& thread_handle);
#endif
...
private:
    ...
    static bool
    IsSupportedWin() WARN_UNUSED_RESULT;
    static void
    WaitUntilInitializedWin();
#endif
};
...
// For logging use only.
CORE_EXPORT std::ostream&
operator<<(std::ostream& os, ThreadTicks time_ticks);
