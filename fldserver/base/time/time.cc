...
#include "fldserver/base/nspr/prtime.h"
...
bool
ConsumePrefix(StringPiece& str, StringPiece expected)
{
...
constexpr Optional<ParsedDecimal>
ConsumeDurationNumber(StringPiece& number_string)
{
    ParsedDecimal res;
...
Optional<TimeDelta>
ConsumeDurationUnit(StringPiece& unit_string)
{
...
// static
Optional<TimeDelta>
TimeDelta::FromString(StringPiece duration_string)
{
    int sign = 1;
...
double
TimeDelta::InMillisecondsF() const
{
    if (!is_inf())
...
TimeDelta::InMilliseconds() const
{
    ...
    return (delta_ < 0) ? std::numeric_limits<int64_t>::min() : std::numeric_limits<int64_t>::max();
}
...
double
TimeDelta::InMicrosecondsF() const
{
    if (!is_inf())
...
// static
Time
Time::FromDeltaSinceWindowsEpoch(TimeDelta delta)
{
    return Time(delta.InMicroseconds());
}

TimeDelta
Time::ToDeltaSinceWindowsEpoch() const
{
...
Time::FromTimeT(time_t tt)
{
    ...
    return (tt == std::numeric_limits<time_t>::max()) ? Max()
...
Time::ToTimeT() const
{
    ...
    if (!is_inf() && ((std::numeric_limits<int64_t>::max() - kTimeTToMicrosecondsOffset) > us_))
...
Time::FromDoubleT(double dt)
{
    ...
    return (dt == 0 || std::isnan(dt)) ? Time() : (UnixEpoch() + TimeDelta::FromSecondsD(dt));
}

double
Time::ToDoubleT() const
{
    if (is_null())
        return 0;  // Preserve 0 so we can tell it doesn't exist.
    if (!is_inf())
...
// static
Time
Time::FromTimeSpec(const timespec& ts)
{
...
double
Time::ToJsTimeIgnoringNull() const
{
...
// static
bool
Time::FromStringInternal(const char* time_string, bool is_local, Time* parsed_time)
{
    DCHECK(time_string);
    DCHECK(parsed_time);
...
// static
bool
Time::ExplodedMostlyEquals(const Exploded& lhs, const Exploded& rhs)
{
    return std::tie(lhs.year,
                    lhs.month,
                    lhs.day_of_month,
                    lhs.hour,
                    lhs.minute,
...
// static
bool
Time::FromMillisecondsSinceUnixEpoch(int64_t unix_milliseconds, Time* time)
{
...
int64_t
Time::ToRoundedDownMillisecondsSinceUnixEpoch() const
{
    constexpr int64_t kEpochOffsetMillis = kTimeTToMicrosecondsOffset / kMicrosecondsPerMillisecond;
...
std::ostream&
operator<<(std::ostream& os, ThreadTicks thread_ticks)
{
...
bool
Time::Exploded::HasValidValues() const
{
    // clang-format off
