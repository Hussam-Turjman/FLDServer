...
#if defined(OS_FUCHSIA) || IS_APPLE
...
{
int64_t
ConvertTimespecToMicros(const struct timespec& ts)
{
...
#if (IS_POSIX && defined(_POSIX_MONOTONIC_CLOCK) && _POSIX_MONOTONIC_CLOCK >= 0)          \
...
MaybeClockNow(clockid_t clk_id)
{
    struct timespec ts;
...
TimeNowIgnoringOverride()
{
    ...
    return Time()
           + TimeDelta::FromMicroseconds((tv.tv_sec * Time::kMicrosecondsPerSecond + tv.tv_usec)
...
ThreadTicksNowIgnoringOverride()
{
    ...
    return ThreadTicks() + TimeDelta::FromMicroseconds(ClockNow(CLOCK_THREAD_CPUTIME_ID));
#else
    NOTREACHED();
    return ThreadTicks();
#endif
