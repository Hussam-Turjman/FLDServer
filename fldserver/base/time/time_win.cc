...
int64_t
FileTimeToMicroseconds(const FILETIME& ft)
{
...
bool
CanConvertToFileTime(int64_t us)
{
    return us >= 0 && us <= (std::numeric_limits<int64_t>::max() / 10);
}

FILETIME
MicrosecondsToFileTime(int64_t us)
{
    DCHECK(CanConvertToFileTime(us))
            << "Out-of-range: Cannot convert " << us << " microseconds to FILETIME units.";
...
UpdateTimerIntervalLocked()
{
    ...
    if (g_last_interval_requested_ms)
    {
        ...
        g_high_res_timer_usage +=
                subtle::TimeTicksNowIgnoringOverride() - g_high_res_timer_last_activation;
...
bool
SafeConvertToWord(int in, WORD* out)
{
...
void
Time::EnableHighResolutionTimer(bool enable)
{
...
bool
Time::IsHighResolutionTimerInUse()
{
    AutoLock lock(*GetHighResLock());
...
// static
bool
Time::FromExploded(bool is_local, const Exploded& exploded, Time* time)
{
...
void
Time::Explode(bool is_local, Exploded* exploded) const
{
...
TimeTicksNowFunction g_time_ticks_now_ignoring_override_function = &InitialNowFunction;
...
QPCValueToTimeDelta(LONGLONG qpc_value)
{
    ...
    {
        return TimeDelta::FromMicroseconds(qpc_value * Time::kMicrosecondsPerSecond
    ...
    return TimeDelta::FromMicroseconds(
            ...
            + ((leftover_ticks * Time::kMicrosecondsPerSecond) / g_qpc_ticks_per_second));
}
...
InitializeNowFunctionPointer()
{
    ...
    if (internal::g_time_ticks_now_function == &subtle::TimeTicksNowIgnoringOverride)
    {
...
TimeTicks::SetMockTickFunction(TickFunctionType ticker)
{
...
// static
ThreadTicks
ThreadTicks::GetForThread(const PlatformThreadHandle& thread_handle)
{
    DCHECK(IsSupported());
    ...
    ::GetThreadTimes(
            thread_handle.platform_handle(), &creation_time, &exit_time, &kernel_time, &user_time);
    ...
    const int64_t us = static_cast<int64_t>(thread_time_seconds * Time::kMicrosecondsPerSecond);
#endif
...
// static
void
ThreadTicks::WaitUntilInitializedWin()
{
#if !defined(ARCH_CPU_ARM64)
...
// static
TimeDelta
TimeDelta::FromFileTime(FILETIME ft)
{
...
// static
TimeDelta
TimeDelta::FromWinrtDateTime(ABI::Windows::Foundation::DateTime dt)
{
...
TimeDelta::ToWinrtDateTime() const
{
