...
CreateCalendar(bool is_local)
{
    ...
    if (is_local)
    {
        calendar = std::make_unique<icu::GregorianCalendar>(icu::Locale::getUS(), status);
    }
    else
    {
...
bool
ExplodeUsingIcuCalendar(int64_t millis_since_unix_epoch, bool is_local, Time::Exploded* exploded)
{
    ...
    static_assert(Time::kTimeTToMicrosecondsOffset % Time::kMicrosecondsPerMillisecond == 0,
    ...
    constexpr int64_t kInputUpperBound = uint64_t{1} << std::numeric_limits<double>::digits;

    if (millis_since_unix_epoch < kInputLowerBound || millis_since_unix_epoch > kInputUpperBound)
    {
        return false;
    }
...
// static
void
Time::ExplodeUsingIcu(int64_t millis_since_unix_epoch, bool is_local, Exploded* exploded)
{
...
// static
bool
Time::FromExplodedUsingIcu(bool is_local,
...
{
    return ExplodeUsingIcu(ToRoundedDownMillisecondsSinceUnixEpoch(), is_local, exploded);
}
