...
public:
    ...
    bool
    CalledOnValidSequence(std::unique_ptr<debug::StackTrace>* out_bound_at) const
    {
        ...
        if (!SequenceCheckerImpl::HasThreadLocalStorageBeenDestroyed() && sequence_token_.IsValid())
        {
...
bool
SequenceCheckerImpl::CalledOnValidSequence(std::unique_ptr<debug::StackTrace>* bound_at) const
{
...
void
SequenceCheckerImpl::DetachFromSequence()
{
    AutoLock auto_lock(lock_);
    core_.reset();
}
...
// static
bool
SequenceCheckerImpl::HasThreadLocalStorageBeenDestroyed()
{
