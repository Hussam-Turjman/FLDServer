...
struct OptionalStorage<T,
                       ...
                       false /* trivially move constructible */> : OptionalStorageBase<T>
{
    using OptionalStorageBase<T>::is_populated_;
    using OptionalStorageBase<T>::value_;
    using OptionalStorageBase<T>::Init;
...
struct OptionalStorage<T,
                       ...
                       true /* trivially move constructible */> : OptionalStorageBase<T>
{
    using OptionalStorageBase<T>::is_populated_;
    using OptionalStorageBase<T>::value_;
    using OptionalStorageBase<T>::Init;
...
protected:
    ...
    OptionalBase&
    operator=(const OptionalBase& other)
    {
        CopyAssign(other);
        return *this;
    }
    ...
    OptionalBase&
    operator=(OptionalBase&& other) noexcept(std::is_nothrow_move_assignable<T>::value&&
    ...
    template <typename U>
    void
    CopyAssign(const OptionalBase<U>& other)
    {
        if (other.storage_.is_populated_)
    ...
    template <typename U>
    void
    MoveAssign(OptionalBase<U>&& other)
    {
        if (other.storage_.is_populated_)
...
struct CopyConstructible<false>
{
    ...
    CopyConstructible&
    operator=(const CopyConstructible&) = default;
    CopyConstructible&
    operator=(CopyConstructible&&) = default;
};
...
struct MoveConstructible<false>
{
    ...
    MoveConstructible&
    operator=(const MoveConstructible&) = default;
    MoveConstructible&
    operator=(MoveConstructible&&) = default;
};
...
struct CopyAssignable<false>
{
    ...
    CopyAssignable&
    operator=(const CopyAssignable&) = delete;
    CopyAssignable&
    operator=(CopyAssignable&&) = default;
};
...
struct MoveAssignable<false>
{
    ...
    MoveAssignable&
    operator=(const MoveAssignable&) = default;
    MoveAssignable&
    operator=(MoveAssignable&&) = delete;
};
...
#undef OPTIONAL_DECLSPEC_EMPTY_BASES
    ...
    template <typename U,
              std::enable_if_t<std::is_constructible<T, const U&>::value
                                       ...
                                       && !std::is_convertible<const U&, T>::value,
                               bool> = false>
    explicit Optional(const Optional<U>& other) : internal::OptionalBase<T>(other)
    {
    }
    ...
    template <typename U,
              std::enable_if_t<std::is_constructible<T, U&&>::value
                                       ...
                                       && !std::is_convertible<U&&, T>::value,
                               bool> = false>
    explicit Optional(Optional<U>&& other) : internal::OptionalBase<T>(std::move(other))
    {
    }
    ...
    value_or(U&& default_value) const&
    {
        ...
        return storage_.is_populated_ ? storage_.value_
    ...
    value_or(U&& default_value) &&
    {
        ...
        static_assert(std::is_convertible<U, T>::value, "U must be convertible to T");
        return storage_.is_populated_ ? std::move(storage_.value_)
                                      : static_cast<T>(std::forward<U>(default_value));
    }
    ...
    std::enable_if_t<std::is_constructible<T, std::initializer_list<U>&, Args&&...>::value, T&>
...
template <class T>
constexpr bool
operator==(const Optional<T>& opt, nullopt_t)
{
    return !opt;
}
...
template <class T>
constexpr bool
operator==(nullopt_t, const Optional<T>& opt)
{
    return !opt;
}
...
template <class T, class U>
constexpr bool
operator==(const U& value, const Optional<T>& opt)
{
...
template <class T>
std::enable_if_t<std::is_move_constructible<T>::value && internal::IsSwappable<T>::value>
...
{
    size_t
    operator()(const base::Optional<T>& opt) const
    {
