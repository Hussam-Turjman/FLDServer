...
#include "fldserver/base/metrics/sample_map.h"
...
// static
HistogramBase*
SparseHistogram::FactoryGet(const std::string& name, int32_t flags)
{
    ...
    if (!histogram)
    {
        ...
        bool should_record = StatisticsRecorder::ShouldRecordHistogram(HashMetricName(name));
        if (!should_record)
...
SparseHistogram::PersistentCreate(PersistentHistogramAllocator* allocator,
                                  const char* name,
...
uint64_t
SparseHistogram::name_hash() const
{
...
bool
SparseHistogram::HasConstructionArguments(Sample expected_minimum,
...
void
SparseHistogram::Add(Sample value)
{
    AddCount(value, 1);
}

void
SparseHistogram::AddCount(Sample value, int count)
{
    if (count <= 0)
    {
        NOTREACHED();
...
std::unique_ptr<HistogramSamples>
SparseHistogram::SnapshotFinalDelta() const
{
    DCHECK(!final_delta_created_);
...
void
SparseHistogram::AddSamples(const HistogramSamples& samples)
{
    base::AutoLock auto_lock(lock_);
...
bool
SparseHistogram::AddSamplesFromPickle(PickleIterator* iter)
{
    base::AutoLock auto_lock(lock_);
...
SparseHistogram::ToGraphDict() const
{
...
void
SparseHistogram::SerializeInfoImpl(Pickle* pickle) const
{
...
SparseHistogram::SparseHistogram(PersistentHistogramAllocator* allocator,
    ...
    // plus 1.
    unlogged_samples_(new PersistentSampleMap(HashMetricName(name), allocator, meta)),
    logged_samples_(new PersistentSampleMap(unlogged_samples_->id() + 1, allocator, logged_meta))
{
}

HistogramBase*
SparseHistogram::DeserializeInfoImpl(PickleIterator* iter)
{
...
void
SparseHistogram::GetParameters(DictionaryValue* params) const
{
