...
class CORE_EXPORT PersistentMemoryAllocator
{
public:
    ...
    public:
        ...
        // a run-time error.
        Iterator(const PersistentMemoryAllocator* allocator, Reference starting_after);
        ...
        void
        Reset(Reference starting_after);
        ...
        Reference
        GetNextOfType(uint32_t type_match);
        ...
        template <typename T>
        const T*
        GetAsObject(Reference ref) const
        {
        ...
        template <typename T>
        const T*
        GetAsArray(Reference ref, uint32_t type_id, size_t count) const
        {
        ...
        Reference
        GetAsReference(const void* memory, uint32_t type_id) const
        {
        ...
        template <typename T>
        Reference
        GetAsReference(const T* obj) const
        {
    ...
    static bool
    IsMemoryAcceptable(const void* data, size_t size, size_t page_size, bool readonly);
    ...
    void
    SetMemoryState(uint8_t memory_state);
    uint8_t
    GetMemoryState() const;
    ...
    void
    CreateTrackingHistograms(base::StringPiece name);
    ...
    data() const
    {
        return const_cast<const char*>(mem_base_);
    }
    size_t
    length() const
    {
    ...
    GetAsObject(Reference ref)
    {
        ...
        return const_cast<T*>(
                reinterpret_cast<volatile T*>(GetBlockData(ref, T::kPersistentTypeId, sizeof(T))));
    }
    template <typename T>
    const T*
    GetAsObject(Reference ref) const
    ...
    template <typename T>
    T*
    GetAsArray(Reference ref, uint32_t type_id, size_t count)
    {
        static_assert(std::is_fundamental<T>::value, "use GetAsObject<>()");
        return const_cast<T*>(
                reinterpret_cast<volatile T*>(GetBlockData(ref, type_id, count * sizeof(T))));
    }
    template <typename T>
    const T*
    GetAsArray(Reference ref, uint32_t type_id, size_t count) const
    {
        ...
        return const_cast<const char*>(
                reinterpret_cast<const volatile T*>(GetBlockData(ref, type_id, count * sizeof(T))));
    }
    ...
    size_t
    GetAllocSize(Reference ref) const;
    ...
    uint32_t
    GetType(Reference ref) const;
    bool
    ChangeType(Reference ref, uint32_t to_type_id, uint32_t from_type_id, bool clear);
    ...
    void
    GetMemoryInfo(MemoryInfo* meminfo) const;
    ...
    Reference
    Allocate(size_t size, uint32_t type_id);
    ...
    New(size_t size)
    {
        ...
        void* mem = const_cast<void*>(GetBlockData(ref, T::kPersistentTypeId, size));
        if (!mem)
            return nullptr;
    ...
    template <typename T>
    T*
    New(Reference ref, uint32_t from_type_id, bool clear)
    {
        ...
        bool success = ChangeType(ref, T::kPersistentTypeId, kTypeIdTransitioning, false);
        DCHECK(success);
        return obj;
    }
    ...
    template <typename T>
    void
    Delete(T* obj, uint32_t new_type)
    {
...
protected:
    ...
    virtual void
    FlushPartial(size_t length, bool sync);
...
private:
    ...
    shared_meta() const
    {
        return reinterpret_cast<const SharedMetadata*>(const_cast<const char*>(mem_base_));
    }
    SharedMetadata*
    shared_meta()
    {
    ...
    Reference
    AllocateImpl(size_t size, uint32_t type_id);

    // Get the block header associated with a specific reference.
    const volatile BlockHeader*
    GetBlock(Reference ref, uint32_t type_id, uint32_t size, bool queue_ok, bool free_ok) const;
    volatile BlockHeader*
    GetBlock(Reference ref, uint32_t type_id, uint32_t size, bool queue_ok, bool free_ok)
    {
    ...
    const volatile void*
    GetBlockData(Reference ref, uint32_t type_id, uint32_t size) const;
    volatile void*
    GetBlockData(Reference ref, uint32_t type_id, uint32_t size)
    {
...
class CORE_EXPORT LocalPersistentMemoryAllocator : public PersistentMemoryAllocator
{
public:
    LocalPersistentMemoryAllocator(size_t size, uint64_t id, base::StringPiece name);
...
private:
    ...
    static Memory
    AllocateLocalMemory(size_t size);

    // Deallocates a block of local |memory| of the specified |size|.
    static void
    DeallocateLocalMemory(void* memory, size_t size, MemoryType type);
...
class CORE_EXPORT WritableSharedPersistentMemoryAllocator : public PersistentMemoryAllocator
{
public:
    WritableSharedPersistentMemoryAllocator(base::WritableSharedMemoryMapping memory,
                                            uint64_t id,
                                            base::StringPiece name);
    ...
    // the rest.
    static bool
    IsSharedMemoryAcceptable(const base::WritableSharedMemoryMapping& memory);
...
// allocation from it. The allocator takes ownership of the mapping object.
class CORE_EXPORT ReadOnlySharedPersistentMemoryAllocator : public PersistentMemoryAllocator
{
public:
    ReadOnlySharedPersistentMemoryAllocator(base::ReadOnlySharedMemoryMapping memory,
                                            uint64_t id,
                                            base::StringPiece name);
    ...
    // the rest.
    static bool
    IsSharedMemoryAcceptable(const base::ReadOnlySharedMemoryMapping& memory);
...
class CORE_EXPORT FilePersistentMemoryAllocator : public PersistentMemoryAllocator
{
public:
    ...
    // the rest.
    static bool
    IsFileAcceptable(const MemoryMappedFile& file, bool read_only);
...
protected:
    ...
    void
    FlushPartial(size_t length, bool sync) override;
...
class CORE_EXPORT DelayedPersistentAllocation
{
public:
