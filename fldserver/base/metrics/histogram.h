...
#include "fldserver/base/metrics/bucket_ranges.h"
...
class CORE_EXPORT Histogram : public HistogramBase
{
public:
    ...
    FactoryGet(const std::string& name,
               ...
               int32_t flags);
    static HistogramBase*
    FactoryTimeGet(const std::string& name,
                   base::TimeDelta minimum,
                   base::TimeDelta maximum,
                   uint32_t bucket_count,
                   int32_t flags);
    static HistogramBase*
    FactoryMicrosecondsTimeGet(const std::string& name,
                               base::TimeDelta minimum,
                               base::TimeDelta maximum,
                               uint32_t bucket_count,
                               int32_t flags);
    ...
    static HistogramBase*
    FactoryTimeGet(const char* name,
                   base::TimeDelta minimum,
                   base::TimeDelta maximum,
                   uint32_t bucket_count,
                   int32_t flags);
    static HistogramBase*
    FactoryMicrosecondsTimeGet(const char* name,
                               base::TimeDelta minimum,
                               base::TimeDelta maximum,
                               uint32_t bucket_count,
                               int32_t flags);
    ...
    static std::unique_ptr<HistogramBase>
    PersistentCreate(const char* name,
                     Sample minimum,
                     Sample maximum,
    ...
    static void
    InitializeBucketRanges(Sample minimum, Sample maximum, BucketRanges* ranges);
    ...
    uint32_t
    FindCorruption(const HistogramSamples& samples) const override;
    ...
    static bool
    InspectConstructionArguments(StringPiece name,
                                 Sample* minimum,
                                 Sample* maximum,
                                 uint32_t* bucket_count);
    ...
    uint64_t
    name_hash() const override;
    HistogramType
    GetHistogramType() const override;
    bool
    HasConstructionArguments(Sample expected_minimum,
                             Sample expected_maximum,
                             uint32_t expected_bucket_count) const override;
    void
    Add(Sample value) override;
    void
    AddCount(Sample value, int count) override;
    std::unique_ptr<HistogramSamples>
    SnapshotSamples() const override;
    std::unique_ptr<HistogramSamples>
    SnapshotDelta() override;
    std::unique_ptr<HistogramSamples>
    SnapshotFinalDelta() const override;
    void
    AddSamples(const HistogramSamples& samples) override;
    bool
    AddSamplesFromPickle(base::PickleIterator* iter) override;
    base::DictionaryValue
    ToGraphDict() const override;

    // Validates the histogram contents and CHECKs on errors.
    // TODO(bcwhite): Remove this after https://crbug/836875.
    void
    ValidateHistogramContents() const override;
...
protected:
    ...
    Histogram(const char* name, Sample minimum, Sample maximum, const BucketRanges* ranges);
    ...
    // HistogramBase implementation:
    void
    SerializeInfoImpl(base::Pickle* pickle) const override;

    // Return a string description of what goes in a given bucket.
    // Most commonly this is the numeric value, but in derived classes it may
    // be a name (or string description) given to the bucket.
    virtual const std::string
    GetAsciiBucketRange(uint32_t it) const;
...
private:
    ...
    friend CORE_EXPORT HistogramBase*
    DeserializeHistogramInfo(base::PickleIterator* iter);
    static HistogramBase*
    DeserializeInfoImpl(base::PickleIterator* iter);
...
// buckets.
class CORE_EXPORT LinearHistogram : public Histogram
{
public:
    ...
    // "description" field.
    static HistogramBase*
    FactoryGetWithRangeDescription(const std::string& name,
                                   Sample minimum,
                                   Sample maximum,
                                   uint32_t bucket_count,
                                   int32_t flags,
...
protected:
    ...
    LinearHistogram(const char* name, Sample minimum, Sample maximum, const BucketRanges* ranges);
    ...
    const std::string
    GetAsciiBucketRange(uint32_t i) const override;
...
class CORE_EXPORT ScaledLinearHistogram
{
...
public:
    ...
    void
    AddScaledCount(Sample value, int count);
...
class CORE_EXPORT BooleanHistogram : public LinearHistogram
{
public:
    static HistogramBase*
    FactoryGet(const std::string& name, int32_t flags);

    // Overload of the above function that takes a const char* |name| param,
    // to avoid code bloat from the std::string constructor being inlined into
    // call sites.
    static HistogramBase*
    FactoryGet(const char* name, int32_t flags);
...
class CORE_EXPORT CustomHistogram : public Histogram
{
public:
    ...
    static HistogramBase*
    FactoryGet(const std::string& name, const std::vector<Sample>& custom_ranges, int32_t flags);

    // Overload of the above function that takes a const char* |name| param,
    // to avoid code bloat from the std::string constructor being inlined into
    // call sites.
    static HistogramBase*
    FactoryGet(const char* name, const std::vector<Sample>& custom_ranges, int32_t flags);
    ...
    ArrayToCustomEnumRanges(base::span<const Sample> values);
...
private:
    ...
    static bool
    ValidateCustomRanges(const std::vector<Sample>& custom_ranges);
