...
#include "fldserver/base/metrics/persistent_histogram_storage.h"
...
PersistentHistogramStorage::PersistentHistogramStorage(
        StringPiece allocator_name, StorageDirManagement storage_dir_management) :
    storage_dir_management_(storage_dir_management)
{
    ...
    GlobalHistogramAllocator::CreateWithPersistentMemory(memory,
                                                         kAllocSize,
                                                         0,
                                                         0,  // No identifier.
                                                         allocator_name);
...
PersistentHistogramStorage::~PersistentHistogramStorage()
{
    ...
    const FilePath file_path = storage_dir
                                       .AppendASCII(StringPrintf("%04d%02d%02d%02d%02d%02d",
                                                                 exploded.year,
                                                                 exploded.month,
    ...
    StringPiece contents(static_cast<const char*>(allocator->data()), allocator->used());
    if (!ImportantFileWriter::WriteFileAtomically(file_path, contents))
    {
        LOG(ERROR) << "Persistent histograms fail to write to file: " << file_path.value();
    }
}
