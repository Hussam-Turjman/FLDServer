...
// previous global one.
class CORE_EXPORT StatisticsRecorder
{
public:
    ...
    // This method is thread safe.
    static void
    RegisterHistogramProvider(const WeakPtr<HistogramProvider>& provider);
    ...
    RegisterOrDeleteDuplicate(HistogramBase* histogram);
    ...
    //
    // This method is thread safe.
    static const BucketRanges*
    RegisterOrDeleteDuplicateRanges(const BucketRanges* ranges);
    ...
    // This method is thread safe.
    static void
    WriteGraph(const std::string& query, std::string* output);
    ...
    // This method is thread safe.
    static std::string
    ToJSON(JSONVerbosityLevel verbosity_level);
    ...
    //
    // This method is thread safe.
    static std::vector<const BucketRanges*>
    GetBucketRanges();
    ...
    // This method is thread safe.
    static HistogramBase*
    FindHistogram(base::StringPiece name);

    // Imports histograms from providers.
    //
    // This method must be called on the UI thread.
    static void
    ImportProvidedHistograms();
    ...
    static void
    PrepareDeltas(bool include_persistent,
                  HistogramBase::Flags flags_to_set,
                  HistogramBase::Flags required_flags,
                  HistogramSnapshotManager* snapshot_manager);

    using OnSampleCallback = base::RepeatingCallback<void(
            const char* /*=histogram_name*/, uint64_t /*=name_hash*/, HistogramBase::Sample)>;
    ...
    // This method is thread safe.
    static bool
    SetCallback(const std::string& histogram_name, OnSampleCallback callback);

    // Clears any callback set on the histogram referred to by |histogram_name|.
    //
    // This method is thread safe.
    static void
    ClearCallback(const std::string& histogram_name);
    ...
    //
    // This method is thread safe.
    static OnSampleCallback
    FindCallback(const std::string& histogram_name);
    ...
    // This method is thread safe.
    static void
    ForgetHistogramForTesting(base::StringPiece name);
    ...
    //
    // This method is thread safe.
    static std::unique_ptr<StatisticsRecorder>
    CreateTemporaryForTesting() WARN_UNUSED_RESULT;
    ...
    // mutate any state.
    static void
    SetRecordChecker(std::unique_ptr<RecordHistogramChecker> record_checker);
    ...
    static Histograms
    WithName(Histograms histograms, const std::string& query);

    // Filters histograms by persistency. Only non-persistent histograms are kept.
    static Histograms
    NonPersistent(Histograms histograms);
    ...
    static void
    SetGlobalSampleCallback(const GlobalSampleCallback& global_sample_callback);

    // Returns the global callback, if any, that should be called every time a
    // histogram sample is added.
    static GlobalSampleCallback
    global_sample_callback()
    {
...
private:
    ...
    typedef std::unordered_map<StringPiece, HistogramBase*, StringPieceHash> HistogramMap;
    ...
    {
        size_t
        operator()(const BucketRanges* a) const;
    };

    struct BucketRangesEqual
    {
        bool
        operator()(const BucketRanges* a, const BucketRanges* b) const;
    };

    typedef std::unordered_set<const BucketRanges*, BucketRangesHash, BucketRangesEqual> RangesMap;
...
*/
    ...
    //
    // This method is thread safe.
    static HistogramProviders
    GetHistogramProviders();
