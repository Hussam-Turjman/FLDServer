...
#include "fldserver/base/metrics/dummy_histogram.h"
...
#include "fldserver/base/metrics/persistent_memory_allocator.h"
#include "fldserver/base/metrics/sample_vector.h"
...
{
bool
ReadHistogramArguments(PickleIterator* iter,
...
{
    if (!iter->ReadString(histogram_name) || !iter->ReadInt(flags) || !iter->ReadInt(declared_min)
        || !iter->ReadInt(declared_max) || !iter->ReadUInt32(bucket_count)
    ...
    if (*declared_max <= 0 || *declared_min <= 0 || *declared_max < *declared_min
        || INT_MAX / sizeof(HistogramBase::Count) <= *bucket_count || *bucket_count < 2)
    {
...
bool
ValidateRangeChecksum(const HistogramBase& histogram, uint32_t range_checksum)
{
...
protected:
    ...
    virtual std::unique_ptr<HistogramBase>
    HeapAlloc(const BucketRanges* ranges)
    {
        return WrapUnique(new Histogram(GetPermanentName(name_), minimum_, maximum_, ranges));
    }
    ...
    // always remain empty.
    virtual void
    FillHistogram(HistogramBase* histogram)
    {
    }
...
Histogram::Factory::Build()
{
    ...
    if (!histogram)
    {
        ...
        bool should_record = StatisticsRecorder::ShouldRecordHistogram(HashMetricName(name_));
        if (!should_record)
        ...
        if (allocator)
        {
            tentative_histogram = allocator->AllocateHistogram(histogram_type_,
                                                               name_,
                                                               minimum_,
                                                               maximum_,
                                                               registered_ranges,
        ...
        histogram = StatisticsRecorder::RegisterOrDeleteDuplicate(tentative_histogram.release());

        // Persistent histograms need some follow-up processing.
        if (histogram_ref)
        {
            allocator->FinalizeHistogram(histogram_ref, histogram == tentative_histogram_ptr);
        }
    }
            ...
            && !histogram->HasConstructionArguments(minimum_, maximum_, bucket_count_)))
    {
        ...
        DLOG(ERROR) << "Histogram " << name_ << " has mismatched construction arguments";
        return DummyHistogram::GetInstance();
    }
    return histogram;
}
...
HistogramBase*
Histogram::FactoryGet(const std::string& name,
                      Sample minimum,
                      Sample maximum,
                      uint32_t bucket_count,
                      int32_t flags)
...
HistogramBase*
Histogram::FactoryTimeGet(const std::string& name,
                          TimeDelta minimum,
                          TimeDelta maximum,
                          uint32_t bucket_count,
                          int32_t flags)
{
    ...
    return FactoryGet(name,
                      static_cast<Sample>(minimum.InMilliseconds()),
...
HistogramBase*
Histogram::FactoryMicrosecondsTimeGet(const std::string& name,
                                      TimeDelta minimum,
                                      TimeDelta maximum,
                                      uint32_t bucket_count,
                                      int32_t flags)
{
    ...
    return FactoryGet(name,
                      static_cast<Sample>(minimum.InMicroseconds()),
...
Histogram::FactoryGet(
        const char* name, Sample minimum, Sample maximum, uint32_t bucket_count, int32_t flags)
{
...
Histogram::FactoryTimeGet(const char* name,
                          ...
                          int32_t flags)
{
    return FactoryTimeGet(std::string(name), minimum, maximum, bucket_count, flags);
}

HistogramBase*
Histogram::FactoryMicrosecondsTimeGet(const char* name,
                                      TimeDelta minimum,
                                      TimeDelta maximum,
                                      uint32_t bucket_count,
                                      int32_t flags)
{
    return FactoryMicrosecondsTimeGet(std::string(name), minimum, maximum, bucket_count, flags);
}

std::unique_ptr<HistogramBase>
Histogram::PersistentCreate(const char* name,
                            Sample minimum,
                            Sample maximum,
...
{
    return WrapUnique(new Histogram(
            name, minimum, maximum, ranges, counts, logged_counts, meta, logged_meta));
}
...
// static
void
Histogram::InitializeBucketRanges(Sample minimum, Sample maximum, BucketRanges* ranges)
{
...
uint32_t
Histogram::FindCorruption(const HistogramSamples& samples) const
{
...
const BucketRanges*
Histogram::bucket_ranges() const
{
    return unlogged_samples_->bucket_ranges();
}

Sample
Histogram::declared_min() const
{
...
Sample
Histogram::ranges(uint32_t i) const
{
    return bucket_ranges()->range(i);
}

uint32_t
Histogram::bucket_count() const
{
...
// static
bool
Histogram::InspectConstructionArguments(StringPiece name,
                                        Sample* minimum,
                                        Sample* maximum,
                                        uint32_t* bucket_count)
{
    ...
    if (*bucket_count > kBucketCount_MAX)
    {
        ...
        {
            DVLOG(1) << "Histogram: " << name << " has bad bucket_count: " << *bucket_count
...
bool
Histogram::HasConstructionArguments(Sample expected_minimum,
                                    Sample expected_maximum,
                                    uint32_t expected_bucket_count) const
{
    return (expected_bucket_count == bucket_count() && expected_minimum == declared_min()
...
void
Histogram::AddCount(int value, int count)
{
...
std::unique_ptr<HistogramSamples>
Histogram::SnapshotFinalDelta() const
{
#if DCHECK_IS_ON()
    DCHECK(!final_delta_created_);
...
void
Histogram::AddSamples(const HistogramSamples& samples)
{
    unlogged_samples_->Add(samples);
}

bool
Histogram::AddSamplesFromPickle(PickleIterator* iter)
{
    return unlogged_samples_->AddFromPickle(iter);
}

base::DictionaryValue
Histogram::ToGraphDict() const
{
...
void
Histogram::ValidateHistogramContents() const
{
...
void
Histogram::SerializeInfoImpl(Pickle* pickle) const
{
...
Histogram::Histogram(const char* name, Sample minimum, Sample maximum, const BucketRanges* ranges) :
    HistogramBase(name)
{
    DCHECK(ranges) << name << ": " << minimum << "-" << maximum;
    unlogged_samples_ = std::make_unique<SampleVector>(HashMetricName(name), ranges);
    logged_samples_ = std::make_unique<SampleVector>(unlogged_samples_->id(), ranges);
}
    ...
    HistogramBase(name)
{
    DCHECK(ranges) << name << ": " << minimum << "-" << maximum;
    unlogged_samples_ =
            std::make_unique<PersistentSampleVector>(HashMetricName(name), ranges, meta, counts);
...
const std::string
Histogram::GetAsciiBucketRange(uint32_t i) const
{
...
// static
HistogramBase*
Histogram::DeserializeInfoImpl(PickleIterator* iter)
{
    std::string histogram_name;
    int flags;
    int declared_min;
    ...
    if (!ReadHistogramArguments(iter,
                                &histogram_name,
                                &flags,
                                &declared_min,
                                &declared_max,
    ...
    HistogramBase* histogram =
            Histogram::FactoryGet(histogram_name, declared_min, declared_max, bucket_count, flags);
    if (!histogram)
        return nullptr;
...
std::unique_ptr<SampleVector>
Histogram::SnapshotAllSamples() const
{
...
std::unique_ptr<SampleVector>
Histogram::SnapshotUnloggedSamples() const
{
...
void
Histogram::GetParameters(DictionaryValue* params) const
{
...
public:
    Factory(const std::string& name,
        ...
        Histogram::Factory(name, LINEAR_HISTOGRAM, minimum, maximum, bucket_count, flags)
    {
...
protected:
    ...
    std::unique_ptr<HistogramBase>
    HeapAlloc(const BucketRanges* ranges) override
    {
        return WrapUnique(new LinearHistogram(GetPermanentName(name_), minimum_, maximum_, ranges));
    }

    void
    FillHistogram(HistogramBase* base_histogram) override
    {
...
HistogramBase*
LinearHistogram::FactoryGet(const std::string& name,
                            Sample minimum,
                            Sample maximum,
                            uint32_t bucket_count,
                            int32_t flags)
{
    return FactoryGetWithRangeDescription(name, minimum, maximum, bucket_count, flags, NULL);
}

HistogramBase*
LinearHistogram::FactoryTimeGet(const std::string& name,
                                TimeDelta minimum,
                                TimeDelta maximum,
                                uint32_t bucket_count,
                                int32_t flags)
...
std::unique_ptr<HistogramBase>
LinearHistogram::PersistentCreate(const char* name,
                                  Sample minimum,
                                  Sample maximum,
                                  const BucketRanges* ranges,
                                  const DelayedPersistentAllocation& counts,
                                  ...
                                  HistogramSamples::Metadata* logged_meta)
{
    return WrapUnique(new LinearHistogram(
            name, minimum, maximum, ranges, counts, logged_counts, meta, logged_meta));
}

HistogramBase*
LinearHistogram::FactoryGetWithRangeDescription(const std::string& name,
                                                Sample minimum,
                                                Sample maximum,
                                                uint32_t bucket_count,
                                                int32_t flags,
                                                const DescriptionPair descriptions[])
{
    ...
    bool valid_arguments =
            Histogram::InspectConstructionArguments(name, &minimum, &maximum, &bucket_count);
    DCHECK(valid_arguments) << name;

    return Factory(name, minimum, maximum, bucket_count, flags, descriptions).Build();
}
...
LinearHistogram::LinearHistogram(const char* name,
                                 ...
                                 HistogramSamples::Metadata* meta,
                                 HistogramSamples::Metadata* logged_meta) :
    Histogram(name, minimum, maximum, ranges, counts, logged_counts, meta, logged_meta)
{
}

const std::string
LinearHistogram::GetAsciiBucketRange(uint32_t i) const
{
    int range = ranges(i);
...
// static
void
LinearHistogram::InitializeBucketRanges(Sample minimum, Sample maximum, BucketRanges* ranges)
{
    double min = minimum;
    double max = maximum;
    size_t bucket_count = ranges->bucket_count();
...
// static
HistogramBase*
LinearHistogram::DeserializeInfoImpl(PickleIterator* iter)
{
    std::string histogram_name;
    int flags;
    int declared_min;
...
ScaledLinearHistogram::ScaledLinearHistogram(const char* name,
                                             ...
                                             int32_t scale,
                                             int32_t flags) :
    ScaledLinearHistogram(std::string(name), minimum, maximum, bucket_count, scale, flags)
{
}
...
ScaledLinearHistogram::ScaledLinearHistogram(const std::string& name,
                                             ...
                                             int32_t scale,
                                             int32_t flags) :
    histogram_(LinearHistogram::FactoryGet(name, minimum, maximum, bucket_count, flags)),
    scale_(scale)
{
    DCHECK(histogram_);
    DCHECK_LT(1, scale);
...
void
ScaledLinearHistogram::AddScaledCount(Sample value, int count)
{
    if (histogram_->GetHistogramType() == DUMMY_HISTOGRAM)
        return;
    if (count == 0)
    ...
    if (remainder > 0)
    {
        remainder = subtle::NoBarrier_AtomicIncrement(&remainders_[value], remainder);
...
HistogramBase*
BooleanHistogram::FactoryGet(const std::string& name, int32_t flags)
{
    return Factory(name, flags).Build();
}

HistogramBase*
BooleanHistogram::FactoryGet(const char* name, int32_t flags)
{
    return FactoryGet(std::string(name), flags);
}

std::unique_ptr<HistogramBase>
BooleanHistogram::PersistentCreate(const char* name,
                                   const BucketRanges* ranges,
                                   const DelayedPersistentAllocation& counts,
                                   const DelayedPersistentAllocation& logged_counts,
                                   HistogramSamples::Metadata* meta,
                                   HistogramSamples::Metadata* logged_meta)
{
    return WrapUnique(new BooleanHistogram(name, ranges, counts, logged_counts, meta, logged_meta));
}
...
BooleanHistogram::BooleanHistogram(const char* name,
                                   ...
                                   HistogramSamples::Metadata* meta,
                                   HistogramSamples::Metadata* logged_meta) :
    LinearHistogram(name, 1, 2, ranges, counts, logged_counts, meta, logged_meta)
{
}

HistogramBase*
BooleanHistogram::DeserializeInfoImpl(PickleIterator* iter)
{
    std::string histogram_name;
    int flags;
    int declared_min;
    ...
    HistogramBase* histogram = BooleanHistogram::FactoryGet(histogram_name, flags);
    if (!histogram)
        return nullptr;
...
{
public:
    Factory(const std::string& name, const std::vector<Sample>* custom_ranges, int32_t flags) :
...
HistogramBase*
CustomHistogram::FactoryGet(const std::string& name,
...
HistogramBase*
CustomHistogram::FactoryGet(const char* name,
                            const std::vector<Sample>& custom_ranges,
                            int32_t flags)
{
...
std::unique_ptr<HistogramBase>
CustomHistogram::PersistentCreate(const char* name,
                                  const BucketRanges* ranges,
                                  const DelayedPersistentAllocation& counts,
                                  const DelayedPersistentAllocation& logged_counts,
                                  HistogramSamples::Metadata* meta,
                                  HistogramSamples::Metadata* logged_meta)
{
    return WrapUnique(new CustomHistogram(name, ranges, counts, logged_counts, meta, logged_meta));
}
...
// static
std::vector<Sample>
CustomHistogram::ArrayToCustomEnumRanges(base::span<const Sample> values)
{
...
CustomHistogram::CustomHistogram(const char* name, const BucketRanges* ranges) :
    Histogram(name, ranges->range(1), ranges->range(ranges->bucket_count() - 1), ranges)
{
}

CustomHistogram::CustomHistogram(const char* name,
                                 const BucketRanges* ranges,
                                 const DelayedPersistentAllocation& counts,
                                 const DelayedPersistentAllocation& logged_counts,
                                 HistogramSamples::Metadata* meta,
...
void
CustomHistogram::SerializeInfoImpl(Pickle* pickle) const
{
...
// static
HistogramBase*
CustomHistogram::DeserializeInfoImpl(PickleIterator* iter)
{
    std::string histogram_name;
    int flags;
    int declared_min;
    ...
    HistogramBase* histogram = CustomHistogram::FactoryGet(histogram_name, sample_ranges, flags);
    if (!histogram)
        return nullptr;
...
// static
bool
CustomHistogram::ValidateCustomRanges(const std::vector<Sample>& custom_ranges)
{
