...
#include "fldserver/base/json/json_string_value_serializer.h"
...
HistogramTypeToString(HistogramType type)
{
    switch (type)
    {
        ...
        case LINEAR_HISTOGRAM: return "LINEAR_HISTOGRAM";
        case BOOLEAN_HISTOGRAM: return "BOOLEAN_HISTOGRAM";
        case CUSTOM_HISTOGRAM: return "CUSTOM_HISTOGRAM";
        case SPARSE_HISTOGRAM: return "SPARSE_HISTOGRAM";
        case DUMMY_HISTOGRAM: return "DUMMY_HISTOGRAM";
    }
    NOTREACHED();
    return "UNKNOWN";
}
...
DeserializeHistogramInfo(PickleIterator* iter)
{
    ...
    switch (type)
    {
        case HISTOGRAM: return Histogram::DeserializeInfoImpl(iter);
        case LINEAR_HISTOGRAM: return LinearHistogram::DeserializeInfoImpl(iter);
        case BOOLEAN_HISTOGRAM: return BooleanHistogram::DeserializeInfoImpl(iter);
        case CUSTOM_HISTOGRAM: return CustomHistogram::DeserializeInfoImpl(iter);
        case SPARSE_HISTOGRAM: return SparseHistogram::DeserializeInfoImpl(iter);
...
HistogramBase::HistogramBase(const char* name) : histogram_name_(name), flags_(kNoFlags)
{
}
...
void
HistogramBase::CheckName(const StringPiece& name) const
{
    DCHECK_EQ(StringPiece(histogram_name()), name);
}

void
HistogramBase::SetFlags(int32_t flags)
{
    flags_.fetch_or(flags, std::memory_order_relaxed);
}

void
HistogramBase::ClearFlags(int32_t flags)
{
    flags_.fetch_and(~flags, std::memory_order_relaxed);
}

void
HistogramBase::AddScaled(Sample value, int count, int scale)
{
    DCHECK_LT(0, scale);
...
void
HistogramBase::AddKilo(Sample value, int count)
{
    AddScaled(value, count, 1000);
}

void
HistogramBase::AddKiB(Sample value, int count)
{
    AddScaled(value, count, 1024);
}

void
HistogramBase::AddTimeMillisecondsGranularity(const TimeDelta& time)
{
    Add(saturated_cast<Sample>(time.InMilliseconds()));
}

void
HistogramBase::AddTimeMicrosecondsGranularity(const TimeDelta& time)
{
...
void
HistogramBase::AddBoolean(bool value)
{
    Add(value ? 1 : 0);
}

void
HistogramBase::SerializeInfo(Pickle* pickle) const
{
...
uint32_t
HistogramBase::FindCorruption(const HistogramSamples& samples) const
{
...
void
HistogramBase::ValidateHistogramContents() const
{
}

void
HistogramBase::WriteJSON(std::string* output, JSONVerbosityLevel verbosity_level) const
{
    Count count = 0;
    int64_t sum = 0;
...
void
HistogramBase::FindAndRunCallback(HistogramBase::Sample sample) const
{
...
void
HistogramBase::GetCountAndBucketData(Count* count, int64_t* sum, ListValue* buckets) const
{
...
void
HistogramBase::WriteAsciiBucketGraph(double x_count, int line_length, std::string* output) const
{
...
const std::string
HistogramBase::GetSimpleAsciiBucketRange(Sample sample) const
{
    return StringPrintf("%d", sample);
}

void
HistogramBase::WriteAsciiBucketValue(Count current, double scaled_sum, std::string* output) const
{
    StringAppendF(output, " (%d = %3.1f%%)", current, current / scaled_sum);
}

void
HistogramBase::WriteAscii(std::string* output) const
{
