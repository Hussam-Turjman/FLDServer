...
#include "fldserver/base/auto_reset.h"
...
#include "fldserver/base/metrics/field_trial_param_associator.h"
...
#include "fldserver/base/process/process_info.h"
...
#if IS_POSIX && !defined(OS_NACL)
#include "fldserver/base/posix/global_descriptors.h"
#endif
...
void
WriteStringPair(Pickle* pickle, const StringPiece& string1, const StringPiece& string2)
{
...
void
PickleFieldTrial(const FieldTrial::State& trial_state, Pickle* pickle)
{
...
FieldTrial::Probability
GetGroupBoundaryValue(FieldTrial::Probability divisor, double entropy_value)
{
...
bool
ParseFieldTrialsString(const std::string& trials_string,
...
void
AddFeatureAndFieldTrialFlags(const char* enable_features_switch,
                             const char* disable_features_switch,
                             CommandLine* cmd_line)
{
    std::string enabled_features;
    std::string disabled_features;
    FeatureList::GetInstance()->GetFeatureOverrides(&enabled_features, &disabled_features);
    ...
    {
        cmd_line->AppendSwitchASCII(switches::kForceFieldTrials, field_trial_states);
    }
}

void
OnOutOfMemory(size_t size)
{
#if defined(OS_NACL)
    NOTREACHED();
#else
...
bool
DeserializeGUIDFromStringPieces(StringPiece first, StringPiece second, UnguessableToken* guid)
{
    uint64_t high = 0;
    uint64_t low = 0;
...
bool
FieldTrial::FieldTrialEntry::GetTrialAndGroupName(StringPiece* trial_name,
...
bool
FieldTrial::FieldTrialEntry::GetParams(std::map<std::string, std::string>* params) const
{
    PickleIterator iter = GetPickleIterator();
    StringPiece tmp;
...
PickleIterator
FieldTrial::FieldTrialEntry::GetPickleIterator() const
{
    const char* src = reinterpret_cast<const char*>(this) + sizeof(FieldTrialEntry);
...
bool
FieldTrial::FieldTrialEntry::ReadStringPair(PickleIterator* iter,
                                            StringPiece* trial_name,
                                            StringPiece* group_name) const
{
...
int
FieldTrial::AppendGroup(const std::string& name, Probability group_probability)
{
...
const std::string&
FieldTrial::GetGroupNameWithoutActivation()
{
    FinalizeGroupChoice();
    return group_name_;
}
...
// static
FieldTrial*
FieldTrial::CreateSimulatedFieldTrial(StringPiece trial_name,
                                      Probability total_probability,
                                      StringPiece default_group_name,
                                      double entropy_value)
{
    return new FieldTrial(trial_name, total_probability, default_group_name, entropy_value);
}
...
void
FieldTrial::SetGroupChoice(const std::string& group_name, int number)
{
    group_ = number;
    if (group_name.empty())
...
void
FieldTrial::FinalizeGroupChoice()
{
    FinalizeGroupChoiceImpl(false);
}

void
FieldTrial::FinalizeGroupChoiceImpl(bool is_locked)
{
    if (group_ != kNotFinalized)
        return;
...
bool
FieldTrial::GetActiveGroup(ActiveGroup* active_group) const
{
...
bool
FieldTrial::GetStateWhileLocked(State* field_trial_state, bool include_disabled)
{
...
// static
FieldTrial*
FieldTrialList::FactoryGetFieldTrial(StringPiece trial_name,
                                     FieldTrial::Probability total_probability,
                                     StringPiece default_group_name,
                                     FieldTrial::RandomizationType randomization_type,
                                     int* default_group_number)
{
    return FactoryGetFieldTrialWithRandomizationSeed(trial_name,
                                                     total_probability,
                                                     default_group_name,
                                                     randomization_type,
                                                     0,
        ...
        const FieldTrial::EntropyProvider* override_entropy_provider)
{
    ...
    if (existing_trial)
    {
        ...
        if (default_group_number && default_group_name != existing_trial->default_group_name())
        {
            ...
            else
            {
                ...
                static_assert(kNonConflictingGroupNumber != FieldTrial::kDefaultGroupNumber,
    ...
    if (randomization_type == FieldTrial::ONE_TIME_RANDOMIZED)
    {
        ...
        entropy_value = entropy_provider->GetEntropyForTrial(trial_name, randomization_seed);
    }
    else
    {
    ...
    FieldTrial* field_trial =
            new FieldTrial(trial_name, total_probability, default_group_name, entropy_value);
...
// static
FieldTrial*
FieldTrialList::Find(StringPiece trial_name)
{
    if (!global_)
        return nullptr;
...
// static
int
FieldTrialList::FindValue(StringPiece trial_name)
{
...
// static
std::string
FieldTrialList::FindFullName(StringPiece trial_name)
{
    FieldTrial* field_trial = Find(trial_name);
    if (field_trial)
...
// static
bool
FieldTrialList::TrialExists(StringPiece trial_name)
{
...
// static
bool
FieldTrialList::IsTrialActive(StringPiece trial_name)
{
    FieldTrial* field_trial = Find(trial_name);
...
// static
void
FieldTrialList::StatesToString(std::string* output)
{
    FieldTrial::ActiveGroups active_groups;
    GetActiveFieldTrialGroups(&active_groups);
    for (const auto& active_group : active_groups)
    {
        DCHECK_EQ(std::string::npos, active_group.trial_name.find(kPersistentStringSeparator));
        DCHECK_EQ(std::string::npos, active_group.group_name.find(kPersistentStringSeparator));
...
// static
void
FieldTrialList::AllStatesToString(std::string* output, bool include_disabled)
{
    if (!global_)
        return;
    AutoLock auto_lock(global_->lock_);
    ...
    for (const auto& registered : global_->registered_)
    {
        ...
        if (!registered.second->GetStateWhileLocked(&trial, include_disabled))
            continue;
        DCHECK_EQ(std::string::npos, trial.trial_name->find(kPersistentStringSeparator));
        DCHECK_EQ(std::string::npos, trial.group_name->find(kPersistentStringSeparator));
        if (trial.activated)
...
// static
std::string
FieldTrialList::AllParamsToString(bool include_disabled, EscapeDataFunc encode_data_func)
{
    FieldTrialParamAssociator* params_associator = FieldTrialParamAssociator::GetInstance();
    std::string output;
...
// static
void
FieldTrialList::GetActiveFieldTrialGroups(FieldTrial::ActiveGroups* active_groups)
{
...
// static
void
FieldTrialList::GetActiveFieldTrialGroupsFromString(const std::string& trials_string,
                                                    FieldTrial::ActiveGroups* active_groups)
{
    std::vector<FieldTrialStringEntry> entries;
...
// static
void
FieldTrialList::GetInitiallyActiveFieldTrials(const CommandLine& command_line,
                                              FieldTrial::ActiveGroups* active_groups)
{
    DCHECK(global_);
    ...
    if (!global_->field_trial_allocator_)
    {
        GetActiveFieldTrialGroupsFromString(
                command_line.GetSwitchValueASCII(switches::kForceFieldTrials), active_groups);
        return;
    }
    ...
    while ((entry = mem_iter.GetNextOfObject<FieldTrial::FieldTrialEntry>()) != nullptr)
    {
        StringPiece trial_name;
        StringPiece group_name;
...
// static
bool
FieldTrialList::CreateTrialsFromString(const std::string& trials_string)
{
    DCHECK(global_);
...
// static
void
FieldTrialList::CreateTrialsFromCommandLine(const CommandLine& cmd_line,
                                            const char* field_trial_handle_switch,
                                            int fd_key)
{
    global_->create_trials_from_command_line_called_ = true;

#if IS_WIN || defined(OS_FUCHSIA) || defined(OS_MAC)
    if (cmd_line.HasSwitch(field_trial_handle_switch))
    {
        ...
        UMA_HISTOGRAM_BOOLEAN("ChildProcess.FieldTrials.CreateFromShmemSuccess", result);
    }
#elif IS_POSIX && !defined(OS_NACL)
    ...
    if (cmd_line.HasSwitch(switches::kForceFieldTrials))
    {
        ...
        UMA_HISTOGRAM_BOOLEAN("ChildProcess.FieldTrials.CreateFromSwitchSuccess", result);
        DCHECK(result);
    }
}
...
// static
void
FieldTrialList::CreateFeaturesFromCommandLine(const CommandLine& command_line,
                                              ...
                                              FeatureList* feature_list)
{
    ...
    feature_list->InitializeFromSharedMemory(global_->field_trial_allocator_.get());
}

#if IS_WIN
// static
void
FieldTrialList::AppendFieldTrialHandleIfNeeded(HandlesToInheritVector* handles)
{
    if (!global_)
        return;
...
// static
void
FieldTrialList::InsertFieldTrialHandleIfNeeded(MachPortsForRendezvous* rendezvous_ports)
{
    if (!global_)
        return;
    InstantiateFieldTrialAllocatorIfNeeded();
    if (global_->readonly_allocator_region_.IsValid())
    {
        rendezvous_ports->emplace(
                ...
                MachRendezvousPort(global_->readonly_allocator_region_.GetPlatformHandle(),
...
// static
int
FieldTrialList::GetFieldTrialDescriptor()
{
    InstantiateFieldTrialAllocatorIfNeeded();
...
// static
void
FieldTrialList::CopyFieldTrialStateToFlags(const char* field_trial_handle_switch,
                                           const char* enable_features_switch,
                                           const char* disable_features_switch,
                                           CommandLine* cmd_line)
{
    ...
    if (!global_ || !global_->readonly_allocator_region_.IsValid())
    {
        AddFeatureAndFieldTrialFlags(enable_features_switch, disable_features_switch, cmd_line);
        return;
    }
    ...
    std::string enabled_features;
    std::string disabled_features;
    FeatureList::GetInstance()->GetCommandLineFeatureOverrides(&enabled_features,
                                                               &disabled_features);
...
// static
FieldTrial*
FieldTrialList::CreateFieldTrial(StringPiece name, StringPiece group_name)
{
    DCHECK(global_);
...
// static
bool
FieldTrialList::AddObserver(Observer* observer)
{
    if (!global_)
        return false;
    AutoLock auto_lock(global_->lock_);
...
// static
void
FieldTrialList::RemoveObserver(Observer* observer)
{
    if (!global_)
        return;
    AutoLock auto_lock(global_->lock_);
...
// static
void
FieldTrialList::OnGroupFinalized(bool is_locked, FieldTrial* field_trial)
{
    if (!global_)
        return;
    if (is_locked)
    {
        AddToAllocatorWhileLocked(global_->field_trial_allocator_.get(), field_trial);
    }
    else
    {
        AutoLock auto_lock(global_->lock_);
...
// static
void
FieldTrialList::NotifyFieldTrialGroupSelection(FieldTrial* field_trial)
{
    if (!global_)
        return;
...
// static
size_t
FieldTrialList::GetFieldTrialCount()
{
    if (!global_)
        return 0;
    AutoLock auto_lock(global_->lock_);
...
// static
bool
FieldTrialList::GetParamsFromSharedMemory(FieldTrial* field_trial,
                                          std::map<std::string, std::string>* params)
{
    DCHECK(global_);
    ...
    size_t allocated_size = global_->field_trial_allocator_->GetAllocSize(field_trial->ref_);
...
FieldTrialList::ClearParamsFromSharedMemoryForTesting()
{
           ...
           != FieldTrialAllocator::kReferenceNull)
    {
        ...
        char* dst = reinterpret_cast<char*>(new_entry) + sizeof(FieldTrial::FieldTrialEntry);
...
// static
void
FieldTrialList::DumpAllFieldTrialsToPersistentAllocator(PersistentMemoryAllocator* allocator)
{
    if (!global_)
        return;
    AutoLock auto_lock(global_->lock_);
...
FieldTrialList::GetAllFieldTrialsFromPersistentAllocator(PersistentMemoryAllocator const& allocator)
{
    std::vector<const FieldTrial::FieldTrialEntry*> entries;
    FieldTrialAllocator::Iterator iter(&allocator);
    const FieldTrial::FieldTrialEntry* entry;
    while ((entry = iter.GetNextOfObject<FieldTrial::FieldTrialEntry>()) != nullptr)
    {
...
// static
void
FieldTrialList::RestoreInstanceForTesting(FieldTrialList* instance)
{
    global_ = instance;
}
...
// static
std::string
FieldTrialList::SerializeSharedMemoryRegionMetadata(const ReadOnlySharedMemoryRegion& shm)
{
    std::stringstream ss;
#if IS_WIN
    // Tell the child process the name of the inherited HANDLE.
    uintptr_t uintptr_handle = reinterpret_cast<uintptr_t>(shm.GetPlatformHandle());
    ss << uintptr_handle << ",";
#elif defined(OS_FUCHSIA)
...
// static
ReadOnlySharedMemoryRegion
FieldTrialList::DeserializeSharedMemoryRegionMetadata(const std::string& switch_value)
{
    ...
    if (!rendezvous)
        return ReadOnlySharedMemoryRegion();
    mac::ScopedMachSendRight scoped_handle = rendezvous->TakeSendRight(field_trial_handle);
...
// static
ReadOnlySharedMemoryRegion
FieldTrialList::DeserializeSharedMemoryRegionMetadata(int fd, const std::string& switch_value)
{
    std::vector<StringPiece> tokens =
            SplitStringPiece(switch_value, ",", KEEP_WHITESPACE, SPLIT_WANT_ALL);
...
// static
bool
FieldTrialList::CreateTrialsFromSwitchValue(const std::string& switch_value)
{
    ReadOnlySharedMemoryRegion shm = DeserializeSharedMemoryRegionMetadata(switch_value);
    if (!shm.IsValid())
        return false;
...
// static
bool
FieldTrialList::CreateTrialsFromDescriptor(int fd_key, const std::string& switch_value)
{
    if (fd_key == -1)
        return false;
    ...
    ReadOnlySharedMemoryRegion shm = DeserializeSharedMemoryRegionMetadata(fd, switch_value);
    if (!shm.IsValid())
        return false;
...
#endif  // IS_POSIX && !defined(OS_NACL)

// static
bool
FieldTrialList::CreateTrialsFromSharedMemoryRegion(const ReadOnlySharedMemoryRegion& shm_region)
{
    ReadOnlySharedMemoryMapping shm_mapping = shm_region.MapAt(0, kFieldTrialAllocationSize);
    if (!shm_mapping.IsValid())
        OnOutOfMemory(kFieldTrialAllocationSize);

    return FieldTrialList::CreateTrialsFromSharedMemoryMapping(std::move(shm_mapping));
}

// static
bool
FieldTrialList::CreateTrialsFromSharedMemoryMapping(ReadOnlySharedMemoryMapping shm_mapping)
{
    global_->field_trial_allocator_ = std::make_unique<ReadOnlySharedPersistentMemoryAllocator>(
...
FieldTrialList::InstantiateFieldTrialAllocatorIfNeeded()
{
    ...
    MappedReadOnlyRegion shm = ReadOnlySharedMemoryRegion::Create(kFieldTrialAllocationSize);

    if (!shm.IsValid())
        OnOutOfMemory(kFieldTrialAllocationSize);

    global_->field_trial_allocator_ = std::make_unique<WritableSharedPersistentMemoryAllocator>(
    ...
    for (const auto& registered : global_->registered_)
    {
        AddToAllocatorWhileLocked(global_->field_trial_allocator_.get(), registered.second);
    }

    // Add all existing features.
    FeatureList::GetInstance()->AddFeaturesToAllocator(global_->field_trial_allocator_.get());
...
// static
void
FieldTrialList::AddToAllocatorWhileLocked(PersistentMemoryAllocator* allocator,
                                          FieldTrial* field_trial)
{
    ...
    FieldTrial::FieldTrialRef ref =
            allocator->Allocate(total_size, FieldTrial::FieldTrialEntry::kPersistentTypeId);
    ...
    FieldTrial::FieldTrialEntry* entry = allocator->GetAsObject<FieldTrial::FieldTrialEntry>(ref);
    ...
    char* dst = reinterpret_cast<char*>(entry) + sizeof(FieldTrial::FieldTrialEntry);
    memcpy(dst, pickle.data(), pickle.size());
...
// static
void
FieldTrialList::ActivateFieldTrialEntryWhileLocked(FieldTrial* field_trial)
{
    FieldTrialAllocator* allocator = global_->field_trial_allocator_.get();
...
FieldTrial*
FieldTrialList::PreLockedFind(StringPiece name)
{
...
// static
void
FieldTrialList::Register(FieldTrial* trial)
{
    if (!global_)
    {
        used_without_global_ = true;
...
// static
FieldTrialList::RegistrationMap
FieldTrialList::GetRegisteredTrials()
{
