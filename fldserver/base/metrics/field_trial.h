...
#include "fldserver/base/memory/read_only_shared_memory_region.h"
...
#include "fldserver/base/observer_list_threadsafe.h"
#include "fldserver/base/pickle.h"
#include "fldserver/base/process/launch.h"
#include "fldserver/base/strings/string_piece.h"
#include "fldserver/base/synchronization/lock.h"
#include "fldserver/fldserver_config.h"

#if defined(OS_MAC)
#include "fldserver/base/mac/mach_port_rendezvous.h"
#endif
...
class CORE_EXPORT FieldTrial : public RefCounted<FieldTrial>
{
public:
    ...
    class CORE_EXPORT EntropyProvider
    {
    public:
        ...
        virtual double
        GetEntropyForTrial(StringPiece trial_name, uint32_t randomization_seed) const = 0;
    };
    ...
    struct CORE_EXPORT FieldTrialEntry
    {
        ...
        bool
        GetTrialAndGroupName(StringPiece* trial_name, StringPiece* group_name) const;
    ...
    FieldTrial&
    operator=(const FieldTrial&) = delete;
    ...
    int
    AppendGroup(const std::string& name, Probability group_probability);
    ...
    static FieldTrial*
    CreateSimulatedFieldTrial(StringPiece trial_name,
...
private:
    ...
    void
    SetGroupChoice(const std::string& group_name, int number);
    ...
    void
    FinalizeGroupChoiceImpl(bool is_locked);
    ...
    // untouched.
    bool
    GetActiveGroup(ActiveGroup* active_group) const;
    ...
    // left untouched.
    bool
    GetStateWhileLocked(State* field_trial_state, bool include_disabled);

    // Returns the group_name. A winner need not have been chosen.
    const std::string&
    group_name_internal() const
    {
        return group_name_;
    }
...
class CORE_EXPORT FieldTrialList
{
public:
    ...
    public:
        ...
        virtual void
        OnFieldTrialGroupFinalized(const std::string& trial_name,
    ...
    // |entropy_provider|.
    explicit FieldTrialList(std::unique_ptr<const FieldTrial::EntropyProvider> entropy_provider);
    FieldTrialList(const FieldTrialList&) = delete;
    FieldTrialList&
    operator=(const FieldTrialList&) = delete;
    ...
    static FieldTrial*
    FactoryGetFieldTrial(StringPiece trial_name,
    ...
    static void
    StatesToString(std::string* output);
    ...
    // by |CreateTrialsFromString()|.
    static void
    AllStatesToString(std::string* output, bool include_disabled);
    ...
    static std::string
    AllParamsToString(bool include_disabled, EscapeDataFunc encode_data_func);
    ...
    // not been disabled.
    static void
    GetActiveFieldTrialGroups(FieldTrial::ActiveGroups* active_groups);

    // Returns the field trials that are marked active in |trials_string|.
    static void
    GetActiveFieldTrialGroupsFromString(const std::string& trials_string,
    ...
    static void
    GetInitiallyActiveFieldTrials(const CommandLine& command_line,
                                  FieldTrial::ActiveGroups* active_groups);
    ...
    static bool
    CreateTrialsFromString(const std::string& trials_string);
    ...
    static void
    CreateTrialsFromCommandLine(const CommandLine& cmd_line,
    ...
    static void
    CreateFeaturesFromCommandLine(const CommandLine& command_line,
    ...
    static void
    AppendFieldTrialHandleIfNeeded(HandlesToInheritVector* handles);
    ...
    static void
    InsertFieldTrialHandleIfNeeded(MachPortsForRendezvous* rendezvous_ports);
    ...
    // descriptor.
    static int
    GetFieldTrialDescriptor();
#endif
    static ReadOnlySharedMemoryRegion
    ...
    static FieldTrial*
    CreateFieldTrial(StringPiece name, StringPiece group_name);
    ...
    static void
    OnGroupFinalized(bool is_locked, FieldTrial* field_trial);

    // Notify all observers that a group has been finalized for |field_trial|.
    static void
    NotifyFieldTrialGroupSelection(FieldTrial* field_trial);
    ...
    static bool
    GetParamsFromSharedMemory(FieldTrial* field_trial, std::map<std::string, std::string>* params);

    // Clears all the params in the allocator.
    static void
    ClearParamsFromSharedMemoryForTesting();

    // Dumps field trial state to an allocator so that it can be analyzed after a
    // crash.
    static void
    DumpAllFieldTrialsToPersistentAllocator(PersistentMemoryAllocator* allocator);
    ...
    GetAllFieldTrialsFromPersistentAllocator(PersistentMemoryAllocator const& allocator);
    ...
    static FieldTrialList*
    BackupInstanceForTesting();

    // For testing, sets the global instance to |instance|.
    static void
    RestoreInstanceForTesting(FieldTrialList* instance);
...
private:
    ...
    friend int
    SerializeSharedMemoryRegionMetadata(void);
    ...
    static std::string
    SerializeSharedMemoryRegionMetadata(const ReadOnlySharedMemoryRegion& shm);
#if IS_WIN || defined(OS_FUCHSIA) || defined(OS_MAC)
    static ReadOnlySharedMemoryRegion
    DeserializeSharedMemoryRegionMetadata(const std::string& switch_value);
#elif IS_POSIX && !defined(OS_NACL)
    static ReadOnlySharedMemoryRegion
    DeserializeSharedMemoryRegionMetadata(int fd, const std::string& switch_value);
#endif
    ...
    static bool
    CreateTrialsFromSwitchValue(const std::string& switch_value);
#elif IS_POSIX && !defined(OS_NACL)
    ...
    // |switch_value| also contains the serialized GUID.
    static bool
    CreateTrialsFromDescriptor(int fd_key, const std::string& switch_value);
#endif
    ...
    static bool
    CreateTrialsFromSharedMemoryRegion(const ReadOnlySharedMemoryRegion& shm_region);
    ...
    static bool
    CreateTrialsFromSharedMemoryMapping(ReadOnlySharedMemoryMapping shm_mapping);
    ...
    // calling this.
    static void
    AddToAllocatorWhileLocked(PersistentMemoryAllocator* allocator, FieldTrial* field_trial);

    // Activate the corresponding field trial entry struct in shared memory.
    static void
    ActivateFieldTrialEntryWhileLocked(FieldTrial* field_trial);
    ...
    FieldTrial*
    PreLockedFind(StringPiece name) EXCLUSIVE_LOCKS_REQUIRED(lock_);
