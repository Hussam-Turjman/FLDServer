...
class CORE_EXPORT PersistentSampleMap : public HistogramSamples
{
public:
    // Constructs a persistent sample map using a PersistentHistogramAllocator
    // as the data source for persistent records.
    PersistentSampleMap(uint64_t id, PersistentHistogramAllocator* allocator, Metadata* meta);
    ...
    static PersistentMemoryAllocator::Reference
    GetNextPersistentRecord(PersistentMemoryAllocator::Iterator& iterator, uint64_t* sample_map_id);
    ...
    static PersistentMemoryAllocator::Reference
    CreatePersistentRecord(PersistentMemoryAllocator* allocator,
...
protected:
    ...
    bool
    AddSubtractImpl(SampleCountIterator* iter, Operator op) override;

    // Gets a pointer to a "count" corresponding to a given |value|. Returns NULL
    // if sample does not exist.
    HistogramBase::Count*
    GetSampleCountStorage(HistogramBase::Sample value);
    ...
    HistogramBase::Count*
    GetOrCreateSampleCountStorage(HistogramBase::Sample value);
...
private:
    ...
    HistogramBase::Count*
    ImportSamples(HistogramBase::Sample until_value, bool import_everything);
