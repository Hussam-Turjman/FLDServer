...
class CORE_EXPORT HistogramDeltaSerialization : public HistogramFlattener
{
public:
    ...
    void
    PrepareAndSerializeDeltas(std::vector<std::string>* serialized_deltas, bool include_persistent);
    ...
    static void
    DeserializeAndAddSamples(const std::vector<std::string>& serialized_deltas);

private:
    // HistogramFlattener implementation.
    void
    RecordDelta(const HistogramBase& histogram, const HistogramSamples& snapshot) override;
