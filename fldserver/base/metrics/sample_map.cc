...
bool
SampleMapIterator::Done() const
{
    return iter_ == end_;
}

void
SampleMapIterator::Next()
{
    DCHECK(!Done());
    ++iter_;
    SkipEmptyBuckets();
...
void
SampleMapIterator::Get(Sample* min, int64_t* max, Count* count) const
{
    DCHECK(!Done());
    if (min)
        *min = iter_->first;
...
void
SampleMapIterator::SkipEmptyBuckets()
{
...
void
SampleMap::Accumulate(Sample value, Count count)
{
...
Count
SampleMap::GetCount(Sample value) const
{
...
bool
SampleMap::AddSubtractImpl(SampleCountIterator* iter, Operator op)
{
    Sample min;
    int64_t max;
    Count count;
