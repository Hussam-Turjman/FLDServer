...
#include "fldserver/base/at_exit.h"
#include "fldserver/base/debug/leak_annotations.h"
#include "fldserver/base/json/string_escape.h"
#include "fldserver/base/logging.h"
#include "fldserver/base/memory/ptr_util.h"
#include "fldserver/base/metrics/histogram.h"
#include "fldserver/base/metrics/histogram_snapshot_manager.h"
#include "fldserver/base/metrics/metrics_hashes.h"
#include "fldserver/base/metrics/persistent_histogram_allocator.h"
#include "fldserver/base/metrics/record_histogram_checker.h"
...
{
bool
HistogramNameLesser(const base::HistogramBase* a, const base::HistogramBase* b)
{
...
// static
std::atomic<StatisticsRecorder::GlobalSampleCallback> StatisticsRecorder::global_sample_callback_{
        nullptr};

size_t
StatisticsRecorder::BucketRangesHash::operator()(const BucketRanges* const a) const
{
    return a->checksum();
}

bool
StatisticsRecorder::BucketRangesEqual::operator()(const BucketRanges* const a,
...
// static
void
StatisticsRecorder::EnsureGlobalRecorderWhileLocked()
{
...
// static
void
StatisticsRecorder::RegisterHistogramProvider(const WeakPtr<HistogramProvider>& provider)
{
    const AutoLock auto_lock(lock_.Get());
...
// static
HistogramBase*
StatisticsRecorder::RegisterOrDeleteDuplicate(HistogramBase* histogram)
{
...
// static
const BucketRanges*
StatisticsRecorder::RegisterOrDeleteDuplicateRanges(const BucketRanges* ranges)
{
...
// static
void
StatisticsRecorder::WriteGraph(const std::string& query, std::string* output)
{
    if (query.length())
    ...
    for (const HistogramBase* const histogram : Sort(WithName(GetHistograms(), query)))
    {
...
// static
std::string
StatisticsRecorder::ToJSON(JSONVerbosityLevel verbosity_level)
{
...
// static
std::vector<const BucketRanges*>
StatisticsRecorder::GetBucketRanges()
{
...
// static
HistogramBase*
StatisticsRecorder::FindHistogram(base::StringPiece name)
{
...
// static
void
StatisticsRecorder::PrepareDeltas(bool include_persistent,
                                  ...
                                  HistogramSnapshotManager* snapshot_manager)
{
    ...
    snapshot_manager->PrepareDeltas(Sort(std::move(histograms)), flags_to_set, required_flags);
}

// static
void
StatisticsRecorder::InitLogOnShutdown()
{
    const AutoLock auto_lock(lock_.Get());
...
// static
bool
StatisticsRecorder::SetCallback(const std::string& name, StatisticsRecorder::OnSampleCallback cb)
{
    DCHECK(!cb.is_null());
    const AutoLock auto_lock(lock_.Get());
    EnsureGlobalRecorderWhileLocked();
    ...
    have_active_callbacks_.store(global_sample_callback() || !top_->callbacks_.empty(),
...
// static
void
StatisticsRecorder::ClearCallback(const std::string& name)
{
    const AutoLock auto_lock(lock_.Get());
    EnsureGlobalRecorderWhileLocked();
...
StatisticsRecorder::FindCallback(const std::string& name)
{
    const AutoLock auto_lock(lock_.Get());
    EnsureGlobalRecorderWhileLocked();
...
// static
void
StatisticsRecorder::SetGlobalSampleCallback(const GlobalSampleCallback& new_global_sample_callback)
{
    const AutoLock auto_lock(lock_.Get());
    EnsureGlobalRecorderWhileLocked();
    ...
    have_active_callbacks_.store(new_global_sample_callback || !top_->callbacks_.empty(),
                                 std::memory_order_relaxed);
}
...
// static
void
StatisticsRecorder::ForgetHistogramForTesting(base::StringPiece name)
{
    const AutoLock auto_lock(lock_.Get());
    EnsureGlobalRecorderWhileLocked();
...
// static
void
StatisticsRecorder::SetRecordChecker(std::unique_ptr<RecordHistogramChecker> record_checker)
{
    const AutoLock auto_lock(lock_.Get());
    EnsureGlobalRecorderWhileLocked();
...
// static
bool
StatisticsRecorder::ShouldRecordHistogram(uint64_t histogram_hash)
{
    const AutoLock auto_lock(lock_.Get());
    EnsureGlobalRecorderWhileLocked();
    return !top_->record_checker_ || top_->record_checker_->ShouldRecord(histogram_hash);
}

// static
StatisticsRecorder::Histograms
...
// static
StatisticsRecorder::Histograms
StatisticsRecorder::Sort(Histograms histograms)
{
...
// static
StatisticsRecorder::Histograms
StatisticsRecorder::WithName(Histograms histograms, const std::string& query)
{
    // Need a C-string query for comparisons against C-string histogram name.
    const char* const query_string = query.c_str();
    histograms.erase(ranges::remove_if(histograms,
...
// static
StatisticsRecorder::Histograms
StatisticsRecorder::NonPersistent(Histograms histograms)
{
    histograms.erase(ranges::remove_if(histograms,
...
// static
void
StatisticsRecorder::InitLogOnShutdownWhileLocked()
{
    lock_.Get().AssertAcquired();
