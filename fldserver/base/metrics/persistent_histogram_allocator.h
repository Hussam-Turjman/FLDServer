...
CORE_EXPORT extern const Feature kPersistentHistogramsFeature;
...
// for each.
class CORE_EXPORT PersistentSparseHistogramDataManager
{
public:
    // Constructs the data manager. The allocator must live longer than any
    // managers that reference it.
    explicit PersistentSparseHistogramDataManager(PersistentMemoryAllocator* allocator);
    ...
    UseSampleMapRecords(uint64_t id, const void* user);
    ...
    template <typename T>
    T*
    GetAsObject(PersistentMemoryAllocator::Reference ref)
    {
...
private:
    ...
    PersistentSampleMapRecords*
    GetSampleMapRecordsWhileLocked(uint64_t id) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    bool
    LoadRecords(PersistentSampleMapRecords* sample_map_records);
...
class CORE_EXPORT PersistentSampleMapRecords
{
public:
    ...
    // a reference to it.
    PersistentMemoryAllocator::Reference
    CreateNew(HistogramBase::Sample value);
...
class CORE_EXPORT PersistentHistogramAllocator
{
public:
    ...
    public:
        ...
        std::unique_ptr<HistogramBase>
        GetNextWithIgnore(Reference ignore);
    ...
    explicit PersistentHistogramAllocator(std::unique_ptr<PersistentMemoryAllocator> memory);
    ...
    Id() const
    {
        return memory_allocator_->Id();
    }
    const char*
    Name() const
    {
        return memory_allocator_->Name();
    }
    const void*
    data() const
    {
        return memory_allocator_->data();
    }
    size_t
    length() const
    {
        return memory_allocator_->length();
    }
    size_t
    size() const
    {
    ...
    std::unique_ptr<HistogramBase>
    AllocateHistogram(HistogramType histogram_type,
    ...
    void
    FinalizeHistogram(Reference ref, bool registered);
    ...
    void
    MergeHistogramDeltaToStatisticsRecorder(HistogramBase* histogram);
    ...
    // get recorded again.
    void
    MergeHistogramFinalDeltaToStatisticsRecorder(const HistogramBase* histogram);
    ...
    void
    CreateTrackingHistograms(StringPiece name);
    void
    ...
    void
    ClearLastCreatedReferenceForTesting();
...
protected:
    ...
    PersistentHistogramAllocator::Reference
    last_created()
    {
    ...
    std::unique_ptr<HistogramBase>
    GetNextHistogramWithIgnore(Iterator* iter, Reference ignore);

private:
    // Create a histogram based on saved (persistent) information about it.
    std::unique_ptr<HistogramBase>
    CreateHistogram(PersistentHistogramData* histogram_data_ptr);
    ...
    HistogramBase*
    GetOrCreateStatisticsRecorderHistogram(const HistogramBase* histogram);
...
class CORE_EXPORT GlobalHistogramAllocator : public PersistentHistogramAllocator
{
public:
    ...
    static void
    CreateWithLocalMemory(size_t size, uint64_t id, StringPiece name);
    ...
    // was set.
    static bool
    CreateWithFile(const FilePath& file_path, size_t size, uint64_t id, StringPiece name);
    ...
    static bool
    CreateWithActiveFile(const FilePath& base_path,
    ...
    // |dir|.
    static bool
    CreateWithActiveFileInDir(const FilePath& dir, size_t size, uint64_t id, StringPiece name);

    // Constructs a filename using a name.
    static FilePath
    ConstructFilePath(const FilePath& dir, StringPiece name);

    // Constructs a filename using a name for an "active" file.
    static FilePath
    ConstructFilePathForActiveFile(const FilePath& dir, StringPiece name);

    // Like above but with timestamp and pid for use in upload directories.
    static FilePath
    ConstructFilePathForUploadDir(const FilePath& dir,
                                  StringPiece name,
                                  base::Time stamp,
                                  ProcessId pid);
    ...
    static FilePath
    ConstructFilePathForUploadDir(const FilePath& dir, StringPiece name);

    // Parses a filename to extract name, timestamp, and pid.
    static bool
    ParseFilePath(const FilePath& path, std::string* out_name, Time* out_stamp, ProcessId* out_pid);

    // Create a "spare" file that can later be made the "active" file. This
    // should be done on a background thread if possible.
    static bool
    CreateSpareFile(const FilePath& spare_path, size_t size);
#endif
    ...
    static void
    CreateWithSharedMemoryRegion(const WritableSharedMemoryRegion& region);
    ...
    static void
    Set(std::unique_ptr<GlobalHistogramAllocator> allocator);

    // Gets a pointer to the global histogram allocator. Returns null if none
    // exists.
    static GlobalHistogramAllocator*
    Get();
...
private:
    ...
    explicit GlobalHistogramAllocator(std::unique_ptr<PersistentMemoryAllocator> memory);
    ...
    static FilePath
    MakeMetricsFilePath(const FilePath& dir, StringPiece name);
