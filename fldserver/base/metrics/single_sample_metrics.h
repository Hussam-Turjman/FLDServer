...
class CORE_EXPORT SingleSampleMetric
{
public:
    virtual ~SingleSampleMetric() = default;

    virtual void
    SetSample(HistogramBase::Sample sample) = 0;
};
...
class CORE_EXPORT SingleSampleMetricsFactory
{
public:
    ...
    static SingleSampleMetricsFactory*
    Get();
    static void
    SetFactory(std::unique_ptr<SingleSampleMetricsFactory> factory);

    // The factory normally persists until process shutdown, but in testing we
    // should avoid leaking it since it sets a global.
    static void
    DeleteFactoryForTesting();
    ...
    virtual std::unique_ptr<SingleSampleMetric>
    CreateCustomCountsMetric(const std::string& histogram_name,
...
class CORE_EXPORT DefaultSingleSampleMetricsFactory : public SingleSampleMetricsFactory
{
public:
    DefaultSingleSampleMetricsFactory() = default;
    DefaultSingleSampleMetricsFactory(const DefaultSingleSampleMetricsFactory&) = delete;
    DefaultSingleSampleMetricsFactory&
    operator=(const DefaultSingleSampleMetricsFactory&) = delete;
...
class CORE_EXPORT DefaultSingleSampleMetric : public SingleSampleMetric
{
public:
    ...
    // SingleSampleMetric:
    void
    SetSample(HistogramBase::Sample sample) override;
