...
struct EnumSizeTraits<Enum, std::enable_if_t<std::is_enum<decltype(Enum::kMaxValue)>::value>>
{
    static constexpr Enum
    Count()
    {
        return static_cast<Enum>(static_cast<std::underlying_type_t<Enum>>(Enum::kMaxValue) + 1);
    }
};
...
#define HISTOGRAM_POINTER_USE(atomic_histogram_pointer,                                            \
    ...
    do                                                                                             \
    {                                                                                              \
        base::HistogramBase* histogram_pointer(reinterpret_cast<base::HistogramBase*>(             \
        ...
        {                                                                                          \
             ...
             * makes things clear.                                                                 \
             */                                                                                    \
            atomic_histogram_pointer->store(reinterpret_cast<uintptr_t>(histogram_pointer),        \
...
#define STATIC_HISTOGRAM_POINTER_BLOCK(constant_histogram_name,                                    \
    ...
    {                                                                                              \
        ...
        HISTOGRAM_POINTER_USE(std::addressof(atomic_histogram_pointer),                            \
                              constant_histogram_name,                                             \
                              histogram_add_method_invocation,                                     \
                              histogram_factory_get_invocation);                                   \
    } while (0)
...
// This is a helper macro used by other macros and shouldn't be used directly.
#define INTERNAL_HISTOGRAM_CUSTOM_COUNTS_WITH_FLAG(name, sample, min, max, bucket_count, flag)     \
    STATIC_HISTOGRAM_POINTER_BLOCK(                                                                \
            name, Add(sample), base::Histogram::FactoryGet(name, min, max, bucket_count, flag))
...
#define INTERNAL_HISTOGRAM_EXACT_LINEAR_WITH_FLAG(name, sample, boundary, flag)                    \
    ...
    {                                                                                              \
        ...
        STATIC_HISTOGRAM_POINTER_BLOCK(                                                            \
                name,                                                                              \
                Add(sample),                                                                       \
                base::LinearHistogram::FactoryGet(name, 1, boundary, boundary + 1, flag));         \
    } while (0)
    ...
    {                                                                                              \
        ...
        public:                                                                                    \
            ScaledLinearHistogramInstance() :                                                      \
                ScaledLinearHistogram(name, 1, boundary, boundary + 1, scale, flag)                \
            {                                                                                      \
            }                                                                                      \
        };                                                                                         \
...
#define INTERNAL_UMA_HISTOGRAM_ENUMERATION_DEDUCE_BOUNDARY(name, sample, flags)                    \
...
#define INTERNAL_UMA_HISTOGRAM_ENUMERATION_SPECIFY_BOUNDARY(name, sample, enum_size, flags)        \
...
#define INTERNAL_HISTOGRAM_ENUMERATION_WITH_FLAG(name, sample, boundary, flag)                     \
    ...
    {                                                                                              \
        ...
        static_assert(static_cast<uintmax_t>(boundary) < static_cast<uintmax_t>(                   \
                              std::numeric_limits<base::HistogramBase::Sample>::max()),            \
                      "|boundary| is out of range of HistogramBase::Sample");                      \
        INTERNAL_HISTOGRAM_EXACT_LINEAR_WITH_FLAG(                                                 \
                name,                                                                              \
                static_cast<base::HistogramBase::Sample>(sample),                                  \
                static_cast<base::HistogramBase::Sample>(boundary),                                \
                flag);                                                                             \
    } while (0)

#define INTERNAL_HISTOGRAM_SCALED_ENUMERATION_WITH_FLAG(name, sample, count, scale, flag)          \
    do                                                                                             \
    {                                                                                              \
        using decayed_sample = std::decay<decltype(sample)>::type;                                 \
        static_assert(std::is_enum<decayed_sample>::value,                                         \
        ...
        {                                                                                          \
            ...
            {                                                                                      \
                case ScopedHistogramTiming::kMicrosecondTimes:                                     \
                    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(name,                                  \
                                                            elapsed,                               \
                                                            base::TimeDelta::FromMicroseconds(1),  \
                                                            base::TimeDelta::FromSeconds(1),       \
                                                            50);                                   \
                    break;                                                                         \
                case ScopedHistogramTiming::kMediumTimes: UMA_HISTOGRAM_TIMES(name, elapsed);      \
                        break;                                                                     \
