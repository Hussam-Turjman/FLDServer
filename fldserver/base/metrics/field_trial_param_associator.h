...
// is thread-safe.
class CORE_EXPORT FieldTrialParamAssociator
{
public:
    ...
    bool
    GetFieldTrialParams(FieldTrial* field_trial, FieldTrialParams* params);
    ...
    bool
    GetFieldTrialParamsWithoutFallback(const std::string& trial_name,
                                       const std::string& group_name,
    ...
    // shared memory.
    void
    ClearAllParamsForTesting();

    // Clears a single field trial param.
    // Note: this does NOT remove the param in shared memory.
    void
    ClearParamsForTesting(const std::string& trial_name, const std::string& group_name);
