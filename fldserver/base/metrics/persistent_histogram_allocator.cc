...
#include "fldserver/base/metrics/persistent_histogram_allocator.h"

#include "fldserver/base/atomicops.h"
#include "fldserver/base/files/file_path.h"
#include "fldserver/base/files/file_util.h"
#include "fldserver/base/files/important_file_writer.h"
...
#include "fldserver/base/memory/shared_memory_mapping.h"
#include "fldserver/base/memory/writable_shared_memory_region.h"
#include "fldserver/base/metrics/histogram.h"
#include "fldserver/base/metrics/histogram_base.h"
#include "fldserver/base/metrics/histogram_samples.h"
#include "fldserver/base/metrics/metrics_hashes.h"
#include "fldserver/base/metrics/persistent_sample_map.h"
#include "fldserver/base/metrics/sparse_histogram.h"
#include "fldserver/base/metrics/statistics_recorder.h"
...
#include "fldserver/base/strings/strcat.h"
...
std::unique_ptr<BucketRanges>
CreateRangesFromData(HistogramBase::Sample* ranges_data, uint32_t ranges_checksum, size_t count)
{
...
size_t
CalculateRequiredCountsBytes(size_t bucket_count)
{
...
const Feature kPersistentHistogramsFeature{"PersistentHistograms", FEATURE_DISABLED_BY_DEFAULT};
...
PersistentSparseHistogramDataManager::~PersistentSparseHistogramDataManager() = default;

PersistentSampleMapRecords*
PersistentSparseHistogramDataManager::UseSampleMapRecords(uint64_t id, const void* user)
{
...
PersistentSampleMapRecords*
PersistentSparseHistogramDataManager::GetSampleMapRecordsWhileLocked(uint64_t id)
{
...
bool
PersistentSparseHistogramDataManager::LoadRecords(PersistentSampleMapRecords* sample_map_records)
{
    ...
    for (int count = 0; !found || count < kMinimumNumberToLoad; ++count)
    {
        ...
        PersistentMemoryAllocator::Reference ref =
                PersistentSampleMap::GetNextPersistentRecord(record_iterator_, &found_id);
...
PersistentSampleMapRecords::PersistentSampleMapRecords(
        PersistentSparseHistogramDataManager* data_manager, uint64_t sample_map_id) :
...
void
PersistentSampleMapRecords::Release(const void* user)
{
...
PersistentSampleMapRecords::GetNext()
{
    DCHECK(user_);
...
PersistentMemoryAllocator::Reference
PersistentSampleMapRecords::CreateNew(HistogramBase::Sample value)
{
    return PersistentSampleMap::CreatePersistentRecord(
            data_manager_->allocator_, sample_map_id_, value);
}
...
PersistentHistogramAllocator::Iterator::Iterator(PersistentHistogramAllocator* allocator) :
...
PersistentHistogramAllocator::PersistentHistogramAllocator(
    ...
    memory_allocator_(std::move(memory)), sparse_histogram_data_manager_(memory_allocator_.get())
{
}
...
std::unique_ptr<HistogramBase>
PersistentHistogramAllocator::GetHistogram(Reference ref)
{
    ...
    PersistentHistogramData* data = memory_allocator_->GetAsObject<PersistentHistogramData>(ref);
    ...
    // unset/invalid).
    if (!data || data->name[0] == '\0' || reinterpret_cast<char*>(data)[length - 1] != '\0'
...
std::unique_ptr<HistogramBase>
PersistentHistogramAllocator::AllocateHistogram(HistogramType histogram_type,
                                                ...
                                                Reference* ref_ptr)
{
    ...
    PersistentHistogramData* histogram_data = memory_allocator_->New<PersistentHistogramData>(
    ...
    if (histogram_type != SPARSE_HISTOGRAM)
    {
        ...
        PersistentMemoryAllocator::Reference ranges_ref = bucket_ranges->persistent_reference();
        if (!ranges_ref)
        {
            size_t ranges_count = bucket_count + 1;
            size_t ranges_bytes = ranges_count * sizeof(HistogramBase::Sample);
            ranges_ref = memory_allocator_->Allocate(ranges_bytes, kTypeIdRangesArray);
            if (ranges_ref)
            {
...
void
PersistentHistogramAllocator::FinalizeHistogram(Reference ref, bool registered)
{
    if (registered)
    {
    ...
    else
    {
        ...
        memory_allocator_->ChangeType(ref,
                                      0,
...
void
PersistentHistogramAllocator::MergeHistogramDeltaToStatisticsRecorder(HistogramBase* histogram)
{
    DCHECK(histogram);
...
PersistentSampleMapRecords*
PersistentHistogramAllocator::UseSampleMapRecords(uint64_t id, const void* user)
{
    return sparse_histogram_data_manager_.UseSampleMapRecords(id, user);
}

void
PersistentHistogramAllocator::CreateTrackingHistograms(StringPiece name)
{
    memory_allocator_->CreateTrackingHistograms(name);
}

void
PersistentHistogramAllocator::UpdateTrackingHistograms()
{
    memory_allocator_->UpdateTrackingHistograms();
}

void
PersistentHistogramAllocator::ClearLastCreatedReferenceForTesting()
{
    subtle::NoBarrier_Store(&last_created_, 0);
}

std::unique_ptr<HistogramBase>
PersistentHistogramAllocator::CreateHistogram(PersistentHistogramData* histogram_data_ptr)
{
    ...
    if (histogram_data_ptr->histogram_type == SPARSE_HISTOGRAM)
    {
        std::unique_ptr<HistogramBase> histogram =
                SparseHistogram::PersistentCreate(this,
    ...
    HistogramBase::Sample* ranges_data = memory_allocator_->GetAsArray<HistogramBase::Sample>(
            histogram_ranges_ref, kTypeIdRangesArray, PersistentMemoryAllocator::kSizeAny);

    const uint32_t max_buckets =
            std::numeric_limits<uint32_t>::max() / sizeof(HistogramBase::Sample);
    size_t required_bytes = (histogram_bucket_count + 1) * sizeof(HistogramBase::Sample);
    size_t allocated_bytes = memory_allocator_->GetAllocSize(histogram_ranges_ref);
    if (!ranges_data || histogram_bucket_count < 2 || histogram_bucket_count >= max_buckets
    ...
    const BucketRanges* ranges =
            StatisticsRecorder::RegisterOrDeleteDuplicateRanges(created_ranges.release());
    ...
    if (counts_bytes == 0
        || (counts_ref != 0 && memory_allocator_->GetAllocSize(counts_ref) < counts_bytes))
    {
        return nullptr;
    }
    ...
    DelayedPersistentAllocation logged_data(memory_allocator_.get(),
                                            &histogram_data_ptr->counts_ref,
                                            kTypeIdCountsArray,
                                            counts_bytes,
                                            counts_bytes / 2,
    ...
    {
        case HISTOGRAM:
            histogram = Histogram::PersistentCreate(name,
                                                    histogram_minimum,
                                                    histogram_maximum,
                                                    ranges,
                                                    counts_data,
        ...
        case LINEAR_HISTOGRAM:
            histogram = LinearHistogram::PersistentCreate(name,
                                                          histogram_minimum,
                                                          histogram_maximum,
                                                          ranges,
                                                          counts_data,
        ...
        case BOOLEAN_HISTOGRAM:
            histogram = BooleanHistogram::PersistentCreate(name,
                                                           ranges,
                                                           counts_data,
                                                           logged_data,
                                                           &histogram_data_ptr->samples_metadata,
        ...
        case CUSTOM_HISTOGRAM:
            histogram = CustomHistogram::PersistentCreate(name,
                                                          ranges,
                                                          counts_data,
                                                          logged_data,
                                                          &histogram_data_ptr->samples_metadata,
...
HistogramBase*
PersistentHistogramAllocator::GetOrCreateStatisticsRecorderHistogram(const HistogramBase* histogram)
{
...
GlobalHistogramAllocator::CreateWithPersistentMemory(
        void* base, size_t size, size_t page_size, uint64_t id, StringPiece name)
{
    Set(WrapUnique(new GlobalHistogramAllocator(
            std::make_unique<PersistentMemoryAllocator>(base, size, page_size, id, name, false))));
}

// static
void
GlobalHistogramAllocator::CreateWithLocalMemory(size_t size, uint64_t id, StringPiece name)
{
    Set(WrapUnique(new GlobalHistogramAllocator(
...
// static
bool
GlobalHistogramAllocator::CreateWithFile(const FilePath& file_path,
                                         size_t size,
                                         uint64_t id,
                                         StringPiece name)
{
    ...
    File file(file_path,
              File::FLAG_OPEN_ALWAYS | File::FLAG_SHARE_DELETE | File::FLAG_READ
                      | File::FLAG_WRITE);
    ...
    {
        success =
                mmfile->Initialize(std::move(file), {0, size}, MemoryMappedFile::READ_WRITE_EXTEND);
    }
    if (!success || !FilePersistentMemoryAllocator::IsFileAcceptable(*mmfile, true))
    {
        return false;
    }

    Set(WrapUnique(new GlobalHistogramAllocator(std::make_unique<FilePersistentMemoryAllocator>(
...
// static
bool
GlobalHistogramAllocator::CreateWithActiveFile(const FilePath& base_path,
                                               ...
                                               StringPiece name)
{
    ...
    return base::GlobalHistogramAllocator::CreateWithFile(active_path, size, id, name);
}

// static
bool
GlobalHistogramAllocator::CreateWithActiveFileInDir(const FilePath& dir,
                                                    size_t size,
                                                    uint64_t id,
                                                    StringPiece name)
{
    ...
    return CreateWithActiveFile(base_path, active_path, spare_path, size, id, name);
}

// static
FilePath
GlobalHistogramAllocator::ConstructFilePath(const FilePath& dir, StringPiece name)
{
    return dir.AppendASCII(name).AddExtension(PersistentMemoryAllocator::kFileExtension);
}

// static
FilePath
GlobalHistogramAllocator::ConstructFilePathForActiveFile(const FilePath& dir, StringPiece name)
{
...
// static
FilePath
GlobalHistogramAllocator::ConstructFilePathForUploadDir(const FilePath& dir,
                                                        StringPiece name,
                                                        base::Time stamp,
                                                        ProcessId pid)
{
    return ConstructFilePath(dir,
                             StringPrintf("%.*s-%lX-%lX",
                                          static_cast<int>(name.length()),
                                          name.data(),
                                          static_cast<long>(stamp.ToTimeT()),
                                          static_cast<long>(pid)));
}

// static
FilePath
GlobalHistogramAllocator::ConstructFilePathForUploadDir(const FilePath& dir, StringPiece name)
{
    return ConstructFilePathForUploadDir(dir, name, Time::Now(), GetCurrentProcId());
}

// static
bool
GlobalHistogramAllocator::ParseFilePath(const FilePath& path,
                                        std::string* out_name,
                                        Time* out_stamp,
                                        ProcessId* out_pid)
{
    ...
    std::vector<base::StringPiece> parts =
            base::SplitStringPiece(filename, "-.", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
    if (parts.size() != 4)
        return false;
...
bool
GlobalHistogramAllocator::CreateSpareFile(const FilePath& spare_path, size_t size)
{
    FilePath temp_spare_path = spare_path.AddExtension(FILE_PATH_LITERAL(".tmp"));
    bool success;
    {
        File spare_file(temp_spare_path,
        ...
        if (success)
        {
            ...
            success = mmfile.Initialize(
                    std::move(spare_file), {0, size}, MemoryMappedFile::READ_WRITE_EXTEND);
        }
    }
...
// static
void
GlobalHistogramAllocator::CreateWithSharedMemoryRegion(const WritableSharedMemoryRegion& region)
{
    ...
    Set(WrapUnique(
            new GlobalHistogramAllocator(std::make_unique<WritableSharedPersistentMemoryAllocator>(
                    std::move(mapping), 0, StringPiece()))));
}

// static
void
GlobalHistogramAllocator::Set(std::unique_ptr<GlobalHistogramAllocator> allocator)
{
    // Releasing or changing an allocator is extremely dangerous because it
    // likely has histograms stored within it. If the backing memory is also
    // also released, future accesses to those histograms will seg-fault.
    CHECK(!subtle::NoBarrier_Load(&g_histogram_allocator));
    subtle::Release_Store(&g_histogram_allocator, reinterpret_cast<uintptr_t>(allocator.release()));
    size_t existing = StatisticsRecorder::GetHistogramCount();

    DVLOG_IF(1, existing) << existing << " histograms were created before persistence was enabled.";
}
...
void
GlobalHistogramAllocator::SetPersistentLocation(const FilePath& location)
{
    persistent_location_ = location;
}

const FilePath&
GlobalHistogramAllocator::GetPersistentLocation() const
{
    return persistent_location_;
}

bool
GlobalHistogramAllocator::WriteToPersistentLocation()
{
#if defined(OS_NACL)
    ...
    if (!ImportantFileWriter::WriteFileAtomically(persistent_location_, contents))
    {
...
void
GlobalHistogramAllocator::DeletePersistentLocation()
{
    ...
    File file(persistent_location_, File::FLAG_OPEN | File::FLAG_READ | File::FLAG_DELETE_ON_CLOSE);
#endif
}
...
void
GlobalHistogramAllocator::ImportHistogramsToStatisticsRecorder()
{
