...
bool
FieldTrialParamAssociator::AssociateFieldTrialParams(const std::string& trial_name,
                                                     ...
                                                     const FieldTrialParams& params)
{
    ...
    {
        DLOG(ERROR) << "You can't override the existing params for field trial: " << trial_name
                    << "." << group_name;
        return false;
    }
...
bool
FieldTrialParamAssociator::GetFieldTrialParams(FieldTrial* field_trial, FieldTrialParams* params)
{
    if (!field_trial)
        return false;
    // First try the local map, falling back to getting it from shared memory.
    if (GetFieldTrialParamsWithoutFallback(
                field_trial->trial_name(), field_trial->group_name(), params))
    {
        return true;
    }
...
bool
FieldTrialParamAssociator::GetFieldTrialParamsWithoutFallback(const std::string& trial_name,
                                                              const std::string& group_name,
...
void
FieldTrialParamAssociator::ClearAllParamsForTesting()
{
    {
        AutoLock scoped_lock(lock_);
...
void
FieldTrialParamAssociator::ClearParamsForTesting(const std::string& trial_name,
                                                 const std::string& group_name)
{
    AutoLock scoped_lock(lock_);
    const FieldTrialRefKey key(trial_name, group_name);
...
void
FieldTrialParamAssociator::ClearAllCachedParamsForTesting()
{
    AutoLock scoped_lock(lock_);
    field_trial_params_.clear();
}
