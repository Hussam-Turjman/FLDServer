...
public:
    ...
    Next() override;
    void
    Get(HistogramBase::Sample* min, int64_t* max, HistogramBase::Count* count) const override;
...
bool
SampleCountPickleIterator::Done() const
{
    return is_done_;
}

void
SampleCountPickleIterator::Next()
{
    DCHECK(!Done());
    if (!iter_->ReadInt(&min_) || !iter_->ReadInt64(&max_) || !iter_->ReadInt(&count_))
    {
        is_done_ = true;
    }
}
...
void
SampleCountPickleIterator::Get(HistogramBase::Sample* min,
                               int64_t* max,
...
static_assert(sizeof(HistogramSamples::AtomicSingleSample) == sizeof(subtle::Atomic32),
              "AtomicSingleSample isn't 32 bits");

HistogramSamples::SingleSample
HistogramSamples::AtomicSingleSample::Load() const
{
...
HistogramSamples::SingleSample
HistogramSamples::AtomicSingleSample::Extract(bool disable)
{
    AtomicSingleSample single_sample =
            subtle::NoBarrier_AtomicExchange(&as_atomic, disable ? kDisabledSingleSample : 0);
    if (single_sample.as_atomic == kDisabledSingleSample)
        single_sample.as_atomic = 0;
    return single_sample.as_parts;
}
...
bool
HistogramSamples::AtomicSingleSample::Accumulate(size_t bucket, HistogramBase::Count count)
{
    if (count == 0)
        return true;
    ...
    do
    {
        ...
        subtle::Atomic32 existing =
                subtle::Release_CompareAndSwap(&as_atomic, original, single_sample.as_atomic);
...
bool
HistogramSamples::AtomicSingleSample::IsDisabled() const
{
...
void
HistogramSamples::Add(const HistogramSamples& other)
{
...
bool
HistogramSamples::AddFromPickle(PickleIterator* iter)
{
    int64_t sum;
...
void
HistogramSamples::Subtract(const HistogramSamples& other)
{
    IncreaseSumAndCount(-other.sum(), -other.redundant_count());
    std::unique_ptr<SampleCountIterator> it = other.Iterator();
...
void
HistogramSamples::Serialize(Pickle* pickle) const
{
    ...
    for (std::unique_ptr<SampleCountIterator> it = Iterator(); !it->Done(); it->Next())
    {
...
bool
HistogramSamples::AccumulateSingleSample(HistogramBase::Sample value,
                                         HistogramBase::Count count,
                                         size_t bucket)
{
...
void
HistogramSamples::IncreaseSumAndCount(int64_t sum, HistogramBase::Count count)
{
...
void
HistogramSamples::RecordNegativeSample(NegativeSampleReason reason, HistogramBase::Count increment)
{
    UMA_HISTOGRAM_ENUMERATION("UMA.NegativeSamples.Reason", reason, MAX_NEGATIVE_SAMPLE_REASONS);
    UMA_HISTOGRAM_CUSTOM_COUNTS("UMA.NegativeSamples.Increment", increment, 1, 1 << 30, 100);
    UmaHistogramSparse("UMA.NegativeSamples.Histogram", static_cast<int32_t>(id()));
}

base::DictionaryValue
HistogramSamples::ToGraphDict(StringPiece histogram_name, int32_t flags) const
{
...
std::string
HistogramSamples::GetAsciiHeader(StringPiece histogram_name, int32_t flags) const
{
    std::string output;
    StringAppendF(&output,
                  "Histogram: %.*s recorded %d samples",
                  static_cast<int>(histogram_name.size()),
                  histogram_name.data(),
                  TotalCount());
    if (flags)
...
std::string
HistogramSamples::GetAsciiBody() const
{
...
void
HistogramSamples::WriteAsciiBucketGraph(double x_count, int line_length, std::string* output) const
{
...
void
HistogramSamples::WriteAsciiBucketValue(HistogramBase::Count current,
                                        double scaled_sum,
...
const std::string
HistogramSamples::GetSimpleAsciiBucketRange(HistogramBase::Sample sample) const
{
...
bool
SampleCountIterator::GetBucketIndex(size_t* index) const
{
    DCHECK(!Done());
    return false;
}
...
bool
SingleSampleIterator::Done() const
{
    return count_ == 0;
}

void
SingleSampleIterator::Next()
{
    DCHECK(!Done());
    count_ = 0;
}
...
void
SingleSampleIterator::Get(HistogramBase::Sample* min,
                          int64_t* max,
                          HistogramBase::Count* count) const
{
    DCHECK(!Done());
...
bool
SingleSampleIterator::GetBucketIndex(size_t* index) const
{
    DCHECK(!Done());
