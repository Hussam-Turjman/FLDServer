...
PersistentSampleMapIterator::PersistentSampleMapIterator(const SampleToCountMap& sample_counts) :
...
bool
PersistentSampleMapIterator::Done() const
{
    return iter_ == end_;
}

void
PersistentSampleMapIterator::Next()
{
    DCHECK(!Done());
    ++iter_;
    SkipEmptyBuckets();
...
void
PersistentSampleMapIterator::Get(Sample* min, int64_t* max, Count* count) const
{
    DCHECK(!Done());
    if (min)
        *min = iter_->first;
...
void
PersistentSampleMapIterator::SkipEmptyBuckets()
{
...
PersistentSampleMap::PersistentSampleMap(uint64_t id,
...
void
PersistentSampleMap::Accumulate(Sample value, Count count)
{
...
Count
PersistentSampleMap::GetCount(Sample value) const
{
...
Count
PersistentSampleMap::TotalCount() const
{
...
PersistentSampleMap::Iterator() const
{
    // Have to override "const" in order to make sure all samples have been
    // loaded before trying to iterate over the map.
    const_cast<PersistentSampleMap*>(this)->ImportSamples(-1, true);
...
PersistentSampleMap::GetNextPersistentRecord(PersistentMemoryAllocator::Iterator& iterator,
...
// static
PersistentMemoryAllocator::Reference
PersistentSampleMap::CreatePersistentRecord(PersistentMemoryAllocator* allocator,
                                            uint64_t sample_map_id,
                                            Sample value)
{
    ...
    if (!record)
    {
        NOTREACHED() << "full=" << allocator->IsFull() << ", corrupt=" << allocator->IsCorrupt();
        return 0;
    }
...
bool
PersistentSampleMap::AddSubtractImpl(SampleCountIterator* iter, Operator op)
{
    Sample min;
    int64_t max;
    Count count;
...
Count*
PersistentSampleMap::GetSampleCountStorage(Sample value)
{
...
Count*
PersistentSampleMap::GetOrCreateSampleCountStorage(Sample value)
{
...
PersistentSampleMap::GetRecords()
{
...
Count*
PersistentSampleMap::ImportSamples(Sample until_value, bool import_everything)
{
