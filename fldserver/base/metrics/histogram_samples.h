...
// exited this code.
class CORE_EXPORT HistogramSamples
{
public:
    ...
    union CORE_EXPORT AtomicSingleSample
    {
        ...
        bool
        Accumulate(size_t bucket, HistogramBase::Count count);
    ...
    struct CORE_EXPORT LocalMetadata : Metadata
    {
        LocalMetadata();
    };
    ...
    virtual void
    Accumulate(HistogramBase::Sample value, HistogramBase::Count count) = 0;
    virtual HistogramBase::Count
    GetCount(HistogramBase::Sample value) const = 0;
    virtual HistogramBase::Count
    TotalCount() const = 0;

    virtual void
    Add(const HistogramSamples& other);

    // Add from serialized samples.
    virtual bool
    AddFromPickle(PickleIterator* iter);

    virtual void
    Subtract(const HistogramSamples& other);

    virtual std::unique_ptr<SampleCountIterator>
    Iterator() const = 0;
    virtual void
    ...
    ToGraphDict(StringPiece histogram_name, int32_t flags) const;
...
protected:
    ...
    virtual bool
    AddSubtractImpl(SampleCountIterator* iter, Operator op) = 0;
    ...
    // the success case.
    bool
    AccumulateSingleSample(HistogramBase::Sample value, HistogramBase::Count count, size_t bucket);

    // Atomically adjust the sum and redundant-count.
    void
    IncreaseSumAndCount(int64_t sum, HistogramBase::Count count);

    // Record a negative-sample observation and the reason why.
    void
    RecordNegativeSample(NegativeSampleReason reason, HistogramBase::Count increment);
    ...
    void
    WriteAsciiBucketGraph(double x_count, int line_length, std::string* output) const;

    // Writes textual description of the bucket contents (relative to histogram).
    // Output is the count in the buckets, as well as the percentage.
    void
    WriteAsciiBucketValue(HistogramBase::Count current,
                          double scaled_sum,
    ...
    virtual std::string
    GetAsciiHeader(StringPiece histogram_name, int32_t flags) const;

    // Returns a string description of what goes in a given bucket.
    const std::string
    GetSimpleAsciiBucketRange(HistogramBase::Sample sample) const;
...
class CORE_EXPORT SampleCountIterator
{
public:
    ...
    // Requires: !Done();
    virtual void
    Get(HistogramBase::Sample* min, int64_t* max, HistogramBase::Count* count) const = 0;
    ...
    // Requires: !Done();
    virtual bool
    GetBucketIndex(size_t* index) const;
};

class CORE_EXPORT SingleSampleIterator : public SampleCountIterator
{
public:
    SingleSampleIterator(HistogramBase::Sample min, int64_t max, HistogramBase::Count count);
    ...
    bool
    GetBucketIndex(size_t* index) const override;
