...
#include "fldserver/base/strings/escape.h"
...
{
    return UnescapeURLComponent(value,
                                UnescapeRule::PATH_SEPARATORS
                                        | UnescapeRule::URL_SPECIAL_CHARS_EXCEPT_PATH_SEPARATORS);
}

bool
AssociateFieldTrialParams(const std::string& trial_name,
...
bool
AssociateFieldTrialParamsFromString(const std::string& params_string,
                                    FieldTrialParamsDecodeStringFunc decode_data_func)
{
    ...
    {
        std::vector<StringPiece> experiment =
                SplitStringPiece(experiment_group, ":", TRIM_WHITESPACE, SPLIT_WANT_ALL);
        ...
        {
            DLOG(ERROR) << StringPrintf("A (trial, group) pair listed more than once. (%s, %s)",
                                        trial.c_str(),
                                        group.c_str());
            return false;
        }
        ...
        if (!result)
        {
            DLOG(ERROR) << "Failed to associate field trial params for group \"" << group
                        << "\" in trial \"" << trial << "\"";
            return false;
        }
    }
...
bool
GetFieldTrialParams(const std::string& trial_name, FieldTrialParams* params)
{
    FieldTrial* trial = FieldTrialList::Find(trial_name);
    return FieldTrialParamAssociator::GetInstance()->GetFieldTrialParams(trial, params);
}

bool
GetFieldTrialParamsByFeature(const Feature& feature, FieldTrialParams* params)
{
...
std::string
GetFieldTrialParamValue(const std::string& trial_name, const std::string& param_name)
{
...
std::string
GetFieldTrialParamValueByFeature(const Feature& feature, const std::string& param_name)
{
    FieldTrialParams params;
...
int
GetFieldTrialParamByFeatureAsInt(const Feature& feature,
                                 const std::string& param_name,
                                 int default_value)
{
    std::string value_as_string = GetFieldTrialParamValueByFeature(feature, param_name);
    int value_as_int = 0;
    if (!StringToInt(value_as_string, &value_as_int))
    {
        if (!value_as_string.empty())
        {
            DLOG(WARNING) << "Failed to parse field trial param " << param_name
                          ...
                          << feature.name << " into an int. Falling back to default value of "
                          << default_value;
        }
...
double
GetFieldTrialParamByFeatureAsDouble(const Feature& feature,
                                    const std::string& param_name,
                                    double default_value)
{
    ...
    if (!StringToDouble(value_as_string, &value_as_double))
    {
        if (!value_as_string.empty())
        {
            DLOG(WARNING) << "Failed to parse field trial param " << param_name
                          << " with string value " << value_as_string << " under feature "
                          << feature.name << " into a double. Falling back to default value of "
                          << default_value;
        }
...
bool
GetFieldTrialParamByFeatureAsBool(const Feature& feature,
                                  const std::string& param_name,
                                  bool default_value)
{
    std::string value_as_string = GetFieldTrialParamValueByFeature(feature, param_name);
    ...
    if (!value_as_string.empty())
    {
        DLOG(WARNING) << "Failed to parse field trial param " << param_name << " with string value "
                      << value_as_string << " under feature " << feature.name
                      << " into a bool. Falling back to default value of " << default_value;
    }
    return default_value;
}

base::TimeDelta
GetFieldTrialParamByFeatureAsTimeDelta(const Feature& feature,
                                       const std::string& param_name,
                                       base::TimeDelta default_value)
{
    ...
    base::Optional<base::TimeDelta> ret = base::TimeDelta::FromString(value_as_string);
    if (!ret.has_value())
    {
        DLOG(WARNING) << "Failed to parse field trial param " << param_name << " with string value "
                      << value_as_string << " under feature " << feature.name
...
std::string
FeatureParam<std::string>::Get() const
{
                    ...
                    int default_value_as_int)
{
    DLOG(WARNING) << "Failed to parse field trial param " << param_name << " with string value "
                  << value_as_string << " under feature " << feature.name
                  << " into an enum. Falling back to default value of " << default_value_as_int;
}
