...
bool
CheckFlag(const volatile std::atomic<uint32_t>* flags, int flag)
{
...
void
SetFlag(volatile std::atomic<uint32_t>* flags, int flag)
{
    uint32_t loaded_flags = flags->load(std::memory_order_relaxed);
    for (;;)
    {
        ...
        if (flags->compare_exchange_weak(
                    loaded_flags, new_flags, std::memory_order_relaxed, std::memory_order_relaxed))
        {
            break;
        }
    }
...
const PersistentMemoryAllocator::Reference PersistentMemoryAllocator::kReferenceQueue =
        offsetof(SharedMetadata, queue);

const base::FilePath::CharType PersistentMemoryAllocator::kFileExtension[] = FILE_PATH_LITERAL(".pm"
                                                                                               "a");

PersistentMemoryAllocator::Iterator::Iterator(const PersistentMemoryAllocator* allocator) :
...
void
PersistentMemoryAllocator::Iterator::Reset()
{
...
void
PersistentMemoryAllocator::Iterator::Reset(Reference starting_after)
{
    if (starting_after == 0)
    {
        Reset();
...
PersistentMemoryAllocator::Iterator::GetNext(uint32_t* type_return)
{
    ...
    const uint32_t freeptr =
            std::min(allocator_->shared_meta()->freeptr.load(std::memory_order_relaxed),
...
// static
bool
PersistentMemoryAllocator::IsMemoryAcceptable(const void* base,
                                              size_t size,
                                              size_t page_size,
                                              bool readonly)
{
...
PersistentMemoryAllocator::PersistentMemoryAllocator(void* base,
                                                     ...
                                                     bool readonly) :
    PersistentMemoryAllocator(Memory(base, MEM_EXTERNAL), size, page_size, id, name, readonly)
{
}
...
//                  "\"queue\" is not aligned properly; must be at end of struct");
    ...
    if (shared_meta()->cookie != kGlobalCookie)
    {
        ...
        volatile BlockHeader* const first_block =
                reinterpret_cast<volatile BlockHeader*>(mem_base_ + sizeof(SharedMetadata));
        if (shared_meta()->cookie != 0 || shared_meta()->size != 0 || shared_meta()->version != 0
            || shared_meta()->freeptr.load(std::memory_order_relaxed) != 0
            || shared_meta()->flags.load(std::memory_order_relaxed) != 0 || shared_meta()->id != 0
            || shared_meta()->name != 0 || shared_meta()->tailptr != 0
            || shared_meta()->queue.cookie != 0
            || shared_meta()->queue.next.load(std::memory_order_relaxed) != 0
            || first_block->size != 0 || first_block->cookie != 0
            || first_block->type_id.load(std::memory_order_relaxed) != 0 || first_block->next != 0)
        {
        ...
        shared_meta()->id = id;
        shared_meta()->freeptr.store(sizeof(SharedMetadata), std::memory_order_release);
        ...
        shared_meta()->memory_state.store(MEMORY_INITIALIZED, std::memory_order_release);
    }
    else
    {
...
uint64_t
PersistentMemoryAllocator::Id() const
{
    return shared_meta()->id;
}

const char*
PersistentMemoryAllocator::Name() const
{
    Reference name_ref = shared_meta()->name;
    const char* name_cstr = GetAsArray<char>(name_ref, 0, PersistentMemoryAllocator::kSizeAny);
    if (!name_cstr)
        return "";
...
void
PersistentMemoryAllocator::CreateTrackingHistograms(base::StringPiece name)
{
  ...
  allocs_histogram_ = Histogram::FactoryGet(
    ...
    used_histogram_ =
            LinearHistogram::FactoryGet("UMA.PersistentAllocator." + name_string + ".UsedPct",
                                        1,
                                        101,
                                        21,
                                        HistogramBase::kUmaTargetedHistogramFlag);
    ...
    errors_histogram_ =
            SparseHistogram::FactoryGet("UMA.PersistentAllocator." + name_string + ".Errors",
                                        HistogramBase::kUmaTargetedHistogramFlag);
}

void
PersistentMemoryAllocator::Flush(bool sync)
{
    FlushPartial(used(), sync);
}

void
PersistentMemoryAllocator::SetMemoryState(uint8_t memory_state)
{
...
uint8_t
PersistentMemoryAllocator::GetMemoryState() const
{
    return shared_meta()->memory_state.load(std::memory_order_relaxed);
}

size_t
PersistentMemoryAllocator::used() const
{
    return std::min(shared_meta()->freeptr.load(std::memory_order_relaxed), mem_size_);
}

PersistentMemoryAllocator::Reference
PersistentMemoryAllocator::GetAsReference(const void* memory, uint32_t type_id) const
{
...
size_t
PersistentMemoryAllocator::GetAllocSize(Reference ref) const
{
...
uint32_t
PersistentMemoryAllocator::GetType(Reference ref) const
{
    const volatile BlockHeader* const block = GetBlock(ref, 0, 0, false, false);
    if (!block)
        return 0;
...
bool
PersistentMemoryAllocator::ChangeType(Reference ref,
                                      uint32_t to_type_id,
                                      ...
                                      bool clear)
{
    ...
    if (clear)
    {
        ...
        if (!block->type_id.compare_exchange_strong(from_type_id,
                                                    kTypeIdTransitioning,
        ...
        volatile std::atomic<int>* data = reinterpret_cast<volatile std::atomic<int>*>(
        ...
        bool success = block->type_id.compare_exchange_strong(
                from_type_id, to_type_id, std::memory_order_release, std::memory_order_relaxed);
    ...
    return block->type_id.compare_exchange_strong(
            from_type_id, to_type_id, std::memory_order_acq_rel, std::memory_order_acquire);
}

PersistentMemoryAllocator::Reference
PersistentMemoryAllocator::Allocate(size_t req_size, uint32_t type_id)
{
...
PersistentMemoryAllocator::Reference
PersistentMemoryAllocator::AllocateImpl(size_t req_size, uint32_t type_id)
{
    DCHECK(!readonly_);
    ...
    for (;;)
    {
        ...
        if (size > page_free)
        {
            ...
            if (shared_meta()->freeptr.compare_exchange_strong(
                        freeptr, new_freeptr, std::memory_order_acq_rel, std::memory_order_acquire))
            {
        ...
        volatile char* mem_begin = reinterpret_cast<volatile char*>(
                (reinterpret_cast<uintptr_t>(block) + sizeof(BlockHeader) + (vm_page_size_ - 1))
                & ~static_cast<uintptr_t>(vm_page_size_ - 1));
        for (volatile char* memory = mem_begin; memory < mem_end; memory += vm_page_size_)
        {
...
void
PersistentMemoryAllocator::GetMemoryInfo(MemoryInfo* meminfo) const
{
    uint32_t remaining =
            std::max(mem_size_ - shared_meta()->freeptr.load(std::memory_order_relaxed),
...
void
PersistentMemoryAllocator::MakeIterable(Reference ref)
{
    DCHECK(!readonly_);
    if (IsCorrupt())
        return;
...
void
PersistentMemoryAllocator::SetCorrupt() const
{
    if (!corrupt_.load(std::memory_order_relaxed)
        && !CheckFlag(const_cast<volatile std::atomic<uint32_t>*>(&shared_meta()->flags),
                      kFlagCorrupt))
    {
    ...
    if (!readonly_)
    {
        SetFlag(const_cast<volatile std::atomic<uint32_t>*>(&shared_meta()->flags), kFlagCorrupt);
    }
}

bool
PersistentMemoryAllocator::IsCorrupt() const
{
...
bool
PersistentMemoryAllocator::IsFull() const
{
...
PersistentMemoryAllocator::GetBlock(
        Reference ref, uint32_t type_id, uint32_t size, bool queue_ok, bool free_ok) const
{
...
void
PersistentMemoryAllocator::FlushPartial(size_t length, bool sync)
{
...
void
PersistentMemoryAllocator::RecordError(int error) const
{
...
const volatile void*
PersistentMemoryAllocator::GetBlockData(Reference ref, uint32_t type_id, uint32_t size) const
{
    DCHECK(size > 0);
...
void
PersistentMemoryAllocator::UpdateTrackingHistograms()
{
    DCHECK(!readonly_);
    if (used_histogram_)
    {
...
LocalPersistentMemoryAllocator::LocalPersistentMemoryAllocator(size_t size,
                                                               uint64_t id,
                                                               base::StringPiece name) :
    PersistentMemoryAllocator(AllocateLocalMemory(size), size, 0, id, name, false)
{
}
...
LocalPersistentMemoryAllocator::AllocateLocalMemory(size_t size)
{
...
#if IS_WIN
    address = ::VirtualAlloc(nullptr, size, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
    if (address)
        return Memory(address, MEM_VIRTUAL);
    UmaHistogramSparse("UMA.LocalPersistentMemoryAllocator.Failures.Win", ::GetLastError());
#elif IS_POSIX || defined(OS_FUCHSIA)
    ...
    address = ::mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_ANON | MAP_SHARED, -1, 0);
    if (address != MAP_FAILED)
        return Memory(address, MEM_VIRTUAL);
    UmaHistogramSparse("UMA.LocalPersistentMemoryAllocator.Failures.Posix", errno);
#else
...
// static
void
LocalPersistentMemoryAllocator::DeallocateLocalMemory(void* memory, size_t size, MemoryType type)
{
    if (type == MEM_MALLOC)
    {
        free(memory);
...
WritableSharedPersistentMemoryAllocator::WritableSharedPersistentMemoryAllocator(
        base::WritableSharedMemoryMapping memory, uint64_t id, base::StringPiece name) :
    PersistentMemoryAllocator(
            Memory(memory.memory(), MEM_SHARED), memory.size(), 0, id, name, false),
    shared_memory_(std::move(memory))
{
}

WritableSharedPersistentMemoryAllocator::~WritableSharedPersistentMemoryAllocator() = default;
...
ReadOnlySharedPersistentMemoryAllocator::ReadOnlySharedPersistentMemoryAllocator(
        base::ReadOnlySharedMemoryMapping memory, uint64_t id, base::StringPiece name) :
    PersistentMemoryAllocator(Memory(const_cast<void*>(memory.memory()), MEM_SHARED),
                              memory.size(),
                              0,
                              id,
                              name,
...
ReadOnlySharedPersistentMemoryAllocator::~ReadOnlySharedPersistentMemoryAllocator() = default;
...
FilePersistentMemoryAllocator::FilePersistentMemoryAllocator(std::unique_ptr<MemoryMappedFile> file,
                                                             size_t max_size,
                                                             uint64_t id,
                                                             base::StringPiece name,
                                                             bool read_only) :
    PersistentMemoryAllocator(Memory(const_cast<uint8_t*>(file->data()), MEM_FILE),
...
// static
bool
FilePersistentMemoryAllocator::IsFileAcceptable(const MemoryMappedFile& file, bool read_only)
{
    return IsMemoryAcceptable(file.data(), file.length(), 0, read_only);
}

void
FilePersistentMemoryAllocator::Cache()
{
    ...
    // can't omit the read.
    int total = 0;
    for (const volatile char* memory = mem_begin; memory < mem_end; memory += vm_page_size_)
    {
        total += *memory;
    }
...
void
FilePersistentMemoryAllocator::FlushPartial(size_t length, bool sync)
{
    if (IsReadonly())
        return;
    ...
    int result = ::msync(const_cast<void*>(data()), length, sync ? MS_SYNC : MS_ASYNC);
    ...
    int result =
            ::msync(const_cast<void*>(data()), length, MS_INVALIDATE | (sync ? MS_SYNC : MS_ASYNC));
    DCHECK_NE(EINVAL, result);
#else
...
DelayedPersistentAllocation::DelayedPersistentAllocation(PersistentMemoryAllocator* allocator,
                                                         subtle::Atomic32* ref,
                                                         uint32_t type,
                                                         size_t size,
                                                         bool make_iterable) :
    DelayedPersistentAllocation(
            allocator, reinterpret_cast<std::atomic<Reference>*>(ref), type, size, 0, make_iterable)
{
}
...
DelayedPersistentAllocation::DelayedPersistentAllocation(PersistentMemoryAllocator* allocator,
                                                         ...
                                                         size_t size,
                                                         bool make_iterable) :
    DelayedPersistentAllocation(allocator, ref, type, size, 0, make_iterable)
{
}
...
void*
DelayedPersistentAllocation::Get() const
{
    ...
    if (!ref)
    {
        ...
        if (reference_->compare_exchange_strong(
