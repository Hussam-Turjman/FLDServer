...
SampleVectorBase::SampleVectorBase(uint64_t id, Metadata* meta, const BucketRanges* bucket_ranges) :
...
void
SampleVectorBase::Accumulate(Sample value, Count count)
{
    ...
    Count new_value = subtle::NoBarrier_AtomicIncrement(&counts()[bucket_index], count);
...
Count
SampleVectorBase::GetCount(Sample value) const
{
    return GetCountAtIndex(GetBucketIndex(value));
}

Count
SampleVectorBase::TotalCount() const
{
    // Handle the single-sample case.
...
Count
SampleVectorBase::GetCountAtIndex(size_t bucket_index) const
{
...
SampleVectorBase::Iterator() const
{
    // Handle the single-sample case.
    SingleSample sample = single_sample().Load();
    if (sample.count != 0)
    {
        return std::make_unique<SingleSampleIterator>(bucket_ranges_->range(sample.bucket),
                                                      bucket_ranges_->range(sample.bucket + 1),
                                                      sample.count,
                                                      sample.bucket);
    }
    ...
    if (counts() || MountExistingCountsStorage())
    {
        return std::make_unique<SampleVectorIterator>(counts(), counts_size(), bucket_ranges_);
    }
...
bool
SampleVectorBase::AddSubtractImpl(SampleCountIterator* iter, HistogramSamples::Operator op)
{
    ...
    if (!counts())
    {
        if (iter->Done())
        {
            ...
            if (single_sample().Accumulate(dest_index,
    ...
    while (true)
    {
        ...
        subtle::NoBarrier_AtomicIncrement(&counts()[dest_index],
                                          op == HistogramSamples::ADD ? count : -count);
...
size_t
SampleVectorBase::GetBucketIndex(Sample value) const
{
...
void
SampleVectorBase::MoveSingleSampleToCounts()
{
    DCHECK(counts());
...
void
SampleVectorBase::MountCountsStorageAndMoveSingleSample()
{
...
SampleVector::SampleVector(const BucketRanges* bucket_ranges) : SampleVector(0, bucket_ranges)
{
}
...
bool
SampleVector::MountExistingCountsStorage() const
{
...
std::string
SampleVector::GetAsciiHeader(StringPiece histogram_name, int32_t flags) const
{
...
std::string
SampleVector::GetAsciiBody() const
{
    Count sample_count = TotalCount();
    ...
    for (uint32_t i = 0; i < bucket_count(); ++i)
    {
        ...
        {
            size_t width = GetSimpleAsciiBucketRange(bucket_ranges()->range(i)).size() + 1;
...
double
SampleVector::GetPeakBucketSize() const
{
    Count max = 0;
    for (uint32_t i = 0; i < bucket_count(); ++i)
    {
...
void
SampleVector::WriteAsciiBucketContext(int64_t past,
                                      Count current,
                                      int64_t remaining,
...
PersistentSampleVector::PersistentSampleVector(uint64_t id,
...
bool
PersistentSampleVector::MountExistingCountsStorage() const
{
    ...
    set_counts(static_cast<HistogramBase::AtomicCount*>(persistent_counts_.Get()));
...
SampleVectorIterator::SampleVectorIterator(const std::vector<HistogramBase::AtomicCount>* counts,
                                           const BucketRanges* bucket_ranges) :
    counts_(&(*counts)[0]), counts_size_(counts->size()), bucket_ranges_(bucket_ranges), index_(0)
{
    DCHECK_GE(bucket_ranges_->bucket_count(), counts_size_);
    SkipEmptyBuckets();
}

SampleVectorIterator::SampleVectorIterator(const HistogramBase::AtomicCount* counts,
                                           size_t counts_size,
                                           const BucketRanges* bucket_ranges) :
    counts_(counts), counts_size_(counts_size), bucket_ranges_(bucket_ranges), index_(0)
{
    DCHECK_GE(bucket_ranges_->bucket_count(), counts_size_);
    SkipEmptyBuckets();
}
...
bool
SampleVectorIterator::Done() const
{
    return index_ >= counts_size_;
}

void
SampleVectorIterator::Next()
{
    DCHECK(!Done());
    index_++;
    SkipEmptyBuckets();
...
void
SampleVectorIterator::Get(HistogramBase::Sample* min,
                          int64_t* max,
...
bool
SampleVectorIterator::GetBucketIndex(size_t* index) const
{
    DCHECK(!Done());
    if (index != nullptr)
        *index = index_;
...
void
SampleVectorIterator::SkipEmptyBuckets()
{
    if (Done())
        return;
