...
#include "fldserver/base/crypto/encryptor.h"
...
#include "fldserver/base/sys_byteorder.h"
#include "fldserver/base/crypto/openssl_util.h"
#include "fldserver/base/crypto/symmetric_key.h"
...
{
const EVP_CIPHER*
GetCipherForKey(const SymmetricKey* key)
{
...
bool
Encryptor::Init(const SymmetricKey* key, Mode mode, base::StringPiece iv)
{
    return Init(key, mode, base::as_bytes(base::make_span(iv)));
}

bool
Encryptor::Init(const SymmetricKey* key, Mode mode, base::span<const uint8_t> iv)
{
    DCHECK(key);
...
bool
Encryptor::Encrypt(base::StringPiece plaintext, std::string* ciphertext)
{
...
bool
Encryptor::Encrypt(base::span<const uint8_t> plaintext, std::vector<uint8_t>* ciphertext)
{
    CHECK(!plaintext.empty() || mode_ == CBC);
...
bool
Encryptor::Decrypt(base::StringPiece ciphertext, std::string* plaintext)
{
...
bool
Encryptor::Decrypt(base::span<const uint8_t> ciphertext, std::vector<uint8_t>* plaintext)
{
    CHECK(!ciphertext.empty());
...
bool
Encryptor::SetCounter(base::StringPiece counter)
{
    return SetCounter(base::as_bytes(base::make_span(counter)));
}

bool
Encryptor::SetCounter(base::span<const uint8_t> counter)
{
    if (mode_ != CTR)
        return false;
    if (counter.size() != 16u)
...
bool
Encryptor::CryptString(bool do_encrypt, base::StringPiece input, std::string* output)
{
    size_t out_size = MaxOutput(do_encrypt, input.size());
    CHECK_GT(out_size + 1, out_size);  // Overflow
    std::string result;
    uint8_t* out_ptr = reinterpret_cast<uint8_t*>(base::WriteInto(&result, out_size + 1));

    base::Optional<size_t> len = (mode_ == CTR) ? CryptCTR(do_encrypt,
                                                           base::as_bytes(base::make_span(input)),
...
bool
Encryptor::CryptBytes(bool do_encrypt,
                      base::span<const uint8_t> input,
                      std::vector<uint8_t>* output)
{
    std::vector<uint8_t> result(MaxOutput(do_encrypt, input.size()));
    base::Optional<size_t> len = (mode_ == CTR) ? CryptCTR(do_encrypt, input, result)
...
size_t
Encryptor::MaxOutput(bool do_encrypt, size_t length)
{
...
Encryptor::Crypt(bool do_encrypt, base::span<const uint8_t> input, base::span<uint8_t> output)
{
    ...
    int out_len;
    if (!EVP_CipherUpdate(ctx.get(), output.data(), &out_len, input.data(), input.size()))
        return base::nullopt;
...
base::Optional<size_t>
Encryptor::CryptCTR(bool do_encrypt, base::span<const uint8_t> input, base::span<uint8_t> output)
{
