...
// static
std::unique_ptr<ECPrivateKey>
ECPrivateKey::CreateFromPrivateKeyInfo(base::span<const uint8_t> input)
{
    OpenSSLErrStackTracer err_tracer(FROM_HERE);
        ...
        base::span<const uint8_t> encrypted_private_key_info)
{
    ...
    CBS cbs;
    CBS_init(&cbs, encrypted_private_key_info.data(), encrypted_private_key_info.size());
    bssl::UniquePtr<EVP_PKEY> pkey(PKCS8_parse_encrypted_private_key(&cbs, "", 0));
...
// static
std::unique_ptr<ECPrivateKey>
ECPrivateKey::DeriveFromSecret(base::span<const uint8_t> secret)
{
...
bool
ECPrivateKey::ExportPrivateKey(std::vector<uint8_t>* output) const
{
    OpenSSLErrStackTracer err_tracer(FROM_HERE);
    uint8_t* der;
    size_t der_len;
...
bool
ECPrivateKey::ExportEncryptedPrivateKey(std::vector<uint8_t>* output) const
{
    OpenSSLErrStackTracer err_tracer(FROM_HERE);
    ...
    if (!CBB_init(cbb.get(), 0)
        || !PKCS8_marshal_encrypted_private_key(cbb.get(),
...
bool
ECPrivateKey::ExportPublicKey(std::vector<uint8_t>* output) const
{
    OpenSSLErrStackTracer err_tracer(FROM_HERE);
    uint8_t* der;
    size_t der_len;
...
bool
ECPrivateKey::ExportRawPublicKey(std::string* output) const
{
    OpenSSLErrStackTracer err_tracer(FROM_HERE);
    ...
    if (!x || !y
        || !EC_POINT_get_affine_coordinates_GFp(EC_KEY_get0_group(ec_key),
                                                ...
                                                y.get(),
                                                nullptr)
        || !BN_bn2bin_padded(buf, 32, x.get()) || !BN_bn2bin_padded(buf + 32, 32, y.get()))
    {
        return false;
    }
