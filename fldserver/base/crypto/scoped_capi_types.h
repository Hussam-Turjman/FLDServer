...
#include "fldserver/base/win/wincrypt_shim.h"
...
{
    void
    operator()(CAPIHandle handle) const
    {
        if (handle)
        {
...
template <typename CAPIHandle, BOOL(WINAPI* Destroyer)(CAPIHandle, DWORD), DWORD flags>
...
inline bool
operator==(CH h, const ScopedCAPIHandle<CH, FP>& b)
{
    return h == b.get();
}
...
typedef ScopedCAPIHandle<HCRYPTPROV, CAPIDestroyerWithFlags<HCRYPTPROV, CryptReleaseContext, 0>>
        ScopedHCRYPTPROV;

typedef ScopedCAPIHandle<HCRYPTKEY, CAPIDestroyer<HCRYPTKEY, CryptDestroyKey>> ScopedHCRYPTKEY;

typedef ScopedCAPIHandle<HCRYPTHASH, CAPIDestroyer<HCRYPTHASH, CryptDestroyHash>> ScopedHCRYPTHASH;
