...
#include "fldserver/base/crypto/nss_util.h"
...
#include "fldserver/base/callback_list.h"
...
#include "fldserver/base/crypto/chaps_support.h"
#include "fldserver/base/crypto/nss_util_internal.h"
...
public:
    ...
    explicit ChromeOSUserData(ScopedPK11Slot public_slot) : public_slot_(std::move(public_slot))
    {
    }
    ...
    GetPublicSlot()
    {
        return ScopedPK11Slot(public_slot_ ? PK11_ReferenceSlot(public_slot_.get()) : nullptr);
    }

    ScopedPK11Slot
    GetPrivateSlot(SlotReadyCallback callback)
    {
        if (private_slot_)
    ...
    void
    SetPrivateSlot(ScopedPK11Slot private_slot)
    {
    ...
    bool
    private_slot_initialization_started() const
    {
...
public:
    ...
    {
        explicit TPMModuleAndSlot(SECMODModule* init_chaps_module) : chaps_module(init_chaps_module)
        {
        }
    ...
    ScopedPK11Slot
    OpenPersistentNSSDBForPath(const std::string& db_name, const base::FilePath& path)
    {
    ...
    void
    InitializeTPMTokenAndSystemSlot(int system_slot_id, base::OnceCallback<void(bool)> callback)
    {
        DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
        ...
        // released.
        std::unique_ptr<TPMModuleAndSlot> tpm_args(new TPMModuleAndSlot(chaps_module_));
        ...
        base::ThreadPool::PostTaskAndReply(
                ...
                base::BindOnce(&ChromeOSTokenManager::InitializeTPMTokenInThreadPool,
                               system_slot_id,
                               tpm_args_ptr),
                base::BindOnce(&ChromeOSTokenManager::OnInitializedTPMTokenAndSystemSlot,
    ...
    static void
    InitializeTPMTokenInThreadPool(CK_SLOT_ID token_slot_id, TPMModuleAndSlot* tpm_args)
    {
        ...
        if (tpm_args->chaps_module)
        {
            tpm_args->tpm_slot = GetTPMSlotForIdInThreadPool(tpm_args->chaps_module, token_slot_id);
        }
    }

    void
    OnInitializedTPMTokenAndSystemSlot(base::OnceCallback<void(bool)> callback,
    ...
    bool
    IsTPMTokenReady(base::OnceClosure callback)
    {
        DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
        if (tpm_slot_)
            return true;
    ...
    // we also control.
    static crypto::ScopedPK11Slot
    GetTPMSlotForIdInThreadPool(SECMODModule* chaps_module, CK_SLOT_ID slot_id)
    {
        DCHECK(chaps_module);
    ...
    InitializeNSSForChromeOSUser(const std::string& username_hash, const base::FilePath& path)
    {
        ...
        std::string db_name =
                base::StringPrintf("%s %s", kUserNSSDatabaseName, username_hash.c_str());
    ...
    bool
    ShouldInitializeTPMForChromeOSUser(const std::string& username_hash)
    {
        DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
        DCHECK(chromeos_user_map_.find(username_hash) != chromeos_user_map_.end());

        return !chromeos_user_map_[username_hash]->private_slot_initialization_started();
    }
    ...
    WillInitializeTPMForChromeOSUser(const std::string& username_hash)
    {
        ...
        chromeos_user_map_[username_hash]->set_private_slot_initialization_started();
    }
    ...
    InitializeTPMForChromeOSUser(const std::string& username_hash, CK_SLOT_ID slot_id)
    {
        DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
        DCHECK(chromeos_user_map_.find(username_hash) != chromeos_user_map_.end());
        DCHECK(chromeos_user_map_[username_hash]->private_slot_initialization_started());
    ...
    void
    OnInitializedTPMForChromeOSUser(const std::string& username_hash,
                                    std::unique_ptr<TPMModuleAndSlot> tpm_args)
    {
        DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
        DVLOG(2) << "Got tpm slot for " << username_hash << " " << !!tpm_args->tpm_slot;
        chromeos_user_map_[username_hash]->SetPrivateSlot(std::move(tpm_args->tpm_slot));
    }
                                  ...
                                  base::OnceCallback<void(ScopedPK11Slot)> callback)
    {
        ...
        return chromeos_user_map_[username_hash]->GetPrivateSlot(std::move(callback));
    }
    ...
    void
    GetSystemNSSKeySlotCallback(base::OnceCallback<void(ScopedPK11Slot)> callback)
    {
        std::move(callback).Run(ScopedPK11Slot(PK11_ReferenceSlot(tpm_slot_.get())));
    }

    ScopedPK11Slot
    GetSystemNSSKeySlot(base::OnceCallback<void(ScopedPK11Slot)> callback)
    {
        DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
        ...
        if (!callback.is_null())
        {
            wrapped_callback = base::BindOnce(&ChromeOSTokenManager::GetSystemNSSKeySlotCallback,
...
base::LazyInstance<ChromeOSTokenManager>::Leaky g_token_manager = LAZY_INSTANCE_INITIALIZER;
}  // namespace
...
SetSystemKeySlotWithoutInitializingTPMForTesting(ScopedPK11Slot slot)
{
    g_token_manager.Get().SetSystemKeySlotWithoutInitializingTPMForTesting(std::move(slot));
}
...
void
InitializeTPMTokenAndSystemSlot(int token_slot_id, base::OnceCallback<void(bool)> callback)
{
    g_token_manager.Get().InitializeTPMTokenAndSystemSlot(token_slot_id, std::move(callback));
}

bool
InitializeNSSForChromeOSUser(const std::string& username_hash, const base::FilePath& path)
{
    return g_token_manager.Get().InitializeNSSForChromeOSUser(username_hash, path);
}

bool
ShouldInitializeTPMForChromeOSUser(const std::string& username_hash)
{
    return g_token_manager.Get().ShouldInitializeTPMForChromeOSUser(username_hash);
}
...
InitializePrivateSoftwareSlotForChromeOSUser(const std::string& username_hash)
{
    g_token_manager.Get().InitializePrivateSoftwareSlotForChromeOSUser(username_hash);
}
...
GetPrivateSlotForChromeOSUser(const std::string& username_hash,
                              base::OnceCallback<void(ScopedPK11Slot)> callback)
{
    return g_token_manager.Get().GetPrivateSlotForChromeOSUser(username_hash, std::move(callback));
}
...
SetPrivateSoftwareSlotForChromeOSUserForTesting(ScopedPK11Slot slot)
{
    g_token_manager.Get().SetPrivateSoftwareSlotForChromeOSUserForTesting(std::move(slot));
}
