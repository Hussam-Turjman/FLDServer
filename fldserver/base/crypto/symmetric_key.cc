...
{
bool
CheckDerivationParameters(SymmetricKey::Algorithm algorithm, size_t key_size_in_bits)
{
    switch (algorithm)
    {
        ...
        case SymmetricKey::HMAC_SHA1: return key_size_in_bits % 8 == 0 && key_size_in_bits != 0;
    }
...
SymmetricKey::GenerateRandomKey(Algorithm algorithm, size_t key_size_in_bits)
{
    ...
    uint8_t* key_data =
            reinterpret_cast<uint8_t*>(base::WriteInto(&key->key_, key_size_in_bytes + 1));
...
// static
std::unique_ptr<SymmetricKey>
SymmetricKey::DeriveKeyFromPasswordUsingPbkdf2(Algorithm algorithm,
                                               ...
                                               size_t key_size_in_bits)
{
    ...
    int rv = PKCS5_PBKDF2_HMAC_SHA1(password.data(),
                                    password.length(),
...
// static
std::unique_ptr<SymmetricKey>
SymmetricKey::DeriveKeyFromPasswordUsingScrypt(Algorithm algorithm,
                                               const std::string& password,
                                               const std::string& salt,
                                               ...
                                               size_t key_size_in_bits)
{
    ...
    int rv = EVP_PBE_scrypt(password.data(),
                            password.length(),
                            reinterpret_cast<const uint8_t*>(salt.data()),
                            salt.length(),
                            cost_parameter,
...
// static
std::unique_ptr<SymmetricKey>
SymmetricKey::Import(Algorithm algorithm, const std::string& raw_key)
{
    if (algorithm == AES)
    {
        // Whitelist supported key sizes to avoid accidentaly relying on
