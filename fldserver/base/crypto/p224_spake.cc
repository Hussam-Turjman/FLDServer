...
#include "fldserver/base/crypto/p224_spake.h"
...
#include "fldserver/base/crypto/random.h"
...
const uint8_t kM_X962[1 + 28 + 28] = {
        0x04, 0x4d, 0x48, 0xc8, 0xea, 0x8d, 0x23, 0x39, 0x2e, 0x07, 0xe8, 0x51, 0xfa, 0x6a, 0xa8,
        0x20, 0x48, 0x09, 0x4e, 0x05, 0x13, 0x72, 0x49, 0x9c, 0x6f, 0xba, 0x62, 0xa7, 0x4b, 0x6c,
        0x18, 0x5c, 0xab, 0xd5, 0x2e, 0x2e, 0x8a, 0x9e, 0x2d, 0x21, 0xb0, 0xec, 0x4e, 0xe1, 0x41,
        0x21, 0x1f, 0xe2, 0x9d, 0x64, 0xea, 0x4d, 0x04, 0x46, 0x3a, 0xe8, 0x33,
};

const uint8_t kN_X962[1 + 28 + 28] = {
        0x04, 0x0b, 0x1c, 0xfc, 0x6a, 0x40, 0x7c, 0xdc, 0xb1, 0x5d, 0xc1, 0x70, 0x4c, 0xd1, 0x3e,
        0xda, 0xab, 0x8f, 0xde, 0xff, 0x8c, 0xfb, 0xfb, 0x50, 0xd2, 0xc8, 0x1d, 0xe2, 0xc2, 0x3e,
        0x14, 0xf6, 0x29, 0x96, 0x08, 0x09, 0x07, 0xb5, 0x6d, 0xd2, 0x82, 0x07, 0x1a, 0xa7, 0xa1,
        0x21, 0xc3, 0x99, 0x34, 0xbc, 0x30, 0xda, 0x5b, 0xcb, 0xc6, 0xa3, 0xcc,
};

// ToBignum returns |big_endian_bytes| interpreted as a big-endian number.
bssl::UniquePtr<BIGNUM>
ToBignum(base::span<const uint8_t> big_endian_bytes)
{
...
bssl::UniquePtr<EC_POINT>
GetPoint(const EC_GROUP* p224, base::span<const uint8_t, 1 + 28 + 28> x962)
{
    bssl::UniquePtr<EC_POINT> point(EC_POINT_new(p224));
    CHECK(EC_POINT_oct2point(p224,
                             point.get(),
                             x962.data(),
                             x962.size(),
                             /*ctx=*/nullptr));
...
// |use_m|.
bssl::UniquePtr<EC_POINT>
GetMask(const EC_GROUP* p224, bool use_m, base::span<const uint8_t> pw)
{
...
std::string
ToMessage(const EC_GROUP* p224, const EC_POINT* in)
{
    ...
    CHECK(EC_POINT_point2oct(
                  p224, in, POINT_CONVERSION_UNCOMPRESSED, x962, sizeof(x962), /*ctx=*/nullptr)
          == sizeof(x962));
...
bssl::UniquePtr<EC_POINT>
FromMessage(const EC_GROUP* p224, base::StringPiece in)
{
    if (in.size() != 56)
    {
        return nullptr;
...
namespace crypto
{
P224EncryptedKeyExchange::P224EncryptedKeyExchange(PeerType peer_type, base::StringPiece password) :
    state_(kStateInitial), is_server_(peer_type == kPeerTypeServer)
{
    ...
    SHA256HashString(std::string(password.data(), password.length()), pw_, sizeof(pw_));

    Init();
}

void
P224EncryptedKeyExchange::Init()
{
    // X = g**x_
...
const std::string&
P224EncryptedKeyExchange::GetNextMessage()
{
...
P224EncryptedKeyExchange::Result
P224EncryptedKeyExchange::ProcessMessage(base::StringPiece message)
{
    if (state_ == kStateRecvHash)
    {
        ...
        if (!SecureMemEqual(message.data(), expected_authenticator_, message.size()))
        {
    ...
    CalculateHash(kPeerTypeClient, client_masked_dh, server_masked_dh, key_, client_hash);
    CalculateHash(kPeerTypeServer, client_masked_dh, server_masked_dh, key_, server_hash);

    const uint8_t* my_hash = is_server_ ? server_hash : client_hash;
    const uint8_t* their_hash = is_server_ ? client_hash : server_hash;

    next_message_ = std::string(reinterpret_cast<const char*>(my_hash), kSHA256Length);
...
void
P224EncryptedKeyExchange::CalculateHash(PeerType peer_type,
...
const std::string&
P224EncryptedKeyExchange::GetKey() const
{
...
const std::string&
P224EncryptedKeyExchange::GetUnverifiedKey() const
{
...
void
P224EncryptedKeyExchange::SetXForTesting(const std::string& x)
{
    memset(&x_, 0, sizeof(x_));
