...
bool
SignatureVerifier::VerifyInit(SignatureAlgorithm signature_algorithm,
                              ...
                              base::span<const uint8_t> public_key_info)
{
    ...
    if (!public_key || CBS_len(&cbs) != 0 || EVP_PKEY_id(public_key.get()) != pkey_type)
    {
        return false;
    }
    ...
    if (!EVP_DigestVerifyInit(
                verify_context_->ctx.get(), &pkey_ctx, digest, nullptr, public_key.get()))
    {
        return false;
    }
    ...
    if (signature_algorithm == RSA_PSS_SHA256)
    {
        if (!EVP_PKEY_CTX_set_rsa_padding(pkey_ctx, RSA_PKCS1_PSS_PADDING)
            ...
            || !EVP_PKEY_CTX_set_rsa_pss_saltlen(pkey_ctx, -1 /* match digest and salt length */))
        {
            return false;
        }
    }
...
void
SignatureVerifier::VerifyUpdate(base::span<const uint8_t> data_part)
{
    DCHECK(verify_context_);
    OpenSSLErrStackTracer err_tracer(FROM_HERE);
    int rv = EVP_DigestVerifyUpdate(verify_context_->ctx.get(), data_part.data(), data_part.size());
    DCHECK_EQ(rv, 1);
}
...
SignatureVerifier::VerifyFinal()
{
    ...
    OpenSSLErrStackTracer err_tracer(FROM_HERE);
    int rv =
            EVP_DigestVerifyFinal(verify_context_->ctx.get(), signature_.data(), signature_.size());
