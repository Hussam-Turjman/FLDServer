...
// tricky.)
class CORE_EXPORT ECPrivateKey
{
public:
    ~ECPrivateKey();
    ...
    static std::unique_ptr<ECPrivateKey>
    Create();
    ...
    static std::unique_ptr<ECPrivateKey>
    CreateFromPrivateKeyInfo(base::span<const uint8_t> input);
    ...
    static std::unique_ptr<ECPrivateKey>
    CreateFromEncryptedPrivateKeyInfo(base::span<const uint8_t> encrypted_private_key_info);
    ...
    static std::unique_ptr<ECPrivateKey>
    DeriveFromSecret(base::span<const uint8_t> secret);

    // Returns a copy of the object.
    std::unique_ptr<ECPrivateKey>
    Copy() const;
    ...
    bool
    ExportPrivateKey(std::vector<uint8_t>* output) const;
    ...
    bool
    ExportEncryptedPrivateKey(std::vector<uint8_t>* output) const;

    // Exports the public key to an X.509 SubjectPublicKeyInfo block.
    bool
    ExportPublicKey(std::vector<uint8_t>* output) const;

    // Exports the public key as an EC point in the uncompressed point format.
    bool
    ExportRawPublicKey(std::string* output) const;
