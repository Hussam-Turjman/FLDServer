...
#include "fldserver/base/crypto/sha2.h"
#include "fldserver/base/crypto/unexportable_key.h"
#include "openssl/bn.h"
//#include "openssl/bytestring.h" // boringssl
#include "openssl/ec.h"
//#include "openssl/ec_key.h" // boringssl
#include "openssl/ecdsa.h"
#include "openssl/evp.h"
//#include "openssl/nid.h" // boringssl
...
using ScopedProvider =
        base::ScopedGeneric<NCRYPT_PROV_HANDLE, NCryptObjectTraits<NCRYPT_PROV_HANDLE>>;
using ScopedKey = base::ScopedGeneric<NCRYPT_KEY_HANDLE, NCryptObjectTraits<NCRYPT_KEY_HANDLE>>;
...
// signing algorithm.
base::Optional<LPCWSTR>
BCryptAlgorithmFor(SignatureVerifier::SignatureAlgorithm algo)
{
    switch (algo)
    {
        case SignatureVerifier::SignatureAlgorithm::RSA_PKCS1_SHA256: return BCRYPT_RSA_ALGORITHM;

        case SignatureVerifier::SignatureAlgorithm::ECDSA_SHA256:
            return BCRYPT_ECDSA_P256_ALGORITHM;

        default: return base::nullopt;
    }
}

// GetBestSupported returns the first element of |acceptable_algorithms| that
// |provider| supports, or |nullopt| if there isn't any.
base::Optional<SignatureVerifier::SignatureAlgorithm>
GetBestSupported(NCRYPT_PROV_HANDLE provider,
                 base::span<const SignatureVerifier::SignatureAlgorithm> acceptable_algorithms)
{
    for (auto algo : acceptable_algorithms)
    {
        ...
        if (!FAILED(NCryptIsAlgSupported(provider,
                                         *bcrypto_algo_name,
                                         /*flags=*/0)))
        {
            return algo;
...
GetKeyProperty(NCRYPT_KEY_HANDLE key, LPCWSTR property)
{
    DWORD size;
    ...
    if (FAILED(NCryptGetProperty(key, property, ret.data(), ret.size(), &size, 0)))
    {
        return base::nullopt;
    }
...
base::Optional<std::vector<uint8_t>>
ExportKey(NCRYPT_KEY_HANDLE key, LPCWSTR format)
{
    DWORD output_size;
    if (FAILED(NCryptExportKey(key, 0, format, nullptr, nullptr, 0, &output_size, 0)))
    {
        return base::nullopt;
    }

    std::vector<uint8_t> output(output_size);
    if (FAILED(NCryptExportKey(
                key, 0, format, nullptr, output.data(), output.size(), &output_size, 0)))
    {
        return base::nullopt;
    }
...
base::Optional<std::vector<uint8_t>>
GetP256ECDSASPKI(NCRYPT_KEY_HANDLE key)
{
    LOG(FATAL) << "Not implemented";
    return base::nullopt;
...
base::Optional<std::vector<uint8_t>>
GetRSASPKI(NCRYPT_KEY_HANDLE key)
{
    LOG(FATAL) << "Not implemented";
    return base::nullopt;
    /*const base::Optional<std::vector<uint8_t>> pub_key =
...
public:
    ...
    std::vector<uint8_t>
    GetSubjectPublicKeyInfo() const override
    {
        return spki_;
    }

    std::vector<uint8_t>
    GetWrappedKey() const override
    {
        return wrapped_;
    }

    base::Optional<std::vector<uint8_t>>
    SignSlowly(base::span<const uint8_t> data) override
    {
        LOG(FATAL) << "Not implemented";
        return base::nullopt;
...
public:
    ...
    SignSlowly(base::span<const uint8_t> data) override
    {
        ...
        DWORD sig_size;
        if (FAILED(NCryptSignHash(key_.get(),
                                  &padding_info,
                                  digest.data(),
                                  digest.size(),
                                  nullptr,
...
public:
    ...
    base::Optional<SignatureVerifier::SignatureAlgorithm>
    SelectAlgorithm(
            base::span<const SignatureVerifier::SignatureAlgorithm> acceptable_algorithms) override
    {
        ScopedProvider provider;
        if (FAILED(NCryptOpenStorageProvider(ScopedProvider::Receiver(provider).get(),
            ...
            base::span<const SignatureVerifier::SignatureAlgorithm> acceptable_algorithms) override
    {
        ...
        if (FAILED(NCryptCreatePersistedKey(provider.get(),
                                            ScopedKey::Receiver(key).get(),
        ...
        switch (*algo)
        {
            case SignatureVerifier::SignatureAlgorithm::ECDSA_SHA256:
                ...
                return std::make_unique<ECDSAKey>(std::move(provider),
                                                  std::move(key),
            ...
            case SignatureVerifier::SignatureAlgorithm::RSA_PKCS1_SHA256:
                ...
                return std::make_unique<RSAKey>(std::move(provider),
                                                std::move(key),
                                                std::move(*wrapped_key),
                                                std::move(spki.value()));
    ...
    std::unique_ptr<UnexportableSigningKey>
    FromWrappedSigningKeySlowly(base::span<const uint8_t> wrapped) override
    {
        base::ScopedBlockingCall scoped_blocking_call(FROM_HERE, base::BlockingType::WILL_BLOCK);
        ...
        ScopedKey key;
        if (FAILED(NCryptImportKey(provider.get(),
                                   /*hImportKey=*/NULL,
                                   BCRYPT_OPAQUE_KEY_BLOB,
                                   /*pParameterList=*/nullptr,
                                   ScopedKey::Receiver(key).get(),
                                   const_cast<PBYTE>(wrapped.data()),
                                   wrapped.size(),
