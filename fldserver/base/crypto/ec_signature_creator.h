...
class CORE_EXPORT ECSignatureCreatorFactory
{
public:
    ...
    virtual std::unique_ptr<ECSignatureCreator>
    Create(ECPrivateKey* key) = 0;
};

// Signs data using a bare private key (as opposed to a full certificate).
// We need this class because SignatureCreator is hardcoded to use
// RSAPrivateKey.
class CORE_EXPORT ECSignatureCreator
{
public:
    ...
    static std::unique_ptr<ECSignatureCreator>
    Create(ECPrivateKey* key);
    ...
    static void
    SetFactoryForTesting(ECSignatureCreatorFactory* factory);
    ...
    //    s     INTEGER }
    virtual bool
    Sign(const uint8_t* data, int data_len, std::vector<uint8_t>* signature) = 0;
    ...
    virtual bool
    DecodeSignature(const std::vector<uint8_t>& signature, std::vector<uint8_t>* out_raw_sig) = 0;
};
