...
#include "fldserver/base/crypto/nss_key_util.h"
...
ScopedSECItem
MakeIDFromSPKI(base::span<const uint8_t> input)
{
                      ...
                      ScopedSECKEYPrivateKey* private_key)
{
    ...
    private_key->reset(PK11_GenerateKeyPair(slot,
                     ...
                     ScopedSECKEYPrivateKey* private_key)
{
    ...
    SECKEYECParams ec_parameters = {
            siDEROID, parameters_buf.data(), static_cast<unsigned>(parameters_buf.size())};
                               ...
                               bool permanent)
{
    ...
    SECStatus rv = SEC_QuickDERDecodeItem(
            arena.get(), &der_private_key_info, SEC_ASN1_GET(SEC_AnyTemplate), &input_item);
    if (rv != SECSuccess)
        return nullptr;
...
FindNSSKeyFromPublicKeyInfo(base::span<const uint8_t> input)
{
    ...
    for (const SECMODModuleList* item = head; item != nullptr; item = item->next)
    {
...
FindNSSKeyFromPublicKeyInfoInSlot(base::span<const uint8_t> input, PK11SlotInfo* slot)
{
    ...
    return ScopedSECKEYPrivateKey(PK11_FindKeyByKeyID(slot, cka_id.get(), nullptr));
}
...
DecodeSubjectPublicKeyInfoNSS(base::span<const uint8_t> input)
{
    ...
    ScopedCERTSubjectPublicKeyInfo spki(SECKEY_DecodeDERSubjectPublicKeyInfo(&key_der));
    return spki;
}
