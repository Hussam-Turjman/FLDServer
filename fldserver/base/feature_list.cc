...
#include "fldserver/base/base_paths.h"
...
struct FeatureEntry
{
    ...
    bool
    GetFeatureAndTrialName(StringPiece* feature_name, StringPiece* trial_name) const
    {
...
bool
IsValidFeatureOrFieldTrialName(const std::string& name)
{
...
bool
SplitIntoTwo(const std::string& separator, StringPiece* first, std::string* second)
{
    ...
    {
        DLOG(ERROR) << "Only one '" << separator << "' is allowed but got: " << *first;
        return false;
    }
    *first = parts[0];
    return true;
...
bool
ParseEnableFeatures(const std::string& enable_features,
                    ...
                    std::string* force_fieldtrial_params)
{
    ...
    for (auto& enable_feature : FeatureList::SplitFeatureListString(enable_features))
    {
        ...
        if (!feature_params.empty())
        {
            ...
            force_fieldtrial_params_list.push_back(study + "." + group + ":" + feature_params);
        }
        enable_features_list.push_back(study.empty() ? feature_name : (feature_name + "<" + study));
    }
...
const Feature kDCheckIsFatalFeature{"DcheckIsFatal", FEATURE_DISABLED_BY_DEFAULT};
...
FeatureList::ScopedDisallowOverrides::ScopedDisallowOverrides(const char* reason)
#if DCHECK_IS_ON()
    :
...
void
FeatureList::InitializeFromCommandLine(const std::string& enable_features,
                                       const std::string& disable_features)
{
    ...
    bool parse_enable_features_result = ParseEnableFeatures(
            enable_features, &parsed_enable_features, &force_fieldtrials, &force_fieldtrial_params);
    DCHECK(parse_enable_features_result) << StringPrintf("The --%s list is unparsable or invalid, "
    ...
    {
        bool associate_params_result =
                AssociateFieldTrialParamsFromString(force_fieldtrial_params, &UnescapeValue);
        DCHECK(associate_params_result)
                << StringPrintf("The field trial parameters part of the --%s list is invalid. Make "
                                "sure "
                                "you %%-encode the following characters in param values: %%:/.,",
                                ::switches::kEnableFeatures);

        bool create_trials_result = FieldTrialList::CreateTrialsFromString(force_fieldtrials);
        DCHECK(create_trials_result) << StringPrintf("Invalid field trials are specified in --%s.",
                                                     ::switches::kEnableFeatures);
    }
    ...
    RegisterOverridesFromCommandLine(parsed_enable_features, OVERRIDE_ENABLE_FEATURE);

    initialized_from_command_line_ = true;
}

void
FeatureList::InitializeFromSharedMemory(PersistentMemoryAllocator* allocator)
{
    DCHECK(!initialized_);
    ...
    {
        OverrideState override_state = static_cast<OverrideState>(entry->override_state);
...
bool
FeatureList::IsFeatureOverridden(const std::string& feature_name) const
{
    return overrides_.count(feature_name);
}

bool
FeatureList::IsFeatureOverriddenFromCommandLine(const std::string& feature_name) const
{
...
bool
FeatureList::IsFeatureOverriddenFromCommandLine(const std::string& feature_name,
...
void
FeatureList::AssociateReportingFieldTrial(const std::string& feature_name,
                                          OverrideState for_overridden_state,
                                          FieldTrial* field_trial)
{
    DCHECK(IsFeatureOverriddenFromCommandLine(feature_name, for_overridden_state));
...
void
FeatureList::RegisterFieldTrialOverride(const std::string& feature_name,
                                        ...
                                        FieldTrial* field_trial)
{
    ...
    DCHECK(!Contains(overrides_, feature_name)
           || !overrides_.find(feature_name)->second.field_trial)
            << "Feature " << feature_name << " has conflicting field trial overrides: "
...
void
FeatureList::RegisterExtraFeatureOverrides(const std::vector<FeatureOverrideInfo>& extra_overrides)
{
    for (const FeatureOverrideInfo& override_info : extra_overrides)
    {
        RegisterOverride(override_info.first.get().name,
                         override_info.second,
                         /* field_trial = */ nullptr);
    }
}

void
FeatureList::AddFeaturesToAllocator(PersistentMemoryAllocator* allocator)
{
    DCHECK(initialized_);
...
void
FeatureList::GetFeatureOverrides(std::string* enable_overrides, std::string* disable_overrides)
{
    GetFeatureOverridesImpl(enable_overrides, disable_overrides, false);
}

void
FeatureList::GetCommandLineFeatureOverrides(std::string* enable_overrides,
...
// static
bool
FeatureList::IsEnabled(const Feature& feature)
{
#if DCHECK_IS_ON()
...
// static
FieldTrial*
FeatureList::GetFieldTrial(const Feature& feature)
{
#if DCHECK_IS_ON()
...
FeatureList::SplitFeatureListString(StringPiece input)
{
...
// static
bool
FeatureList::InitializeInstance(const std::string& enable_features,
                                const std::string& disable_features)
{
    return InitializeInstance(
            enable_features, disable_features, std::vector<FeatureOverrideInfo>());
}
...
// static
void
FeatureList::SetInstance(std::unique_ptr<FeatureList> instance)
{
    ...
    if (FeatureList::IsEnabled(kDCheckIsFatalFeature)
        || CommandLine::ForCurrentProcess()->HasSwitch("gtest_internal_run_death_test"))
    {
...
// static
void
FeatureList::RestoreInstanceForTesting(std::unique_ptr<FeatureList> instance)
{
    DCHECK(!g_feature_list_instance);
    // Note: Intentional leak of global singleton.
    g_feature_list_instance = instance.release();
...
// static
void
FeatureList::ForbidUseForCurrentModule()
{
#if DCHECK_IS_ON()
...
void
FeatureList::FinalizeInitialization()
{
    DCHECK(!initialized_);
...
bool
FeatureList::IsFeatureEnabled(const Feature& feature)
{
    DCHECK(initialized_);
...
FieldTrial*
FeatureList::GetAssociatedFieldTrial(const Feature& feature)
{
    DCHECK(initialized_);
    DCHECK(IsValidFeatureOrFieldTrialName(feature.name)) << feature.name;
    DCHECK(CheckFeatureIdentity(feature)) << feature.name;
...
void
FeatureList::RegisterOverridesFromCommandLine(const std::string& feature_list,
...
void
FeatureList::RegisterOverride(StringPiece feature_name,
                              OverrideState overridden_state,
                              FieldTrial* field_trial)
{
    DCHECK(!initialized_);
    ...
    overrides_.emplace(std::string(feature_name), OverrideEntry(overridden_state, field_trial));
}

void
FeatureList::GetFeatureOverridesImpl(std::string* enable_overrides,
                                     std::string* disable_overrides,
                                     bool command_line_only)
{
    ...
    for (const auto& entry : overrides_)
    {
        ...
        switch (entry.second.overridden_state)
        {
            ...
            case OVERRIDE_ENABLE_FEATURE: target_list = enable_overrides; break;
            case OVERRIDE_DISABLE_FEATURE: target_list = disable_overrides; break;
        }
...
bool
FeatureList::CheckFeatureIdentity(const Feature& feature)
{
...
FeatureList::OverrideEntry::OverrideEntry(OverrideState overridden_state, FieldTrial* field_trial) :
