...
#include "fldserver/base/bind_post_task_internal.h"
...
BindPostTask(scoped_refptr<TaskRunner> task_runner,
             ...
             const Location& location = FROM_HERE)
{
    ...
    return base::BindOnce(
            ...
            std::make_unique<Helper>(std::move(task_runner), location, std::move(callback)));
}
             ...
             const Location& location = FROM_HERE)
{
    ...
    using Helper = internal::BindPostTaskTrampoline<RepeatingCallback<void(Args...)>>;
