...
//
class COMPONENT_EXPORT(IPC) ChannelMojo : public Channel,
...
public:
    ...
    static std::unique_ptr<ChannelFactory>
    CreateServerFactory(mojo::ScopedMessagePipeHandle handle,
                        const scoped_refptr<base::SingleThreadTaskRunner>& ipc_task_runner,
                        const scoped_refptr<base::SingleThreadTaskRunner>& proxy_task_runner);

    static std::unique_ptr<ChannelFactory>
    CreateClientFactory(mojo::ScopedMessagePipeHandle handle,
                        const scoped_refptr<base::SingleThreadTaskRunner>& ipc_task_runner,
                        const scoped_refptr<base::SingleThreadTaskRunner>& proxy_task_runner);
    ...
    GetAssociatedInterfaceSupport() override;
    ...
    void
    OnPeerPidReceived(int32_t peer_pid) override;
    void
    OnMessageReceived(const Message& message) override;
    void
    OnBrokenDataReceived() override;
    void
    OnPipeError() override;
    void
    OnAssociatedInterfaceRequest(const std::string& name,
...
private:
    ...
    void
    ForwardMessage(mojo::Message message);

    // Channel::AssociatedInterfaceSupport:
    std::unique_ptr<mojo::ThreadSafeForwarder<mojom::Channel>>
    CreateThreadSafeChannel() override;
    void
    AddGenericAssociatedInterface(const std::string& name,
    ...
    std::map<std::string, GenericAssociatedInterfaceFactory> associated_interfaces_;
