...
void
Logging::SetConsumer(Consumer* consumer)
{
    consumer_ = consumer;
}
...
Logging::OnSendLogs()
{
    ...
    Message* msg = new Message(MSG_ROUTING_CONTROL, IPC_LOGGING_ID, Message::PRIORITY_NORMAL);
...
void
Logging::SetIPCSender(IPC::Sender* sender)
{
    sender_ = sender;
}

void
Logging::OnReceivedLoggingMessage(const Message& message)
{
...
void
Logging::OnSendMessage(Message* message)
{
    if (!Enabled())
        return;
...
void
Logging::OnPreDispatchMessage(const Message& message)
{
    message.set_received_time(Time::Now().ToInternalValue());
}

void
Logging::OnPostDispatchMessage(const Message& message)
{
    ...
    else
    {
        main_thread_->PostTask(FROM_HERE,
                               base::BindOnce(&Logging::Log, base::Unretained(this), data));
    }
}

void
Logging::GetMessageText(uint32_t type,
                        std::string* name,
...
const char*
Logging::ANSIEscape(ANSIColor color)
{
    if (!enabled_color_)
        return "";
    switch (color)
    {
        case ANSI_COLOR_RESET: return "\033[m";
        case ANSI_COLOR_BLACK: return "\033[0;30m";
        case ANSI_COLOR_RED: return "\033[0;31m";
        case ANSI_COLOR_GREEN: return "\033[0;32m";
        case ANSI_COLOR_YELLOW: return "\033[0;33m";
        case ANSI_COLOR_BLUE: return "\033[0;34m";
        case ANSI_COLOR_MAGENTA: return "\033[0;35m";
        case ANSI_COLOR_CYAN: return "\033[0;36m";
        case ANSI_COLOR_WHITE: return "\033[0;37m";
    }
    return "";
}
...
void
Logging::Log(const LogData& data)
{
    if (consumer_)
    {
    ...
    if (enabled_on_stderr_)
    {
        ...
        double receive_delay =
                (Time::FromInternalValue(data.receive) - Time::FromInternalValue(data.sent))
                        .InSecondsF();
        double dispatch_delay =
                (Time::FromInternalValue(data.dispatch) - Time::FromInternalValue(data.sent))
                        .InSecondsF();
        fprintf(stderr,
...
GenerateLogData(const Message& message, LogData* data, bool get_params)
{
    ...
    else
    {
        ...
        Logging::GetMessageText(
                message.type(), &message_name, &message, get_params ? &params : NULL);
