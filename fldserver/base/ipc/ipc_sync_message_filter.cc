...
#if 0
bool
SyncMessageFilter::Send(Message* message)
{
    if (!message->is_sync())
    {
        {
        ...
        io_task_runner_->PostTask(
                FROM_HERE, base::BindOnce(&SyncMessageFilter::SendOnIOThread, this, message));
        return true;
    }

    base::WaitableEvent done_event(base::WaitableEvent::ResetPolicy::MANUAL,
                                   base::WaitableEvent::InitialState::NOT_SIGNALED);
    PendingSyncMsg pending_message(SyncMessage::GetMessageId(*message),
...

    {
        bool done = false;
        bool shutdown = false;
        scoped_refptr<mojo::SyncHandleRegistry> registry = mojo::SyncHandleRegistry::current();
        ...
        mojo::SyncHandleRegistry::EventCallbackSubscription done_subscription =
                registry->RegisterEvent(&done_event, base::BindRepeating(&OnEventReady, &done));
...
#endif
void
SyncMessageFilter::OnFilterAdded(Channel* channel)
{
...
bool
SyncMessageFilter::OnMessageReceived(const Message& message)
{
    base::AutoLock auto_lock(lock_);
...
void
SyncMessageFilter::SendOnIOThread(Message* message)
{
    if (channel_)
    {
...
#if 0
void
SyncMessageFilter::GetGenericRemoteAssociatedInterface(const std::string& interface_name,
                                                       mojo::ScopedInterfaceEndpointHandle handle)
{
    ...
    support->GetGenericRemoteAssociatedInterface(interface_name, std::move(handle));
}
#endif
}  // namespace IPC
