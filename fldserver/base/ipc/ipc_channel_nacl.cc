...
#include "fldserver/base/ipc/ipc_channel_nacl.h"
...
#include "fldserver/base/message_loop/message_pump_for_io.h"
...
#include "fldserver/base/threading/simple_thread.h"
...
#include "fldserver/native_client/src/public/imc_syscalls.h"
#include "fldserver/native_client/src/public/imc_types.h"
...
{
bool
ReadDataOnReaderThread(int pipe, MessageContents* contents)
{
    DCHECK(pipe >= 0);
    if (pipe < 0)
        return false;
    ...
    NaClAbiNaClImcMsgHdr msg = {&iov, 1, &contents->fds[0], contents->fds.size()};
...
class ChannelNacl::ReaderThreadRunner : public base::DelegateSimpleThread::Delegate
{
public:
    ...
    ReaderThreadRunner(
            int pipe,
            base::RepeatingCallback<void(std::unique_ptr<MessageContents>)> data_read_callback,
...
private:
    int pipe_;
    base::RepeatingCallback<void(std::unique_ptr<MessageContents>)> data_read_callback_;
...
void
ChannelNacl::ReaderThreadRunner::Run()
{
    while (true)
    {
        ...
        if (success)
        {
            main_task_runner_->PostTask(
                    FROM_HERE, base::BindOnce(data_read_callback_, std::move(msg_contents)));
        }
        else
        {
...
ChannelNacl::ChannelNacl(const IPC::ChannelHandle& channel_handle, Mode mode, Listener* listener) :
    ChannelReader(listener), mode_(mode), waiting_connect_(true), pipe_(-1), weak_ptr_factory_(this)
{
...
ChannelNacl::Connect()
{
    ...
    reader_thread_runner_ = std::make_unique<ReaderThreadRunner>(
            pipe_,
            base::BindRepeating(&ChannelNacl::DidRecvMsg, weak_ptr_factory_.GetWeakPtr()),
            base::BindRepeating(&ChannelNacl::ReadDidFail, weak_ptr_factory_.GetWeakPtr()),
            base::ThreadTaskRunnerHandle::Get());
    reader_thread_ = std::make_unique<base::DelegateSimpleThread>(reader_thread_runner_.get(),
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(
            FROM_HERE,
            base::BindOnce(&ChannelNacl::CallOnChannelConnected, weak_ptr_factory_.GetWeakPtr()));
...
bool
ChannelNacl::Send(Message* message)
{
    DCHECK(!message->HasAttachments());
    DVLOG(2) << "sending message @" << message << " on channel @" << this << " with type "
             << message->type();
...
void
ChannelNacl::DidRecvMsg(std::unique_ptr<MessageContents> contents)
{
    ...
    {
        input_attachments_.push_back(new internal::PlatformFileAttachment(base::ScopedFD(fd)));
    }
    contents->fds.clear();
...
void
ChannelNacl::ReadDidFail()
{
    Close();
}

bool
ChannelNacl::CreatePipe(const IPC::ChannelHandle& channel_handle)
{
    DCHECK(pipe_ == -1);
...
ChannelNacl::ProcessOutgoingMessages()
{
    ...
    while (!output_queue_.empty())
    {
        ...
        {
            scoped_refptr<MessageAttachment> attachment = msg->attachment_set()->GetAttachmentAt(i);
            DCHECK_EQ(MessageAttachment::Type::PLATFORM_FILE, attachment->GetType());
            fds.push_back(
                    static_cast<internal::PlatformFileAttachment&>(*attachment).TakePlatformFile());
        }

        NaClAbiNaClImcMsgIoVec iov = {const_cast<void*>(msg->data()), msg->size()};
        ...
        if (bytes_written < 0)
        {
            ...
            DCHECK(errno == EPIPE);
            Close();
            PLOG(ERROR) << "pipe_ error on " << pipe_
                        << " Currently writing message of size: " << msg->size();
            return false;
        }
        else
        {
        ...
        // Message sent OK!
        DVLOG(2) << "sent message @" << msg.get() << " with type " << msg->type() << " on fd "
                 << pipe_;
    }
    return true;
}
...
void
ChannelNacl::CallOnChannelConnected()
{
    listener()->OnChannelConnected(-1);
}

ChannelNacl::ReadState
ChannelNacl::ReadData(char* buffer, int buffer_len, int* bytes_read)
{
    *bytes_read = 0;
    if (pipe_ == -1)
        return READ_FAILED;
    ...
    while (!read_queue_.empty() && *bytes_read < buffer_len)
    {
        ...
        else
        {
            ...
            // the code simple).
            std::copy(vec->begin(), vec->begin() + bytes_to_read, buffer + *bytes_read);
...
bool
ChannelNacl::ShouldDispatchInputMessage(Message* msg)
{
    return true;
}

bool
ChannelNacl::GetAttachments(Message* msg)
{
...
bool
ChannelNacl::DidEmptyInputBuffers()
{
...
void
ChannelNacl::HandleInternalMessage(const Message& msg)
{
...
Channel::Create(const IPC::ChannelHandle& channel_handle, Mode mode, Listener* listener)
{
