...
class ChannelNacl : public Channel, public internal::ChannelReader
{
public:
    // Mirror methods of Channel, see ipc_channel.h for description.
    ChannelNacl(const IPC::ChannelHandle& channel_handle, Mode mode, Listener* listener);
    ...
    void
    DidRecvMsg(std::unique_ptr<MessageContents> contents);
    void
    ReadDidFail();
...
private:
    ...
    bool
    CreatePipe(const IPC::ChannelHandle& channel_handle);
    bool
    ...
    ReadState
    ReadData(char* buffer, int buffer_len, int* bytes_read) override;
    bool
    ShouldDispatchInputMessage(Message* msg) override;
    bool
    GetAttachments(Message* msg) override;
    bool
    DidEmptyInputBuffers() override;
    void
    HandleInternalMessage(const Message& msg) override;
