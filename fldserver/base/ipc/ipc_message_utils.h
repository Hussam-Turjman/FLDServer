...
#include "fldserver/base/containers/small_map.h"
...
#include "fldserver/base/memory/platform_shared_memory_region.h"
#include "fldserver/base/memory/read_only_shared_memory_region.h"
#include "fldserver/base/memory/unsafe_shared_memory_region.h"
...
#include "fldserver/base/util/type_safety/id_type.h"
#include "fldserver/fldserver_config.h"
#include "fldserver/base/ipc/ipc_param_traits.h"
...
template <class P>
inline void
WriteParam(base::Pickle* m, const P& p)
{
...
ReadParam(const base::Pickle* m, base::PickleIterator* iter, P* p)
{
    typedef typename SimilarTypeTraits<P>::Type Type;
...
template <class P>
inline void
LogParam(const P& p, std::string* l)
{
    typedef typename SimilarTypeTraits<P>::Type Type;
...
struct ParamTraits<bool>
{
    ...
    static void
    Write(base::Pickle* m, const param_type& p)
    {
        m->WriteBool(p);
    }
    static bool
...
struct COMPONENT_EXPORT(IPC) ParamTraits<std::vector<char>>
{
    ...
    Write(base::Pickle* m, const param_type& p);
    static bool
    Read(const base::Pickle*, base::PickleIterator* iter, param_type* r);
    static void
    Log(const param_type& p, std::string* l);
};
...
#if defined(OS_ANDROID)
template <>
struct COMPONENT_EXPORT(IPC) ParamTraits<base::android::ScopedHardwareBufferHandle>
{
...
template <>
struct COMPONENT_EXPORT(IPC) ParamTraits<base::subtle::PlatformSharedMemoryRegion>
{
...
template <>
struct COMPONENT_EXPORT(IPC) ParamTraits<base::subtle::PlatformSharedMemoryRegion::Mode>
{
...
struct TupleParamTraitsHelper
{
    ...
    static bool
    Read(const base::Pickle* m, base::PickleIterator* iter, T* r)
    {
        return ReadParam(m, iter, &std::get<index>(*r)) && Next::Read(m, iter, r);
    }

    static void
    Log(const T& p, std::string* l)
    {
...
struct ParamTraits<std::tuple<Args...>>
{
    ...
    using Helper = TupleParamTraitsHelper<param_type, 0, std::tuple_size<param_type>::value>;
...
template <typename NormalMap, int kArraySize, typename EqualKey, typename MapInit>
...
{
    static void
    Write(base::Pickle* m, const Message& p);
    static bool
    Read(const base::Pickle* m, base::PickleIterator* iter, Message* r);
    static void
    Log(const Message& p, std::string* l);
};
...
void
GenerateLogData(const Message& message, LogData* data, bool get_params);

#if BUILDFLAG(IPC_MESSAGE_LOG_ENABLED)
inline void
AddOutputParamsToLog(const Message* msg, std::string* l)
{
...
inline void
LogReplyParamsToMessage(const ReplyParamType& reply_params, const Message* msg)
{
...
inline void
ConnectMessageAndReply(const Message* msg, Message* reply)
{
    if (msg->sent_time())
    {
