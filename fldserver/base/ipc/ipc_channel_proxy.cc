...
#include "fldserver/base/ipc/ipc_channel_proxy.h"
...
#include "fldserver/base/ipc/ipc_channel_factory.h"
#include "fldserver/base/ipc/ipc_listener.h"
#include "fldserver/base/ipc/ipc_logging.h"
#include "fldserver/base/ipc/ipc_message_macros.h"
#include "fldserver/base/ipc/message_filter.h"
#include "fldserver/base/ipc/message_filter_router.h"
    ...
    peer_pid_(base::kNullProcessId)
{
    ...
    DCHECK(!listener || (ipc_task_runner_.get() != default_listener_task_runner_.get()));
}
...
void
ChannelProxy::Context::CreateChannel(std::unique_ptr<ChannelFactory> factory)
{
    ...
    Channel::AssociatedInterfaceSupport* support = channel_->GetAssociatedInterfaceSupport();
    if (support)
    {
...
bool
ChannelProxy::Context::TryFilters(const Message& message)
{
...
void
ChannelProxy::Context::PauseChannel()
{
    DCHECK(channel_);
    channel_->Pause();
}
...
// Called on the IPC::Channel thread
void
ChannelProxy::Context::UnpauseChannel(bool flush)
{
    DCHECK(channel_);
...
// Called on the IPC::Channel thread
void
ChannelProxy::Context::FlushChannel()
{
    DCHECK(channel_);
    channel_->Flush();
}
...
// Called on the IPC::Channel thread
bool
ChannelProxy::Context::OnMessageReceived(const Message& message)
{
...
// Called on the IPC::Channel thread
bool
ChannelProxy::Context::OnMessageReceivedNoFilter(const Message& message)
{
    GetTaskRunner(message.routing_id())
            ->PostTask(FROM_HERE, base::BindOnce(&Context::OnDispatchMessage, this, message));
    return true;
}

// Called on the IPC::Channel thread
void
ChannelProxy::Context::OnChannelConnected(int32_t peer_pid)
{
    ...
    default_listener_task_runner_->PostTask(FROM_HERE,
                                            base::BindOnce(&Context::OnDispatchConnected, this));
}

// Called on the IPC::Channel thread
void
ChannelProxy::Context::OnChannelError()
{
    ...
    // See above comment about using default_listener_task_runner_ here.
    default_listener_task_runner_->PostTask(FROM_HERE,
                                            base::BindOnce(&Context::OnDispatchError, this));
}

// Called on the IPC::Channel thread
void
ChannelProxy::Context::OnAssociatedInterfaceRequest(const std::string& interface_name,
                                                    mojo::ScopedInterfaceEndpointHandle handle)
{
    default_listener_task_runner_->PostTask(
            FROM_HERE,
            base::BindOnce(&Context::OnDispatchAssociatedInterfaceRequest,
                           this,
                           interface_name,
                           std::move(handle)));
}
...
// Called on the IPC::Channel thread
void
ChannelProxy::Context::OnChannelOpened()
{
    DCHECK(channel_);
...
// Called on the IPC::Channel thread
void
ChannelProxy::Context::OnChannelClosed()
{
...
// Called on the IPC::Channel thread
void
ChannelProxy::Context::OnSendMessage(std::unique_ptr<Message> message)
{
    if (quota_checker_)
...
// Called on the IPC::Channel thread
void
ChannelProxy::Context::OnAddFilter()
{
...
// Called on the IPC::Channel thread
void
ChannelProxy::Context::OnRemoveFilter(MessageFilter* filter)
{
    // No lock necessary for |peer_pid_| because it is only modified on this
    // thread.
    if (peer_pid_ == base::kNullProcessId)
...
void
ChannelProxy::Context::AddFilter(MessageFilter* filter)
{
    base::AutoLock auto_lock(pending_filters_lock_);
    pending_filters_.push_back(base::WrapRefCounted(filter));
    ipc_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Context::OnAddFilter, this));
}

// Called on the listener's thread
void
ChannelProxy::Context::OnDispatchMessage(const Message& message)
{
    if (!listener_)
        return;
...
// Called on the listener's thread.
void
ChannelProxy::Context::AddListenerTaskRunner(
        int32_t routing_id, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
...
// Called on the listener's thread.
void
ChannelProxy::Context::RemoveListenerTaskRunner(int32_t routing_id)
{
    DCHECK(default_listener_task_runner_->BelongsToCurrentThread());
    base::AutoLock lock(listener_thread_task_runners_lock_);
...
// Called on the listener's thread
void
ChannelProxy::Context::OnDispatchConnected()
{
...
// Called on the listener's thread
void
ChannelProxy::Context::OnDispatchError()
{
    if (listener_)
...
// Called on the listener's thread
void
ChannelProxy::Context::OnDispatchBadMessage(const Message& message)
{
    if (listener_)
        listener_->OnBadMessageReceived(message);
}
...
// Called on the listener's thread
void
ChannelProxy::Context::OnDispatchAssociatedInterfaceRequest(
        const std::string& interface_name, mojo::ScopedInterfaceEndpointHandle handle)
{
    if (listener_)
...
void
ChannelProxy::Context::ClearChannel()
{
    base::AutoLock l(channel_lifetime_lock_);
    channel_.reset();
}
...
void
ChannelProxy::Context::AddGenericAssociatedInterfaceForIOThread(
        const std::string& name, const GenericAssociatedInterfaceFactory& factory)
{
    base::AutoLock l(channel_lifetime_lock_);
    if (!channel_)
    {
...
void
ChannelProxy::Context::Send(Message* message)
{
    if (quota_checker_)
    ...
    ipc_task_runner()->PostTask(
            FROM_HERE,
            base::BindOnce(&ChannelProxy::Context::OnSendMessage, this, base::WrapUnique(message)));
}

//-----------------------------------------------------------------------------

// static
std::unique_ptr<ChannelProxy>
ChannelProxy::Create(const IPC::ChannelHandle& channel_handle,
                     Channel::Mode mode,
                     Listener* listener,
                     const scoped_refptr<base::SingleThreadTaskRunner>& ipc_task_runner,
                     const scoped_refptr<base::SingleThreadTaskRunner>& listener_task_runner)
...
// static
std::unique_ptr<ChannelProxy>
ChannelProxy::Create(std::unique_ptr<ChannelFactory> factory,
                     Listener* listener,
                     const scoped_refptr<base::SingleThreadTaskRunner>& ipc_task_runner,
                     const scoped_refptr<base::SingleThreadTaskRunner>& listener_task_runner)
{
...
ChannelProxy::ChannelProxy(Context* context) : context_(context), did_init_(false)
{
...
ChannelProxy::ChannelProxy(
        ...
        const scoped_refptr<base::SingleThreadTaskRunner>& ipc_task_runner,
        const scoped_refptr<base::SingleThreadTaskRunner>& listener_task_runner) :
    context_(new Context(listener, ipc_task_runner, listener_task_runner)), did_init_(false)
{
#if defined(ENABLE_IPC_FUZZER)
    outgoing_message_filter_ = nullptr;
#endif
...
void
ChannelProxy::Init(const IPC::ChannelHandle& channel_handle,
                   Channel::Mode mode,
                   bool create_pipe_now)
{
    ...
    Init(ChannelFactory::Create(channel_handle, mode, context_->ipc_task_runner()),
         create_pipe_now);
}

void
ChannelProxy::Init(std::unique_ptr<ChannelFactory> factory, bool create_pipe_now)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(!did_init_);
    ...
    else
    {
        context_->ipc_task_runner()->PostTask(
                FROM_HERE, base::BindOnce(&Context::CreateChannel, context_, std::move(factory)));
    }

    // complete initialization on the background thread
    context_->ipc_task_runner()->PostTask(FROM_HERE,
                                          base::BindOnce(&Context::OnChannelOpened, context_));
...
{
    context_->ipc_task_runner()->PostTask(FROM_HERE,
                                          base::BindOnce(&Context::PauseChannel, context_));
}

void
ChannelProxy::Unpause(bool flush)
{
    context_->ipc_task_runner()->PostTask(
...
{
    context_->ipc_task_runner()->PostTask(FROM_HERE,
                                          base::BindOnce(&Context::FlushChannel, context_));
}
...
ChannelProxy::Close()
{
    ...
    {
        context_->ipc_task_runner()->PostTask(FROM_HERE,
                                              base::BindOnce(&Context::OnChannelClosed, context_));
    }
}

bool
ChannelProxy::Send(Message* message)
{
...
void
ChannelProxy::SendInternal(Message* message)
{
    DCHECK(did_init_);
...
void
ChannelProxy::AddFilter(MessageFilter* filter)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void
ChannelProxy::RemoveFilter(MessageFilter* filter)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    context_->ipc_task_runner()->PostTask(
            FROM_HERE,
            base::BindOnce(&Context::OnRemoveFilter, context_, base::RetainedRef(filter)));
}

void
ChannelProxy::AddGenericAssociatedInterfaceForIOThread(
        const std::string& name, const GenericAssociatedInterfaceFactory& factory)
{
...
void
ChannelProxy::GetGenericRemoteAssociatedInterface(const std::string& name,
                                                  mojo::ScopedInterfaceEndpointHandle handle)
{
    DCHECK(did_init_);
    context()->thread_safe_channel().GetAssociatedInterface(
            name, mojo::PendingAssociatedReceiver<mojom::GenericInterface>(std::move(handle)));
}
