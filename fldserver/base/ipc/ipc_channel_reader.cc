...
#include "fldserver/base/ipc/ipc_channel_reader.h"
...
ChannelReader::ChannelReader(Listener* listener) :
    listener_(listener), max_input_buffer_size_(Channel::kMaximumReadBufferSize)
{
...
bool
ChannelReader::IsInternalMessage(const Message& m)
{
    return m.routing_id() == MSG_ROUTING_NONE && m.type() >= Channel::CLOSE_FD_MESSAGE_TYPE
           && m.type() <= Channel::HELLO_MESSAGE_TYPE;
}

bool
ChannelReader::IsHelloMessage(const Message& m)
{
    return m.routing_id() == MSG_ROUTING_NONE && m.type() == Channel::HELLO_MESSAGE_TYPE;
}

void
ChannelReader::CleanUp()
{
}

void
ChannelReader::DispatchMessage(Message* m)
{
    EMIT_TRACE_EVENT(*m);
...
bool
ChannelReader::TranslateInputData(const char* input_data, int input_data_len)
{
    const char* p;
    const char* end;
    ...
    size_t next_message_buffer_size =
            next_message_size ? next_message_size + Channel::kReadBufferSize - 1 : 0;
...
bool
ChannelReader::HandleTranslatedMessage(Message* translated_message)
{
...
bool
ChannelReader::HandleExternalMessage(Message* external_message)
{
...
void
ChannelReader::HandleDispatchError(const Message& message)
{
...
bool
ChannelReader::CheckMessageSize(size_t size)
{
