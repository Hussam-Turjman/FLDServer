...
#include "fldserver/base/trace_event/memory_allocator_dump.h"
#include "fldserver/base/trace_event/memory_dump_manager.h"
#include "fldserver/base/trace_event/memory_dump_provider.h"
...
class ControllerMemoryDumpProvider : public base::trace_event::MemoryDumpProvider
{
public:
    ...
    void
    AddController(ChannelAssociatedGroupController* controller)
    {
    ...
    void
    RemoveController(ChannelAssociatedGroupController* controller)
    {
        base::AutoLock lock(lock_);
    ...
    bool
    OnMemoryDump(const base::trace_event::MemoryDumpArgs& args,
...
struct MessageMemoryDumpInfo
{
    ...
    bool
    operator==(const MessageMemoryDumpInfo& other) const
    {
...
{
    size_t
    operator()(const MessageMemoryDumpInfo& info) const
    {
        return base::HashInts(info.id, info.profiler_tag ? base::FastHash(info.profiler_tag) : 0);
    }
};

class ChannelAssociatedGroupController : public mojo::AssociatedGroupController,
...
public:
        ...
        control_message_proxy_(&control_message_proxy_thunk_)
    {
        ...
        control_message_handler_.SetDescription("IPC::mojom::Bootstrap [primary] "
                                                "PipeControlMessageHandler");
        dispatcher_.SetValidator(std::make_unique<mojo::MessageHeaderValidator>("IPC::mojom::"
                                                                                "Bootstrap "
                                                                                "[primary] "
                                                                                "MessageHeaderValid"
                                                                                "ator"));
    ...
    void
    GetTopQueuedMessageMemoryDumpInfo(MessageMemoryDumpInfo* info, size_t* count)
    {
        std::unordered_map<MessageMemoryDumpInfo, size_t, MessageMemoryDumpInfoHash> counts;
    ...
    void
    Bind(mojo::ScopedMessagePipeHandle handle)
    {
        ...
        connector_->set_connection_error_handler(base::BindOnce(
                &ChannelAssociatedGroupController::OnPipeError, base::Unretained(this)));
    ...
    void
    CreateChannelEndpoints(mojo::AssociatedRemote<mojom::Channel>* sender,
                           mojo::PendingAssociatedReceiver<mojom::Channel>* receiver)
    {
        ...
        sender->Bind(mojo::PendingAssociatedRemote<mojom::Channel>(std::move(sender_handle), 0));
        *receiver = mojo::PendingAssociatedReceiver<mojom::Channel>(std::move(receiver_handle));
    }
    ...
    mojo::InterfaceId
    AssociateInterface(mojo::ScopedInterfaceEndpointHandle handle_to_send) override
    {
        ...
        if (!NotifyAssociation(&handle_to_send, id))
        {
            ...
            control_message_proxy_.NotifyPeerEndpointClosed(id, handle_to_send.disconnect_reason());
        }
        return id;
    }

    mojo::ScopedInterfaceEndpointHandle
    CreateLocalEndpointHandle(mojo::InterfaceId id) override
    {
        ...
        if (!mojo::IsPrimaryInterfaceId(id)
            && set_interface_id_namespace_bit_ == mojo::HasInterfaceIdNamespaceBitSet(id))
        {
            return mojo::ScopedInterfaceEndpointHandle();
        }
    ...
    void
    CloseEndpointHandle(mojo::InterfaceId id,
    ...
    AttachEndpointClient(const mojo::ScopedInterfaceEndpointHandle& handle,
    ...
    void
    DetachEndpointClient(const mojo::ScopedInterfaceEndpointHandle& handle) override
    {
        const mojo::InterfaceId id = handle.id();
    ...
    bool
    PrefersSerializedMessages() override
    {
        return true;
    }
...
private:
    ...
    public:
        ...
        MessageWrapper(ChannelAssociatedGroupController* controller, mojo::Message message) :
    ...
    public:
        ...
        const base::Optional<mojo::DisconnectReason>&
        disconnect_reason() const
        {
            return disconnect_reason_;
        }

        void
        set_disconnect_reason(const base::Optional<mojo::DisconnectReason>& disconnect_reason)
        {
        ...
        void
        AttachClient(mojo::InterfaceEndpointClient* client,
        ...
        uint32_t
        EnqueueSyncMessage(MessageWrapper message)
        {
            controller_->lock_.AssertAcquired();
        ...
        // mojo::InterfaceEndpointController:
        bool
        SendMessage(mojo::Message* message) override
        {
            DCHECK(task_runner_->RunsTasksInCurrentSequence());
        ...
        void
        AllowWokenUpBySyncWatchOnSameThread() override
        {
            DCHECK(task_runner_->RunsTasksInCurrentSequence());
        ...
        bool
        SyncWatch(const bool& should_stop) override
        {
            DCHECK(task_runner_->RunsTasksInCurrentSequence());
        ...
        bool
        SyncWatchExclusive(uint64_t request_id) override
        {
        ...
        void
        RegisterExternalSyncWaiter(uint64_t request_id) override
        {
        }
    ...
    private:
        ...
        void
        OnSyncMessageEventReady()
        {
            DCHECK(task_runner_->RunsTasksInCurrentSequence());

            scoped_refptr<Endpoint> keepalive(this);
            scoped_refptr<AssociatedGroupController> controller_keepalive(controller_);
            base::AutoLock locker(controller_->lock_);
            bool more_to_process = false;
            if (!sync_messages_.empty())
            {
                MessageWrapper message_wrapper = std::move(sync_messages_.front().second);
                sync_messages_.pop();
                ...
                {
                    base::AutoUnlock unlocker(controller_->lock_);
                    dispatch_succeeded = client->HandleIncomingMessage(&message_wrapper.value());
                }
        ...
        EnsureSyncWatcherExists()
        {
            ...
            base::AutoLock locker(controller_->lock_);
            sync_watcher_ =
                    std::make_unique<mojo::SequenceLocalSyncEventWatcher>(base::BindRepeating(
                            &Endpoint::OnSyncMessageEventReady, base::Unretained(this)));
            if (peer_closed_ || !sync_messages_.empty())
                SignalSyncMessageEvent();
        }

        uint32_t
        GenerateSyncMessageId()
        {
            // Overflow is fine.
    ...
    {
    public:
        explicit ControlMessageProxyThunk(ChannelAssociatedGroupController* controller) :
    ...
    private:
        // MessageReceiver:
        bool
        Accept(mojo::Message* message) override
        {
            return controller_->SendMessage(message);
        }
    ...
    bool
    SendMessage(mojo::Message* message)
    {
        ...
        else
        {
            ...
            task_runner_->PostTask(
                    FROM_HERE,
                    base::BindOnce(&ChannelAssociatedGroupController::SendMessageOnPrimaryThread,
                                   this,
                                   std::move(*message)));
            return true;
        }
    ...
    void
    SendMessageOnPrimaryThread(mojo::Message message)
    {
        DCHECK(thread_checker_.CalledOnValidThread());
    ...
    void
    NotifyEndpointOfError(Endpoint* endpoint, bool force_async)
    {
        ...
        if (endpoint->task_runner()->RunsTasksInCurrentSequence() && !force_async)
        {
            ...
            base::Optional<mojo::DisconnectReason> reason(endpoint->disconnect_reason());
        ...
        else
        {
            endpoint->task_runner()->PostTask(
                    ...
                    base::BindOnce(&ChannelAssociatedGroupController::
                                   ...
                                   this,
                                   endpoint->id(),
                                   base::Unretained(endpoint)));
        }
    }

    void
    NotifyEndpointOfErrorOnEndpointThread(mojo::InterfaceId id, Endpoint* endpoint)
    {
        base::AutoLock locker(lock_);
    ...
    void
    MarkClosedAndMaybeRemove(Endpoint* endpoint)
    {
        lock_.AssertAcquired();
        endpoint->set_closed();
    ...
    void
    MarkPeerClosedAndMaybeRemove(Endpoint* endpoint)
    {
        lock_.AssertAcquired();
        endpoint->set_peer_closed();
    ...
    Endpoint*
    FindOrInsertEndpoint(mojo::InterfaceId id, bool* inserted)
    {
        lock_.AssertAcquired();
    ...
    Endpoint*
    FindEndpoint(mojo::InterfaceId id)
    {
        lock_.AssertAcquired();
        auto iter = endpoints_.find(id);
    ...
    Accept(mojo::Message* message) override
    {
        ...
        if (!client || !endpoint->task_runner()->RunsTasksInCurrentSequence())
        {
            ...
            if (message->has_flag(mojo::Message::kFlagIsSync))
            {
                ...
                uint32_t message_id = endpoint->EnqueueSyncMessage(std::move(message_wrapper));
    ...
    void
    AcceptOnProxyThread(mojo::Message message)
    {
    ...
    void
    AcceptSyncMessage(mojo::InterfaceId interface_id, uint32_t message_id)
    {
        DCHECK(proxy_task_runner_->BelongsToCurrentThread());
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("mojom"),
    ...
    bool
    OnPeerAssociatedEndpointClosed(mojo::InterfaceId id,
                                   const base::Optional<mojo::DisconnectReason>& reason) override
    {
        DCHECK(thread_checker_.CalledOnValidThread());
    ...
    bool
    WaitForFlushToComplete(mojo::ScopedMessagePipeHandle flush_pipe) override
    {
...
bool
ControllerMemoryDumpProvider::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args,
                                           base::trace_event::ProcessMemoryDump* pmd)
{
    ...
    for (auto* controller : controllers_)
    {
        ...
        dump->AddScalar(
                "top_message_count", base::trace_event::MemoryAllocatorDump::kUnitsObjects, count);
...
{
public:
    MojoBootstrapImpl(mojo::ScopedMessagePipeHandle handle,
                      const scoped_refptr<ChannelAssociatedGroupController> controller) :
        controller_(controller), associated_group_(controller), handle_(std::move(handle))
    {
    }
...
private:
    void
    Connect(mojo::AssociatedRemote<mojom::Channel>* sender,
...
// static
std::unique_ptr<MojoBootstrap>
MojoBootstrap::Create(mojo::ScopedMessagePipeHandle handle,
                      Channel::Mode mode,
                      const scoped_refptr<base::SingleThreadTaskRunner>& ipc_task_runner,
                      const scoped_refptr<base::SingleThreadTaskRunner>& proxy_task_runner,
                      const scoped_refptr<mojo::internal::MessageQuotaChecker>& quota_checker)
