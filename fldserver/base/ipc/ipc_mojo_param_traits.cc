...
#include "fldserver/base/ipc/ipc_mojo_message_helper.h"

namespace IPC
{
void
ParamTraits<mojo::MessagePipeHandle>::Write(base::Pickle* m, const param_type& p)
{
...
bool
ParamTraits<mojo::MessagePipeHandle>::Read(const base::Pickle* m,
...
void
ParamTraits<mojo::MessagePipeHandle>::Log(const param_type& p, std::string* l)
{
...
void
ParamTraits<mojo::DataPipeConsumerHandle>::Write(base::Pickle* m, const param_type& p)
{
    WriteParam(m, p.is_valid());
    if (!p.is_valid())
        return;
...
bool
ParamTraits<mojo::DataPipeConsumerHandle>::Read(const base::Pickle* m,
                                                base::PickleIterator* iter,
                                                param_type* r)
{
    bool is_valid;
...
void
ParamTraits<mojo::DataPipeConsumerHandle>::Log(const param_type& p, std::string* l)
{
