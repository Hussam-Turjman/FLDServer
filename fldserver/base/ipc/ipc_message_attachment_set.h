...
public:
    ...
    bool
    AddAttachment(scoped_refptr<MessageAttachment> attachment, size_t* index);

    // Similar to the above method, but without output variables.
    bool
    AddAttachment(scoped_refptr<MessageAttachment> attachment);
    ...
    GetAttachmentAt(unsigned index);
