...
{
unsigned
count_attachments_of_type(const std::vector<scoped_refptr<MessageAttachment>>& attachments,
...
MessageAttachmentSet::MessageAttachmentSet() : consumed_descriptor_highwater_(0)
{
}
...
unsigned
MessageAttachmentSet::num_descriptors() const
{
    return count_attachments_of_type(attachments_, MessageAttachment::Type::PLATFORM_FILE);
}

unsigned
MessageAttachmentSet::size() const
{
    return static_cast<unsigned>(attachments_.size());
}

bool
MessageAttachmentSet::AddAttachment(scoped_refptr<MessageAttachment> attachment, size_t* index)
{
...
bool
MessageAttachmentSet::AddAttachment(scoped_refptr<MessageAttachment> attachment)
{
    size_t index;
...
MessageAttachmentSet::GetAttachmentAt(unsigned index)
{
    if (index >= size())
    {
...
void
MessageAttachmentSet::CommitAllDescriptors()
{
    attachments_.clear();
