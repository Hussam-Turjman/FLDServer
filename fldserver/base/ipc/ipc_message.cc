...
void
Message::SetHeaderValues(int32_t routing, uint32_t type, uint32_t flags)
{
...
void
Message::EnsureMessageAttachmentSet()
{
...
#if BUILDFLAG(IPC_MESSAGE_LOG_ENABLED)
void
Message::set_sent_time(int64_t time)
{
...
void
Message::set_received_time(int64_t time) const
{
    received_time_ = time;
}
...
Message::NextMessageInfo::NextMessageInfo() :
    message_size(0), message_found(false), pickle_end(nullptr), message_end(nullptr)
{
}
...
// static
void
Message::FindNext(const char* range_start, const char* range_end, NextMessageInfo* info)
{
    DCHECK(info);
...
bool
Message::WriteAttachment(scoped_refptr<base::Pickle::Attachment> attachment)
{
    size_t index;
    bool success = attachment_set()->AddAttachment(
            base::WrapRefCounted(static_cast<MessageAttachment*>(attachment.get())), &index);
    DCHECK(success);
...
bool
Message::ReadAttachment(base::PickleIterator* iter,
