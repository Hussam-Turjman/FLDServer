...
#include "fldserver/buildflags.h"
...
public:
    ...
    bool
    should_unblock() const
    {
        return (header()->flags & UNBLOCK_BIT) != 0;
    }

    void
    set_dispatch_error() const
    {
        dispatch_error_ = true;
    }

    bool
    dispatch_error() const
    {
    ...
    // call.
    void
    SetHeaderValues(int32_t routing, uint32_t type, uint32_t flags);

    template <class T, class S, class P>
    static bool
    Dispatch(const Message* msg, T* obj, S* sender, P* parameter, void (T::*func)())
    {
        (obj->*func)();
        return true;
    }
    ...
    template <class T, class S, class P>
    static bool
    Dispatch(const Message* msg, T* obj, S* sender, P* parameter, void (T::*func)(P*))
    {
        (obj->*func)(parameter);
        return true;
    }
    ...
    void
    set_output_params(const std::string& op) const
    {
        output_params_ = op;
    }
    const std::string&
    ...
    void
    set_sync_log_data(LogData* data) const
    {
        log_data_ = data;
    }
    LogData*
