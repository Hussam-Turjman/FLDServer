...
#include "fldserver/base/containers/id_map.h"
...
public:
    ...
    virtual bool
    OnControlMessageReceived(const Message& msg);
    ...
    // that route id.
    virtual bool
    RouteMessage(const Message& msg);
    ...
    bool
    AddRoute(int32_t routing_id, Listener* listener);

    // Called to remove a listener for a particular message routing ID.
    void
    RemoveRoute(int32_t routing_id);

    // Returns the Listener associated with |routing_id|.
    Listener*
    GetRoute(int32_t routing_id);
