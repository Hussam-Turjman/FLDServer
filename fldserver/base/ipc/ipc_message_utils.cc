...
#include "fldserver/base/ipc/ipc_message_utils.h"
...
#include "fldserver/base/unguessable_token.h"
#include "fldserver/base/values.h"
#include "fldserver/fldserver_config.h"
#include "fldserver/base/ipc/ipc_channel_handle.h"
#include "fldserver/base/ipc/ipc_message_attachment.h"
#include "fldserver/base/ipc/ipc_message_attachment_set.h"
#include "fldserver/base/ipc/ipc_mojo_param_traits.h"

#include <stddef.h>
#include <stdint.h>

#if defined(OS_MAC)
#include "fldserver/base/ipc/mach_port_mac.h"
#endif
...
#include "fldserver/base/fuchsia/fuchsia_logging.h"
#include "fldserver/base/ipc/handle_attachment_fuchsia.h"
#endif

#if defined(OS_ANDROID)
#include "fldserver/base/android/scoped_hardware_buffer_handle.h"
...
void
LogBytes(const std::vector<CharType>& data, std::string* out)
{
#if IS_WIN
    ...
    {
        out->append(base::StringPrintf(" and %u more bytes",
...
bool
ReadValue(const base::Pickle* m, base::PickleIterator* iter, base::Value* value, int recursion);

void
WriteValue(base::Pickle* m, const base::Value* value, int recursion)
{
    bool result;
    ...
    switch (value->type())
    {
        case base::Value::Type::NONE: break;
...
// object.
bool
ReadDictionaryValue(const base::Pickle* m,
                    ...
                    int recursion)
{
    ...
    *value = base::DictionaryValue(base::Value::LegacyDictStorage(std::move(entries)));
    return true;
}
...
// object.
bool
ReadListValue(const base::Pickle* m,
              base::PickleIterator* iter,
...
ReadValue(const base::Pickle* m, base::PickleIterator* iter, base::Value* value, int recursion)
{
    ...
    {
        case base::Value::Type::NONE: *value = base::Value(); break;
        case base::Value::Type::BOOLEAN: {
            bool val;
...
LogData::LogData() : routing_id(0), type(0), sent(0), receive(0), dispatch(0)
{
}
...
void
ParamTraits<bool>::Log(const param_type& p, std::string* l)
{
    l->append(p ? "true" : "false");
}

void
ParamTraits<signed char>::Write(base::Pickle* m, const param_type& p)
{
    m->WriteBytes(&p, sizeof(param_type));
}

bool
ParamTraits<signed char>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    const char* data;
...
void
ParamTraits<signed char>::Log(const param_type& p, std::string* l)
{
    l->append(base::NumberToString(p));
}

void
ParamTraits<unsigned char>::Write(base::Pickle* m, const param_type& p)
{
    m->WriteBytes(&p, sizeof(param_type));
}

bool
ParamTraits<unsigned char>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    const char* data;
    if (!iter->ReadBytes(&data, sizeof(param_type)))
        return false;
...
void
ParamTraits<unsigned char>::Log(const param_type& p, std::string* l)
{
    l->append(base::NumberToString(p));
}

void
ParamTraits<unsigned short>::Write(base::Pickle* m, const param_type& p)
{
    m->WriteBytes(&p, sizeof(param_type));
}

bool
ParamTraits<unsigned short>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    const char* data;
    if (!iter->ReadBytes(&data, sizeof(param_type)))
        return false;
...
void
ParamTraits<unsigned short>::Log(const param_type& p, std::string* l)
{
    l->append(base::NumberToString(p));
}

void
ParamTraits<int>::Log(const param_type& p, std::string* l)
{
    l->append(base::NumberToString(p));
}

void
ParamTraits<unsigned int>::Log(const param_type& p, std::string* l)
{
    l->append(base::NumberToString(p));
}

#if IS_WIN || defined(OS_LINUX) || defined(OS_CHROMEOS) || defined(OS_FUCHSIA)            \
        || (defined(OS_ANDROID) && defined(ARCH_CPU_64_BITS))
void
ParamTraits<long>::Log(const param_type& p, std::string* l)
{
    l->append(base::NumberToString(p));
}

void
ParamTraits<unsigned long>::Log(const param_type& p, std::string* l)
{
    l->append(base::NumberToString(p));
}
#endif
...
void
ParamTraits<long long>::Log(const param_type& p, std::string* l)
{
    l->append(base::NumberToString(p));
}

void
ParamTraits<unsigned long long>::Log(const param_type& p, std::string* l)
{
    l->append(base::NumberToString(p));
}

void
ParamTraits<float>::Log(const param_type& p, std::string* l)
{
    l->append(base::StringPrintf("%e", p));
}

void
ParamTraits<double>::Write(base::Pickle* m, const param_type& p)
{
    m->WriteBytes(reinterpret_cast<const char*>(&p), sizeof(param_type));
}

bool
ParamTraits<double>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    const char* data;
...
void
ParamTraits<double>::Log(const param_type& p, std::string* l)
{
    l->append(base::StringPrintf("%e", p));
}

void
ParamTraits<std::string>::Log(const param_type& p, std::string* l)
{
    l->append(p);
}

void
ParamTraits<std::u16string>::Log(const param_type& p, std::string* l)
{
...
#if IS_WIN
bool
ParamTraits<std::wstring>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
...
void
ParamTraits<std::wstring>::Log(const param_type& p, std::string* l)
{
...
void
ParamTraits<std::vector<char>>::Write(base::Pickle* m, const param_type& p)
{
    if (p.empty())
    {
        m->WriteData(NULL, 0);
...
bool
ParamTraits<std::vector<char>>::Read(const base::Pickle* m,
                                     base::PickleIterator* iter,
                                     param_type* r)
{
    const char* data;
...
void
ParamTraits<std::vector<char>>::Log(const param_type& p, std::string* l)
{
    LogBytes(p, l);
}

void
ParamTraits<std::vector<unsigned char>>::Write(base::Pickle* m, const param_type& p)
{
    if (p.empty())
    {
        m->WriteData(NULL, 0);
    ...
    {
        m->WriteData(reinterpret_cast<const char*>(&p.front()), base::checked_cast<int>(p.size()));
    }
}

bool
ParamTraits<std::vector<unsigned char>>::Read(const base::Pickle* m,
                                              base::PickleIterator* iter,
                                              param_type* r)
{
    const char* data;
...
void
ParamTraits<std::vector<unsigned char>>::Log(const param_type& p, std::string* l)
{
    LogBytes(p, l);
}

void
ParamTraits<std::vector<bool>>::Write(base::Pickle* m, const param_type& p)
{
...
bool
ParamTraits<std::vector<bool>>::Read(const base::Pickle* m,
                                     base::PickleIterator* iter,
                                     param_type* r)
{
    int size;
...
void
ParamTraits<std::vector<bool>>::Log(const param_type& p, std::string* l)
{
    for (size_t i = 0; i < p.size(); ++i)
    {
        if (i != 0)
...
void
ParamTraits<base::DictionaryValue>::Write(base::Pickle* m, const param_type& p)
{
    WriteValue(m, &p, 0);
}

bool
ParamTraits<base::DictionaryValue>::Read(const base::Pickle* m,
                                         base::PickleIterator* iter,
                                         param_type* r)
{
    int type;
    if (!ReadParam(m, iter, &type) || type != static_cast<int>(base::Value::Type::DICTIONARY))
        return false;
...
void
ParamTraits<base::DictionaryValue>::Log(const param_type& p, std::string* l)
{
    std::string json;
...
void
ParamTraits<base::FileDescriptor>::Write(base::Pickle* m, const param_type& p)
{
    ...
    if (p.auto_close)
    {
        if (!m->WriteAttachment(new internal::PlatformFileAttachment(base::ScopedFD(p.fd))))
            NOTREACHED();
    }
    else
    {
...
bool
ParamTraits<base::FileDescriptor>::Read(const base::Pickle* m,
                                        base::PickleIterator* iter,
                                        param_type* r)
{
    ...
    *r = base::FileDescriptor(
            static_cast<internal::PlatformFileAttachment*>(attachment.get())->TakePlatformFile(),
            true);
    return true;
}

void
ParamTraits<base::FileDescriptor>::Log(const param_type& p, std::string* l)
{
    if (p.auto_close)
    {
...
void
ParamTraits<base::ScopedFD>::Write(base::Pickle* m, const param_type& p)
{
    // This serialization must be kept in sync with
    // nacl_message_scanner.cc:WriteHandle().
    ...
    if (!m->WriteAttachment(
                new internal::PlatformFileAttachment(std::move(const_cast<param_type&>(p)))))
    {
        NOTREACHED();
    }
}
...
bool
ParamTraits<base::ScopedFD>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    r->reset();
...
void
ParamTraits<base::ScopedFD>::Log(const param_type& p, std::string* l)
{
...
#if IS_WIN
void
ParamTraits<base::win::ScopedHandle>::Write(base::Pickle* m, const param_type& p)
{
...
bool
ParamTraits<base::win::ScopedHandle>::Read(const base::Pickle* m,
                                           base::PickleIterator* iter,
                                           param_type* r)
{
    r->Close();
...
void
ParamTraits<base::win::ScopedHandle>::Log(const param_type& p, std::string* l)
{
...
#if defined(OS_FUCHSIA)
void
ParamTraits<zx::vmo>::Write(base::Pickle* m, const param_type& p)
{
    // This serialization must be kept in sync with
    // nacl_message_scanner.cc:WriteHandle().
    const bool valid = p.is_valid();
    ...
    if (!m->WriteAttachment(
                new internal::HandleAttachmentFuchsia(std::move(const_cast<param_type&>(p)))))
    {
        NOTREACHED();
    }
}
...
bool
ParamTraits<zx::vmo>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    r->reset();
    ...
    *r = zx::vmo(static_cast<internal::HandleAttachmentFuchsia*>(attachment.get())->Take());
    return true;
}

void
ParamTraits<zx::vmo>::Log(const param_type& p, std::string* l)
{
    l->append("ZirconVMO");
}

void
ParamTraits<zx::channel>::Write(base::Pickle* m, const param_type& p)
{
    // This serialization must be kept in sync with
    // nacl_message_scanner.cc:WriteHandle().
    const bool valid = p.is_valid();
...
bool
ParamTraits<zx::channel>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    r->reset();
    ...
    *r = zx::channel(static_cast<internal::HandleAttachmentFuchsia*>(attachment.get())->Take());
    return true;
}

void
ParamTraits<zx::channel>::Log(const param_type& p, std::string* l)
{
...
#if defined(OS_ANDROID)
void
ParamTraits<base::android::ScopedHardwareBufferHandle>::Write(base::Pickle* m, const param_type& p)
{
    ...
    WriteParam(m,
               base::FileDescriptor(p.SerializeAsFileDescriptor().release(),
                                    true /* auto_close */));
...
bool
ParamTraits<base::android::ScopedHardwareBufferHandle>::Read(const base::Pickle* m,
                                                             base::PickleIterator* iter,
                                                             param_type* r)
{
    *r = base::android::ScopedHardwareBufferHandle();
    ...
    mojo::ScopedHandle tracking_pipe =
            static_cast<MessageAttachment*>(tracking_pipe_attachment.get())->TakeMojoHandle();
...
void
ParamTraits<base::android::ScopedHardwareBufferHandle>::Log(const param_type& p, std::string* l)
{
    l->append(base::StringPrintf("base::android::ScopedHardwareBufferHandle(%p)", p.get()));
}
#endif  // defined(OS_ANDROID)

void
ParamTraits<base::ReadOnlySharedMemoryRegion>::Write(base::Pickle* m, const param_type& p)
{
...
bool
ParamTraits<base::ReadOnlySharedMemoryRegion>::Read(const base::Pickle* m,
                                                    base::PickleIterator* iter,
                                                    param_type* r)
{
    base::subtle::PlatformSharedMemoryRegion handle;
...
void
ParamTraits<base::ReadOnlySharedMemoryRegion>::Log(const param_type& p, std::string* l)
{
...
void
ParamTraits<base::WritableSharedMemoryRegion>::Write(base::Pickle* m, const param_type& p)
{
    base::subtle::PlatformSharedMemoryRegion handle =
...
bool
ParamTraits<base::WritableSharedMemoryRegion>::Read(const base::Pickle* m,
                                                    base::PickleIterator* iter,
                                                    param_type* r)
{
    base::subtle::PlatformSharedMemoryRegion handle;
...
void
ParamTraits<base::WritableSharedMemoryRegion>::Log(const param_type& p, std::string* l)
{
...
void
ParamTraits<base::UnsafeSharedMemoryRegion>::Write(base::Pickle* m, const param_type& p)
{
    base::subtle::PlatformSharedMemoryRegion handle =
...
bool
ParamTraits<base::UnsafeSharedMemoryRegion>::Read(const base::Pickle* m,
                                                  base::PickleIterator* iter,
                                                  param_type* r)
{
    base::subtle::PlatformSharedMemoryRegion handle;
...
void
ParamTraits<base::UnsafeSharedMemoryRegion>::Log(const param_type& p, std::string* l)
{
...
void
ParamTraits<base::subtle::PlatformSharedMemoryRegion>::Write(base::Pickle* m, const param_type& p)
{
    // This serialization must be kept in sync with
    // nacl_message_scanner.cc::WriteHandle().
    const bool valid = p.IsValid();
    ...
    WriteParam(m, vmo);
#elif defined(OS_MAC)
    base::mac::ScopedMachSendRight h = const_cast<param_type&>(p).PassPlatformHandle();
...
#elif IS_POSIX
    base::subtle::ScopedFDPair h = const_cast<param_type&>(p).PassPlatformHandle();
    m->WriteAttachment(new internal::PlatformFileAttachment(std::move(h.fd)));
    if (p.GetMode() == base::subtle::PlatformSharedMemoryRegion::Mode::kWritable)
    {
        m->WriteAttachment(new internal::PlatformFileAttachment(std::move(h.readonly_fd)));
    }
#endif
}

bool
ParamTraits<base::subtle::PlatformSharedMemoryRegion>::Read(const base::Pickle* m,
                                                            base::PickleIterator* iter,
                                                            param_type* r)
{
    bool valid;
    ...
    if (!ReadParam(m, iter, &mode) || !ReadParam(m, iter, &shm_size)
        || !base::IsValueInRangeForNumericType<size_t>(shm_size) || !ReadParam(m, iter, &guid))
    {
        return false;
    }
    ...
    *r = base::subtle::PlatformSharedMemoryRegion::Take(
    ...
    if (!ReadParam(m, iter, &vmo))
        return false;
    *r = base::subtle::PlatformSharedMemoryRegion::Take(std::move(vmo), mode, size, guid);
#elif defined(OS_MAC)
    MachPortMac mach_port_mac;
    if (!ReadParam(m, iter, &mach_port_mac))
        return false;
    *r = base::subtle::PlatformSharedMemoryRegion::Take(
            base::mac::ScopedMachSendRight(mach_port_mac.get_mach_port()), mode, size, guid);
#elif IS_POSIX
    scoped_refptr<base::Pickle::Attachment> attachment;
    if (!m->ReadAttachment(iter, &attachment))
        return false;
...
#if defined(OS_ANDROID)
    *r = base::subtle::PlatformSharedMemoryRegion::Take(
            base::ScopedFD(static_cast<internal::PlatformFileAttachment*>(attachment.get())
                                   ->TakePlatformFile()),
            mode,
            size,
            guid);
...
void
ParamTraits<base::subtle::PlatformSharedMemoryRegion>::Log(const param_type& p, std::string* l)
{
#if defined(OS_FUCHSIA)
    l->append("Handle: ");
...
void
ParamTraits<base::subtle::PlatformSharedMemoryRegion::Mode>::Write(base::Pickle* m,
...
bool
ParamTraits<base::subtle::PlatformSharedMemoryRegion::Mode>::Read(const base::Pickle* m,
                                                                  base::PickleIterator* iter,
                                                                  param_type* p)
{
    int value;
...
void
ParamTraits<base::subtle::PlatformSharedMemoryRegion::Mode>::Log(const param_type& p,
                                                                 std::string* l)
{
...
#if IS_WIN
void
ParamTraits<PlatformFileForTransit>::Write(base::Pickle* m, const param_type& p)
{
...
bool
ParamTraits<PlatformFileForTransit>::Read(const base::Pickle* m,
                                          base::PickleIterator* iter,
                                          param_type* r)
{
    bool is_valid;
...
void
ParamTraits<PlatformFileForTransit>::Log(const param_type& p, std::string* l)
{
...
void
ParamTraits<base::FilePath>::Write(base::Pickle* m, const param_type& p)
{
    p.WriteToPickle(m);
}

bool
ParamTraits<base::FilePath>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    return r->ReadFromPickle(iter);
}

void
ParamTraits<base::FilePath>::Log(const param_type& p, std::string* l)
{
    ParamTraits<base::FilePath::StringType>::Log(p.value(), l);
}

void
ParamTraits<base::ListValue>::Write(base::Pickle* m, const param_type& p)
{
    WriteValue(m, &p, 0);
}

bool
ParamTraits<base::ListValue>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    int type;
    if (!ReadParam(m, iter, &type) || type != static_cast<int>(base::Value::Type::LIST))
        return false;
...
void
ParamTraits<base::ListValue>::Log(const param_type& p, std::string* l)
{
    std::string json;
    base::JSONWriter::Write(p, &json);
    l->append(json);
...
void
ParamTraits<base::Value>::Write(base::Pickle* m, const param_type& p)
{
    WriteValue(m, &p, 0);
}

bool
ParamTraits<base::Value>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    return ReadValue(m, iter, r, 0);
}

void
ParamTraits<base::Value>::Log(const param_type& p, std::string* l)
{
    std::string json;
    base::JSONWriter::Write(p, &json);
    l->append(json);
...
void
ParamTraits<base::File::Info>::Write(base::Pickle* m, const param_type& p)
{
    WriteParam(m, p.size);
...
bool
ParamTraits<base::File::Info>::Read(const base::Pickle* m,
                                    base::PickleIterator* iter,
                                    param_type* p)
{
    double last_modified, last_accessed, creation_time;
    if (!ReadParam(m, iter, &p->size) || !ReadParam(m, iter, &p->is_directory)
        || !ReadParam(m, iter, &last_modified) || !ReadParam(m, iter, &last_accessed)
...
void
ParamTraits<base::File::Info>::Log(const param_type& p, std::string* l)
{
    l->append("(");
    LogParam(p.size, l);
    l->append(",");
...
void
ParamTraits<base::Time>::Write(base::Pickle* m, const param_type& p)
{
    ParamTraits<int64_t>::Write(m, p.ToInternalValue());
}

bool
ParamTraits<base::Time>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    int64_t value;
...
void
ParamTraits<base::Time>::Log(const param_type& p, std::string* l)
{
    ParamTraits<int64_t>::Log(p.ToInternalValue(), l);
}

void
ParamTraits<base::TimeDelta>::Write(base::Pickle* m, const param_type& p)
{
    ParamTraits<int64_t>::Write(m, p.ToInternalValue());
}

bool
ParamTraits<base::TimeDelta>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    int64_t value;
...
void
ParamTraits<base::TimeDelta>::Log(const param_type& p, std::string* l)
{
    ParamTraits<int64_t>::Log(p.ToInternalValue(), l);
}

void
ParamTraits<base::TimeTicks>::Write(base::Pickle* m, const param_type& p)
{
    ParamTraits<int64_t>::Write(m, p.ToInternalValue());
}

bool
ParamTraits<base::TimeTicks>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    int64_t value;
    bool ret = ParamTraits<int64_t>::Read(m, iter, &value);
    if (ret)
...
void
ParamTraits<base::TimeTicks>::Log(const param_type& p, std::string* l)
{
    ParamTraits<int64_t>::Log(p.ToInternalValue(), l);
}
...
void
ParamTraits<base::UnguessableToken>::Write(base::Pickle* m, const param_type& p)
{
    DCHECK(!p.is_empty());
...
bool
ParamTraits<base::UnguessableToken>::Read(const base::Pickle* m,
                                          base::PickleIterator* iter,
                                          param_type* r)
{
    uint64_t high, low;
    if (!ParamTraits<uint64_t>::Read(m, iter, &high) || !ParamTraits<uint64_t>::Read(m, iter, &low))
        return false;
...
void
ParamTraits<base::UnguessableToken>::Log(const param_type& p, std::string* l)
{
    l->append(p.ToString());
}

void
ParamTraits<IPC::ChannelHandle>::Write(base::Pickle* m, const param_type& p)
{
...
bool
ParamTraits<IPC::ChannelHandle>::Read(const base::Pickle* m,
                                      base::PickleIterator* iter,
                                      param_type* r)
{
#if defined(OS_NACL_SFI)
...
void
ParamTraits<IPC::ChannelHandle>::Log(const param_type& p, std::string* l)
{
...
void
ParamTraits<LogData>::Write(base::Pickle* m, const param_type& p)
{
...
bool
ParamTraits<LogData>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    return ReadParam(m, iter, &r->channel) && ReadParam(m, iter, &r->routing_id)
           && ReadParam(m, iter, &r->type) && ReadParam(m, iter, &r->flags)
           && ReadParam(m, iter, &r->sent) && ReadParam(m, iter, &r->receive)
           && ReadParam(m, iter, &r->dispatch) && ReadParam(m, iter, &r->message_name)
           && ReadParam(m, iter, &r->params);
}

void
ParamTraits<LogData>::Log(const param_type& p, std::string* l)
{
    // Doesn't make sense to implement this!
}

void
ParamTraits<Message>::Write(base::Pickle* m, const Message& p)
{
#if IS_POSIX || defined(OS_FUCHSIA)
...
bool
ParamTraits<Message>::Read(const base::Pickle* m, base::PickleIterator* iter, Message* r)
{
...
void
ParamTraits<Message>::Log(const Message& p, std::string* l)
{
...
void
ParamTraits<HANDLE>::Write(base::Pickle* m, const param_type& p)
{
    m->WriteInt(HandleToLong(p));
}

bool
ParamTraits<HANDLE>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    int32_t temp;
...
void
ParamTraits<HANDLE>::Log(const param_type& p, std::string* l)
{
    l->append(base::StringPrintf("0x%p", p));
}

void
ParamTraits<MSG>::Write(base::Pickle* m, const param_type& p)
{
    m->WriteData(reinterpret_cast<const char*>(&p), sizeof(MSG));
}

bool
ParamTraits<MSG>::Read(const base::Pickle* m, base::PickleIterator* iter, param_type* r)
{
    const char* data;
    int data_size = 0;
...
void
ParamTraits<MSG>::Log(const param_type& p, std::string* l)
{
    l->append("<MSG>");
}
