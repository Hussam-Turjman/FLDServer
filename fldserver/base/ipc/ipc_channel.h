...
#include "fldserver/base/ipc/ipc.mojom-forward.h"
...
public:
    ...
    public:
        ...
        template <typename Interface>
        void
        AddAssociatedInterface(const AssociatedReceiverFactory<Interface>& factory)
        {
            AddGenericAssociatedInterface(
                    Interface::Name_,
                    base::BindRepeating(&BindPendingAssociatedReceiver<Interface>, factory));
        }

        // Template helper to request a remote associated interface.
        template <typename Interface>
        void
        GetRemoteAssociatedInterface(mojo::PendingAssociatedReceiver<Interface> receiver)
        {
    ...
    private:
        template <typename Interface>
        static void
        BindPendingAssociatedReceiver(const AssociatedReceiverFactory<Interface>& factory,
    ...
    Create(const IPC::ChannelHandle& channel_handle, Mode mode, Listener* listener);

    static std::unique_ptr<Channel>
    CreateClient(const IPC::ChannelHandle& channel_handle,
                 Listener* listener,
                 const scoped_refptr<base::SingleThreadTaskRunner>& ipc_task_runner);

    static std::unique_ptr<Channel>
    CreateServer(const IPC::ChannelHandle& channel_handle,
                 Listener* listener,
                 const scoped_refptr<base::SingleThreadTaskRunner>& ipc_task_runner);
    ...
    bool
    Send(Message* message) override = 0;
