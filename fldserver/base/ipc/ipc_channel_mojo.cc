...
#include "fldserver/base/ipc/ipc_channel_mojo.h"
...
#include "fldserver/base/ipc/native_handle_type_converters.h"
...
{
public:
    MojoChannelFactory(mojo::ScopedMessagePipeHandle handle,
                       Channel::Mode mode,
    ...
    BuildChannel(Listener* listener) override
    {
        return ChannelMojo::Create(std::move(handle_),
                                   mode_,
                                   listener,
                                   ipc_task_runner_,
                                   proxy_task_runner_,
    ...
    scoped_refptr<mojo::internal::MessageQuotaChecker>
    GetQuotaChecker() override
    {
...
public:
    ...
    ThreadSafeChannelProxy(scoped_refptr<base::SingleThreadTaskRunner> task_runner,
                           Forwarder forwarder,
    ...
    SendMessage(mojo::Message& message) override
    {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(forwarder_, std::move(message)));
    }

    void
    SendMessageWithResponder(mojo::Message& message,
...
// static
std::unique_ptr<ChannelMojo>
ChannelMojo::Create(mojo::ScopedMessagePipeHandle handle,
                    Mode mode,
                    Listener* listener,
                    const scoped_refptr<base::SingleThreadTaskRunner>& ipc_task_runner,
                    const scoped_refptr<base::SingleThreadTaskRunner>& proxy_task_runner,
...
{
    return base::WrapUnique(new ChannelMojo(
            std::move(handle), mode, listener, ipc_task_runner, proxy_task_runner, quota_checker));
}
        ...
        const scoped_refptr<base::SingleThreadTaskRunner>& proxy_task_runner)
{
    return std::make_unique<MojoChannelFactory>(
            std::move(handle), Channel::MODE_SERVER, ipc_task_runner, proxy_task_runner);
}
...
{
    return std::make_unique<MojoChannelFactory>(
            std::move(handle), Channel::MODE_CLIENT, ipc_task_runner, proxy_task_runner);
}
    ...
    task_runner_(ipc_task_runner), pipe_(handle.get()), listener_(listener)
{
    ...
    bootstrap_ = MojoBootstrap::Create(
            std::move(handle), mode, ipc_task_runner, proxy_task_runner, quota_checker);
}

void
ChannelMojo::ForwardMessage(mojo::Message message)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    if (!message_reader_ || !message_reader_->sender().is_bound())
        return;
    message_reader_->sender().internal_state()->ForwardMessage(std::move(message));
}
...
void
ChannelMojo::Unpause(bool flush)
{
    bootstrap_->Unpause();
    if (flush)
        Flush();
...
ChannelMojo::Close()
{
    ...
    std::unique_ptr<internal::MessagePipeReader> reader = std::move(message_reader_);
    reader.reset();
...
ChannelMojo::OnPipeError()
{
    ...
    {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelMojo::OnPipeError, weak_ptr_));
    }
}

void
ChannelMojo::OnAssociatedInterfaceRequest(const std::string& name,
...
bool
ChannelMojo::Send(Message* message)
{
...
void
ChannelMojo::OnPeerPidReceived(int32_t peer_pid)
{
    listener_->OnChannelConnected(peer_pid);
}

void
ChannelMojo::OnMessageReceived(const Message& message)
{
    const Message* message_ptr = &message;
    TRACE_IPC_MESSAGE_SEND("ipc,toplevel", "ChannelMojo::OnMessageReceived", message_ptr);
...
void
ChannelMojo::OnBrokenDataReceived()
{
    listener_->OnBadMessageReceived(Message());
}
...
ChannelMojo::ReadFromMessageAttachmentSet(
        Message* message, base::Optional<std::vector<mojo::native::SerializedHandlePtr>>* handles)
{
    DCHECK(!*handles);
    ...
    for (unsigned i = 0; result == MOJO_RESULT_OK && i < set->size(); ++i)
    {
        ...
        serialized_handle->type =
                mojo::ConvertTo<mojo::native::SerializedHandleType>(attachment->GetType());
...
ChannelMojo::WriteToMessageAttachmentSet(
        base::Optional<std::vector<mojo::native::SerializedHandlePtr>> handles, Message* message)
{
    if (!handles)
    ...
    for (size_t i = 0; i < handles->size(); ++i)
    {
        ...
        bool ok = message->attachment_set()->AddAttachment(std::move(unwrapped_attachment));
        DCHECK(ok);
        if (!ok)
        {
...
void
ChannelMojo::AddGenericAssociatedInterface(const std::string& name,
...
void
ChannelMojo::GetGenericRemoteAssociatedInterface(const std::string& name,
                                                 mojo::ScopedInterfaceEndpointHandle handle)
{
    if (message_reader_)
    {
