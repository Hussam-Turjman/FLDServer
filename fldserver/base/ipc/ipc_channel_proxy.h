...
#if 0
#include "fldserver/base/ipc/ipc.mojom.h"
...
public:
    ...
    static std::unique_ptr<ChannelProxy>
    Create(const IPC::ChannelHandle& channel_handle,
           Channel::Mode mode,
           Listener* listener,
    ...
    static std::unique_ptr<ChannelProxy>
    Create(std::unique_ptr<ChannelFactory> factory,
           Listener* listener,
           const scoped_refptr<base::SingleThreadTaskRunner>& ipc_task_runner,
           const scoped_refptr<base::SingleThreadTaskRunner>& listener_task_runner);
    ...
    // thread.
    void
    Init(const IPC::ChannelHandle& channel_handle, Channel::Mode mode, bool create_pipe_now);
    void
    Init(std::unique_ptr<ChannelFactory> factory, bool create_pipe_now);
    ...
    void
    AddGenericAssociatedInterfaceForIOThread(const std::string& name,
    ...
    template <typename Interface>
    void
    AddAssociatedInterfaceForIOThread(const AssociatedInterfaceFactory<Interface>& factory)
    {
        AddGenericAssociatedInterfaceForIOThread(
                Interface::Name_,
                base::BindRepeating(&ChannelProxy::BindPendingAssociatedReceiver<Interface>,
                                    factory));
    }

    // Requests an associated interface from the remote endpoint.
    void
    GetGenericRemoteAssociatedInterface(const std::string& name,
                                        mojo::ScopedInterfaceEndpointHandle handle);

    // Template helper to receive associated interfaces from the remote endpoint.
    template <typename Interface>
    void
    GetRemoteAssociatedInterface(mojo::AssociatedRemote<Interface>* proxy)
    {
        GetGenericRemoteAssociatedInterface(Interface::Name_,
                                            proxy->BindNewEndpointAndPassReceiver().PassHandle());
    }

#if defined(ENABLE_IPC_FUZZER)
    void
    set_outgoing_message_filter(OutgoingMessageFilter* filter)
    {
    ...
    template <typename Interface>
    void
    GetThreadSafeRemoteAssociatedInterface(
            scoped_refptr<mojo::SharedAssociatedRemote<Interface>>* out_remote)
    {
        ...
        *out_remote = mojo::SharedAssociatedRemote<Interface>::Create(std::move(pending_remote),
                                                                      ipc_task_runner());
    }
#endif
...
protected:
    ...
    public:
        ...
        void
        OnDispatchMessage(const Message& message);
        ...
        void
        AddListenerTaskRunner(int32_t routing_id,
                              scoped_refptr<base::SingleThreadTaskRunner> task_runner);

        // Removes task runner for |routing_id|.
        void
        RemoveListenerTaskRunner(int32_t routing_id);

        // Called on the IPC::Channel thread.
        // Returns the task runner associated with |routing_id|.
        scoped_refptr<base::SingleThreadTaskRunner>
        GetTaskRunner(int32_t routing_id);
    ...
    protected:
        ...
        void
        OnChannelConnected(int32_t peer_pid) override;
        void
        ...
        bool
        OnMessageReceivedNoFilter(const Message& message);

        // Gives the filters a chance at processing |message|.
        // Returns true if the message was processed, false otherwise.
        bool
        TryFilters(const Message& message);

        void
        PauseChannel();
        void
        UnpauseChannel(bool flush);
        void
        FlushChannel();
    ...
    private:
        ...
        // Create the Channel
        void
        CreateChannel(std::unique_ptr<ChannelFactory> factory);

        // Methods called on the IO thread.
        void
        OnSendMessage(std::unique_ptr<Message> message_ptr);
        void
        OnAddFilter();
        void
        OnRemoveFilter(MessageFilter* filter);
        ...
        OnDispatchError();
        void
        OnDispatchBadMessage(const Message& message);
#if 0
        void
        OnDispatchAssociatedInterfaceRequest(const std::string& interface_name,
                                             mojo::ScopedInterfaceEndpointHandle handle);
#endif
        void
        ClearChannel();
        ...
        std::map<int32_t, scoped_refptr<base::SingleThreadTaskRunner>>
                listener_thread_task_runners_ GUARDED_BY(listener_thread_task_runners_lock_);
        ...
        std::unique_ptr<mojo::ThreadSafeForwarder<mojom::Channel>> thread_safe_channel_;
...
private:
    ...
    template <typename Interface>
    static void
    BindPendingAssociatedReceiver(const AssociatedInterfaceFactory<Interface>& factory,
                                  mojo::ScopedInterfaceEndpointHandle handle)
    {
