...
public:
    ...
    public:
        virtual void
        OnPeerPidReceived(int32_t peer_pid) = 0;
        virtual void
        OnMessageReceived(const Message& message) = 0;
        virtual void
        OnBrokenDataReceived() = 0;
        virtual void
        OnPipeError() = 0;
        virtual void
    ...
    void
    GetRemoteInterface(const std::string& name, mojo::ScopedInterfaceEndpointHandle handle);

    mojo::AssociatedRemote<mojom::Channel>&
    sender()
    {
        return sender_;
    }
...
protected:
    ...
    OnPipeClosed();
    void
    OnPipeError(MojoResult error);

private:
    // mojom::Channel:
    void
    SetPeerPid(int32_t peer_pid) override;
    void
    Receive(MessageView message_view) override;
    void
    GetAssociatedInterface(
            const std::string& name,
            mojo::PendingAssociatedReceiver<mojom::GenericInterface> receiver) override;
