...
public:
    ...
    void
    set_listener(Listener* listener)
    {
        listener_ = listener;
    }
...
protected:
    ...
    virtual ReadState
    ReadData(char* buffer, int buffer_len, int* bytes_read) = 0;
    ...
    // pipe if necessary.
    virtual bool
    ShouldDispatchInputMessage(Message* msg) = 0;
...
private:
  /*  FRIEND_TEST_ALL_PREFIXES(ChannelReaderTest, AttachmentAlreadyBrokered);
    ...
    bool
    TranslateInputData(const char* input_data, int input_data_len);
    ...
    // Returns |false| on unrecoverable error.
    bool
    HandleTranslatedMessage(Message* translated_message);
