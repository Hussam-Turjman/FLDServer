...
#include "fldserver/base/ipc/ipc_sync_channel.h"
...
#include "fldserver/base/trace_event/trace_event.h"
...
#include "fldserver/base/ipc/trace_ipc_message.h"
#endif
...
public:
    ...
    void
    BlockDispatch(bool* dispatch_flag)
    {
        dispatch_flag_ = dispatch_flag;
    }
    ...
    UnblockDispatch()
    {
        dispatch_flag_ = nullptr;
    }

    // Called on IPC thread when a synchronous message or reply arrives.
    void
    QueueMessage(const Message& msg, SyncChannel::SyncContext* context)
    {
        ...
        if (!was_task_pending)
        {
            listener_task_runner_->PostTask(
                    FROM_HERE,
                    base::BindOnce(&ReceivedSyncMsgQueue::DispatchMessagesTask,
                                   this,
    ...
    void
    QueueReply(const Message& msg, SyncChannel::SyncContext* context)
    {
    ...
    // messages.
    void
    DispatchMessagesTask(SyncContext* context)
    {
        {
            base::AutoLock auto_lock(message_lock_);
            task_pending_ = false;
    ...
    void
    DispatchMessages(SyncContext* dispatching_context)
    {
    ...
    void
    RemoveContext(SyncContext* context)
    {
        base::AutoLock auto_lock(message_lock_);
    ...
    static base::LazyInstance<base::ThreadLocalPointer<ReceivedSyncMsgQueue>>::DestructorAtExit
            lazy_tls_ptr_;
...
base::LazyInstance<base::ThreadLocalPointer<SyncChannel::ReceivedSyncMsgQueue>>::DestructorAtExit
...
void
SyncChannel::SyncContext::OnSendDoneEventSignaled(base::RunLoop* nested_loop,
...
bool
SyncChannel::SyncContext::Push(SyncMessage* sync_msg)
{
...
SyncChannel::SyncContext::Pop()
{
    ...
    ipc_task_runner()->PostTask(
            FROM_HERE, base::BindOnce(&ReceivedSyncMsgQueue::DispatchReplies, received_sync_msgs_));
...
void
SyncChannel::SyncContext::DispatchMessages()
{
    received_sync_msgs_->DispatchMessages(this);
}

bool
SyncChannel::SyncContext::TryToUnblockListener(const Message* msg)
{
    base::AutoLock auto_lock(deserializers_lock_);
...
bool
SyncChannel::SyncContext::OnMessageReceived(const Message& msg)
{
...
void
SyncChannel::SyncContext::OnChannelError()
{
    CancelPendingSends();
...
void
SyncChannel::SyncContext::OnChannelOpened()
{
    if (shutdown_event_)
    {
...
void
SyncChannel::SyncContext::OnChannelClosed()
{
    CancelPendingSends();
    shutdown_watcher_.StopWatching();
...
void
SyncChannel::SyncContext::OnShutdownEventSignaled(WaitableEvent* event)
{
...
// static
std::unique_ptr<SyncChannel>
SyncChannel::Create(const IPC::ChannelHandle& channel_handle,
                    Channel::Mode mode,
                    Listener* listener,
                    const scoped_refptr<base::SingleThreadTaskRunner>& ipc_task_runner,
                    const scoped_refptr<base::SingleThreadTaskRunner>& listener_task_runner,
...
// static
std::unique_ptr<SyncChannel>
SyncChannel::Create(Listener* listener,
                    const scoped_refptr<base::SingleThreadTaskRunner>& ipc_task_runner,
                    const scoped_refptr<base::SingleThreadTaskRunner>& listener_task_runner,
                    WaitableEvent* shutdown_event)
{
    return base::WrapUnique(
            new SyncChannel(listener, ipc_task_runner, listener_task_runner, shutdown_event));
}

SyncChannel::SyncChannel(Listener* listener,
                         const scoped_refptr<base::SingleThreadTaskRunner>& ipc_task_runner,
                         const scoped_refptr<base::SingleThreadTaskRunner>& listener_task_runner,
                         WaitableEvent* shutdown_event) :
    ChannelProxy(new SyncContext(listener, ipc_task_runner, listener_task_runner, shutdown_event)),
...
void
SyncChannel::AddListenerTaskRunner(int32_t routing_id,
...
void
SyncChannel::RemoveListenerTaskRunner(int32_t routing_id)
{
...
void
SyncChannel::SetRestrictDispatchChannelGroup(int group)
{
    sync_context()->set_restrict_dispatch_group(group);
}

scoped_refptr<SyncMessageFilter>
SyncChannel::CreateSyncMessageFilter()
{
    scoped_refptr<SyncMessageFilter> filter =
            new SyncMessageFilter(sync_context()->shutdown_event());
...
bool
SyncChannel::Send(Message* message)
{
#if BUILDFLAG(IPC_MESSAGE_LOG_ENABLED)
    std::string name;
    Logging::GetInstance()->GetMessageText(message->type(), &name, message, nullptr);
...
void
SyncChannel::WaitForReply(mojo::SyncHandleRegistry* registry, SyncContext* context)
{
    context->DispatchMessages();
    ...
    while (true)
    {
        bool dispatch = false;
        {
            ...
            mojo::SyncHandleRegistry::EventCallbackSubscription send_done_subscription =
                    registry->RegisterEvent(context->GetSendDoneEvent(),
...
void
SyncChannel::OnDispatchEventSignaled(base::WaitableEvent* event)
{
