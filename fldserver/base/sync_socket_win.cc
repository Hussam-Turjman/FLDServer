...
#include "fldserver/base/sync_socket.h"
...
const wchar_t kPipeNameFormat[] = L"\\\\.\\pipe\\blitz.sync.%u.%u.%lu";
...
bool
CreatePairImpl(ScopedHandle* socket_a, ScopedHandle* socket_b, bool overlapped)
{
    ...
    do
    {
        ...
        handle_a.Set(CreateNamedPipeW(name,
                                      flags,
...
DWORD
GetNextChunkSize(size_t current_pos, size_t max_size)
{
    // The following statement is for 64 bit portability.
    return static_cast<DWORD>(((max_size - current_pos) <= UINT_MAX) ? (max_size - current_pos)
                                                                     : UINT_MAX);
}
...
size_t
CancelableFileOperation(Function operation,
                        HANDLE file,
                        BufferType* buffer,
                        size_t length,
                        ...
                        DWORD timeout_in_ms)
{
    ...
    do
    {
        ...
        const BOOL operation_ok =
                operation(file, static_cast<BufferType*>(buffer) + count, chunk, &len, &ol);
        if (!operation_ok)
        {
...
// static
bool
SyncSocket::CreatePair(SyncSocket* socket_a, SyncSocket* socket_b)
{
...
size_t
SyncSocket::Send(const void* buffer, size_t length)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    DCHECK_GT(length, 0u);
    DCHECK_LE(length, kMaxMessageLength);
    ...
    while (count < length)
    {
        ...
        if (::WriteFile(handle(), static_cast<const char*>(buffer) + count, chunk, &len, NULL)
            == FALSE)
        {
            return count;
        }
...
size_t
SyncSocket::ReceiveWithTimeout(void* buffer, size_t length, TimeDelta timeout)
{
    NOTIMPLEMENTED();
    return 0;
}
...
size_t
SyncSocket::Receive(void* buffer, size_t length)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    DCHECK_GT(length, 0u);
    DCHECK_LE(length, kMaxMessageLength);
    ...
    while (count < length)
    {
        DWORD len;
        DWORD chunk = GetNextChunkSize(count, length);
        if (::ReadFile(handle(), static_cast<char*>(buffer) + count, chunk, &len, NULL) == FALSE)
        {
            return count;
        }
        count += len;
...
bool
SyncSocket::IsValid() const
{
    return handle_.IsValid();
}

SyncSocket::Handle
SyncSocket::handle() const
{
    return handle_.Get();
}
...
size_t
CancelableSyncSocket::Send(const void* buffer, size_t length)
{
    static const DWORD kWaitTimeOutInMs = 500;
    return CancelableFileOperation(&::WriteFile,
                                   handle(),
...
size_t
CancelableSyncSocket::Receive(void* buffer, size_t length)
{
    return CancelableFileOperation(&::ReadFile,
                                   handle(),
...
size_t
CancelableSyncSocket::ReceiveWithTimeout(void* buffer, size_t length, TimeDelta timeout)
{
    return CancelableFileOperation(&::ReadFile,
                                   handle(),
                                   reinterpret_cast<char*>(buffer),
...
HANDLE
CancelableSyncSocket::file_operation_event_handle() const
{
    return file_operation_.handle();
}
HANDLE
CancelableSyncSocket::shutdown_event_handle() const
{
    return shutdown_event_.handle();
}

// static
bool
CancelableSyncSocket::CreatePair(CancelableSyncSocket* socket_a, CancelableSyncSocket* socket_b)
{
