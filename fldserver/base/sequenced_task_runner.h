...
#include "fldserver/base/sequenced_task_runner_helpers.h"
...
class CORE_EXPORT SequencedTaskRunner : public TaskRunner
{
public:
    ...
    bool
    PostNonNestableTask(const Location& from_here, OnceClosure task);
    ...
    template <class T>
    bool
    DeleteSoon(const Location& from_here, const T* object)
    {
        return DeleteOrReleaseSoonInternal(from_here, &DeleteHelper<T>::DoDelete, object);
    }

    template <class T>
    bool
    DeleteSoon(const Location& from_here, std::unique_ptr<T> object)
    {
    ...
    template <class T>
    void
    ReleaseSoon(const Location& from_here, scoped_refptr<T>&& object)
    {
        if (!object)
            return;

        DeleteOrReleaseSoonInternal(from_here, &ReleaseHelper<T>::DoRelease, object.release());
    }
    ...
    //   the current thread.
    virtual bool
    RunsTasksInCurrentSequence() const = 0;
...
private:
    bool
    DeleteOrReleaseSoonInternal(const Location& from_here,
...
struct CORE_EXPORT OnTaskRunnerDeleter
{
