...
#include "fldserver/base/numerics/safe_math_shared_impl.h"
        ...
        typename std::enable_if<std::is_integral<T>::value && std::is_integral<U>::value>::type>
{
    ...
    Do(T x, U y, V* result)
    {
        ...
        using Promotion = typename std::conditional<(IntegerBitsPlusSign<FastPromotion>::value
        ...
        typename std::enable_if<std::is_arithmetic<T>::value && std::is_arithmetic<U>::value>::type>
{
    ...
    Do(T x, U y, V* result)
    {
        const result_type tmp = IsGreater<T, U>::Test(x, y) ? static_cast<result_type>(x)
        ...
        typename std::enable_if<std::is_arithmetic<T>::value && std::is_arithmetic<U>::value>::type>
{
    ...
    Do(T x, U y, V* result)
    {
        const result_type tmp = IsLess<T, U>::Test(x, y) ? static_cast<result_type>(x)
                                                         : static_cast<result_type>(y);
        if (!IsValueInRangeForNumericType<V>(tmp))
            return false;
        *result = static_cast<V>(tmp);
...
{
    static const NumericRepresentation value = std::is_integral<NumericType>::value
                                                       ? NUMERIC_INTEGER
...
private:
    ...
    template <typename Src>
    static constexpr T
    WellDefinedConversionOrZero(Src value, bool is_valid)
    {
...
public:
    ...
    constexpr explicit CheckedNumericState(Src value = 0.0, bool is_valid = true) :
        value_(WellDefinedConversionOrNaN(value,
...
private:
    ...
    template <typename Src>
    static constexpr T
    WellDefinedConversionOrNaN(Src value, bool is_valid)
    {
        using SrcType = typename internal::UnderlyingType<Src>::type;
