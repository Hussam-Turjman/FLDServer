...
size_t
IterateAndEraseIf(Container& container, Predicate pred)
{
...
constexpr auto
size(const Container& c) -> decltype(c.size())
{
    return c.size();
}
...
template <typename Container>
constexpr auto
empty(const Container& c) -> decltype(c.empty())
{
    return c.empty();
}
...
template <typename T>
constexpr bool
empty(std::initializer_list<T> il) noexcept
{
    return il.size() == 0;
}
...
template <typename Container>
constexpr auto
data(Container& c) -> decltype(c.data())
{
    return c.data();
}
...
CharT*
data(std::basic_string<CharT, Traits, Allocator>& str)
{
...
template <typename Container>
constexpr auto
data(const Container& c) -> decltype(c.data())
{
    return c.data();
}
...
template <typename T>
constexpr const T*
data(std::initializer_list<T> il) noexcept
{
    return il.begin();
}
...
template <typename T, size_t N>
constexpr T*
data(std::array<T, N>& array) noexcept
{
...
template <typename T, size_t N>
constexpr const T*
data(const std::array<T, N>& array) noexcept
{
    return !array.empty() ? &array[0] : nullptr;
}
...
template <class A>
const typename A::container_type&
GetUnderlyingContainer(const A& adapter)
{
...
constexpr auto
ConstCastIterator(Container& c, ConstIter it)
{
...
constexpr auto
ConstCastIterator(std::forward_list<T, Allocator>& c,
...
InsertOrAssignImpl(Map& map, Key&& key, Value&& value)
{
    ...
    return {map.emplace_hint(lower, std::forward<Key>(key), std::forward<Value>(value)), true};
}

template <typename Map, typename Key, typename Value>
typename Map::iterator
InsertOrAssignImpl(Map& map, typename Map::const_iterator hint, Key&& key, Value&& value)
{
    ...
    if ((hint == map.begin() || key_comp(std::prev(hint)->first, key)))
    {
        if (hint == map.end() || key_comp(key, hint->first))
        {
            ...
            return map.emplace_hint(hint, std::forward<Key>(key), std::forward<Value>(value));
        }
    ...
    return InsertOrAssignImpl(map, std::forward<Key>(key), std::forward<Value>(value)).first;
}

template <typename Map, typename Key, typename... Args>
std::pair<typename Map::iterator, bool>
TryEmplaceImpl(Map& map, Key&& key, Args&&... args)
{
    auto lower = map.lower_bound(key);
    if (lower != map.end() && !map.key_comp()(key, lower->first))
    {
...
template <typename Map, typename Key, typename... Args>
typename Map::iterator
TryEmplaceImpl(Map& map, typename Map::const_iterator hint, Key&& key, Args&&... args)
{
    auto&& key_comp = map.key_comp();
    if ((hint == map.begin() || key_comp(std::prev(hint)->first, key)))
    {
    ...
    // hint was not helpful, dispatch to hintless version.
    return TryEmplaceImpl(map, std::forward<Key>(key), std::forward<Args>(args)...).first;
}
...
{
    return internal::InsertOrAssignImpl(map, std::move(key), std::forward<Value>(value));
}
...
template <typename Map, typename Value>
typename Map::iterator
InsertOrAssign(Map& map,
               typename Map::const_iterator hint,
               const typename Map::key_type& key,
               Value&& value)
{
    return internal::InsertOrAssignImpl(map, hint, key, std::forward<Value>(value));
}
...
InsertOrAssign(Map& map,
               ...
               Value&& value)
{
    return internal::InsertOrAssignImpl(map, hint, std::move(key), std::forward<Value>(value));
}
...
template <typename Map, typename... Args>
std::pair<typename Map::iterator, bool>
TryEmplace(Map& map, typename Map::key_type&& key, Args&&... args)
{
    return internal::TryEmplaceImpl(map, std::move(key), std::forward<Args>(args)...);
}
...
TryEmplace(Map& map,
           ...
           Args&&... args)
{
    return internal::TryEmplaceImpl(map, hint, std::move(key), std::forward<Args>(args)...);
}

// Returns a new ResultType containing the difference of two sorted containers.
template <typename ResultType, typename Arg1, typename Arg2>
ResultType
STLSetDifference(const Arg1& a1, const Arg2& a2)
{
...
template <typename ResultType, typename Arg1, typename Arg2>
ResultType
STLSetUnion(const Arg1& a1, const Arg2& a2)
{
    DCHECK(ranges::is_sorted(a1));
    DCHECK(ranges::is_sorted(a2));
    ResultType result;
...
template <typename ResultType, typename Arg1, typename Arg2>
ResultType
STLSetIntersection(const Arg1& a1, const Arg2& a2)
{
    DCHECK(ranges::is_sorted(a1));
    DCHECK(ranges::is_sorted(a2));
    ResultType result;
    std::set_intersection(
            a1.begin(), a1.end(), a2.begin(), a2.end(), std::inserter(result, result.end()));
    return result;
}
...
size_t
Erase(std::basic_string<CharT, Traits, Allocator>& container, const Value& value)
{
...
size_t
EraseIf(std::basic_string<CharT, Traits, Allocator>& container, Predicate pred)
{
...
size_t
Erase(std::deque<T, Allocator>& container, const Value& value)
{
    auto it = std::remove(container.begin(), container.end(), value);
    size_t removed = std::distance(it, container.end());
    container.erase(it, container.end());
...
size_t
EraseIf(std::deque<T, Allocator>& container, Predicate pred)
{
    auto it = std::remove_if(container.begin(), container.end(), pred);
    size_t removed = std::distance(it, container.end());
    container.erase(it, container.end());
...
template <class T, class Allocator, class Value>
size_t
Erase(std::vector<T, Allocator>& container, const Value& value)
{
    auto it = std::remove(container.begin(), container.end(), value);
    size_t removed = std::distance(it, container.end());
    container.erase(it, container.end());
...
template <class T, class Allocator, class Predicate>
size_t
EraseIf(std::vector<T, Allocator>& container, Predicate pred)
{
    auto it = std::remove_if(container.begin(), container.end(), pred);
    size_t removed = std::distance(it, container.end());
    container.erase(it, container.end());
...
template <class T, class Allocator, class Predicate>
size_t
EraseIf(std::forward_list<T, Allocator>& container, Predicate pred)
{
...
template <class T, class Allocator, class Predicate>
size_t
EraseIf(std::list<T, Allocator>& container, Predicate pred)
{
    size_t old_size = container.size();
    container.remove_if(pred);
    return old_size - container.size();
...
size_t
EraseIf(std::map<Key, T, Compare, Allocator>& container, Predicate pred)
{
...
template <class Key, class T, class Compare, class Allocator, class Predicate>
size_t
EraseIf(std::multimap<Key, T, Compare, Allocator>& container, Predicate pred)
{
    return internal::IterateAndEraseIf(container, pred);
}
...
size_t
EraseIf(std::set<Key, Compare, Allocator>& container, Predicate pred)
{
    return internal::IterateAndEraseIf(container, pred);
}
...
template <class Key, class Compare, class Allocator, class Predicate>
size_t
EraseIf(std::multiset<Key, Compare, Allocator>& container, Predicate pred)
{
    return internal::IterateAndEraseIf(container, pred);
}
...
size_t
EraseIf(std::unordered_map<Key, T, Hash, KeyEqual, Allocator>& container, Predicate pred)
{
    return internal::IterateAndEraseIf(container, pred);
}
...
template <class Key, class T, class Hash, class KeyEqual, class Allocator, class Predicate>
size_t
EraseIf(std::unordered_multimap<Key, T, Hash, KeyEqual, Allocator>& container, Predicate pred)
{
    return internal::IterateAndEraseIf(container, pred);
}
...
size_t
EraseIf(std::unordered_set<Key, Hash, KeyEqual, Allocator>& container, Predicate pred)
{
    return internal::IterateAndEraseIf(container, pred);
}
...
template <class Key, class Hash, class KeyEqual, class Allocator, class Predicate>
size_t
EraseIf(std::unordered_multiset<Key, Hash, KeyEqual, Allocator>& container, Predicate pred)
{
    return internal::IterateAndEraseIf(container, pred);
}
...
template <class T, class Allocator, class Value>
size_t
Erase(std::forward_list<T, Allocator>& container, const Value& value)
{
...
template <class T, class Allocator, class Value>
size_t
Erase(std::list<T, Allocator>& container, const Value& value)
{
...
{
public:
    explicit IsNotIn(const Collection& collection) : i_(collection.begin()), end_(collection.end())
    {
    }

    bool
    operator()(const typename Collection::value_type& x)
    {
        while (i_ != end_ && *i_ < x)
            ++i_;
        if (i_ == end_)
...
template <class T>
T*
OptionalOrNullptr(base::Optional<T>& optional)
{
...
template <class T>
const T*
OptionalOrNullptr(const base::Optional<T>& optional)
{
    return optional.has_value() ? &optional.value() : nullptr;
}
