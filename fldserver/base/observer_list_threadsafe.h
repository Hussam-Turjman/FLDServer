...
namespace internal
{
class CORE_EXPORT ObserverListThreadSafeBase
...
public:
    ...
    operator=(const ObserverListThreadSafeBase&) = delete;
...
protected:
    ...
    {
        static void
        Run(void (ReceiverType::*m)(Params...), Params... params, ObserverType* obj)
        {
...
public:
    ...
    explicit ObserverListThreadSafe(ObserverListPolicy policy) : policy_(policy)
    {
    }
    ObserverListThreadSafe(const ObserverListThreadSafe&) = delete;
    ObserverListThreadSafe&
    operator=(const ObserverListThreadSafe&) = delete;
    ...
    AddObserver(ObserverType* observer)
    {
        ...
        const scoped_refptr<SequencedTaskRunner> task_runner = SequencedTaskRunnerHandle::Get();
        ...
        if (policy_ == ObserverListPolicy::ALL)
        {
            ...
            if (current_notification && current_notification->observer_list == this)
            {
                ...
                task_runner->PostTask(current_notification->from_here,
    ...
    void
    Notify(const Location& from_here, Method m, Params&&... params)
    {
        RepeatingCallback<void(ObserverType*)> method = BindRepeating(
                &Dispatcher<ObserverType, Method>::Run, m, std::forward<Params>(params)...);
        ...
        for (const auto& observer : observers_)
        {
            observer.second.task_runner->PostTask(
                    ...
                    BindOnce(&ObserverListThreadSafe<ObserverType>::NotifyWrapper,
                             ...
                             observer.first,
                             NotificationData(
                                     this, observer.second.observer_id, from_here, method)));
        }
    }
...
private:
    ...
    void
    NotifyWrapper(ObserverType* observer, const NotificationData& notification)
    {
        {
            AutoLock auto_lock(lock_);
            ...
            if (it == observers_.end() || it->second.observer_id != notification.observer_id)
            {
                return;
            }
        ...
        const NotificationDataBase* const previous_notification = tls_current_notification.Get();
    ...
    // be notified.
    std::unordered_map<ObserverType*, ObserverTaskRunnerInfo> observers_ GUARDED_BY(lock_);
};
