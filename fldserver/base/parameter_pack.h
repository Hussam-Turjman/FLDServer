...
any_of(std::initializer_list<bool> ilist)
{
    for (auto c : ilist)
    {
        if (c)
...
inline constexpr bool
all_of(std::initializer_list<bool> ilist)
{
    for (auto c : ilist)
    {
        if (!c)
...
count(std::initializer_list<T> ilist, T value)
{
    size_t c = 0;
...
struct ParameterPack
{
    ...
    template <typename Type>
    using IsUniqueInPack = bool_constant<count({std::is_same<Type, Ts>::value...}, true) == 1>;
    ...
    using IsAllSameType = bool_constant<all_of({std::is_same<NthType<0>, Ts>::value...})>;
};
