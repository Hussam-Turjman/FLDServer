...
// static
void
ThreadCheckerImpl::EnableStackLogging()
{
...
bool
ThreadCheckerImpl::CalledOnValidThread(std::unique_ptr<debug::StackTrace>* out_bound_at) const
{
...
std::unique_ptr<debug::StackTrace>
ThreadCheckerImpl::GetBoundAt() const
{
    AutoLock auto_lock(lock_);
    if (!bound_at_)
        return nullptr;
...
void
ThreadCheckerImpl::EnsureAssignedLockRequired() const
{
