...
{
    return Singleton<ThreadIdNameManager, LeakySingletonTraits<ThreadIdNameManager>>::get();
}

const char*
ThreadIdNameManager::GetDefaultInternedString()
{
    return g_default_name->c_str();
}

void
ThreadIdNameManager::RegisterThread(PlatformThreadHandle::Handle handle, PlatformThreadId id)
{
    AutoLock locked(lock_);
...
void
ThreadIdNameManager::AddObserver(Observer* obs)
{
    AutoLock locked(lock_);
...
void
ThreadIdNameManager::RemoveObserver(Observer* obs)
{
    AutoLock locked(lock_);
    DCHECK(base::Contains(observers_, obs));
...
void
ThreadIdNameManager::SetName(const std::string& name)
{
...
const char*
ThreadIdNameManager::GetName(PlatformThreadId id)
{
    AutoLock locked(lock_);
    ...
    auto handle_to_name_iter = thread_handle_to_interned_name_.find(id_to_handle_iter->second);
    return handle_to_name_iter->second->c_str();
}

const char*
ThreadIdNameManager::GetNameForCurrentThread()
{
...
void
ThreadIdNameManager::RemoveName(PlatformThreadHandle::Handle handle, PlatformThreadId id)
{
    AutoLock locked(lock_);
