...
class LOCKABLE CORE_EXPORT ThreadCheckerImpl
{
public:
    static void
    EnableStackLogging();
    ...
    ThreadCheckerImpl&
    operator=(ThreadCheckerImpl&& other);
    ...
    bool
    CalledOnValidThread(std::unique_ptr<debug::StackTrace>* out_bound_at = nullptr) const
            WARN_UNUSED_RESULT;
...
private:
    void
    EnsureAssignedLockRequired() const EXCLUSIVE_LOCKS_REQUIRED(lock_);
