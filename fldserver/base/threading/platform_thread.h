...
public:
    ...
    bool
    operator==(PlatformThreadRef other) const
    {
...
public:
    ...
    bool
    is_equal(const PlatformThreadHandle& other) const
    {
...
class CORE_EXPORT PlatformThread
{
public:
    ...
    static bool
    Create(size_t stack_size, Delegate* delegate, PlatformThreadHandle* thread_handle)
    {
        return CreateWithPriority(stack_size, delegate, thread_handle, ThreadPriority::NORMAL);
    }

    // CreateWithPriority() does the same thing as Create() except the priority of
    // the thread is set based on |priority|.
    static bool
    CreateWithPriority(size_t stack_size,
                       Delegate* delegate,
    ...
    static bool
    CreateNonJoinable(size_t stack_size, Delegate* delegate);
    ...
    static bool
    CreateNonJoinableWithPriority(size_t stack_size, Delegate* delegate, ThreadPriority priority);

    // Joins with a thread created via the Create function.  This function blocks
    // the caller until the designated thread exits.  This will invalidate
    // |thread_handle|.
    static void
    Join(PlatformThreadHandle thread_handle);

    // Detaches and releases the thread handle. The thread is no longer joinable
    // and |thread_handle| is invalidated after this call.
    static void
    Detach(PlatformThreadHandle thread_handle);

    // Returns true if SetCurrentThreadPriority() should be able to increase the
    // priority of a thread to |priority|.
    static bool
    CanIncreaseThreadPriority(ThreadPriority priority);
    ...
    static void
    SetCurrentThreadPriority(ThreadPriority priority);
    ...
    static TimeDelta
    GetRealtimePeriod(Delegate* delegate);
    ...
    static void
    SetThreadPriority(PlatformThreadId process_id,
    ...
    // feature status.
    static void
    InitializeOptimizedRealtimeThreadingFeature();

    // Stores the period value in TLS.
    static void
    SetCurrentThreadRealtimePeriodValue(TimeDelta realtime_period);
#endif

private:
    static void
    SetCurrentThreadPriorityImpl(ThreadPriority priority);
