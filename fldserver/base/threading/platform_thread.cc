...
const Feature kThreadPrioritiesFeature{"ThreadPriorities", FEATURE_ENABLED_BY_DEFAULT};
...
// static
void
PlatformThread::SetCurrentThreadPriority(ThreadPriority priority)
{
...
TimeDelta
PlatformThread::GetRealtimePeriod(Delegate* delegate)
{
    if (g_use_thread_priorities.load())
...
TimeDelta
PlatformThread::Delegate::GetRealtimePeriod()
{
    return TimeDelta();
}
...
InitializeThreadPrioritiesFeature()
{
    ...
    if (FeatureList::GetInstance() && !FeatureList::IsEnabled(kThreadPrioritiesFeature))
    {
