...
#include "fldserver/base/threading/hang_watcher.h"

#include "fldserver/base/bind.h"
#include "fldserver/base/callback_helpers.h"
#include "fldserver/base/containers/flat_map.h"
#include "fldserver/base/debug/alias.h"
#include "fldserver/base/debug/crash_logging.h"
#include "fldserver/base/debug/dump_without_crashing.h"
...
#include "fldserver/base/threading/thread_checker.h"
...
// static
const base::Feature HangWatcher::kEnableHangWatcher{"EnableHangWatcher",
                                                    base::FEATURE_ENABLED_BY_DEFAULT};

constexpr base::FeatureParam<int> kIOThreadLogLevel{&HangWatcher::kEnableHangWatcher,
                                                    "io_thread_log_level",
                                                    static_cast<int>(LoggingLevel::kUmaOnly)};
constexpr base::FeatureParam<int> kUIThreadLogLevel{&HangWatcher::kEnableHangWatcher,
                                                    "ui_thread_log_level",
                                                    static_cast<int>(LoggingLevel::kUmaOnly)};
constexpr base::FeatureParam<int> kThreadPoolLogLevel{&HangWatcher::kEnableHangWatcher,
                                                      "threadpool_log_level",
                                                      static_cast<int>(LoggingLevel::kNone)};

// static
const base::TimeDelta WatchHangsInScope::kDefaultHangWatchTime = base::TimeDelta::FromSeconds(10);
...
void
LogHungThreadCountHistogram(HangWatcher::ThreadType thread_type, int count)
{
    ...
    switch (thread_type)
    {
        case HangWatcher::ThreadType::kIOThread:
            UMA_HISTOGRAM_BOOLEAN("HangWatcher.IsThreadHung.BrowserProcess."
                                  "IOThread",
                                  any_thread_hung);
            break;
...
bool
ThreadTypeLoggingLevelGreaterOrEqual(HangWatcher::ThreadType thread_type,
                                     LoggingLevel logging_level)
{
    switch (thread_type)
    {
        case HangWatcher::ThreadType::kIOThread:
            return g_io_thread_log_level.load(std::memory_order_relaxed) >= logging_level;
        case HangWatcher::ThreadType::kUIThread:
            return g_ui_thread_log_level.load(std::memory_order_relaxed) >= logging_level;
        case HangWatcher::ThreadType::kThreadPoolThread:
            return g_threadpool_log_level.load(std::memory_order_relaxed) >= logging_level;
    }
}
}  // namespace
...
WatchHangsInScope::WatchHangsInScope(TimeDelta timeout)
{
    ...
    DCHECK(current_hang_watch_state) << "WatchHangsInScope can only be used on a thread that "
                                        "registered for hang watching with "
                                        "HangWatcher::RegisterThread.";

#if DCHECK_IS_ON()
    previous_watch_hangs_in_scope_ = current_hang_watch_state->GetCurrentWatchHangsInScope();
    current_hang_watch_state->SetCurrentWatchHangsInScope(this);
#endif

    uint64_t old_flags;
    base::TimeTicks old_deadline;
    std::tie(old_flags, old_deadline) = current_hang_watch_state->GetFlagsAndDeadline();

    const bool hangs_ignored_for_current_scope = internal::HangWatchDeadline::IsFlagSet(
            internal::HangWatchDeadline::Flag::kIgnoreCurrentWatchHangsInScope, old_flags);

    const bool has_active_hang_watch_disabled = internal::HangWatchDeadline::IsFlagSet(
            internal::HangWatchDeadline::Flag::kHasActiveIgnoreHangsInScope, old_flags);
...
WatchHangsInScope::~WatchHangsInScope()
{
    ...
    if (current_hang_watch_state->IsFlagSet(internal::HangWatchDeadline::Flag::kShouldBlockOnHang))
    ...
    current_hang_watch_state->SetCurrentWatchHangsInScope(previous_watch_hangs_in_scope_);
#endif
...
HangWatcher::InitializeOnMainThread()
{
    ...
    {
        g_threadpool_log_level.store(static_cast<LoggingLevel>(kThreadPoolLogLevel.Get()),
                                     std::memory_order_relaxed);
        g_io_thread_log_level.store(static_cast<LoggingLevel>(kIOThreadLogLevel.Get()),
                                    std::memory_order_relaxed);
        g_ui_thread_log_level.store(static_cast<LoggingLevel>(kUIThreadLogLevel.Get()),
                                    std::memory_order_relaxed);
    }
}

void
HangWatcher::UnitializeOnMainThreadForTesting()
{
...
// static
bool
HangWatcher::IsThreadPoolHangWatchingEnabled()
{
    return g_threadpool_log_level.load(std::memory_order_relaxed) != LoggingLevel::kNone;
}

// static
bool
HangWatcher::IsIOThreadHangWatchingEnabled()
{
    return g_io_thread_log_level.load(std::memory_order_relaxed) != LoggingLevel::kNone;
}

// static
bool
HangWatcher::IsUIThreadHangWatchingEnabled()
{
    return g_ui_thread_log_level.load(std::memory_order_relaxed) != LoggingLevel::kNone;
}

// static
bool
HangWatcher::IsCrashReportingEnabled()
{
    if (g_ui_thread_log_level.load(std::memory_order_relaxed) == LoggingLevel::kUmaAndCrash)
    {
        return true;
    }
    if (g_io_thread_log_level.load(std::memory_order_relaxed) == LoggingLevel::kUmaAndCrash)
    {
        return true;
    }
    if (g_threadpool_log_level.load(std::memory_order_relaxed) == LoggingLevel::kUmaAndCrash)
    {
        return true;
    }
    return false;
...
HangWatcher::HangWatcher() :
    ...
    memory_pressure_listener_(
            FROM_HERE, base::BindRepeating(&HangWatcher::OnMemoryPressure, base::Unretained(this)))
{
...
HangWatcher::GetTimeSinceLastCriticalMemoryPressureCrashKey()
{
    ...
    constexpr debug::CrashKeySize kCrashKeyContentSize = debug::CrashKeySize::Size32;
    DCHECK_GE(static_cast<uint64_t>(kCrashKeyContentSize),
              base::NumberToString(std::numeric_limits<int64_t>::max()).size());

    static debug::CrashKeyString* crash_key =
            AllocateCrashKeyString("seconds-since-last-memory-pressure", kCrashKeyContentSize);
    ...
    if (last_critical_memory_pressure_time.is_null())
    {
        ...
        static_assert(base::size(kNoMemoryPressureMsg)
    ...
    else
    {
        ...
        return debug::ScopedCrashKeyString(
                crash_key,
                base::NumberToString(time_since_last_critical_memory_pressure.InSeconds()));
    }
}
#endif

void
HangWatcher::OnMemoryPressure(
        base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level)
{
    if (memory_pressure_level == base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL)
    {
        last_critical_memory_pressure_.store(base::TimeTicks::Now(), std::memory_order_relaxed);
    }
}
...
HangWatcher::Wait()
{
    while (true)
    {
        ...
        constexpr base::TimeDelta kWaitDriftTolerance = base::TimeDelta::FromMilliseconds(100);
        ...
        const bool wait_was_normal = wait_time <= (monitor_period_ + kWaitDriftTolerance);

        UMA_HISTOGRAM_TIMES("HangWatcher.SleepDrift.BrowserProcess", wait_time - monitor_period_);
...
HangWatcher::Run()
{
    ...
    while (keep_monitoring_.load(std::memory_order_relaxed))
    {
        ...
        if (!IsWatchListEmpty() && keep_monitoring_.load(std::memory_order_relaxed))
        {
            Monitor();
...
ScopedClosureRunner
HangWatcher::RegisterThreadInternal(ThreadType thread_type)
{
    AutoLock auto_lock(watch_state_lock_);

    watch_states_.push_back(
            internal::HangWatchState::CreateHangWatchStateForCurrentThread(thread_type));

    return ScopedClosureRunner(
            BindOnce(&HangWatcher::UnregisterThread, Unretained(HangWatcher::GetInstance())));
}

// static
ScopedClosureRunner
HangWatcher::RegisterThread(ThreadType thread_type)
{
    if (!GetInstance())
    {
        return ScopedClosureRunner();
...
base::TimeTicks
HangWatcher::WatchStateSnapShot::GetHighestDeadline() const
{
...
HangWatcher::WatchStateSnapShot::WatchStateSnapShot(const HangWatchStates& watch_states,
                                                    base::TimeTicks deadline_ignore_threshold)
{
    ...
    for (const auto& watch_state : watch_states)
    {
        ...
        if (internal::HangWatchDeadline::IsFlagSet(
                    internal::HangWatchDeadline::Flag::kIgnoreCurrentWatchHangsInScope, flags))
        {
            continue;
        }
        ...
        const size_t hang_count_index = static_cast<size_t>(watch_state.get()->thread_type());
    ...
    for (size_t i = 0; i < kHangCountArraySize; ++i)
    {
        ...
        const HangWatcher::ThreadType thread_type = static_cast<HangWatcher::ThreadType>(i);
        if (hang_count != kInvalidHangCount
            && ThreadTypeLoggingLevelGreaterOrEqual(thread_type, LoggingLevel::kUmaOnly))
...
HangWatcher::WatchStateSnapShot::WatchStateSnapShot(const WatchStateSnapShot& other) = default;

HangWatcher::WatchStateSnapShot::~WatchStateSnapShot() = default;

std::string
HangWatcher::WatchStateSnapShot::PrepareHungThreadListCrashKey() const
{
    DCHECK(IsActionable());
...
bool
HangWatcher::WatchStateSnapShot::IsActionable() const
{
    return !hung_watch_state_copies_.empty();
}

HangWatcher::WatchStateSnapShot
HangWatcher::GrabWatchStateSnapshotForTesting() const
{
...
HangWatcher::Monitor()
{
    ...
    WatchStateSnapShot watch_state_snapshot(watch_states_, deadline_ignore_threshold_);
...
void
HangWatcher::DoDumpWithoutCrashing(const WatchStateSnapShot& watch_state_snapshot)
{
    ...
    static debug::CrashKeyString* crash_key =
            AllocateCrashKeyString("list-of-hung-threads", debug::CrashKeySize::Size256);

    const debug::ScopedCrashKeyString list_of_hung_threads_crash_key_string(
            crash_key, list_of_hung_thread_ids);

    const debug::ScopedCrashKeyString time_since_last_critical_memory_pressure_crash_key_string =
    ...
    base::TimeTicks latest_expired_deadline = watch_state_snapshot.GetHighestDeadline();
...
void
HangWatcher::SetAfterMonitorClosureForTesting(base::RepeatingClosure closure)
{
...
void
HangWatcher::SetOnHangClosureForTesting(base::RepeatingClosure closure)
{
    DCHECK_CALLED_ON_VALID_THREAD(constructing_thread_checker_);
...
void
HangWatcher::SetMonitoringPeriodForTesting(base::TimeDelta period)
{
    DCHECK_CALLED_ON_VALID_THREAD(constructing_thread_checker_);
...
void
HangWatcher::SetAfterWaitCallbackForTesting(RepeatingCallback<void(TimeTicks)> callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(constructing_thread_checker_);
...
void
HangWatcher::SignalMonitorEventForTesting()
{
    DCHECK_CALLED_ON_VALID_THREAD(constructing_thread_checker_);
    should_monitor_.Signal();
}
...
void
HangWatcher::SetTickClockForTesting(const base::TickClock* tick_clock)
{
    tick_clock_ = tick_clock;
}

void
HangWatcher::BlockIfCaptureInProgress()
{
...
HangWatcher::UnregisterThread()
{
    ...
    auto it = ranges::find_if(
            watch_states_,
            [current_hang_watch_state](const std::unique_ptr<internal::HangWatchState>& state) {
...
constexpr uint64_t kPersistentFlagsAndDeadlineMask =
        kOnlyDeadlineMask
        | static_cast<uint64_t>(HangWatchDeadline::Flag::kIgnoreCurrentWatchHangsInScope)
        | static_cast<uint64_t>(HangWatchDeadline::Flag::kHasActiveIgnoreHangsInScope);
}  // namespace

// Flag binary representation assertions.
static_assert(static_cast<uint64_t>(HangWatchDeadline::Flag::kMinValue) > kOnlyDeadlineMask,
              "Invalid numerical value for flag. Would interfere with bits of data.");
static_assert(static_cast<uint64_t>(HangWatchDeadline::Flag::kMaxValue) <= kMaximumFlag,
              "A flag can only set a single bit.");

HangWatchDeadline::HangWatchDeadline() = default;
HangWatchDeadline::~HangWatchDeadline() = default;

std::pair<uint64_t, TimeTicks>
HangWatchDeadline::GetFlagsAndDeadline() const
{
    uint64_t bits = bits_.load(std::memory_order_relaxed);
    return std::make_pair(ExtractFlags(bits), DeadlineFromBits(ExtractDeadline((bits))));
}

TimeTicks
HangWatchDeadline::GetDeadline() const
{
    return DeadlineFromBits(ExtractDeadline(bits_.load(std::memory_order_relaxed)));
}

// static
TimeTicks
HangWatchDeadline::Max()
{
...
// static
bool
HangWatchDeadline::IsFlagSet(Flag flag, uint64_t flags)
{
    return static_cast<uint64_t>(flag) & flags;
}

void
HangWatchDeadline::SetDeadline(TimeTicks new_deadline)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    const uint64_t new_flags = ExtractFlags(old_bits & kPersistentFlagsAndDeadlineMask);
...
bool
HangWatchDeadline::SetShouldBlockOnHang(uint64_t old_flags, TimeTicks old_deadline)
{
    ...
    uint64_t old_bits = old_flags | static_cast<uint64_t>(old_deadline.ToInternalValue());
    const uint64_t desired_bits = old_bits | static_cast<uint64_t>(Flag::kShouldBlockOnHang);
    ...
    return bits_.compare_exchange_weak(
            old_bits, desired_bits, std::memory_order_relaxed, std::memory_order_relaxed);
}

void
HangWatchDeadline::SetHasActiveIgnoreHangsInScope()
{
    SetPersistentFlag(Flag::kHasActiveIgnoreHangsInScope);
}

void
HangWatchDeadline::UnsetHasActiveIgnoreHangsInScope()
{
    ClearPersistentFlag(Flag::kHasActiveIgnoreHangsInScope);
}

void
HangWatchDeadline::SetIgnoreCurrentWatchHangsInScope()
{
    SetPersistentFlag(Flag::kIgnoreCurrentWatchHangsInScope);
}

void
HangWatchDeadline::UnsetIgnoreCurrentWatchHangsInScope()
{
    ClearPersistentFlag(Flag::kIgnoreCurrentWatchHangsInScope);
}

void
HangWatchDeadline::SetPersistentFlag(Flag flag)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (switch_bits_callback_for_testing_)
...
void
HangWatchDeadline::ClearPersistentFlag(Flag flag)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (switch_bits_callback_for_testing_)
        SwitchBitsForTesting();
...
// static
uint64_t
HangWatchDeadline::ExtractFlags(uint64_t bits)
{
...
// static
uint64_t
HangWatchDeadline::ExtractDeadline(uint64_t bits)
{
...
// static
TimeTicks
HangWatchDeadline::DeadlineFromBits(uint64_t bits)
{
    // |kOnlyDeadlineMask| has all the deadline bits set to 1 so is the largest
    // representable value.
    DCHECK(bits <= kOnlyDeadlineMask) << "Flags bits are set. Remove them before returning "
                                         "deadline.";
    return TimeTicks::FromInternalValue(bits);
}

bool
HangWatchDeadline::IsFlagSet(Flag flag) const
{
    return bits_.load(std::memory_order_relaxed) & static_cast<uint64_t>(flag);
}

void
HangWatchDeadline::SetSwitchBitsClosureForTesting(RepeatingCallback<uint64_t(void)> closure)
{
    switch_bits_callback_for_testing_ = closure;
}

void
HangWatchDeadline::ResetSwitchBitsClosureForTesting()
{
...
uint64_t
HangWatchDeadline::SwitchBitsForTesting()
{
    DCHECK(switch_bits_callback_for_testing_);
...
HangWatchState::CreateHangWatchStateForCurrentThread(HangWatcher::ThreadType thread_type)
{
    // Allocate a watch state object for this thread.
    std::unique_ptr<HangWatchState> hang_state = std::make_unique<HangWatchState>(thread_type);
...
TimeTicks
HangWatchState::GetDeadline() const
{
    return deadline_.GetDeadline();
}

std::pair<uint64_t, TimeTicks>
HangWatchState::GetFlagsAndDeadline() const
{
    return deadline_.GetFlagsAndDeadline();
}

void
HangWatchState::SetDeadline(TimeTicks deadline)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
bool
HangWatchState::IsOverDeadline() const
{
    return TimeTicks::Now() > deadline_.GetDeadline();
}

void
HangWatchState::SetHasActiveIgnoreHangsInScope()
{
    deadline_.SetHasActiveIgnoreHangsInScope();
}

void
HangWatchState::UnsetHasActiveIgnoreHangsInScope()
{
    deadline_.UnsetHasActiveIgnoreHangsInScope();
}

void
HangWatchState::SetIgnoreCurrentWatchHangsInScope()
{
    deadline_.SetIgnoreCurrentWatchHangsInScope();
}

void
HangWatchState::UnsetIgnoreCurrentWatchHangsInScope()
{
    deadline_.UnsetIgnoreCurrentWatchHangsInScope();
}

bool
HangWatchState::SetShouldBlockOnHang(uint64_t old_flags, TimeTicks old_deadline)
{
    return deadline_.SetShouldBlockOnHang(old_flags, old_deadline);
}

bool
HangWatchState::IsFlagSet(HangWatchDeadline::Flag flag)
{
...
#if DCHECK_IS_ON()
void
HangWatchState::SetCurrentWatchHangsInScope(WatchHangsInScope* current_hang_watch_scope_enable)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
WatchHangsInScope*
HangWatchState::GetCurrentWatchHangsInScope()
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
HangWatchDeadline*
HangWatchState::GetHangWatchDeadlineForTesting()
{
    return &deadline_;
}

void
HangWatchState::IncrementNestingLevel()
{
    ++nesting_level_;
}

void
HangWatchState::DecrementNestingLevel()
{
    --nesting_level_;
}
...
PlatformThreadId
HangWatchState::GetThreadID() const
{
    return thread_id_;
}
