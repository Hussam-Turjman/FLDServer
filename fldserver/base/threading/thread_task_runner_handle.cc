...
namespace
{
base::LazyInstance<base::ThreadLocalPointer<ThreadTaskRunnerHandle>>::Leaky thread_task_runner_tls =
...
ThreadTaskRunnerHandle::Get()
{
    ...
    CHECK(current) << "Error: This caller requires a single-threaded context (i.e. the "
...
ThreadTaskRunnerHandle::ThreadTaskRunnerHandle(scoped_refptr<SingleThreadTaskRunner> task_runner) :
    task_runner_(std::move(task_runner)), sequenced_task_runner_handle_(task_runner_)
{
...
ThreadTaskRunnerHandleOverride::ThreadTaskRunnerHandleOverride(
        scoped_refptr<SingleThreadTaskRunner> overriding_task_runner, bool allow_nested_runloop)
{
    ...
    {
        top_level_thread_task_runner_handle_.emplace(std::move(overriding_task_runner));
        return;
    }
