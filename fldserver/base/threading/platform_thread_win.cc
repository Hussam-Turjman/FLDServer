...
#include "fldserver/base/threading/platform_thread_win.h"
...
#include "fldserver/base/debug/profiler.h"
...
#include "fldserver/base/allocator/partition_allocator/starscan/stack/stack.h"
#endif
...
void
SetNameInternal(PlatformThreadId thread_id, const char* name)
{
    ...
    __try
    {
        RaiseException(kVCThreadNameException,
                       0,
...
DWORD __stdcall ThreadFunc(void* params)
{
    ...
    if (did_dup)
    {
        ...
        ThreadIdNameManager::GetInstance()->RegisterThread(scoped_platform_handle.Get(),
...
// is created.
bool
CreateThreadInternal(size_t stack_size,
                     PlatformThread::Delegate* delegate,
                     ...
                     ThreadPriority priority)
{
    ...
    if (!thread_handle)
    {
        ...
        switch (last_error)
        {
            ...
            case ERROR_COMMITMENT_LIMIT: TerminateBecauseOutOfMemory(stack_size); break;

            default:
                static auto* last_error_crash_key = debug::AllocateCrashKeyString(
                        "create_thread_last_error", debug::CrashKeySize::Size32);
                debug::SetCrashKeyString(last_error_crash_key, base::NumberToString(last_error));
                break;
        }
...
AssertMemoryPriority(HANDLE thread, int memory_priority)
{
    ...
    static const auto get_thread_information_fn =
            reinterpret_cast<decltype(&::GetThreadInformation)>(
                    ::GetProcAddress(::GetModuleHandle(L"Kernel32.dll"), "GetThreadInformation"));
    ...
    DCHECK(get_thread_information_fn(thread,
                                     ::ThreadMemoryPriority,
                                     &memory_priority_information,
                                     sizeof(memory_priority_information)));

    DCHECK_EQ(memory_priority, static_cast<int>(memory_priority_information.MemoryPriority));
#endif
}
...
// static
void
PlatformThread::Sleep(TimeDelta duration)
{
...
// static
void
PlatformThread::SetName(const std::string& name)
{
    ...
    {
        set_thread_description_func(::GetCurrentThread(), base::UTF8ToWide(name).c_str());
    }
...
// static
bool
PlatformThread::CreateNonJoinable(size_t stack_size, Delegate* delegate)
{
...
PlatformThread::CreateNonJoinableWithPriority(size_t stack_size,
                                              ...
                                              ThreadPriority priority)
{
    return CreateThreadInternal(stack_size, delegate, nullptr /* non-joinable */, priority);
}

// static
void
PlatformThread::Join(PlatformThreadHandle thread_handle)
{
    ...
    CHECK_EQ(WAIT_OBJECT_0, WaitForSingleObject(thread_handle.platform_handle(), INFINITE));
    CloseHandle(thread_handle.platform_handle());
}

// static
void
PlatformThread::Detach(PlatformThreadHandle thread_handle)
{
    CloseHandle(thread_handle.platform_handle());
}
...
// static
bool
PlatformThread::CanIncreaseThreadPriority(ThreadPriority priority)
{
    return true;
}
...
// static
void
PlatformThread::SetCurrentThreadPriorityImpl(ThreadPriority priority)
{
    PlatformThreadHandle::Handle thread_handle = PlatformThread::CurrentHandle().platform_handle();
    ...
    switch (priority)
    {
        case ThreadPriority::BACKGROUND:
            ...
            break;
        case ThreadPriority::NORMAL: desired_priority = THREAD_PRIORITY_NORMAL; break;
        case ThreadPriority::DISPLAY: desired_priority = THREAD_PRIORITY_ABOVE_NORMAL; break;
        case ThreadPriority::REALTIME_AUDIO:
            desired_priority = THREAD_PRIORITY_TIME_CRITICAL;
            break;
        default: NOTREACHED() << "Unknown priority."; break;
    }
    ...
    DPLOG_IF(ERROR, !success) << "Failed to set thread priority to " << desired_priority;
...
{
    static_assert(THREAD_PRIORITY_IDLE < 0,
                  "THREAD_PRIORITY_IDLE is >= 0 and will incorrectly cause errors.");
    static_assert(THREAD_PRIORITY_LOWEST < 0,
    ...
    const int priority = ::GetThreadPriority(PlatformThread::CurrentHandle().platform_handle());
    ...
    switch (priority)
    {
        case kWin7BackgroundThreadModePriority:
            DCHECK_EQ(win::GetVersion(), win::Version::WIN7);
            return ThreadPriority::BACKGROUND;
        case kWin7NormalPriority: DCHECK_EQ(win::GetVersion(), win::Version::WIN7); FALLTHROUGH;
        case THREAD_PRIORITY_NORMAL: return ThreadPriority::NORMAL;
        case kWinNormalPriority1: FALLTHROUGH;
        case kWinNormalPriority2: return ThreadPriority::NORMAL;
        case THREAD_PRIORITY_ABOVE_NORMAL:
        case THREAD_PRIORITY_HIGHEST: return ThreadPriority::DISPLAY;
        case THREAD_PRIORITY_TIME_CRITICAL: return ThreadPriority::REALTIME_AUDIO;
        case THREAD_PRIORITY_ERROR_RETURN: DPCHECK(false) << "::GetThreadPriority error";
    }
...
// static
size_t
PlatformThread::GetDefaultThreadStackSize()
{
    return 0;
}
