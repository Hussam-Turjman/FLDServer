...
namespace base
{
SimpleThread::SimpleThread(const std::string& name) : SimpleThread(name, Options())
{
}
...
SimpleThread::SimpleThread(const std::string& name, const Options& options) :
    name_(name),
    options_(options),
    event_(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED)
{
}
...
SimpleThread::~SimpleThread()
{
    ...
    DCHECK(!options_.joinable || HasBeenJoined()) << "Joinable SimpleThread destroyed without "
                                                     "being Join()ed.";
}
...
void
SimpleThread::StartAsync()
{
    DCHECK(!HasStartBeenAttempted()) << "Tried to Start a thread multiple times.";
    start_called_ = true;
    BeforeStart();
    bool success = options_.joinable ? PlatformThread::CreateWithPriority(
                           options_.stack_size, this, &thread_, options_.priority)
...
bool
SimpleThread::HasBeenStarted()
{
    return event_.IsSignaled();
}

void
SimpleThread::ThreadMain()
{
...
DelegateSimpleThreadPool::DelegateSimpleThreadPool(const std::string& name_prefix,
                                                   int num_threads) :
    name_prefix_(name_prefix),
    num_threads_(num_threads),
    dry_(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED)
{
}
...
void
DelegateSimpleThreadPool::Start()
{
...
void
DelegateSimpleThreadPool::JoinAll()
{
...
void
DelegateSimpleThreadPool::AddWork(Delegate* delegate, int repeat_count)
{
