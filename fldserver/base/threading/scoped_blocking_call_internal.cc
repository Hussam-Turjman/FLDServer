...
LazyInstance<ThreadLocalPointer<BlockingObserver>>::Leaky tls_blocking_observer =
        LAZY_INSTANCE_INITIALIZER;

// Last ScopedBlockingCall instantiated on this thread.
LazyInstance<ThreadLocalPointer<UncheckedScopedBlockingCall>>::Leaky tls_last_scoped_blocking_call =
        LAZY_INSTANCE_INITIALIZER;
...
void
SetBlockingObserverForCurrentThread(BlockingObserver* blocking_observer)
{
...
IOJankMonitoringWindow::ScopedMonitoredCall::~ScopedMonitoredCall()
{
    ...
    {
        assigned_jank_window_->OnBlockingCallCompleted(call_start_, TimeTicks::Now());
    }
}

void
IOJankMonitoringWindow::ScopedMonitoredCall::Cancel()
{
    assigned_jank_window_ = nullptr;
}

IOJankMonitoringWindow::IOJankMonitoringWindow(TimeTicks start_time) : start_time_(start_time)
{
}

// static
void
IOJankMonitoringWindow::CancelMonitoringForTesting()
{
...
IOJankMonitoringWindow::MonitorNextJankWindowIfNecessary(TimeTicks recent_now)
{
...

    {
        ...
        TimeTicks next_window_start_time =
                current_jank_window_ref ? current_jank_window_ref->start_time_ + kMonitoringWindow
                                        : recent_now;
        ...
        next_jank_window = MakeRefCounted<IOJankMonitoringWindow>(next_window_start_time);
...
void
IOJankMonitoringWindow::OnBlockingCallCompleted(TimeTicks call_start, TimeTicks call_end)
{
    ...
    const int jank_start_index = ClampFloor((call_start - start_time_) / kIOJankInterval);

    // Round the jank duration so the total number of intervals marked janky is as
    // close as possible to the actual jank duration.
    const int num_janky_intervals = ClampRound((call_end - call_start) / kIOJankInterval);

    AddJank(jank_start_index, num_janky_intervals);
}

void
IOJankMonitoringWindow::AddJank(int local_jank_start_index, int num_janky_intervals)
{
...
// static
Lock&
IOJankMonitoringWindow::current_jank_window_lock()
{
...
{
    static NoDestructor<scoped_refptr<IOJankMonitoringWindow>> current_jank_window;
    return *current_jank_window;
}

// static
IOJankReportingCallback&
IOJankMonitoringWindow::reporting_callback_storage()
{
    static NoDestructor<IOJankReportingCallback> reporting_callback;
    return *reporting_callback;
}

UncheckedScopedBlockingCall::UncheckedScopedBlockingCall(const Location& from_here,
    ...
    is_will_block_(
            ...
            || (previous_scoped_blocking_call_ && previous_scoped_blocking_call_->is_will_block_)),
    scoped_activity_(from_here, 0, kActivityTrackerId, 0)
{
    ...
    {
        const bool is_monitored_type = blocking_call_type == BlockingCallType::kRegular
                                       && !is_will_block_;
...
{
    {
        ...
        DCHECK(internal::IOJankMonitoringWindow::reporting_callback_storage().is_null());
    ...
    internal::IOJankMonitoringWindow::MonitorNextJankWindowIfNecessary(TimeTicks::Now());
}
