...
class CORE_EXPORT Thread : PlatformThread::Delegate
{
public:
    ...
    public:
        ...
        // amount.
        virtual void
        BindToCurrentThread(TimerSlack timer_slack) = 0;
    };

    struct CORE_EXPORT Options
    {
        using MessagePumpFactory = RepeatingCallback<std::unique_ptr<MessagePump>()>;
    ...
    void
    init_com_with_mta(bool use_mta)
    {
        DCHECK(!delegate_);
    ...
    // callback.
    bool
    StartWithOptions(const Options& options);
    ...
    task_runner() const
    {
        ...
        DCHECK(owning_sequence_checker_.CalledOnValidSequence()
               || (id_event_.IsSignaled() && id_ == PlatformThread::CurrentId()) || delegate_);
...
protected:
    ...
    static void
    SetThreadWasQuitProperly(bool flag);
    static bool
