...
// reports.
class CORE_EXPORT WatchHangsInScope
{
public:
    ...
    WatchHangsInScope&
    operator=(const WatchHangsInScope&) = delete;
...
//
class CORE_EXPORT IgnoreHangsInScope
{
public:
    IgnoreHangsInScope();
    ~IgnoreHangsInScope();
    ...
    IgnoreHangsInScope&
    operator=(const IgnoreHangsInScope&) = delete;
...
class CORE_EXPORT HangWatcher : public DelegateSimpleThread::Delegate
{
public:
    ...
    HangWatcher&
    operator=(const HangWatcher&) = delete;
    ...
    // thread with.
    static ScopedClosureRunner
    RegisterThread(ThreadType thread_type) WARN_UNUSED_RESULT;
    ...
    void
    SetAfterMonitorClosureForTesting(base::RepeatingClosure closure);
    ...
    // deadlocks are possible.
    void
    SetOnHangClosureForTesting(base::RepeatingClosure closure);

    // Set a monitoring period other than the default. Use only for
    // testing.
    void
    SetMonitoringPeriodForTesting(base::TimeDelta period);

    // Choose a callback to invoke right after waiting to monitor in Wait(). Use
    // only for testing.
    void
    SetAfterWaitCallbackForTesting(RepeatingCallback<void(TimeTicks)> callback);
    ...
    void
    SetTickClockForTesting(const base::TickClock* tick_clock);
...
private:
    ...
    ScopedClosureRunner
    RegisterThreadInternal(ThreadType thread_type)
    ...
    GetTimeSinceLastCriticalMemoryPressureCrashKey() WARN_UNUSED_RESULT;
#endif
    ...
    public:
        ...
        // IsActionable().
        std::string
        PrepareHungThreadListCrashKey() const;

        // Return the highest deadline included in this snapshot.
        base::TimeTicks
        GetHighestDeadline() const;
    ...
    // different threads.
    std::atomic<base::TimeTicks> last_critical_memory_pressure_{base::TimeTicks()};
...
class CORE_EXPORT HangWatchDeadline
{
public:
    ...
    HangWatchDeadline&
    operator=(const HangWatchDeadline&) = delete;
    ...
    bool
    SetShouldBlockOnHang(uint64_t old_flags, TimeTicks old_deadline);

    // Sets the kHasActiveIgnoreHangsInScope flag.
    void
    SetHasActiveIgnoreHangsInScope();

    // Clears the kHasActiveIgnoreHangsInScope flag.
    void
    UnsetHasActiveIgnoreHangsInScope();
    ...
    // only for testing.
    void
    SetSwitchBitsClosureForTesting(RepeatingCallback<uint64_t(void)> closure);
...
private:
    ...
    static uint64_t
    ExtractFlags(uint64_t bits);

    // Extract the deadline bits from |bits|.
    static uint64_t
    ExtractDeadline(uint64_t bits);
...
class CORE_EXPORT HangWatchState
{
public:
    ...
    HangWatchState&
    operator=(const HangWatchState&) = delete;

    // Allocates a new state object bound to the calling thread and returns an
    // owning pointer to it.
    static std::unique_ptr<HangWatchState>
    CreateHangWatchStateForCurrentThread(HangWatcher::ThreadType thread_type);
    ...
    void
    SetDeadline(TimeTicks deadline);
