...
#define INTERNAL_SCOPED_THREAD_PRIORITY_CONCAT1(a, b) INTERNAL_SCOPED_THREAD_PRIORITY_CONCAT(a, b)
...
#define SCOPED_MAY_LOAD_LIBRARY_AT_BACKGROUND_PRIORITY()                                           \
    static std::atomic_bool INTERNAL_SCOPED_THREAD_PRIORITY_APPEND_LINE(already_loaded){false};    \
    base::internal::ScopedMayLoadLibraryAtBackgroundPriority                                       \
            INTERNAL_SCOPED_THREAD_PRIORITY_APPEND_LINE(                                           \
                    scoped_may_load_library_at_background_priority)(                               \
                    FROM_HERE, &INTERNAL_SCOPED_THREAD_PRIORITY_APPEND_LINE(already_loaded));
...
namespace internal
{
class CORE_EXPORT ScopedMayLoadLibraryAtBackgroundPriority
{
public:
