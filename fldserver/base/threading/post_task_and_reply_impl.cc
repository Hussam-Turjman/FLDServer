...
public:
    ...
    ~PostTaskAndReplyRelay()
    {
        ...
        if (!reply_task_runner_->RunsTasksInCurrentSequence())
        {
            ...
            auto relay_to_delete = std::make_unique<PostTaskAndReplyRelay>(std::move(*this));
    ...
    PostTaskAndReplyRelay&
    operator=(PostTaskAndReplyRelay&&) = delete;

    // Static function is used because it is not possible to bind a method call to
    // a non-pointer type.
    static void
    RunTaskAndPostReply(PostTaskAndReplyRelay relay)
    {
        DCHECK(relay.task_);
        ...
        reply_task_runner_raw->PostTask(
                from_here, BindOnce(&PostTaskAndReplyRelay::RunReply, std::move(relay)));
    }
...
private:
    ...
    // a non-pointer type.
    static void
    RunReply(PostTaskAndReplyRelay relay)
    {
        DCHECK(!relay.task_);
        DCHECK(relay.reply_);
...
{
bool
PostTaskAndReplyImpl::PostTaskAndReply(const Location& from_here,
                                       OnceClosure task,
                                       OnceClosure reply)
{
    ...
    const bool post_task_success = PostTask(
            ...
            BindOnce(&PostTaskAndReplyRelay::RunTaskAndPostReply,
                     PostTaskAndReplyRelay(from_here,
                                           std::move(task),
                                           std::move(reply),
