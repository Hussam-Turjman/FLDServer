...
class CORE_EXPORT ThreadTaskRunnerHandle
{
public:
    // Gets the SingleThreadTaskRunner for the current thread.
    static const scoped_refptr<SingleThreadTaskRunner>&
    Get() WARN_UNUSED_RESULT;

    // Returns true if the SingleThreadTaskRunner is already created for
    // the current thread.
    static bool
    IsSet() WARN_UNUSED_RESULT;

    // Binds |task_runner| to the current thread. |task_runner| must belong
    // to the current thread for this to succeed.
    explicit ThreadTaskRunnerHandle(scoped_refptr<SingleThreadTaskRunner> task_runner);
...
class CORE_EXPORT ThreadTaskRunnerHandleOverride
{
public:
    ThreadTaskRunnerHandleOverride(const ThreadTaskRunnerHandleOverride&) = delete;
