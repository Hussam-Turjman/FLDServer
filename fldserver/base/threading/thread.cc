...
#include "fldserver/base/task/simple_task_executor.h"
...
base::LazyInstance<base::ThreadLocalBoolean>::Leaky lazy_tls_bool = LAZY_INSTANCE_INITIALIZER;
...
public:
    explicit SequenceManagerThreadDelegate(
        ...
        sequence_manager_(sequence_manager::internal::SequenceManagerImpl::CreateUnbound(
                sequence_manager::SequenceManager::Settings::Builder()
                        .SetMessagePumpType(message_pump_type)
                        .Build())),
        default_task_queue_(sequence_manager_->CreateTaskQueue(
                sequence_manager::TaskQueue::Spec("default_tq").SetTimeDomain(time_domain))),
    ...
    void
    BindToCurrentThread(TimerSlack timer_slack) override
    {
        sequence_manager_->BindToMessagePump(std::move(message_pump_factory_).Run());
        sequence_manager_->SetTimerSlack(timer_slack);
        simple_task_executor_.emplace(GetDefaultTaskRunner());
    }

private:
    std::unique_ptr<sequence_manager::internal::SequenceManagerImpl> sequence_manager_;
...
Thread::Thread(const std::string& name) :
    id_event_(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED),
    name_(name),
    start_event_(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED)
{
...
bool
Thread::StartWithOptions(const Options& options)
{
    DCHECK(owning_sequence_checker_.CalledOnValidSequence());
    DCHECK(!delegate_);
    DCHECK(!IsRunning());
    DCHECK(!stopping_) << "Starting a non-joinable thread a second time? That's "
                       << "not allowed!";
#if IS_WIN
    DCHECK((com_status_ != STA) || (options.message_pump_type == MessagePumpType::UI));
#endif
    ...
    {
        delegate_ = std::make_unique<SequenceManagerThreadDelegate>(MessagePumpType::CUSTOM,
    ...
    // fixed).
    {
        ...
        bool success = options.joinable ? PlatformThread::CreateWithPriority(
                               options.stack_size, this, &thread_, options.priority)
...
bool
Thread::WaitUntilThreadStarted() const
{
    DCHECK(owning_sequence_checker_.CalledOnValidSequence());
    if (!delegate_)
        return false;
...
Thread::FlushForTesting()
{
    ...
    task_runner()->PostTask(FROM_HERE, BindOnce(&WaitableEvent::Signal, Unretained(&done)));
    done.Wait();
}
...
Thread::StopSoon()
{
    ...
    task_runner()->PostTask(FROM_HERE, base::BindOnce(&Thread::ThreadQuitHelper, Unretained(this)));
}

void
Thread::DetachFromSequence()
{
    DCHECK(owning_sequence_checker_.CalledOnValidSequence());
    owning_sequence_checker_.DetachFromSequence();
}
...
// static
void
Thread::SetThreadWasQuitProperly(bool flag)
{
...
// static
bool
Thread::GetThreadWasQuitProperly()
{
...
Thread::ThreadMain()
{
    ...
    if (CurrentIOThread::IsSet())
    {
        file_descriptor_watcher =
                std::make_unique<FileDescriptorWatcher>(delegate_->GetDefaultTaskRunner());
    }
#endif
    ...
    if (com_status_ != NONE)
    {
        com_initializer.reset((com_status_ == STA) ? new win::ScopedCOMInitializer()
