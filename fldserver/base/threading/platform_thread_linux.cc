...
FilePath
ThreadPriorityToCgroupDirectory(const FilePath& cgroup_filepath, ThreadPriority priority)
{
    switch (priority)
    {
        case ThreadPriority::NORMAL: return cgroup_filepath;
        case ThreadPriority::BACKGROUND:
            return cgroup_filepath.Append(FILE_PATH_LITERAL("non-urgent"));
        case ThreadPriority::DISPLAY: FALLTHROUGH;
...
void
SetThreadCgroup(PlatformThreadId thread_id, const FilePath& cgroup_directory)
{
...
void
SetThreadCgroupForThreadPriority(PlatformThreadId thread_id,
...
// FindThreadID).
void
SetThreadLatencySensitivity(ProcessId process_id,
                            ...
                            ThreadPriority priority)
{
    ...
    if (sched_getattr(thread_id, &attr, sizeof(attr), 0) == -1 || attr.size != sizeof(attr))
        return;

    switch (priority)
    {
        case ThreadPriority::NORMAL: FALLTHROUGH;
        case ThreadPriority::BACKGROUND: break;
        case ThreadPriority::DISPLAY:
            // Display needs a boost for consistent 60 fps compositing.
            FALLTHROUGH;
        case ThreadPriority::REALTIME_AUDIO: is_urgent = true; break;
    }

    if (is_urgent && latency_sensitive_urgent)
    {
        PLOG_IF(ERROR, !WriteFile(latency_sensitive_file, "1", 1)) << "Failed to write latency "
                                                                      "file.\n";
    }
    else
    {
        PLOG_IF(ERROR, !WriteFile(latency_sensitive_file, "0", 1)) << "Failed to write latency "
                                                                      "file.\n";
    }
    ...
    if (sched_setattr(thread_id, &attr, 0) == -1)
    {
        ...
        PLOG_IF(ERROR, errno != E2BIG) << "Failed to set sched_util_min, performance may be "
                                          "effected.\n";
    }
}
#endif
...
void
SetThreadCgroupsForThreadPriority(PlatformThreadId thread_id, ThreadPriority priority)
{
...
// static
void
PlatformThread::SetThreadPriority(ProcessId process_id,
                                  PlatformThreadId thread_id,
                                  ThreadPriority priority)
{
...
#if 0 || 0
void
PlatformThread::InitThreadPostFieldTrial()
{
    ...
    if (CommandLine::ForCurrentProcess()->HasSwitch(switches::kSchedulerBoostUrgent))
    {
        std::string boost_switch_str = CommandLine::ForCurrentProcess()->GetSwitchValueASCII(
                switches::kSchedulerBoostUrgent);

        int boost_switch_val;
        if (!StringToInt(boost_switch_str, &boost_switch_val) || boost_switch_val < 0
            || boost_switch_val > 100)
        {
    ...
    g_scheduler_boost_adj =
            GetFieldTrialParamByFeatureAsInt(kSchedUtilHints, "BoostUrgent", boost_def);
    g_scheduler_limit_adj =
            GetFieldTrialParamByFeatureAsInt(kSchedUtilHints, "LimitNonUrgent", kSchedulerLimitDef);
