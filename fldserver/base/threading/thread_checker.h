...
#define THREAD_CHECKER_INTERNAL_CONCAT(a, b) THREAD_CHECKER_INTERNAL_CONCAT2(a, b)
#define THREAD_CHECKER_INTERNAL_UID(prefix) THREAD_CHECKER_INTERNAL_CONCAT(prefix, __LINE__)
...
public:
    ...
    operator=(ThreadCheckerDoNothing&& other) = default;

    bool CalledOnValidThread(std::unique_ptr<void*> = nullptr) const WARN_UNUSED_RESULT
    {
        return true;
    }
    void
...
public:
    ...
    explicit ScopedValidateThreadChecker(const ThreadChecker& checker, const StringPiece& msg)
            EXCLUSIVE_LOCK_FUNCTION(checker)
    {
        std::unique_ptr<debug::StackTrace> bound_at;
        DCHECK(checker.CalledOnValidThread(&bound_at))
