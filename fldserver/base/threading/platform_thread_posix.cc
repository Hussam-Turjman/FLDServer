...
#if !IS_APPLE && !defined(OS_FUCHSIA) && !defined(OS_NACL)
#include "fldserver/base/posix/can_lower_nice_to.h"
#endif
...
bool
CreateThread(size_t stack_size,
             bool joinable,
             PlatformThread::Delegate* delegate,
...
// implementations.
#if !IS_APPLE && !defined(OS_FUCHSIA)
...
PlatformThread::SetCurrentThreadPriorityImpl(ThreadPriority priority)
{
    ...
    {
        DVPLOG(1) << "Failed to set nice value of thread (" << PlatformThread::CurrentId()
                  << ") to " << nice_setting;
    }
#endif  // defined(OS_NACL)
}
...
#endif  // !IS_APPLE && !defined(OS_FUCHSIA)
