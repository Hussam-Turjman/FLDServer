...
#include "fldserver/base/memory/aligned_memory.h"
...
struct DefaultCrossThreadBindTraits
{
    ...
    {
        return base::BindOnce(std::forward<Functor>(functor), std::forward<Args>(args)...);
    }
    ...
    template <typename Signature>
    static inline bool
    PostTask(SequencedTaskRunner& task_runner,
    ...
    static inline bool
    PostTaskAndReply(SequencedTaskRunner& task_runner,
                     const Location& location,
                     OnceClosure&& task,
                     OnceClosure&& reply)
    {
        return task_runner.PostTaskAndReply(location, std::move(task), std::move(reply));
    }

    template <typename TaskReturnType, typename ReplyArgType>
    static inline bool
    PostTaskAndReplyWithResult(SequencedTaskRunner& task_runner,
                               const Location& location,
                               OnceCallback<TaskReturnType()>&& task,
                               OnceCallback<void(ReplyArgType)>&& reply)
    {
        return task_runner.PostTaskAndReplyWithResult(location, std::move(task), std::move(reply));
    }
...
template <typename T, class CrossThreadBindTraits = internal::DefaultCrossThreadBindTraits>
class SequenceBound
{
public:
    template <typename Signature>
    using CrossThreadTask = typename CrossThreadBindTraits::template CrossThreadTask<Signature>;
    ...
    explicit SequenceBound(scoped_refptr<SequencedTaskRunner> task_runner, Args&&... args) :
        impl_task_runner_(std::move(task_runner))
    {
        ...
        storage_ = AlignedAlloc(sizeof(T), sizeof(void*) > alignof(T) ? sizeof(void*) : alignof(T));
    ...
    SequenceBound&
    operator=(const SequenceBound&) = delete;

    // Move construction and assignment.
    SequenceBound(SequenceBound&& other)
    {
        MoveRecordFrom(other);
    }

    SequenceBound&
    operator=(SequenceBound&& other)
    {
        Reset();
        MoveRecordFrom(other);
        return *this;
    ...
    template <typename From>
    SequenceBound&
    operator=(SequenceBound<From, CrossThreadBindTraits>&& other)
    {
        Reset();
        MoveRecordFrom(other);
        return *this;
    ...
    auto
    AsyncCall(R (T::*method)(Args...), const Location& location = Location::Current()) const
    {
        return AsyncCallBuilder<R (T::*)(Args...), R, std::tuple<Args...>>(this, &location, method);
    }

    template <typename R, typename... Args>
    auto
    AsyncCall(R (T::*method)(Args...) const, const Location& location = Location::Current()) const
    {
    ...
    template <typename R, typename... Args>
    auto
    AsyncCall(internal::IgnoreResultHelper<R (T::*)(Args...) const> method,
              const Location& location = Location::Current()) const
    {
        return AsyncCallBuilder<internal::IgnoreResultHelper<R (T::*)(Args...) const>,
                                void,
    ...
    template <typename R, typename... Args>
    auto
    AsyncCall(internal::IgnoreResultHelper<R (T::*)(Args...)> method,
              const Location& location = Location::Current()) const
    {
    ...
    void
    PostTaskWithThisObject(const Location& from_here, ConstPostTaskCallback callback) const
    {
        DCHECK(!is_null());
        ...
        CrossThreadBindTraits::PostTask(*impl_task_runner_,
                                        from_here,
    ...
    void
    PostTaskWithThisObject(const Location& from_here, PostTaskCallback callback) const
    {
        DCHECK(!is_null());
        CrossThreadBindTraits::PostTask(
                *impl_task_runner_,
    ...
    Reset()
    {
        ...
        CrossThreadBindTraits::PostTask(
                *impl_task_runner_,
                FROM_HERE,
                CrossThreadBindTraits::BindOnce(&DeleteOwnerRecord,
                                                CrossThreadBindTraits::Unretained(t_),
        ...
        CrossThreadBindTraits::PostTask(
                *impl_task_runner_,
                ...
                CrossThreadBindTraits::BindOnce(
                        [](T* t, void* storage) { DeleteOwnerRecord(t, storage); }, t_, storage_)
...
private:
    ...
    using EnableIfIsCrossThreadTask =
            typename CrossThreadBindTraits::template EnableIfIsCrossThreadTask<CallbackType>;
    ...
    protected:
        AsyncCallBuilderBase(const SequenceBound* sequence_bound,
                             const Location* location,
                             MethodRef method) :
            sequence_bound_(sequence_bound), location_(location), method_(method)
        {
        ...
        AsyncCallBuilderBase&
        operator=(AsyncCallBuilderBase&&) = default;
    ...
    public:
        ...
        ~AsyncCallBuilderImpl()
        {
            ...
            {
                CrossThreadBindTraits::PostTask(
                        *this->sequence_bound_->impl_task_runner_,
                        *this->location_,
                        CrossThreadBindTraits::BindOnce(
                                this->method_,
        ...
        void
        Then(OnceClosure then_callback) &&
        {
    ...
    private:
        ...
        AsyncCallBuilderImpl&
        operator=(AsyncCallBuilderImpl&&) = default;
    };
    ...
    public:
        ...
        void
        Then(CallbackType<void(ThenArg)> then_callback) &&
        {
            this->sequence_bound_->PostTaskAndThenHelper(
                    *this->location_,
                    CrossThreadBindTraits::BindOnce(
    ...
    protected:
        AsyncCallWithBoundArgsBuilderBase(const SequenceBound* sequence_bound,
            ...
            sequence_bound_(sequence_bound), location_(location), callback_(std::move(callback))
        {
            DCHECK(sequence_bound_);
            DCHECK(sequence_bound_->t_);
        }
        ...
        // move.
        AsyncCallWithBoundArgsBuilderBase(AsyncCallWithBoundArgsBuilderBase&&) noexcept = default;
        AsyncCallWithBoundArgsBuilderBase&
        operator=(AsyncCallWithBoundArgsBuilderBase&&) noexcept = default;
    ...
    public:
        ...
        {
            this->sequence_bound_->PostTaskAndThenHelper(
                    *this->location_, std::move(this->callback_), std::move(then_callback));
            this->sequence_bound_ = nullptr;
        }

    protected:
        using AsyncCallWithBoundArgsBuilderBase<ReturnType>::AsyncCallWithBoundArgsBuilderBase;

    private:
        friend SequenceBound;

        AsyncCallWithBoundArgsBuilderDefault(AsyncCallWithBoundArgsBuilderDefault&&) = default;
        AsyncCallWithBoundArgsBuilderDefault&
        operator=(AsyncCallWithBoundArgsBuilderDefault&&) = default;
    };

    class AsyncCallWithBoundArgsBuilderVoid : public AsyncCallWithBoundArgsBuilderBase<void>
    {
    public:
        // Note: despite being here, this is actually still protected, since it is
        // protected on the base class.
        using AsyncCallWithBoundArgsBuilderBase<void>::AsyncCallWithBoundArgsBuilderBase;

        ~AsyncCallWithBoundArgsBuilderVoid()
        {
            if (this->sequence_bound_)
            {
                CrossThreadBindTraits::PostTask(*this->sequence_bound_->impl_task_runner_,
                                                *this->location_,
        ...
        void
        Then(CrossThreadTask<void()> then_callback) &&
        {
            this->sequence_bound_->PostTaskAndThenHelper(
                    *this->location_, std::move(this->callback_), std::move(then_callback));
            this->sequence_bound_ = nullptr;
    ...
    private:
        ...
        AsyncCallWithBoundArgsBuilderVoid(AsyncCallWithBoundArgsBuilderVoid&&) = default;
        AsyncCallWithBoundArgsBuilderVoid&
        operator=(AsyncCallWithBoundArgsBuilderVoid&&) = default;
    };

    template <typename ReturnType>
    using AsyncCallWithBoundArgsBuilder =
            typename std::conditional<std::is_void<ReturnType>::value,
                                      AsyncCallWithBoundArgsBuilderVoid,
                                      AsyncCallWithBoundArgsBuilderDefault<ReturnType>>::type;

    void
    PostTaskAndThenHelper(const Location& location,
                          CrossThreadTask<void()> callback,
                          CrossThreadTask<void()> then_callback) const
    {
        CrossThreadBindTraits::PostTaskAndReply(
                *impl_task_runner_, location, std::move(callback), std::move(then_callback));
    }
    ...
    {
        CrossThreadTask<void(ThenArg)>&& once_then_callback = std::move(then_callback);
        CrossThreadBindTraits::PostTaskAndReplyWithResult(
                *impl_task_runner_, location, std::move(callback), std::move(once_then_callback));
    }
    ...
    template <typename... Args>
    static void
    ConstructOwnerRecord(T* t, std::decay_t<Args>&&... args)
    {
        new (t) T(std::move(args)...);
    }

    static void
    DeleteOwnerRecord(T* t, void* storage)
    {
        t->~T();
        AlignedFree(storage);
    }
