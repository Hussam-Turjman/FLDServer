...
void CORE_EXPORT EnableIOJankMonitoringForProcess(IOJankReportingCallback);
...
class CORE_EXPORT BlockingObserver
{
public:
    ...
    virtual void
    BlockingStarted(BlockingType blocking_type) = 0;
...
class CORE_EXPORT IOJankMonitoringWindow : public RefCountedThreadSafe<IOJankMonitoringWindow>
{
public:
    ...
    operator=(const IOJankMonitoringWindow&) = delete;

    // Cancels monitoring and clears this class' static state.
    static void
    CancelMonitoringForTesting();
    ...
    public:
        ...
        ScopedMonitoredCall&
        operator=(const ScopedMonitoredCall&) = delete;
...
private:
    ...
    static scoped_refptr<IOJankMonitoringWindow>
    MonitorNextJankWindowIfNecessary(TimeTicks recent_now);
    ...
    void
    OnBlockingCallCompleted(TimeTicks call_start, TimeTicks call_end);
    ...
    void
    AddJank(int local_jank_start_index, int num_janky_intervals);

    static Lock&
    current_jank_window_lock();
    static scoped_refptr<IOJankMonitoringWindow>&
    current_jank_window_storage() EXCLUSIVE_LOCKS_REQUIRED(current_jank_window_lock());

    // Storage for callback used to report monitoring results.
    // NullCallback if monitoring was not enabled for this process.
    static IOJankReportingCallback&
    reporting_callback_storage() EXCLUSIVE_LOCKS_REQUIRED(current_jank_window_lock());
...
class CORE_EXPORT UncheckedScopedBlockingCall
{
public:
