...
static_assert(static_cast<int>(TlsVectorState::kMaxValue) <= kVectorStateBitMask,
              "number of states must fit in header");
static_assert(static_cast<int>(TlsVectorState::kUninitialized) == 0, "kUninitialized must be null");
...
void
SetTlsVectorValue(PlatformThreadLocalStorage::TLSKey key,
                  ...
                  TlsVectorState state)
{
    ...
    PlatformThreadLocalStorage::SetTLSValue(
            key,
            reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(tls_data)
                                    | static_cast<uintptr_t>(state)));
}

// Returns the tls vector and current state from the raw tls value.
TlsVectorState
GetTlsVectorStateAndValue(void* tls_value, TlsVectorEntry** entry = nullptr)
{
    if (entry)
    {
        *entry = reinterpret_cast<TlsVectorEntry*>(reinterpret_cast<uintptr_t>(tls_value)
                                                   & ~kVectorStateBitMask);
    }
...
TlsVectorState
GetTlsVectorStateAndValue(PlatformThreadLocalStorage::TLSKey key, TlsVectorEntry** entry = nullptr)
{
    return GetTlsVectorStateAndValue(PlatformThreadLocalStorage::GetTLSValue(key), entry);
}
...
ConstructTlsVector()
{
    ...
    if (key == PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES)
    {
        ...
        PlatformThreadLocalStorage::TLSKey old_key =
                PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES;
        if (!g_native_tls_key.compare_exchange_strong(
                    old_key, key, std::memory_order_relaxed, std::memory_order_relaxed))
        {
...
void
OnThreadExitInternal(TlsVectorEntry* tls_data)
{
    DCHECK(tls_data);
    ...
    while (need_to_scan_destructors)
    {
        ...
        for (int slot = 0; slot < kThreadLocalStorageSize; ++slot)
        {
            ...
            base::ThreadLocalStorage::TLSDestructorFunc destructor = tls_metadata[slot].destructor;
            if (!destructor)
                continue;
...
void
PlatformThreadLocalStorage::OnThreadExit(void* value)
{
...
ThreadLocalStorage::HasBeenDestroyed()
{
    ...
    return state == TlsVectorState::kDestroying || state == TlsVectorState::kDestroyed;
}

void
ThreadLocalStorage::Slot::Initialize(TLSDestructorFunc destructor)
{
    PlatformThreadLocalStorage::TLSKey key = g_native_tls_key.load(std::memory_order_relaxed);
    if (key == PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES
    ...
    // Grab a new slot.
    {
        ...
        for (int i = 0; i < kThreadLocalStorageSize; ++i)
        {
            ...
            size_t slot_candidate = (g_last_assigned_slot + 1 + i) % kThreadLocalStorageSize;
...
void*
ThreadLocalStorage::Slot::Get() const
{
    TlsVectorEntry* tls_data = nullptr;
    const TlsVectorState state =
            GetTlsVectorStateAndValue(g_native_tls_key.load(std::memory_order_relaxed), &tls_data);
    DCHECK_NE(state, TlsVectorState::kDestroyed);
    if (!tls_data)
        return nullptr;
    DCHECK_NE(slot_, kInvalidSlotValue);
...
void
ThreadLocalStorage::Slot::Set(void* value)
{
    TlsVectorEntry* tls_data = nullptr;
    const TlsVectorState state =
            GetTlsVectorStateAndValue(g_native_tls_key.load(std::memory_order_relaxed), &tls_data);
