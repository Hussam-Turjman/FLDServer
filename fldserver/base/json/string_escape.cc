...
bool
EscapeSpecialCodePoint(uint32_t code_point, std::string* dest)
{
    ...
    switch (code_point)
    {
        ...
        case 0x2028: dest->append("\\u2028"); break;
        case 0x2029: dest->append("\\u2029"); break;
...
template <typename S>
bool
EscapeJSONStringImpl(const S& str, bool put_in_quotes, std::string* dest)
{
    ...
    CHECK_LE(str.length(), static_cast<size_t>(std::numeric_limits<int32_t>::max()));
...
bool
EscapeJSONString(StringPiece str, bool put_in_quotes, std::string* dest)
{
    return EscapeJSONStringImpl(str, put_in_quotes, dest);
}

bool
EscapeJSONString(StringPiece16 str, bool put_in_quotes, std::string* dest)
{
    return EscapeJSONStringImpl(str, put_in_quotes, dest);
}

std::string
GetQuotedJSONString(StringPiece str)
{
    std::string dest;
...
std::string
GetQuotedJSONString(StringPiece16 str)
{
    std::string dest;
    EscapeJSONStringImpl(str, true, &dest);
    return dest;
...
std::string
EscapeBytesAsInvalidJSONString(StringPiece str, bool put_in_quotes)
{
    std::string dest;
