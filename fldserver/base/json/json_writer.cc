...
// static
bool
JSONWriter::Write(const Value& node, std::string* json, size_t max_depth)
{
...
// static
bool
JSONWriter::WriteWithOptions(const Value& node, int options, std::string* json, size_t max_depth)
{
    json->clear();
...
JSONWriter::JSONWriter(int options, std::string* json, size_t max_depth) :
    ...
    omit_double_type_preservation_((options & OPTIONS_OMIT_DOUBLE_TYPE_PRESERVATION) != 0),
...
bool
JSONWriter::BuildJSONString(const Value& node, size_t depth)
{
    internal::StackMarker depth_check(max_depth_, &stack_depth_);

    switch (node.type())
    {
        case Value::Type::NONE: json_string_->append("null"); return true;
        ...
        case Value::Type::INTEGER: json_string_->append(NumberToString(node.GetInt())); return true;

        case Value::Type::DOUBLE: {
            double value = node.GetDouble();
            if (omit_double_type_preservation_ && value <= std::numeric_limits<int64_t>::max()
                && value >= std::numeric_limits<int64_t>::min() && std::floor(value) == value)
            {
...
void
JSONWriter::IndentLine(size_t depth)
{
