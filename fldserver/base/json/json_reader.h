...
class CORE_EXPORT JSONReader
{
public:
    struct CORE_EXPORT ValueWithError
    {
        ValueWithError();
        ValueWithError(ValueWithError&& other);
        ValueWithError&
        operator=(ValueWithError&& other);
        ~ValueWithError();
    ...
    ReadDeprecated(StringPiece json,
                   int options = JSON_PARSE_RFC,
                   size_t max_depth = internal::kAbsoluteMaxDepth);
    ...
    static ValueWithError
    ReadAndReturnValueWithError(StringPiece json, int options = JSON_PARSE_RFC);
