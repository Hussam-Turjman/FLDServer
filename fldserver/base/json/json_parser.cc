...
#include "fldserver/base/json/json_parser.h"
...
static_assert(JSONParser::JSON_PARSE_ERROR_COUNT < 1000, "JSONParser error out of bounds");

std::string
ErrorCodeToString(JSONParser::JsonParseError error_code)
{
    switch (error_code)
    {
        case JSONParser::JSON_NO_ERROR: return std::string();
        case JSONParser::JSON_SYNTAX_ERROR: return JSONParser::kSyntaxError;
        case JSONParser::JSON_INVALID_ESCAPE: return JSONParser::kInvalidEscape;
        case JSONParser::JSON_UNEXPECTED_TOKEN: return JSONParser::kUnexpectedToken;
        case JSONParser::JSON_TRAILING_COMMA: return JSONParser::kTrailingComma;
        case JSONParser::JSON_TOO_MUCH_NESTING: return JSONParser::kTooMuchNesting;
        case JSONParser::JSON_UNEXPECTED_DATA_AFTER_ROOT:
            return JSONParser::kUnexpectedDataAfterRoot;
        case JSONParser::JSON_UNSUPPORTED_ENCODING: return JSONParser::kUnsupportedEncoding;
        case JSONParser::JSON_UNQUOTED_DICTIONARY_KEY: return JSONParser::kUnquotedDictionaryKey;
        case JSONParser::JSON_TOO_LARGE: return JSONParser::kInputTooLarge;
        case JSONParser::JSON_UNREPRESENTABLE_NUMBER: return JSONParser::kUnrepresentableNumber;
        case JSONParser::JSON_PARSE_ERROR_COUNT: break;
    }
    NOTREACHED();
    return std::string();
}
...
// permitted.
bool
UnprefixedHexStringToInt(StringPiece input, int* output)
{
...
const char JSONParser::kUnexpectedDataAfterRoot[] = "Unexpected data after root element.";
const char JSONParser::kUnsupportedEncoding[] = "Unsupported encoding. JSON must be UTF-8.";
const char JSONParser::kUnquotedDictionaryKey[] = "Dictionary keys must be quoted.";
const char JSONParser::kInputTooLarge[] = "Input string is too large (>2GB).";
const char JSONParser::kUnrepresentableNumber[] = "Number cannot be represented.";
...
Optional<Value>
JSONParser::Parse(StringPiece input)
{
    input_ = input;
    index_ = 0;
...
{
    return FormatErrorMessage(error_line_, error_column_, ErrorCodeToString(error_code_));
}

int
JSONParser::error_line() const
{
    return error_line_;
}

int
JSONParser::error_column() const
{
...
JSONParser::StringBuilder::StringBuilder(const char* pos) : pos_(pos), length_(0)
{
}

JSONParser::StringBuilder::~StringBuilder() = default;

JSONParser::StringBuilder&
JSONParser::StringBuilder::operator=(StringBuilder&& other) = default;

void
JSONParser::StringBuilder::Append(uint32_t point)
{
...
void
JSONParser::StringBuilder::Convert()
{
    if (string_)
        return;
...
std::string
JSONParser::StringBuilder::DestructiveAsString()
{
    if (string_)
...
Optional<StringPiece>
JSONParser::PeekChars(size_t count)
{
...
Optional<StringPiece>
JSONParser::ConsumeChars(size_t count)
{
...
JSONParser::GetNextToken()
{
    ...
    switch (*c)
    {
        case '{': return T_OBJECT_BEGIN;
        case '}': return T_OBJECT_END;
        case '[': return T_ARRAY_BEGIN;
        case ']': return T_ARRAY_END;
        ...
        case 't': return T_BOOL_TRUE;
        case 'f': return T_BOOL_FALSE;
        case 'n': return T_NULL;
        case ',': return T_LIST_SEPARATOR;
        case ':': return T_OBJECT_PAIR_SEPARATOR;
        default: return T_INVALID_TOKEN;
    }
}

void
JSONParser::EatWhitespaceAndComments()
{
...
JSONParser::ParseToken(Token token)
{
    switch (token)
    {
        case T_OBJECT_BEGIN: return ConsumeDictionary();
        case T_ARRAY_BEGIN: return ConsumeList();
        case T_STRING: return ConsumeString();
        case T_NUMBER: return ConsumeNumber();
        case T_BOOL_TRUE:
        case T_BOOL_FALSE:
        case T_NULL: return ConsumeLiteral();
        default: ReportError(JSON_UNEXPECTED_TOKEN, 0); return nullopt;
    }
}
...
bool
JSONParser::ConsumeStringRaw(StringBuilder* out)
{
    if (ConsumeChar() != '"')
    {
        ReportError(JSON_UNEXPECTED_TOKEN, 0);
    ...
    while (PeekChar())
    {
        ...
        if (!ReadUnicodeCharacter(
                    input_.data(), static_cast<int32_t>(input_.length()), &index_, &next_char)
        ...
        else
        {
            ...
            switch ((*escape_sequence)[1])
            {
                ...
                default: ReportError(JSON_INVALID_ESCAPE, -1); return false;
            }
        }
    }
...
bool
JSONParser::DecodeUTF16(uint32_t* out_code_point)
{
...
JSONParser::ConsumeNumber()
{
    ...
    switch (GetNextToken())
    {
        ...
        case T_END_OF_INPUT: break;
        default: ReportError(JSON_SYNTAX_ERROR, 0); return nullopt;
    }
...
bool
JSONParser::ReadInt(bool allow_leading_zeros)
{
    size_t len = 0;
    char first = 0;
...
void
JSONParser::ReportError(JsonParseError code, int column_adjust)
{
    error_code_ = code;
...
// static
std::string
JSONParser::FormatErrorMessage(int line, int column, const std::string& description)
{
    if (line || column)
    {
        return StringPrintf("Line: %i, column: %i, %s", line, column, description.c_str());
    }
    return description;
}
