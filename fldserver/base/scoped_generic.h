...
public:
    ...
    ScopedGeneric(const element_type& value, const traits_type& traits) : data_(value, traits)
    {
        TrackAcquire(data_.generic);
    }

    // Move constructor. Allows initialization from a ScopedGeneric rvalue.
    ScopedGeneric(ScopedGeneric<T, Traits>&& rvalue) : data_(rvalue.release(), rvalue.get_traits())
    {
        TrackAcquire(data_.generic);
    }
    ScopedGeneric(const ScopedGeneric&) = delete;
    ScopedGeneric&
    operator=(const ScopedGeneric&) = delete;
    ...
    ScopedGeneric&
    operator=(ScopedGeneric<T, Traits>&& rvalue)
    {
    ...
    void
    reset(const element_type& value = traits_type::InvalidValue())
    {
    ...
    public:
        ...
        {
            CHECK(!scoped_generic_->receiving_) << "attempted to construct Receiver for "
                                                   "ScopedGeneric with existing "
                                                   "Receiver";
    ...
    bool
    operator==(const element_type& value) const
    {
...
private:
    ...
    typename std::enable_if_t<std::is_base_of<ScopedGenericOwnershipTracking, Traits>::value, Void>
    ...
    bool
    operator==(const ScopedGeneric<T2, Traits2>& p2) const;
    template <typename T2, typename Traits2>
    bool
    operator!=(const ScopedGeneric<T2, Traits2>& p2) const;
...
void
swap(const ScopedGeneric<T, Traits>& a, const ScopedGeneric<T, Traits>& b)
{
    a.swap(b);
}
...
template <class T, class Traits>
bool
operator==(const T& value, const ScopedGeneric<T, Traits>& scoped)
{
