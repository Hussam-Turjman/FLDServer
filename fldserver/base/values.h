...
#include "fldserver/base/containers/checked_range.h"
...
#include "fldserver/base/value_iterators.h"
...
public:
    ...
    static Value
    FromUniquePtrValue(std::unique_ptr<Value> val);
    static std::unique_ptr<Value>
    ToUniquePtrValue(Value val);
    static const DictionaryValue&
    AsDictionaryValue(const Value& val);
    static const ListValue&
    AsListValue(const Value& val);
    ...
    GetInt() const;
    double
    GetDouble() const;  // Implicitly converts from int if necessary.
    const std::string&
    GetString() const;
    std::string&
    GetString();
    ...
    void
    Append(StringPiece value);
    void
    Append(std::string&& value);
    void
    Append(const char16_t* value);
    ...
    Insert(CheckedContiguousConstIterator<Value> pos, Value&& value);

    // Erases the Value pointed to by `iter`. Returns false if `iter` is out of
    // bounds.
    // Note: This requires that `type()` is Type::LIST.
    bool
    EraseListIter(CheckedContiguousConstIterator<Value> iter);

    // Erases all Values that compare equal to `val`. Returns the number of
    // deleted Values.
    // Note: This requires that `type()` is Type::LIST.
    size_t
    EraseListValue(const Value& val);
    ...
    size_t
    EraseListValueIf(Predicate pred)
    {
    ...
    Value*
    FindKey(StringPiece key);
    const Value*
    FindKey(StringPiece key) const;
    ...
    Value*
    FindKeyOfType(StringPiece key, Type type);
    const Value*
    FindKeyOfType(StringPiece key, Type type) const;
    ...
    Value*
    SetBoolKey(StringPiece key, bool val);
    Value*
    SetIntKey(StringPiece key, int val);
    Value*
    SetDoubleKey(StringPiece key, double val);
    Value*
    SetStringKey(StringPiece key, StringPiece val);
    Value*
    SetStringKey(StringPiece key, StringPiece16 val);
    // NOTE: The following two overloads are provided as performance / code
    // generation optimizations.
    Value*
    SetStringKey(StringPiece key, const char* val);
    Value*
    SetStringKey(StringPiece key, std::string&& val);
    ...
    Value*
    FindPath(StringPiece path);
    const Value*
    FindPath(StringPiece path) const;
    ...
    Value*
    FindPath(std::initializer_list<StringPiece> path);
    Value*
    FindPath(span<const StringPiece> path);
    const Value*
    FindPath(std::initializer_list<StringPiece> path) const;
    const Value*
    FindPath(span<const StringPiece> path) const;
    ...
    Value*
    FindPathOfType(StringPiece path, Type type);
    const Value*
    FindPathOfType(StringPiece path, Type type) const;
    ...
    base::Optional<bool>
    FindBoolPath(StringPiece path) const;
    base::Optional<int>
    FindIntPath(StringPiece path) const;
    base::Optional<double>
    FindDoublePath(StringPiece path) const;
    const std::string*
    FindStringPath(StringPiece path) const;
    std::string*
    FindStringPath(StringPiece path);
    const BlobStorage*
    FindBlobPath(StringPiece path) const;
    Value*
    FindDictPath(StringPiece path);
    const Value*
    FindDictPath(StringPiece path) const;
    Value*
    FindListPath(StringPiece path);
    const Value*
    FindListPath(StringPiece path) const;

    // The following forms are deprecated too, use the ones that take the path
    // as a single StringPiece instead.
    Value*
    FindPathOfType(std::initializer_list<StringPiece> path, Type type);
    Value*
    FindPathOfType(span<const StringPiece> path, Type type);
    const Value*
    FindPathOfType(std::initializer_list<StringPiece> path, Type type) const;
    const Value*
    FindPathOfType(span<const StringPiece> path, Type type) const;
    ...
    Value*
    SetPath(StringPiece path, Value&& value);

    // These setters are more convenient and efficient than the corresponding
    // SetPath(...) call.
    Value*
    SetBoolPath(StringPiece path, bool value);
    Value*
    SetIntPath(StringPiece path, int value);
    Value*
    SetDoublePath(StringPiece path, double value);
    Value*
    SetStringPath(StringPiece path, StringPiece value);
    Value*
    SetStringPath(StringPiece path, const char* value);
    Value*
    SetStringPath(StringPiece path, std::string&& value);
    Value*
    SetStringPath(StringPiece path, StringPiece16 value);

    // Deprecated: use the `SetPath(StringPiece, ...)` methods above instead.
    Value*
    SetPath(std::initializer_list<StringPiece> path, Value&& value);
    Value*
    SetPath(span<const StringPiece> path, Value&& value);
    ...
    // Type::DICTIONARY.
    void
    MergeDictionary(const Value* dictionary);
    ...
    bool
    GetAsBoolean(bool* out_value) const;
    // DEPRECATED, use `GetIfInt()` instead.
    bool
    GetAsInteger(int* out_value) const;
    // DEPRECATED, use `GetIfDouble()` instead.
    bool
    GetAsDouble(double* out_value) const;
    // DEPRECATED, use `GetIfString()` instead.
    bool
    GetAsString(std::string* out_value) const;
    bool
    GetAsString(std::u16string* out_value) const;
    bool
    GetAsString(const Value** out_value) const;
    bool
    GetAsString(StringPiece* out_value) const;
    // ListValue::From is the equivalent for std::unique_ptr conversions.
    // DEPRECATED, use `is_list()` instead.
    bool
    GetAsList(ListValue** out_value);
    bool
    GetAsList(const ListValue** out_value) const;
    // DictionaryValue::From is the equivalent for std::unique_ptr conversions.
    bool
    GetAsDictionary(DictionaryValue** out_value);
    bool
    GetAsDictionary(const DictionaryValue** out_value) const;
    // Note: Do not add more types. See the file-level comment above for why.
    ...
    // TODO(crbug.com/646113): Delete this and migrate callsites.
    bool
    Equals(const Value* other) const;
...
protected:
    ...
    list() const
    {
        return absl::get<ListStorage>(data_);
    }
    ListStorage&
    list()
    {
...
private:
    ...
    using DoubleStorage = struct
    {
        alignas(4) char v[sizeof(double)];
    };
    ...
    double
    AsDoubleInternal() const;

    // NOTE: Using a movable reference here is done for performance (it avoids
    // creating + moving + destroying a temporary unique ptr).
    Value*
    SetKeyInternal(StringPiece key, std::unique_ptr<Value>&& val_ptr);
    Value*
    SetPathInternal(StringPiece path, std::unique_ptr<Value>&& value_ptr);
...
public:
    ...
    static std::unique_ptr<DictionaryValue>
    From(std::unique_ptr<Value> value);
    ...
    Value*
    Set(StringPiece path, std::unique_ptr<Value> in_value);

    // Convenience forms of Set().  These methods will replace any existing
    // value at that path, even if it has a different type.
    // DEPRECATED, use `Value::SetBoolKey()` or `Value::SetBoolPath()`.
    Value*
    SetBoolean(StringPiece path, bool in_value);
    // DEPRECATED, use `Value::SetIntPath()`.
    Value*
    SetInteger(StringPiece path, int in_value);
    // DEPRECATED, use `Value::SetDoublePath()`.
    Value*
    SetDouble(StringPiece path, double in_value);
    // DEPRECATED, use `Value::SetStringPath()`.
    Value*
    SetString(StringPiece path, StringPiece in_value);
    // DEPRECATED, use `Value::SetStringPath()`.
    Value*
    SetString(StringPiece path, const std::u16string& in_value);
    // DEPRECATED, use `Value::SetPath()`.
    DictionaryValue*
    SetDictionary(StringPiece path, std::unique_ptr<DictionaryValue> in_value);
    // DEPRECATED, use `Value::SetPath()`.
    ListValue*
    SetList(StringPiece path, std::unique_ptr<ListValue> in_value);

    // Like Set(), but without special treatment of '.'.  This allows e.g. URLs to
    // be used as paths.
    // DEPRECATED, use `Value::SetKey(key, value)` instead.
    Value*
    SetWithoutPathExpansion(StringPiece key, std::unique_ptr<Value> in_value);
    ...
    bool
    Get(StringPiece path, const Value** out_value) const;
    // DEPRECATED, use `Value::FindPath(path)` instead.
    bool
    Get(StringPiece path, Value** out_value);
    ...
    bool
    GetBoolean(StringPiece path, bool* out_value) const;
    // DEPRECATED, use `Value::FindIntPath(path)` instead.
    bool
    GetInteger(StringPiece path, int* out_value) const;
    // Values of both type Type::INTEGER and Type::DOUBLE can be obtained as
    // doubles.
    // DEPRECATED, use `Value::FindDoublePath(path)`.
    bool
    GetDouble(StringPiece path, double* out_value) const;
    // DEPRECATED, use `Value::FindStringPath(path)` instead.
    bool
    GetString(StringPiece path, std::string* out_value) const;
    // DEPRECATED, use `Value::FindStringPath(path)` instead.
    bool
    GetString(StringPiece path, std::u16string* out_value) const;
    // DEPRECATED, use `Value::FindString(path)` and `IsAsciiString()` instead.
    bool
    GetStringASCII(StringPiece path, std::string* out_value) const;
    // DEPRECATED, use `Value::FindBlobPath(path)` instead.
    bool
    GetBinary(StringPiece path, const Value** out_value) const;
    // DEPRECATED, use `Value::FindBlobPath(path)` instead.
    bool
    GetBinary(StringPiece path, Value** out_value);
    // DEPRECATED, use `Value::FindPath(path)` and Value's Dictionary API
    // instead.
    bool
    GetDictionary(StringPiece path, const DictionaryValue** out_value) const;
    // DEPRECATED, use `Value::FindPath(path)` and Value's Dictionary API
    // instead.
    bool
    GetDictionary(StringPiece path, DictionaryValue** out_value);
    // DEPRECATED, use `Value::FindPath(path)` and `Value::GetList()` instead.
    bool
    GetList(StringPiece path, const ListValue** out_value) const;
    // DEPRECATED, use `Value::FindPath(path)` and `Value::GetList()` instead.
    bool
    GetList(StringPiece path, ListValue** out_value);

    // Like `Get()`, but without special treatment of '.'.  This allows e.g. URLs
    // to be used as paths.
    // DEPRECATED, use `Value::FindKey(key)` instead.
    bool
    GetWithoutPathExpansion(StringPiece key, const Value** out_value) const;
    // DEPRECATED, use `Value::FindKey(key)` instead.
    bool
    GetWithoutPathExpansion(StringPiece key, Value** out_value);
    // DEPRECATED, use `Value::FindBoolKey(key)` instead.
    bool
    GetBooleanWithoutPathExpansion(StringPiece key, bool* out_value) const;
    // DEPRECATED, use `Value::FindIntKey(key)` instead.
    bool
    GetIntegerWithoutPathExpansion(StringPiece key, int* out_value) const;
    // DEPRECATED, use `Value::FindDoubleKey(key)` instead.
    bool
    GetDoubleWithoutPathExpansion(StringPiece key, double* out_value) const;
    // DEPRECATED, use `Value::FindStringKey(key)` instead.
    bool
    GetStringWithoutPathExpansion(StringPiece key, std::string* out_value) const;
    // DEPRECATED, use `Value::FindStringKey(key)` and UTF8ToUTF16() instead.
    bool
    GetStringWithoutPathExpansion(StringPiece key, std::u16string* out_value) const;
    // DEPRECATED, use `Value::FindDictKey(key)` instead.
    bool
    GetDictionaryWithoutPathExpansion(StringPiece key, const DictionaryValue** out_value) const;
    // DEPRECATED, use `Value::FindDictKey(key)` instead.
    bool
    GetDictionaryWithoutPathExpansion(StringPiece key, DictionaryValue** out_value);
    // DEPRECATED, use `Value::FindListKey(key)` instead.
    bool
    GetListWithoutPathExpansion(StringPiece key, const ListValue** out_value) const;
    // DEPRECATED, use `Value::FindListKey(key)` instead.
    bool
    GetListWithoutPathExpansion(StringPiece key, ListValue** out_value);
    ...
    // instead.
    bool
    Remove(StringPiece path, std::unique_ptr<Value>* out_value);
    ...
    // instead.
    bool
    RemoveWithoutPathExpansion(StringPiece key, std::unique_ptr<Value>* out_value);
    ...
    // instead.
    bool
    RemovePath(StringPiece path, std::unique_ptr<Value>* out_value);
...
public:
    ...
    static std::unique_ptr<ListValue>
    From(std::unique_ptr<Value> value);
    ...
    bool
    Set(size_t index, std::unique_ptr<Value> in_value);
    ...
    // DEPRECATED, use `GetList()::operator[] instead.
    bool
    Get(size_t index, const Value** out_value) const;
    bool
    Get(size_t index, Value** out_value);
    ...
    bool
    GetBoolean(size_t index, bool* out_value) const;
    // DEPRECATED, use `GetList()::operator[]::GetInt()` instead.
    bool
    GetInteger(size_t index, int* out_value) const;
    // Values of both type Type::INTEGER and Type::DOUBLE can be obtained as
    // doubles.
    // DEPRECATED, use `GetList()::operator[]::GetDouble()` instead.
    bool
    GetDouble(size_t index, double* out_value) const;
    // DEPRECATED, use `GetList()::operator[]::GetString()` instead.
    bool
    GetString(size_t index, std::string* out_value) const;
    bool
    GetString(size_t index, std::u16string* out_value) const;

    bool
    GetDictionary(size_t index, const DictionaryValue** out_value) const;
    bool
    GetDictionary(size_t index, DictionaryValue** out_value);

    using Value::GetList;
    // DEPRECATED, use `GetList()::operator[]::GetList()` instead.
    bool
    GetList(size_t index, const ListValue** out_value) const;
    bool
    GetList(size_t index, ListValue** out_value);
    ...
    bool
    Remove(size_t index, std::unique_ptr<Value>* out_value);
    ...
    // DEPRECATED, use `GetList()::erase()` instead.
    bool
    Remove(const Value& value, size_t* index);
    ...
    // DEPRECATED, use `GetList()::erase()` instead.
    iterator
    Erase(iterator iter, std::unique_ptr<Value>* out_value);

    using Value::Append;
    // Appends a Value to the end of the list.
    // DEPRECATED, use `Value::Append()` instead.
    void
    Append(std::unique_ptr<Value> in_value);
    ...
    void
    AppendInteger(int in_value);
    void
    AppendDouble(double in_value);
    void
    AppendString(StringPiece in_value);
    void
    AppendString(const std::u16string& in_value);
    // DEPRECATED, use `Value::Append()` in a loop instead.
    void
    AppendStrings(const std::vector<std::string>& in_values);
    ...
    bool
    AppendIfNotPresent(std::unique_ptr<Value> in_value);
    ...
    bool
    Insert(size_t index, std::unique_ptr<Value> in_value);
...
// Value objects.
class CORE_EXPORT ValueSerializer
{
public:
    virtual ~ValueSerializer();

    virtual bool
    Serialize(const Value& root) = 0;
};

// This interface is implemented by classes that know how to deserialize Value
// objects.
class CORE_EXPORT ValueDeserializer
{
public:
    ...
    Deserialize(int* error_code, std::string* error_message) = 0;
    ...
    static inline bool
    ErrorCodeIsDataError(int error_code)
    {
...
CORE_EXPORT std::ostream&
operator<<(std::ostream& out, const Value& value);

CORE_EXPORT inline std::ostream&
operator<<(std::ostream& out, const DictionaryValue& value)
{
    return out << static_cast<const Value&>(value);
}

CORE_EXPORT inline std::ostream&
operator<<(std::ostream& out, const ListValue& value)
{
    return out << static_cast<const Value&>(value);
}
