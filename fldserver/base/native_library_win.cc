...
// forward declare
HMODULE
AddDllDirectory(PCWSTR new_directory);
...
void
LogLibrarayLoadResultToUMA(LoadLibraryResult result)
{
    UMA_HISTOGRAM_ENUMERATION(
            "LibraryLoader.LoadNativeLibraryWindows", result, LoadLibraryResult::END);
}
...
AreSearchFlagsAvailable()
{
    ...
    // method.
    static const auto add_dll_dir_func = reinterpret_cast<decltype(AddDllDirectory)*>(
...
// LoadLibraryResult.
LoadLibraryResult
GetLoadLibraryResult(bool are_search_flags_available, bool has_load_library_succeeded)
{
...
NativeLibrary
LoadNativeLibraryHelper(const FilePath& library_path, NativeLibraryLoadError* error)
{
    ...
    if (are_search_flags_available)
    {
        ...
        // directory.
        module = ::LoadLibraryExW(library_path.value().c_str(),
                                  nullptr,
    ...
    LogLibrarayLoadResultToUMA(GetLoadLibraryResult(are_search_flags_available, !!module));

    return module;
}

NativeLibrary
LoadSystemLibraryHelper(const FilePath& library_path, NativeLibraryLoadError* error)
{
    ...
    BOOL module_found = ::GetModuleHandleExW(0, library_path.value().c_str(), &module);
    if (!module_found)
    {
        bool are_search_flags_available = AreSearchFlagsAvailable();
...
FilePath
GetSystemLibraryName(FilePath::StringPieceType name)
{
...
void
UnloadNativeLibrary(NativeLibrary library)
{
    FreeLibrary(library);
}

void*
GetFunctionPointerFromNativeLibrary(NativeLibrary library, StringPiece name)
{
    return reinterpret_cast<void*>(GetProcAddress(library, name.data()));
}

std::string
GetNativeLibraryName(StringPiece name)
{
...
std::string
GetLoadableModuleName(StringPiece name)
{
    return GetNativeLibraryName(name);
}

NativeLibrary
LoadSystemLibrary(FilePath::StringPieceType name, NativeLibraryLoadError* error)
{
...
NativeLibrary
PinSystemLibrary(FilePath::StringPieceType name, NativeLibraryLoadError* error)
{
    FilePath library_path = GetSystemLibraryName(name);
    if (library_path.empty())
    {
