...
#include "fldserver/base/posix/unix_domain_socket.h"
...
#if !defined(OS_NACL_NONSFI)
bool
CreateSocketPair(ScopedFD* one, ScopedFD* two)
{
    int raw_socks[2];
#if IS_APPLE
    ...
    if (setsockopt(raw_socks[0], SOL_SOCKET, SO_NOSIGPIPE, &no_sigpipe, sizeof(no_sigpipe)) != 0)
        return false;
    if (setsockopt(raw_socks[1], SOL_SOCKET, SO_NOSIGPIPE, &no_sigpipe, sizeof(no_sigpipe)) != 0)
        return false;
#endif
...
// static
bool
UnixDomainSocket::EnableReceiveProcessId(int fd)
{
#if !IS_APPLE
    const int enable = 1;
...
// static
bool
UnixDomainSocket::SendMsg(int fd, const void* buf, size_t length, const std::vector<int>& fds)
{
    struct msghdr msg = {};
...
// SO_NOSIGPIPE is set during socket creation.
    ...
    DPCHECK(getsockopt(fd, SOL_SOCKET, SO_NOSIGPIPE, &no_sigpipe, &no_sigpipe_len) == 0)
...
// static
ssize_t
UnixDomainSocket::RecvMsg(int fd, void* buf, size_t length, std::vector<ScopedFD>* fds)
{
...
UnixDomainSocket::RecvMsgWithPid(
        int fd, void* buf, size_t length, std::vector<ScopedFD>* fds, ProcessId* pid)
{
...
// static
ssize_t
UnixDomainSocket::RecvMsgWithFlags(
        int fd, void* buf, size_t length, int flags, std::vector<ScopedFD>* fds, ProcessId* out_pid)
{
    fds->clear();
...
#if !defined(OS_NACL_NONSFI) && !IS_APPLE
            // The PNaCl toolchain for Non-SFI binary build and macOS do not support
            // ucred. macOS supports xucred, but this structure is insufficient.
            + CMSG_SPACE(sizeof(struct ucred))
#endif  // !defined(OS_NACL_NONSFI) && !IS_APPLE
            ;
    ...
    if (msg.msg_flags & MSG_TRUNC || msg.msg_flags & MSG_CTRUNC)
    {
        if (msg.msg_flags & MSG_CTRUNC)
        {
            ...
            LOG(ERROR) << "recvmsg returned MSG_CTRUNC flag, buffer len is " << msg.msg_controllen;
        }
...
UnixDomainSocket::SendRecvMsg(
        int fd, uint8_t* reply, unsigned max_reply_len, int* result_fd, const Pickle& request)
{
    return UnixDomainSocket::SendRecvMsgWithFlags(fd,
                                                  reply,
                                                  max_reply_len,
                                                  0, /* recvmsg_flags */
                                                  result_fd,
