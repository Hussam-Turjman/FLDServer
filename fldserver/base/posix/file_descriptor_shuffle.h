...
class CORE_EXPORT FileDescriptorTableInjection : public InjectionDelegate
{
    bool
    Duplicate(int* result, int fd) override;
    bool
    Move(int src, int dest) override;
    void
    Close(int fd) override;
};
...
CORE_EXPORT bool
PerformInjectiveMultimap(const InjectiveMultimap& map, InjectionDelegate* delegate);

CORE_EXPORT bool
PerformInjectiveMultimapDestructive(InjectiveMultimap* map, InjectionDelegate* delegate);

// This function will not call malloc but will mutate |map|
inline bool
ShuffleFileDescriptors(InjectiveMultimap* map)
{
