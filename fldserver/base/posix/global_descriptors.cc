...
GlobalDescriptors::Descriptor::Descriptor(Key key, int fd, base::MemoryMappedFile::Region region) :
...
{
    typedef Singleton<base::GlobalDescriptors, LeakySingletonTraits<base::GlobalDescriptors>>
            GlobalDescriptorsSingleton;
    return GlobalDescriptorsSingleton::get();
}

int
GlobalDescriptors::Get(Key key) const
{
...
int
GlobalDescriptors::MaybeGet(Key key) const
{
...
base::ScopedFD
GlobalDescriptors::TakeFD(Key key, base::MemoryMappedFile::Region* region)
{
    base::ScopedFD fd;
...
void
GlobalDescriptors::Set(Key key, int fd)
{
    Set(key, fd, base::MemoryMappedFile::Region::kWholeFile);
}

void
GlobalDescriptors::Set(Key key, int fd, base::MemoryMappedFile::Region region)
{
...
GlobalDescriptors::GetRegion(Key key) const
{
    for (const auto& i : descriptors_)
    {
        if (i.key == key)
...
void
GlobalDescriptors::Reset(const Mapping& mapping)
{
