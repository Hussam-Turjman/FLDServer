...
CORE_EXPORT char*
CheckOpValueStr(int v);
CORE_EXPORT char*
CheckOpValueStr(unsigned v);
CORE_EXPORT char*
CheckOpValueStr(long v);
CORE_EXPORT char*
CheckOpValueStr(unsigned long v);
CORE_EXPORT char*
CheckOpValueStr(long long v);
CORE_EXPORT char*
CheckOpValueStr(unsigned long long v);
CORE_EXPORT char*
CheckOpValueStr(const void* v);
CORE_EXPORT char*
CheckOpValueStr(std::nullptr_t v);
CORE_EXPORT char*
CheckOpValueStr(double v);

// Convert a streamable value to string out-of-line to avoid <sstream>.
CORE_EXPORT char*
StreamValToStr(const void* v, void (*stream_func)(std::ostream&, const void*));
...
CheckOpValueStr(const T& v)
{
    ...
    const void* vp =
            const_cast<const void*>(reinterpret_cast<const volatile void*>(__builtin_addressof(v)));
#else
...
template <typename T>
inline typename std::enable_if<!base::internal::SupportsOstreamOperator<const T&>::value
...
template <typename T>
inline typename std::enable_if<std::is_function<typename std::remove_pointer<T>::type>::value,
                               char*>::type
CheckOpValueStr(const T& v)
{
...
#define CHECK_OP(name, op, val1, val2)                                                             \
    ...
    default:                                                                                       \
        if (::logging::CheckOpResult true_if_passed =                                              \
            ...
            ::logging::CheckError::CheckOp(__FILE__, __LINE__, &true_if_passed).stream()
...
#define DEFINE_CHECK_OP_IMPL(name, op)                                                             \
    template <typename T,                                                                          \
              typename U,                                                                          \
              std::enable_if_t<!std::is_fundamental<T>::value || !std::is_fundamental<U>::value,   \
                               int> = 0>                                                           \
    ...
    {                                                                                              \
        ...
        return ::logging::CheckOpResult(expr_str, CheckOpValueStr(v1), CheckOpValueStr(v2));       \
    }                                                                                              \
    template <typename T,                                                                          \
              typename U,                                                                          \
              std::enable_if_t<std::is_fundamental<T>::value && std::is_fundamental<U>::value,     \
                               int> = 0>                                                           \
    constexpr ::logging::CheckOpResult Check##name##Impl(T v1, U v2, const char* expr_str)         \
    {                                                                                              \
        if (ANALYZER_ASSUME_TRUE(v1 op v2))                                                        \
            return ::logging::CheckOpResult();                                                     \
        return ::logging::CheckOpResult(expr_str, CheckOpValueStr(v1), CheckOpValueStr(v2));       \
...
#define DCHECK_OP(name, op, val1, val2)                                                            \
    ...
    default:                                                                                       \
        if (::logging::CheckOpResult true_if_passed =                                              \
            ...
            ::logging::CheckError::DCheckOp(__FILE__, __LINE__, &true_if_passed).stream()
...
#define DCHECK_OP(name, op, val1, val2)                                                            \
    EAT_CHECK_STREAM_PARAMS(                                                                       \
            (::logging::CheckOpValueStr(val1), ::logging::CheckOpValueStr(val2), (val1) op(val2)))
