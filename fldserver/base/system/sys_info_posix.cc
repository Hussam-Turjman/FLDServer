...
base::LazyInstance<base::internal::LazySysInfoValue<int, NumberOfProcessors>>::Leaky
...
base::LazyInstance<base::internal::LazySysInfoValue<int64_t, AmountOfVirtualMemory>>::Leaky
        g_lazy_virtual_memory = LAZY_INSTANCE_INITIALIZER;

#if IS_LINUX || defined(OS_CHROMEOS)
bool
IsStatsZeroIfUnlimited(const base::FilePath& path)
{
    struct statfs stats;
...
#endif  //! IS_APPLE && !defined(OS_ANDROID)
                                       ...
                                       int32_t* bugfix_version)
{
    ...
    int num_read = sscanf(info.release, "%d.%d.%d", major_version, minor_version, bugfix_version);
    if (num_read < 1)
        *major_version = 0;
    if (num_read < 2)
        *minor_version = 0;
