...
public:
    ...
    // or -1 on failure.
    static int64_t
    AmountOfFreeDiskSpace(const FilePath& path);

    // Return the total disk space in bytes on the volume containing |path|, or -1
    // on failure.
    static int64_t
    AmountOfTotalDiskSpace(const FilePath& path);
    ...
    static void
    SetAmountOfTotalDiskSpace(const FilePath& path, int64_t bytes);
#endif
    ...
    static void
    GetHardwareInfo(base::OnceCallback<void(HardwareInfo)> callback);
    ...
    static void
    OperatingSystemVersionNumbers(int32_t* major_version,
    ...
    static bool
    GetLsbReleaseValue(const std::string& key, std::string* value);
    ...
    static void
    SetChromeOSVersionInfoForTest(const std::string& lsb_release, const Time& lsb_release_time);
...
private:
    ...
    static int64_t
    AmountOfAvailablePhysicalMemory(const SystemMemoryInfoKB& meminfo);
#endif
};
