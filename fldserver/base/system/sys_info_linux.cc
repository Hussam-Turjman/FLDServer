...
base::LazyInstance<base::internal::LazySysInfoValue<int64_t, AmountOfPhysicalMemory>>::Leaky
...
// static
int64_t
SysInfo::AmountOfAvailablePhysicalMemory(const SystemMemoryInfoKB& info)
{
    // See details here:
    ...
    int64_t res_kb = info.available != 0 ? info.available - info.active_file
...
SysInfo::CPUModelName()
{
    ...
    {
        if (!PathExists(FilePath(base::StringPrintf(kSocIdDirectory, soc_instance))))
            break;

        std::string soc_id;
        ReadFileToString(FilePath(base::StringPrintf(kSocIdFile, soc_instance)), &soc_id);
...
SysInfo::GetHardwareInfoSync()
{
    ...
    if (ReadFileToStringWithMaxSize(
                FilePath("/sys/devices/virtual/dmi/id/sys_vendor"), &data, kMaxStringSize))
    {
        TrimWhitespaceASCII(data, TrimPositions::TRIM_ALL, &info.manufacturer);
    }
    if (ReadFileToStringWithMaxSize(
                FilePath("/sys/devices/virtual/dmi/id/product_name"), &data, kMaxStringSize))
    {
