...
class CORE_EXPORT SystemMonitor
{
public:
    ...
    void
    AddDevicesChangedObserver(DevicesChangedObserver* obs);

    // Remove an existing observer.
    // Can be called from any thread.
    // Must not be called from within a notification callback.
    void
    RemoveDevicesChangedObserver(DevicesChangedObserver* obs);
    ...
    void
    ProcessDevicesChanged(DeviceType device_type);

private:
    // Functions to trigger notifications.
    void
    NotifyDevicesChanged(DeviceType device_type);

    scoped_refptr<ObserverListThreadSafe<DevicesChangedObserver>> devices_changed_observer_list_;
