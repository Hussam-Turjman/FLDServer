...
{
int64_t
AmountOfMemory(DWORDLONG MEMORYSTATUSEX::*memory_field)
{
...
bool
GetDiskSpaceInfo(const base::FilePath& path, int64_t* available_bytes, int64_t* total_bytes)
{
...
// static
int64_t
SysInfo::AmountOfPhysicalMemoryImpl()
{
...
// static
int64_t
SysInfo::AmountOfFreeDiskSpace(const FilePath& path)
{
...
// static
int64_t
SysInfo::AmountOfTotalDiskSpace(const FilePath& path)
{
    base::ScopedBlockingCall scoped_blocking_call(FROM_HERE, base::BlockingType::MAY_BLOCK);
...
SysInfo::OperatingSystemVersion()
{
    ...
    std::string version(StringPrintf(
            "%d.%d.%d", version_number.major, version_number.minor, version_number.build));
...
SysInfo::OperatingSystemArchitecture()
{
    ...
    switch (arch)
    {
        case win::OSInfo::X86_ARCHITECTURE: return "x86";
        case win::OSInfo::X64_ARCHITECTURE: return "x86_64";
        case win::OSInfo::IA64_ARCHITECTURE: return "ia64";
        case win::OSInfo::ARM64_ARCHITECTURE: return "arm64";
        default: return "";
    }
}
...
// static
void
SysInfo::OperatingSystemVersionNumbers(int32_t* major_version,
...
// static
std::string
ReadHardwareInfoFromRegistry(const wchar_t* reg_value_name)
{
    ...
    for (const wchar_t* system_info_reg_key_path : kSystemInfoRegKeyPaths)
    {
        ...
        if (system_information_key.Open(HKEY_LOCAL_MACHINE, system_info_reg_key_path, KEY_READ)
            == ERROR_SUCCESS)
        {
            if ((system_information_key.ReadValue(reg_value_name, &value) == ERROR_SUCCESS)
                && !value.empty())
            {
                break;
            }
...
// static
SysInfo::HardwareInfo
SysInfo::GetHardwareInfoSync()
{
