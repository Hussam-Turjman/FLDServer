...
VlogInfo::VmodulePattern::VmodulePattern(const std::string& pattern) :
    pattern(pattern), vlog_level(VlogInfo::kDefaultVlogLevel), match_target(MATCH_MODULE)
{
    ...
    min_log_level_(min_log_level)
{
    ...
    for (base::StringPairs::const_iterator it = kv_pairs.begin(); it != kv_pairs.end(); ++it)
    {
        VmodulePattern pattern(it->first);
        if (!base::StringToInt(it->second, &pattern.vlog_level))
        {
            DLOG(WARNING) << "Parsed vlog level for \"" << it->first << "=" << it->second
...
base::StringPiece
GetModule(const base::StringPiece& file)
{
...
int
VlogInfo::GetVlogLevel(const base::StringPiece& file) const
{
    if (!vmodule_levels_.empty())
    {
        ...
        {
            base::StringPiece target((it.match_target == VmodulePattern::MATCH_FILE) ? file
                                                                                     : module);
...
void
VlogInfo::SetMaxVlogLevel(int level)
{
...
int
VlogInfo::GetMaxVlogLevel() const
{
    return -*min_log_level_;
}

bool
MatchVlogPattern(const base::StringPiece& string, const base::StringPiece& vlog_pattern)
{
