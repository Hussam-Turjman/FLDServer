...
#include "fldserver/base/containers/linked_list.h"
#include "fldserver/base/memory/weak_ptr.h"
#include "fldserver/base/observer_list_types.h"
...
class CORE_EXPORT UncheckedObserverAdapter
{
public:
    explicit UncheckedObserverAdapter(const void* observer) : ptr_(const_cast<void*>(observer))
    {
    }
    UncheckedObserverAdapter(const UncheckedObserverAdapter&) = delete;
    UncheckedObserverAdapter&
    operator=(const UncheckedObserverAdapter&) = delete;
    UncheckedObserverAdapter(UncheckedObserverAdapter&& other) = default;
    UncheckedObserverAdapter&
    operator=(UncheckedObserverAdapter&& other) = default;
    ...
    bool
    IsMarkedForRemoval() const
    {
        return !ptr_;
    }
    bool
    IsEqual(const void* rhs) const
    {
        return ptr_ == rhs;
    }
    ...
    static ObserverType*
    Get(const UncheckedObserverAdapter& adapter)
    {
        static_assert(!std::is_base_of<CheckedObserver, ObserverType>::value,
...
class CORE_EXPORT CheckedObserverAdapter
{
public:
    ...
    operator=(CheckedObserverAdapter&& other);
    CheckedObserverAdapter(const CheckedObserverAdapter&) = delete;
    CheckedObserverAdapter&
    operator=(const CheckedObserverAdapter&) = delete;
    ~CheckedObserverAdapter();
    ...
    bool
    IsEqual(const CheckedObserver* rhs) const
    {
    ...
    template <class ObserverType>
    static ObserverType*
    Get(const CheckedObserverAdapter& adapter)
    {
        static_assert(std::is_base_of<CheckedObserver, ObserverType>::value,
...
public:
    ...
    explicit WeakLinkNode(ObserverList* list)
    {
        SetList(list);
    }
    WeakLinkNode(const WeakLinkNode&) = delete;
    WeakLinkNode&
    operator=(const WeakLinkNode&) = delete;
    ...
    bool
    IsOnlyRemainingNode() const
    {
        return list_ && list_->live_iterators_.head() == list_->live_iterators_.tail();
    }

    void
    SetList(ObserverList* list)
    {
        DCHECK(!list_);
        DCHECK(list);
        list_ = list;
