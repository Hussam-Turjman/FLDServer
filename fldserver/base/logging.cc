...
#include "fldserver/base/debug/task_trace.h"
...
#include <string>
#include <utility>
//#include "fldserver/base/test/scoped_logging_settings.h"
#include "fldserver/base/threading/platform_thread.h"
#include "fldserver/base/vlog.h"
#include "fldserver/fldserver_config.h"
...
#if IS_POSIX || defined(OS_FUCHSIA)
#include "fldserver/base/posix/safe_strerror.h"
#endif
...
const char*
log_severity_name(int severity)
{
...
base::stack<LogAssertHandlerFunction>&
...
TickCount()
{
    ...
    uint64_t absolute_micro =
            static_cast<int64_t>(ts.tv_sec) * 1000000 + static_cast<int64_t>(ts.tv_nsec) / 1000;
...
void
DeleteFilePath(const PathString& log_name)
{
#if IS_WIN
...
InitializeLogFileHandle()
{
    ...
    g_log_file = CreateFile(g_log_file_name->c_str(),
                            FILE_APPEND_DATA,
...
inline fx_log_severity_t
LogSeverityToFuchsiaLogSeverity(LogSeverity severity)
{
    switch (severity)
    {
        case LOGGING_INFO: return FX_LOG_INFO;
        case LOGGING_WARNING: return FX_LOG_WARNING;
        case LOGGING_ERROR: return FX_LOG_ERROR;
        case LOGGING_FATAL:
...
CORE_EXPORT logging::LogSeverity LOGGING_DCHECK = LOGGING_INFO;
...
bool
BaseInitLoggingImpl(const LoggingSettings& settings)
{
#if defined(OS_NACL)
    // Can log only to the system debug log and stderr.
    CHECK_EQ(settings.logging_dest & ~(LOG_TO_SYSTEM_DEBUG_LOG | LOG_TO_STDERR), 0u);
#endif
    ...
    {
        std::string log_tag =
                base::CommandLine::ForCurrentProcess()->GetProgram().BaseName().AsUTF8Unsafe();
...
// and debugging.
bool
ShouldLogToStderr(int severity)
{
...
void
SetLogItems(bool enable_process_id,
...
{
    DCHECK(!prefix || base::ContainsOnlyChars(prefix, "abcdefghijklmnopqrstuvwxyz"));
    g_log_prefix = prefix;
}

void
SetShowErrorDialogs(bool enable_dialogs)
{
...
void
SetLogMessageHandler(LogMessageHandlerFunction handler)
{
...
void
DisplayDebugMessageInDialog(const std::string& str)
{
    if (str.empty())
        return;
    ...
    {
        MessageBoxW(nullptr,
                    base::as_wcstr(base::UTF8ToUTF16(str)),
                    L"Fatal error",
...
LogMessage::~LogMessage()
{
...
#if !defined(OFFICIAL_BUILD) && !defined(OS_NACL) && !defined(__UCLIBC__) && !defined(OS_AIX)
    ...
    if ((g_logging_destination & LOG_TO_SYSTEM_DEBUG_LOG) != 0)
    {
        ...
        if (log_to_system)
        {
            ...
            CFStringRef main_bundle_id_cf = main_bundle ? CFBundleGetIdentifier(main_bundle)
                                                        : nullptr;
            std::string main_bundle_id = main_bundle_id_cf
                                                 ? base::SysCFStringRefToUTF8(main_bundle_id_cf)
                                                 : std::string("");
#if defined(USE_ASL)
            ...
            public:
                ...
                ASLClient&
                operator=(const ASLClient&) = delete;
                ~ASLClient()
                {
                    asl_close(client_);
                }
            ...
            public:
                ...
                ASLMessage&
                operator=(const ASLMessage&) = delete;
                ~ASLMessage()
                {
                    asl_free(message_);
                }
...
#define ASL_LEVEL_STR_X(level) #level
                switch (severity)
                {
                    case LOGGING_INFO: return ASL_LEVEL_STR(ASL_LEVEL_INFO);
                    case LOGGING_WARNING: return ASL_LEVEL_STR(ASL_LEVEL_WARNING);
                    case LOGGING_ERROR: return ASL_LEVEL_STR(ASL_LEVEL_ERR);
                    case LOGGING_FATAL: return ASL_LEVEL_STR(ASL_LEVEL_CRIT);
                    default:
...
#undef ASL_LEVEL_STR_X
            ...
            const os_log_type_t os_log_type = [](LogSeverity severity) {
                switch (severity)
                {
                    case LOGGING_INFO: return OS_LOG_TYPE_INFO;
                    case LOGGING_WARNING: return OS_LOG_TYPE_DEFAULT;
                    case LOGGING_ERROR: return OS_LOG_TYPE_ERROR;
                    case LOGGING_FATAL: return OS_LOG_TYPE_FAULT;
                    default: return severity < 0 ? OS_LOG_TYPE_DEBUG : OS_LOG_TYPE_DEFAULT;
                }
            }(severity_);
        ...
        switch (severity_)
        {
            case LOGGING_INFO: priority = ANDROID_LOG_INFO; break;
            case LOGGING_WARNING: priority = ANDROID_LOG_WARN; break;
            case LOGGING_ERROR: priority = ANDROID_LOG_ERROR; break;
            case LOGGING_FATAL: priority = ANDROID_LOG_FATAL; break;
        }
        ...
        if (logger)
        {
            ...
            fx_logger_log_with_source(logger,
...
#error Unsupported platform
    ...
    if (severity_ == LOGGING_FATAL)
    {
        ...
        {
            LogAssertHandlerFunction log_assert_handler = GetLogAssertHandlerStack().top();

            if (log_assert_handler)
            {
                log_assert_handler.Run(file_,
                                       line_,
...
void
LogMessage::Init(const char* file, int line)
{
...
#endif  // 0
    {
        ...
        if (g_log_timestamp)
        {
            ...
            stream_ << std::setfill('0') << std::setw(2) << local_time.wMonth << std::setw(2)
                    << local_time.wDay << '/' << std::setw(2) << local_time.wHour << std::setw(2)
            ...
            stream_ << std::setfill('0') << std::setw(2) << 1 + tm_time->tm_mon << std::setw(2)
                    << tm_time->tm_mday << '/' << std::setw(2) << tm_time->tm_hour << std::setw(2)
                    << tm_time->tm_min << std::setw(2) << tm_time->tm_sec << '.' << std::setw(6)
                    << tv.tv_usec << ':';
#else
#error Unsupported platform
#endif
...
SystemErrorCodeToString(SystemErrorCode error_code)
{
    ...
    DWORD len = FormatMessageA(flags, nullptr, error_code, 0, msgbuf, base::size(msgbuf), nullptr);
    if (len)
    {
    ...
    return base::StringPrintf(
            "Error (0x%lX) while retrieving error. (0x%lX)", GetLastError(), error_code);
#elif IS_POSIX || defined(OS_FUCHSIA)
...
#if 0
void
ScopedLoggingSettings::SetLogFormat(LogFormat log_format) const
{
...
RawLog(int level, const char* message)
{
    if (level >= g_min_log_level && message)
    {
        ...
        {
            rv = HANDLE_EINTR(
                    write(STDERR_FILENO, message + bytes_written, message_len - bytes_written));
            if (rv < 0)
            {
...
std::ostream&
std::operator<<(std::ostream& out, const std::wstring& wstr)
{
    return out << base::WStringPiece(wstr);
}

std::ostream&
std::operator<<(std::ostream& out, const char16_t* str16)
{
    return out << (str16 ? base::StringPiece16(str16) : base::StringPiece16());
}

std::ostream&
std::operator<<(std::ostream& out, const std::u16string& str16)
{
