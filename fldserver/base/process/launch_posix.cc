...
#include "fldserver/base/threading/platform_thread_internal_posix.h"
...
#if (!IS_LINUX && !defined(OS_AIX) && !defined(OS_CHROMEOS))                                       \
...
// our own.
int
sys_rt_sigaction(int sig, const struct kernel_sigaction* act, struct kernel_sigaction* oact)
{
...
void
ResetChildSignalHandlersToDefaults(void)
{
...
void
CloseSuperfluousFds(const base::InjectiveMultimap& saved_mapping)
{
...
Process
LaunchProcess(const std::vector<std::string>& argv, const LaunchOptions& options)
{
    ...
    if (options.clone_flags)
    {
        ...
        RAW_CHECK(!(options.clone_flags & (CLONE_SIGHAND | CLONE_THREAD | CLONE_VM)));

        // We specify a null ptid and ctid.
        RAW_CHECK(!(options.clone_flags
    ...
    memset(reinterpret_cast<void*>(malloc), 0xff, 8);
        ...
        if (options.ctrl_terminal_fd >= 0)
        {
            ...
            {
                if (HANDLE_EINTR(ioctl(options.ctrl_terminal_fd, TIOCSCTTY, nullptr)) == -1)
                {
...
#define PR_SET_NO_NEW_PRIVS 38
        ...
        const char* executable_path = !options.real_path.empty() ? options.real_path.value().c_str()
                                                                 : argv_cstr[0];
...
static bool
GetAppOutputInternal(const std::vector<std::string>& argv,
                     char* const envp[],
                     bool include_stderr,
                     std::string* output,
                     bool do_search_path,
    ...
    switch (pid)
    {
        ...
        case 0: {
            ...
            fd_shuffle1.push_back(
                    InjectionArc(include_stderr ? pipe_fd[1] : dev_null, STDERR_FILENO, true));
        ...
        default: {
            ...
            {
                char buffer[256];
                ssize_t bytes_read = HANDLE_EINTR(read(pipe_fd[0], buffer, sizeof(buffer)));
                if (bytes_read <= 0)
                    break;
...
bool
GetAppOutput(const std::vector<std::string>& argv, std::string* output)
{
    // Run |execve()| with the current environment.
    int exit_code;
    bool result = GetAppOutputInternal(argv, nullptr, false, output, true, &exit_code);
...
GetAppOutputAndError(const CommandLine& cl, std::string* output)
{
    // Run |execve()| with the current environment.
    int exit_code;
    bool result = GetAppOutputInternal(cl.argv(), nullptr, true, output, true, &exit_code);
    return result && exit_code == EXIT_SUCCESS;
}

bool
GetAppOutputAndError(const std::vector<std::string>& argv, std::string* output)
{
    int exit_code;
    bool result = GetAppOutputInternal(argv, nullptr, true, output, true, &exit_code);
    return result && exit_code == EXIT_SUCCESS;
}
...
{
    // Run |execve()| with the current environment.
    return GetAppOutputInternal(cl.argv(), nullptr, false, output, true, exit_code);
}

#endif  // !defined(OS_NACL_NONSFI)

#if IS_LINUX || defined(OS_CHROMEOS) || defined(OS_NACL_NONSFI) || defined(OS_AIX)
namespace
{
...
#endif
NOINLINE pid_t
CloneAndLongjmpInChild(unsigned long flags, pid_t* ptid, pid_t* ctid, jmp_buf* env)
{
    ...
    alignas(16) char stack_buf[PTHREAD_STACK_MIN];
        ...
        || defined(ARCH_CPU_S390_FAMILY) || defined(ARCH_CPU_PPC64_FAMILY)
...
#endif  // IS_LINUX || defined(OS_CHROMEOS) || defined(OS_NACL_NONSFI)
