...
namespace
{
#if IS_POSIX || defined(OS_FUCHSIA) || IS_WIN
// Parses a null-terminated input string of an environment block. The key is
// placed into the given string, and the total length of the line, including
// the terminating null, is returned.
size_t
ParseEnvLine(const NativeEnvironmentString::value_type* input, NativeEnvironmentString* key)
{
...
std::unique_ptr<char* []>
AlterEnvironment(const char* const* const env, const EnvironmentMap& changes) {
    ...
    char* storage_data = reinterpret_cast<char*>(&result.get()[result_indices.size() + 1]);
