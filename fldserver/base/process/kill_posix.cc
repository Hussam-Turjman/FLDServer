...
#include "fldserver/base/process/process_iterator.h"
...
{
TerminationStatus
GetTerminationStatusImpl(ProcessHandle handle, bool can_block, int* exit_code)
{
    DCHECK(exit_code);

    int status = 0;
    const pid_t result = HANDLE_EINTR(waitpid(handle, &status, can_block ? 0 : WNOHANG));
    if (result == -1)
    {
    ...
    *exit_code = status;
    ...
    if (WIFSIGNALED(status))
    {
        switch (WTERMSIG(status))
        {
            ...
            case SIGSEGV:
            case SIGTRAP:
            case SIGSYS: return TERMINATION_STATUS_PROCESS_CRASHED;
            case SIGKILL:
#if 0
...
#endif
            case SIGINT:
            case SIGTERM: return TERMINATION_STATUS_PROCESS_WAS_KILLED;
            default: break;
        }
    }
...
bool
WaitForProcessesToExit(const FilePath::StringType& executable_name,
                       TimeDelta wait,
...
bool
CleanupProcesses(const FilePath::StringType& executable_name,
                 TimeDelta wait,
                 int exit_code,
                 const ProcessFilter* filter)
{
...
void
EnsureProcessTerminated(Process process)
{
...
void
EnsureProcessGetsReaped(Process process)
{
    DCHECK(!process.is_current());
    ...
    PlatformThread::CreateNonJoinable(0, new BackgroundReaper(std::move(process), TimeDelta()));
}
