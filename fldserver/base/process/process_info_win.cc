...
GetCurrentProcessIntegrityLevel()
{
    ...
    if (::GetTokenInformation(
                scoped_process_token.Get(), TokenIntegrityLevel, nullptr, 0, &token_info_length)
    ...
    if (!::GetTokenInformation(scoped_process_token.Get(),
                               TokenIntegrityLevel,
                               token_label,
                               token_info_length,
                               &token_info_length))
...
IsCurrentProcessElevated()
{
    ...
    if (!GetTokenInformation(
                scoped_process_token.Get(), TokenElevation, &elevation, sizeof(elevation), &size))
    {
