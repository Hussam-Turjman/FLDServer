...
public:
    ...
    static Process
    OpenWithAccess(ProcessId pid, DWORD desired_access);
#endif
    ...
    TerminateCurrentProcessImmediately(int exit_code);
    ...
    bool
    Terminate(int exit_code, bool wait) const;
    ...
    WaitExitStatus
    WaitForExitOrEvent(const base::win::ScopedHandle& stop_event_handle, int* exit_code) const;
#endif  // OS_WIN
    ...
    // not required.
    bool
    WaitForExit(int* exit_code) const;

    // Same as WaitForExit() but only waits for up to |timeout|.
    // NOTE: |exit_code| is optional, nullptr can be passed if the exit code
    // is not required.
    bool
    WaitForExitWithTimeout(TimeDelta timeout, int* exit_code) const;
    ...
    bool
    IsProcessBackgrounded(PortProvider* port_provider) const;
    ...
    bool
    SetProcessBackgrounded(PortProvider* port_provider, bool value);
#else
    // A process is backgrounded when it's priority is lower than normal.
    // Return true if this process is backgrounded, false otherwise.
    bool
    IsProcessBackgrounded() const;
