...
Process::Process(Process&& other) :
    process_(other.process_.Take()), is_current_process_(other.is_current_process_)
{
    other.Close();
}
...
// static
Process
Process::OpenWithAccess(ProcessId pid, DWORD desired_access)
{
...
// static
bool
Process::CanBackgroundProcesses()
{
    return true;
}
...
// static
void
Process::TerminateCurrentProcessImmediately(int exit_code)
{
#if 0
...
Time
Process::CreationTime() const
{
    FILETIME creation_time = {};
    FILETIME ignore1 = {};
    FILETIME ignore2 = {};
    FILETIME ignore3 = {};
    if (!::GetProcessTimes(Handle(), &creation_time, &ignore1, &ignore2, &ignore3))
    {
        return Time();
    }
...
bool
Process::Terminate(int exit_code, bool wait) const
{
...
Process::WaitExitStatus
Process::WaitForExitOrEvent(const base::win::ScopedHandle& stop_event_handle, int* exit_code) const
{
    // Record the event that this thread is blocking upon (for hang diagnosis).
    base::debug::ScopedProcessWaitActivity process_activity(this);

    HANDLE events[] = {Handle(), stop_event_handle.Get()};
    DWORD wait_result = ::WaitForMultipleObjects(base::size(events), events, FALSE, INFINITE);
...
bool
Process::WaitForExit(int* exit_code) const
{
    return WaitForExitWithTimeout(TimeDelta::FromMilliseconds(INFINITE), exit_code);
}

bool
Process::WaitForExitWithTimeout(TimeDelta timeout, int* exit_code) const
{
    // Record the event that this thread is blocking upon (for hang diagnosis).
