...
struct CGroups
{
    ...
    CGroups()
    {
        ...
        enabled = base::GetFileSystemType(foreground_file, &foreground_type)
                  ...
                  && foreground_type == FILE_SYSTEM_CGROUP && background_type == FILE_SYSTEM_CGROUP;
    }
...
{
    int64_t start_ticks =
            is_current() ? internal::ReadProcSelfStatsAndGetFieldAsInt64(internal::VM_STARTTIME)
...
Process::CanBackgroundProcesses()
{
    ...
    static const bool can_reraise_priority = internal::CanLowerNiceTo(kForegroundPriority);
...
Process::IsProcessBackgrounded() const
{
    ...
    if (CGroups::Get().enabled)
    {
        ...
        std::string proc;
        if (base::ReadFileToString(base::FilePath(StringPrintf(kProcPath, process_)), &proc))
        {
...
bool
Process::SetProcessBackgrounded(bool background)
{
    DCHECK(IsValid());
...
#if 0
bool
IsProcessBackgroundedCGroup(const StringPiece& cgroup_contents)
{
...
ProcessId
Process::GetPidInNamespace() const
{
    std::string status;
    {
        // Synchronously reading files in /proc does not hit the disk.
        ThreadRestrictions::ScopedAllowIO allow_io;
        FilePath status_file = FilePath("/proc").Append(NumberToString(process_)).Append("status");
