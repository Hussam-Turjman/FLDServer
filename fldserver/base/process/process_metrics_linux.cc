...
#include "fldserver/base/cpu.h"
#include "fldserver/base/files/dir_reader_posix.h"
...
#include "fldserver/base/process/internal_linux.h"
#include "fldserver/base/process/process_metrics.h"
#include "fldserver/base/process/process_metrics_iocounters.h"
...
#include "fldserver/base/strings/string_tokenizer.h"
...
{
void
TrimKeyValuePairs(StringPairs* pairs)
{
    for (auto& pair : *pairs)
    {
...
bool
ReadProcFileToTrimmedStringPairs(pid_t pid, StringPiece filename, StringPairs* key_value_pairs)
{
...
size_t
ReadProcStatusAndGetFieldAsSizeT(pid_t pid, StringPiece field)
{
    StringPairs pairs;
...
bool
ReadProcStatusAndGetFieldAsUint64(pid_t pid, StringPiece field, uint64_t* result)
{
    StringPairs pairs;
    if (!ReadProcFileToTrimmedStringPairs(pid, "status", &pairs))
        return false;
...
int64_t
ParseTotalCPUTimeFromStats(const std::vector<std::string>& proc_stats)
{
...
GetProcessCPU(pid_t pid)
{
    ...
    if (!internal::ReadProcStats(pid, &buffer) || !internal::ParseProcStats(buffer, &proc_stats))
    {
        return -1;
    }
...
SupportsPerTaskTimeInState()
{
    ...
    std::string contents;
    return internal::ReadProcFile(time_in_state_path, &contents) && StartsWith(contents, "cpu");
}
...
size_t
ProcessMetrics::GetResidentSetSize() const
{
...
bool
ProcessMetrics::GetCumulativeCPUUsagePerThread(CPUUsagePerThread& cpu_per_thread)
{
    cpu_per_thread.clear();

    internal::ForEachProcessTask(
            process_, [&cpu_per_thread](PlatformThreadId tid, const FilePath& task_path) {
...
bool
ProcessMetrics::GetPerThreadCumulativeCPUTimeInState(TimeInStatePerThread& time_in_state_per_thread)
{
    ...
    internal::ForEachProcessTask(
            process_,
            [&time_in_state_per_thread, &success, this](PlatformThreadId tid,
...
bool
ProcessMetrics::GetIOCounters(IoCounters* io_counters) const
{
    StringPairs pairs;
...
uint64_t
ProcessMetrics::GetVmSwapBytes() const
{
    return ReadProcStatusAndGetFieldAsSizeT(process_, "VmSwap") * 1024;
}

bool
ProcessMetrics::GetPageFaultCounts(PageFaultCounts* counts) const
{
    ...
    counts->minor = internal::GetProcStatsFieldAsInt64(proc_stats, internal::VM_MINFLT);
    counts->major = internal::GetProcStatsFieldAsInt64(proc_stats, internal::VM_MAJFLT);
    return true;
}
#endif  // IS_LINUX || defined(OS_CHROMEOS) || defined(OS_ANDROID)

int
ProcessMetrics::GetOpenFdCount() const
{
...
int
ProcessMetrics::GetOpenFdSoftLimit() const
{
    ...
    for (const auto& line :
         SplitStringPiece(limits_contents, "\n", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY))
    {
        if (!StartsWith(line, "Max open files"))
            continue;

        auto tokens = SplitStringPiece(line, " ", TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
        if (tokens.size() > 3)
        {
            int limit = -1;
...
int
ParseProcStatCPU(StringPiece input)
{
...
int
GetNumberOfThreads(ProcessHandle process)
{
    return internal::ReadProcStatsAndGetFieldAsInt64(process, internal::VM_NUMTHREADS);
}

bool
ProcessMetrics::ParseProcTimeInState(const std::string& content,
                                     ...
                                     TimeInStatePerThread& time_in_state_per_thread)
{
    ...
    for (size_t pos = 0; pos <= max_pos;)
    {
        ...
        if (!isdigit(next_char))
        {
            ...
            int matches =
                    sscanf(begin + pos, "cpu%" PRIu32 "\n%n", &current_core_index, &num_chars);
            if (matches != 1)
                return false;
        ...
        else if (header_seen)
        {
            ...
            uint64_t time;
            int matches = sscanf(
                    begin + pos, "%" PRIu64 " %" PRIu64 "\n%n", &frequency, &time, &num_chars);
            if (matches != 2)
                return false;
            ...
            if (time > 0)
            {
                time_in_state_per_thread.push_back({tid,
                                                    current_core_type,
                                                    current_core_index,
                                                    frequency,
...
CPU::CoreType
ProcessMetrics::GetCoreType(int core_index)
{
...
bool
ParseProcMeminfo(StringPiece meminfo_data, SystemMemoryInfoKB* meminfo)
{
    ...
    for (const StringPiece& line :
         SplitStringPiece(meminfo_data, "\n", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY))
    {
        std::vector<StringPiece> tokens =
                SplitStringPiece(line, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
        // HugePages_* only has a number and no suffix so there may not be exactly 3
        // tokens.
        if (tokens.size() <= 1)
        {
            DLOG(WARNING) << "meminfo: tokens: " << tokens.size() << " malformed line: " << line;
            continue;
        }
...
bool
ParseProcVmstat(StringPiece vmstat_data, VmStatInfo* vmstat)
{
...
bool
GetSystemMemoryInfo(SystemMemoryInfoKB* meminfo)
{
...
bool
GetVmStatInfo(VmStatInfo* vmstat)
{
...
bool
IsValidDiskName(StringPiece candidate)
{
...
bool
GetSystemDiskInfo(SystemDiskInfo* diskinfo)
{
    ...
    std::vector<StringPiece> diskinfo_lines =
            SplitStringPiece(diskinfo_data, "\n", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY);
    ...
    {
        std::vector<StringPiece> disk_fields =
                SplitStringPiece(line, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
...
std::unique_ptr<Value>
GraphicsMemoryInfoKB::ToValue() const
{
    auto res = std::make_unique<DictionaryValue>();
...
bool
ParseZramMmStat(StringPiece mm_stat_data, SwapInfo* swap_info)
{
    ...
    std::vector<StringPiece> tokens =
            SplitStringPiece(mm_stat_data, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
    if (tokens.size() < 7)
    {
...
bool
ParseZramStat(StringPiece stat_data, SwapInfo* swap_info)
{
    ...
    std::vector<StringPiece> tokens =
            SplitStringPiece(stat_data, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
    if (tokens.size() < 11)
    {
        DLOG(WARNING) << "zram stat: tokens: " << tokens.size() << " malformed line: " << stat_data;
        return false;
    }
...
{
bool
IgnoreZramFirstPage(uint64_t orig_data_size, SwapInfo* swap_info)
{
...
void
ParseZramPath(SwapInfo* swap_info)
{
...
bool
GetSwapInfoImpl(SwapInfo* swap_info)
{
...
bool
GetSwapInfo(SwapInfo* swap_info)
{
...
bool
GetGraphicsMemoryInfo(GraphicsMemoryInfoKB* gpu_meminfo)
{
    ...
    static bool is_newer_kernel = base::StartsWith(base::SysInfo::KernelVersion(), "5.");
    ...
    {
        int64_t mali_size = -1;
        int num_res = sscanf(mali_memory_data.c_str(), "%" SCNd64 " bytes", &mali_size);
        if (num_res == 1)
