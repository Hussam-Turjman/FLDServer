...
#include "fldserver/base/process/environment_internal.h"
#include "fldserver/base/process/kill.h"
...
#include "fldserver/base/win/scoped_process_information.h"
#include "fldserver/base/win/startup_information.h"
...
{
bool
GetAppOutputInternal(CommandLine::StringPieceType cl,
                     bool include_stderr,
                     std::string* output,
                     int* exit_code)
{
    ...
    if (!CreateProcess(nullptr,
                       (data(writable_command_line_string)),
                       nullptr,
                       nullptr,
    ...
    if (tracker)
        tracker->RecordProcessLaunch(proc_info.process_id(), CommandLine::StringType(cl));
    ...
    TerminationStatus status = GetTerminationStatus(proc_info.process_handle(), exit_code);
    debug::GlobalActivityTracker::RecordProcessExitIfEnabled(proc_info.process_id(), *exit_code);
...
void
RouteStdioToConsole(bool create_console_if_not_found)
{
...
Process
LaunchProcess(const CommandLine& cmdline, const LaunchOptions& options)
{
    return LaunchProcess(cmdline.GetCommandLineString(), options);
}

Process
LaunchProcess(const CommandLine::StringType& cmdline, const LaunchOptions& options)
{
    ...
    STARTUPINFO* startup_info = reinterpret_cast<STARTUPINFO*>(startup_info_wrapper.startup_info());
    DWORD flags = 0;

    // Count extended attributes before reserving space.
    int attribute_count = 0;
    // Count PROC_THREAD_ATTRIBUTE_MITIGATION_POLICY.
    if (options.disable_cetcompat && base::win::GetVersion() >= base::win::Version::WIN10_20H1)
    {
        ++attribute_count;
    }
    ...
    {
        if (!startup_info_wrapper.InitializeProcThreadAttributeList(attribute_count))
        {
            DPLOG(ERROR);
            return Process();
        }
    ...
    if (options.disable_cetcompat && base::win::GetVersion() >= base::win::Version::WIN10_20H1)
    {
        ...
        if (!startup_info_wrapper.UpdateProcThreadAttribute(PROC_THREAD_ATTRIBUTE_MITIGATION_POLICY,
                                                            &process_mitigations[0],
    ...
    if (!options.handles_to_inherit.empty())
    {
        ...
        {
            BOOL result = SetHandleInformation(handle, HANDLE_FLAG_INHERIT, HANDLE_FLAG_INHERIT);
            PCHECK(result);
        }
    ...
    LPCTSTR current_directory = reinterpret_cast<LPCTSTR>(
            options.current_directory.empty() ? nullptr
    ...
    if (options.as_user)
    {
        ...
        BOOL launched = CreateProcessAsUser(options.as_user,
                                            nullptr,
                                            (data(writable_cmdline)),
                                            nullptr,
                                            nullptr,
                                            inherit_handles,
                                            flags,
        ...
        if (!launched)
        {
            DPLOG(ERROR) << "Command line:" << std::endl << WideToUTF8(cmdline) << std::endl;
            return Process();
        }
    }
    else
    {
        ...
        if (options.clear_environment || !options.environment.empty())
        {
            if (options.clear_environment)
            {
                ...
                env_storage = internal::AlterEnvironment(kEmptyEnvironment, options.environment);
            }
            else
            {
                wchar_t* old_environment = reinterpret_cast<wchar_t*>(GetEnvironmentStrings());
                if (!old_environment)
                {
                    DPLOG(ERROR);
                    return Process();
                }
                env_storage = internal::AlterEnvironment(old_environment, options.environment);
    ...
    if (options.job_handle
        && !AssignProcessToJobObject(options.job_handle, process_info.process_handle()))
    {
    ...
    debug::GlobalActivityTracker::RecordProcessLaunchIfEnabled(process_info.process_id(), cmdline);
    return Process(process_info.TakeProcessHandle());
}

Process
LaunchElevatedProcess(const CommandLine& cmdline, const LaunchOptions& options)
{
...
bool
SetJobObjectLimitFlags(HANDLE job_object, DWORD limit_flags)
{
    JOBOBJECT_EXTENDED_LIMIT_INFORMATION limit_info = {};
    limit_info.BasicLimitInformation.LimitFlags = limit_flags;
    return 0
           != SetInformationJobObject(
                   job_object, JobObjectExtendedLimitInformation, &limit_info, sizeof(limit_info));
}

bool
GetAppOutput(const CommandLine& cl, std::string* output)
{
    return GetAppOutput(cl.GetCommandLineString(), output);
}

bool
GetAppOutputAndError(const CommandLine& cl, std::string* output)
{
    int exit_code;
    return GetAppOutputInternal(cl.GetCommandLineString(), true, output, &exit_code);
}

bool
GetAppOutputWithExitCode(const CommandLine& cl, std::string* output, int* exit_code)
{
    return GetAppOutputInternal(cl.GetCommandLineString(), false, output, exit_code);
}

bool
GetAppOutput(CommandLine::StringPieceType cl, std::string* output)
{
    int exit_code;
