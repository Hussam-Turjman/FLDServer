...
ProcessMetrics::GetCumulativeCPUUsage()
{
    ...
    if (!GetProcessTimes(process_.Get(), &creation_time, &exit_time, &kernel_time, &user_time))
    {
    ...
    return TimeDelta::FromFileTime(kernel_time) + TimeDelta::FromFileTime(user_time);
}
...
SystemPerformanceInfo::SystemPerformanceInfo(const SystemPerformanceInfo& other) = default;

std::unique_ptr<Value>
SystemPerformanceInfo::ToValue() const
{
    ...
    result->SetDouble("read_transfer_count", strict_cast<double>(read_transfer_count));
    result->SetDouble("write_transfer_count", strict_cast<double>(write_transfer_count));
    result->SetDouble("other_transfer_count", strict_cast<double>(other_transfer_count));
    result->SetDouble("read_operation_count", strict_cast<double>(read_operation_count));
    result->SetDouble("write_operation_count", strict_cast<double>(write_operation_count));
    result->SetDouble("other_operation_count", strict_cast<double>(other_operation_count));
    result->SetDouble("pagefile_pages_written", strict_cast<double>(pagefile_pages_written));
    result->SetDouble("pagefile_pages_write_ios", strict_cast<double>(pagefile_pages_write_ios));
...
GetSystemPerformanceInfo(SystemPerformanceInfo* info)
{
    ...
    SYSTEM_PERFORMANCE_INFORMATION counters = {};
    {
        ...
        base::ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
        if (query_system_information_ptr(::SystemPerformanceInformation,
                                         &counters,
