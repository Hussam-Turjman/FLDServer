...
#include "fldserver/base/posix/file_descriptor_shuffle.h"
#endif
...
struct CORE_EXPORT LaunchOptions
{
#if (IS_POSIX || defined(OS_FUCHSIA)) && !IS_APPLE
    // Delegate to be run in between fork and exec in the subprocess (see
    // pre_exec_delegate below)
    class CORE_EXPORT PreExecDelegate
    {
    public:
...
#if IS_WIN || IS_POSIX || defined(OS_FUCHSIA)
    ...
    static uint32_t
    AddHandleToTransfer(HandlesToTransferVector* handles_to_transfer, zx_handle_t handle);
...
CORE_EXPORT void
CloseSuperfluousFds(const InjectiveMultimap& saved_map);
#endif  // IS_APPLE
#endif  // IS_WIN
...
CORE_EXPORT LaunchOptions
LaunchOptionsForTest();

#if IS_LINUX || defined(OS_CHROMEOS) || defined(OS_NACL_NONSFI)
