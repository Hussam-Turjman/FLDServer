...
#include "fldserver/base/memory/page_size.h"

#if IS_APPLE
#include "fldserver/base/process/port_provider_mac.h"
...
CORE_EXPORT int64_t
TimeValToMicroseconds(const struct timeval& tv);
...
class CORE_EXPORT ProcessMetrics
{
public:
    ~ProcessMetrics();
...
#if !defined(OS_MAC)
    static std::unique_ptr<ProcessMetrics>
    ...
    static std::unique_ptr<ProcessMetrics>
    CreateProcessMetrics(ProcessHandle process, PortProvider* port_provider);
...
#if IS_LINUX || defined(OS_CHROMEOS) || defined(OS_ANDROID) || defined(OS_AIX)
    ...
    bool
    GetCumulativeCPUUsagePerThread(CPUUsagePerThread&);
    ...
    bool
    GetPerThreadCumulativeCPUTimeInState(TimeInStatePerThread&);
    ...
    // Exposed for testing.
    bool
    ParseProcTimeInState(const std::string& content,
    ...
    // otherwise.
    bool
    GetIOCounters(IoCounters* io_counters) const;
    ...
    int
    GetOpenFdSoftLimit() const;
#endif  // IS_POSIX

#if IS_LINUX || defined(OS_CHROMEOS) || defined(OS_ANDROID)
    // Bytes of swap as reported by /proc/[pid]/status.
    uint64_t
    GetVmSwapBytes() const;

    // Minor and major page fault count as reported by /proc/[pid]/stat.
    // Returns true for success.
    bool
    GetPageFaultCounts(PageFaultCounts* counts) const;
#endif  // IS_LINUX || defined(OS_CHROMEOS) || defined(OS_ANDROID)
...
private:
    ...
    int
    CalculatePackageIdleWakeupsPerSecond(uint64_t absolute_package_idle_wakeups);
#endif

#if IS_LINUX || defined(OS_CHROMEOS) || defined(OS_ANDROID) || defined(OS_AIX)
    CPU::CoreType
    GetCoreType(int core_index);
#endif  // IS_LINUX || defined(OS_CHROMEOS) || defined(OS_ANDROID) ||
        // defined(OS_AIX)
    ...
    mach_port_t
    TaskForPid(ProcessHandle process) const;
...
CORE_EXPORT void
IncreaseFdLimitTo(unsigned int max_descriptors);
#endif  // IS_POSIX

#if IS_WIN || IS_APPLE || IS_LINUX || defined(OS_CHROMEOS) || defined(OS_ANDROID)                  \
...
struct CORE_EXPORT SystemMemoryInfoKB
{
    SystemMemoryInfoKB();
...
#if defined(OS_ANDROID) || IS_LINUX || defined(OS_CHROMEOS) || defined(OS_AIX)                     \
        || defined(OS_FUCHSIA)
    int buffers = 0;
    int cached = 0;
    int active_anon = 0;
...
#endif  // IS_WIN || IS_APPLE || IS_LINUX ||
...
CORE_EXPORT extern const char kProcSelfExe[];

// Parses a string containing the contents of /proc/meminfo
// returns true on success or false for a parsing error
// Exposed for testing.
CORE_EXPORT bool
ParseProcMeminfo(StringPiece input, SystemMemoryInfoKB* meminfo);

// Data from /proc/vmstat.
struct CORE_EXPORT VmStatInfo
{
    // Serializes the platform specific fields to value.
    std::unique_ptr<DictionaryValue>
    ToValue() const;
...
// Exposed for testing.
CORE_EXPORT bool
ParseProcVmstat(StringPiece input, VmStatInfo* vmstat);

// Data from /proc/diskstats about system-wide disk I/O.
struct CORE_EXPORT SystemDiskInfo
{
    SystemDiskInfo();
...
struct CORE_EXPORT SwapInfo
{
    SwapInfo() :
        num_reads(0), num_writes(0), compr_data_size(0), orig_data_size(0), mem_used_total(0)
    {
    }
...
struct CORE_EXPORT GraphicsMemoryInfoKB
{
    // Serializes the platform specific fields to value.
    std::unique_ptr<Value>
    ToValue() const;
...
struct CORE_EXPORT SystemPerformanceInfo
{
...
CORE_EXPORT bool
GetSystemPerformanceInfo(SystemPerformanceInfo* info);
...
class CORE_EXPORT SystemMetrics
{
public:
    SystemMetrics();
...
CORE_EXPORT MachVMRegionResult
GetTopInfo(mach_port_t task,
           mach_vm_size_t* size,
