...
#include <tlhelp32.h>
#include <windows.h>
#elif IS_APPLE || defined(OS_OPENBSD)
...
struct ProcessEntry : public PROCESSENTRY32
{
    ...
    exe_file() const
    {
        return reinterpret_cast<const wchar_t*>(szExeFile);
    }
};
#elif IS_POSIX || defined(OS_FUCHSIA)
struct CORE_EXPORT ProcessEntry
{
    ProcessEntry();
...
public:
    ...
    virtual bool
    Includes(const ProcessEntry& entry) const = 0;
...
// false.
class CORE_EXPORT ProcessIterator
{
public:
...
private:
    ...
    void
    InitProcessEntry(ProcessEntry* entry);

#if IS_WIN
    HANDLE snapshot_;
    bool started_iteration_;
#elif IS_APPLE || defined(OS_BSD)
...
class CORE_EXPORT NamedProcessIterator : public ProcessIterator
{
public:
...
CORE_EXPORT int
GetProcessCount(const FilePath::StringType& executable_name, const ProcessFilter* filter);
