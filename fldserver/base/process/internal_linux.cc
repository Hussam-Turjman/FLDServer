...
bool
ReadProcFile(const FilePath& file, std::string* buffer)
{
...
bool
ReadProcStats(pid_t pid, std::string* buffer)
{
...
bool
ParseProcStats(const std::string& stats_data, std::vector<std::string>* proc_stats)
{
    ...
    // Split the rest.
    std::vector<std::string> other_stats = SplitString(stats_data.substr(close_parens_idx + 2),
                                                       " ",
...
void
ParseProcStat(const std::string& contents, ProcStatMap* output)
{
...
int64_t
GetProcStatsFieldAsInt64(const std::vector<std::string>& proc_stats, ProcStatsFields field_num)
{
...
size_t
GetProcStatsFieldAsSizeT(const std::vector<std::string>& proc_stats, ProcStatsFields field_num)
{
    DCHECK_GE(field_num, VM_PPID);
    CHECK_LT(static_cast<size_t>(field_num), proc_stats.size());
...
int64_t
ReadStatFileAndGetFieldAsInt64(const FilePath& stat_file, ProcStatsFields field_num)
{
...
int64_t
ReadProcStatsAndGetFieldAsInt64(pid_t pid, ProcStatsFields field_num)
{
    FilePath stat_file = internal::GetProcPidDir(pid).Append(kStatFile);
...
int64_t
ReadProcSelfStatsAndGetFieldAsInt64(ProcStatsFields field_num)
{
...
size_t
ReadProcStatsAndGetFieldAsSizeT(pid_t pid, ProcStatsFields field_num)
{
    std::string stats_data;
...
GetUserCpuTimeSinceBoot()
{
    ...
    std::vector<std::string> cpu =
            SplitString(cpu_it->second, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
...
ClockTicksToTimeDelta(int clock_ticks)
{
    ...
    return TimeDelta::FromMicroseconds(Time::kMicrosecondsPerSecond * clock_ticks / kHertz);
}
