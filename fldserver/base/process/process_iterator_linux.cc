...
// that are strings.
std::string
GetProcStatsFieldAsString(const std::vector<std::string>& proc_stats,
...
// delimiter.
bool
GetProcCmdline(pid_t pid, std::vector<std::string>* proc_cmd_line_args)
{
    ...
    *proc_cmd_line_args = SplitString(cmd_line, delimiters, KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY);
    return true;
}

}  // namespace

ProcessIterator::ProcessIterator(const ProcessFilter* filter) : filter_(filter)
{
...
bool
ProcessIterator::CheckForNextProcess()
{
...
bool
NamedProcessIterator::IncludeEntry()
{
