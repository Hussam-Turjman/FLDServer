...
ThreadLocalPointer<const TaskToken>&
GetTlsCurrentTaskToken()
{
...
bool
SequenceToken::operator==(const SequenceToken& other) const
{
...
bool
SequenceToken::IsValid() const
{
    return token_ != kInvalidSequenceToken;
}

int
SequenceToken::ToInternalValue() const
{
    return token_;
}
...
{
    const SequenceToken* current_sequence_token = GetTlsCurrentSequenceToken().Get();
    return current_sequence_token ? *current_sequence_token : SequenceToken();
}

bool
TaskToken::operator==(const TaskToken& other) const
{
    return token_ == other.token_ && IsValid();
}
