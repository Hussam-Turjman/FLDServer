...
#include "fldserver/base/logging_win.h"
...
{
    return base::Singleton<LogEventProvider,
                           base::StaticMemorySingletonTraits<LogEventProvider>>::get();
}

bool
LogEventProvider::LogMessage(logging::LogSeverity severity,
                             const char* file,
                             int line,
                             ...
                             const std::string& message)
{
    ...
    if (severity >= 0)
    {
        switch (severity)
        {
            case LOGGING_INFO: level = TRACE_LEVEL_INFORMATION; break;
            case LOGGING_WARNING: level = TRACE_LEVEL_WARNING; break;
            case LOGGING_ERROR: level = TRACE_LEVEL_ERROR; break;
            case LOGGING_FATAL: level = TRACE_LEVEL_FATAL; break;
        }
    }
    else
    {  // severity < 0 is VLOG verbosity levels.
    ...
    if (provider->enable_flags() & ENABLE_LOG_MESSAGE_ONLY)
    {
        ...
        event.SetField(0, message.length() + 1 - message_start, message.c_str() + message_start);
    ...
    else
    {
        ...
        event.SetField(4, message.length() + 1 - message_start, message.c_str() + message_start);
...
void
LogEventProvider::Initialize(const GUID& provider_name)
{
    LogEventProvider* provider = LogEventProvider::GetInstance();
...
void
LogEventProvider::OnEventsEnabled()
{
...
void
LogEventProvider::OnEventsDisabled()
{
